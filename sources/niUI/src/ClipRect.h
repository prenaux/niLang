...
static inline tBool _ClipBlitRectf(sRectf& aDestClipped,
                                   const sRectf& aDestRect, sRectf& aSrcClipped,
                                   ...
                                   const sRectf& aClipRect)
{
  ...
  aSrcClipped.SetTopLeft(
    Vec2<tF32>(aSrcRect.x + diffPosX, aSrcRect.y + diffPosY));
  tF32 diffSizeX = toClip.GetWidth() / aDestRect.GetWidth();
  tF32 diffSizeY = toClip.GetHeight() / aDestRect.GetHeight();
  aSrcClipped.SetSize(aSrcRect.GetWidth() * diffSizeX,
...
static inline tBool _ClipBlitRecti(sRecti& aDestClipped,
                                   const sRectf& aDestRect, sRecti& aSrcClipped,
                                   const sRectf& aSrcRect,
                                   const sRectf& aClipRect)
{
  sRectf d, s;
...
static tBool _BlitBlend(iBitmap2D* apDestBmp, const sRectf& aDestRect,
                        iBitmap2D* apSrcBmp, const sRectf& aSrcRect,
                        const sRectf& aClipRect, eBlendMode aBlendMode,
    ...
    if (rectDst.GetSize() != rectSrc.GetSize()) {
      if (aBlendMode != eBlendMode_NoBlending) {
        return apDestBmp->BlitAlphaStretch(
          apSrcBmp, rectSrc.Left(), rectSrc.Top(), rectDst.Left(),
          rectDst.Top(), rectSrc.GetWidth(), rectSrc.GetHeight(),
          rectDst.GetWidth(), rectDst.GetHeight(), colFront, colBack,
          eBlendMode_Translucent);
      }
      else {
        return apDestBmp->BlitStretch(apSrcBmp, rectSrc.Left(), rectSrc.Top(),
                                      rectDst.Left(), rectDst.Top(),
                                      rectSrc.GetWidth(), rectSrc.GetHeight(),
                                      rectDst.GetWidth(), rectDst.GetHeight());
      }
    }
    else {
      if (aBlendMode != eBlendMode_NoBlending) {
        return apDestBmp->BlitAlpha(
          apSrcBmp, rectSrc.Left(), rectSrc.Top(), rectDst.Left(),
          rectDst.Top(), rectSrc.GetWidth(), rectSrc.GetHeight(),
          ePixelFormatBlit_Normal, colFront, colBack, eBlendMode_Translucent);
      }
      else {
        return apDestBmp->Blit(apSrcBmp, rectSrc.Left(), rectSrc.Top(),
                               rectDst.Left(), rectDst.Top(),
                               rectSrc.GetWidth(), rectSrc.GetHeight(),
...
static __forceinline tBool _BlitBlend(iBitmap2D* apDestBmp,
                                      ...
                                      const sRectf& aSrcRect,
                                      const sRectf& aClipRect,
                                      eBlendMode aBlendMode, tU32 anColor)
{
  const sColor4f colFront = { ULColorGetRf(anColor), ULColorGetGf(anColor),
                              ULColorGetBf(anColor), ULColorGetAf(anColor) };
  return _BlitBlend(apDestBmp, aDestRect, apSrcBmp, aSrcRect, aClipRect,
                    aBlendMode, colFront);
}
