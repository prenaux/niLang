...
sVec3f cNUSpline::GetPosition(tF32 afTime)
{
  ...
  if (mvNodes.size() < 3 || mType == eNUSplineType_Linear) {
    ...
    return VecHermite(r, mvNodes[i].mvPos, startVel, mvNodes[i + 1].mvPos,
                      endVel, t);
  }
}
...
tBool cNUSpline::UpdateSpline()
{
  ...
  for (i = 1; i < mvNodes.size() - 1; ++i) {
    sVec3f np;
    VecNormalize(np, mvNodes[i + 1].mvPos - mvNodes[i].mvPos);
    sVec3f pp;
    VecNormalize(pp, mvNodes[i - 1].mvPos - mvNodes[i].mvPos);
...
sVec3f cNUSpline::GetStartVel(tU32 anIndex) const
{
  ...
  sVec3f temp = (3.f * (mvNodes[anIndex + 1].mvPos - mvNodes[anIndex].mvPos)) *
                ni::FInvert(mvNodes[anIndex].mfDist);
...
{
  niAssert(anIndex > 0);
  sVec3f temp = (3.f * (mvNodes[anIndex].mvPos - mvNodes[anIndex - 1].mvPos)) *
                ni::FInvert(mvNodes[anIndex - 1].mfDist);
  return (temp - mvNodes[anIndex - 1].mvVel) * 0.5f;
}
...
void cNUSpline::Smooth(tU32 anCount)
{
  ...
  for (tU32 count = 0; count < anCount; ++count) {
    ...
    for (tU32 i = 1; i < mvNodes.size() - 1; ++i) {
      newVel = GetEndVel(i) * mvNodes[i].mfDist +
               GetStartVel(i) * mvNodes[i - 1].mfDist;
...
void cNUSpline::Constrain()
{
  ...
  for (tU32 i = 1; i < mvNodes.size() - 1; ++i) {
    float r0 = ni::FDiv(VecLength(mvNodes[i].mvPos - mvNodes[i - 1].mvPos),
                        mvNodes[i - 1].mfDist);
    float r1 = ni::FDiv(VecLength(mvNodes[i + 1].mvPos - mvNodes[i].mvPos),
                        mvNodes[i].mfDist);
...
///////////////////////////////////////////////
tBool __stdcall cNUSpline::SerializeDataTable(iDataTable* apDT,
                                              tSerializeFlags aFlags)
{
  ...
  if (niFlagIs(aFlags, eSerializeFlags_Write)) {
    Ptr<iDataTableWriteStack> dt =
      ni::GetLang()->CreateDataTableWriteStack(apDT);
  ...
  // Read
    ...
    this->SetType(
      (eNUSplineType)dt->GetEnum(_A("type"), niEnumExpr(eNUSplineType)));
    ...
    if (dt->PushFail(_A("Nodes"))) {
      ...
      for (; i < numNodes; ++i) {
        tU32 nPropIndex =
          dt->GetTop()->GetPropertyIndex(niFmt(_A("node%d"), i));
    ...
    sVec4f vCurrent = this->GetNumNodes()
                        ? this->GetNode(this->GetNumNodes() - 1)
                        : Vec4<tF32>(0, 0, 0, 1);
    for (; i < numNodes; ++i) {
      AddNode(vCurrent);
