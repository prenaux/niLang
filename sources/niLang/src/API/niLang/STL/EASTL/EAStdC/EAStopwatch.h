...
class EASTDC_API Stopwatch {
 public:
  ...
  enum Units {
    kUnitsCycles =
      0, /// Stopwatch clock ticks. May or may not match CPU clock ticks 1:1, depending on your hardware and operating system. Some CPUs' low level cycle count instruction counts every 16 cycles instead of every cycle.
    kUnitsCPUCycles =
      1, /// CPU clock ticks (or similar equivalent for the platform). Not recommended for use in shipping softare as many systems alter their CPU frequencies at runtime.
    kUnitsNanoseconds =
      2, /// For a 1GHz processor, 1 nanosecond is the same as 1 clock tick.
    kUnitsMilliseconds =
      4, /// For a 1GHz processor, 1 millisecond is the same as 1,000,000 clock ticks.
 ...
 private:
  ...
  float
    mfStopwatchCyclesToUnitsCoefficient; /// Coefficient is defined seconds per cycle (assuming you want to measure seconds). This is the inverse of the frequency, done this way for speed. Time passed = cycle count * coefficient.
...
class EASTDC_API LimitStopwatch : public Stopwatch {
 public:
  /// LimitStopwatch
  /// Constructor
  LimitStopwatch(int nUnits = kUnitsCycles, uint64_t nLimit = 0,
                 bool bStartImmediately = false)
      : Stopwatch(nUnits, bStartImmediately)
      , mnEndTime(nLimit)
  {
  }
 ...
 protected:
  uint64_t
    mnEndTime; /// The precomputed end time used by limit timing functions.
...
inline void EA::StdC::Stopwatch::Stop()
{
  ...
  {
    const uint64_t nCurrentTime(
      mnUnits == kUnitsCPUCycles ? GetCPUCycle() : GetStopwatchCycle());
...
inline int EA::StdC::Stopwatch::GetUnits()
{
  return mnUnits;
}
...
inline uint64_t EA::StdC::Stopwatch::GetElapsedTime() const
{
  ...
  if (
    mnStartTime) // We we are currently running, then take into account time passed since last start.
  {
  ...
  return (
    uint64_t)((nFinalTotalElapsedTime64 * mfStopwatchCyclesToUnitsCoefficient) +
              0.49999f);
}
...
inline float EA::StdC::Stopwatch::GetElapsedTimeFloat() const
{
  ...
  return (nFinalTotalElapsedTime64 * mfStopwatchCyclesToUnitsCoefficient) +
         0.49999f;
}
...
#endif
  double perfFreq =
    static_cast<double>(clock::period::num) / clock::period::den;
  return perfFreq / 1000.0;
}
} // namespace Internal

inline double EA::StdC::Stopwatch::GetUnitsPerCPUCycle(
  EA::StdC::Stopwatch::Units units)
{
  switch (units) {
  case kUnitsCycles:
  case kUnitsCPUCycles: return Internal::GetPlatformCycleFrequency();
  ...
  case kUnitsSeconds:
  case kUnitsMinutes:
  case kUnitsUserDefined: break;
  }

  return 1.0;
}

inline double EA::StdC::Stopwatch::GetUnitsPerStopwatchCycle(
  EA::StdC::Stopwatch::Units units)
{
  switch (units) {
  case kUnitsCycles:
