...
namespace agg {
template <class LoResT = int8u, class HiResT = int8u, unsigned GammaShift = 8,
...
class gamma_lut {
 public:
  ...
  void gamma(agg_real g)
  {
    ...
    unsigned i;
    for (i = 0; i < gamma_size; i++) {
      m_dir_gamma[i] = (HiResT)uround(pow(i / agg_real(gamma_mask), m_gamma) *
                                      agg_real(hi_res_mask));
    }

    agg_real inv_g = 1.0 / g;
    for (i = 0; i < hi_res_size; i++) {
      m_inv_gamma[i] = (LoResT)uround(pow(i / agg_real(hi_res_mask), inv_g) *
                                      agg_real(gamma_mask));
    }
  }
