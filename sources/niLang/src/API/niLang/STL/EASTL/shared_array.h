...
///
  ...
  #define EASTL_SHARED_ARRAY_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX             \
      " shared_array" // Unless the user overrides something, this is "EASTL shared_array".
#endif

/// EASTL_SHARED_ARRAY_DEFAULT_ALLOCATOR
///
#ifndef EASTL_SHARED_ARRAY_DEFAULT_ALLOCATOR
  #define EASTL_SHARED_ARRAY_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_SHARED_ARRAY_DEFAULT_NAME)
#endif

/// class shared_array
/// A shared_array is the same as shared_ptr but for arrays.
template <typename T, typename Allocator = EASTLAllocatorType,
          typename Deleter = smart_array_deleter<T>>
class shared_array {
 protected:
  /// this_type
  ...
  allocator_type
    mAllocator; /// The allocator used to manage new/delete of mpRefCount.
...
template <typename T, typename A, typename D>
inline void swap(shared_array<T, A, D>& sharedArray1,
                 shared_array<T, A, D>& sharedArray2)
{
...
template <typename T, typename TA, typename TD, typename U, typename UA,
          typename UD>
inline bool operator==(const shared_array<T, TA, TD>& sharedArray1,
                       const shared_array<U, UA, UD>& sharedArray2)
{
                      ...
                      const shared_array<U, UA, UD>& sharedArray2)
{
  return (
    sharedArray1.get() <
    sharedArray2.get()); // Alternatively use: std::less<T*>(a.get(), b.get());
}
