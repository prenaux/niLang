...
class linked_ptr : public linked_ptr_base {
 ...
 public:
  ...
  int use_count() const
  {
    ...
    for (const linked_ptr_base* pCurrent =
           static_cast<const linked_ptr_base*>(this);
         pCurrent->mpNext != static_cast<const linked_ptr_base*>(this);
...
inline bool operator==(const linked_ptr<T, TD>& linkedPtr1,
                       const linked_ptr<U, UD>& linkedPtr2)
{
