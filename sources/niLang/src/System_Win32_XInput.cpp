...
typedef void(WINAPI* tpfnXInputEnable)(_In_ BOOL enable);
static tpfnXInputEnable _pfnXInputEnable = NULL;

typedef DWORD(WINAPI* tpfnXInputGetCapabilities)(
  _In_ DWORD dwUserIndex, _In_ DWORD dwFlags,
  _Out_ XINPUT_CAPABILITIES* pCapabilities);
static tpfnXInputGetCapabilities _pfnXInputGetCapabilities = NULL;

typedef DWORD(WINAPI* tpfnXInputGetState)(_In_ DWORD dwUserIndex,
                                          _Out_ XINPUT_STATE* pState);
static tpfnXInputGetState _pfnXInputGetState = NULL;

typedef DWORD(WINAPI* tpfnXInputSetState)(_In_ DWORD dwUserIndex,
...
typedef DWORD(WINAPI* tpfnXInputGetBatteryInformation)(
  _In_ DWORD dwUserIndex, _In_ BYTE devType,
 ...
 public:
  ...
  tBool __stdcall Update() niImpl
  {
    ...
    if (_pfnXInputGetState(mnIndex, &state) != ERROR_SUCCESS) {
...

      {
        ...
        if (magnitude > DEADZONE_LEFT_THUMB) {
          ...
          bSomethingChanged =
            bSomethingChanged || (mStates.mvLeft != sVec2f::Zero());
...

      {
        ...
        if (magnitude > DEADZONE_RIGHT_THUMB) {
          ...
          float normalizedMagnitude =
            magnitude / (32767 - DEADZONE_RIGHT_THUMB);
          ...
          bSomethingChanged =
            bSomethingChanged || (mStates.mvRight != sVec2f::Zero());
...

      {
        ...
        if (magnitude > DEADZONE_TRIGGER) {
          ...
          bSomethingChanged =
            bSomethingChanged || (!niFloatIsZero(mStates.mfLeftTrigger));
          magnitude = 0.0;
        }
...

      {
        ...
        if (magnitude > DEADZONE_TRIGGER) {
          ...
          bSomethingChanged =
            bSomethingChanged || (!niFloatIsZero(mStates.mfRightTrigger));
          magnitude = 0.0;
        }
      ...
      {
        bSomethingChanged =
          bSomethingChanged || (mStates.mButtons != state.Gamepad.wButtons);
    ...
    if (bSomethingChanged) {
      niDebugFmt((
        "... UPDATED XINPUT %d: Left:%s, Right:%s, LT: %g, RT: %g, Buttons: %x",
        mnIndex, mStates.mvLeft, mStates.mvRight, mStates.mfLeftTrigger,
        mStates.mfRightTrigger, mStates.mButtons));
    }
  #endif
  ...
  tF32 __stdcall GetButton(tU32 anButton) niImpl
  {
    ...
    case eGameCtrlButton_LeftTrigger: return mStates.mfLeftTrigger;
    case eGameCtrlButton_RightTrigger: return mStates.mfRightTrigger;
  ...
  {
    switch (anAxis) {
    case eGameCtrlAxis_LX: return mStates.mvLeft.x;
    case eGameCtrlAxis_LY: return mStates.mvLeft.y;
    case eGameCtrlAxis_RX: return mStates.mvRight.x;
    case eGameCtrlAxis_RY: return mStates.mvRight.y;
    }
    return 0.0f;
  }
  ...
  void __stdcall Vibrate(const sVec2f& aSpeed) niImpl
  {
    if (_pfnXInputSetState) {
      XINPUT_VIBRATION vibration = { (WORD)(aSpeed.x * 65535.0f),
  ...
  tF32 __stdcall GetBatteryLevel() const
  {
    if (_pfnXInputGetBatteryInformation) {
      ...
      case BATTERY_LEVEL_LOW: return 0.33f;
      case BATTERY_LEVEL_MEDIUM: return 0.66f;
...
tBool __stdcall XInputGameCtrls_Startup()
{
  ...
  _pfnXInputEnable =
    (tpfnXInputEnable)ni_dll_get_proc(_xinputDLL, "XInputEnable");
  if (!_pfnXInputEnable) {
  ...
  _pfnXInputGetCapabilities = (tpfnXInputGetCapabilities)ni_dll_get_proc(
    _xinputDLL, "XInputGetCapabilities");
  ...
  _pfnXInputGetState =
    (tpfnXInputGetState)ni_dll_get_proc(_xinputDLL, "XInputGetState");
  ...
  _pfnXInputSetState =
    (tpfnXInputSetState)ni_dll_get_proc(_xinputDLL, "XInputSetState");
  if (!_pfnXInputSetState) {
    niWarning(
      "Can't get XInputSetState in XInput DLL, vibrate won't be available.");
  }

  _pfnXInputGetBatteryInformation =
    (tpfnXInputGetBatteryInformation)ni_dll_get_proc(
      _xinputDLL, "XInputGetBatteryInformation");
  if (!_pfnXInputGetBatteryInformation) {
    niWarning(
      "Can't get XInputGetBatteryInformation in XInput DLL, battery info won't be available.");
  }
  ...
  niLog(Info, niFmt("Input, found '%d' game controllers, %d connected.",
                    _xinputCtrls.size(), numConnected));
