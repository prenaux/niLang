...
int niUIGpuFuncs_IntersectionType_InvalidTriangle;
int niUIGpuFuncs_IntersectionType_None;
int niUIGpuFuncs_IntersectionType_Triangle;
int niUIGpuFuncs_IntersectionType_BoundingVolume;
...
void niUIGpuFuncs_IntersectionResult_SetFromTriangleData(inout /* mut */ niUIGpuFuncs_IntersectionResult r, uint aVBIndex, uint aIBIndex, uint aPrimIndex, uint aFirstIndex, vec2 aIntersectionBaryCentrics, mat4x3 aObjectToWorld);
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_lit_textured_cube_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS, sampler aSS);
void niUIGpuFuncs_IntersectionType_static_initialize() {

  // TypeStatic: IntersectionType
  niUIGpuFuncs_IntersectionType_InvalidTriangle = -1;
  niUIGpuFuncs_IntersectionType_None = 0;
  niUIGpuFuncs_IntersectionType_Triangle = 1;
  niUIGpuFuncs_IntersectionType_BoundingVolume = 2;
}
...
vec4 niUIGpuFuncs_RayInstanceData_SampleMaterialDiffuseColor(niUIGpuFuncs_RayInstanceData aInstData, sampler aSS, vec2 aUV) {
  ...
  vec4 _tmp_2;
  bool _tmp_3 = (aInstData.texIndex > 0);
  if (_tmp_3) {
    _tmp_2 = (texture(sampler2D(nil_builtin_GetTexture2D[nonuniformEXT(aInstData.texIndex)],aSS),aUV)*materialColor);
  }
  else {
    {
      _tmp_2 = materialColor;
    }
  }
  return _tmp_2;
}
...
bool niUIGpuFuncs_IntersectionResult_SetFromCommittedIntersection(inout /* mut */ niUIGpuFuncs_IntersectionResult r, rayQueryEXT aRayQuery) {
  ...
  if (_tmp_k) {
    ...
    niUIGpuFuncs_IntersectionResult_SetFromTriangleData(r,r.instData.vbIndex,r.instData.ibIndex,r.primIndex,r.instData.firstIndex,rayQueryGetIntersectionBarycentricsEXT(aRayQuery,true),rayQueryGetIntersectionObjectToWorldEXT(aRayQuery,true));
  }
  else {
...
void niUIGpuFuncs_IntersectionResult_SetFromTriangleData(inout /* mut */ niUIGpuFuncs_IntersectionResult r, uint aVBIndex, uint aIBIndex, uint aPrimIndex, uint aFirstIndex, vec2 aIntersectionBaryCentrics, mat4x3 aObjectToWorld) {
  ...
  if (_tmp_P) {
  ...
  else {
    {
      ...
      niUIGpuFuncs_VertexRay v0 = nil_builtin_GetVertexRay[nonuniformEXT(aVBIndex)].v[nonuniformEXT(inds.x)];
      niUIGpuFuncs_VertexRay v1 = nil_builtin_GetVertexRay[nonuniformEXT(aVBIndex)].v[nonuniformEXT(inds.y)];
      niUIGpuFuncs_VertexRay v2 = nil_builtin_GetVertexRay[nonuniformEXT(aVBIndex)].v[nonuniformEXT(inds.z)];
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_lit_textured_cube_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS, sampler aSS) {
  ...
  mat4 _tmp_L4 = aUniforms.cameraInvView;
  vec3 origin = vec3(_tmp_L4[3][0],_tmp_L4[3][1],_tmp_L4[3][2]);
  ...
  vec4 color;
  bool _tmp_B5 = (intersection.type == niUIGpuFuncs_IntersectionType_Triangle);
  if (_tmp_B5) {
    ...
    vec3 _tmp_n6 = C;
    color = vec4(_tmp_n6.x,_tmp_n6.y,_tmp_n6.z,1.0);
  }
  else {
    bool _tmp_o6 = (intersection.type == niUIGpuFuncs_IntersectionType_BoundingVolume);
    if (_tmp_o6) {
      color = vec4(1.0,0.0,0.0,1.0);
    }
    else {
      bool _tmp_w6 = (intersection.type < niUIGpuFuncs_IntersectionType_None);
      if (_tmp_w6) {
        color = vec4(1.0,0.0,1.0,1.0);
      }
      else {
        {
          vec3 _tmp_F6 = niUIGpuFuncs_ComputeSimpleSkyColor(dir,sunDir);
          color = vec4(_tmp_F6.x,_tmp_F6.y,_tmp_F6.z,0.0);
        }
      }
    }
  }
  vec4 _tmp_K6 = color;
  return nish_std_PixelOutput_new(_tmp_K6);
}
...
// Pixel Shader main: niUIGpuFuncs_raytracer_lit_textured_cube_ps
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = niUIGpuFuncs_raytracer_lit_textured_cube_ps(aInput, aUniforms, IN_1_aAS, IN_1_aSS);
