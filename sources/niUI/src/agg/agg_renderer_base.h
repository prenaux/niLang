...
class renderer_base {
 public:
  ...
  {
    return x >= m_clip_box.x1 && y >= m_clip_box.y1 && x <= m_clip_box.x2 &&
           y <= m_clip_box.y2;
  }
  ...
  //--------------------------------------------------------------------
  void blend_bar(int x1, int y1, int x2, int y2, const color_type& c,
                 cover_type cover)
  {
    rect_i rc(x1, y1, x2, y2);
    rc.normalize();
  ...
  //--------------------------------------------------------------------
  void blend_solid_hspan(int x, int y, int len, const color_type& c,
  ...
  //--------------------------------------------------------------------
  void blend_color_vspan(int x, int y, int len, const color_type* colors,
                         const cover_type* covers,
                         cover_type cover = agg::cover_full)
  {
    if (x > xmax())
  ...
  void copy_from(const RenBuf& src, const rect_i* rect_src_ptr = 0, int dx = 0,
                 int dy = 0)
  {
  ...
  void blend_from(const SrcPixelFormatRenderer& src,
                  const rect_i* rect_src_ptr = 0, int dx = 0, int dy = 0,
                  cover_type cover = agg::cover_full)
  {
    rect_i rsrc(0, 0, src.width(), src.height());
    if (rect_src_ptr) {
      ...
      while (rc.y2 > 0) {
        ...
        if (rw.ptr) {
          ...
          if (len > 0) {
            ...
            if (len > 0) {
              m_ren->blend_from(src, x1dst, rdst.y1, x1src, rsrc.y1, len,
                                cover);
            }
          }
        }
