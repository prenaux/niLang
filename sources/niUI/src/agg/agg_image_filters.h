...
class image_filter_lut
{
 public:
  ...
  {
    agg_real r = filter.radius();
    realloc_lut(r);
    unsigned i;
    unsigned pivot = diameter() << (image_subpixel_shift - 1);
    for(i = 0; i < pivot; i++)
    {
      agg_real x = agg_real(i) / agg_real(image_subpixel_scale);
      agg_real y = filter.calc_weight(x);
  ...
  agg_real       radius()       const { return m_radius;   }
 ...
 private:
  void realloc_lut(agg_real radius);
...
{
  static agg_real radius() { return 1.0; }
  static agg_real calc_weight(agg_real x)
  {
    return 1.0 - x;
  }
};
...
{
  static agg_real radius() { return 1.5; }
  static agg_real calc_weight(agg_real x)
  {
    agg_real t;
...
{
  static agg_real pow3(agg_real x)
  {
    return (x <= 0.0) ? 0.0 : x * x * x;
  }

 public:
  static agg_real radius() { return 2.0; }
  static agg_real calc_weight(agg_real x)
  {
    return
        (1.0/6.0) *
...
class image_filter_kaiser
{
 ...
 public:
  image_filter_kaiser(agg_real b = 6.33) :
      a(b), epsilon(1e-12)
  {
    i0a = 1.0 / bessel_i0(b);
  }
  ...
  static agg_real radius() { return 1.0; }
  agg_real calc_weight(agg_real x) const
  {
    return bessel_i0(a * sqrt(1. - x * x)) * i0a;
  }

 private:
  agg_real bessel_i0(agg_real x) const
  {
    int i;
    agg_real sum, y, t;
...
{
  agg_real p0, p2, p3;
  agg_real q0, q1, q2, q3;

 public:
  image_filter_mitchell(agg_real b = 1.0/3.0, agg_real c = 1.0/3.0) :
      p0((6.0 - 2.0 * b) / 6.0),
...
{
  static agg_real radius() { return 3.0; }
  static agg_real calc_weight(agg_real x)
  {
    if(x < 1.0)
    {
...
{
  static agg_real radius() { return 3.2383; }
  static agg_real calc_weight(agg_real x)
  {
...
{
 public:
  image_filter_sinc(agg_real r) : m_radius(r < 2.0 ? 2.0 : r) {}
  agg_real radius() const { return m_radius; }
  agg_real calc_weight(agg_real x) const
  {
    if(x == 0.0) return 1.0;
...
{
 public:
  image_filter_lanczos(agg_real r) : m_radius(r < 2.0 ? 2.0 : r) {}
  agg_real radius() const { return m_radius; }
  agg_real calc_weight(agg_real x) const
  {
    if(x == 0.0) return 1.0;
...
{
 public:
  image_filter_blackman(agg_real r) : m_radius(r < 2.0 ? 2.0 : r) {}
  agg_real radius() const { return m_radius; }
  agg_real calc_weight(agg_real x) const
  {
    if(x == 0.0) return 1.0;
