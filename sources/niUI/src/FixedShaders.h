...
#define SHADER_MATERIAL_HAS_CHANNEL(CHANNEL) \
  (matDesc.mChannels[eMaterialChannel_##CHANNEL].mTexture.IsOK())
...
struct sFixedShaders : ImplRC<ni::iUnknown> {
  ...
  static eFixedVertexShader GetFixedVertexShader(const tFVF aFVF,
                                                 const sMaterialDesc& aMatDesc)
  {
...
case eVertexFormat_##CVF:                                              \
  return (matFlags & eMaterialFlags_Vertex) ? eFixedVertexShader_##CVF \
                                            : eFixedVertexShader_##VF;
    ...
    case eFVF_Position | eFVF_Normal | eFVF_ColorA | eFVF_Tex3:
      return (matFlags & eMaterialFlags_Vertex) ? eFixedVertexShader_PNAT2
                                                : eFixedVertexShader_PNT2;
    case eFVF_PositionB4 | eFVF_Indices | eFVF_Normal | eFVF_ColorA | eFVF_Tex3:
      return (matFlags & eMaterialFlags_Vertex) ? eFixedVertexShader_PB4INAT1
                                                : eFixedVertexShader_PB4INT1;
    }
  ...
  tBool Initialize(ni::iGraphics* g)
  {
    ...
    niLoop (i, niCountOf(_vertexShaders)) {
      _vertexShaders[i] = LoadShader(
        g, GetFixedShaderPath(GetFixedVertexShaderName((eFixedVertexShader)i)));
      if (!_vertexShaders[i].IsOK()) {
        niError(niFmt("Can't load fixed vertex shader '%s'.",
                      GetFixedVertexShaderName((eFixedVertexShader)i)));
        return ni::eFalse;
      }
    }
    niLoop (i, niCountOf(_pixelShaders)) {
      _pixelShaders[i] = LoadShader(
        g, GetFixedShaderPath(GetFixedPixelShaderName((e_fixed_ps)i)));
      if (!_pixelShaders[i].IsOK()) {
        niError(niFmt("Can't load fixed pixel shader '%s'.",
                      GetFixedPixelShaderName((e_fixed_ps)i)));
        return ni::eFalse;
      }
    }
  ...
  iShader* GetVertexShader(const tFVF aFVF, const sMaterialDesc& aMatDesc) const
  {
    ...
    if (vs >= eFixedVertexShader_Last) {
      niWarning(
        niFmt("Can't get vertex shader for FVF '%s', mat: %s, matFlag: %x",
              FVFToString(aFVF), aMatDesc.mhspName,
              niFlagsToChars(eMaterialFlags, aMatDesc.mFlags)));
      return NULL;
    }
