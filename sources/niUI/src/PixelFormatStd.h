...
#define INFO32_EQUAL()                   \
  ...
  {                                      \
    ulAMask = (tU32)inf.qAMask;          \
    ulALShift = inf.ulALShift;           \
    ulARShift = inf.ulARShift;           \
    ulASigned = inf.ulASigned;           \
    ulRMask = (tU32)inf.qRMask;          \
    ulRLShift = inf.ulRLShift;           \
    ulRRShift = inf.ulRRShift;           \
    ulRSigned = inf.ulRSigned;           \
    ulGMask = (tU32)inf.qGMask;          \
    ulGLShift = inf.ulGLShift;           \
    ulGRShift = inf.ulGRShift;           \
    ulGSigned = inf.ulGSigned;           \
    ulBMask = (tU32)inf.qBMask;          \
    ulBLShift = inf.ulBLShift;           \
    ulBRShift = inf.ulBRShift;           \
    ulBSigned = inf.ulBSigned;           \
    fAMul = inf.fAMul;                   \
    fRMul = inf.fRMul;                   \
    fGMul = inf.fGMul;                   \
    fBMul = inf.fBMul;                   \
...
#pragma pack(push, 4)
  struct sInfo64 {
    ...
    union {
      ...
      struct {
        tU32 ulARShift, ulALShift;
        tI64 qAMask;
        tF32 fAMul;
        tU32 ulASigned;
        tU32 ulRRShift, ulRLShift;
        tI64 qRMask;
        tF32 fRMul;
        tU32 ulRSigned;
        tU32 ulGRShift, ulGLShift;
        tI64 qGMask;
        tF32 fGMul;
        tU32 ulGSigned;
        tU32 ulBRShift, ulBLShift;
        tI64 qBMask;
        tF32 fBMul;
        tU32 ulBSigned;
      };
  ...
  struct sInfo32 {
    ...
    union {
      ...
      struct {
        tU32 ulARShift, ulALShift, ulAMask;
        tF32 fAMul;
        tU32 ulASigned;
        tU32 ulRRShift, ulRLShift, ulRMask;
        tF32 fRMul;
        tU32 ulRSigned;
        tU32 ulGRShift, ulGLShift, ulGMask;
        tF32 fGMul;
        tU32 ulGSigned;
        tU32 ulBRShift, ulBLShift, ulBMask;
        tF32 fBMul;
        tU32 ulBSigned;
      };
    };
...
#pragma pack(pop)
  ...
  struct sBase : public ni::Impl_HeapAlloc {
    ...
    virtual tPtr BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                              tU32 a = 0) const = 0;
    virtual tPtr BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                             tF32 a = 0.0f) const = 0;
...
#define PXFFLOAT_16BITS niBit(1)
 ...
 public:
  ...
  {
    return BuildPixelul(pOut, r << 24, g << 24, b << 24, a << 24);
  }
  tPtr __stdcall BuildPixelus(tPtr pOut, tU16 r, tU16 g, tU16 b,
                              tU16 a = 0) const
  {
    return BuildPixelul(pOut, r << 16, g << 16, b << 16, a << 16);
  }
  tPtr __stdcall BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                              tU32 a = 0) const;
  tPtr __stdcall BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                             tF32 a = 0.0f) const;

  tPtr __stdcall BeginUnpackPixels(tPtr pSurface, tU32 ulPitch, tU32 ulX,
  ...
  tBool __stdcall Clear(tPtr pDst, tU32 ulDestPitch, tU32 dx, tU32 dy, tU32 w,
                        tU32 h, tPtr apClearColor)
  {
    return eFalse;
  }

  tBool __stdcall Blit(tPtr pDst, tU32 ulDestPitch, tU32 dx, tU32 dy, tPtr pSrc,
                       tU32 ulSrcPitch, iPixelFormat* pSrcFmt, tU32 sx, tU32 sy,
                       tU32 w, tU32 h, ePixelFormatBlit blitFlags) const;

  tBool __stdcall BlitStretch(tPtr pDst, tU32 ulDestPitch, tU32 ulDestWidth,
                              tU32 ulDestHeight, tU32 dx, tU32 dy, tU32 dw,
                              tU32 dh, tPtr pSrc, tU32 ulSrcPitch,
                              iPixelFormat* pSrcFmt, tU32 sx, tU32 sy, tU32 sw,
                              tU32 sh) const;
  ...
  tBool __stdcall BlitAlpha(tPtr pDst, tU32 ulDestPitch, tU32 dx, tU32 dy,
                            ...
                            eBlendMode aBlendMode) const
  {
    return eFalse;
  }

  tBool __stdcall BlitAlphaStretch(tPtr pDst, tU32 ulDestPitch,
                                   tU32 ulDestWidth, tU32 ulDestHeight, tU32 dx,
                                   tU32 dy, tU32 dw, tU32 dh, tPtr pSrc,
                                   tU32 ulSrcPitch, iPixelFormat* pSrcFmt,
                                   tU32 sx, tU32 sy, tU32 sw, tU32 sh,
                                   const sColor4f& aSourceColor,
                                   const sColor4f& aDestColor,
                                   eBlendMode aBlendMode) const
  {
