...
struct matrix_pivot
{
  static int pivot(agg_real m[Rows][Cols], unsigned row)
  {
    int k = int(row);
    agg_real max_val, tmp;
...
struct simul_eq
{
  static bool solve(const agg_real left[Size][Size],
                    const agg_real right[Size][RightCols],
                    agg_real result[Size][RightCols])
  {
    unsigned i, j, k;
    agg_real a1;

    agg_real tmp[Size][Size + RightCols];
