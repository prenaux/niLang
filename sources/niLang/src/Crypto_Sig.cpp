...
niExportFunc(ni::tBool) CryptoSigVerify(const char* signatureHex,
                                        ...
                                        const char* payload)
{
  ...
  ni::Ptr<ni::iCryptoHash> hash =
    HashString(ni::CreateHash("SHA256"), payload, ni::StrLen(payload));
  ...
  r = mbedtls_pk_parse_public_key(&ctx, (unsigned char*)publicKeyPEM,
                                  ni::StrSizeZ(publicKeyPEM));
  if (r != 0) {
  ...
  r = mbedtls_rsa_pkcs1_verify(rsa_ctx, MBEDTLS_MD_SHA256, sizeof(hashBuffer),
                               hashBuffer, sigData.data());
  if (r != 0) {
  ...
  ni::eRawToStringEncoding aSecretFormat)
{
  ...
  case ni::eRawToStringEncoding_Hex: {
    ...
    ni::HexDecode((ni::tPtr)decodedSecret.data(),
                  (ni::tInt)decodedSecret.size(), aSecret, slen);
    break;
  }
  case ni::eRawToStringEncoding_Base64: {
    const ni::tInt decodedSecretSize =
      ni::Base64DecodeOutputSize(aSecret, slen);
    niCheck(decodedSecretSize >= 0, NULL);
    decodedSecret.resize(decodedSecretSize);
    ni::Base64Decode((ni::tPtr)decodedSecret.data(),
                     (ni::tInt)decodedSecret.size(), aSecret, slen);
    break;
  }
  case ni::eRawToStringEncoding_Base32: {
    const ni::tInt decodedSecretSize =
      ni::Base32DecodeOutputSize(aSecret, slen);
    niCheck(decodedSecretSize >= 0, NULL);
    decodedSecret.resize(decodedSecretSize);
    ni::Base32Decode((ni::tPtr)decodedSecret.data(),
                     (ni::tInt)decodedSecret.size(), aSecret, slen);
    break;
  }
  default: {
  ...
  mbedtls_md_hmac_starts(&ctx, (const unsigned char*)decodedSecret.data(),
                         decodedSecret.size());
  mbedtls_md_hmac_update(&ctx, (const unsigned char*)payload.data(),
                         payload.size());
