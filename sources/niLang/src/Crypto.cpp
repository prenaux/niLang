...
struct cCrypto : public ImplLocal<iCrypto> {
  ...
  virtual cString __stdcall Digest(const achar* aaszData, const achar* aType,
                                   eRawToStringEncoding aEncoding)
  {
  ...
  virtual ni::iFile* __stdcall HmacSignature(
    ...
    ni::eRawToStringEncoding aSecretFormat) niImpl
  {
    return CryptoHmacSignature(aAlgo, apOutput, apPayload, aPayloadSize,
                               aSecret, aSecretFormat);
  }
};
