...
TheoraVideoClip::TheoraVideoClip(ni::iFile* data_source,
    ...
    , mFrameDuration(0)
    , mPriority(1)
    , mName(data_source->GetSourcePath())
    , mWidth(0)
    , mHeight(0)
...
int TheoraVideoClip::discardOutdatedFrames(float absTime)
{
	...
	astl::list<TheoraVideoFrame*>& queue = mFrameQueue->_getFrameQueue();
	...
	if (nPop > 0)
	{
		...
		astl::string log = getName() + ": dropped frame ";
...
astl::string TheoraVideoClip::getName()
{
	return mName;
}
...
void TheoraVideoClip::setOutputMode(TheoraOutputMode mode)
{
	if (mode == TH_UNDEFINED) {
		niError((_ASTR("Invalid output mode: TH_UNDEFINED for video: ") + mName).c_str());
    return;
	}
  if (mOutputMode == mode) {
    return;
