...
niExportFunc(bool) sq_aux_gettypedarg_(const ni::achar* section, HSQUIRRELVM v,
                                       int idx, SQObjectType type,
                                       SQObjectPtr** o)
{
  *o = &stack_get(v, idx);
  if (_sqtype(**o) != type) {
    ...
    niRTError(v,
              niFmt(_A("[%s] wrong argument type, expected '%s' got '%.50s'"),
...
niExportFunc(bool) sq_aux_gettypedarg(HSQUIRRELVM v, int idx, SQObjectType type,
                                      SQObjectPtr** o)
{
  return sq_aux_gettypedarg_(_A("unknown"), v, idx, type, o);
}

#define _GETSAFE_OBJ(ZONE, v, idx, type, o)           \
  {                                                   \
    if (!sq_aux_gettypedarg_(ZONE, v, idx, type, &o)) \
      return SQ_ERROR;                                \
  }
#define sq_aux_paramscheck(v, count)                      \
  {                                                       \
    if (sq_gettop(v) < count) {                           \
      niRTError(v, _A("not enough params in the stack")); \
      return SQ_ERROR;                                    \
    }                                                     \
  }
...
niExportFunc(int) sq_aux_invalidtype(HSQUIRRELVM v, SQObjectType type)
{
  return sq_throwerror(
    v, niFmt(_A("unexpected type %s"), v->_ss->GetTypeNameStr(type)));
}
...
niExportFunc(SQRESULT) sq_getclosureinfo(HSQUIRRELVM v, int idx,
                                         ni::tU32* nparams, ni::tU32* nfreevars)
{
...
niExportFunc(SQRESULT) sq_setnativeclosurename(HSQUIRRELVM v, int idx,
                                               const SQChar* name)
{
  SQObjectPtr o = stack_get(v, idx);
...
niExportFunc(SQInt) sq_getsize(HSQUIRRELVM v, int idx)
{
  ...
  default: return sq_aux_invalidtype(v, type);
  }
}
...
niExportFunc(SQRESULT) sq_createslot(HSQUIRRELVM v, int idx)
{
  ...
  if (_sqtype(self) == OT_TABLE) {
    ...
    if (_sqtype(key) == OT_NULL)
      return sq_throwerror(v, _A("null is not a valid key"));
...
niExportFunc(SQRESULT) sq_setdelegate(HSQUIRRELVM v, int idx)
{
  ...
  case OT_TABLE:
    if (_sqtype(mt) == OT_TABLE) {
      if (!_table(self)->SetDelegate(_table(mt)))
        return sq_throwerror(v, _A("delagate cycle"));
      v->Pop();
    }
  ...
  case OT_USERDATA:
    if (_sqtype(mt) == OT_TABLE) {
      ...
      return sq_aux_invalidtype(v, type);
    break;
  default: return sq_aux_invalidtype(v, type); break;
  }
  return SQ_OK;
}
...
static cString GetObjectString(const SQSharedState& aSS, const SQObjectPtr& obj)
{
  cString val;
  switch (_sqtype(obj)) {
...
niExportFunc(SQRESULT) sq_rawget(HSQUIRRELVM v, int idx)
{
  ...
  v->Pop(1);
  return sq_throwerror(
    v, niFmt(_A("sq_rawget (%d), the index '%s' doesn't exist"), counter++,
             GetObjectString(*v->_ss, self).Chars()));
}
...
niExportFunc(const SQChar*) sq_getlocal(HSQUIRRELVM v, unsigned int level,
                                        unsigned int idx)
{
  ...
  if (lvl < cstksize) {
    ...
    return func->GetLocal(v, stackbase, idx,
                          (ci._ip - (&func->_instructions[0])) - 1);
  }
  return NULL;
}
...
niExportFunc(SQRESULT) sq_writeclosure(HSQUIRRELVM v, iFile* apFile)
{
  ...
  if (!WriteSQFunctionProto(v, as_nn(apFile),
                            _funcproto(_closure(*o)->_function)))
    return SQ_ERROR;
  return SQ_OK;
}
...
niExportFunc(SQRESULT) sq_readclosure(HSQUIRRELVM v, iFile* apFile)
{
  ...
  SQObjectPtr closure =
    SQClosure::Create(_funcproto(func), _table(v->_roottable));
  if (!ReadSQFunctionProto(v, as_nn(apFile),
                           _funcproto(_closure(closure)->_function)))
    return SQ_ERROR;
  v->Push(closure);
  return SQ_OK;
}

niExportFunc(SQRESULT) sq_setfreevariable(HSQUIRRELVM v, int idx,
                                          unsigned int nval)
{
  SQObjectPtr& self = stack_get(v, idx);
  switch (_sqtype(self)) {
  case OT_CLOSURE:
    if (_closure(self)->_outervalues.size() > nval) {
      ...
      return sq_throwerror(v, _A("invalid free var index"));
    break;
  default: return sq_aux_invalidtype(v, _sqtype(self));
  }
  v->Pop(1);
  return SQ_OK;
}
...
niExportFunc(SQRESULT) sq_next(HSQUIRRELVM v, int idx)
{
  ...
  default: return sq_aux_invalidtype(v, _sqtype(o));
  }
  v->Push(realkey);
  v->Push(val);
  return SQ_OK;
...
static SQInt buf_lexfeed(ni::tPtr file)
{
...
niExportFunc(SQRESULT) sq_compilestring(HSQUIRRELVM v, iHString* ahspSourceName,
                                        const achar* aaszSourceCode)
{
  SQObjectPtr o;
  sCompileErrors errors;
  tHStringNN hspSource =
    HStringIsEmpty(ahspSourceName) ? _H("nosourcename") : AsNN(ahspSourceName);
  if (!CompileString(hspSource, aaszSourceCode, errors, o)) {
    niAssert(errors.HasError());

    if (v->_ss->_compilererrorhandler) {
      SQObjectPtr ret;
      v->_ss->_compilererrorhandler(
        v, errors.GetLastError().msg.Chars(), niHStr(hspSource),
        errors.GetLastError().line, errors.GetLastError().col);
    }

    return sq_throwerror(v, niFmt("%s:%d:%d: error: compilestring, %s",
                                  hspSource, errors.GetLastError().line,
                                  errors.GetLastError().col,
                                  errors.GetLastError().msg));
  }

  // Lint the output funcproto
  {
    niPanicAssert(sq_isfuncproto(o));
    _funcproto(o)->LintTraceRoot();
  }

  // Push the new closure
  {
    SQObjectPtr closure =
      SQClosure::Create(_funcproto(o), _table(v->_roottable));
    v->Push(closure);
  }
