...
struct fill_imp {
  ...
  static void do_fill(ForwardIterator first, ForwardIterator last,
                      const T& value)
  {
...
#elif defined(EA_COMPILER_MICROSOFT) && \
  (defined(EA_PROCESSOR_X86) || defined(EA_PROCESSOR_X86_64))
  #if defined(EA_PROCESSOR_X86_64)
template <typename Value>
inline void fill(uint64_t* first, uint64_t* last, Value c)
...
inline void fill(
  ...
  const char&
    c) // It's debateable whether we should use 'char& c' or 'char c' here.
{
...
inline void fill(
  char* first, char* last,
  const int c) // This is used for cases like 'fill(first, last, 0)'.
{
  memset(first, (unsigned char)c, (size_t)(last - first));
}
...
inline void fill(signed char* first, signed char* last, const int c)
{
  ...
  defined(__ICL) // ICL = Intel compiler
...
struct fill_n_imp<true> {
  ...
  {
    typedef
      typename eastl::iterator_traits<OutputIterator>::value_type value_type;
