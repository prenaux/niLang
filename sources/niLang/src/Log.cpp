...
static tBool _IsSeparateWindowsConsole()
{
  ...
  ni::GetLang()->SetProperty("ni.log.console_process_count",
                             niFmt("%d", (tU32)count));
  return (count <= 1);
}
...
static void _SetWindowsConsoleColors(HANDLE aHandle, const ni::tU32 logType)
{
  ...
  static WORD _previousConsoleFgColor =
...
static void _AppendPosixConsoleColors(ni::cString& final,
                                      const ni::tU32 logType,
                                      ni::tBool abOverwriteNewLine)
{
...
static void _SetConsoleColors(ni::cString& final, tBool aIsStdout,
                              const ni::tU32 logType,
                              eSetConsoleColorMode aMode)
{
  ...
  niRunOnce
  {
    ...
    ni::GetLang()->SetProperty("ni.log.env_no_color",
                               envNoColor ? "yes" : "no");
    if (!envNoColor) {
      if (isatty(fileno(stdout))) {
        ...
        ni::GetLang()->SetProperty("ni.log.env_inside_emacs",
                                   envInsideEmacs ? "yes" : "no");
        _coloredOutput = ni::eTrue;
      }
      ni::GetLang()->SetProperty("ni.log.is_tty", _isTTY ? "yes" : "no");
    }

    _coloredOutput = ni::GetLang()
                       ->GetProperty("ni.log.colored_output")
                       .Bool(!envNoColor && (_isTTY || _coloredOutput));
    ni::GetLang()->SetProperty("ni.log.colored_output",
                               _coloredOutput ? "yes" : "no");
  ...
  if (_coloredOutput) {
    ...
    if (_isTTY) {
      ...
      {
        _SetWindowsConsoleColors(
          GetStdHandle(aIsStdout ? STD_OUTPUT_HANDLE : STD_ERROR_HANDLE),
          logType);
      }
      return;
    }
...
#ifdef __JSCC__
  switch (aType & eLogFlags_All) {
  case eLogFlags_Error: return false;
  default: return true;
  }
#else
  return false;
...
niExportFunc(void) ni_log_format_message(cString& final, tU32 aType,
                                         const char* aaszFile,
                                         ...
                                         const tF64 afPrevTime)
{
...

  {
    ...
    // TYPE/MSG [time] (file:line)
    const char* typeMark =
      niFlagIs(aType, eLogFlags_NoLogTypePrefix) ? NULL : _LogType(aType);
    ...
    while (1) {
      if (!*p || *p == '\n' || *p == '\r') {
        if (curSize) {
          ...
          if (numLines++ == 0) {
            if (!niFlagIs(aType, eLogFlags_FormatMSVC) &&
                niStringIsOK(aaszFile))
            {
              if (afTime >= 0) {
              ...
              if (niStringIsOK(aaszFile)) {
                ...
                itFile.to_end();
                int slashPos = ni::Max(
                  0, StrCharItRFindFirstOf(itFile, slashChars,
                                           niCountOf(slashChars), NULL) +
                       1);
 ...
 public:
  ...
  tU32 Get(astl::vector<cString>* logs, tSize numLogs)
  {
    ...
    if (logs && (numLogs > 0)) {
      ...
      for (astl::deque<cString>::const_iterator it = _logs.end() - numLogs;
           it != _logs.end(); ++it)
      {
        logs->push_back(*it);
      }
...
niExportFunc(void) ni_log(tLogFlags logType, const char* logMsg,
                          const char* logFile, int logLine, const char* logFunc)
{
  ...
  if (!niFlagIs(logType, eLogFlags_NoCallbackOutput) && _logCallback) {
    ...
    if (_inCallback.Get() < 10) {
      ...
      callbackRet =
        _logCallback(logType, logTime, logFile, logFunc, logLine, logMsg);
    }
    else {
       ...
       (_logStderrEnabled.get() && !isStdoutLogtype)))
  {
    ...
    if (lastLogs) {
      cString final;
      ni_log_format_message(final, logType, logFile, logLine, logFunc, logMsg,
                            logTime, _fPrevTime);
  ...
  #ifdef HAS_CONSOLE_COLORS
      _SetConsoleColors(final, isStdoutLogtype, logType,
                        eSetConsoleColorMode::BEFORE_LINE);
  #endif // HAS_CONSOLE_COLORS
      ni_log_format_message(final, logType, logFile, logLine, logFunc, logMsg,
                            logTime, _fPrevTime);
  #ifdef HAS_CONSOLE_COLORS
      _SetConsoleColors(final, isStdoutLogtype, 0,
                        eSetConsoleColorMode::END_OF_LINE);
  #endif
  ...
  #ifdef HAS_CONSOLE_COLORS
      _SetConsoleColors(final, isStdoutLogtype, 0,
                        eSetConsoleColorMode::AFTER_FLUSH);
  #endif
    }
#endif // LOG_STDIO
    ...
    if (_logFileEnabled.get()) {
      if (!_logFileInitialized) {
        ...
        if (!path.IsEmpty()) {
          _logFile = ni::GetRootFS()->FileOpen(path.GetPath().Chars(),
                                               _logFileOverwrite.get()
                                                 ? ni::eFileOpenMode_Write
    ...
    {
      cString formatted;
      ni_log_format_message(formatted, logType | eLogFlags_FormatMSVC, logFile,
                            logLine, logFunc, logMsg, logTime, _fPrevTime);
...
void __stdcall cLang::Log(tLogFlags type, const achar* msg, const achar* file,
                          tU32 line, const achar* func)
{
