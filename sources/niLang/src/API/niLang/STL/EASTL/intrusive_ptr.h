...
class intrusive_ptr {
 ...
 public:
      ...
      : mpObject(p)
  {
    if (mpObject && bAddRef)
      intrusive_ptr_add_ref(
        mpObject); // Intentionally do not prefix the call with eastl:: but instead allow namespace lookup to resolve the namespace.
  }
  ...
  intrusive_ptr& operator=(T* pObject)
  {
    if (pObject != mpObject) {
      T* const pTemp =
        mpObject; // Create temporary to prevent possible problems with re-entrancy.
      if (pObject)
...
template <typename T>
inline void swap(intrusive_ptr<T>& intrusivePtr1,
                 intrusive_ptr<T>& intrusivePtr2)
{
