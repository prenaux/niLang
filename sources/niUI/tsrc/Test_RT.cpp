...
struct RTBase : public ni::cWidgetSinkImpl<> {
  struct TopLeftCanvas : public ni::cWidgetSinkImpl<> {
    ...
    tBool __stdcall OnSinkAttached() niImpl
    {
      ...
      Ptr<iGraphicsContext> ptrGC =
        mpWidget->GetGraphics()->CreateImageGraphicsContext(ptrImage);
      niPanicAssert(ptrGC.IsOK());
      Ptr<iCanvas> ptrCanvas =
        mpWidget->GetGraphics()->CreateCanvas(ptrGC, NULL);
  ...
  {
    Ptr<iWidget> right =
      mpWidget->GetUIContext()->CreateWidgetRaw("MyCanvas", mpWidget);
  ...
  tBool __stdcall OnLeftClickDown(const sVec2f& avMousePos,
                                  const sVec2f& avNCMousePos) niImpl
  {
    _ToggleAnimation();
    return eFalse;
  }
  ...
  static tBool DrawFPSRects(iCanvas* apCanvas, iFont* apFont)
  {
    ...
    const sRectf rect = ni::sRectf(apCanvas->GetViewport().GetWidth() - 30 - 15,
                                   20, 30.0f, 30.0f);
    apCanvas->SetMatrix(MatrixRotationPivot(
      u,
      ni::MatrixRotationZ<tF32>(
        t, ni::WrapRad(tF32(ni::GetLang()->GetFrameNumber()) / 100.0f)),
      Vec3f(rect.GetCenter().x, rect.GetCenter().y, 0)));
    apCanvas->BlitFill(rect, 0xFFFF00FF);
    const sRectf rect2 = ni::sRectf(
      apCanvas->GetViewport().GetWidth() - 30 - 30 - 25, 20, 30.0f, 30.0f);
    apCanvas->SetMatrix(MatrixRotationPivot(
      u,
      ni::MatrixRotationZ<tF32>(
        t, (tF32)ni::WrapRad(ni::GetLang()->TimerInSeconds())),
      Vec3f(rect2.GetCenter().x, rect2.GetCenter().y, 0)));
  ...
  void _DrawRight(iCanvas* apCanvas)
  {
    ...
    if (topLeftCanvas.IsOK()) {
      Ptr<iTexture> leftTexture =
        topLeftCanvas->GetGraphicsContext()->GetRenderTarget(0);
      CHECK_RETURN_IF_FAILED(leftTexture.IsOK());
      if (!_topRightOverlay.IsOK()) {
        _topRightOverlay =
          mpWidget->GetGraphics()->CreateOverlayTexture(leftTexture);
      }
      else {
    ...
    if (botLeftCanvas.IsOK()) {
      Ptr<iTexture> leftTexture =
        botLeftCanvas->GetGraphicsContext()->GetRenderTarget(0);
      CHECK_RETURN_IF_FAILED(leftTexture.IsOK());
...
struct RT1 : public RTBase {
  ...
  void _DrawTopLeft(iCanvas* apCanvas) niImpl
  {
    ...
    const tF32 cellSize =
      ni::Floor((ni::Min(apCanvas->GetViewport().GetWidth(),
                         apCanvas->GetViewport().GetHeight()) -
                 20) /
                10.f);
    niLoop (i, 10) {
      niLoop (j, 10) {
        const sRectf rect = ni::sRectf(
          (tF32)i * cellSize + 10, (tF32)j * cellSize + 10, cellSize, cellSize);
        if (i == 0 && j == 0) {
        ...
        apCanvas->BlitText(mpWidget->GetFont(), rect, 0,
                           niFmt("A%d", (i + j * 10)));
      }
    }
    apCanvas->Flush();
    apCanvas->GetGraphicsContext()->Display(eGraphicsDisplayFlags_Skip,
                                            sRecti::Null());
  }
...
struct RT2 : public RTBase {
  ...
  void _DrawTopLeft(iCanvas* apCanvas) niImpl
  {
    ...
    apCanvas->BlitText(mpWidget->GetFont(), apCanvas->GetViewport().ToFloat(),
                       eFontFormatFlags_Right, "No Depth Buffer");

    apCanvas->ResetStates();
    apCanvas->GetGraphicsContext()->GetFixedStates()->SetLookAtMatrices(
      eTrue, Vec3f(0, 40, -50), Vec3f(0, 10, 0), Vec3f(0, 1, 0), niPif / 2,
  ...
  {
    apCanvas->BlitText(mpWidget->GetFont(), apCanvas->GetViewport().ToFloat(),
                       eFontFormatFlags_Right, "With Depth Buffer");
    ...
    if (!mptrDSMaterial.IsOK()) {
      ...
      mptrDSMaterial->SetDepthStencilStates(
        eCompiledStates_DS_DepthTestAndWrite);
      mptrDSMaterial->SetFlags(
        mptrDSMaterial->GetFlags() | eMaterialFlags_NoLighting |
        eMaterialFlags_DoubleSided | eMaterialFlags_Vertex);
      mptrDSMaterial->SetChannelSamplerStates(eMaterialChannel_Base,
