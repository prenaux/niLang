#ifndef __ZIPARCH_H_BA94A8DB_1EE0_4F61_8640_B02F70C5F414__
#define __ZIPARCH_H_BA94A8DB_1EE0_4F61_8640_B02F70C5F414__
...
#include "API/niLang/Types.h"
  ...
  #include <time.h>
  #define DECLARE_ZIP_HANDLE(name) \
    struct name##__ {              \
      int unused;                  \
    };                             \
    typedef struct name##__* name
  #define ZIP_MAX_PATH 2048
...
const char* GetZipMessage(ZRESULT code);
  ...
  #define ZR_NOTMMAP \
    0x00020000 // tried to ZipGetMemory, but that only works on mmap zipfiles, which yours wasn't
...
//     ... meanwhile ni::tU32 WINAPI ThreadFunc(void *dat)
//                   { HANDLE hwrite = (HANDLE)dat;
//                     char buf[1000]={17};
//                     ni::tU32 writ; WriteFile(hwrite,buf,1000,&writ,NULL);
//                     CloseHandle(hwrite);
//                     return 0;
//                   }
...
//     ni::tU32 writ; WriteFile(hfz,zbuf,zlen,&writ,NULL);
//     CloseHandle(hfz);
//     CloseZip(hz);
//
...
//     WaitForSingleObject(hthread,INFINITE);
//     CloseHandle(hthread);
//     ... meanwhile ni::tU32 WINAPI ZipReceiverThread(void *dat)
//                   { HANDLE hread = (HANDLE)dat;
//                     char buf[1000];
//                     while (true)
//                     { ni::tU32 red; ReadFile(hread,buf,1000,&red,NULL);
...
astl::set<ni::cString>* ZipGetStoredFileExtensions(HZIP hz);

#endif // #if niMinFeatures(15) && !defined niEmbedded

#endif // __ZIPARCH_H_BA94A8DB_1EE0_4F61_8640_B02F70C5F414__
