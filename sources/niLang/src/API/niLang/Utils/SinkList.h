...
template <typename T>
struct SinkList
    : public ImplRC<iSinkList, eImplFlags_DontInherit1, iCollection> {
 ...
 public:
  ...
  typedef typename tImmutableCollection::const_reverse_iterator
  ...
  tBool __stdcall AddSink(T* apSink)
  {
    ...
    mCollection = ni::Nonnull<tImmutableCollection>{ tImmutableCollection::Add(
      mCollection, apSink) };
    return eTrue;
  }
  ...
  tBool __stdcall AddFrontSink(T* apSink)
  {
    ...
    typename tImmutableCollection::tContainer newContainer =
      mCollection->GetUnderlyingContainer();
  ...
  virtual tBool __stdcall ContainsAll(
    const iCollection* apCollection) const niImpl
  {
    __sync_lock();
 ...
 private:
  ...
  {
    static Nonnull<tImmutableCollection> _EMPTY{
      tImmutableCollection::Create()
    };
    return _EMPTY;
  }
...
#define niLoopSink(TYPE, NAME, C)                         \
  if ((!niIsOK(C)) || (C)->IsEmpty() || (C)->GetMute()) { \
  }                                                       \
  else                                                    \
    for (ni::Nonnull<ni::SinkList<TYPE>::sIterator> NAME( \
           (C)->_ImmutableIterator());                    \
