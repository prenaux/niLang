...
nish_std_PixelOutput TestGpuFuncs_raytracer_lit_cube_ps(nish_std_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
nish_std_PixelOutput TestGpuFuncs_raytracer_lit_cube_ps(nish_std_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_E2 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_E2) {
    ...
    bool _tmp_54 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_54) {
  ...
  vec4 _tmp_i4 = color;
  return nish_std_PixelOutput_new(_tmp_i4);
}
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = TestGpuFuncs_raytracer_lit_cube_ps(aInput, aUniforms, IN_1_aAS);
