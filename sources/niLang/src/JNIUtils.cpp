...
niExportFunc(jclass) niJVM_GetStdClass(JNIEnv* e, eJVMStdClass i)
{
  if (niJVM_classes[i] == NULL) {
    ...
    if (niJVM_classes[i] == NULL || e->ExceptionCheck()) {
      niError(niFmt("Error creating global reference of class %s.",
                    niJVM_classNames[i]));
      return NULL;
    }
...
static int _loadCount = 0;
  ...
  #define LOAD_STATIC_ID(CLASS, VARNAME, NAME, TYPE)                         \
    DEBUG_LOG(                                                               \
      ("### LOAD_STATIC_ID(%s,%s,%s,%s)\n", #CLASS, #VARNAME, #NAME, TYPE)); \
    niJVM_##VARNAME = e->GetStaticMethodID(                                  \
      niJVM_GetStdClass(e, eJVMStdClass_##CLASS), NAME, TYPE);               \
    if (niJVM_##VARNAME == NULL || e->ExceptionCheck()) {                    \
      DEBUG_LOG(                                                             \
        ("Error getting '" NAME "' static method ID of " #CLASS " class.")); \
      return 0;                                                              \
    }
  ...
  #define LOAD_FUN_ID(CLASS, VARNAME, NAME, TYPE)                             \
    DEBUG_LOG(                                                                \
      ("### LOAD_FUN_ID(%s,%s,%s,%s)\n", #CLASS, #VARNAME, #NAME, TYPE));     \
    niJVM_##VARNAME =                                                         \
      e->GetMethodID(niJVM_GetStdClass(e, eJVMStdClass_##CLASS), NAME, TYPE); \
    if (niJVM_##VARNAME == NULL || e->ExceptionCheck()) {                     \
  ...
  #define LOAD_VAR_ID(CLASS, VARNAME, NAME, TYPE)                            \
    DEBUG_LOG(                                                               \
      ("### LOAD_VAR_ID(%s,%s,%s,%s)\n", #CLASS, #VARNAME, #NAME, TYPE));    \
    niJVM_##VARNAME =                                                        \
      e->GetFieldID(niJVM_GetStdClass(e, eJVMStdClass_##CLASS), NAME, TYPE); \
    if (niJVM_##VARNAME == NULL || e->ExceptionCheck()) {                    \
...
niExportFunc(jint) niJVM_OnLoad(JavaVM* vm, void** appEnv, jint aJNIVer)
{
  if (vm->GetEnv(appEnv, aJNIVer) != JNI_OK) {
    DEBUG_LOG(
      ("Could not get JNIEnv for JNI_VERSION_1_4 inside JNI_OnLoad())."));
    return 0;
  }
  ...
  if (_loadCount == 0) {
    // Loader
    LOAD_STATIC_ID(Loader, putMemberOffsetMethodID, "putMemberOffset",
                   "(Ljava/lang/String;Ljava/lang/String;I)V");
    LOAD_STATIC_ID(Loader, setProperty, "setProperty",
                   "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;");
    LOAD_STATIC_ID(Loader, getProperty, "getProperty",
    ...
    // RefCountedPointer
    LOAD_FUN_ID(RefCountedPointer, refCountedConstructorMethodID, "<init>",
                "(JJ)V");
    LOAD_VAR_ID(RefCountedPointer, refCountedObjPtrFieldID, "objPtr", "J");
    LOAD_STATIC_ID(RefCountedPointer, refCounted_cache, "cache",
                   "(JJLjava/lang/Object;)V");
    LOAD_STATIC_ID(RefCountedPointer, refCounted_obtain, "obtain",
    ...
    LOAD_STATIC_ID(Short, Short_static_valueOf, "valueOf",
                   "(S)Ljava/lang/Short;");
    LOAD_STATIC_ID(Integer, Integer_static_valueOf, "valueOf",
                   "(I)Ljava/lang/Integer;");
    LOAD_STATIC_ID(Long, Long_static_valueOf, "valueOf", "(J)Ljava/lang/Long;");
    LOAD_STATIC_ID(Float, Float_static_valueOf, "valueOf",
                   "(F)Ljava/lang/Float;");
    LOAD_STATIC_ID(Double, Double_static_valueOf, "valueOf",
                   "(D)Ljava/lang/Double;");

    // Boolean
    LOAD_FUN_ID(Boolean, Boolean_booleanValue, "booleanValue", "()Z");
    LOAD_STATIC_ID(Boolean, Boolean_static_valueOf, "valueOf",
...
niExportFunc(jint) niJVM_OnUnload(JavaVM* vm, void** appEnv, jint aJNIVer)
{
  if (vm->GetEnv(appEnv, aJNIVer) != JNI_OK) {
    DEBUG_LOG(
      ("Could not get JNIEnv for JNI_VERSION_1_4 inside JNI_OnUnLoad())."));
    return -1;
  }
  if (_loadCount == 0)
    return JNI_OK;
...
niExportFunc(void) niJVM_JString2String(JNIEnv* apEnv, ni::cString& astrOut,
                                        jstring aJString)
{
  jboolean iscopy;
...
niExportFunc(void) niJVM_SetColor3ub(JNIEnv* e, jobject o,
...
niExportFunc(void) niJVM_SetColor4ub(JNIEnv* e, jobject o,
...
niExportFuncCPP(ni::Var) niJVM_GetVar(JNIEnv* e, jobject o)
{
  ...
  if (!o) {
    ...
    if (niJVM_InstanceOf(e, o, IUnknown)) {
      ...
      jboolean isCopy;
      const char* utfJString =
        e->GetStringUTFChars(static_cast<jstring>(o), &isCopy);
...
niExportFunc(jobject) niJVM_NewVar(JNIEnv* e, const ni::Var& v)
{
  ...
  case eType_IUnknown: {
    if (!niIsOK(v.mpIUnknown))
      ...
      return niJVM_IUnknownNew(e, niJVM_Class(e, IUnknown),
                               &niGetInterfaceUUID(ni::iUnknown), v.mpIUnknown);
  }
  default:
    if (VarIsString(v)) {
      ...
      niWarning(niFmt("Variant[%s] can't be convert to Object, returning null",
                      GetTypeString(v.mType)));
      break;
    }
  }
  niJVM_CheckException(e, NULL);
...
struct sJvmRefTableWeakRef {
  ...
  jobject New(JNIEnv* apEnv, jclass aClass, const tUUID* apIID,
              iUnknown* apIUnknown)
  {
    if (!apIUnknown)
      return NULL;
    ...
    if (it != _objects.end()) {
      REF_LOG(("sJvmRefTable::New(%p,%p,%p) - Found: %p [numRefs:%d]\n",
               (void*)apIID, (void*)apIUnknown, (void*)aClass,
               (void*)it->second, apIUnknown->GetNumRefs()));
    ...
    apIUnknown->AddRef();
    jobject objectRef =
      apEnv->NewObject(aClass, niJVM_refCountedConstructorMethodID,
                       (jlong)apIID, (jlong)apIUnknown);
    REF_LOG(("sJvmRefTable::New(%p,%p,%p) - New: %p [numRefs:%d]\n",
             (void*)apIID, (void*)apIUnknown, (void*)aClass, (void*)objectRef,
             apIUnknown->GetNumRefs()));
  ...
  jint Dispose(JNIEnv* apEnv, tIntPtr anIID, tIntPtr anIUnknown)
  {
    ...
    if (it == _objects.end()) {
      DEBUG_LOG(
        ("sJvmRefTable::Dispose(%p,%p) - Not Found !\n", anIID, anIUnknown));
      return ~0;
    }
    else {
...
struct sJvmRefTableNew {
              ...
              iUnknown* apIUnknown)
  {
    ...
    REF_LOG(("sJvmRefTable::New(%p,%p,%p) [numRefs:%d]\n", (void*)apIID,
             (void*)apIUnknown, (void*)aClass, apIUnknown->GetNumRefs()));
    apIUnknown->AddRef();
    jobject objectRef =
      apEnv->NewObject(aClass, niJVM_refCountedConstructorMethodID,
                       (jlong)apIID, (jlong)apIUnknown);
...
struct sJvmRefTableObtain {
              ...
              iUnknown* apIUnknown)
  {
    ...
    objectRef = apEnv->CallStaticObjectMethodA(
      niJVM_Class(apEnv, RefCountedPointer), niJVM_refCounted_obtain, args);
    niJVM_CheckException(apEnv, NULL) if (objectRef != NULL)
    {
    ...
    objectRef = apEnv->NewObject(aClass, niJVM_refCountedConstructorMethodID,
                                 (jlong)apIID, (jlong)apIUnknown);

    args[0].j = (jlong)apIID;
    args[1].j = (jlong)apIUnknown;
    args[2].l = objectRef;
    apEnv->CallStaticVoidMethodA(niJVM_Class(apEnv, RefCountedPointer),
                                 niJVM_refCounted_cache, args);
...
struct sJvmIUnknownTable {
  ...
  jobject New(JNIEnv* apEnv, jclass aClass, const tUUID* apIID,
              const iUnknown* apIUnknown)
  {
    __sync_lock();
    return GetSingleton_refTable()->New(apEnv, aClass, apIID,
                                        const_cast<ni::iUnknown*>(apIUnknown));
  }
  jint Dispose(JNIEnv* apEnv, tIntPtr anIID, tIntPtr anIUnknown)
  {
  #ifdef JVM_DISPOSE_IN_SERVICE
...
niExportFunc(jobject) niJVM_IUnknownNew(JNIEnv* apEnv, jclass aClass,
                                        const tUUID* apIID,
                                        const iUnknown* apIUnknown)
{
  return _iunknownTable.New(apEnv, aClass, apIID, apIUnknown);
}
niExportFunc(jint) niJVM_IUnknownDispose(JNIEnv* apEnv, tIntPtr anIID,
                                         tIntPtr anIUnknown)
{
...
niExportFunc(jint) niJVM_PutMemberOffset(JNIEnv* e, const char* className,
                                         const char* member, int offset)
{
  ...
  args[2].i = offset;
  e->CallStaticVoidMethodA(niJVM_Class(e, Loader),
                           niJVM_putMemberOffsetMethodID, args);
  e->DeleteLocalRef(args[0].l);
  e->DeleteLocalRef(args[1].l);
  if (e->ExceptionCheck()) {
    niError(niFmt("Error in putMemberOffset ID for (%s,%s,%d).", className,
                  member, offset));
    return 0;
  }
  return 1;
 ...
 */
niExportFunc(ni::tBool) niJVM_SetProperty(JNIEnv* e, ni::cString& ret,
                                          ...
                                          const ni::achar* aaszValue)
{
  ...
  if (e->ExceptionCheck()) {
    niError(
      niFmt("Exception in setProperty('%s','%s').", aProperty, aaszValue));
    return ni::eFalse;
  }
  return ni::eTrue;
}
 ...
 */
niExportFunc(ni::tBool) niJVM_GetProperty(JNIEnv* e, ni::cString& ret,
                                          const ni::achar* aProperty)
{
  niAssert(e != NULL);
  jvalue args[1];
...
niExportFunc(void*) niJVM_LockNIOBuffer(JNIEnv* e, jobject jBuffer,
                                        jarray* outBufferArray)
{
  ...
  if (!dataPtr) {
    if (niJVM_nioByteBuffer_array &&
        niJVM_InstanceOf(e, jBuffer, NIOByteBuffer))
    {
      *outBufferArray = (jarray)niJVM_CallMethod(e, jBuffer, Object,
                                                 nioByteBuffer, array, NULL);
    }
    else if (niJVM_nioIntBuffer_array &&
             niJVM_InstanceOf(e, jBuffer, NIOIntBuffer))
    {
      *outBufferArray =
        (jarray)niJVM_CallMethod(e, jBuffer, Object, nioIntBuffer, array, NULL);
    }
    else if (niJVM_nioCharBuffer_array &&
             niJVM_InstanceOf(e, jBuffer, NIOCharBuffer))
    {
      *outBufferArray = (jarray)niJVM_CallMethod(e, jBuffer, Object,
                                                 nioCharBuffer, array, NULL);
    }
    else if (niJVM_nioDoubleBuffer_array &&
             niJVM_InstanceOf(e, jBuffer, NIODoubleBuffer))
    {
      *outBufferArray = (jarray)niJVM_CallMethod(e, jBuffer, Object,
                                                 nioDoubleBuffer, array, NULL);
    }
    else if (niJVM_nioFloatBuffer_array &&
             niJVM_InstanceOf(e, jBuffer, NIOFloatBuffer))
    {
      *outBufferArray = (jarray)niJVM_CallMethod(e, jBuffer, Object,
                                                 nioFloatBuffer, array, NULL);
    }
    else if (niJVM_nioLongBuffer_array &&
             niJVM_InstanceOf(e, jBuffer, NIOLongBuffer))
    {
      *outBufferArray = (jarray)niJVM_CallMethod(e, jBuffer, Object,
                                                 nioLongBuffer, array, NULL);
    }
    else if (niJVM_nioShortBuffer_array &&
             niJVM_InstanceOf(e, jBuffer, NIOShortBuffer))
    {
      *outBufferArray = (jarray)niJVM_CallMethod(e, jBuffer, Object,
                                                 nioShortBuffer, array, NULL);
    }
    else {
...
niExportFunc(void) niJVM_UnlockNIOBuffer(JNIEnv* e, jobject jBuffer,
                                         void* dataPtr, jarray jBufferArray)
{
