...
///
  ...
  #define EASTL_HASHTABLE_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX          \
      " hashtable" // Unless the user overrides something, this is "EASTL hashtable".
#endif

/// EASTL_HASHTABLE_DEFAULT_ALLOCATOR
///
#ifndef EASTL_HASHTABLE_DEFAULT_ALLOCATOR
  #define EASTL_HASHTABLE_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_HASHTABLE_DEFAULT_NAME)
#endif
...
///
niExportFunc(const void**) GetHashtableEmptyBucketArray();
...
EA_DISABLE_VC_WARNING(
  4625 4626) // "copy constructor / assignment operator could not be generated because a base class copy constructor is inaccessible or deleted"
#ifdef EA_COMPILER_MSVC_2015
EA_DISABLE_VC_WARNING(
  5026) // disable warning: "move constructor was implicitly defined as deleted"
#endif
...
struct hash_node<Value, true> {
  ...
  hash_node* mpNext;
  eastl_size_t
    mnHashCode; // See config.h for the definition of eastl_size_t, which defaults to size_t.
} EASTL_MAY_ALIAS;
...
static_assert(Internal::has_hashcode_member<hash_node<int, true>>::value,
              "contains a mnHashCode member");
static_assert(!Internal::has_hashcode_member<hash_node<int, false>>::value,
              "doesn't contain a mnHashCode member");
...
#define ENABLE_IF_HAS_HASHCODE(T, RT) \
  typename eastl::enable_if<Internal::has_hashcode_member<T>::value, RT>::type*
#define ENABLE_IF_HASHCODE_EASTLSIZET(T, RT)                               \
  typename eastl::enable_if<eastl::is_convertible<T, eastl_size_t>::value, \
                            RT>::type
...
struct hashtable_iterator_base {
 ...
 protected:
  template <typename, typename, typename, typename, typename, typename,
            typename, typename, typename, bool, bool, bool>
  ...
  friend bool operator==(const hashtable_iterator_base<V, b>&,
                         const hashtable_iterator_base<V, b>&);
 ...
 public:
  ...
  void increment_bucket()
  {
    ++mpBucket;
    while (*mpBucket ==
           NULL) // We store an extra bucket with some non-NULL value at the end
...
struct hashtable_iterator
    : public hashtable_iterator_base<Value, bCacheHashCode> {
 public:
...
distance_fw_impl(Iterator /*first*/, Iterator /*last*/,
...
template <typename Iterator>
inline typename eastl::iterator_traits<Iterator>::difference_type ht_distance(
...
struct hash_code_base<Key, Value, ExtractKey, Equal, H1, H2, H, false> {
 protected:
  ExtractKey
    mExtractKey; // To do: Make this member go away entirely, as it never has any data.
  Equal
    mEqual; // To do: Make this instance use zero space when it is zero size.
 ...
 public:
  ...
  Equal equal_function()
    const // Deprecated. Use key_eq() instead, as key_eq is what the new C++ standard
  {
    return mEqual;
 ...
 protected:
  ...
  hash_code_base(const ExtractKey& extractKey, const Equal& eq, const H1&,
                 const H2&, const H& h)
  ...
  bucket_index_t bucket_index(const Key& key, hash_code_t,
                              uint32_t nBucketCount) const
  {
    return (bucket_index_t)mRangedHash(key, nBucketCount);
  }

  bucket_index_t bucket_index(const hash_node<Value, false>* pNode,
                              uint32_t nBucketCount) const
  {
    return (bucket_index_t)mRangedHash(mExtractKey(pNode->mValue),
                                       nBucketCount);
  }

  bool compare(const Key& key, hash_code_t,
               hash_node<Value, false>* pNode) const
  {
...
struct hash_code_base<Key, Value, ExtractKey, Equal, H1, H2,
 ...
 protected:
  ...
  hash_code_base(const ExtractKey& ex, const Equal& eq, const H1& h1,
                 const H2& h2, const default_ranged_hash&)
      : mExtractKey(ex)
      , mEqual(eq)
      , m_h1(h1)
      , m_h2(h2)
  ...
  bucket_index_t bucket_index(const Key&, hash_code_t c,
                              uint32_t nBucketCount) const
  {
    return (bucket_index_t)m_h2(c, nBucketCount);
  }

  bucket_index_t bucket_index(const node_type* pNode,
                              uint32_t nBucketCount) const
  {
    return (bucket_index_t)m_h2((hash_code_t)m_h1(mExtractKey(pNode->mValue)),
                                nBucketCount);
  }
...
template <typename Key, typename Value, typename Allocator, typename ExtractKey,
          ...
          typename RehashPolicy, bool bCacheHashCode, bool bMutableIterators,
          bool bUniqueKeys>
class hashtable
    : public rehash_base<RehashPolicy,
                         hashtable<Key, Value, Allocator, ExtractKey, Equal, H1,
                                   H2, H, RehashPolicy, bCacheHashCode,
                                   bMutableIterators, bUniqueKeys>>,
      public hash_code_base<Key, Value, ExtractKey, Equal, H1, H2, H,
                            bCacheHashCode> {
 public:
  typedef Key key_type;
  typedef Value value_type;
  typedef typename ExtractKey::result_type mapped_type;
  typedef hash_code_base<Key, Value, ExtractKey, Equal, H1, H2, H,
                         bCacheHashCode>
    hash_code_base_type;
  ...
  typedef node_iterator<value_type, !bMutableIterators, bCacheHashCode>
    local_iterator;
  typedef node_iterator<value_type, true, bCacheHashCode> const_local_iterator;
  typedef hashtable_iterator<value_type, !bMutableIterators, bCacheHashCode>
    iterator;
  ...
  typedef typename type_select<bUniqueKeys, eastl::pair<iterator, bool>,
                               iterator>::type insert_return_type;
  typedef hashtable<Key, Value, Allocator, ExtractKey, Equal, H1, H2, H,
                    RehashPolicy, bCacheHashCode, bMutableIterators,
                    bUniqueKeys>
    this_type;
  ...
  enum {
    ...
    kAllocFlagBuckets = eastl::
      kHashtableAllocFlagBuckets // Flag to allocator which indicates that we are allocating buckets and not nodes.
  };
 ...
 protected:
  ...
  RehashPolicy
    mRehashPolicy; // To do: Use base class optimization to make this go away.
  allocator_type
    mAllocator; // To do: Use base class optimization to make this go away.

 public:
  hashtable(
    size_type nBucketCount, const H1&, const H2&, const H&, const Equal&,
    const ExtractKey&,
    const allocator_type& allocator = EASTL_HASHTABLE_DEFAULT_ALLOCATOR);

  template <typename FowardIterator>
  hashtable(
    FowardIterator first, FowardIterator last, size_type nBucketCount,
  ...
  template <class... Args>
  iterator try_emplace(const_iterator position, const key_type& k,
                       Args&&... args);
  template <class... Args>
  ...
  template <class P, class = typename eastl::enable_if_t<
  ...
  template <
    class
    P> // See comments below for the const value_type& equivalent to this function.
  ...
  insert_return_type insert(hash_code_t c, node_type* pNodeNew,
  ...
  void clear(
    bool
      clearBuckets); // If clearBuckets is true, we free the bucket memory and set the bucket count back to the newly constructed count.
  void reset_lose_memory()
    EA_NOEXCEPT; // This is a unilateral reset to an initially empty state. No destructors are called, no deallocation occurs.
  ...
  template <typename U, typename UHash, typename BinaryPredicate>
  const_iterator find_as(const U& u, UHash uhash,
  ...
  eastl::pair<const_iterator, const_iterator> find_range_by_hash(
    hash_code_t c) const;
 ...
 protected:
  ...
  iterator DoGetResultIterator(
    BoolConstantT, const insert_return_type& irt,
  ...
  template <typename BoolConstantT>
  eastl::pair<iterator, bool> DoInsertValueExtra(
    BoolConstantT, const key_type& k, hash_code_t c, node_type* pNodeNew,
    value_type&& value, ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

  template <typename BoolConstantT>
  eastl::pair<iterator, bool> DoInsertValue(
    BoolConstantT, value_type&& value,
    ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

  template <typename BoolConstantT>
  iterator DoInsertValueExtra(BoolConstantT, const key_type& k, hash_code_t c,
                              node_type* pNodeNew, value_type&& value,
                              DISABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

  template <typename BoolConstantT>
  iterator DoInsertValue(BoolConstantT, value_type&& value,
                         DISABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

  template <typename BoolConstantT>
  eastl::pair<iterator, bool> DoInsertValueExtra(
    BoolConstantT, const key_type& k, hash_code_t c, node_type* pNodeNew,
    const value_type& value, ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

  template <typename BoolConstantT>
  eastl::pair<iterator, bool> DoInsertValue(
    BoolConstantT, const value_type& value,
    ENABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

  template <typename BoolConstantT>
  iterator DoInsertValueExtra(BoolConstantT, const key_type& k, hash_code_t c,
                              node_type* pNodeNew, const value_type& value,
                              DISABLE_IF_TRUETYPE(BoolConstantT) = nullptr);

  template <typename BoolConstantT>
  iterator DoInsertValue(BoolConstantT, const value_type& value,
                         DISABLE_IF_TRUETYPE(BoolConstantT) = nullptr);
  ...
  node_type* DoFindNode(node_type* pNode, const key_type& k,
                        hash_code_t c) const;
  ...
  node_type* DoFindNodeT(node_type* pNode, const U& u,
                         BinaryPredicate predicate) const;
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(
  size_type nBucketCount, const H1& h1, const H2& h2, const H& h, const Eq& eq,
  const EK& ek, const allocator_type& allocator)
    ...
    , mAllocator(allocator)
{
  if (
    nBucketCount <
    2) // If we are starting in an initially empty state, with no memory allocation done.
    reset_lose_memory();
  else // Else we are creating a potentially non-empty hashtable...
  {
    EASTL_ASSERT(nBucketCount < 10000000);
    mnBucketCount =
      (size_type)mRehashPolicy.GetNextBucketCount((uint32_t)nBucketCount);
    mpBucketArray = DoAllocateBuckets(
      mnBucketCount); // mnBucketCount will always be at least 2.
  }
}
...
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::hashtable(
  FowardIterator first, FowardIterator last, size_type nBucketCount,
  const H1& h1, const H2& h2, const H& h, const Eq& eq, const EK& ek,
  const allocator_type& allocator)
    : rehash_base<rehash_policy_type, hashtable>()
    , hash_code_base<key_type, value_type, extract_key_type, key_equal, h1_type,
                     h2_type, h_type, kCacheHashCode>(ek, eq, h1, h2, h)
    ,
    ...
    , mAllocator(allocator)
{
  if (nBucketCount < 2) {
    ...
    mnBucketCount =
      (size_type)mRehashPolicy.GetBucketCount((uint32_t)nElementCount);
  }
  else {
    EASTL_ASSERT(nBucketCount < 10000000);
    ...
    , mAllocator(x.mAllocator)
{
  ...
  {
    mpBucketArray =
      DoAllocateBuckets(mnBucketCount); // mnBucketCount will be at least 2.
    ...
    , mAllocator(allocator)
{
  reset_lose_memory(); // We do this here the same as we do it in the default ctor because it puts the container in a proper initial empty state. This code would be cleaner if we could rely on being able to use C++11 delegating constructors and just call the default ctor here.
  swap(
    x); // swap will directly or indirectly handle the possibility that mAllocator != x.mAllocator.
}

template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
inline const typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                                bU>::allocator_type&
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::get_allocator() const
  EA_NOEXCEPT
{
  return mAllocator;
}
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                          bU>::allocator_type&
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                      bU>::set_allocator(const allocator_type& allocator)
{
  mAllocator = allocator;
}
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
inline
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::this_type&
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::operator=(
    const this_type& x)
{
  if (this != &x) {
    clear();
...
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNodeFromKey(
  const key_type& key)
{
  node_type* const pNode = (node_type*)allocate_memory(
    mAllocator, sizeof(node_type), EASTL_ALIGN_OF(value_type), 0);
  EASTL_ASSERT_MSG(
    pNode != nullptr,
  ...
  try {
#endif
    ::new (eastl::addressof(pNode->mValue))
      value_type(pair_first_construct, key);
    pNode->mpNext = NULL;
    return pNode;
#if EASTL_EXCEPTIONS_ENABLED
  } catch (...) {
...
#endif
    ::new (eastl::addressof(pNode->mValue))
      value_type(pair_first_construct, eastl::move(key));
    pNode->mpNext = NULL;
    return pNode;
#if EASTL_EXCEPTIONS_ENABLED
  } catch (...) {
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFreeNode(
  node_type* pNode)
{
  pNode->~node_type();
  EASTLFree(mAllocator, pNode, sizeof(node_type));
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFreeNodes(
  node_type** pNodeArray, size_type n)
{
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::node_type**
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateBuckets(
  size_type n)
{
  ...
  EASTL_ASSERT(
    n >
    1); // We reserve an mnBucketCount of 1 for the shared gpEmptyBucketArray.
  EASTL_CT_ASSERT(
    kHashtableAllocFlagBuckets ==
  ...
  node_type** const pBucketArray = (node_type**)EASTLAllocAlignedFlags(
    mAllocator, (n + 1) * sizeof(node_type*), EASTL_ALIGN_OF(node_type*), 0,
    kHashtableAllocFlagBuckets);
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                      bU>::DoFreeBuckets(node_type** pBucketArray, size_type n)
{
  ...
  if (n > 1)
    EASTLFree(
      mAllocator, pBucketArray,
      (n + 1) *
        sizeof(
          node_type*)); // '+1' because DoAllocateBuckets allocates nBucketCount + 1 buckets in order to have a NULL sentinel at the end.
}
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                      bU>::rehash_policy(const rehash_policy_type& rehashPolicy)
{
  mRehashPolicy = rehashPolicy;

  const size_type nBuckets =
    rehashPolicy.GetBucketCount((uint32_t)mnElementCount);
...
inline
  ...
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find(
    const key_type& k) const
{
  const hash_code_t c = get_hash_code(k);
  const size_type n = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
...
template <typename U, typename UHash, typename BinaryPredicate>
inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_as(
  const U& other, UHash uhash, BinaryPredicate predicate)
{
  const hash_code_t c = (hash_code_t)uhash(other);
  const size_type n =
    (size_type)(c %
                mnBucketCount); // This assumes we are using the mod range policy.
...
inline
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::const_iterator
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_as(
    const U& other, UHash uhash, BinaryPredicate predicate) const
{
  const hash_code_t c = (hash_code_t)uhash(other);
  const size_type n =
    (size_type)(c %
...
{
  return hashTable.find_as(u, eastl::hash<U>(),
                           eastl::equal_to_2<const typename H::key_type, U>());
}
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                               bU>::const_iterator,
            typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                               bU>::const_iterator>
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::find_range_by_hash(
  hash_code_t c) const
{
  ...
  if (pNodeStart) {
    eastl::pair<const_iterator, const_iterator> pair(
  ...
  hash_code_t c)
{
  ...
  if (pNodeStart) {
    eastl::pair<iterator, iterator> pair(
...
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::count(
  const key_type& k) const EA_NOEXCEPT
{
  const hash_code_t c = get_hash_code(k);
  const size_type n = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
  size_type result = 0;
...
eastl::pair<
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator,
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator>
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::equal_range(
  const key_type& k)
{
  const hash_code_t c = get_hash_code(k);
  const size_type n = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
...
inline
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::node_type*
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFindNode(
    node_type* pNode, const key_type& k, hash_code_t c) const
{
  for (; pNode; pNode = pNode->mpNext) {
    if (compare(k, c, pNode))
      return pNode;
...
inline
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::node_type*
  hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFindNodeT(
    node_type* pNode, const U& other, BinaryPredicate predicate) const
{
  for (; pNode; pNode = pNode->mpNext) {
    if (
      predicate(
...
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
  BoolConstantT, Args&&... args) // true_type means bUniqueKeys is true.
{
  ...
  {
    const eastl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired(
      (uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);
    ...
    try {
      ...
      EASTL_ASSERT((uintptr_t)mpBucketArray !=
                   (uintptr_t)&GetHashtableEmptyBucketArray()[0]);
      pNodeNew->mpNext = mpBucketArray[n];
      mpBucketArray[n] = pNodeNew;
      ++mnElementCount;

      return eastl::pair<iterator, bool>(iterator(pNodeNew, mpBucketArray + n),
                                         true);
#if EASTL_EXCEPTIONS_ENABLED
    } catch (...) {
      DoFreeNode(pNodeNew);
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
template <typename BoolConstantT, class... Args,
          DISABLE_IF_TRUETYPE(BoolConstantT)>
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(
  BoolConstantT, Args&&... args) // false_type means bUniqueKeys is false.
{
  const eastl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired(
    (uint32_t)mnBucketCount, (uint32_t)mnElementCount, (uint32_t)1);
  ...
  if (pNodePrev == NULL) {
    EASTL_ASSERT((void**)mpBucketArray != &GetHashtableEmptyBucketArray()[0]);
    pNodeNew->mpNext = mpBucketArray[n];
    mpBucketArray[n] = pNodeNew;
  }
  else {
...
template <class... Args>
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::node_type*
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoAllocateNode(
  Args&&... args)
{
  node_type* const pNode = (node_type*)allocate_memory(
    mAllocator, sizeof(node_type), EASTL_ALIGN_OF(value_type), 0);
...
#endif
    ::new (eastl::addressof(pNode->mValue))
      value_type(eastl::forward<Args>(args)...);
    pNode->mpNext = NULL;
    return pNode;
#if EASTL_EXCEPTIONS_ENABLED
  } catch (...) {
...
// do a rehash if the allocation throws.
#if EASTL_EXCEPTIONS_ENABLED
    bool
      nodeAllocated; // If exceptions are enabled then we we need to track if we allocated the node so we can free it in the catch block.
#endif

    if (pNodeNew) {
      ::new (eastl::addressof(pNodeNew->mValue)) value_type(eastl::move(
        value)); // It's expected that pNodeNew was allocated with allocate_uninitialized_node.
#if EASTL_EXCEPTIONS_ENABLED
      nodeAllocated = false;
#endif
    }
    ...
    try {
      ...
      if (
        nodeAllocated) // If we allocated the node within this function, free it. Else let the caller retain ownership of it.
        DoFreeNode(pNodeNew);
      throw;
    }
#endif
  ...
  DISABLE_IF_TRUETYPE(BoolConstantT)) // false_type means bUniqueKeys is false.
{
  ...
  if (bRehash.first)
    DoRehash(
      bRehash
        .second); // Note: We don't need to wrap this call with try/catch because there's nothing we would need to do in the catch.
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::node_type*
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
          bU>::allocate_uninitialized_node()
{
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
               bU>::free_uninitialized_node(node_type* pNode)
{
...
template <class... Args>
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                   bU>::insert_return_type
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::emplace(Args&&... args)
{
  return DoInsertValue(
    has_unique_keys_type(),
    eastl::forward<Args>(
      args)...); // Need to use forward instead of move because Args&& is a "universal reference" instead of an rvalue reference.
}
...
template <class... Args>
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::emplace_hint(
  const_iterator, Args&&... args)
{
  // We currently ignore the iterator argument as a hint.
  insert_return_type result =
    DoInsertValue(has_unique_keys_type(), eastl::forward<Args>(args)...);
...
inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                          bU>::insert_return_type
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::try_emplace(
  const key_type& key, Args&&... args)
{
  return DoInsertValue(has_unique_keys_type(), piecewise_construct,
...
{
  return DoInsertValue(has_unique_keys_type(), piecewise_construct,
                       forward_as_tuple(eastl::move(key)),
                       forward_as_tuple(forward<Args>(args)...));
}
...
inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::try_emplace(
  const_iterator, const key_type& key, Args&&... args)
{
  insert_return_type result =
    DoInsertValue(has_unique_keys_type(),
                  value_type(piecewise_construct, forward_as_tuple(key),
                             forward_as_tuple(forward<Args>(args)...)));
  ...
  const_iterator, key_type&& key, Args&&... args)
{
  insert_return_type result = DoInsertValue(
    has_unique_keys_type(),
    value_type(piecewise_construct, forward_as_tuple(eastl::move(key)),
               forward_as_tuple(forward<Args>(args)...)));
...
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                   bU>::insert_return_type
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(
...
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                   bU>::insert_return_type
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(
  hash_code_t c, node_type* pNodeNew, P&& otherValue)
{
  // pNodeNew->mValue is expected to be uninitialized.
  value_type value(eastl::forward<P>(
    otherValue)); // Need to use forward instead of move because P&& is a "universal reference" instead of an rvalue reference.
  const key_type& k = mExtractKey(value);
  return DoInsertValueExtra(has_unique_keys_type(), k, c, pNodeNew,
                            eastl::move(value));
}
  ...
  const_iterator, value_type&& value)
{
  // We currently ignore the iterator argument as a hint.
  insert_return_type result =
    DoInsertValue(has_unique_keys_type(), value_type(eastl::move(value)));
  return DoGetResultIterator(has_unique_keys_type(), result);
}
...
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                   bU>::insert_return_type
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(
  hash_code_t c, node_type* pNodeNew, const value_type& value)
{
  // pNodeNew->mValue is expected to be uninitialized.
  const key_type& k = mExtractKey(value);
  ...
  const_iterator, const value_type& value)
{
  ...
  return result
    .first; // Note by Paul Pedriana while perusing this code: This code will fail to compile when bU is false (i.e. for multiset, multimap).
}

template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert(
  std::initializer_list<value_type> ilist)
{
  insert(ilist.begin(), ilist.end());
}
  ...
  InputIterator first, InputIterator last)
{
  ...
  const eastl::pair<bool, uint32_t> bRehash = mRehashPolicy.GetRehashRequired(
    (uint32_t)mnBucketCount, (uint32_t)mnElementCount, nElementAdd);
...
eastl::pair<
  typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator,
  bool>
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert_or_assign(
  const key_type& k, M&& obj)
{
  auto iter = find(k);
  if (iter == end()) {
    return insert(value_type(piecewise_construct, forward_as_tuple(k),
                             forward_as_tuple(eastl::forward<M>(obj))));
  }
  else {
...
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert_or_assign(
  const_iterator, const key_type& k, M&& obj)
{
  return insert_or_assign(k, eastl::forward<M>(obj))
    .first; // we ignore the iterator hint
}
...
typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::insert_or_assign(
  const_iterator, key_type&& k, M&& obj)
{
  return insert_or_assign(eastl::move(k), eastl::forward<M>(obj))
    .first; // we ignore the iterator hint
}
...
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::erase(const_iterator i)
{
  iterator iNext(
    i.mpNode,
    i.mpBucket); // Convert from const_iterator to iterator while constructing.
  ++iNext;
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
inline typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator
hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::erase(
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          ...
          bool bU>
inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM,
                      bU>::reset_lose_memory() EA_NOEXCEPT
{
...
#ifdef _MSC_VER
  mpBucketArray = (node_type**)&GetHashtableEmptyBucketArray()[0];
#else
  void* p = &GetHashtableEmptyBucketArray()[0];
  memcpy(
    &mpBucketArray, &p,
    sizeof(
      mpBucketArray)); // Other compilers implement strict aliasing and casting is thus unsafe.
#endif
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::reserve(
  size_type nElementCount)
{
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::rehash(
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoRehash(
  size_type nNewBucketCount)
{
  node_type** const pBucketArray = DoAllocateBuckets(
    nNewBucketCount); // nNewBucketCount should always be >= 2.
  ...
  try {
    ...
    for (size_type i = 0; i < mnBucketCount; ++i) {
      while ((pNode = mpBucketArray[i]) !=
             NULL) // Using '!=' disables compiler warnings.
      {
        const size_type nNewBucketIndex =
          (size_type)bucket_index(pNode, (uint32_t)nNewBucketCount);
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
inline bool hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::validate()
  const
{
  const void** pEmptyBucketArray = GetHashtableEmptyBucketArray();
  // Verify our empty bucket array is unmodified.
  if (pEmptyBucketArray[0] != NULL)
    return false;

  if (pEmptyBucketArray[1] != (void*)uintptr_t(~0))
    return false;
  ...
  if ((void**)mpBucketArray == &pEmptyBucketArray[0]) {
    if (
      mnElementCount) // gpEmptyBucketArray is used only for empty hash tables.
      return false;

    if (mnBucketCount !=
        1) // gpEmptyBucketArray is used exactly an only for mnBucketCount == 1.
      return false;
  }
  else {
...
template <typename K, typename V, typename A, typename EK, typename Eq,
          typename H1, typename H2, typename H, typename RP, bool bC, bool bM,
          bool bU>
int hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::validate_iterator(
