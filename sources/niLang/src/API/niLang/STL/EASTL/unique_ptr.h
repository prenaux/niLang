...
class unique_ptr {
 ...
 public:
  ...
  {
    static_assert(
      !eastl::is_pointer<deleter_type>::value,
  ...
  unique_ptr(
    ...
    typename eastl::conditional<
      eastl::is_reference<deleter_type>::value, deleter_type,
  ...
  unique_ptr(pointer pValue,
    ...
    EA_NOEXCEPT : mPair(pValue, eastl::move(deleter))
  {
    static_assert(
      !eastl::is_reference<deleter_type>::value,
  ...
  unique_ptr(
    ...
    typename enable_if<
      ...
      (is_same<deleter_type, E>::value ||
       !is_reference<deleter_type>::value)>::type* = 0) EA_NOEXCEPT
  ...
  typename add_lvalue_reference<T>::type operator*()
    const // Not noexcept, because the pointer may be NULL.
  {
    return *mPair.first();
  }
...
class unique_ptr<T[], Deleter> {
 ...
 public:
  ...
  template <
    typename P> // Pointers to types derived from T are rejected by the constructors, and by reset.
  explicit unique_ptr(
    P pArray, typename eastl::enable_if<
                Internal::is_array_cv_convertible<P, pointer>::value>::type* =
                0) EA_NOEXCEPT : mPair(pArray)
  {
    static_assert(
      !eastl::is_pointer<deleter_type>::value,
      "unique_ptr deleter default-constructed with null pointer. Use a different constructor or change your deleter to a class.");
  ...
  unique_ptr(P pArray,
             ...
             typename eastl::enable_if<
               Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0)
    EA_NOEXCEPT : mPair(pArray, eastl::move(deleter))
  {
    static_assert(
      !eastl::is_reference<deleter_type>::value,
      "deleter_type reference refers to an rvalue deleter. The reference will probably become invalid before used. Change the deleter_type to not be a reference or construct with permanent deleter.");
  ...
  unique_ptr(unique_ptr<U, E>&& u,
             typename enable_if<
               ...
               (!eastl::is_reference<deleter_type>::value ||
                eastl::is_same<E, deleter_type>::value)>::type* = 0) EA_NOEXCEPT
      : mPair(u.release(), eastl::forward<E>(u.get_deleter()))
  {
  }
...
// It's not possible to create a unique_ptr for arrays of a known bound (e.g. int[4] as opposed to int[]).
template <typename T, typename... Args>
typename Internal::unique_type<T>::unique_type_bounded_array make_unique(
  Args&&...) = delete;
