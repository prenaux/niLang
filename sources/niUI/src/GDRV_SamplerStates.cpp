...
//----------------------------------------------------------------------------
//
// Section: SamplerStates
//
//----------------------------------------------------------------------------

template <tBool IS_RO>
class cSamplerStates : public ImplRC<iSamplerStates>,
...
#define END()                                    \
  astl::upsert(mmapSamplerStates, (tIntPtr)cs,   \
...
///////////////////////////////////////////////
iSamplerStates* __stdcall cGraphics::GetCompiledSamplerStates(
  tIntPtr aHandle) const
{
  if (!aHandle)
    return NULL;
  tSSMap::const_iterator it = mmapSamplerStates.find(aHandle);
  if (it == mmapSamplerStates.end())
    return NULL;
