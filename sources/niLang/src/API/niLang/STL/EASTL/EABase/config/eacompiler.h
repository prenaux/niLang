...
//
  ...
  #if (__cplusplus >= \
       201103L) // Clang and GCC defines this like so in C++11 mode.
    #define EA_COMPILER_CPP11_ENABLED 1
  #elif defined(__GNUC__) && defined(__GXX_EXPERIMENTAL_CXX0X__)
    #define EA_COMPILER_CPP11_ENABLED 1
  #elif defined(_MSC_VER) && \
    _MSC_VER >=              \
      1600 // Microsoft unilaterally enables its C++11 support; there is no way to disable it.
    #define EA_COMPILER_CPP11_ENABLED 1
...
// C++14 support.
  ...
  #if (__cplusplus >= \
       201402L) // Clang and GCC defines this like so in C++14 mode.
...
// GCC (a.k.a. GNUC)
  ...
  #define EA_COMPILER_STRING                                   \
    EA_COMPILER_NAME " compiler, version " INTERNAL_STRINGIZE( \
      __GNUC__) "." INTERNAL_STRINGIZE(__GNUC_MINOR__)
    ...
    ((__GNUC_MINOR__ == 1) || \
     (__GNUC_MINOR__ ==       \
      2)) // If GCC 3.1 or 3.2 (but not pre 3.1 or post 3.2)...
...
// The Intel compiler is based on the EDG compiler front-end.
  ...
  #define EA_COMPILER_NAME "Intel C++"
    ...
    #define EA_COMPILER_STRING                                                                             \
      EA_COMPILER_NAME                                                                                     \
        " compiler, version " INTERNAL_STRINGIZE(EA_COMPILER_VERSION) ", EDG version " INTERNAL_STRINGIZE( \
          __EDG_VERSION__) ", VC++ version " INTERNAL_STRINGIZE(_MSC_VER)
  #elif defined(__GNUC__)
    #define EA_COMPILER_STRING                                                                             \
      EA_COMPILER_NAME                                                                                     \
        " compiler, version " INTERNAL_STRINGIZE(EA_COMPILER_VERSION) ", EDG version " INTERNAL_STRINGIZE( \
          __EDG_VERSION__) ", GCC version " INTERNAL_STRINGIZE(__GNUC__)
  #else
    #define EA_COMPILER_STRING                                   \
      EA_COMPILER_NAME " compiler, version " INTERNAL_STRINGIZE( \
        EA_COMPILER_VERSION) ", EDG version " INTERNAL_STRINGIZE(__EDG_VERSION__)
  #endif
  ...
  #define EA_STANDARD_LIBRARY_MICROSOFT 1
    ...
    #define EA_COMPILER_NO_EXPLICIT_FUNCTION_TEMPLATE_ARGUMENTS \
      1 // The compiler compiles this OK, but executes it wrong. Fixed in VC7.0
    #define EA_COMPILER_NO_VOID_RETURNS \
      1 // The compiler fails to compile such cases. Fixed in VC7.0
    #define EA_COMPILER_NO_EXCEPTION_STD_NAMESPACE \
      1 // The compiler fails to compile such cases. Fixed in VC7.0
    #define EA_COMPILER_NO_DEDUCED_TYPENAME \
      1 // The compiler fails to compile such cases. Fixed in VC7.0
    #define EA_COMPILER_NO_STATIC_CONSTANTS \
      1 // The compiler fails to compile such cases. Fixed in VC7.0
    #define EA_COMPILER_NO_COVARIANT_RETURN_TYPE \
      1 // The compiler fails to compile such cases. Fixed in VC7.1
    #define EA_COMPILER_NO_ARGUMENT_DEPENDENT_LOOKUP \
      1 // The compiler compiles this OK, but executes it wrong. Fixed in VC7.1
    #define EA_COMPILER_NO_TEMPLATE_TEMPLATES \
      1 // The compiler fails to compile such cases. Fixed in VC7.1
    #define EA_COMPILER_NO_TEMPLATE_PARTIAL_SPECIALIZATION \
      1 // The compiler fails to compile such cases. Fixed in VC7.1
    ...
    #define EA_COMPILER_NO_MEMBER_FUNCTION_SPECIALIZATION \
      1 // This is the case only for VC7.0 and not VC6 or VC7.1+. Fixed in VC7.1
    ...
    #define EA_COMPILER_MSVC_2011 \
      1 // Microsoft changed the name to VS2012 before shipping, despite referring to it as VS2011 up to just a few weeks before shipping.
...
// IBM
  ...
  #define EA_COMPILER_STRING \
    "IBM XL C compiler, version " INTERNAL_STRINGIZE(__xlC__)
...
// Unknown
  ...
  #define EA_COMPILER_STRING \
    EA_COMPILER_NAME         \
      " compiler, version " INTERNAL_STRINGIZE(EA_COMPILER_VERSION)
#endif
...
//
  ...
  #define EA_COMPILER_NO_RTTI 1
#elif defined(__ARMCC_VERSION) && defined(__TARGET_CPU_MPCORE) && \
  !defined(__RTTI)
  #define EA_COMPILER_NO_RTTI 1
#endif
...
//
    ...
    defined(                       \
      _NO_EX) // GCC on some platforms defines _NO_EX when exceptions are disabled.
    #define EA_COMPILER_NO_EXCEPTIONS 1

  #elif defined(EA_COMPILER_CLANG) && \
    !EA_COMPILER_HAS_FEATURE(         \
      cxx_exceptions) // Use __has_feature(cxx_exceptions) with Clang so that its correctly detected when used in Objective C++
    #define EA_COMPILER_NO_EXCEPTIONS 1

  #elif (defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_INTEL) || \
         defined(EA_COMPILER_RVCT)) &&                              \
    !defined(                                                       \
      __EXCEPTIONS) // GCC and most EDG-based compilers define __EXCEPTIONS when exception handling is enabled.
    #define EA_COMPILER_NO_EXCEPTIONS 1

  #elif (defined(EA_COMPILER_BORLAND) || defined(EA_COMPILER_MSVC)) && \
    !defined(_CPPUNWIND)
...
//This is duplicated from EABase's eacompilertraits.h
    ...
    #define EA_DISABLE_ALL_VC_WARNINGS()           \
      __pragma(warning(push, 0)) __pragma(warning( \
        disable : 4244 4265 4267 4350 4472 4509 4548 4623 4710 4985 6320 4755 4625 4626 4702)) // Some warnings need to be explicitly called out.
  #else
    #define EA_DISABLE_ALL_VC_WARNINGS()
  #endif
#endif
...
//This is duplicated from EABase's eahave.h
  ...
  !defined(EA_NO_HAVE_DINKUMWARE_CPP_LIBRARY)
...
EA_RESTORE_ALL_VC_WARNINGS()
  ...
  #if defined(__cplusplus) && \
    defined(_CPPLIB_VER) /* If using the Dinkumware Standard library... */
...
//
    ...
    #define EA_THROW_SPEC_DELETE_NONE() throw()
      ...
      #define EA_THROW_SPEC_NEW(X)                                           \
        __pragma(warning(push)) __pragma(warning(disable : 4987)) _THROWS(X) \
  ...
  #endif
#elif defined(EA_COMPILER_NO_EXCEPTIONS) && !defined(EA_COMPILER_RVCT) && \
  !defined(EA_PLATFORM_LINUX) && !defined(EA_PLATFORM_APPLE) &&           \
...
//
  ...
  #elif defined(__GNUC__) && \
    (((__GNUC__ * 100) + __GNUC_MINOR__)) < 401 // If earlier than GCC 4.1..
    #define EA_COMPILER_NO_VARIADIC_MACROS 1
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (_MSC_VER >= 1600) // VS2010+
                       // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 403) // EDG 4.3+.
  ...
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    EA_COMPILER_HAS_FEATURE(cxx_rvalue_references)
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4005) // GCC 4.5+
    // supported.
  #else
...
//
  ...
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 401) // EDG 4.1+.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    defined(__apple_build_version__) && (EA_COMPILER_VERSION >= 401)
    // Extern template is supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    !defined(__apple_build_version__) // Clang other than Apple's Clang
    // Extern template is supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4006) // GCC 4.6+
    // Extern template is supported.
  #else
...
//
    ...
    (defined(_MSC_VER) && (EA_COMPILER_VERSION >= 1700)) // VS2012+...
                                                         // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 405) // EDG 4.5+.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && \
    (defined(__clang__) && (EA_COMPILER_VERSION >= 300)) // Clang 3.x+
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && \
    (defined(__GNUC__) && (EA_COMPILER_VERSION >= 4006)) // GCC 4.6+
    // supported.
  #else
...
//
    ...
    (defined(_MSC_VER) &&                   \
     (EA_COMPILER_VERSION >=                \
      1900)) // VS2015+... Not present in VC++ up to and including VS2013.
             // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 406) // EDG 4.6+.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    EA_COMPILER_HAS_FEATURE(cxx_constexpr)
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4006) // GCC 4.6+
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1900) // VS 2015+
    // supported.
  #else
...
//
    ...
    (defined(_MSC_VER) && (EA_COMPILER_VERSION >= 1911)) // VS2017 15.3+
                                                         // supported.
  #elif defined(EA_COMPILER_CPP17_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 309) // Clang 3.9+
    // supported.
  #elif defined(EA_COMPILER_CPP17_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 7000) // GCC 7+
    // supported.
  #else
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >                                       \
     1600) // VC++ > VS2010, even without C++11 support. VS2010 does support override, however will generate warnings due to the keyword being 'non-standard'
           // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 209) // Clang 2.9+
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4007) // GCC 4.7+
    // supported.
  #else
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1500) // VS2008+, even without C++11 support.
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1600) // VS2010+
                                  // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 401) // EDG 4.1+.
    // supported with the exception of the usage of braced initializer lists as of EDG 4.3.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 209) // Clang 2.9+, including Apple's Clang.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4004) // GCC 4.4+
    // supported.
  #else
...
//
    ...
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4003) // GCC 4.3+
    // supported.
  #else
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1800) // VS2013+
  ...
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 300) // Clang 3.0+, including Apple's Clang
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4004) // GCC 4.4+
    // supported.
...
//
  ...
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 401) && defined(__apple_build_version__)
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 301) &&                                 \
    !defined(                                                       \
      __apple_build_version__) // Clang 3.1+, not including Apple's Clang.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4004) // GCC 4.4+
    // supported.
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1700) // VS2012+
    // supported. A subset of this is actually supported by VS2010.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 400) // EDG 4.0+.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 209) // Clang 2.9+, including Apple's Clang.
    // supported.
...
//
    ...
    (EA_COMPILER_VERSION >= 1800) // VS2013+.
                                  // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (_MSC_FULL_VER == 170051025) // VS2012 November Preview for Windows only.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 403) // EDG 4.3+.
...
//
    ...
    // supported, though 4.1 has partial support for variadic templates.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 300) &&                                 \
    !defined(                                                       \
      __apple_build_version__) // Clang 3.0+, not including Apple's Clang.
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4007) // GCC 4.7+
    // supported, though GCC 4.3 has partial support for variadic templates.
...
//
  ...
  #if defined(_MSC_VER) && \
    (_MSC_FULL_VER >= 190023918) // VS2015 Update 2 and above.
                                 // supported.
  #elif defined(EA_COMPILER_CPP14_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 304) &&                                 \
    !defined(                                                       \
      __apple_build_version__) // Clang 3.4+, not including Apple's Clang.
    // supported.
  #elif defined(EA_COMPILER_CPP14_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 5000) // GCC 5+
    // supported.
...
//
    ...
    // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4008) // GCC 4.8+
    // supported.
  #else
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 401) &&                               \
    defined(__apple_build_version__) // Apple clang 4.1+
                                     // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 402) // EDG 4.2+.
...
//
  ...
  #if defined(EA_COMPILER_CPP17_ENABLED)
    // supported.
  #elif defined(EA_COMPILER_MSVC) && defined(__cplusplus) && \
    (EA_COMPILER_VERSION >= 1912) // VS2017 15.3+
    // supported.
  #else
...
//
  ...
  // Not supported by VC++ as of VS2013.
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    EA_COMPILER_HAS_FEATURE(cxx_inheriting_constructors) // Clang
                                                         // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4008) // GCC 4.8+
    // supported.
...
//
  ...
  // Not supported by VC++ as of VS2013.
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 405) // EDG 4.5+.
                             // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 209) // Clang 2.9+, including Apple's Clang.
...
//
    ...
    #if (                   \
      EA_COMPILER_VERSION < \
      1700) // VS2010 generates a warning, but the C++ language now allows it.
      #pragma warning( \
          disable : 4836) // nonstandard extension used: local types or unnamed types cannot be used as template arguments.
    #endif
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__EDG_VERSION__) && \
    (__EDG_VERSION__ >= 402) // EDG 4.2+.
    // supported.
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1900) // VS2014+
                                  // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__clang__) && \
    (EA_COMPILER_VERSION >= 401) &&                                 \
    defined(__apple_build_version__) // Apple clang 4.1+
...
//
  ...
  #if defined( \
    EA_COMPILER_NO_UNICODE_STRING_LITERALS) // Some compilers have had support for char16_t prior to support for u"", but it's not useful to have the former without the latter.
...
//
  ...
  #if defined(EA_COMPILER_CPP11_ENABLED) && defined(_MSC_VER) && \
    (EA_COMPILER_VERSION >= 1900) // VS2015+
                                  // supported.
  #elif defined(EA_COMPILER_CPP11_ENABLED) && defined(__GNUC__) && \
    (EA_COMPILER_VERSION >= 4008) // GCC 4.8+
    // supported.
