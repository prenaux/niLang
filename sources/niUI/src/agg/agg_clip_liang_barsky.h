...
inline unsigned clip_liang_barsky(T x1, T y1, T x2, T y2,
                                  const rect_base<T>& clip_box, T* x, T* y)
{
  const agg_real nearzero = 1e-30;

  agg_real deltax = x2 - x1;
  agg_real deltay = y2 - y1;
  agg_real xin;
  agg_real xout;
  agg_real yin;
  agg_real yout;
...
//----------------------------------------------------------------------------
template <class T>
bool clip_move_point(T x1, T y1, T x2, T y2, const rect_base<T>& clip_box, T* x,
                     T* y, unsigned flags)
{
  T bound;
  ...
  if (flags & clipping_flags_x_clipped) {
    ...
    *y = (T)(agg_real(bound - x1) * (y2 - y1) / (x2 - x1) + y1);
    *x = bound;
  }
  ...
  if (flags & clipping_flags_y_clipped) {
    ...
    *x = (T)(agg_real(bound - y1) * (x2 - x1) / (y2 - y1) + x1);
    *y = bound;
  }
  return true;
}
