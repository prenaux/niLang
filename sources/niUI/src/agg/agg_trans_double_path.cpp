...
//------------------------------------------------------------------------
void trans_double_path::move_to1(agg_real x, agg_real y)
{
...
//------------------------------------------------------------------------
void trans_double_path::line_to1(agg_real x, agg_real y)
{
...
//------------------------------------------------------------------------
void trans_double_path::move_to2(agg_real x, agg_real y)
{
...
//------------------------------------------------------------------------
void trans_double_path::line_to2(agg_real x, agg_real y)
{
...
//------------------------------------------------------------------------
agg_real trans_double_path::finalize_path(vertex_storage& vertices)
{
  unsigned i;
  agg_real dist;
  agg_real d;
  ...
  if (vertices.size() > 2) {
    ...
    {
      d =
        vertices[vertices.size() - 3].dist + vertices[vertices.size() - 2].dist;
...
//------------------------------------------------------------------------
agg_real trans_double_path::total_length1() const
{
  if (m_base_length >= 1e-10)
    return m_base_length;
  return (m_status1 == ready) ? m_src_vertices1[m_src_vertices1.size() - 1].dist
                              : 0.0;
}

//------------------------------------------------------------------------
agg_real trans_double_path::total_length2() const
{
  if (m_base_length >= 1e-10)
    return m_base_length;
  return (m_status2 == ready) ? m_src_vertices2[m_src_vertices2.size() - 1].dist
                              : 0.0;
}

//------------------------------------------------------------------------
void trans_double_path::transform1(const vertex_storage& vertices,
                                   agg_real kindex, agg_real kx, agg_real* x,
                                   agg_real* y) const
{
  agg_real x1 = 0.0;
  agg_real y1 = 0.0;
...
//------------------------------------------------------------------------
void trans_double_path::transform(agg_real* x, agg_real* y) const
{
  if (m_status1 == ready && m_status2 == ready) {
    ...
    agg_real x2 = *x;
    agg_real y2 = *y;
    agg_real dd = m_src_vertices2[m_src_vertices2.size() - 1].dist /
