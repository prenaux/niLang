...
template <typename /*InputIteratorCategory*/, bool /*isMove*/,
          bool /*canMemmove*/>
struct move_and_copy_helper {
  template <typename InputIterator, typename OutputIterator>
  static OutputIterator move_or_copy(InputIterator first, InputIterator last,
...
struct move_and_copy_helper<EASTL_ITC_NS::random_access_iterator_tag, false,
  ...
  static OutputIterator move_or_copy(InputIterator first, InputIterator last,
                                     OutputIterator result)
  {
    typedef typename eastl::iterator_traits<InputIterator>::difference_type
      difference_type;
...
template <>
struct move_and_copy_helper<EASTL_ITC_NS::random_access_iterator_tag, true,
                            false> {
  template <typename InputIterator, typename OutputIterator>
  static OutputIterator move_or_copy(InputIterator first, InputIterator last,
                                     OutputIterator result)
...
struct move_and_copy_helper<EASTL_ITC_NS::random_access_iterator_tag, isMove,
                            true> {
  template <typename T>
...
inline OutputIterator move_and_copy_chooser(InputIterator first,
                                            InputIterator last,
                                            OutputIterator result)
{
  typedef typename eastl::iterator_traits<InputIterator>::iterator_category IIC;
  typedef
    typename eastl::iterator_traits<OutputIterator>::iterator_category OIC;
  typedef
    typename eastl::iterator_traits<InputIterator>::value_type value_type_input;
  typedef typename eastl::iterator_traits<OutputIterator>::value_type
    value_type_output;
  ...
  const bool canBeMemmoved =
    ...
    (eastl::is_pointer<InputIterator>::value ||
     eastl::is_same<IIC, eastl::contiguous_iterator_tag>::value) &&
    (eastl::is_pointer<OutputIterator>::value ||
     eastl::is_same<OIC, eastl::contiguous_iterator_tag>::value);

  return eastl::move_and_copy_helper<IIC, isMove, canBeMemmoved>::move_or_copy(
    first, last,
    result); // Need to chose based on the input iterator tag and not the output iterator tag, because containers accept input ranges of iterator types different than self.
}
...
template <bool isMove, typename InputIterator, typename OutputIterator>
inline OutputIterator move_and_copy_unwrapper(InputIterator first,
                                              InputIterator last,
                                              OutputIterator result)
{
  return OutputIterator(eastl::move_and_copy_chooser<isMove>(
    ...
    eastl::unwrap_iterator(
      result))); // Have to convert to OutputIterator because result.base() could be a T*
}
...
template <typename InputIterator, typename OutputIterator>
inline OutputIterator move(InputIterator first, InputIterator last,
                           OutputIterator result)
{
  return eastl::move_and_copy_unwrapper<true>(
    eastl::unwrap_iterator(first), eastl::unwrap_iterator(last), result);
}
                           ...
                           OutputIterator result)
{
  ...
  return eastl::move_and_copy_unwrapper<isMove>(
    eastl::unwrap_iterator(first), eastl::unwrap_iterator(last), result);
}
} // namespace eastl
