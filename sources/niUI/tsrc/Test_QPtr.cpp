...
struct QueryTest : public ni::cWidgetSinkImpl<> {
  ...
  tBool __stdcall OnSinkAttached() niImpl
  {
    ...
    Ptr<iWidget> theWidget =
      mpWidget->GetUIContext()->CreateWidgetRaw("MockWidget", NULL);
    CHECK(theWidget.IsOK());
    CHECK_EQUAL(
      2,
      theWidget
        ->GetNumRefs()); // one ref in Ptr<> and one as child of the RootWidget
    CHECK_EQUAL(mpWidget->GetUIContext()->GetRootWidget(),
                theWidget->GetParent());
    ...
    CHECK_EQUAL(
      2,
      theWidget
        ->GetNumRefs()); // Refcount incorrect with incomplete refcounting in QPtr
    CHECK_EQUAL(
      3,
      mock
        ->GetNumRefs()); // Refcount incorrect with incomplete refcounting in QPtr
