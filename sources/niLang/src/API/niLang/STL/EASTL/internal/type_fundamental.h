...
template <>
struct has_void_arg<> : public eastl::false_type {};

template <typename A0, typename... Args>
struct has_void_arg<A0, Args...> {
  static const bool value =
    (eastl::is_void<A0>::value || eastl::has_void_arg<Args...>::value);
};
...
#if defined(EA_COMPILER_CPP11_ENABLED) && !defined(EA_COMPILER_NO_DECLTYPE) && \
  !defined(_MSC_VER) // VC++'s handling of decltype(nullptr) is broken.
  #define EASTL_TYPE_TRAIT_is_null_pointer_CONFORMANCE 1

template <typename T>
struct is_null_pointer
    : public eastl::is_same<typename eastl::remove_cv<T>::type,
                            decltype(nullptr)> {
}; // A C++11 compiler defines nullptr, but you need a C++11 standard library to declare std::nullptr_t. So it's safer to compare against decltype(nullptr) than to use std::nullptr_t, because we may have a C++11 compiler but C++98 library (happens with Apple frequently).
#else
  #define EASTL_TYPE_TRAIT_is_null_pointer_CONFORMANCE 1
...
template <typename T>
struct is_integral
    : public eastl::is_integral_helper<typename eastl::remove_cv<T>::type> {};
...
#define EASTL_DECLARE_INTEGRAL(T)                             \
  ...
  template <>                                                 \
  struct is_integral<const T> : public true_type {};          \
  template <>                                                 \
  struct is_integral<volatile T> : public true_type {};       \
  template <>                                                 \
  struct is_integral<const volatile T> : public true_type {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_floating_point_CONFORMANCE \
  1 // is_floating_point is conforming.
...
template <typename T>
struct is_floating_point : public eastl::is_floating_point_helper<
                             typename eastl::remove_cv<T>::type> {};
...
#define EASTL_DECLARE_FLOATING_POINT(T)                             \
  ...
  template <>                                                       \
  struct is_floating_point<const T> : public true_type {};          \
  template <>                                                       \
  struct is_floating_point<volatile T> : public true_type {};       \
  template <>                                                       \
  struct is_floating_point<const volatile T> : public true_type {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_arithmetic_CONFORMANCE \
  1 // is_arithmetic is conforming.
...
#define EASTL_TYPE_TRAIT_is_fundamental_CONFORMANCE \
  1 // is_fundamental is conforming.
...
template <typename T>
struct is_hat_type_helper : public false_type {};

#if (EABASE_VERSION_N > 20607 && defined(EA_COMPILER_WINRTCX_ENABLED)) || \
  defined(__cplusplus_winrt)
template <typename T>
struct is_hat_type_helper<T ^> : public true_type {};
#endif
