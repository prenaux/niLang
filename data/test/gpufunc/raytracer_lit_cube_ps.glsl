...
vec3 lib_shader_Vec3TransformCoord(vec3 v, mat4 m) {
  vec3 _tmp_b = v;
  vec4 r = (m * vec4(_tmp_b.x,_tmp_b.y,_tmp_b.z,1.0));
  return ((r/r.w).xyz);
}
vec3 lib_shader_Vec3TransformNormal_2_Vec3_Matrix4x3(vec3 v, mat4x3 m) {
  vec3 _tmp_k = v;
  vec3 r = (m * vec4(_tmp_k.x,_tmp_k.y,_tmp_k.z,0.0));
  return (r.xyz);
}
...
TestGpuFuncs_PixelOutput TestGpuFuncs_raytracer_lit_cube_ps(lib_shader_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
TestGpuFuncs_PixelOutput TestGpuFuncs_raytracer_lit_cube_ps(lib_shader_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_E2 = (intersectionType == lib_shader_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_E2) {
    bool _tmp_H2 = (instData.vbIndex == 0);
    if (_tmp_H2) {
    ...
    else {
      {
        ...
        vec3 C = ((TestGpuFuncs_DirLight(worldPos,worldNormal,worldLightDir0,lightColor0,cosBias,shadowColor,aAS)+TestGpuFuncs_DirLight(worldPos,worldNormal,worldLightDir1,lightColor1,cosBias,shadowColor,aAS))+TestGpuFuncs_DirLight(worldPos,worldNormal,worldLightDir2,lightColor2,cosBias,shadowColor,aAS));
        vec3 _tmp_44 = C;
    ...
    bool _tmp_54 = (intersectionType == lib_shader_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_54) {
  ...
  vec4 _tmp_i4 = color;
  return TestGpuFuncs_PixelOutput_new(_tmp_i4);
}
// MODULE END TestGpuFuncs

// Pixel Shader main: TestGpuFuncs_raytracer_lit_cube_ps
...
void main(void) {
  ...
  TestGpuFuncs_PixelOutput _rval_ = TestGpuFuncs_raytracer_lit_cube_ps(aInput, aUniforms, IN_1_aAS);
