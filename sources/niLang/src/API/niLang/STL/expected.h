...
#define ASTL_EXPECTED_IS_TRIVIALLY_DESTRUCTIBLE(T) std::is_trivially_destructible<T>
#else
#define ASTL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)                         \
...
public:
  ...
  ASTL_EXPECTED_11_CONSTEXPR E &&value() && { return std::move(m_val); }
...
struct expected_operations_base : expected_storage_base<T, E> {
  ...
  ASTL_EXPECTED_11_CONSTEXPR T &&get() && { return std::move(this->m_val); }
...
#else
  template <class F>
  ASTL_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected &>(),
                                                   std::declval<F &&>()))
  map_error(F &&f) & {
    return map_error_impl(*this, std::forward<F>(f));
  }
...
public:
  ...
  ASTL_EXPECTED_11_CONSTEXPR E &&error() && { return std::move(err().value()); }
