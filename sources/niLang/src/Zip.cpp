...
///////////////////////////////////////////////
tBool __stdcall cZip::ZipUncompressBuffer(tPtr apDest, tU32 anDestSize,
                                          tPtr apSrc, tU32 anSrcSize)
{
...
///////////////////////////////////////////////
tBool __stdcall cZip::ZipUncompressBufferInFile(iFile* apDest, tU32 anDestSize,
                                                tPtr apSrc, tU32 anSrcSize)
{
...
///////////////////////////////////////////////
tBool __stdcall cZip::ZipUncompressFile(iFile* apDest, tU32 anDestSize,
                                        iFile* apSrc, tU32 anSrcSize)
{
  tU8* pDestBuffer = (tU8*)niMalloc(anDestSize);
  if (!pDestBuffer) {
    niError(_A("Can't allocate dest buffer."));
...
///////////////////////////////////////////////
tBool __stdcall cZip::ZipUncompressFileInBuffer(tPtr apDest, tU32 anDestSize,
                                                iFile* apSrc, tU32 anSrcSize)
{
  tU8* pSrcBuffer = (tU8*)niMalloc(anSrcSize);
  if (!pSrcBuffer) {
...
///////////////////////////////////////////////
tU32 __stdcall cZip::ZipCompressBuffer(tPtr apDest, tPtr apSrc, tU32 anSrcSize,
                                       tU32 anLevel)
{
...
///////////////////////////////////////////////
tU32 __stdcall cZip::ZipCompressBufferInFile(iFile* apDest, tPtr apSrc,
                                             tU32 anSrcSize, tU32 anLevel)
{
  uLongf destLen = compressBound(anSrcSize);
...
///////////////////////////////////////////////
tU32 __stdcall cZip::ZipCompressFile(iFile* apDest, iFile* apSrc,
                                     tU32 anSrcSize, tU32 anLevel)
{
  uLongf destLen = compressBound(anSrcSize);
  tU8* pDestBuffer = (tU8*)niMalloc(destLen);
...
///////////////////////////////////////////////
tBool __stdcall cZip::ZipInflateFileInBuffer(tPtr apDest, tU32 anDestSize,
                                             iFile* apSrc)
{
...
///////////////////////////////////////////////
tI32 cZip::UnzLocateFile(tHandle file, const achar* szFileName,
                         tBool bCaseSensitivity)
{
  return niUnzip_LocateFile(unzFile(file), cString(szFileName).Chars(),
                            bCaseSensitivity);
}
...
///////////////////////////////////////////////
tI32 cZip::UnzGetCurrentFileInfo(tHandle file, unz_file_info* pfile_info,
                                 cString& strFilename, cString& strExtraField,
                                 cString& strComment)
{
  char cszFilename[AMAX_SIZE];
  char cszExtraField[AMAX_SIZE];
  char cszComment[AMAX_SIZE];
  tI32 ret = niUnzip_GetCurrentFileInfo(unzFile(file), pfile_info, cszFilename,
                                        AMAX_SIZE, cszExtraField, AMAX_SIZE,
                                        cszComment, AMAX_SIZE);
...
///////////////////////////////////////////////
tI32 __stdcall cZip::DeflateSetDictionary(z_streamp strm, const tU8* dictionary,
                                          tI32 dictLength)
{
...
///////////////////////////////////////////////
tI32 __stdcall cZip::DeflateTune(z_streamp strm, tI32 good_length,
...
///////////////////////////////////////////////
tI32 __stdcall cZip::InflateSetDictionary(z_streamp strm, const tU8* dictionary,
                                          tI32 dictLength)
{
...
///////////////////////////////////////////////
tI32 __stdcall cZip::InflateBack(z_streamp strm, zlib_in_func aIn,
...
///////////////////////////////////////////////
tI32 __stdcall cZip::Compress(tU8* dest, tU32* apDestLen, const tU8* source,
                              tU32 sourceLen)
{
...
///////////////////////////////////////////////
tI32 __stdcall cZip::Compress2(tU8* dest, tU32* apDestLen, const tU8* source,
                               tU32 sourceLen, tI32 level)
{
  uLongf destLen = apDestLen ? *apDestLen : 0;
...
///////////////////////////////////////////////
tI32 __stdcall cZip::Uncompress(tU8* dest, tU32* apDestLen, const tU8* source,
                                tU32 sourceLen)
{
  uLongf destLen = apDestLen ? *apDestLen : 0;
...
///////////////////////////////////////////////
tI32 __stdcall cZip::DeflateInit2(z_streamp strm, tI32 level, tI32 method,
                                  tI32 windowBits, tI32 memLevel, tI32 strategy)
{
...
///////////////////////////////////////////////
tI32 __stdcall cZip::InflateBackInit(z_streamp strm, tI32 windowBits,
...
iFile* __stdcall cZip::GZipOpen(iFileBase* apFile, tU32 aulCompressionMode)
{
  ...
  #else
  Ptr<iFile> ptrFile =
    ni::CreateFile(niNew cGZipFile(apFile, aulCompressionMode, false));
  if (!niIsOK(ptrFile)) {
    return NULL;
  }
...
iFile* __stdcall cZip::ZipOpen(iFileBase* apFile, tU32 aulCompressionMode)
{
  ...
  #else
  Ptr<iFile> ptrFile =
    ni::CreateFile(niNew cZipFile(apFile, aulCompressionMode));
  if (!niIsOK(ptrFile)) {
    return NULL;
  }
  return ptrFile.GetRawAndSetNull();
...
struct ZipBufferEncoder : public ImplRC<iBufferEncoder> {
  ni::Ptr<iZip> _zip;
  const tU32 _level;
  ZipBufferEncoder(iZip* apZip, tU32 anCompressionLevel)
      : _level(anCompressionLevel)
  {
...
iFile* __stdcall cZip::CreateFileZipBufferEncoder(iFileBase* apBase,
                                                  tU32 anCLevel)
{
...
iFile* __stdcall cZip::CreateFileZipBufferDecoder(iFileBase* apBase,
                                                  tSize aDecSize)
{
  niCheck(niIsOK(apBase), NULL);
...
iFile* __stdcall cZip::CreateFileRawBufferDecoder(iFileBase* apBase,
                                                  tSize aDecSize)
{
  niCheck(niIsOK(apBase), NULL);
...
class cZipArchWrite : public ni::ImplRC<iZipArchWrite> {
 ...
 public:
  cZipArchWrite(iFile* apFile, const char* pwd,
                const tStringCVec* apStoredExtensions)
  {
    niAssert(niIsOK(apFile));
    mhZip = CreateZipHandle(
      apFile, pwd, apStoredExtensions ? apStoredExtensions->data() : NULL,
...
#if niMinFeatures(15)
iZipArchWrite* __stdcall cZip::CreateZipArchive(
  ni::iFile* apDest, const char* aaszPwd, const tStringCVec* apStoredExtensions)
{
...
struct URLFileHandler_Zip : public URLFileHandler_Manifest {
  ...
  virtual iFile* __stdcall URLOpen(const achar* aURL)
  {
    ...
    if (ni::GetZip()->UnzLocateFile(mhFile, path.Chars(), 2) ==
    ...
    cString fn, extra, comment;
    ni::GetZip()->UnzGetCurrentFileInfo(mhFile, &fileinfo, fn, extra, comment);

    Ptr<iFile> ptrFile =
      ni::CreateFileMemoryAlloc(fileinfo.uncompressed_size, path.Chars());
    if (!niIsOK(ptrFile)) {
      niError(niFmt(_A("Can't open mapped file for url %s."), aURL));
      return NULL;
    }

    ni::GetZip()->UnzReadCurrentFile(mhFile, ptrFile->GetBase(),
...
niExportFunc(iUnknown*) New_niLang_URLFileHandlerZip(const Var& avarA,
                                                     const Var& avarB)
{
