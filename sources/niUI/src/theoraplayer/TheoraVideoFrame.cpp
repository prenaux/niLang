...
TheoraVideoFrame::TheoraVideoFrame(TheoraVideoClip* parent)
{
	...
	const unsigned int size = mParent->getStride() * mParent->mHeight * mBpp;
  mBuffer = niTMalloc(unsigned char, size);
...
iBitmap2D* TheoraVideoFrame::getBitmap(ni::iGraphics* apGraphics, const char* aPixelFormat) {
  if (!mptrBitmap.IsOK()) {
    mptrBitmap = apGraphics->CreateBitmap2DMemory(this->getWidth(), this->getHeight(),
                                                  aPixelFormat,
                                                  this->getStride() * mBpp,
                                                  this->getBuffer(), eFalse);
  }
  return mptrBitmap;
}
...
void TheoraVideoFrame::decode(struct TheoraPixelTransform* t)
{
	...
	else
	{
		...
		char s[128];
		sprintf(s, "%.2f", diff / N);
		TheoraVideoManager::getSingleton().logMessage("YUV Decoding time: " + astl::string(s) + " ms\n");
#else
		conversion_functions[mParent->getOutputMode()](t);
#endif
	}
