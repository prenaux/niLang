...
auto invoke_impl(R C::*func, T&& obj, Args&&... args) ->
  typename enable_if<is_base_of<C, decay_t<decltype(obj)>>::value,
                     decltype((eastl::forward<T>(obj).*
                               func)(eastl::forward<Args>(args)...))>::type
{
  return (eastl::forward<T>(obj).*func)(eastl::forward<Args>(args)...);
}

template <typename F, typename... Args>
auto invoke_impl(F&& func, Args&&... args)
...
struct invoke_result_impl<
  F,
  void_t<decltype(invoke_impl(eastl::declval<decay_t<F>>(),
                              eastl::declval<Args>()...))>,
  Args...> {
  typedef decltype(invoke_impl(eastl::declval<decay_t<F>>(),
...
struct is_invocable_impl<
  F, void_t<typename eastl::invoke_result<F, Args...>::type>, Args...>
...
struct is_invocable_r_impl<
  R, F, void_t<typename invoke_result<F, Args...>::type>, Args...>
...
template <typename F, typename... Args>
EASTL_CPP17_INLINE_VARIABLE EA_CONSTEXPR bool is_invocable_v =
  is_invocable<F, Args...>::value;

template <typename R, typename F, typename... Args>
EASTL_CPP17_INLINE_VARIABLE EA_CONSTEXPR bool is_invocable_r_v =
...
class reference_wrapper {
 public:
  ...
  typename eastl::result_of<T&(ArgTypes&&...)>::type operator()(
    ArgTypes&&...) const;
...
template <typename T>
reference_wrapper<T>::reference_wrapper(T& v) EA_NOEXCEPT : val(addressof(v))
{
}

template <typename T>
reference_wrapper<T>::reference_wrapper(const reference_wrapper<T>& other)
  EA_NOEXCEPT : val(other.val)
{
}
...
template <typename T>
template <typename... ArgTypes>
typename eastl::result_of<T&(ArgTypes&&...)>::type reference_wrapper<
...
template <typename T>
struct is_reference_wrapper_helper : public eastl::false_type {};
...
template <typename T>
struct is_reference_wrapper : public eastl::is_reference_wrapper_helper<
...
template <typename M, typename C, typename T>
auto invoke_impl(M(C::*member), T&& obj) -> typename enable_if<
  is_reference_wrapper<typename remove_reference<T>::type>::value,
