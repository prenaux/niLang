...
inline ni::tBool __stdcall WinRegWrite(
  const WCHAR* aaszPath, const WCHAR* aaszVal,
  const WCHAR* aaszType niDefaultParam(L"REG_SZ"))
{
  ...
  case REG_EXPAND_SZ: {
    r =
      niWin32API(RegSetValueEx)(hKey, strValueName, 0, type, (BYTE*)aaszVal,
  ...
  case REG_DWORD: {
    ...
    r = niWin32API(RegSetValueEx)(hKey, strValueName, 0, type, (BYTE*)&v,
                                  sizeof(v));
    break;
  }
  }
...
inline ni::tBool __stdcall WinRegWrite(
  const ni::cchar* aaszPath, const ni::cchar* aaszVal,
...
inline WCHAR* __stdcall WinRegRead(WCHAR* apOut, const WCHAR* aaszPath)
{
  ...
  WCHAR strValueName[AMAX_PATH] = L"";
  WinRegPath_GetValueName(strValueName, strValuePath);
  LONG r = niWin32API(RegQueryValueEx)(hKey, strValueName, NULL, &type,
                                       (BYTE*)retBuffer, &retSize);
  if (r != ERROR_SUCCESS) {
  ...
  switch (type) {
  case REG_SZ:
  case REG_EXPAND_SZ: wcscpy(apOut, retBuffer); break;
  case REG_DWORD: {
    UTF16Buffer wOut;
    wOut.Adopt(apOut);
    niCAssert(sizeof(retBuffer) >= AMAX_SIZE);
    StringIntToStr((char*)retBuffer, sizeof(retBuffer),
                   *(ni::tU32*)(retBuffer));
...
inline ni::cchar* __stdcall WinRegRead(ni::cchar* apOut,
                                       const ni::cchar* aaszPath)
{
  UTF16Buffer wPath;
  niWin32_UTF8ToUTF16(wPath, aaszPath);
