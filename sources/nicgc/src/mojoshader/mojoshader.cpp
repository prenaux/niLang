...
// This make RCP & RSQ use precisely the same semantics as D3D9.
// #define MOJOSHADER_GL_FLT_MAX_RCP_RSQ
...
static bool _GLSL_useFloatColor = false;
void MOJOSHADER_GLSL_SetFloatColor(bool abUseFloatColor) {
  _GLSL_useFloatColor = abUseFloatColor;
}
bool MOJOSHADER_GLSL_GetFloatColor() {
  return _GLSL_useFloatColor;
}

static bool _GLSL_useStandardDerivatives = false;
void MOJOSHADER_GLSL_SetStandardDerivatives(bool abUseStandardDerivatives) {
  _GLSL_useStandardDerivatives = abUseStandardDerivatives;
}
bool MOJOSHADER_GLSL_GetStandardDerivatives() {
  return _GLSL_useStandardDerivatives;
}

static eMOJOSHADERTextureLod _GLSL_useTextureLod = eMOJOSHADERTextureLod_None;
void MOJOSHADER_GLSL_SetTextureLod(eMOJOSHADERTextureLod abUseTextureLod) {
  _GLSL_useTextureLod = abUseTextureLod;
}
eMOJOSHADERTextureLod MOJOSHADER_GLSL_GetTextureLod() {
  return _GLSL_useTextureLod;
}
...
static const char *get_GLSL_varying_name_in_buf(Context *ctx, RegisterType rt,
                                                int regnum, char *buf,
                                                const size_t len)
{
  snprintf(buf, len, "var_%d", regnum);
  return buf;
} // get_GLSL_varname_in_buf
...
static void emit_GLSL_start(Context *ctx, const char *profilestr)
{
  ...
  else if (strcmp(profilestr, MOJOSHADER_PROFILE_GLSL) == 0)
  {
...
#if defined MOJOSHADER_GLSL_GLES
    if (_GLSL_useStandardDerivatives) {
      if (shader_is_pixel(ctx)) {
        output_line(ctx, "#extension GL_OES_standard_derivatives : enable");
      }
    }
    if (_GLSL_useTextureLod == eMOJOSHADERTextureLod_EXT) {
      if (shader_is_pixel(ctx)) {
        output_line(ctx, "#extension GL_EXT_shader_texture_lod : enable");
      }
    }

    output_line(ctx, "precision highp float;");
    output_line(ctx, "#define high_float highp float");

#elif defined MOJOSHADER_GLSL_DESKTOP
    output_line(ctx, "#version 110");
    output_line(ctx, "#define high_float float");
    if (_GLSL_useTextureLod == eMOJOSHADERTextureLod_ARB) {
      if (shader_is_pixel(ctx)) {
        output_line(ctx, "#extension GL_ARB_shader_texture_lod : enable");
      }
    }
    else if (_GLSL_useTextureLod == eMOJOSHADERTextureLod_EXT) {
      if (shader_is_pixel(ctx)) {
        output_line(ctx, "#extension GL_EXT_shader_texture_lod : enable");
      }
    }
    ...
    if (shader_is_pixel(ctx)) {
      if (_GLSL_useStandardDerivatives) {
        output_line(ctx, "vec2 my_ddx(vec2 v) { return dFdx(v); }");
        output_line(ctx, "vec2 my_ddy(vec2 v) { return -dFdy(v); }");
        output_line(ctx, "vec3 my_ddx(vec3 v) { return dFdx(v); }");
        output_line(ctx, "vec3 my_ddy(vec3 v) { return -dFdy(v); }");
        output_line(ctx, "vec4 my_ddx(vec4 v) { return dFdx(v); }");
        output_line(ctx, "vec4 my_ddy(vec4 v) { return -dFdy(v); }");
      }
      else {
        output_line(ctx, "vec2 my_ddx(vec2 v) { return v+vec2(1.0,0); }");
        output_line(ctx, "vec2 my_ddy(vec2 v) { return v+vec2(0,1.0); }");
        output_line(ctx, "vec3 my_ddx(vec3 v) { return v+vec3(1,0,0); }");
        output_line(ctx, "vec3 my_ddy(vec3 v) { return v+vec3(0,1,0); }");
        output_line(ctx, "vec4 my_ddx(vec4 v) { return v+vec4(1,0,0,0); }");
        output_line(ctx, "vec4 my_ddy(vec4 v) { return v+vec4(0,1,0,0); }");
      }

      // texture2DProj isn't available on all platforms...
      output_line(ctx, "vec4 safeTexture2DProj(sampler2D s, vec4 coord) {");
      output_line(ctx, "  return texture2D(s,coord.xy/coord.w);");
      output_line(ctx, "}");
    }

#ifdef MOJOSHADER_GL_FLT_MAX_RCP_RSQ
                                ...
                                int flags)
{
    ...
    if (regtype == REG_TYPE_INPUT)
    {
...
#if 1
      if (index == 10) {
        // Reserved index for ColorA, which needs a RGB swap.
        // Not pretty, but does the job...
        output_line(ctx, "vec4 unpackULColor4(vec4 c) {");
        output_line(ctx, "  return c.bgra;");
        output_line(ctx, "}");
        output_line(ctx, "attribute vec4 %s;", var);
        output_line(ctx, "#define %s unpackULColor4(%s)", var, var);
      }
      else {
        output_line(ctx, "attribute vec4 %s;", var);
      }
      ...
      switch (usage)
      {
        case MOJOSHADER_USAGE_COLOR:
          if (_GLSL_useFloatColor) {
            output_line(ctx, "// FloatColor");
            output_line(ctx, "const float _4c = 63.0;");
            output_line(ctx, "const float _4p = 64.0;");
            output_line(ctx, "const float _4pp = _4p * _4p;");
            output_line(ctx, "const float _4ppp = _4p * _4p * _4p;");
            output_line(ctx, "vec4 unpackFloatColor4(high_float f) {");
            output_line(ctx, "  vec4 color;");
            output_line(ctx, "  float r = f;");
            output_line(ctx, "  color.a = floor(r / _4ppp);");
            output_line(ctx, "  r -= (color.a * _4ppp);");
            output_line(ctx, "  color.b = floor(r / _4pp);");
            output_line(ctx, "  r -= (color.b * _4pp);");
            output_line(ctx, "  color.g = floor(r / _4p);");
            output_line(ctx, "  r -= (color.g * _4p);");
            output_line(ctx, "  color.r = floor(r);");
            output_line(ctx, "  return color / _4c;");
            output_line(ctx, "}");
            output_line(ctx, "attribute float %s;", var);
            output_line(ctx, "#define %s unpackFloatColor4(%s)", var, var);
          }
          else {
            output_line(ctx, "vec4 unpackULColor4(vec4 c) {");
            output_line(ctx, "  return c.bgra;");
    ...
    else if (regtype == REG_TYPE_OUTPUT)
    {
      ...
      switch (usage)
      {
        ...
        case MOJOSHADER_USAGE_COLOR:
          ...
          outputEndif = true;
          push_output(ctx, &ctx->globals);
          output_line(ctx, "#if 1");
          get_GLSL_varying_name_in_buf(ctx, regtype, regnum, varying, sizeof (varying));
          output_line(ctx, "varying vec4 vc_%s;", varying);
          output_line(ctx, "#define %s vc_%s", var, varying);
        ...
        case MOJOSHADER_USAGE_TEXCOORD:
          ...
          output_line(ctx, "#if 1");
          get_GLSL_varying_name_in_buf(ctx, regtype, regnum, varying, sizeof (varying));
          output_line(ctx, "varying vec4 vt_%s;", varying);
          output_line(ctx, "#define %s vt_%s", var, varying);
          output_line(ctx, "#else");
          pop_output(ctx);
  ...
  else if (shader_is_pixel(ctx))
  {
    ...
    else if (regtype == REG_TYPE_MISCTYPE)
    {
      ...
      {
        push_output(ctx, &ctx->globals);
        output_line(ctx, "float %s = (gl_FrontFacing == true) ? 1.0 : -1.0;", var);
        pop_output(ctx);
      } // if
...
#else
  char src0[64];
  make_GLSL_srcarg_string_masked(ctx, 0, src0, sizeof (src0));
  char code[128];
...
static void emit_GLSL_RSQ(Context *ctx)
{
  ...
  make_GLSL_srcarg_string_masked(ctx, 0, src0, sizeof (src0));
  char code[128];
  make_GLSL_destarg_assign(ctx, code, sizeof (code), "inversesqrt(abs(%s))", src0);
  output_line(ctx, "%s", code);
#endif
} // emit_GLSL_RSQ
...
enum eTEXLDFLAG {
  eTEXLDFLAG_NONE = 0,
  eTEXLDFLAG_GRAD = niBit(0),
  eTEXLDFLAG_LOD = niBit(1)
};

static void glsl_texld(Context *ctx, const eTEXLDFLAG flag)
{
  ...
  else
  {
    ...
    const char *biassep = "";
    char bias[64] = { '\0' };
    if ((flag & eTEXLDFLAG_LOD) || (ctx->instruction_controls == CONTROL_TEXLDB))
    {
      biassep = ", ";
    ...
    switch ((const TextureType) sreg->index)
    {
      case TEXTURE_TYPE_2D:
        ...
        {
          funcname = "safeTexture2DProj";
          make_GLSL_srcarg_string_full(ctx, 0, src0, sizeof (src0));
        } // if
        else  // texld/texldb
        {
          if (flag & eTEXLDFLAG_LOD)
          {
            funcname = (_GLSL_useTextureLod == eMOJOSHADERTextureLod_ARB) ?
    ...
    if (flag & eTEXLDFLAG_GRAD)
    {
      // PIERRE: TODO, add LOD bias support
      make_GLSL_destarg_assign(ctx, code, sizeof (code),
...
{
  glsl_texld(ctx, eTEXLDFLAG_NONE);
} // emit_GLSL_TEXLD
...
static void emit_GLSL_DSX(Context *ctx)
{
  char src0[64]; make_GLSL_srcarg_string_masked(ctx, 0, src0, sizeof (src0));
  char code[128];
  make_GLSL_destarg_assign(ctx, code, sizeof (code), "my_ddx(%s)", src0);
  output_line(ctx, "%s", code);
} // emit_GLSL_DSX
...
static void emit_GLSL_DSY(Context *ctx)
{
  char src0[64]; make_GLSL_srcarg_string_masked(ctx, 0, src0, sizeof (src0));
  char code[128];
  // PIERRE: Of course the derivative is in inverted in GLSL relative to HLSL...
  make_GLSL_destarg_assign(ctx, code, sizeof (code), "my_ddy(%s)", src0);
  output_line(ctx, "%s", code);
} // emit_GLSL_DSY
...
static void emit_GLSL_TEXLDD(Context *ctx)
{
  ...
  if (!ctx->glsl_generated_texldd_setup)
  {
    ...
    output_line(ctx, "#define texture2DProjGrad(a,b,c,d) safeTexture2DProj(a,b)");
    output_line(ctx, "#endif");
    output_blank_line(ctx);
    pop_output(ctx);
  } // if

  glsl_texld(ctx, eTEXLDFLAG_GRAD);
} // emit_GLSL_TEXLDD
...
{
  // PIERRE: HAHAHAHA seriously.... they actually put this in the specs:
  ...
  // detail."
  // This of course is arbitrarly enforced even on HW that does support
  // TexLDD fine in HW when you use an API that hsnt be "designed" by
  // morons.
  // So yeah... we have to detect whether that unbelivable feature is
  // available.
  if (MOJOSHADER_GLSL_GetTextureLod()) {
    glsl_texld(ctx, eTEXLDFLAG_LOD);
  }
  else {
    glsl_texld(ctx, eTEXLDFLAG_NONE);
  }
...
static int parse_source_token(Context *ctx, SourceArgInfo *info)
{
  ...
  if (!isfail(ctx)) {
    ...
    // !!! FIXME: this test passes if you write to the register in this same
    //            instruction, because we parse the destination token first.
    //            Microsoft's shader validation explicitly checks temp
    //            registers for this...do they check other writable ones?
