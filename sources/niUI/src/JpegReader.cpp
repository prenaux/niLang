...
struct rw_source {
  ...
  // when it needs more data from the file.
  {
    ...
    if (src->m_start_of_file && bytes_read >= 4) {
      if (src->m_buffer[0] == 0xFF && src->m_buffer[1] == 0xD9 &&
          src->m_buffer[2] == 0xFF && src->m_buffer[3] == 0xD8)
      {
        src->m_buffer[1] = 0xD8;
        src->m_buffer[3] = 0xD9;
      }
...
eColorSpace __stdcall cJpegReader::GetColorSpace() const
{
  ...
  case JCS_CMYK: return eColorSpace_CMYK;
  case JCS_YCbCr: return eColorSpace_YCbCr;
  default: break;
  }
...
iBitmap2D* cJpegReader::ReadBitmap(iGraphics* apGraphics)
{
  ...
  Ptr<iBitmap2D> ptrBmp =
    apGraphics->CreateBitmap2D(GetWidth(), GetHeight(), aszFormat);
  if (!ptrBmp.IsOK()) {
  ...
  niLoop (i, GetHeight()) {
    ...
    if (lines_read != 1) {
      niError(
        niFmt(_A("Can't read scanline '%d' (height:%d)."), i, GetHeight()));
      EndRead();
      return NULL;
    }
  }
