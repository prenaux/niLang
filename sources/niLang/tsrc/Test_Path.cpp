...
static const char* const _kTestPathAbsWithDrive =
  "n:/not_a_dir/not_a_file.noext";
static const char* const _kTestPathAbsVol =
  "/not_a_volume/not_a_dir/not_a_file.noext";
...
TEST_FIXTURE(FPath, PathRel)
{
  ...
  CHECK_EQUAL(binDir + _ASTR(_kTestPathRel),
              ni::GetRootFS()->GetAbsolutePath(_kTestPathRel));
}
...
{
  CHECK_EQUAL(_ASTR(_kTestPathAbsWithDrive),
              ni::GetRootFS()->GetAbsolutePath(_kTestPathAbsWithDrive));
}
...
TEST_FIXTURE(FPath, Base)
{
  {
    const cPath pathAbsDrive(_kTestPathAbsWithDrive);
    CHECK_EQUAL(pathAbsDrive.GetPath(),
                cPath("n:/not_a_dir/", "not_a_file.noext").GetPath());
    CHECK_EQUAL(pathAbsDrive.GetPath(),
                cPath("n:/not_a_dir", "not_a_file.noext").GetPath());
  }

  {
    ...
    CHECK_EQUAL(
      pathAbsVol.GetPath(),
      cPath("/not_a_volume/not_a_dir/", "not_a_file.noext").GetPath());
    CHECK_EQUAL(pathAbsVol.GetPath(),
                cPath("/not_a_volume/not_a_dir", "not_a_file.noext").GetPath());
  }

  {
    const cPath pathRel(_kTestPathRel);
    CHECK_EQUAL(pathRel.GetPath(),
                cPath("not_a_dir/", "not_a_file.noext").GetPath());
    CHECK_EQUAL(pathRel.GetPath(),
                cPath("not_a_dir", "not_a_file.noext").GetPath());
  }

  {
    ...
    path.PushDirectory();
    {
      ...
      CHECK_EQUAL(_ASTR("not_a_dir/back_dir/back_dir2/not_a_file.noext"),
                  path.GetPath());
    ...
    path.PushDirectory();
    {
      ...
      CHECK_EQUAL(_ASTR("front_dir/not_a_dir/not_a_file.noext"),
                  path.GetPath());
      path.AddDirectoryFront("front_dir2");
      CHECK_EQUAL(_ASTR("front_dir2/front_dir/not_a_dir/not_a_file.noext"),
                  path.GetPath());
    }
    path.PopDirectory();
    CHECK_EQUAL(pathRel.GetPath(), path.GetPath());
  ...
  {
    cPath path;
    path.SetProtocol("http").SetFile("BarFile.foo").SetDirectory("foodir");
    ...
    CHECK_EQUAL(_ASTR("file://fagiano/another/back/BarFile.foo"),
                path.GetPath());
  }
}
