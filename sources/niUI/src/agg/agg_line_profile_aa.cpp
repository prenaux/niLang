...
//---------------------------------------------------------------------
void line_profile_aa::width(agg_real w)
{
  if (w < 0.0)
    w = 0.0;
...
//---------------------------------------------------------------------
line_profile_aa::value_type* line_profile_aa::profile(agg_real w)
{
...
//---------------------------------------------------------------------
void line_profile_aa::set(agg_real center_width, agg_real smoother_width)
{
  agg_real base_val = 1.0;
  if (center_width == 0.0)
    center_width = 1.0 / subpixel_scale;
  if (smoother_width == 0.0)
    smoother_width = 1.0 / subpixel_scale;

  agg_real width = center_width + smoother_width;
  if (width < m_min_width) {
    agg_real k = width / m_min_width;
    base_val *= k;
    center_width /= k;
    smoother_width /= k;
  }
  ...
  for (i = 0; i < subpixel_smoother_width; i++) {
    *ch_smoother++ = m_gamma[unsigned(
      (base_val - base_val * (agg_real(i) / subpixel_smoother_width)) *
      aa_mask)];
  }
