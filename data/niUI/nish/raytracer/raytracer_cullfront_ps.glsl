...
uint nish_std_RayFlags_CullFrontFacingTriangles;
...
void nish_std_RayFlags_static_initialize() {
  // TypeStatic: RayFlags
  nish_std_RayFlags_CullFrontFacingTriangles = 32;
}
...
niUIGpuFuncs_IntersectionResult niUIGpuFuncs_TraceRay_3_uint_RayDesc_RayInstances(uint aRayFlags, nish_std_RayDesc aRayDesc, accelerationStructureEXT aAS);
nish_std_PixelOutput niUIGpuFuncs_raytracer_cullfront_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
niUIGpuFuncs_IntersectionResult niUIGpuFuncs_TraceRay_3_uint_RayDesc_RayInstances(uint aRayFlags, nish_std_RayDesc aRayDesc, accelerationStructureEXT aAS) {
  return niUIGpuFuncs_TraceRay_6_uint_Vec3_Vec3_float_float_RayInstances(aRayFlags,aRayDesc.origin,aRayDesc.direction,aRayDesc.tMin,aRayDesc.tMax,aAS);
}
nish_std_PixelOutput niUIGpuFuncs_raytracer_cullfront_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  nish_std_RayDesc rayDesc = niUIGpuFuncs_MakeRayDesc(aInput,aUniforms);
  niUIGpuFuncs_IntersectionResult intersection = niUIGpuFuncs_TraceRay_3_uint_RayDesc_RayInstances(nish_std_RayFlags_CullFrontFacingTriangles,rayDesc,aAS);
  vec4 color;
  bool _tmp_S4 = niUIGpuFuncs__op__gte_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_Triangle);
  if (_tmp_S4) {
    vec3 tribary = intersection.bary;
    float edgeThickness = 0.02;
    vec4 edgeColor = vec4(1.0,1.0,1.0,1.0);
    bool _tmp_45 = niUIGpuFuncs_IsTriangleEdge((tribary.xy),edgeThickness);
    if (_tmp_45) {
      color = edgeColor;
    }
    else {
      {
        vec3 _tmp_95 = nish_std_UIntToHashColor(intersection.primIndex);
        color = vec4(_tmp_95.x,_tmp_95.y,_tmp_95.z,1.0);
      }
    }
  }
  else {
    bool _tmp_c5 = niUIGpuFuncs__op__lt_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_None);
    if (_tmp_c5) {
  ...
  vec4 _tmp_q5 = color;
  return nish_std_PixelOutput_new(_tmp_q5);
}
...
// Pixel Shader main: niUIGpuFuncs_raytracer_cullfront_ps
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = niUIGpuFuncs_raytracer_cullfront_ps(aInput, aUniforms, IN_1_aAS);
