...
class rasterizer_outline_aa {
 ...
 public:
  ...
  {
    m_line_join =
      m_ren->accurate_join_only() ? outline_miter_accurate_join : join;
  }
  ...
  //------------------------------------------------------------------------
  void move_to_d(agg_real x, agg_real y)
  {
    move_to(Coord::conv(x), Coord::conv(y));
  }

  //------------------------------------------------------------------------
  void line_to_d(agg_real x, agg_real y)
  {
...
void rasterizer_outline_aa<Renderer, Coord>::draw(draw_vars& dv, unsigned start,
                                                  unsigned end)
{
  unsigned i;
  ...
  for (i = start; i < end; i++) {
    ...
    if (m_line_join == outline_round_join && (dv.flags & 2) == 0) {
      m_ren->pie(dv.curr.x2, dv.curr.y2, dv.curr.x2 + (dv.curr.y2 - dv.curr.y1),
    ...
    switch (m_line_join) {
    case outline_no_join: dv.flags = 3; break;
...
void rasterizer_outline_aa<Renderer, Coord>::render(bool close_polygon)
{
  ...
  if (close_polygon) {
    if (m_src_vertices.size() >= 3) {
      ...
      case outline_miter_accurate_join: dv.flags = 0; break;
      }
    ...
    case 2: {
      ...
      m_ren->line3(lp, x1 + (y2 - y1), y1 - (x2 - x1), x2 + (y2 - y1),
                   y2 - (x2 - x1));
      if (m_round_cap) {
    ...
    case 3: {
      ...
      if (m_line_join == outline_round_join) {
        m_ren->line3(lp1, x1 + (y2 - y1), y1 - (x2 - x1), x2 + (y2 - y1),
                     y2 - (x2 - x1));

        m_ren->pie(x2, y2, x2 + (y2 - y1), y2 - (x2 - x1), x2 + (y3 - y2),
                   y2 - (x3 - x2));

        m_ren->line3(lp2, x2 + (y3 - y2), y2 - (x3 - x2), x3 + (y3 - y2),
                     y3 - (x3 - x2));
      }
      else {
    ...
    default: {
      ...
      if ((dv.flags & 1) == 0) {
        if (m_line_join == outline_round_join) {
          m_ren->line3(prev, x1 + (y2 - y1), y1 - (x2 - x1), x2 + (y2 - y1),
                       y2 - (x2 - x1));
          m_ren->pie(prev.x2, prev.y2, x2 + (y2 - y1), y2 - (x2 - x1),
      ...
      if ((dv.flags & 1) == 0) {
        if (m_line_join == outline_round_join) {
          m_ren->line3(dv.curr, dv.curr.x1 + (dv.curr.y2 - dv.curr.y1),
                       dv.curr.y1 - (dv.curr.x2 - dv.curr.x1),
        ...
        m_ren->line2(dv.curr, dv.curr.x2 + (dv.curr.y2 - dv.curr.y1),
                     dv.curr.y2 - (dv.curr.x2 - dv.curr.x1));
      }
      if (m_round_cap) {
