...
template<class Source> class line_image_scale
{
 public:
  ...
  agg_real width()  const { return m_source.width(); }
  agg_real height() const { return m_height; }
...
template<class Filter> class line_image_pattern
{
 public:
  ...
  agg_real width()      const { return m_height; }
...
class distance_interpolator4
{
 public:
  ...
  distance_interpolator4(int x1,  int y1, int x2, int y2,
                         ...
                         int len, agg_real scale, int x, int y) :
      m_dx(x2 - x1),
      m_dy(y2 - y1),
      ...
      m_dist(iround(agg_real(x + line_subpixel_scale/2 - x2) * agg_real(m_dy) -
...
class renderer_outline_image
{
 public:
  ...
  //---------------------------------------------------------------------
  void   scale_x(agg_real s) { m_scale_x = s; }
  agg_real scale_x() const   { return m_scale_x; }

  //---------------------------------------------------------------------
  void   start_x(agg_real s) { m_start = iround(s * line_subpixel_scale); }
  agg_real start_x() const   { return agg_real(m_start) / line_subpixel_scale; }

  //---------------------------------------------------------------------
  int subpixel_width() const { return m_pattern->line_width(); }
  int pattern_width() const { return m_pattern->pattern_width(); }
  agg_real width() const { return agg_real(subpixel_width()) / line_subpixel_scale; }
