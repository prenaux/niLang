...
struct rw_dest {
  ...
  // Write the output buffer into the stream.
  {
    ...
    if (dest->m_out_stream->WriteRaw(dest->m_buffer, IO_BUF_SIZE) !=
        IO_BUF_SIZE)
    {
      // Error.
  ...
  // data, and make sure we get deleted.
  {
    ...
    if (datacount > 0) {
      if ((int)dest->m_out_stream->WriteRaw(dest->m_buffer, datacount) !=
          datacount)
      {
        // Error.
...
#define JPEG_CATCH_(RETT, RETF, COND)     \
  if (setjmp(mpErr->setjmp_buffer)) {     \
    if (COND) {                           \
      mpErr->Cleanup();                   \
...
///////////////////////////////////////////////
tBool cJpegWriter::BeginWrite(iFile* apDest, tU32 anWidth, tU32 anHeight,
                              eColorSpace aInCS, tU32 anInC, tU32 anQuality,
                              tJpegWriteFlags aFlags)
{
  ...
  case eColorSpace_RGB:
    ...
    break;
  case eColorSpace_YCbCr: mInfo.in_color_space = JCS_YCbCr; break;
  case eColorSpace_YCoCg:
    mInfo.in_color_space = anInC == 4 ? JCS_YCoCgA : JCS_YCoCg;
    break;
  case eColorSpace_CMYK: mInfo.in_color_space = JCS_CMYK; break;
  default:
    niError(niFmt("Unsupported color space '%s'.",
                  niEnumToChars(eColorSpace, aInCS)));
    return eFalse;
  }
...
///////////////////////////////////////////////
tBool cJpegWriter::WriteBitmap(iFile* apDest, const iBitmap2D* apBmp,
                               tU32 anQuality, tJpegWriteFlags aFlags)
{
  ...
  else if (ni::StrCmp(apBmp->GetPixelFormat()->GetFormat(), _A("R8G8B8A8")) ==
           0)
  {
    numC = 4;
    ...
    niError(niFmt(_A("Unsupported pixel format '%s'."),
                  apBmp->GetPixelFormat()->GetFormat()));
    return eFalse;
  }

  if (!BeginWrite(apDest, apBmp->GetWidth(), apBmp->GetHeight(), cs, numC,
                  anQuality, nFlags))
  {
