...
vec3 lib_shader_Vec3TransformNormal_2_Vec3_Matrix4x3(vec3 v, mat4x3 m) {
  vec3 _tmp_q = v;
  vec3 r = (m * vec4(_tmp_q.x,_tmp_q.y,_tmp_q.z,0.0));
  return (r.xyz);
}
...
TestGpuFuncs_PixelOutput TestGpuFuncs_raytracer_pos_world_ps(lib_shader_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
vec3 TestGpuFuncs_PositionToHeat(vec3 aPos, float aMaxDist) {
  ...
  vec3 _tmp_s1;
  bool _tmp_t1 = (distance < centerDist);
  if (_tmp_t1) {
    float t = (distance / centerDist);
    _tmp_s1 = mix(centerColor,bandColor1,t);
  }
  else {
    {
      ...
      bool _tmp_R1 = (isRedToBlue < 0.5);
      if (_tmp_R1) {
        _tmp_s1 = mix(bandColor1,bandColor2,cycleT);
      }
      else {
        {
          _tmp_s1 = mix(bandColor2,bandColor1,cycleT);
        }
      }
    }
  }
...
float TestGpuFuncs_PositionToWorldChecker(vec3 aPos, float aGridSize, vec3 aNormal) {
  ...
  float checker;
  bool _tmp_42 = ((absNormal.x > absNormal.y) && (absNormal.x > absNormal.z));
  if (_tmp_42) {
    checker = mod((gridCoords.y + gridCoords.z),2.0);
  }
  else {
    bool _tmp_l2 = (absNormal.y > absNormal.z);
    if (_tmp_l2) {
...
TestGpuFuncs_PixelOutput TestGpuFuncs_raytracer_pos_world_ps(lib_shader_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_N2 = (intersectionType == lib_shader_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_N2) {
    ...
    bool _tmp_X3 = (intersectionType == lib_shader_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_X3) {
  ...
  vec4 _tmp_a4 = color;
  return TestGpuFuncs_PixelOutput_new(_tmp_a4);
}
...
void main(void) {
  ...
  TestGpuFuncs_PixelOutput _rval_ = TestGpuFuncs_raytracer_pos_world_ps(aInput, aUniforms, IN_1_aAS);
