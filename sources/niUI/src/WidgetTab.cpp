...
///////////////////////////////////////////////
static tF32 _ComputeWidgetButtonWidthFromTextSize(ni::iWidget* apWidget,
                                                  const sRectf& avMargin)
{
  return apWidget->GetFont()
    ->ComputeTextSize(sRectf::Null(), niHStr(apWidget->GetLocalizedText()), 0)
    .GetWidth();
}
///////////////////////////////////////////////
static tF32 _UpdateWidgetButtonSize(ni::iWidget* apWidget,
                                    const sRectf& avMargin, tF32 afTextSize)
{
...
class cTabPageContainer : public ImplRC<iWidgetSink> {
 public:
  cTabPageContainer(cTabWidget* tw)
  {
    mpParent = tw;
  }
      ...
      if (c) {
        if (mpParent->_IsTooSmall()) {
          ...
          if (selrect.GetLeft() != 0) {
            // draw left line
            c->BlitOverlayFrame(
              sRectf(0, 0, selrect.GetLeft(), frame->GetFrame().y1()), frame,
              eRectFrameFlags_TopEdge | eRectFrameFlags_TopLeftCorner);
          }
          if (selrect.GetRight() != rect.GetRight()) {
            // draw right line
            c->BlitOverlayFrame(
              sRectf(selrect.GetRight(), 0,
                     rect.GetRight() - selrect.GetRight(),
                     frame->GetFrame().y1()),
              frame, eRectFrameFlags_TopEdge | eRectFrameFlags_TopRightCorner);
          }
        }
      }
      break;
...
void cTabWidget::_AdjustChildrenLayout()
{
  ...
  for (tTabPageVec::const_iterator itr = mvecPages.begin();
       itr != mvecPages.end(); ++itr)
  {
    maxBtTextSize =
      ni::Max(maxBtTextSize, _ComputeWidgetButtonWidthFromTextSize(
                               itr->pButton, skin.tabButtonFrame->GetFrame()));
  }
       ...
       itr != mvecPages.end(); ++itr)
  {
    maxBtHeight =
      ni::Max(maxBtHeight,
              _UpdateWidgetButtonSize(
                itr->pButton, skin.tabButtonFrame->GetFrame(), maxBtTextSize));
  }
...
///////////////////////////////////////////////
tBool __stdcall cTabWidget::OnWidgetSink(iWidget* apWidget, tU32 nMsg,
                                         const Var& varParam0,
                                         const Var& varParam1)
{
  niGuardObject((iWidgetSink*)this);
...
#pragma niTodo("This is kinda a hack, investigate a better solution")
  ...
  case eUIMessage_SinkAttached:
    mpPageContainer = mpWidget->GetUIContext()->CreateWidget(
      _A("Canvas"), mpWidget, sRectf(), eWidgetStyle_DontSerialize);
  ...
  case eUIMessage_Size:
    ...
    return eFalse;
  case eUIMessage_Paint: _AdjustChildrenLayout(); return eTrue;
  case eUIMessage_NCPaint: _AdjustChildrenLayout(); return eTrue;
  case eUIMessage_StyleChanged: _NeedsAdjusting(); return eFalse;
  ...
  case eUIMessage_Notify: {
    ...
    if (ptrWidget.IsOK() && ptrMsg.IsOK()) {
      if (ptrMsg->GetID() == eUIMessage_LeftClickDown ||
          ptrMsg->GetID() == eUIMessage_RightClickDown)
      {
        ...
        if (ptrWidget.ptr() == mSelectedPage.pButton) {
          ...
          if (pwMenu.IsOK()) {
            ...
            pwMenu->SetAbsolutePosition(ptrMsg->GetB().GetVec2fValue() +
                                        ptrWidget->GetAbsolutePosition());
            ptrMenu->Open();
          }
        }
      }
...
void cTabWidget::_ActivateTab(iWidget* button)
{
  ...
  mpWidget->SendCommand(mpWidget->GetParent(), eWidgetTabCmd_ActivatePage,
                        mpWidget);
}
...
{
  for (tTabPageVec::const_iterator it = mvecPages.begin();
       it != mvecPages.end(); ++it)
  {
    if (it->pPage == apPage) {
      return eTrue;
...
tBool __stdcall cTabWidget::SetActivePage(iWidget* apPage)
{
  TabPage page;
  if (apPage) {
    for (tTabPageVec::iterator it = mvecPages.begin(); it != mvecPages.end();
         ++it)
    {
      if (it->pPage == apPage) {
        page = *it;
...
{
  skin.regularNormalFrame =
    mpWidget->FindSkinElement(_H("TabPage"), NULL, _H("Frame"));
  skin.regularTabButtonFrame =
    mpWidget->FindSkinElement(_H("TabButton"), NULL, _H("Frame"));
  skin.focusNormalFrame =
    mpWidget->FindSkinElement(_H("FocusTabPage"), NULL, _H("Frame"));
  skin.focusTabButtonFrame =
    mpWidget->FindSkinElement(_H("FocusTabButton"), NULL, _H("Frame"));
  //     _UpdateStyle();
  _NeedsAdjusting();
}
