...
#if defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_MSVC) || \
  defined(EA_WCHAR_T_NON_NATIVE) || defined(EA_PLATFORM_KETTLE)
...
// would have put this into the compilertraits header.
  ...
  (defined(EA_COMPILER_IS_C99) || defined(INT8_MIN) || \
   defined(EA_COMPILER_HAS_C99_TYPES) || defined(_SN_STDINT_H))
  ...
  #if !defined(EA_COMPILER_MSVC) || \
    (defined(EA_COMPILER_MSVC) && EA_COMPILER_VERSION >= 1800)
  ...
  #include <float.h>  // FLT_EVAL_METHOD.
    ...
    (defined(__FLT_EVAL_METHOD__) || \
     defined(                        \
       _FEVAL)) // GCC 3.x defines __FLT_EVAL_METHOD__ instead of the C99 standard FLT_EVAL_METHOD.
  ...
  // don't define float_t and double_t.
    ...
    (defined(EA_PLATFORM_ANDROID) && \
     !(defined(EA_ANDROID_SDK_LEVEL) && EA_ANDROID_SDK_LEVEL >= 21))
...
typedef double double_t;
  ...
  // The CodeSourcery definitions of PRIxPTR and SCNxPTR are broken for 32 bit systems.
    ...
    (defined(__have_long64) || defined(__have_longlong64))
    #undef PRIdPTR
    #define PRIdPTR "d"
    #undef PRIiPTR
    #define PRIiPTR "i"
...
// ------------------------------------------------------------------------
  ...
  // VC++ 7.0 and earlier don't handle the LL suffix.
      ...
      #define INT8_C(x) \
        int8_t(         \
          x) // x##i8 doesn't work satisfactorilly because -128i8 generates an out of range warning.
    #endif
    #ifndef UINT8_C
      ...
      #define INT16_C(x) \
        int16_t(         \
          x) // x##i16 doesn't work satisfactorilly because -32768i8 generates an out of range warning.
    #endif
    #ifndef UINT16_C
      ...
      #define INT8_C(x) \
        int8_t(         \
          x) // For the majority of compilers and platforms, long is 32 bits and long long is 64 bits.
    #endif
    #ifndef UINT8_C
      #define UINT8_C(x) uint8_t(x)
    #endif
      ...
      #define UINT16_C(x) \
        uint16_t(         \
          x) // Possibly we should make this be uint16_t(x##u). Let's see how compilers react before changing this.
    #endif
    #ifndef INT32_C
    ...
    #ifndef INT64_C
      #define INT64_C(x) \
        x##LL // The way to deal with this is to compare ULONG_MAX to 0xffffffff and if not equal, then remove the L.
    #endif
    #ifndef UINT64_C
...
typedef double double_t;
  ...
  #if defined(EA_COMPILER_MSVC) || \
    defined(                       \
      EA_COMPILER_BORLAND) // VC++ 7.1+ understands long long as a data type but doesn't accept %ll as a printf specifier.
  ...
  // Printf format specifiers
    ...
    #define PRId8 \
      "c" // This may not work properly but it at least will not crash. Try using 16 bit versions instead.
    #define PRIi8 "c" //  "
    #define PRIo8 "o" //  "
    #define PRIu8 "u" //  "
    #define PRIx8 "x" //  "
  ...
  #define PRId32 \
    "d" // This works for both 32 bit and 64 bit systems, as we assume LP64 conventions.
  #define PRIi32 "i"
  #define PRIo32 "o"
  #define PRIu32 "u"
  #define PRIx32 "x"
  ...
  #define PRIX64 EA_PRI_64_LENGTH_SPECIFIER "X"
    ...
    #define PRIdPTR \
      PRId32 // Usage of pointer values will generate warnings with
    #define PRIiPTR \
      PRIi32 // some compilers because they are defined in terms of
    #define PRIoPTR \
      PRIo32 // integers. However, you can't simply use "p" because
    #define PRIuPTR \
      PRIu32 // 'p' is interpreted in a specific and often different
  ...
  #else // VC++, Borland, etc. which have no way to specify 8 bit values other than %c.
    #define SCNd8 \
      "c" // This will not work properly but it at least will not crash. Try using 16 bit versions instead.
    #define SCNi8 "c" //  "
    #define SCNo8 "c" //  "
    #define SCNu8 "c" //  "
    #define SCNx8 "c" //  "
  ...
  #define SCNx64 EA_SCN_64_LENGTH_SPECIFIER "x"
    ...
    #define SCNdPTR \
      SCNd32 // Usage of pointer values will generate warnings with
    #define SCNiPTR \
      SCNi32 // some compilers because they are defined in terms of
    #define SCNoPTR \
      SCNo32 // integers. However, you can't simply use "p" because
    #define SCNuPTR \
      SCNu32 // 'p' is interpreted in a specific and often different
...
//
  ...
  #if defined(EA_COMPILER_MSVC) ||                                  \
    (defined(EA_COMPILER_INTEL) && defined(EA_PLATFORM_WINDOWS)) || \
    defined(EA_COMPILER_BORLAND)
    #if defined(__cplusplus)
...
//
  ...
  !defined(EA_COMPILER_HAS_C99_TYPES)
  #if (EA_PLATFORM_PTR_SIZE == 4)
...
#if !defined(_UINTPTR_T_DEFINED) && !defined(_uintptr_t_defined) && \
  !defined(EA_COMPILER_HAS_C99_TYPES)
  #if (EA_PLATFORM_PTR_SIZE == 4)
...
// takes a unicode string.
  ...
  // To do: Change this to be based on EA_COMPILER_NO_NEW_CHARACTER_TYPES.
    ...
    _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
    #define EA_CHAR16_NATIVE 1
  #elif defined(EA_COMPILER_CLANG) && defined(EA_COMPILER_CPP11_ENABLED)
    #if __has_feature(cxx_unicode_literals)
      #define EA_CHAR16_NATIVE 1
    #elif (EA_COMPILER_VERSION >= 300) && \
      !(defined(EA_PLATFORM_IPHONE) || defined(EA_PLATFORM_OSX))
      #define EA_CHAR16_NATIVE 1
    ...
    #endif
  #elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && \
    defined(__CHAR16_TYPE__) &&                                 \
    defined(EA_COMPILER_CPP11_ENABLED) // EDG 4.4+.
    #define EA_CHAR16_NATIVE 1
  #elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004) && \
    !defined(EA_COMPILER_EDG) &&                                      \
    (defined(EA_COMPILER_CPP11_ENABLED) ||                            \
     defined(                                                         \
       __STDC_VERSION__)) // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
    #define EA_CHAR16_NATIVE 1
  #else
    #define EA_CHAR16_NATIVE 0
  #endif
...
#if !defined( \
  EA_CHAR32_NATIVE) // Microsoft currently ties char32_t language support to char16_t language support. So we use CHAR16_T here.
  // To do: Change this to be based on EA_COMPILER_NO_NEW_CHARACTER_TYPES.
  #if defined(_MSC_VER) && (_MSC_VER >= 1600) && \
    defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) &&   \
    _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
...
typedef char char8_t;
    ...
    // In GCC C, __CHAR16_TYPE__ and __CHAR32_TYPE__ are defined instead, and we must define char16_t and char32_t from these.
      ...
      defined(__CHAR16_TYPE__) // If using GCC and compiling in C...
...
typedef __CHAR32_TYPE__ char32_t;
      ...
      defined(_MSC_VER) && \
      (_MSC_VER >=         \
       1600)) // if VS2010+ or using platforms that use Dinkumware under a compiler that doesn't natively support C++11 char16_t.
      #if !defined(_CHAR16T)
        #define _CHAR16T
      #endif
...
#define EA_LIMITS_DIGITS(T) \
  ((EA_LIMITS_IS_SIGNED(T) ? EA_LIMITS_DIGITS_S(T) : EA_LIMITS_DIGITS_U(T)))
#define EA_LIMITS_IS_SIGNED(T) ((T)(-1) < 0)
#define EA_LIMITS_MIN_S(T) ((T)((T)1 << EA_LIMITS_DIGITS_S(T)))
#define EA_LIMITS_MIN_U(T) ((T)0)
#define EA_LIMITS_MIN(T) \
  ((EA_LIMITS_IS_SIGNED(T) ? EA_LIMITS_MIN_S(T) : EA_LIMITS_MIN_U(T)))
#define EA_LIMITS_MAX_S(T) \
  ((T)(((((T)1 << (EA_LIMITS_DIGITS(T) - 1)) - 1) << 1) + 1))
#define EA_LIMITS_MAX_U(T) ((T) ~(T)0)
#define EA_LIMITS_MAX(T) \
  ((EA_LIMITS_IS_SIGNED(T) ? EA_LIMITS_MAX_S(T) : EA_LIMITS_MAX_U(T)))
...
#ifndef EA_CHAR16
  #if EA_CHAR16_NATIVE && \
    !defined(_MSC_VER) // Microsoft doesn't support char16_t string literals.
    #define EA_CHAR16_(s) u##s
    #define EA_CHAR16(s) EA_CHAR16_(s)
  #elif (EA_WCHAR_SIZE == 2)
    #if defined(_MSC_VER) && (_MSC_VER >= 1900) && \
      defined(__cplusplus) // VS2015 supports u"" string literals.
      #define EA_CHAR16_(s) u##s
      #define EA_CHAR16(s) EA_CHAR16_(s)
    #else
...
#ifndef EA_CHAR32
  #if EA_CHAR32_NATIVE && \
    !defined(_MSC_VER) // Microsoft doesn't support char32_t string literals.
...
//
  ...
  #define EA_STATIC_ASSERT_CONCATENATE_HELPER(a, b) \
    EA_STATIC_ASSERT_TOKEN_PASTE(a, b)

  #if defined( \
    __COUNTER__) // If this extension is available, which allows multiple statements per line...
    #define static_assert(expression, description)      \
      typedef char EA_STATIC_ASSERT_CONCATENATE_HELPER( \
        compileTimeAssert,                              \
        __COUNTER__)[((expression) != 0)                \
                       ? 1                              \
                       : -1] EA_STATIC_ASSERT_UNUSED_ATTRIBUTE
  #else
    #define static_assert(expression, description)      \
      typedef char EA_STATIC_ASSERT_CONCATENATE_HELPER( \
        compileTimeAssert,                              \
        __LINE__)[((expression) != 0) ? 1               \
                                      : -1] EA_STATIC_ASSERT_UNUSED_ATTRIBUTE
  #endif
...
#define EA_IS_ENABLED(x) \
  (333 ==                \
   333 * 111 / ((x 0) * (((x 0) == 333 ? 1 : 0) + ((x 0) == 111 ? 1 : 0))))
