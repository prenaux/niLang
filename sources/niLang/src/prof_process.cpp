...
void Prof_update(int record_data)
{
  ...
  for (i = 0; i < NUM_PROFILE_TRACKER_HISTORY_SLOTS; i++) {
    precomputed_factors[i] = ni::Pow(0.1f, (float)(dt/times_to_reach_90_percent[i]));
  }
  ...
  if (update_index < NUM_THROWAWAY_UPDATES) {
    ...
    update(&int_timestamps_per_second, timestamps_per_second, precomputed_factors);
  }

  {
    ...
    ss_variance = int_timestamps_per_second.variances[ss_slot] - ss_val*ss_val;
...
static Prof_Report *allocate_buffer(int n)
{
  ...
  pob->num_record = n;
  pob->record = (Prof_Report_Record *)niMalloc(sizeof(*pob->record) * pob->num_record);
...
Prof_Report *Prof_create_report(void)
{
  ...
  if (speedstep_warning)
    pob->title[1] = ni::StrDupModule("WARNING: SpeedStep-like timer inconsistencies detected.  Results are unreliable.");
...
void Prof_graph(int num_frames, void (*callback)(int id, int x0, int x1, float *values, void *data), void *data)
{
  ...
  if (display_frame != 0) {
    float value[2] = { 2.0, 0 };
    callback(0, NUM_FRAME_SLOTS-display_frame, NUM_FRAME_SLOTS-display_frame, value, data);
  }
#endif
}
