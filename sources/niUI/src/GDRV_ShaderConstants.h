...
class cShaderConstants
 ...
 public:
  ...
  virtual tU32 __stdcall AddConstant(iHString* ahspName,
                                     eShaderRegisterType aType, tU32 anSize)
  {
    niCheckSilent(aType == eShaderRegisterType_Sampler ||
    ...
    if (aType == eShaderRegisterType_Sampler) {
      ...
      niCheckSilent(mvFloatRegisters.size() + mvIntRegisters.size() + anSize <=
                      mnMaxRegisters,
                    eInvalidHandle);
  ...
  virtual tBool __stdcall SetFloatArray(tU32 anConstIndex,
                                        const tVec4fCVec* apV, tU32 anSize = 1)
  {
  ...
  virtual sMatrixf __stdcall GetFloatMatrix(tU32 anConstIndex) const
  {
    ...
    const sConstant& c = mvConstants[anConstIndex];
    niCheckSilent(c.Type == eShaderRegisterType_ConstFloat,
                  sMatrixf::Identity());
  ...
  tSize __stdcall Serialize(iFile* apFile, eSerializeMode aMode)
  {
    ...
    case eSerializeMode_WriteRaw: {
      ...
      if (!mvFloatRegisters.empty()) {
        nSize += apFile->WriteF32Array(mvFloatRegisters[0].ptr(),
                                       (tU32)mvFloatRegisters.size() * 4);
      }
      nSize += apFile->WriteLE32((tU32)mvIntRegisters.size());
      if (!mvIntRegisters.empty()) {
        nSize += apFile->WriteLE32Array((tU32*)mvIntRegisters[0].ptr(),
                                        (tU32)mvIntRegisters.size() * 4);
      }
