...
#define IS_ZERO(ARG) \
  ((_sqtype(ARG) == OT_NULL) || (sq_isnumeric(ARG) && _int(ARG) == 0))
...
#define ARITH_OP(op, trg, _a1_, _a2_)                                         \
  {                                                                           \
    ...
    if (sq_isnumeric(o1) && sq_isnumeric(o2)) {                               \
      ...
      if (#op[0] == '+' &&                                                    \
          (_sqtype(ro1) == OT_STRING || _sqtype(ro2) == OT_STRING))           \
      {                                                                       \
        _GUARD(StringCat(ro1, ro2, trg));                                     \
      }                                                                       \
      else if (!ArithMetaMethod(#op[0], ro1, ro2, trg)) {                     \
        VM_ERRORT(niFmt("arith op '%c' on between '%s' and '%s'", #op[0],     \
  ...
  {                                                                            \
    ...
    if (sq_isnumeric(o1) && sq_isnumeric(o2)) {                                \
      ...
      if (#op[0] == '+' &&                                                     \
          (_sqtype(ro1) == OT_STRING || _sqtype(ro2) == OT_STRING))            \
      {                                                                        \
        if (!v->StringCat(ro1, ro2, trg))                                      \
          return false;                                                        \
      }                                                                        \
      else if (!v->ArithMetaMethod(#op[0], ro1, ro2, trg)) {                   \
        VM_ERRORB_(v,                                                          \
                   niFmt(_A("arith op '%c' on between '%s' and '%s'"), #op[0], \
                         _ss->GetTypeNameStr(ro1), _ss->GetTypeNameStr(ro2))); \
      }                                                                        \
    }                                                                          \
  }
...
#define BW_OP_EX(op, trg, _a1_, _a2_)                                     \
  {                                                                       \
    ...
    {                                                                     \
      ...
      VM_ERRORT(niFmt(_A("bitwise op between '%s' and '%s'"),             \
...
#define EQ_OP(v, TEST)                                \
  {                                                   \
    int r = 0;                                        \
    _GUARDV(v, v->ObjEq(STK(arg2), COND_LITERAL, r)); \
    TARGET = ((TEST) ? _notnull_ : _null_);           \
  }
#define CMP_OP(v, TEST)                                \
  {                                                    \
    int r = 0;                                         \
    _GUARDV(v, v->ObjCmp(STK(arg2), COND_LITERAL, r)); \
    TARGET = ((TEST) ? _notnull_ : _null_);            \
  }
...
#define EX_LOOP_INIT() _i_ = _ci->_ip++;
...
///////////////////////////////////////////////
bool SQVM::Execute(const SQObjectPtr& aClosure, int target, int nargs,
                   int stackbase, SQObjectPtr& outres)
{
  ...
  try {
...

    {
      ...
      _ci->_root = true;
      if (_sqtype(_debughook) != OT_NULL &&
          _rawval(_debughook) != _rawval(_ci->_closurePtr))
        CallDebugHook(_A('c'));
    }
    ...
    //SQ_TRY
    {
      EX_LOOP()
      {
        ...
        EX_SWITCH()
        {
          ...
          EX_BEGIN(_OP_GETK)
          {
            if (!Get(STK(arg2), (*_ci->_literals)[arg1], temp_reg, NULL,
                     _i_->_ext))
            {
          ...
          {
            if (arg3 == 0xFF) {
              ARITH_OP(-, temp_reg, STK(arg1), STK(arg2));
              TARGET = temp_reg;
              if (arg1 != arg0)
                STK(arg1) = TARGET;
            }
              ...
              if ((_i_->_ext & _OPEXT_GET_SAFE) && tmp.IsNull()) {
                ...
                ARITH_OP(-, temp_reg, tmp, STK(arg3));
                TARGET = temp_reg;
          ...
          EX_BEGIN(_OP_PLUSEQ)
          {
            if (arg3 == 0xFF) {
              ...
              _GUARD(DerefInc(TARGET, STK(arg1), STK(arg2), STK(arg3), false,
                              _i_->_ext));
            }
          }
          EX_END_CONTINUE();
          ...
          {
            if (arg3 == 0xFF) {
              BW_OP_EX(<<, temp_reg, STK(arg1), STK(arg2));
              TARGET = temp_reg;
              if (arg1 != arg0)
                STK(arg1) = TARGET;
            }
              ...
              if ((_i_->_ext & _OPEXT_GET_SAFE) && tmp.IsNull()) {
                ...
                BW_OP_EX(<<, temp_reg, tmp, STK(arg3));
                TARGET = temp_reg;
                if (!Set(tself, tkey, TARGET, _i_->_ext)) {
                  Raise_IdxError(tkey);
                  SQ_THROW();
          ...
          EX_BEGIN(_OP_INC)
          {
            if (arg3 == 0xFF) {
              ...
              _GUARD(DerefInc(TARGET, STK(arg1), STK(arg2), _one_, false,
                              _i_->_ext));
            }
          }
          EX_END_CONTINUE();
          ...
          EX_BEGIN(_OP_PINC)
          {
            if (arg3 == 0xFF) {
              ...
              _GUARD(
                DerefInc(TARGET, STK(arg1), STK(arg2), _one_, true, _i_->_ext));
            }
          }
          EX_END_CONTINUE();
          ...
          EX_BEGIN(_OP_DEC)
          {
            if (arg3 == 0xFF) {
              ...
              _GUARD(DerefInc(TARGET, STK(arg1), STK(arg2), _minusone_, false,
                              _i_->_ext));
            }
          }
          EX_END_CONTINUE();
          ...
          EX_BEGIN(_OP_PDEC)
          {
            if (arg3 == 0xFF) {
              ...
              _GUARD(DerefInc(TARGET, STK(arg1), STK(arg2), _minusone_, true,
                              _i_->_ext));
            }
          }
          EX_END_CONTINUE();
...
common_call:
            ...
            case OT_CLOSURE: {
              {
                _GUARD(StartCall(_closure(temp_reg), ct_target, arg3,
                                 ct_tailcall ? _stackbase : _stackbase + arg2,
                                 ct_tailcall));
              }
              if (_sqtype(_debughook) != OT_NULL &&
                  _rawval(_debughook) != _rawval(_ci->_closurePtr))
            ...
            case OT_NATIVECLOSURE: {
              _GUARD(CallNative(_nativeclosure(temp_reg), arg3,
                                _stackbase + arg2, temp_reg))
            ...
            case OT_TABLE: {
              if (_table(temp_reg)->CanCallMetaMethod()) {
                ...
                for (int i = 0; i < arg3; i++)
                  Push(STK(arg2 + i));
                if (CallMetaMethod(_table(temp_reg), MT_STD_CALL, arg3 + 1,
                                   temp_reg))
                {
                  STK(ct_target) = temp_reg;
                  break;
              ...
              VM_ERRORT(niFmt(_A("attempt to call '%s'"),
                              _ss->GetTypeNameStr(temp_reg)));
            }
            case OT_USERDATA: {
              if (_userdata(temp_reg)->GetType() == eScriptType_MethodDef) {
                _GUARD(CallMethodDef((sScriptTypeMethodDef*)_userdata(temp_reg),
            ...
            case OT_IUNKNOWN: {
              ...
              if (CallIUnknownMetaMethod(_iunknown(temp_reg), MT_STD_CALL, arg3,
                                         temp_reg))
              {
                STK(ct_target) = temp_reg;
                break;
          ...
          EX_BEGIN(_OP_EXISTS)
          {
            TARGET = DoGet(STK(arg1), STK(arg2), temp_reg, NULL,
                           _OPEXT_GET_RAW | _OPEXT_GET_SAFE)
                       ? _notnull_
                       : _null_;
          }
          EX_END_CONTINUE();
          ...
          EX_BEGIN(_OP_NEG)
          {
            ...
            switch (_sqtype(o)) {
            case OT_INTEGER: TARGET = SQInt(-_int(o)); break;
            case OT_FLOAT: TARGET = SQFloat(-_float(o)); break;
            case OT_TABLE: {
            ...
            case OT_USERDATA:
              if (_userdata(o)->GetDelegate()) {
                Push(o);
                if (CallMetaMethod(_userdata(o)->GetDelegate(), MT_ARITH_UNM, 1,
                                   temp_reg))
                {
                  TARGET = temp_reg;
                  break;
            ...
            case OT_IUNKNOWN:
              Push(o);
              if (CallIUnknownMetaMethod(_iunknown(o), MT_ARITH_UNM, 1,
                                         temp_reg))
              {
                TARGET = temp_reg;
                break;
              ...
              break;
            default:
              VM_ERRORT(niFmt(_A("attempt to negate a %s"),
          ...
          EX_BEGIN(_OP_CLOSURE)
          {
            SQFunctionProto* func =
              _funcproto(_funcproto(_closure(_ci->_closurePtr)->_function)
                           ->_functions[arg1]);
              ...
              if (nouters) {
                ...
                for (int i = 0; i < nouters; i++) {
                  ...
                  if (!v._blocal) { //environment object
                    ...
                    if (!Get(STK(0), v._src, closure->_outervalues.back(),
                             nullptr, 0))
                    {
...
#define oVal STK(arg2 + 1)
                ...
                if (Table_GetMetaMethod(closure, _table(oThis), MT_STD_NEXTI)) {
                  ...
                  if (CallMetaMethod(closure, 2, itr)) {
                    ...
                    if (_sqtype(itr) != OT_ARRAY || _array(itr)->Size() < 2) {
                      VM_ERRORT(niFmt(_A(
                        "_nexti returned an invalid iterator, should be an array with at least two elements [key,value]")));
                    }
                    oRefPos = itr;
            ...
            case OT_IUNKNOWN: {
              SQObjectPtr itr;
              if (!iunknown_nexti(this, _iunknown(oThis), oRefPos, oKey, oVal,
                                  itr))
              {
                break;
              }
            ...
            case OT_USERDATA:
              if (_userdata(oThis)->GetDelegate()) {
                ...
                Push(oThis);
                Push(oRefPos);
                if (CallMetaMethod(_userdata(oThis)->GetDelegate(),
                                   MT_STD_NEXTI, 2, itr))
                {
                  oRefPos = oKey = itr;
                  if (_sqtype(itr) == OT_NULL)
            ...
            default:
              VM_ERRORT(
                niFmt(_A("cannot iterate %s"), _ss->GetTypeNameStr(STK(arg0))));
            }
            _ci->_ip += sarg1;
          }
          EX_END_CONTINUE();
          ...
          {
            _etraps.push_back(SQExceptionTrap(
              _top, _stackbase,
              &(*_ci->_iv)[(_ci->_ip - (&(*_ci->_iv)[0])) + arg1], arg0));
            traps++;
            _ci->_etraps++;
          }
          EX_END_CONTINUE();
  ...
  if (_ci) {
    ...
    if (_ci) {
      do {
        ...
        pop_callinfo(this);
        if ((_ci && _sqtype(_ci->_closurePtr) != OT_CLOSURE) ||
            exitafterthisone)
          break;
      } while (_callsstack.size());
    }
...
bool SQVM::DerefInc(SQObjectPtr& target, SQObjectPtr& self, SQObjectPtr& key,
...
bool SQVM::Call(SQObjectPtr& closure, int nparams, int stackbase,
                SQObjectPtr& outres)
{
