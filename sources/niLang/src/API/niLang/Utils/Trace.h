...
#define niTraceFmt(FMT)                                            \
  {                                                                \
    ...
    ni::ni_log(ni::eLogFlags_Debug, traceFmt.Chars(), niTraceFile, \
               niTraceLine, NULL);                                 \
  }

#define niTraceFmtIf(COND, FMT) \
  if (COND) {                   \
    niTraceFmt(FMT);            \
  }
...
#define niModuleShouldTrace_(MODULE, TRACENAME) \
  niModuleTraceObject_(MODULE, TRACENAME).get()
#define niModuleShouldTrace(MODULE) niModuleTraceObject(MODULE).get()

#define niModuleTrace_(MODULE, TRACENAME, FMT) \
  niTraceFmtIf(niModuleTraceObject_(MODULE, TRACENAME).get(), FMT)
#define niModuleTrace(MODULE, FMT) niModuleTrace_(MODULE, Trace, FMT)

#define niDeclareModuleProperty(MODULE, TRACENAME, DEFAULT)         \
  static ni::sPropertyBool niModuleTraceObject_(MODULE, TRACENAME)( \
    niModuleTraceName_(MODULE, TRACENAME), DEFAULT)
#define niDeclareModuleTrace_(MODULE, TRACENAME) \
  niDeclareModuleProperty(MODULE, TRACENAME, ni::eFalse)
