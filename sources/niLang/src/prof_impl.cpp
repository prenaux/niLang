...
class cProfiler : public ImplLocal<ni::iProf> {
 ...
 public:
  ...
  virtual ni::sProfilerZoneStack* __stdcall StackAppend(
    ni::sProfilerZone* apZoneStack)
  {
  ...
  virtual tBool __stdcall DrawTable(ni::iProfDraw* drawer, tF32 sx, tF32 sy,
                                    tF32 full_width, tF32 height,
                                    tI32 precision)
  {
    niCheckIsOK(drawer, eFalse);
    Prof_draw(drawer, sx, sy, full_width, height, precision);
    return eTrue;
  }

  virtual tBool __stdcall DrawGraph(ni::iProfDraw* drawer, tF32 sx, tF32 sy,
  ...
  virtual cString __stdcall TextReport(tU32 cols, tU32 rows)
  {
    ...
    struct _Local {
      static void printText(void* c, float x, float y, const char* str,
    ...
    Prof_draw_text((void*)&text, 0, 0, (float)cols, (float)rows, 1, 5, NULL,
                   NULL, NULL, NULL, NULL, _Local::printText,
                   _Local::textWidth);
