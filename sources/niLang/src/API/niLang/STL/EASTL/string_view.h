...
class basic_string_view {
 ...
 public:
  ...
  EA_CONSTEXPR basic_string_view(const basic_string_view& other)
    EA_NOEXCEPT = default;
  ...
  EA_CONSTEXPR basic_string_view(const T* s)
      : mpBegin(s)
      , mnCount(s != nullptr ? CharStrlen(s) : 0)
  {
  }
  ...
  {
    return const_reverse_iterator(mpBegin + mnCount);
  }
  ...
  {
    return [&] {
      EASTL_ASSERT_MSG(!empty(),
                       "behavior is undefined if string_view is empty");
    }(),
           mpBegin[0];
  }
  ...
  EA_CPP14_CONSTEXPR void remove_prefix(size_type n)
  {
    EASTL_ASSERT_MSG(
      n <= mnCount,
      "behavior is undefined if moving past the end of the string");
    mpBegin += n;
    mnCount -= n;
  }
  ...
  EA_CPP14_CONSTEXPR void remove_suffix(size_type n)
  {
    EASTL_ASSERT_MSG(
      n <= mnCount,
      "behavior is undefined if moving past the beginning of the string");
    mnCount -= n;
  }
  ...
  size_type copy(T* pDestination, size_type count, size_type pos = 0) const
  {
    ...
    auto* pResult = CharStringUninitializedCopy(
      mpBegin + pos, mpBegin + pos + count, pDestination);
    // *pResult = 0; // don't write the null-terminator
    return pResult - pDestination;
  }

  EA_CPP14_CONSTEXPR basic_string_view substr(size_type pos = 0,
  ...
  static EA_CPP14_CONSTEXPR int compare(const T* pBegin1, const T* pEnd1,
                                        const T* pBegin2, const T* pEnd2)
  {
  ...
  {
    return compare(mpBegin, mpBegin + mnCount, sw.mpBegin,
                   sw.mpBegin + sw.mnCount);
  }

  EA_CONSTEXPR int compare(size_type pos1, size_type count1,
                           basic_string_view sw) const
  {
    return substr(pos1, count1).compare(sw);
  }

  EA_CONSTEXPR int compare(size_type pos1, size_type count1,
                           basic_string_view sw, size_type pos2,
  ...
  EA_CONSTEXPR int compare(size_type pos1, size_type count1, const T* s,
                           size_type count2) const
  {
  ...
  EA_CPP14_CONSTEXPR size_type find(basic_string_view sw,
                                    size_type pos = 0) const EA_NOEXCEPT
  {
    auto* pEnd = mpBegin + mnCount;
    if (EASTL_LIKELY(((npos - sw.size()) >= pos) &&
                     (pos + sw.size()) <= mnCount))
    {
      const value_type* const pTemp =
        eastl::search(mpBegin + pos, pEnd, sw.data(), sw.data() + sw.size());
  ...
  EA_CONSTEXPR size_type rfind(basic_string_view sw,
                               size_type pos = npos) const EA_NOEXCEPT
  {
    return rfind(sw.mpBegin, pos, sw.mnCount);
  }

  EA_CPP14_CONSTEXPR size_type rfind(T c,
                                     size_type pos = npos) const EA_NOEXCEPT
  {
    if (EASTL_LIKELY(mnCount)) {
      const value_type* const pEnd =
        mpBegin + eastl::min_alt(mnCount - 1, pos) + 1;
  ...
  EA_CPP14_CONSTEXPR size_type rfind(const T* s, size_type pos,
                                     size_type n) const
  {
    ...
    if (EASTL_LIKELY(n <= mnCount)) {
      if (EASTL_LIKELY(n)) {
        const const_iterator pEnd =
          mpBegin + eastl::min_alt(mnCount - n, pos) + n;
        const const_iterator pResult =
          CharTypeStringRSearch(mpBegin, pEnd, s, s + n);
  ...
  EA_CONSTEXPR size_type find_first_of(basic_string_view sw,
                                       size_type pos = 0) const EA_NOEXCEPT
  {
  ...
  EA_CPP14_CONSTEXPR size_type find_first_of(const T* s, size_type pos,
                                             size_type n) const
  {
    ...
    if (EASTL_LIKELY((pos < mnCount))) {
      ...
      const const_iterator pResult =
        CharTypeStringFindFirstOf(pBegin, pEnd, s, s + n);
  ...
  EA_CONSTEXPR size_type find_last_of(basic_string_view sw,
                                      size_type pos = npos) const EA_NOEXCEPT
  {
    return find_last_of(sw.mpBegin, pos, sw.mnCount);
  }

  EA_CONSTEXPR size_type find_last_of(T c,
                                      size_type pos = npos) const EA_NOEXCEPT
  {
    return rfind(c, pos);
  }

  EA_CPP14_CONSTEXPR size_type find_last_of(const T* s, size_type pos,
                                            size_type n) const
  {
    ...
    if (EASTL_LIKELY(mnCount)) {
      ...
      const value_type* const pResult =
        CharTypeStringRFindFirstOf(pEnd, mpBegin, s, s + n);
  ...
  EA_CONSTEXPR size_type find_first_not_of(basic_string_view sw,
                                           size_type pos = 0) const EA_NOEXCEPT
  {
    return find_first_not_of(sw.mpBegin, pos, sw.mnCount);
  }

  EA_CPP14_CONSTEXPR size_type
  find_first_not_of(T c, size_type pos = 0) const EA_NOEXCEPT
  {
    if (EASTL_LIKELY(pos <= mnCount)) {
      const auto pEnd = mpBegin + mnCount;
      // Todo: Possibly make a specialized version of CharTypeStringFindFirstNotOf(pBegin, pEnd, c).
      const const_iterator pResult =
        CharTypeStringFindFirstNotOf(mpBegin + pos, pEnd, &c, &c + 1);
  ...
  EA_CPP14_CONSTEXPR size_type find_first_not_of(const T* s, size_type pos,
                                                 size_type n) const
  {
    if (EASTL_LIKELY(pos <= mnCount)) {
      const auto pEnd = mpBegin + mnCount;
      const const_iterator pResult =
        CharTypeStringFindFirstNotOf(mpBegin + pos, pEnd, s, s + n);
  ...
  EA_CONSTEXPR size_type
  find_last_not_of(basic_string_view sw, size_type pos = npos) const EA_NOEXCEPT
  {
    return find_last_not_of(sw.mpBegin, pos, sw.mnCount);
  }

  EA_CPP14_CONSTEXPR size_type
  find_last_not_of(T c, size_type pos = npos) const EA_NOEXCEPT
  {
    if (EASTL_LIKELY(mnCount)) {
      ...
      const value_type* const pEnd =
        mpBegin + eastl::min_alt(mnCount - 1, pos) + 1;
      const value_type* const pResult =
        CharTypeStringRFindFirstNotOf(pEnd, mpBegin, &c, &c + 1);
  ...
  EA_CPP14_CONSTEXPR size_type find_last_not_of(const T* s, size_type pos,
                                                size_type n) const
  {
    if (EASTL_LIKELY(mnCount)) {
      const value_type* const pEnd =
        mpBegin + eastl::min_alt(mnCount - 1, pos) + 1;
      const value_type* const pResult =
        CharTypeStringRFindFirstNotOf(pEnd, mpBegin, s, s + n);
  ...
  EA_CONSTEXPR size_type find_last_not_of(const T* s,
...
inline EA_CONSTEXPR bool operator==(basic_string_view<CharT> lhs,
                                    basic_string_view<CharT> rhs) EA_NOEXCEPT
{
...
inline EA_CONSTEXPR bool operator==(
  type_identity_t<basic_string_view<CharT>> lhs,
  basic_string_view<CharT> rhs) EA_NOEXCEPT
{
  return (lhs.size() == rhs.size()) && (lhs.compare(rhs) == 0);
}
...
inline EA_CONSTEXPR bool operator==(
  ...
  type_identity_t<basic_string_view<CharT>> rhs) EA_NOEXCEPT
{
  return (lhs.size() == rhs.size()) && (lhs.compare(rhs) == 0);
}
...
inline EA_CONSTEXPR auto operator<=>(basic_string_view<CharT> lhs,
                                     basic_string_view<CharT> rhs) EA_NOEXCEPT
{
...
template <class CharT>
inline EA_CONSTEXPR auto operator<=>(
  basic_string_view<CharT> lhs,
  typename basic_string_view<CharT>::const_pointer rhs) EA_NOEXCEPT
{
...
// C++17 string types
typedef basic_string_view<char8_t>
  u8string_view; // C++20 feature, but always present for consistency.
...
EA_CONSTEXPR inline string_view operator"" sv(const char* str,
                                              size_t len) EA_NOEXCEPT
{
  return { str, len };
}
EA_CONSTEXPR inline u16string_view operator"" sv(const char16_t* str,
                                                 size_t len) EA_NOEXCEPT
{
  return { str, len };
}
EA_CONSTEXPR inline u32string_view operator"" sv(const char32_t* str,
                                                 size_t len) EA_NOEXCEPT
{
  return { str, len };
}
EA_CONSTEXPR inline wstring_view operator"" sv(const wchar_t* str,
                                               size_t len) EA_NOEXCEPT
{
  return { str, len };
}
...
EA_CONSTEXPR inline u16string_view operator"" _sv(const char16_t* str,
                                                  size_t len) EA_NOEXCEPT
{
  return { str, len };
}
EA_CONSTEXPR inline u32string_view operator"" _sv(const char32_t* str,
                                                  size_t len) EA_NOEXCEPT
{
  return { str, len };
}
EA_CONSTEXPR inline wstring_view operator"" _sv(const wchar_t* str,
                                                size_t len) EA_NOEXCEPT
{
  return { str, len };
}
EA_RESTORE_CLANG_WARNING() // -Wreserved-identifier
  ...
  #if EA_CHAR8_UNIQUE
EA_CONSTEXPR inline u8string_view operator"" sv(const char8_t* str,
                                                size_t len) EA_NOEXCEPT
{
  return { str, len };
}
EA_CONSTEXPR inline u8string_view operator"" _sv(const char8_t* str,
                                                 size_t len) EA_NOEXCEPT
{
  return { str, len };
}
