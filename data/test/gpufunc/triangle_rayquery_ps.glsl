...
TestGpuFuncs_PixelOutput TestGpuFuncs_triangle_rayquery_ps(lib_shader_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec3 _tmp_J = lib_shader_UIntToHashColor(uint(rayQueryGetIntersectionInstanceCustomIndexEXT(rayQuery,true)));
  ...
  if (_tmp_P) {
    ...
    bool _tmp_S = (intersectionType == lib_shader_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_S) {
