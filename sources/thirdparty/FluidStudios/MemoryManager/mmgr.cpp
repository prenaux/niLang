...
typedef ni::ThreadMutex MUTEX;

static inline ni::ThreadMutex& _get_allocMutex() {
  static ni::ThreadMutex _allocMutex;
  return _allocMutex;
}

#define MUTEX_LOCK(mutex) _get_##mutex().ThreadLock();
#define MUTEX_UNLOCK(mutex) _get_##mutex().ThreadUnlock();
...
static void log(
  const char *format,
  const ni::Var& a, const ni::Var& b = niVarNull, const ni::Var& c = niVarNull)
{
  niPrintln(niFmt(format,a,b,c));
}

static void log(
  const char *format,
  const ni::Var& a, const ni::Var& b, const ni::Var& c,
  const ni::Var& d, const ni::Var& e = niVarNull, const ni::Var& f = niVarNull,
  const ni::Var& g = niVarNull, const ni::Var& h = niVarNull, const ni::Var& i = niVarNull,
  const ni::Var& j = niVarNull, const ni::Var& k = niVarNull, const ni::Var& l = niVarNull)
{
  niPrintln(niFmt(format,a,b,c,d,e,f,g,h,i,j,k,l));
}

// ---------------------------------------------------------------------------------------------------------------------------------
void mmgrDumpMemoryReportHeader()
{
...
// ---------------------------------------------------------------------------------------------------------------------------------
void mmgrDumpMemoryAllocationReport()
{
	log("Alloc.        Addr           Size           Addr           Size                        BreakOn BreakOn");
	log("Number      Reported       Reported        Actual         Actual     Unused    Method  Dealloc Realloc  Allocated by");
	log("------ ------------------ ---------- ------------------ ---------- ---------- -------- ------- ------- ---------------------------------------------------");
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while (ptr)
		{
			log("% 6d 0x%016zX 0x%08zX 0x%016zX 0x%08zX 0x%08X %-8s    %c       %c    %s",
...
// ---------------------------------------------------------------------------------------------------------------------------------
void mmgrDumpMemoryLeakReport()
{
  // Header
  log(" ------------------------------------------------------------------------------");
  log("                Memory leak report for:  %s", ni::GetCurrentTime()->Format(NULL));
  // use LF instead of CRLF
  log(" ------------------------------------------------------------------------------");
  if (stats.totalAllocUnitCount)
  {
    log("%d memory leak%s found:\n", stats.totalAllocUnitCount, stats.totalAllocUnitCount == 1 ? "" : "s");
  }
  else
  {
    log("Congratulations! No memory leaks found!");
  ...
  if (stats.totalAllocUnitCount)
  {
    mmgrDumpMemoryAllocationReport();
  }
...
void	*mmgrAllocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, size_t alignment, size_t reportedSize)
{
  ...
  reportedSize &= ~(sizeof(uint32_t) - 1);
	...
	MUTEX_LOCK(allocMutex);
	{
		...
		if (sourceFile)
      ni::StrZCpy(au->sourceFile, sizeof(au->sourceFile) - 1, sourceFileStripper(sourceFile));
		else
      ni::StrZCpy(au->sourceFile, 2, "??");
		if (sourceFunc)
      ni::StrZCpy(au->sourceFunc, sizeof(au->sourceFunc) - 1, sourceFunc);
		else
      ni::StrZCpy(au->sourceFunc, 2, "??");
...
void	*mmgrReallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, size_t reportedSize, void *reportedAddress)
{
...

	{
		...
		void	*oldData = malloc(ni::Min(oldReportedSize, reportedSize));
		memcpy(oldData, oldReportedAddress, ni::Min(oldReportedSize, reportedSize));
		...
		{
			// Copy old data
			memcpy(au->reportedAddress, oldData, ni::Min(oldReportedSize, reportedSize));
			au->offset = offset;
		}
...
void mmgrDumpMemoryReport()
{
  // Header
  log(" ----------------------------------------------------------------------------------------------------------------------------------\n");
  log("|                                             Memory report for: %s                                          |\n", ni::GetCurrentTime()->Format(NULL));
  log(" ----------------------------------------------------------------------------------------------------------------------------------\n");
  log("\n");
  ...
  log(" ---------------------------------------------------------------------------------------------------------------------------------- \n");
  log("|                                                      A C C U M U L A T E D                                                       |\n");
  log(" ---------------------------------------------------------------------------------------------------------------------------------- \n");
  log("              Allocation unit count: %s\n", memorySizeString(stats.accumulatedAllocUnitCount));
  ...
  log("|                                                           U N U S E D                                                            |\n");
  log(" ---------------------------------------------------------------------------------------------------------------------------------- \n");
  log("Memory allocated but not in use: %s\n", memorySizeString(mmgrCalcAllUnused()));
  log("\n");
}
