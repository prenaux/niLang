...
void cEditBoxWidget::ComputeWidestLine()
{
  ...
  for (LineVec::const_iterator itr = lines.begin(); itr != lines.end(); ++itr) {
    ...
    TextLineMetric_AddText(&tlm, font, 0, 0, itr->GetChars(),
                           itr->GetSizeInBytes(), eFalse);
...
void cEditBoxWidget::AdjustChildrenLayout(tF32 w, tF32 h)
{
  if (mpWidget->GetStyle() & eWidgetEditBoxStyle_MultiLine) {
    ...
    mpsbScrollBarLeft->SetScrollRange(
      Vec2<tF32>(0, (tF32)(nLines - nLinePerPage + 1)));
...
void cEditBoxWidget::AutoScroll(tBool abForceRefresh)
{
  if (mpsbScrollBarLeft &&
      (mpWidget->GetStyle() & eWidgetEditBoxStyle_MultiLine))
  {
  ...
  const tF32 leftScrollBarWidth =
    mpLeftScrollbar ? mpLeftScrollbar->GetSize().x : 0.0f;
  const tF32 w = mpWidget->GetClientSize().x -
                 ((mpLeftScrollbar && mpLeftScrollbar->GetVisible())
                    ? mpLeftScrollbar->GetSize().x
                    : 0);
  ...
  sTextLineMetric tlm;
  TextLineMetric_Clear(&tlm);
  TextLineMetric_AddText(&tlm, font, 0, 0, line.GetChars(),
                         line.GetSizeInBytes(), eFalse);
  ...
  if (w > fullTxtW || curpos._col == 0) {
    ...
    if (abForceRefresh || curX < scrollMargin || curX > w - scrollMargin) {
      ...
      mfHScrollOffs =
        ni::Max(mfHScrollOffs, w - (fullTxtW + charWidth)) - leftScrollBarWidth;
    }
  }
...
tBool cEditBoxWidget::OnWidgetSink(iWidget* apWidget, tU32 nMsg,
                                   const Var& varParam0, const Var& varParam1)
{
  ...
  case eUIMessage_SinkAttached:
    ...
    AutoScroll();
    break;
  case eUIMessage_SkinChanged: InitSkin(); return eFalse;
  ...
  case eUIMessage_NCPaint: {
    ...
    if (c) {
      ...
      c->BlitOverlayFrame(mpWidget->GetWidgetRect(), frame,
                          eRectFrameFlags_All);
    }
  } break;
  ...
  case eUIMessage_SetFocus: {
    ...
    if (!(mpWidget->GetStyle() & eWidgetEditBoxStyle_NoSelect)) {
      if (mpWidget->GetStyle() & eWidgetEditBoxStyle_SelectAllOnSetFocus) {
        MoveCursorEnd(false);
        AutoScroll();
        mSelection = Selection(mbufText.GetStartPosition(),
                               mbufText.GetEndPosition(), stNormal);
        if (varParam0.mBool) {
  ...
  case eUIMessage_MouseMove:
    ...
    break;
  case eUIMessage_StyleChanged: UpdateStyle(); break;
  case eUIMessage_TextChanged: SetTextBuffer(mpWidget->GetText()); break;
...
void cEditBoxWidget::Paint(iCanvas* apCanvas)
{
  ...
  const tBool multiline =
    (mpWidget->GetStyle() & eWidgetEditBoxStyle_MultiLine) ? eTrue : eFalse;
  ...
  const tF32 rectWidth =
    rect.GetWidth() - ((mpLeftScrollbar && mpLeftScrollbar->GetVisible())
                         ? mpLeftScrollbar->GetSize().x
                         : 0);
  const tF32 rectHeight =
    rect.GetHeight() - ((mpBottomScrollbar && mpBottomScrollbar->GetVisible())
                          ? mpBottomScrollbar->GetSize().y
                          : 0);

  const tF32 fontSizeByRes =
    ni::FDiv(font->GetSize().x, (tF32)font->GetResolution());
  ...
  for (LineVec::const_iterator itr = lines.begin(); itr != lines.end(); ++itr) {
    if (curline >= firstvisibleline) {
      ...
      if (line.IsEmpty()) {
        ...
        for (tU32 ch = charIt.peek_next(), prevch = 0; !charIt.is_end();
             charIt.next(), ch = charIt.peek_next(), prevch = ch)
        {
          ...
          const tBool isSelected =
            (!mSelection.IsEmpty() && mSelection.IsIn(curline, col));
        ...
        if (bDrawSel) {
          apCanvas->BlitFillAlpha(sRectf(selStartX, y, selEndX - selStartX, h),
                                  backsel);
        }
        if (!mGlyphCache.empty()) {
          niLoop (i, mGlyphCache.size()) {
            mGlyphCache[i].gm.PushToCanvas(
              apCanvas, (cFont*)font, mGlyphCache[i].color, sVec3f::Zero());
          }
        }
      }

      y += h;
      if (bDrawCursor && cursor._line == curline && cursorx >= 0 &&
          cursorx <= rectWidth)
      {
        cursory = y;
      }
  ...
  if (cursory >= 0) {
    apCanvas->BlitFill(
      sRectf(Vec2(cursorx - 1.0f, cursory - h), Vec2(cursorx + 1.0f, cursory)),
      frontnormal);
  }
}
...
void cEditBoxWidget::MoveCursor(const sVec2i& pos, tBool forcelogicalcol)
{
  ...
  Position realnewpos =
    mbufText.SetCursorPos((Position&)pos, forcelogicalcol ? true : false);
  if (niFlagIsNot(mpWidget->GetStyle(), eWidgetEditBoxStyle_NoSelect) &&
      bSelection)
  { //select from cusor to new position
...
void __stdcall cEditBoxWidget::MoveCursorWordDelta(tI32 word)
{
  ...
  niLoop (i, count) {
    ...
    if (bPrev) {
      ...
      StrCharIt it(line.GetChars() + StrOffset(line.GetChars(), c),
                   line.GetSizeInBytes());
...
void __stdcall cEditBoxWidget::MoveCursorParaDelta(const tI32 aDelta)
{
  ...
  {
    Position closestPos =
      mbufText.GetClosestValidPos(mbufText.GetCursor(), aDelta);
...
bool cEditBoxWidget::OnKeyDown(tU32 key, tU32 mod, tU32 c)
{
  ...
  default: bInsertChar = eTrue; break;
  }

  if (bInsertChar) {
    const tU32 tabChar =
      niFlagIs(mpWidget->GetStyle(), eWidgetEditBoxStyle_CanInsertTabs) ? '\t'
                                                                        : ' ';
    // Insert char
    if (!niFlagIs(mpWidget->GetStyle(), eWidgetEditBoxStyle_ReadOnly) &&
        ((c >= 32) || (c == tabChar)))
...
void cEditBoxWidget::UpdateStyle()
{
  if (mpWidget->GetStyle() & eWidgetEditBoxStyle_MultiLine) {
    ...
    mpLeftScrollbar =
      pUICtx->CreateWidget(_A("ScrollBar"), mpWidget, sRectf(), 0);
    mpLeftScrollbar->SetVisible(eFalse);
    mpsbScrollBarLeft = ni::QueryInterface<iWidgetScrollBar>(mpLeftScrollbar);
    mpsbScrollBarLeft->SetScrollPosition(0);
    mpBottomScrollbar = pUICtx->CreateWidget(
      _A("ScrollBar"), mpWidget, sRectf(), eWidgetScrollBarStyle_Horz);
    mpBottomScrollbar->SetVisible(eFalse);
    mpsbScrollBarBottom =
      ni::QueryInterface<iWidgetScrollBar>(mpBottomScrollbar);
...
void cEditBoxWidget::InitSkin()
{
  ...
  skin.colFrontNormal = mpWidget->FindSkinColor(Vec4<tF32>(0, 0, 0, 1), NULL,
                                                NULL, _H("TextFrontNormal"));
  skin.colFrontSel = mpWidget->FindSkinColor(Vec4<tF32>(1, 1, 1, 1), NULL, NULL,
                                             _H("TextFrontSel"));
  skin.colBackSel = mpWidget->FindSkinColor(
    Vec4<tF32>(49.0f / 255.0f, 106.0f / 255.0f, 197.0f / 255.0f, 1.0), NULL,
    NULL, _H("TextBackSel"));
}
...
{
  if (apDT && niFlagIsNot(mpWidget->GetStyle(), eWidgetEditBoxStyle_NoSelect) &&
      !mSelection.IsEmpty())
  {
    cString strText;
...
{
  return Selection(Position(ni::Max(aRange.x, aClamp._start._line),
                            ni::Max(aRange.y, aClamp._start._col)),
                   Position(ni::Min(aRange.z, aClamp._end._line),
                            ni::Min(aRange.w, aClamp._end._col)),
                   stNormal);
}
Selection _ToSelection(const sVec4i& aRange)
{
  return Selection(Position(aRange.x, aRange.y), Position(aRange.z, aRange.y),
                   stNormal);
}
sVec4i _FromSelection(const Selection& aSel)
{
  return Vec4i(aSel._start._line, aSel._start._col, aSel._end._line,
               aSel._end._col);
}
...
{
  const Selection sel =
    _ToSelectionClamped(aRange, Selection(mbufText.GetStartPosition(),
                                          mbufText.GetEndPosition(), stNormal));
  cString strData;
...
{
  return _FromSelection(Selection(mbufText.GetStartPosition(),
                                  mbufText.GetEndPosition(), stNormal));
}
