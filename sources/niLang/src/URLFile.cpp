...
niExportFunc(iUnknown*) New_niLang_URLFileHandlerFileSystem(const Var& avarA,
                                                            const Var& avarB)
{
...
struct URLFileHandler_ManifestFileSystem : public URLFileHandler_Manifest {
  ...
  tBool _GetExistingPath(const achar* aURL, cString& aPath)
  {
...

    {
      ...
      TRACE_FILEHANDLER(
        ("... URLFileHandler_ManifestFileSystem.GetExisting: %s", testPath));
    ...
    TRACE_FILEHANDLER(
      ("... URLFileHandler_ManifestFileSystem.GetExisting.asis: %s", aPath));
  ...
  virtual iFile* __stdcall URLOpen(const achar* aURL)
  {
    ...
    if (!_GetExistingPath(aURL, path)) {
      TRACE_FILEHANDLER(
        ("... URLFileHandler_ManifestFileSystem.URLOpen.noexisting: %s", aURL));
      return NULL;
    }
    TRACE_FILEHANDLER(
      ("... URLFileHandler_ManifestFileSystem.URLOpen: %s", path));
    return _fileSystem->FileOpen(path.Chars(), eFileOpenMode_Read);
  }
...
niExportFunc(iUnknown*) New_niLang_URLFileHandlerManifestFileSystem(
...
niExportFunc(iUnknown*) New_niLang_URLFileHandlerURL(const Var& avarA,
                                                     const Var& avarB)
{
...
struct URLFileHandler_Prefixed : public ImplRC<iURLFileHandler> {
  ...
  URLFileHandler_Prefixed(iURLFileHandler* apURLFileHandler,
                          const achar* aPrefix)
  {
...
niExportFunc(iUnknown*) New_niLang_URLFileHandlerPrefixed(const Var& avarA,
                                                          const Var& avarB)
{
...
static void _RegisterDefaultURLFileHandler()
{
  ...
  const tBool hasDefaultHandler =
    (imap->find("URLFileHandler.default") != imap->end());
  if (!hasDefaultHandler) {
    Ptr<iURLFileHandler> defaultHandler =
      niNew URLFileHandler_ManifestFileSystem(ni::GetRootFS());
    astl::upsert(*imap, _ASTR("URLFileHandler.default"), defaultHandler);
  }
  const tBool hasFileHandler =
    (imap->find("URLFileHandler.file") != imap->end());
  if (!hasFileHandler) {
    Ptr<iURLFileHandler> fileHandler =
      niNew URLFileHandler_FileSystem(ni::GetRootFS());
...
static cString _TryPathAndBasePath(const cPath& aResPath,
                                   const cPath& aBasePath)
{
  cString ret;
...
{
  TRACE_FFP_TRYALLDIRS(
    ("... _TryPathAllDirs: %s, %s", aResPath.GetPath(), aBasePath.GetPath()));
...
cString __stdcall _URLFindFilePath(const achar* aszRes,
                                   const achar* aszBasePath,
                                   const cString* aLoaderPrefix)
{
  ...
  if (aLoaderPrefix && ext.empty()) {
    ...
    if (ret.empty()) {
      for (tGlobalInstanceCMap::const_iterator it =
             mapGI->lower_bound(*aLoaderPrefix);
           it != mapGI->end() && it->first.StartsWith(aLoaderPrefix->Chars());
           ++it)
      {
        cPath path(aszRes);
    ...
    if (ret.empty()) {
      TRACE_FFP_WARN(niFmt("Couldn't find directly '%s' (%s), trying harder...",
                           aszRes, aszBasePath));
  ...
  if (!ret.empty() && ret.size() >= 2 && (ret[0] == '/' || ret[1] == ':')) {
    ...
    TRACE_FFP_WARN(niFmt(
      "Found in an absolute path '%s' (%s), trying to find a relative path...",
      aszRes, aszBasePath));
    ret = _TryPathAllDirs(ret.Chars(), basePath);
    if (ret.empty() || (ret.size() >= 2 && (ret[0] == '/' || ret[1] == ':'))) {
      TRACE_FFP_WARN(niFmt(
        "Couldn't find a relative path for '%s' (%s), you're going to have a bad time, move the file asap.",
        aszRes, aszBasePath));
      ret = origAbsolutPath;
    }
  }
...
cString __stdcall cLang::URLFindFilePath(const achar* aszRes,
                                         const achar* aszBasePath,
                                         const achar* aLoaderPrefix)
{
