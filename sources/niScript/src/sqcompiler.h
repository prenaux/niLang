...
#define COMPILE_CHECK(r)     \
  {                          \
    eCompileResult r__ = r;  \
    if (COMPILE_FAILED(r__)) \
      return r__;            \
  }
...
struct sCompileErrors {
  ...
  {
    return _AddError(eCompileResult_CompilerError, aLineCol, aMsg).type;
  }
  eCompileResult LexerError(sVec2i aLineCol, const char* aMsg)
  {
    return _AddError(eCompileResult_LexerError, aLineCol, aMsg).type;
  }
  eCompileResult InternalError(sVec2i aLineCol, const char* aMsg)
  {
    return _AddError(eCompileResult_InternalError, aLineCol, aMsg).type;
  }
};
