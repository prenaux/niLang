...
class any {
  ...
  //
  union storage {
    typedef aligned_storage_t<4 * sizeof(void*), alignment_of<void*>::value>
      internal_storage_t;
  ...
  //
  template <typename T>
  using use_internal_storage =
    bool_constant<is_nothrow_move_constructible<T>::value &&
  ...
  friend const ValueType* any_cast(const any* pAny) EA_NOEXCEPT;
  template <class ValueType>
  friend ValueType* any_cast(any* pAny) EA_NOEXCEPT;
  template <class ValueType>
  ...
  template <class ValueType>
  friend const ValueType* unsafe_any_cast(const any* pAny) EA_NOEXCEPT;
  template <class ValueType>
  friend ValueType* unsafe_any_cast(any* pAny) EA_NOEXCEPT;
  ...
  struct storage_handler_internal {
    ...
    static void construct_inplace(storage& s, std::initializer_list<U> il,
                                  Args&&... args)
    {
    ...
    {
      T& t = *static_cast<T*>(
        static_cast<void*>(&refAny.m_storage.internal_storage));
      EA_UNUSED(t);
      t.~T();
    ...
    static void* handler_func(storage_operation op, const any* pThis,
                              any* pOther)
    {
      switch (op) {
  ...
  struct storage_handler_external {
    ...
    {
      s.external_storage =
        Internal::DefaultConstruct<T>(eastl::forward<Args>(args)...);
    }

    template <class NT, class U, class... Args>
    static inline void construct_inplace(storage& s,
                                         std::initializer_list<U> il,
                                         Args&&... args)
    {
      s.external_storage =
        Internal::DefaultConstruct<NT>(il, eastl::forward<Args>(args)...);
    }

    static inline void destroy(any& refAny)
    {
      Internal::DefaultDestroy(
        static_cast<T*>(refAny.m_storage.external_storage));
                              ...
                              any* pOther)
    {
      ...
      case storage_operation::COPY: {
        ...
        EASTL_ASSERT(pOther);
        construct(pOther->m_storage,
                  *static_cast<T*>(pThis->m_storage.external_storage));
      } break;
  ...
  using storage_handler =
    typename conditional<use_internal_storage<T>::value,
 ...
 public:
  ...
  EA_CONSTEXPR
#endif
  any() EA_NOEXCEPT : m_storage(), m_handler(nullptr)
  {
  }
  ...
  template <class ValueType>
  any(ValueType&& value,
      typename eastl::enable_if<!eastl::is_same<
        typename eastl::decay<ValueType>::type, any>::value>::type* = 0)
  {
    typedef decay_t<ValueType> DecayedValueType;
    static_assert(is_copy_constructible<DecayedValueType>::value,
                  "ValueType must be copy-constructible");
    storage_handler<DecayedValueType>::construct(
      m_storage, eastl::forward<ValueType>(value));
  ...
  explicit any(in_place_type_t<T>, Args&&... args)
  {
    ...
    static_assert(eastl::is_constructible<T, Args...>::value,
                  "T must be constructible with Args...");

    StorageHandlerT::construct_inplace(m_storage,
      ...
      void>::type* = 0)
  {
    ...
    StorageHandlerT::construct_inplace(m_storage, il,
                                       eastl::forward<Args>(args)...);
    m_handler = &StorageHandlerT::handler_func;
  }
  ...
  {
    static_assert(is_copy_constructible<decay_t<ValueType>>::value,
                  "ValueType must be copy-constructible");
  ...
  template <class NT, class U, class... Args>
  typename eastl::enable_if<
    eastl::is_constructible<NT, std::initializer_list<U>&, Args...>::value,
    void>::type
...
{
  static_assert(eastl::is_reference<ValueType>::value ||
                  eastl::is_copy_constructible<ValueType>::value,
                "ValueType must be a reference or copy constructible");

  auto* p = any_cast<
    typename add_const<typename remove_reference<ValueType>::type>::type>(
    &operand);
...
inline const ValueType* any_cast(const any* pAny) EA_NOEXCEPT
{
  return (pAny &&
          pAny
            ->m_handler //== &any::storage_handler<decay_t<ValueType>>::handler_func
#if EASTL_RTTI_ENABLED
          && pAny->type() == typeid(typename remove_reference<ValueType>::type)
#endif
            )
           ? static_cast<const ValueType*>(
               pAny->m_handler(any::storage_operation::GET, pAny, nullptr))
           : nullptr;
}
