...
TEST_FIXTURE(FDTSerialize,Base64) {
  ...
  {
    Ptr<iDataTable> ptrDT = CreateDataTableFromXML(_dtXML.c_str(),_dtXML_Prop_with_space.size());
    CHECK(ptrDT.IsOK());
    ...
    CHECK_EQUAL(96, compressedString.size());
    CHECK_EQUAL(
        compressedString,
        _ASTR("QwAAAHjas3HNK8ksqVRIzkksLs5LzE21VXIpzc2tVFKAcFwrSpwTc/2LkjJLQhKL0lNLlOxsnBKLU/XtbPQhWu0ALckXkQ=="));
  }
  {
