...
struct is_functor_inplace_allocatable {
  static constexpr bool value =
    ...
    (eastl::alignment_of_v<functor_storage<SIZE_IN_BYTES>> %
     eastl::alignment_of_v<Functor>) == 0;
};
 ...
 public:
  ...
  class function_manager_base {
   public:
    ...
    static void CopyFunctor(FunctorStorageType& to,
                            const FunctorStorageType& from)
    {
      ::new (GetFunctorPtr(to)) Functor(*GetFunctorPtr(from));
    }

    static void MoveFunctor(FunctorStorageType& to,
                            FunctorStorageType& from) EA_NOEXCEPT
    {
      ::new (GetFunctorPtr(to)) Functor(eastl::move(*GetFunctorPtr(from)));
    }

    static void* Manager(void* to, void* from,
                         typename function_base_detail::ManagerOperations ops)
      EA_NOEXCEPT
    {
      switch (ops) {
  ...
  class function_manager_base<
    Functor, typename eastl::enable_if<!is_functor_inplace_allocatable<
               Functor, SIZE_IN_BYTES>::value>::type> {
   public:
    static Functor* GetFunctorPtr(const FunctorStorageType& storage) EA_NOEXCEPT
    {
      return storage.template GetStorageTypeRef<Functor*>();
    ...
    static Functor*& GetFunctorPtrRef(const FunctorStorageType& storage)
      EA_NOEXCEPT
    {
      return storage.template GetStorageTypeRef<Functor*>();
    }
   ...
   public:
    ...
    static void* GetTypeInfo() EA_NOEXCEPT
    {
      return reinterpret_cast<void*>(
        const_cast<std::type_info*>(&typeid(Functor)));
    }
      ...
      EA_NOEXCEPT
    {
      ...
      case MGROPS_GET_FUNC_PTR: {
        return static_cast<void*>(
          Base::GetFunctorPtr(*static_cast<FunctorStorageType*>(to)));
      } break;
      default: {
    ...
    {
      return eastl::invoke(*Base::GetFunctorPtr(functor),
...
#define EASTL_INTERNAL_FUNCTION_VALID_FUNCTION_ARGS(FUNCTOR, RET, ARGS, BASE, \
                                                    MYSELF)                   \
  typename eastl::enable_if<                                                  \
    eastl::is_invocable_r<RET, FUNCTOR, ARGS>::value &&                       \
    !eastl::is_base_of_v<BASE, eastl::decay_t<FUNCTOR>> &&                    \
    !eastl::is_same_v<eastl::decay_t<FUNCTOR>, MYSELF>>::type

#define EASTL_INTERNAL_FUNCTION_DETAIL_VALID_FUNCTION_ARGS(FUNCTOR, RET, ARGS, \
                                                           MYSELF)             \
  EASTL_INTERNAL_FUNCTION_VALID_FUNCTION_ARGS(FUNCTOR, RET, ARGS, MYSELF,      \
                                              MYSELF)
...
class function_detail<SIZE_IN_BYTES, R(Args...)>
    : public function_base_detail<SIZE_IN_BYTES> {
 public:
  using result_type = R;

 protected:
  using Base = function_base_detail<SIZE_IN_BYTES>;
  using FunctorStorageType =
    typename function_base_detail<SIZE_IN_BYTES>::FunctorStorageType;
  using Base::mStorage;
 ...
 public:
  ...
  template <typename Functor,
            typename = EASTL_INTERNAL_FUNCTION_DETAIL_VALID_FUNCTION_ARGS(
              Functor, R, Args..., function_detail)>
  ...
  void swap(function_detail& other) EA_NOEXCEPT
  {
    ...
    if (other.HaveManager()) {
      (void)(*other.mMgrFuncPtr)(static_cast<void*>(&tempStorage),
                                 static_cast<void*>(&other.mStorage),
                                 Base::ManagerOperations::MGROPS_MOVE_FUNCTOR);
    }

    if (HaveManager()) {
      (void)(*mMgrFuncPtr)(static_cast<void*>(&other.mStorage),
                           static_cast<void*>(&mStorage),
                           Base::ManagerOperations::MGROPS_MOVE_FUNCTOR);
    }

    if (other.HaveManager()) {
      (void)(*other.mMgrFuncPtr)(static_cast<void*>(&mStorage),
                                 static_cast<void*>(&tempStorage),
                                 Base::ManagerOperations::MGROPS_MOVE_FUNCTOR);
    }
  ...
  {
    if (HaveManager()) {
      void* ret = (*mMgrFuncPtr)(nullptr, nullptr,
                                 Base::ManagerOperations::MGROPS_GET_TYPE_INFO);
 ...
 private:
  ...
  void CreateForwardFunctor(Functor&& functor)
  {
    ...
    using FunctionManagerType =
      typename Base::template function_manager<DecayedFunctorType, R, Args...>;
    ...
    if (internal::is_null(functor)) {
      ...
      FunctionManagerType::CreateFunctor(mStorage,
                                         eastl::forward<Functor>(functor));
    }
  }

 private:
  typedef void* (*ManagerFuncPtr)(void*, void*,
