...
TEST_FIXTURE(FSinkList, Remove)
{
  ...
  auto getListMask2 = [&](tMySinkList* apLst) -> sVec4i {
    sVec4i has = { 0, 0, 0, 0 };
    niLoopit (tMySinkList::tImmutableCollection::const_iterator, it,
              *apLst->_ImmutableCollection())
    {
      ++has.w;
      if (*it == a) {
        niAssert(has.x == 0);
...
TEST_FIXTURE(FSinkList, Remove2)
{
  ...
  CHECK_EQUAL(2, a->mprotected_nNumRefs);
  {
    ...
    {
      Ptr<tMySinkList::sIterator> it =
        (tMySinkList::sIterator*)sinkLst->Find(a.ptr());
      niAssert(it.IsOK());
      it = nullptr;
