...
/*debug*/
niExportFunc(SQRESULT) sq_stackinfos(HSQUIRRELVM v, int level,
                                     SQStackInfos* si);
niExportFunc(void) sq_setdebughook(HSQUIRRELVM v);
  ...
  #define sq_isfuncproto(o) (sq_type(o) == OT_FUNCPROTO)
...
enum eScriptType {
  ...
  eScriptType_FunctionProto = OT_FUNCPROTO,
  eScriptType_IUnknown = OT_IUNKNOWN,
...
niExportFunc(int) sqa_getvar_astype(HSQUIRRELVM v, int idx, ni::Var* aVar,
                                    const ni::tType aType);
...
niExportFunc(int) sqa_pushMethodDef(HSQUIRRELVM v,
                                    ...
                                    const ni::sMethodDef* apMethDef);
niExportFunc(int) sqa_getMethodDef(HSQUIRRELVM v, int idx,
                                   const ni::sInterfaceDef** appInterfaceDef,
                                   const ni::sMethodDef** appMethDef);

niExportFunc(int) sqa_pushInterfaceDef(HSQUIRRELVM v,
                                       const ni::sInterfaceDef* apInterfaceDef);
niExportFunc(int) sqa_getInterfaceDef(
  HSQUIRRELVM v, int idx, const ni::sInterfaceDef** appInterfaceDef);
...
niExportFuncCPP(cString) sqa_gettypestring(int type);
    ...
    #define niSqUnGuardSimple(v)                                        \
      niAssertMsg(sq_gettop(v) == __sq_top,                             \
                  niFmt(_A("VM stack corrupted, expected %d, have %d"), \
                        __sq_top, sq_gettop(v)));
  #else
