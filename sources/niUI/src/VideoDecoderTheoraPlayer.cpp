...
class cVideoDecoderTheoraPlayer
    : public ni::ImplRC<ni::iVideoDecoder, ni::eImplFlags_Default> {
  niBeginClass(cVideoDecoderTheoraPlayer);

 public:
  ///////////////////////////////////////////////
  cVideoDecoderTheoraPlayer(iGraphics* apGraphics, iHString* ahspName,
                            iFile* apFile, tVideoDecoderFlags aFlags)
      : mFlags(aFlags)
      , mbUpdateOnBind(eTrue)
  {
    ...
    niExec_ (pLoadingExecutor, _this, _fp, withAlpha) {
      TheoraVideoClip* pTheoraVideoClip = Theora_createVideoClip(
        _fp, withAlpha ? VIDEO_OUTPUTMODE_ALPHA : VIDEO_OUTPUTMODE, 2, false);
      if (!pTheoraVideoClip) {
        niError(
          niFmt("Can't create the video clip '%s'.", _fp->GetSourcePath()));
        return ni::eFalse;
      }
      ...
      niExec (Main, _this, pTheoraVideoClip, _fp, withAlpha) {
        ...
        if (niFlagIs(_this->mFlags, eVideoDecoderFlags_TargetTexture)) {
          Ptr<iTexture> ptrTex = _this->mptrGraphics->CreateTexture(
            NULL, eBitmapType_2D, VIDEO_PIXELFORMAT, 0,
            pTheoraVideoClip->getWidth(), pTheoraVideoClip->getHeight(), 0,
            eTextureFlags_Dynamic);
          if (!ptrTex.IsOK()) {
            niError(niFmt(_A("Can't create target texture for video '%s'."),
                          _fp->GetSourcePath()));
            return ni::eFalse;
          }
        ...
        niLog(Info,
              niFmt("Video created: '%s' %s, %dx%d, %gsecs, %gfps\n",
                    _fp->GetSourcePath(), withAlpha ? "rgba" : "rgb",
                    pTheoraVideoClip->getWidth(), pTheoraVideoClip->getHeight(),
                    _this->GetLength(), _this->GetVideoFps()));
  ...
  ~cVideoDecoderTheoraPlayer()
  {
    ...
    if (mpTheoraVideoClip) {
      niLog(Info,
            niFmt("Destroying video clip %p", (tIntPtr)mpTheoraVideoClip));
  ...
  virtual tBool __stdcall Update(tBool abUpdateTarget, tF32 afFrameTime)
  {
    ...
    if (abUpdateTarget && mbTargetDirty &&
  ...
  void _UpdateTarget()
  {
    ...
    niExec (CPU, _this) {
      ...
      if (f) {
        niExec (Main, _this, f) {
          _this->mptrTargetBitmap =
            f->getBitmap(_this->mptrGraphics, VIDEO_PIXELFORMAT);
          if (niFlagIs(_this->mFlags, eVideoDecoderFlags_TargetTexture) &&
              _this->mpTargetTexture)
          {
...
iVideoDecoder* CreateVideoDecoder_TheoraPlayer(iGraphics* apGraphics,
                                               iHString* ahspName,
                                               iFile* apFile,
