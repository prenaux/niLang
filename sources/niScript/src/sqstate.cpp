...
SQSharedState::SQSharedState()
{
  ...
  niLet createDelegate = [](SQSharedState* ss,
                            SQRegFunction* funcz) -> SQObjectPtr {
  ...
  _string_default_delegate =
    createDelegate(this, _string_default_delegate_funcz);
  _number_default_delegate =
    createDelegate(this, _number_default_delegate_funcz);
  _closure_default_delegate =
    createDelegate(this, _closure_default_delegate_funcz);
  _idxprop_default_delegate =
    createDelegate(this, _idxprop_default_delegate_funcz);
  ...
  _matrixf_default_delegate =
    createDelegate(this, _matrixf_default_delegate_funcz);
  ...
  _method_default_delegate =
    createDelegate(this, _method_default_delegate_funcz);
  _interface_default_delegate =
    createDelegate(this, _interface_default_delegate_funcz);
  _error_code_default_delegate =
    createDelegate(this, _error_code_default_delegate_funcz);
  _resolved_type_default_delegate =
    createDelegate(this, _resolved_type_default_delegate_funcz);
...
const SQObjectPtr& SQSharedState::GetTypeNameObj(SQObjectType type) const
{
  ...
  case _RT_NULL: return _typeStr_null;
  case _RT_INTEGER: return _typeStr_int;
  case _RT_FLOAT: return _typeStr_float;
  case _RT_STRING: return _typeStr_string;
  case _RT_TABLE: return _typeStr_table;
  case _RT_ARRAY: return _typeStr_array;
  case _RT_CLOSURE:
  case _RT_NATIVECLOSURE: return _typeStr_function;
  case _RT_USERDATA: return _typeStr_userdata;
  case _RT_FUNCPROTO: return _typeStr_function;
  case _RT_IUNKNOWN: return _typeStr_iunknown;
...
Ptr<SQNativeClosure> CreateSQNativeClosure(ain<SQRegFunction> aRegFunc)
{
  ...
  if (niStringIsOK(aRegFunc.typemask)) {
    if (!CompileTypemask(nc->_typecheck, aRegFunc.typemask)) {
      niError(niFmt("'%s' CompileTypemask '%s' failed.", aRegFunc.name,
                    aRegFunc.typemask));
      return nullptr;
    }
  }
