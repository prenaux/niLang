...
struct sTestWindowSink : public ImplRC<iMessageHandler> {
  ...
  void __stdcall HandleMessage(const tU32 anMsg, const Var& a, const Var& b)
  {
    ...
    case eOSWindowMessage_KeyDown:
      ...
      switch (a.mU32) {
      case eKey_F:
        w->SetFullScreen(w->GetFullScreen() == eInvalidHandle ? 0
                                                              : eInvalidHandle);
        break;

      case eKey_Z: w->SetCursorPosition(sVec2i::Zero()); break;
      case eKey_X: w->SetCursorPosition(w->GetClientSize() / 2); break;
      case eKey_C: w->SetCursorPosition(w->GetClientSize()); break;
      case eKey_Q: w->SetCursor(eOSCursor_None); break;
      case eKey_W: w->SetCursor(eOSCursor_Arrow); break;
      case eKey_E: w->SetCursor(eOSCursor_Text); break;
      case eKey_K: w->SetCursorCapture(!w->GetCursorCapture()); break;
      case eKey_R: w->RedrawWindow(); break;
      case eKey_n0: w->CenterWindow(); break;
      ...
      case eKey_O: {
        ...
        niCheck_(
          ...
          ;);
        niDebugFmt(
          ("... Copied Clipboard: '%s'", _clipboardDT->GetString("text")));
        break;
      }
      case eKey_P: {
        ...
        niDebugFmt(
          ("... Pasted Clipboard: '%s'", _clipboardDT->GetString("text")));
        break;
      }
      ...
      case eKey_n3:
        ...
        niDebugFmt(("... SetSize: new rect: %s, client size: %s, size: %s",
                    w->GetRect(), w->GetClientSize(), w->GetSize()));
        break;
        // Window Size
      case eKey_n4:
        niDebugFmt(("... SetSize noop: curr rect: %s", w->GetRect()));
        w->SetSize(w->GetSize());
        niDebugFmt(("... SetSize noop: new rect: %s, client size: %s, size: %s",
                    w->GetRect(), w->GetClientSize(), w->GetSize()));
        break;
      ...
      case eKey_n5:
        ...
        niDebugFmt(
          ("... SetClientSize: new rect: %s, client size: %s, size: %s",
           w->GetRect(), w->GetClientSize(), w->GetSize()));
        break;
        // Client Size
      case eKey_n6:
        ...
        niDebugFmt(
          ("... SetClientSize noop: new rect: %s, client size: %s, size: %s",
           w->GetRect(), w->GetClientSize(), w->GetClientSize()));
        break;
      case eKey_n7: w->SetFocus(); break;
      }
      break;
    ...
    case eOSWindowMessage_MouseMove:
      ...
      niDebugFmt((
        _A("eOSWindowMessage_MouseMove: [%d,%d] [%d,%d] (contentsScale: %g)\n"),
        a.mV2L[0], a.mV2L[1], b.mV2L[0], b.mV2L[1], w->GetContentsScale()));
#endif
      break;
    }
  }
...
TEST_FIXTURE(FOSWindow, Create)
{
  ...
  {
    Ptr<iOSWindow> wnd =
      ni::GetLang()->CreateWindow(NULL, "HelloWindow", sRecti(50, 50, 400, 300),
                                  0, eOSWindowStyleFlags_Regular);
...
TEST_FIXTURE(FOSWindow, MessageBox)
{
  ...
  if (isInteractive) {
    ...
    eOSMessageBoxReturn r =
      ni::GetLang()->MessageBox(wnd, NULL, "Hello World !", 0);
    niDebugFmt(
      ("... MessageBox Returned: %s", niEnumToChars(eOSMessageBoxReturn, r)));
  }
}
...
TEST_FIXTURE(FOSWindow, MessageBoxWarning)
{
  ...
  if (isInteractive) {
    ...
    eOSMessageBoxReturn r = ni::GetLang()->MessageBox(
      wnd, NULL, "Hello Warning !", eOSMessageBoxFlags_IconWarning);
    niDebugFmt(
      ("... MessageBox Returned: %s", niEnumToChars(eOSMessageBoxReturn, r)));
    niDebugFmt(("... isInteractive end of '%s'.", m_testName));
  }
...
TEST_FIXTURE(FOSWindow, MessageBoxInfoYesNo)
{
  ...
  if (isInteractive) {
    ...
    eOSMessageBoxReturn r = ni::GetLang()->MessageBox(
      wnd, NULL, "Hello Info !",
      eOSMessageBoxFlags_IconInfo | eOSMessageBoxFlags_YesNo);
    niDebugFmt(
      ("... MessageBox Returned: %s", niEnumToChars(eOSMessageBoxReturn, r)));
    niDebugFmt(("... isInteractive end of '%s'.", m_testName));
  }
