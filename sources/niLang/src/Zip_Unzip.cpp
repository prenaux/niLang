...
typedef struct {
  ...
  unsigned long
    byte_before_the_zipfile; /* byte before the zipfile, (>0 for sfx)*/

  bool encrypted;        // is it encrypted?
  unsigned long keys[3]; // decryption keys, initialized by unzOpenCurrentFile
  int
    encheadleft; // the first call(s) to unzReadCurrentFile will read this many encryption-header bytes first
  char
    crcenctest; // if encrypted, we'll check the encryption buffer against this
...
niExportFunc(unzFile) niUnzip_Open(ni::iFile* fin)
{
  ...
  unsigned long number_disk_with_CD =
    0; /* number the the disk with central dir, used
  ...
  if (unzlocal_getShort(fin, &us.gi.number_entry) != UNZ_OK) {
    niError(
      "Couldn't get total number of entries in central directory on disk.");
    err = UNZ_ERRNO;
  }
  ...
  unsigned long commentBufferSize)
{
  ...
  s->file->ReadRaw(&file_info,
                   sizeof(file_info) - 2 * 4); // 2*4 is the size of 2 my vars
                                     ...
                                     int iCaseSensitivity)
{
  ...
  while (err == UNZ_OK) {
    ...
    niUnzip_GetCurrentFileInfo(file, NULL, szCurrentFileName,
                               sizeof(szCurrentFileName) - 1, NULL, 0, NULL, 0);
    if (niUnzip_StringFileNameCompare(szCurrentFileName, szFileName,
                                      iCaseSensitivity) == 0)
      return UNZ_OK;
                                       ...
                                       unsigned long pos)
{
  ...
  s->num_file = num;
  unzlocal_GetCurrentFileInfoInternal(file, &s->cur_file_info,
                                      &s->cur_file_info_internal, NULL, 0, NULL,
                                      0, NULL, 0);
  return 1;
}
...
local int unzlocal_CheckCurrentFileCoherencyHeader(
  unz_s* s, uInt* piSizeVar, unsigned long* poffset_local_extrafield,
  uInt* psize_local_extrafield)
{
  ...
  *psize_local_extrafield = 0;
  ...
  if (!s->file->SeekSet(s->cur_file_info_internal.offset_curfile +
...
niExportFunc(int) niUnzip_OpenCurrentFile(unzFile file, const char* password)
{
  ...
  if (extlochead)
    pfile_in_zip_read_info->crcenctest =
      (char)((s->cur_file_info.dosDate >> 8) & 0xff);
  else
  ...
  while (pfile_in_zip_read_info->stream.avail_out > 0) {
        ...
        (pfile_in_zip_read_info->rest_read_compressed > 0))
    {
      ...
      if (!pfile_in_zip_read_info->file->SeekSet(
        ...
        return UNZ_ERRNO;
      if (pfile_in_zip_read_info->file->ReadRaw(
      ...
      if (pfile_in_zip_read_info->encrypted) {
        ...
        for (unsigned int i = 0; i < uReadThis; ++i) {
          streamInBuf[i] =
            zdecode(pfile_in_zip_read_info->keys, streamInBuf[i]);
        }
      }
    }
                                             ...
                                             unsigned len)
{
  ...
  if (pfile_in_zip_read_info->file->ReadRaw(buf, (uInt)size_to_read) !=
      size_to_read)
    return UNZ_ERRNO;
