...
#define niMessageID_PackChars(A, B, C, D, BYTE)                               \
  niMessageID_PackBased(niMessageID_CharToBase(A), niMessageID_CharToBase(B), \
                        niMessageID_CharToBase(C), niMessageID_CharToBase(D), \
                        BYTE)

#define niMessageID(A, B, C, D, BYTE) niMessageID_PackChars(A, B, C, D, BYTE)
...
#define niMessageID_GetCharA(MSGID) \
  niMessageID_BaseToChar(niMessageID_GetBaseA(MSGID))
#define niMessageID_GetCharB(MSGID) \
  niMessageID_BaseToChar(niMessageID_GetBaseB(MSGID))
#define niMessageID_GetCharC(MSGID) \
  niMessageID_BaseToChar(niMessageID_GetBaseC(MSGID))
#define niMessageID_GetCharD(MSGID) \
  niMessageID_BaseToChar(niMessageID_GetBaseD(MSGID))

#define niMessageID_MaskA(MSGID) \
  niMessageID_PackBased(niMessageID_GetBaseA(MSGID), 0, 0, 0, 0)

#define niMessageID_MaskAB(MSGID)                    \
  niMessageID_PackBased(niMessageID_GetBaseA(MSGID), \
                        niMessageID_GetBaseB(MSGID), 0, 0, 0)

#define niMessageID_MaskABC(MSGID)                   \
  niMessageID_PackBased(niMessageID_GetBaseA(MSGID), \
                        niMessageID_GetBaseB(MSGID), \
                        niMessageID_GetBaseC(MSGID), 0, 0)

#define niMessageID_MaskABCD(MSGID)                           \
  niMessageID_PackBased(                                      \
    niMessageID_GetBaseA(MSGID), niMessageID_GetBaseB(MSGID), \
    niMessageID_GetBaseC(MSGID), niMessageID_GetBaseD(MSGID), 0)

#define niMessageID_Fmt(MSGID)                                    \
  niFmt(_A("%c%c%c%c%02X"), niMessageID_GetCharA(MSGID),          \
        niMessageID_GetCharB(MSGID), niMessageID_GetCharC(MSGID), \
        niMessageID_GetCharD(MSGID), niMessageID_GetBYTE(MSGID))
...
inline const ni::achar* MessageID_ToString(ni::achar aaszOut[7],
                                           ni::tU32 anMessageID)
{
