...
static tI32 MS_ADPCM_nibble(struct sMsADPCMDecodeState* state, tU8 nybble,
                            tI16* coeff)
{
  const tI32 max_audioval = ((1 << (16 - 1)) - 1);
  const tI32 min_audioval = -(1 << (16 - 1));
  const tI32 adaptive[] = { 230, 230, 230, 230, 307, 409, 512, 614,
...
static tBool MS_ADPCM_decode(sMsADPCMDecoder& aDecoder, tU8** audio_buf,
                             tU32* audio_len)
{
  ...
  *audio_len = (encoded_len / aDecoder.wavefmt.blockalign) *
               aDecoder.wSamplesPerBlock * aDecoder.wavefmt.channels *
               sizeof(tI16);
...
static tI32 IMA_ADPCM_nibble(sImaADPCMDecodeState* state, tU8 nybble)
{
  const tI32 max_audioval = ((1 << (16 - 1)) - 1);
  const tI32 min_audioval = -(1 << (16 - 1));
  const int index_table[16] = { -1, -1, -1, -1, 2, 4, 6, 8,
                                -1, -1, -1, -1, 2, 4, 6, 8 };
...
static void Fill_IMA_ADPCM_block(tU8* decoded, tU8* encoded, int channel,
                                 int numchannels,
...
static tBool IMA_ADPCM_decode(sImaADPCMDecoder& aDecoder, tU8** audio_buf,
                              tU32* audio_len)
{
  struct sImaADPCMDecodeState* state;
  tU8 *freeable, *encoded, *decoded;
  ...
  if (channels > niCountOf(aDecoder.state)) {
    niError(niFmt(_A("IMA ADPCM decoder can only handle %d channels"),
                  niCountOf(aDecoder.state)));
    return eFalse;
  }
 ...
 public:
  ...
  cSoundDataWAV(iFile* apFile)
  {
    ...
    // Decode the audio data format
    {
      ...
      if (chunk.magic != FMT) {
        niError(
          niFmt("Expected FMT chunk, got '%X'.", niSwapBE32(chunk.magic)));
        return;
      }
      ...
      default: {
        niError(niFmt(_A("Unknown WAVE data format: 0x%.4x."),
                      niSwapLE16(format->encoding)));
        return;
      }
      }
      ...
      case 4: {
        if (bMsADPCM || bImaADPCM) {
          mFormat = (niSwapLE16(format->channels) == 2) ? eSoundFormat_Stereo16
                                                        : eSoundFormat_Mono16;
        }
        else {
      ...
      case 8: {
        mFormat = (niSwapLE16(format->channels) == 2) ? eSoundFormat_Stereo8
                                                      : eSoundFormat_Mono8;
        break;
      }
      ...
      default: {
        niError(niFmt(_A("Can't decode a %d bits format."),
                      niSwapLE16(format->bitspersample)));
        break;
      }
      }
    ...
    if (mFormat == eSoundFormat_Mono8)
      audio_len &= ~1;
    else if (mFormat == eSoundFormat_Mono16)
      audio_len &= ~2;
    else if (mFormat == eSoundFormat_Stereo8)
      audio_len &= ~2;
    else if (mFormat == eSoundFormat_Stereo16)
      audio_len &= ~4;
...
#pragma niTodo("Should reopen a file handle and not use auto seek...")
    if (!mptrData.IsOK())
      return NULL;
    Ptr<iFile> ptrFile =
      ni::CreateFileWindow(mptrData->GetFileBase(), 0, 0, NULL, eTrue);
