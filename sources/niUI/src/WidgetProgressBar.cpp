...
void cProgressBarWidget::ComputeClientRect(tF32 w, tF32 h)
{
  mpWidget->SetClientRect(
    skin.normalFrame->ComputeFrameCenter(sRectf(0, 0, w, h)));
}

tBool cProgressBarWidget::OnWidgetSink(iWidget* apWidget, tU32 nMsg,
                                       const Var& varParam0,
                                       const Var& varParam1)
{
  ...
  case eUIMessage_NCPaint: {
    ...
    if (c) {
      ...
      c->BlitOverlayFrame(mpWidget->GetWidgetRect(), skin.normalFrame,
                          eRectFrameFlags_All);
    }
  } break;
  case eUIMessage_Paint: {
    ...
    if (c) {
      sRectf clirect =
        sRectf(0, 0, mpWidget->GetClientSize().x, mpWidget->GetClientSize().y);
      ...
      {
        c->BlitText(mpWidget->GetFont(), clirect,
