...
tBool __stdcall cUIContext::AddSkinFromRes(iHString* ahspRes)
{
  ...
  if (!SerializeDataTable(NULL, eSerializeMode_Read, ptrDT, fp)) {
    niError(niFmt(_A("Can't read skin datatable from resource '%s'."),
                  niHStr(ahspRes)));
    return eFalse;
  }
...
tBool __stdcall cUIContext::SetDefaultSkin(iHString* ahspName)
{
  ...
  if (nSkinIndex == eInvalidHandle) {
    niError(
      niFmt(_A("Can't find skin '%s' to set as default."), niHStr(ahspName)));
    return eFalse;
  }
...
{
  tU32 nCount = 0;
  for (tSkinMap::const_iterator it = mmapSkins.begin(); it != mmapSkins.end();
       ++it, ++nCount)
  {
    if (anIndex == nCount)
      return it->first;
...
///////////////////////////////////////////////
cString _GetSkinFontPath(const iHString* ahspClass, const iHString* ahspState,
                         const iHString* ahspName)
{
  cString r;
  r += _A("/");
...
iFont* __stdcall cUIContext::FindSkinFont(iHString* ahspSkin,
                                          iHString* ahspClass,
                                          iHString* ahspState,
                                          iHString* ahspName) const
{
  ...
  if (ptrDT.IsOK() && ptrDT != mptrErrorSkin) {
    ...
    if (!font.IsOK()) {
      font = ptrDT->GetVarFromPath(
        _GetSkinFontPath(ahspClass, ahspState, ahspName).Chars(), niVarNull);
    }

    // Class/"Normal"/"Fonts"/Name
    if (!font.IsOK()) {
      font = ptrDT->GetVarFromPath(
        _GetSkinFontPath(ahspClass, NULL, ahspName).Chars(), niVarNull);
    }

    // "Default"/State/"Fonts"/Name
    if (!font.IsOK()) {
      font = ptrDT->GetVarFromPath(
        _GetSkinFontPath(NULL, ahspState, ahspName).Chars(), niVarNull);
    }

    // "Default"/"Normal"/"Fonts"/Name
    if (!font.IsOK()) {
      font = ptrDT->GetVarFromPath(
        _GetSkinFontPath(NULL, NULL, ahspName).Chars(), niVarNull);
    }
  }
...
iOverlay* __stdcall cUIContext::FindSkinCursor(iHString* ahspSkin,
                                               iHString* ahspClass,
                                               iHString* ahspState,
                                               iHString* ahspName) const
{
  ...
  if (ptrDT.IsOK() && ptrDT != mptrErrorSkin) {
    ...
    // Class/"Cursors"/Name
    overlay = ptrDT->GetVarFromPath(
      _GetSkinCursorPath(ahspClass, ahspName).Chars(), niVarNull);
    if (overlay.IsOK())
      return overlay;

    // "Default"/"Cursors"/Name
    overlay = ptrDT->GetVarFromPath(_GetSkinCursorPath(NULL, ahspName).Chars(),
                                    niVarNull);
    if (overlay.IsOK())
      return overlay;
  }
...
///////////////////////////////////////////////
cString _GetSkinElementPath(const iHString* ahspClass,
                            const iHString* ahspState, const iHString* ahspName)
{
  cString r;
  r += _A("/");
  r += ni::HStringIsNotEmpty(ahspClass) ? niHStr(ahspClass) : _A("Default");
...
iOverlay* __stdcall cUIContext::FindSkinElement(iHString* ahspSkin,
                                                iHString* ahspClass,
                                                iHString* ahspState,
                                                iHString* ahspName) const
{
  ...
  if (ptrDT.IsOK() && ptrDT != mptrErrorSkin) {
    ...
    // Class/State/Name
    overlay = ptrDT->GetVarFromPath(
      _GetSkinElementPath(ahspClass, ahspState, ahspName).Chars(), niVarNull);
    if (overlay.IsOK())
      return overlay;

    // Class/"Normal"/Name
    overlay = ptrDT->GetVarFromPath(
      _GetSkinElementPath(ahspClass, NULL, ahspName).Chars(), niVarNull);
    if (overlay.IsOK())
      return overlay;

    // "Default"/State/Name
    overlay = ptrDT->GetVarFromPath(
      _GetSkinElementPath(NULL, ahspState, ahspName).Chars(), niVarNull);
    if (overlay.IsOK())
      return overlay;

    // "Default"/"Normal"/Name
    overlay = ptrDT->GetVarFromPath(
      _GetSkinElementPath(NULL, NULL, ahspName).Chars(), niVarNull);
    if (overlay.IsOK())
      return overlay;
  }
...
///////////////////////////////////////////////
sColor4f __stdcall cUIContext::FindSkinColor(const sColor4f& aDefault,
                                             iHString* ahspSkin,
                                             iHString* ahspClass,
                                             iHString* ahspState,
                                             iHString* ahspName) const
...
///////////////////////////////////////////////
static Ptr<iOverlay> _ReadOverlayDT(iGraphics* apGraphics,
                                    const sOverlaySource& aTopSource,
                                    iDataTable* dt, iMaterialLibrary* apMatLib,
                                    ...
                                    tLoadedImageMap& mapImages)
{
...

  {
    ...
    if (!HStringIsEmpty(hspMatName)) {
      if (apMatLib) {
        ...
        niWarning(
          "Skin specifies a material without a material library defined.");
      }
    }
  }
  ...
  ptrOvr->SetBlendMode((eBlendMode)dt->GetEnumDefault(
    _A("blendmode"), niEnumExpr(eBlendMode), eBlendMode_NoBlending));
  ptrOvr->SetFiltering(
    dt->GetBoolDefault(_A("filtering"), ptrOvr->GetFiltering()));
  ...
  if (dt->HasProperty("color_top")) {
    ptrOvr->SetCornerColor(
      eRectCorners_Top,
      dt->GetVec4Default(_A("color_top"),
                         ptrOvr->GetCornerColor(eRectCorners_Top)));
  }
  if (dt->HasProperty("color_bottom")) {
    ptrOvr->SetCornerColor(
      eRectCorners_Bottom,
      dt->GetVec4Default(_A("color_bottom"),
                         ptrOvr->GetCornerColor(eRectCorners_Bottom)));
  }
  if (dt->HasProperty("color_left")) {
    ptrOvr->SetCornerColor(
      eRectCorners_Left,
      dt->GetVec4Default(_A("color_left"),
                         ptrOvr->GetCornerColor(eRectCorners_Left)));
  }
  if (dt->HasProperty("color_right")) {
    ptrOvr->SetCornerColor(
      eRectCorners_Right,
      dt->GetVec4Default(_A("color_right"),
                         ptrOvr->GetCornerColor(eRectCorners_Right)));
  }
  if (dt->HasProperty("color_top_left")) {
    ptrOvr->SetCornerColor(
      eRectCorners_TopLeft,
      dt->GetVec4Default(_A("color_top_left"),
                         ptrOvr->GetCornerColor(eRectCorners_TopLeft)));
  }
  if (dt->HasProperty("color_top_right")) {
    ptrOvr->SetCornerColor(
      eRectCorners_TopRight,
      dt->GetVec4Default(_A("color_top_right"),
                         ptrOvr->GetCornerColor(eRectCorners_TopRight)));
  }
  if (dt->HasProperty("color_bottom_left")) {
    ptrOvr->SetCornerColor(
      eRectCorners_BottomLeft,
      dt->GetVec4Default(_A("color_bottom_left"),
                         ptrOvr->GetCornerColor(eRectCorners_BottomLeft)));
  }
  if (dt->HasProperty("color_bottom_right")) {
    ptrOvr->SetCornerColor(
      eRectCorners_BottomRight,
      dt->GetVec4Default(_A("color_bottom_right"),
                         ptrOvr->GetCornerColor(eRectCorners_BottomRight)));
  }
  _UISkinTrace(
    niFmt(_A("Added element '%s' (%s) (%s)."),
...
tBool cUIContext::_InitializeSkinDataTable(iDataTable* apDT)
{
  ...
  QPtr<iMaterialLibrary> ptrMatLib;
  {
    ...
    if (dtMatLib.IsOK()) {
      ...
      if (!ptrMatLib->SerializeDataTable(dtMatLib,
                                         ni::eMaterialSerializeFlags_Read))
      {
        niWarning(niFmt("Skin '%s', Couldn't load the material library.",
                        apDT->GetString("name")));
      }
      else {
  ...
  for (tU32 i = 0; i < dt->GetNumChildren(); ++i) {
    dt->PushChild(i);
    Ptr<iHString> hspClassMaterial =
      dt->GetHStringDefault("material", hspDefaultMaterial);
    for (tU32 c = 0; c < dt->GetNumChildren(); ++c) {
      ...
      if (ni::StrEq(dt->GetName(), _A("Cursors"))) {
        for (tU32 j = 0; j < dt->GetNumChildren(); ++j) {
          dt->PushChild(j);
          Ptr<iOverlay> ptrOvr =
            _ReadOverlayDT(mptrGraphics, stkOverlay.top(), dt->GetTop(),
                           ptrMatLib, hspClassMaterial, mapImages);
          dt->GetTop()->SetIUnknown(_A("_overlay"), ptrOvr);
          _UISkinTrace(niFmt(_A("Added cursor '%s'."),
                             ni::GetLang()->GetAbsoluteDataTablePath(
                               dt->GetTop(), eInvalidHandle)));
          dt->Pop();
        }
      }
      // ---- State ----
        ...
        for (tU32 j = 0; j < dt->GetNumChildren(); ++j) {
          ...
          if (ni::StrEq(dt->GetName(), _A("Fonts"))) {
            for (tU32 k = 0; k < dt->GetNumChildren(); ++k) {
              ...
              if (!ptrFont.IsOK()) {
                niError(niFmt(_A("Can't load font '%s'."), niHStr(hspPath)));
                return eFalse;
              }
              Ptr<iMaterial> ptrMaterial;
              const tU32 nFontMaterialIdx =
                dt->GetTop()->GetPropertyIndex("material");
              if (nFontMaterialIdx != eInvalidHandle) {
                Ptr<iHString> hspFontMaterial =
                  dt->GetTop()->GetHStringFromIndex(nFontMaterialIdx);
              ...
              if (dt->GetTop()->GetPropertyIndex(_A("resolution")) !=
                  eInvalidHandle)
              {
                nFontResolution =
                  dt->GetIntDefault(_A("resolution"), nFontResolution);
              }
              else if (dt->GetTop()->GetPropertyIndex(_A("pixel_size")) !=
                       eInvalidHandle)
              {
                nFontResolution =
                  dt->GetIntDefault(_A("pixel_size"), nFontResolution);
              }
              if (dt->GetTop()->GetPropertyIndex(_A("scaled_size")) !=
                  eInvalidHandle)
              {
              ...
              ptrFont->SetSizeAndResolution(vFontSize, nFontResolution,
                                            mfContentsScale);

              if (dt->GetTop()->GetPropertyIndex(_A("line_spacing")) !=
                  eInvalidHandle)
              {
                ptrFont->SetLineSpacing(dt->GetFloatDefault(
                  _A("line_spacing"), ptrFont->GetLineSpacing()));
              }

              ptrFont->SetColor(ULColorBuild(dt->GetVec4Default(
                _A("color"), Vec4<tF32>(ptrFont->GetColor()))));
              dt->GetTop()->SetIUnknown(_A("_font"), ptrFont);
              _UISkinTrace(niFmt(_A("Added font '%s' ('%s','%s')."), hspName,
          ...
          // --- Element ---
            ...
            Ptr<iOverlay> ptrOvr =
              _ReadOverlayDT(mptrGraphics, stkOverlay.top(), dt->GetTop(),
                             ptrMatLib, hspClassMaterial, mapImages);
            dt->GetTop()->SetIUnknown(_A("_overlay"), ptrOvr);
            _UISkinTrace(niFmt(_A("Added element '%s'."),
                               ni::GetLang()->GetAbsoluteDataTablePath(
                                 dt->GetTop(), eInvalidHandle)));
          }
          dt->Pop();
