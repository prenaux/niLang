...
*/
  ...
  #elif defined(EA_PLATFORM_XBOXONE) || defined(XBOX) || \
    defined(EA_PLATFORM_CAPILANO)
    // XBox One
  ...
  #elif defined(EA_PLATFORM_LRB) || defined(__LRB__) || \
    (defined(__EDG__) && defined(__ICC) && defined(__x86_64__))
  ...
  // Samsung SMART TV - a Linux-based smart TV
    ...
    #if defined(EA_PLATFORM_IPHONE) || defined(__IPHONE__) || \
    ...
    #elif defined(EA_PLATFORM_OSX) || defined(__MACH__) || \
      (defined(__MSL__) && (__dest_os == __mac_os_x))
  ...
  // __ARM_ARCH_7A__ is defined by GCC on an ARM v7l (Raspberry Pi 2)
    ...
    #define EA_PLATFORM_UNIX 1
    #define EA_PLATFORM_POSIX \
      1 // BSD's posix complaince is not identical to Linux's
  ...
  // _M_ARM is defined by the VC++ compiler.
    ...
    #define EA_WINAPI_FAMILY_DESKTOP_APP \
      (EA_WINAPI_PARTITION_DESKTOP | EA_WINAPI_PARTITION_APP)
    ...
    #define EA_WINAPI_FAMILY_PARTITION(Partition) \
      ((EA_WINAPI_FAMILY & Partition) == Partition)
  ...
  // __sparc is defined by the Sun and GCC compilers.
    ...
    #define EA_PLATFORM_JSCC
    #define EA_SYSTEM_LITTLE_ENDIAN 1
    #define EA_PLATFORM_DESCRIPTION "JSCC"
    ...
    #if defined(EA_PROCESSOR_ARM32) || defined(EA_PROCESSOR_ARM64) || \
      defined(EA_PROCESSOR_ARM7)
  ...
  //
      ...
      defined(_M_IA64) || defined(__ia64__) || defined(__arch64__) ||      \
      defined(__aarch64__) || defined(__mips64__) || defined(__64BIT__) || \
      defined(__Ptr_Is_64)
...
// This the expected best guess values for the targets that we can make at compilation time.
      ...
      #define EA_CACHE_LINE_SIZE \
        32 // This varies between implementations and is usually 32 or 64.
    #elif defined(EA_PROCESSOR_ARM64)
      #define EA_CACHE_LINE_SIZE \
        64 // Cache line Cortex-A8  (64 bytes) http://shervinemami.info/armAssembly.html however this remains to be mostly an assumption at this stage
