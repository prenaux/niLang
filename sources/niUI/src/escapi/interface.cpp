...
HRESULT InitDevice(int aDevice, int aPreferredMode)
{
	if (gDevice[aDevice])
	{
	...
	HRESULT hr = gDevice[aDevice]->initCapture(aDevice,aPreferredMode);
	if (FAILED(hr))
	{
		delete gDevice[aDevice];
		gDevice[aDevice] = 0;
...
void CheckForFail(int aDevice)
{
	...
	if (gDevice[aDevice]->mRedoFromStart)
	{
		...
		gDevice[aDevice]->deinitCapture();
		HRESULT hr = gDevice[aDevice]->initCapture(aDevice,0);
		if (FAILED(hr))
		{
			delete gDevice[aDevice];
			gDevice[aDevice] = 0;
