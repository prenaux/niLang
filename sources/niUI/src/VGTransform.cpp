...
class cVGTransform : public ImplRC<iVGTransform> {
 public:
  ...
  tBool __stdcall Copy(const iVGTransform* apTransform)
  {
    ...
    GetTransform() =
      niStaticCast(const cVGTransform*, apTransform)->GetTransform();
    return eTrue;
  }
  ...
  {
    GetTransform().multiply(agg::trans_affine(aMatrix._11, aMatrix._12,
                                              aMatrix._21, aMatrix._22,
                                              aMatrix._41, aMatrix._42));
  }
  virtual void __stdcall PreMultiplyMatrix(const sMatrixf& aMatrix)
  {
    GetTransform().premultiply(agg::trans_affine(aMatrix._11, aMatrix._12,
                                                 aMatrix._21, aMatrix._22,
                                                 aMatrix._41, aMatrix._42));
  }
  ...
  void __stdcall LineSegment(const sVec2f& aStart, const sVec2f& aEnd,
                             tF32 afDist)
  {
    GetTransform().multiply(agg::trans_affine_line_segment(
      aStart.x, aStart.y, aEnd.x, aEnd.y, afDist));
  }
  void __stdcall PreLineSegment(const sVec2f& aStart, const sVec2f& aEnd,
                                tF32 afDist)
  {
    GetTransform().premultiply(agg::trans_affine_line_segment(
      aStart.x, aStart.y, aEnd.x, aEnd.y, afDist));
  }

  inline agg::trans_affine& GetTransform()
  {
    return mTransform.top();
  }
  inline const agg::trans_affine& GetTransform() const
  {
    return mTransform.top();
  }
...
niExportFuncCPP(agg::trans_affine&) AGGGetTransform(
  const iVGTransform* apTransform)
{
  return niConstStaticCast(iVGTransform*, cVGTransform*, apTransform)
    ->GetTransform();
}
