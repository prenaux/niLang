...
#define ni_list_foreach(list, itname)                             \
  tListIt itname;                                                 \
  for (itname = ni_list_begin(list); itname != ni_list_end(list); \
       itname = ni_list_next(itname))
#define ni_list_foreachi(list, itname, iname)                                \
  tListIt itname;                                                            \
  size_t iname;                                                              \
  for (iname = 0, itname = ni_list_begin(list); itname != ni_list_end(list); \
...
static __forceinline size_t ni_list_size(tList* list)
{
  size_t size = 0;
  tListIt position;
  for (position = ni_list_begin(list); position != ni_list_end(list);
...
#define ni_tlist_findr(rit, ctype, begin, end, data)         \
  {                                                          \
    for (rit = begin; rit != end; rit = ni_list_next(rit)) { \
...
#define ni_tlist_find(rit, ctype, list, data) \
  ni_tlist_findr(rit, ctype, ni_list_begin(list), ni_list_end(list), data)
