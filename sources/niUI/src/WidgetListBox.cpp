...
cWidgetListBox::cWidgetListBox(iWidget* apWidget)
{
  ...
  mpWidget = apWidget;
  // vertical scroll bar
  mptrVtScroll = mpWidget->GetUIContext()->CreateWidget(
  ...
  mptrHzScroll = mpWidget->GetUIContext()->CreateWidget(
    _A("ScrollBar"), mpWidget, sRectf(),
    eWidgetStyle_NoClip | eWidgetStyle_NCRelative | eWidgetScrollBarStyle_Horz);
...
///////////////////////////////////////////////
tBool __stdcall cWidgetListBox::SetColumn(tU32 anColumn, const achar* aaszName,
                                          tU32 anSize)
{
...
tU32 cWidgetListBox::GetColumnWidth(tU32 anColumn) const
{
  ...
  return mvColumns[anColumn]
    .nSetSize; // return the set size here, not the computed width
}
...
///////////////////////////////////////////////
tU32 cWidgetListBox::_ComputeColumnWidth(const tU32 anCol,
...
///////////////////////////////////////////////
tBool cWidgetListBox::SetItemText(tU32 anColumn, tU32 anItem,
...
///////////////////////////////////////////////
tBool cWidgetListBox::SetItemTextColor(tU32 anColumn, tU32 anItem,
                                       tU32 anTextColor)
{
  if (anItem >= GetNumItems())
    return eFalse;
...
///////////////////////////////////////////////
tBool __stdcall cWidgetListBox::SetItemWidget(tU32 anColumn, tU32 anItem,
                                              iWidget* apWidget)
{
  if (anItem >= GetNumItems())
    return eFalse;
...
///////////////////////////////////////////////
iWidget* __stdcall cWidgetListBox::GetItemWidget(tU32 anColumn,
                                                 tU32 anItem) const
{
  if (anItem >= GetNumItems())
    return NULL;
...
///////////////////////////////////////////////
tU32 __stdcall cWidgetListBox::GetItemFromText(tU32 anColumn,
                                               const achar* aaszText) const
{
  if (anColumn != 0 && anColumn >= GetNumColumns())
    return eInvalidHandle;
  niLoop (i, mvItems.size()) {
...
///////////////////////////////////////////////
tBool __stdcall cWidgetListBox::OnWidgetSink(iWidget* apWidget, tU32 nMsg,
                                             const Var& varParam0,
                                             const Var& varParam1)
{
  ...
  case eUIMessage_NCSize: DoUpdateLayout(eTrue); break;
  case eUIMessage_NCPaint: {
    ...
    if (c) {
      ...
      c->BlitOverlayFrame(
        rect, apWidget->GetHasFocus() ? skin.focusedFrame : skin.normalFrame,
        eRectFrameFlags_All); // here we should draw all frame;

      // The Columns should draw after the UI background
      if (GetNumColumns() &&
          (apWidget->GetStyle() & eWidgetListBoxStyle_HasHeader))
      {
        Paint_Header(c, rect);
      }
    }
  ...
  case eUIMessage_NCMouseMove:
    if (mnHeaderHandlePressed != eInvalidHandle) {
      tI32 diff =
        ni::FloatToIntNearest(varParam1.GetVec2f().x - mvHeaderPivot.x);
      if (diff != 0) {
  ...
  case eUIMessage_Command:
    if (mpWidget->GetParent()) {
      Ptr<iWidgetCommand> ptrCmd =
        ni::VarQueryInterface<iWidgetCommand>(varParam0);
      if (ptrCmd.IsOK() && (ptrCmd->GetSender() == mptrHzScroll ||
                            ptrCmd->GetSender() == mptrVtScroll))
      {
...
void cWidgetListBox::DoUpdateLayout(tBool abForce)
{
  if (abForce || mInternalFlags.bShouldSortItems) {
    if (mnSortKey != eInvalidHandle) {
      astl::stable_sort(mvItems.begin(), mvItems.end(),
                        [&](const astl::shared_ptr<sItem>& a,
                            const astl::shared_ptr<sItem>& b) {
  ...
  if (mInternalFlags.bShouldAutoScroll) {
    ...
    if (ptrVSB.IsOK() && mptrVtScroll->GetVisible()) {
      ...
      tU32 destPos =
        mvSelection.empty() ? mvItems.size() - 1 : mvSelection.back();
      ...
      else if (destPos >= scrollpos && destPos < (scrollpos + pagesize - 1)) {
        // niDebugFmt(("... inview"));
      }
      else {
        // niDebugFmt(("... tomiddle"));
...
void cWidgetListBox::OnKeyDown(tU32 key)
{
  ...
  case eKey_Down: {
    if (!mvSelection.empty() &&
        ((tI32)mvSelection.back() < (tI32)mvItems.size() - 1))
    {
      if (selMode == SEL_RANGE) {
        AddSelection(mvSelection.back() + 1);
      }
  ...
  case eKey_PgUp: {
    if (!mvSelection.empty() && mvSelection.back() > 0) {
      ...
      if (selMode == SEL_RANGE) {
        tI32 nSelEnd =
          (tI32)ni::Max(0, (tI32)mvSelection.back() - (tI32)nNumLines);
  ...
  case eKey_PgDn: {
        ...
        ((tI32)mvSelection.back() < (tI32)mvItems.size() - 1))
    {
      ...
      if (selMode == SEL_RANGE) {
        tI32 nSelEnd =
          (tI32)ni::Min(mvItems.size() - 1, mvSelection.back() + nNumLines);
        for (tI32 i = (tI32)mvSelection.back() + 1; i <= nSelEnd; ++i) {
          AddSelection(i);
        }
      }
        ...
        SetSelected(
          ni::Min(mvItems.size() - 1, mvSelection.back() + nNumLines));
      }
      AutoScroll();
    }
    break;
  ...
  case eKey_End: {
    if (mvSelection.empty() || mvSelection.back() != mvItems.size() - 1) {
      if (selMode == SEL_RANGE) {
        for (tI32 i = tI32(mvSelection.back() + 1);
             i <= tI32(mvItems.size() - 1); ++i)
        {
          AddSelection(i);
        }
      }
...
tU32 cWidgetListBox::RecomputeWidestItem()
{
  ...
  const tU32 defColW = _ComputeDefaultColumnWidth();
  const tF32 sepw =
    niFlagIs(mpWidget->GetStyle(), eWidgetListBoxStyle_HasHeader) ? LB_SEP_W
                                                                  : 0;
  mfWidestItem = 0;
  if (numCols == 1) {
    niLoop (i, mvItems.size()) {
      const sItem& item = *mvItems[i];
      const tF32 x =
        font->ComputeTextSize(sRectf(0, 0), item.vData[0].strText.c_str(), 0)
          .GetWidth();
...
void cWidgetListBox::UpdateWidgetScrollBars(tF32 w, tF32 h)
{
  ...
  if (bHorzScollbar) {
    ...
    ptrHSB->SetScrollRange(
      Vec2<tF32>(0, (tF32)((mfWidestItem / fw) - nHorzItemsPage)));
    rectHz = sRectf(borders.x, vSize.y - kfScrollBarSize - borders.w, mfRealW,
                    kfScrollBarSize);
  }
  else {
    mptrHzScroll->SetVisible(eFalse);
  ...
  if (bVertScollbar) {
    ...
    ptrVSB->SetScrollRange(
      Vec2<tF32>(0, (tF32)(1 + mvItems.size() - nItemsPerPage)));
    tF32 vertY = borders.y;
    if (GetNumColumns() &&
        (mpWidget->GetStyle() & eWidgetListBoxStyle_HasHeader))
    {
      vertY += mrectHeader.GetBottom();
    }
    rectVt = sRectf(vSize.x - kfScrollBarSize - borders.z, vertY,
...
void cWidgetListBox::NotifySelChange()
{
  ...
  mpWidget->SendCommand(mpWidget->GetParent(),
                        eWidgetListBoxCmd_SelectionChanged, mnInputSelected);
...
tBool cWidgetListBox::SelectItemByPos(const sVec2f& avAbsPos)
{
  ...
  const tU32 selMode = GetModSelMode();
  switch (selMode) {
  case SEL_SINGLE: SetSelected(nNewSelection); break;
  case SEL_ADD:
  ...
  case SEL_RANGE:
    if (mvSelection.empty()) {
      ...
      if (nNewSelection < mvSelection.back()) {
        for (tI32 i = tI32(mvSelection.back() - 1); i >= tI32(nNewSelection);
             --i)
        {
          AddSelection(i);
        }
...
void cWidgetListBox::Paint_Items(iCanvas* apCanvas)
{
  ...
  const tBool hasHeader =
    ((numCols > 1) && (mpWidget->GetStyle() & eWidgetListBoxStyle_HasHeader));
  ...
  const sVec2f vAbsPos =
    mpWidget->GetAbsolutePosition() + mpWidget->GetClientPosition();
  const tU32 defColW = _ComputeDefaultColumnWidth();

  auto paintItem = [this, apCanvas, font, vAbsPos, defColW, &rect, fh, sepw,
                    clientW, numCols](const sItem& item, const tU32 itemIndex,
                                      const tF32 startX, const tF32 y) -> tF32 {
    if (this->GetIsItemSelected(itemIndex)) {
      iOverlay* sel = mpWidget->GetHasFocus() ? skin.sel : skin.selNF;
      apCanvas->BlitOverlay(sRectf(0, y, rect.GetWidth(), fh), sel);
    }
    ...
    niLoop (col, mvColumns.size()) {
      ...
      if (pW) {
        ...
        font->SetColor(itemColData.nTextColor ? itemColData.nTextColor
                                              : wasFontColor);
        apCanvas->BlitText(font, textRect,
...
void cWidgetListBox::Paint_Header(iCanvas* apCanvas, const sRectf& rect)
{
  ...
  niLoop (colIndex, numCols) {
    ...
    const tBool hasSortArrow =
      (colIndex == mnSortKey) && (style & eWidgetListBoxStyle_HeaderSort);
    ...
    apCanvas->BlitLineAlpha(Vec2f(x + colW - 1, y),
                            Vec2f(x + colW - 1, y + mrectHeader.GetHeight()),
                            sepColor);

    const sRectf textRect = apCanvas->BlitText(
      headerFont,
      Rectf(x + headerFrame.x, y + headerFrame.y,
            colW - (hasSortArrow ? sortArrowSize + headerFrame.y : 0) - 3, fh),
...
///////////////////////////////////////////////
tBool __stdcall cWidgetListBox::AddSelectedItem(tU32 anCol,
                                                const achar* aaszText)
{
...
///////////////////////////////////////////////
tBool __stdcall cWidgetListBox::SetSelectedItem(tU32 anCol,
                                                const achar* aaszText)
{
  tU32 nItem = GetItemFromText(anCol, aaszText);
  if (nItem != eInvalidHandle)
...
///////////////////////////////////////////////
const achar* __stdcall cWidgetListBox::GetSelectedItemText(tU32 anCol,
                                                           tU32 anIndex) const
{
...
///////////////////////////////////////////////
iWidget* __stdcall cWidgetListBox::GetSelectedItemWidget(tU32 anCol,
                                                         tU32 anIndex) const
{
...
void cWidgetListBox::InitSkin()
{
  ...
  skin.focusedFrame =
    mpWidget->FindSkinElement(NULL, _H("Focused"), _H("Frame"));
...
void cWidgetListBox::UpdateSelectOnMove()
{
  if (GetModSelMode() == SEL_SINGLE &&
      (mpWidget->GetCapture() ||
       niFlagIs(mpWidget->GetStyle(), eWidgetListBoxStyle_SelectOnMove)))
  {
