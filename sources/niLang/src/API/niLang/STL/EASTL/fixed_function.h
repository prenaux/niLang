...
#define EASTL_INTERNAL_FIXED_FUNCTION_STATIC_ASSERT(TYPE)      \
  static_assert(                                               \
    sizeof(TYPE) <= sizeof(typename Base::FunctorStorageType), \
...
class fixed_function<SIZE_IN_BYTES, R(Args...)>
    : public internal::function_detail<SIZE_IN_BYTES, R(Args...)> {
 private:
 ...
 public:
  ...
  fixed_function(std::nullptr_t p) EA_NOEXCEPT : Base(p)
  {
  }
  ...
  template <typename Functor,
            typename = EASTL_INTERNAL_FUNCTION_VALID_FUNCTION_ARGS(
              Functor, R, Args..., Base, fixed_function)>
  ...
  {
    EASTL_INTERNAL_FIXED_FUNCTION_STATIC_ASSERT(
      eastl::reference_wrapper<Functor>);
    Base::operator=(f);
    return *this;
  }
...
bool operator==(const fixed_function<S, R(Args...)>& f,
                std::nullptr_t) EA_NOEXCEPT
{
  return !f;
}
...
template <int S, typename R, typename... Args>
void swap(fixed_function<S, R(Args...)>& lhs,
          fixed_function<S, R(Args...)>& rhs)
{
  lhs.swap(rhs);
}
