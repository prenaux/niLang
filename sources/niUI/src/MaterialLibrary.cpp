...
class cMaterialLibrary
    : public ImplRC<ni::iMaterialLibrary, eImplFlags_Default> {
  ...
  struct sTexture {
    ...
    tBool Read0(iGraphics* apGraphics, iFile* apFile)
    {
      ...
      _tex =
        apGraphics->CreateTextureFromBitmap(_H(_finalName), bmp, _texFlags);
      if (!_tex.IsOK()) {
        _tex = NULL;
      }
    ...
    ///////////////////////////////////////////////
    void InitWrite(iTexture* apTexture, iHString* ahspName,
                   tMaterialSerializeFlags aFlags)
    {
      ...
      if (maxRes || compression)
        _finalName << "_";
      if (compression)
        _finalName << "c" << compression;
      if (maxRes)
        _finalName << "r" << maxRes;
    }
    ...
    tBool WriteRaw(iGraphics* apGraphics, iFile* apFile)
    {
      ...
      // Compression ratio...
      tU32 compression =
        eMaterialSerializeFlags_GetTextureCompression(_matSerFlags);
      // Resize to max res ?
      ...
      if (maxRes) {
        if (bmp->GetPixelFormat()->GetCaps() & ePixelFormatCaps_BlockCompressed)
        {
          ...
          Ptr<iPixelFormat> pxf = apGraphics->CreatePixelFormat(
            bmp->GetPixelFormat()->GetNumABits() > 1 ? _A("R8G8B8A8")
                                                     : _A("R8G8B8"));
        ...
        tU32 newW =
          ni::GetNearestPow2((tU32)((tF32)bmp->GetWidth() / ratio), eTrue);
        tU32 newH =
          ni::GetNearestPow2((tU32)((tF32)bmp->GetHeight() / ratio), eTrue);
    ...
    tBool Read1(iGraphics* apGraphics, iFile* apFile)
    {
      ...
      if (!bmp->GetNumMipMaps() && IsPow2(bmp->GetWidth()) &&
 ...
 public:
  ...
  virtual ni::tBool __stdcall AddMaterial(ni::iMaterial* apMaterial)
  {
    ...
    if (astl::push_back_once(mvMaterials, apMaterial)) {
      niCallSinkVoid_(MaterialLibrarySink, _sinkList, MaterialAdded,
                      (this, apMaterial));
      return eTrue;
    }
    return eFalse;
  ...
  virtual ni::tBool __stdcall RemoveMaterial(ni::iMaterial* apMaterial)
  {
    ...
    niCallSinkVoid_(MaterialLibrarySink, _sinkList, MaterialRemoved,
                    (this, apMaterial));
    return ni::eTrue;
  }
  ...
  virtual ni::tBool __stdcall SerializeDataTable(
    ni::iDataTable* apDT, tMaterialSerializeFlags anFlags)
  {
    if (!niIsOK(apDT)) {
    ...
    if (niFlagIs(anFlags, eMaterialSerializeFlags_Read)) {
      niLoop (i, apDT->GetNumChildren()) {
        ...
        if (!m.IsOK()) {
          ...
          if (!m.IsOK()) {
            niWarning(niFmt(_A("Can't find nor create material '%s'."),
                            niHStr(hspName)));
            continue;
          }
          m->SetName(hspName);
        ...
        { // Read the material from the datatable
          if (!m->Serialize(ni::eSerializeMode_Read, c, mhspBasePath)) {
            niWarning(niFmt(_A("Can't serialize read material '%s'."),
                            niHStr(hspName)));
            continue;
          }
        }
    ...
    ///// WRITE /////
      ...
      ni::Ptr<ni::iDataTableWriteStack> dt =
        ni::CreateDataTableWriteStack(apDT);
      niLoop (i, GetNumMaterials()) {
        ...
        if (anFlags & eMaterialSerializeFlags_NamesOnly) {
          ...
          mvMaterials[i]._mat->Serialize(ni::eSerializeMode_Write, dt->GetTop(),
                                         mhspBasePath);
        }
        dt->Pop();
      }
                                            ...
                                            tMaterialSerializeFlags aFlags)
  {
    ...
    if (niFlagIs(aFlags, eMaterialSerializeFlags_Write)) {
          ...
          niLoop (j, eMaterialChannel_Last) {
            ...
            if (niIsOK(tex)) {
              tTextureMap::iterator it =
                mapTexs.find(tex->GetDeviceResourceName());
              if (it != mapTexs.end()) {
      ...
      // Write the datatable
      {
        ...
        dt->SetEnum(_A("serialize_flags"), niFlagsExpr(eMaterialSerializeFlags),
                    aFlags);
          ...
          niLoop (i, mvMaterials.size()) {
            ...
            niLoop (j, eMaterialChannel_Last) {
              ...
              if (it != mapTexs.end()) {
                ...
                if (!dtCh.IsOK()) {
                  niError(niFmt("Can't get Channel[%d] table of material '%d'.",
                                j, i));
                  return eFalse;
                }
        ...
        // serialize
        if (!ni::SerializeDataTable(_A("dtz"), eSerializeMode_Write, dt,
                                    apFile))
        {
      ...
      if (niFlagIs(aFlags, eMaterialSerializeFlags_Textures)) {
        ...
        niLoopit (tTextureMap::iterator, it, mapTexs) {
          ...
          if (!t.WriteRaw(_graphics, apFile)) {
            niError(
              niFmt(_A("Can't write texture '%s'."), niHStr(t._baseName)));
            return eFalse;
          }
        }
      }
    ...
    //// READ /////
      ...
      tMaterialSerializeFlags dtFlags = dt->GetEnum(
        _A("serialize_flags"), niFlagsExpr(eMaterialSerializeFlags));
      ...
      if (niFlagIs(dtFlags, eMaterialSerializeFlags_Textures)) {
        ...
        case _kfccMatLib0: {
          ...
          niLoop (i, numTexs) {
            ...
            if (!t.Read0(_graphics, apFile)) {
              niError(
                niFmt(_A("Can't read texture '%d' (%s)."), i, t._finalName));
              return eFalse;
            }
        ...
        default: niError(_A("Invalid material library fourCC.")); return eFalse;
        }
      }
      {
        // Load the datatable
