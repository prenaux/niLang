...
template <typename BaseImpl, typename T>
struct sQueryInterfaceTpl {
  static inline iUnknown* _QI(BaseImpl* apThis, const ni::tUUID& aIID)
  {
    if (T::_GetInterfaceUUID() == aIID)
      return niIUnknownCast2(T, apThis);
    else
      return nullptr;
  }
...
template <typename BaseImpl, typename T, typename B>
struct sQueryInterfaceInheritTpl {
  static inline iUnknown* _QI(BaseImpl* apThis, const ni::tUUID& aIID)
  {
    if (T::_GetInterfaceUUID() == aIID)
      return niIUnknownCast2(T, apThis);
    else
      return B::_QI(apThis, aIID);
  }
  static inline tBool _LI(iMutableCollection* apLst)
  {
    apLst->Add(niGetInterfaceUUID(T));
...
#define niImpl_QueryInterfaceTpl(A) \
  ni::ImplMeta::sQueryInterfaceTpl<BaseImpl, A>
#define niImpl_QueryInterfaceInheritTpl(A, B) \
  ni::ImplMeta::sQueryInterfaceInheritTpl<BaseImpl, A, B>
 ...
 public:                                                                       \
  ...
  {                                                                            \
    ...
    if (aIID == niGetInterfaceUUID(iUnknown))                                  \
      return niIUnknownCast2(T0, this);                                        \
    return nullptr;                                                            \
  }                                                                            \
  virtual void __stdcall ListInterfaces(iMutableCollection* apLst, tU32)       \
    const niOverride                                                           \
  {                                                                            \
...
#define niImpl_Inherit(MEM) \
  Impl_Inherit<ni::Impl_##MEM, T0, FLAGS, T1, T2, T3, T4>
...
struct Impl_HeapAlloc {
  inline void* operator new(size_t anSize, const achar* file, int line,
                            const achar* fun)
  {
  ...
  inline void operator delete(void* apMem, const achar* file, int line,
                              const achar* fun)
  {
...
struct Impl_NoHeapAlloc {
 protected:
  void* operator new(size_t anSize, const achar* file, int line,
                     const achar* fun);
  void* operator new[](size_t anSize, const achar* file, int line,
                       const achar* fun);
...
template <typename TMEM, typename T0, tImplFlags FLAGS, typename T1,
          typename T2, typename T3, typename T4>
struct niEmptyBases Impl_Inherit
    : public TMEM,
      public T0,
      public niMetaSelect(niFlagIsNot(FLAGS, eImplFlags_DontInherit1), T1,
                          cUnknown1),
      public niMetaSelect(niFlagIsNot(FLAGS, eImplFlags_DontInherit2), T2,
                          cUnknown2),
      public niMetaSelect(niFlagIsNot(FLAGS, eImplFlags_DontInherit3), T3,
                          cUnknown3),
      public niMetaSelect(niFlagIsNot(FLAGS, eImplFlags_DontInherit4), T4,
                          cUnknown4) {
 public:
...
template <typename T0, tImplFlags FLAGS = eImplFlags_Default,
          typename T1 = cUnknown1, typename T2 = cUnknown2,
          typename T3 = cUnknown3, typename T4 = cUnknown4>
...
static_assert(sizeof(ni::ImplAggregate<iUnknown>) ==
              sizeof(void*) + sizeof(ni::iUnknown*));
