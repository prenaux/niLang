 ...
 */
class cVideoDecoderWebcam
    : public ni::ImplRC<ni::iVideoDecoder, ni::eImplFlags_Default> {
  niBeginClass(cVideoDecoderWebcam);

 public:
  ///////////////////////////////////////////////
  cVideoDecoderWebcam(iGraphics* apGraphics, iHString* ahspName, iFile* apFile,
      ...
      , mbUpdateOnBind(eTrue)
  {
    ...
    niLog(Info,
          niFmt("Initializing webcam: %d, mode: %d, width: %d, height: %d.",
                webcamIndex, preferredmode, mCapture.mWidth, mCapture.mHeight));

    const tBool bSwapRGB = dt->GetBoolDefault("swap_rgb", eFalse);

    mptrTargetBitmap = mptrGraphics->CreateBitmap2D(
      mCapture.mWidth, mCapture.mHeight,
      bSwapRGB ? WEBCAM_PIXEL_FORMAT2 : WEBCAM_PIXEL_FORMAT1);
    if (!mptrTargetBitmap.IsOK()) {
      niError(niFmt(
        _A(
          "Can't create target bitmap for webcam '%s', index: %d, name: '%s'."),
    ...
    if (niFlagIs(mFlags, eVideoDecoderFlags_TargetTexture)) {
      Ptr<iTexture> ptrTex = mptrGraphics->CreateTexture(
        ...
        0, mCapture.mWidth, mCapture.mHeight, 0,
        eTextureFlags_Dynamic | eTextureFlags_Overlay);
      if (!ptrTex.IsOK()) {
        niError(niFmt(
          _A(
            "Can't create target texture for webcam  webcam '%s', index: %d, name: '%s'."),
          apFile->GetSourcePath(), webcamIndex, _getWebcamName(webcamIndex)));
        return;
      }
    ...
    niExec (IO, _this, _fp, preferredmode, webcamIndex) {
      ...
      if (niFlagIs(_this->mFlags, eVideoDecoderFlags_TargetTexture)) {
        _this->mptrGraphics->BlitBitmapToTexture(
          _this->mptrTargetBitmap, _this->mpTargetTexture->mptrTexture, 0);
      }
      ...
      if (!ESCAPI_initCapture(webcamIndex, preferredmode, &_this->mCapture)) {
        ...
        niError(niFmt(
          _A("Can't init capture for webcam '%s', index: %d, name: '%s'."),
      ...
      niDebugFmt(
        ("... initCapture time: %gs", ni::TimerInSeconds() - timeStart));
...
iVideoDecoder* CreateVideoDecoder_Webcam(iGraphics* apGraphics,
