...
///////////////////////////////////////////////
iWidgetMenuItem* __stdcall cWidgetMenu::AddItem(const achar* aaszName,
                                                iHString* ahspID,
...
tBool __stdcall cWidgetMenu::RemoveItem(iWidgetMenuItem* apItem)
{
  ...
  for (tU32 i = 0; i < mpWidget->GetNumChildren(); ++i) {
    iWidgetMenu* pSubMenu =
      ni::QueryInterface<iWidgetMenu>(mpWidget->GetChildFromIndex(i));
...
{
  if (anIndex >= mvItems.size())
    return NULL;
  return mvItems[anIndex];
}

///////////////////////////////////////////////
iWidgetMenuItem* __stdcall cWidgetMenu::GetItemFromName(
...
///////////////////////////////////////////////
tBool __stdcall cWidgetMenu::OnWidgetSink(iWidget* apWidget, tU32 anMsg,
                                          const Var& avarA, const Var& avarB)
{
  ...
  case eUIMessage_Paint: {
    ...
    c->BlitFill(sRectf(Vec2<tF32>(tF32(mnLeftMargin - 2), 0),
                       Vec2<tF32>(rect.GetWidth(), rect.GetHeight())),
                skin._kcolBackNormal);
    c->BlitFill(sRectf(Vec2<tF32>(0, 0),
                       Vec2<tF32>(tF32(mnLeftMargin - 2), rect.GetHeight())),
    ...
    for (x = mnLeftMargin + _knLeftMarginTextSpace, y = _knTopMargin, i = 0;
         i < mvItems.size(); ++i)
    {
      if (i == mnSelectedItem)
        nSelectedItemY = y;
    ...
    if (mpActiveSubMenu && !mpActiveSubMenu->GetVisible()) {
      static_cast<cWidgetMenu*>(
        ni::QueryInterface<iWidgetMenu>(mpActiveSubMenu))
        ->UpdateSizes();
      mpActiveSubMenu->SetPosition(
        Vec2<tF32>(tF32(mnWidth), tF32(nSelectedItemY)) +
      ...
      if (rectAbs.GetRight() >
          mpWidget->GetUIContext()->GetRootWidget()->GetSize().x)
      {
        newPos.x = -rectAbs.GetWidth();
      }
      if (rectAbs.GetBottom() >
          mpWidget->GetUIContext()->GetRootWidget()->GetSize().y)
      {
        newPos.y =
          tF32(nSelectedItemY) - rectAbs.GetHeight() + tF32(mnItemHeight);
      }
      ni::VecMaximize(newPos, newPos + mpWidget->GetAbsolutePosition(),
                      Vec2f(0.0f, mnItemHeight));
      ...
      mpWidget->SendCommand(mpWidget->GetParent(), eWidgetMenuCmd_Opened,
                            mpActiveSubMenu);
    }
  ...
  case eUIMessage_KeyUp: {
    sMouseMessageResult res =
      ProcessKeyInput(avarA.mU32, avarB.mU32, anMsg == eUIMessage_KeyDown);
  ...
  case eUIMessage_NCRightClickDown: {
    ...
    mpWidget->GetUIContext()->SendWindowMessage(
      ...
      (anMsg == eUIMessage_NCRightClickDown) ? ePointerButton_Right
                                             : ePointerButton_Left,
      niVarNull);
    return eTrue;
  }
  ...
  case eUIMessage_Wheel: {
    ...
    if (mpActiveSubMenu && mpActiveSubMenu->GetVisible()) {
      const sRectf rootAbsRect =
        mpWidget->GetUIContext()->GetRootWidget()->GetAbsoluteRect();
      ...
      if (rectAbs.GetHeight() > (rootAbsRect.GetHeight() - mnItemHeight)) {
        const tF32 fMinY =
          rootAbsRect.GetBottom() - (tF32)mnItemHeight - rectAbs.GetHeight();
        const tF32 fMaxY = rootAbsRect.GetTop() + (tF32)mnItemHeight;
        const sVec2f relPos =
          mpActiveSubMenu->GetPosition() - mpWidget->GetAbsolutePosition();
        mpActiveSubMenu->SetPosition(
          Vec2f(mpWidget->GetAbsolutePosition().x + relPos.x,
                ni::Clamp(mpWidget->GetAbsolutePosition().y + relPos.y +
                            (fWheelDelta * mnItemHeight),
                          fMinY, fMaxY)));
      }
    }
    else {
...
void cWidgetMenu::UpdateSizes()
{
  ...
  tU32 nIconWidth =
    (tU32)ni::Max(skin.radio->GetSize().x, skin.check->GetSize().x);
  ...
  for (it = mvItems.begin(); it != mvItems.end(); ++it) {
    ...
    if (niFlagIs(flags, eWidgetMenuItemFlags_Check) ||
        niFlagIs(flags, eWidgetMenuItemFlags_Group) || ptrItem->GetSelected())
    {
    ...
    if (niFlagIs(flags, eWidgetMenuItemFlags_Separator) &&
        niFlagIsNot(flags, eWidgetMenuItemFlags_Title))
    {
  ...
  for (it = mvItems.begin(); it != mvItems.end(); ++it) {
    if (niFlagIs((*it)->GetFlags(), eWidgetMenuItemFlags_Separator) &&
        niFlagIsNot((*it)->GetFlags(), eWidgetMenuItemFlags_Title))
      mnHeight += _knSeparatorHeight;
    else
...
///////////////////////////////////////////////
tU32 cWidgetMenu::DrawItemRect(iCanvas* c, iFont* apFont, tU32 anIndex, tU32 aX,
                               tU32 aY)
{
  Ptr<iWidgetMenuItem> ptrItem =
    (anIndex < mvItems.size()) ? mvItems[anIndex].ptr() : NULL;
  if (!ptrItem.IsOK())
    return mnItemHeight;
...
///////////////////////////////////////////////
tU32 cWidgetMenu::DrawItemOvr(iCanvas* c, iFont* apFont, tU32 anIndex, tU32 aX,
                              tU32 aY)
{
  Ptr<iWidgetMenuItem> ptrItem =
    (anIndex < mvItems.size()) ? mvItems[anIndex].ptr() : NULL;
  ...
  if (niFlagIs(ptrItem->GetFlags(), eWidgetMenuItemFlags_Title)) {
    ...
    if (ptrItem->GetSelected()) {
      ...
      if (niFlagIs(ptrItem->GetFlags(), eWidgetMenuItemFlags_Check))
        ...
        pOvr = skin.radio;
      sVec2f pos = { tF32(aX - pOvr->GetSize().x - 3),
                     tF32(aY + (mnItemHeight >> 1) - (pOvr->GetSize().y / 2)) };
      c->BlitOverlay(sRectf(pos.x, pos.y, pOvr->GetSize().x, pOvr->GetSize().y),
                     pOvr);
    }
    ...
    if (niFlagIs(ptrItem->GetFlags(), eWidgetMenuItemFlags_SubMenu)) {
      ...
      sVec2f pos = { tF32(mnWidth - pOvr->GetSize().x - 2),
                     tF32(aY + (mnItemHeight >> 1) - (pOvr->GetSize().y / 2)) };
      c->BlitOverlay(sRectf(pos.x, pos.y, pOvr->GetSize().x, pOvr->GetSize().y),
                     pOvr);
    }
...
///////////////////////////////////////////////
tU32 cWidgetMenu::DrawItemText(iCanvas* c, iFont* apFont, tU32 anIndex, tU32 aX,
                               tU32 aY)
{
  ...
  if (niFlagIs(ptrItem->GetFlags(), eWidgetMenuItemFlags_Title)) {
    ...
    c->BlitText(apFont, rect, eFontFormatFlags_CenterV,
    ...
    sRectf rect =
      sRectf(tF32(aX), tF32(aY), tF32(mnWidth - 4), tF32(mnItemHeight));
    // rect.MoveTo(Vec2<tF32>(tF32(mnLeftMargin),tF32(aY)));
    c->BlitText(apFont, rect, eFontFormatFlags_CenterV,
                ptrItem->GetLocalizedName());
    if (niFlagIs(ptrItem->GetFlags(), eWidgetMenuItemFlags_Disabled)) {
...
tU32 cWidgetMenu::FindItemFromPos(const sVec2f& avPos)
{
  tU32 y = _knTopMargin;
  for (tWidgetMenuItemVecCIt it = mvItems.begin(); it != mvItems.end(); ++it) {
    if (niFlagIs((*it)->GetFlags(), eWidgetMenuItemFlags_Title) ||
        niFlagIs((*it)->GetFlags(), eWidgetMenuItemFlags_Disabled))
    {
      y += mnItemHeight;
    }
...
{
  Ptr<iWidgetMenuItem> ptrWasSelectedItem =
    (mnSelectedItem < mvItems.size()) ? mvItems[mnSelectedItem].ptr() : NULL;
  ...
  if (mnSelectedItem != anNewSel) {
    if (mnSelectedItem != eInvalidHandle &&
        niFlagIs(mvItems[mnSelectedItem]->GetFlags(),
                 eWidgetMenuItemFlags_SubMenu))
    {
      CloseSubMenu();
    }
    if (anNewSel != eInvalidHandle &&
        niFlagIs(mvItems[anNewSel]->GetFlags(), eWidgetMenuItemFlags_SubMenu))
      mpWidget->SetTimer(0, 0.3f);
  }

  mnSelectedItem = anNewSel;

  Ptr<iWidgetMenuItem> ptrSelectedItem =
    (mnSelectedItem < mvItems.size()) ? mvItems[mnSelectedItem].ptr() : NULL;
  if (ptrWasSelectedItem != ptrSelectedItem) {
    mpWidget->SendCommand(mpWidget->GetParent(), eWidgetMenuCmd_FocusedItem,
                          ptrSelectedItem.ptr(), ptrWasSelectedItem.ptr());
  }
}

///////////////////////////////////////////////
cWidgetMenu::sMouseMessageResult cWidgetMenu::ProcessKeyInput(tU32 aKey,
                                                              tU32 anKeyMod,
                                                              tBool abIsDown)
{
  sMouseMessageResult res;
...
tBool cWidgetMenu::_TestMouseIntersect(const sVec2f& avMousePos)
{
  ...
  if (mpActiveSubMenu) {
    cWidgetMenu* pMenu = static_cast<cWidgetMenu*>(
      ni::QueryInterface<iWidgetMenu>(mpActiveSubMenu));
    return pMenu->_TestMouseIntersect(vAbsMousePos -
                                      mpActiveSubMenu->GetAbsolutePosition());
  }
...
///////////////////////////////////////////////
cWidgetMenu::sMouseMessageResult cWidgetMenu::ProcessMouseMessage(
  tU32 anMsg, const sVec2f& avMousePos)
{
  sMouseMessageResult res;
  sVec2f vAbsMousePos = mpWidget->GetAbsolutePosition() + avMousePos;
  ...
  if (mpActiveSubMenu) {
    cWidgetMenu* pMenu = static_cast<cWidgetMenu*>(
      ni::QueryInterface<iWidgetMenu>(mpActiveSubMenu));
    res = pMenu->ProcessMouseMessage(
      anMsg, vAbsMousePos - mpActiveSubMenu->GetAbsolutePosition());
  }
...
///////////////////////////////////////////////
void cWidgetMenu::ProcessMouseClickResult(const sMouseMessageResult& aRes,
                                          iWidget* apRoot)
{
  if (!aRes.bIntersect) {
    Close();
    ...
    case eWidgetMenuCmd_UnChecked: {
      mpWidget->SendCommand(mpWidget->GetParent(), aRes.Msg,
                            aRes.ptrItem.ptr());
    ...
    case eWidgetMenuCmd_Clicked: {
      ...
      if (niFlagIsNot(nStyle, eWidgetMenuStyle_NoCloseOnItemClicked)) {
        if (niFlagIs(aRes.ptrItem.ptr()->GetFlags(),
                     eWidgetMenuItemFlags_SubMenu))
        {
          ShowSubMenu();
          break;
...
{
  if (mpActiveSubMenu || mnSelectedItem == eInvalidHandle ||
      niFlagIsNot(mvItems[mnSelectedItem]->GetFlags(),
                  eWidgetMenuItemFlags_SubMenu))
    return;
  niAssert(mnSelectedItem != eInvalidHandle);
  niAssert(niFlagIs(mvItems[mnSelectedItem]->GetFlags(),
                    eWidgetMenuItemFlags_SubMenu));
...
void cWidgetMenu::CloseSubMenu()
{
  ...
  if (mpActiveSubMenu) {
    ...
    mpWidget->SendCommand(mpWidget->GetParent(), eWidgetMenuCmd_Closed,
                          mpActiveSubMenu);
    mpActiveSubMenu = NULL;
  }
}
...
tBool __stdcall cWidgetMenu::Close()
{
  ...
  if (ptrWasSelectedItem.IsOK()) {
    mpWidget->SendCommand(mpWidget->GetParent(), eWidgetMenuCmd_FocusedItem, 0,
                          ptrWasSelectedItem.ptr());
  }
...
tBool __stdcall cWidgetMenu::SortItems()
{
  ...
  struct _Local {
    static int compare(const Ptr<iWidgetMenuItem>& aLeft,
                       const Ptr<iWidgetMenuItem>& aRight)
    {
...
void cWidgetMenu::InitSkin()
{
  ...
  skin._kcolBorder =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("Border")));
  skin._kcolBorderSelected = ULColorBuild(
    mpWidget->FindSkinColor(color, NULL, NULL, _H("BorderSelected")));
  skin._kcolSeparator =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("Separator")));
  skin._kcolBackNormal =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("BackNormal")));
  skin._kcolBackSelected = ULColorBuild(
    mpWidget->FindSkinColor(color, NULL, NULL, _H("BackSelected")));
  skin._kcolLeftMargin =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("LeftMargin")));
  skin._kcolBackTitle =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("BackTitle")));
  skin._kcolBorderTitle =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("BorderTitle")));
  skin._kcolTextDisabled = ULColorBuild(
    mpWidget->FindSkinColor(color, NULL, NULL, _H("TextDisabled")));
}
