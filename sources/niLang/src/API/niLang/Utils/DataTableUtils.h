...
///////////////////////////////////////////////
inline Ptr<iDataTable> LoadDataTable(iFile* fp,
                                     const achar* aaszSerFormat = "xml")
{
  niCheckIsOK(fp, NULL);
  Nonnull<iDataTable> dt(ni::GetLang()->CreateDataTable(""));
  if (!ni::GetLang()->SerializeDataTable(aaszSerFormat, eSerializeMode_Read, dt,
                                         fp))
  {
...
///////////////////////////////////////////////
inline Ptr<iDataTable> LoadDataTable(const achar* aURL,
                                     const achar* aaszSerFormat = "xml")
{
...
///////////////////////////////////////////////
inline tBool WriteDataTable(const iDataTable* apDT, const achar* aPath,
                            const achar* aaszSerFormat = "xml")
{
...
///////////////////////////////////////////////
inline Ptr<iDataTable> CreateDataTableFromXML(const achar* aaszText,
                                              tI32 anSize = -1)
{
  if (anSize < 0) {
...
inline cString DataTableToXML(const iDataTable* apDT, tBool abStream = eTrue)
{
  ...
  if (!ni::GetLang()->SerializeDataTable(abStream ? "xml-stream" : "xml",
                                         eSerializeMode_Write,
                                         const_cast<iDataTable*>(apDT), fp))
    return AZEROSTR;
  fp->SeekSet(0);
...
inline cString DataTableToCompressedString(const iDataTable* apDT)
{
...

  {
    ...
    if (!ni::GetLang()->SerializeDataTable(
          _kDataTableCompressedStringMode, eSerializeMode_Write,
          const_cast<iDataTable*>(apDT), fpXml))
      return AZEROSTR;
    fpXml->SeekSet(0);
    fp->WriteLE32(0);
...
///////////////////////////////////////////////
inline Ptr<iDataTable> CreateDataTableFromCompressedString(
  const achar* aaszText)
{
  ...
  Nonnull<iFile> fp(ni::GetLang()->CreateFileDynamicMemory());
  {
    ...
    if (!ni::GetZip()->ZipUncompressBufferInFile(
          fp, decompressedSize, output + 4,
          (tU32)fpDecodedBytes->GetSize() - 4))
      return NULL;
    fp->SeekSet(0);
  }

  Nonnull<iDataTable> dt(ni::GetLang()->CreateDataTable(""));
  if (!ni::GetLang()->SerializeDataTable(_kDataTableCompressedStringMode,
                                         eSerializeMode_Read, dt, fp))
    return NULL;
