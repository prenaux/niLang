...
///
  ...
  #define EASTL_LIST_MAP_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX         \
      " list_map" // Unless the user overrides something, this is "EASTL list_map".
#endif

/// EASTL_MAP_DEFAULT_ALLOCATOR
///
#ifndef EASTL_LIST_MAP_DEFAULT_ALLOCATOR
  #define EASTL_LIST_MAP_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_LIST_MAP_DEFAULT_NAME)
#endif
...
class list_map
    : protected rbtree<
        Key, eastl::list_map_data<eastl::pair<const Key, T>>, Compare,
        Allocator,
        eastl::use_value_first<eastl::list_map_data<eastl::pair<const Key, T>>>,
        true, true> {
 public:
  typedef rbtree<
    Key, eastl::list_map_data<eastl::pair<const Key, T>>, Compare, Allocator,
    eastl::use_value_first<eastl::list_map_data<eastl::pair<const Key, T>>>,
    true, true>
    base_type;
  ...
  typedef typename eastl::pair<const Key, T>
    value_type; // This is intentionally different from base_type::value_type
  typedef value_type& reference;
  typedef const value_type& const_reference;
  typedef typename base_type::node_type
    node_type; // Despite the internal and external values being different, we're keeping the node type the same as the base
    // in order to allow for pool allocation.  See EASTL/map.h for more information.
  typedef
    typename eastl::list_map_iterator<value_type, value_type*, value_type&>
      iterator; // This is intentionally different from base_type::iterator
  typedef typename eastl::list_map_iterator<value_type, const value_type*,
                                            const value_type&>
    const_iterator; // This is intentionally different from base_type::const_iterator
  typedef eastl::reverse_iterator<iterator> reverse_iterator;
  typedef eastl::reverse_iterator<const_iterator> const_reverse_iterator;
  typedef typename base_type::allocator_type allocator_type;
  typedef typename eastl::pair<iterator, bool>
    insert_return_type; // This is intentionally removed, as list_map doesn't support insert() functions, in favor of list like push_back and push_front
  typedef typename eastl::use_first<value_type>
    extract_key; // This is intentionally different from base_type::extract_key
 ...
 protected:
  typedef typename eastl::list_map_data<eastl::pair<const Key, T>>
    internal_value_type;
 ...
 public:
  ...
  list_map(const Compare& compare,
           const allocator_type& allocator = EASTL_MAP_DEFAULT_ALLOCATOR);
...
// map like functionality which list_map does not support, due to abmiguity with list like functionality:
  ...
  list_map(InputIterator first, InputIterator last, const Compare& compare,
  ...
  reverse_iterator erase(reverse_iterator first,
                         reverse_iterator last) = delete;
  ...
  eastl::pair<const_iterator, const_iterator> equal_range(
    const key_type& key) const = delete;

  mapped_type& operator[](const key_type& key) =
    delete; // Of map, multimap, set, and multimap, only map has operator[].
#endif
...
{
  mpNext =
    NULL; // GCC 4.8 is generating warnings about referencing these values in list_map::push_front unless we
  mpPrev =
    NULL; // initialize them here. The compiler seems to be mistaken, as our code isn't actually using them unintialized.
}
...
inline list_map<Key, T, Compare, Allocator>::list_map(
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<
  Key, T, Compare, Allocator>::begin() EA_NOEXCEPT
{
  return iterator(mNode.mpNext);
}

template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<
  Key, T, Compare, Allocator>::begin() const EA_NOEXCEPT
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<
  Key, T, Compare, Allocator>::cbegin() const EA_NOEXCEPT
{
  return const_iterator(mNode.mpNext);
}
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<
  Key, T, Compare, Allocator>::end() EA_NOEXCEPT
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<
  Key, T, Compare, Allocator>::end() const EA_NOEXCEPT
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<
  Key, T, Compare, Allocator>::cend() const EA_NOEXCEPT
{
  return const_iterator(&mNode);
}

template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::reverse_iterator list_map<
  Key, T, Compare, Allocator>::rbegin() EA_NOEXCEPT
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::reverse_iterator list_map<
  Key, T, Compare, Allocator>::rend() EA_NOEXCEPT
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::reference list_map<
  Key, T, Compare, Allocator>::front()
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::const_reference list_map<
  Key, T, Compare, Allocator>::front() const
{
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::const_reference list_map<
  Key, T, Compare, Allocator>::back() const
{
#if EASTL_EMPTY_REFERENCE_ASSERT_ENABLED
    // We allow the user to reference an empty container.
#elif EASTL_ASSERT_ENABLED
...
bool list_map<Key, T, Compare, Allocator>::push_front(const value_type& value)
{
  ...
  typename base_type::insert_return_type baseReturn =
    base_type::insert(tempValue);
...
template <typename Key, typename T, typename Compare, typename Allocator>
bool list_map<Key, T, Compare, Allocator>::push_front(const key_type& key,
...
template <typename Key, typename T, typename Compare, typename Allocator>
bool list_map<Key, T, Compare, Allocator>::push_back(const key_type& key,
                                                     const mapped_type& value)
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<
  Key, T, Compare, Allocator>::find(const key_type& key)
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<
  Key, T, Compare, Allocator>::find(const key_type& key) const
{
...
template <typename U, typename Compare2>
inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<
  Key, T, Compare, Allocator>::find_as(const U& u, Compare2 compare2)
{
...
template <typename U, typename Compare2>
inline typename list_map<Key, T, Compare, Allocator>::const_iterator list_map<
  Key, T, Compare, Allocator>::find_as(const U& u, Compare2 compare2) const
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::size_type list_map<
  Key, T, Compare, Allocator>::count(const key_type& key) const
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::size_type list_map<
  Key, T, Compare, Allocator>::erase(const key_type& key)
{
  typename base_type::iterator baseIter = base_type::find(key);
  if (baseIter != base_type::end()) {
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::iterator list_map<
  Key, T, Compare, Allocator>::erase(const_iterator position)
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
inline typename list_map<Key, T, Compare, Allocator>::reverse_iterator list_map<
  Key, T, Compare, Allocator>::erase(const_reverse_iterator position)
{
...
template <typename Key, typename T, typename Compare, typename Allocator>
int list_map<Key, T, Compare, Allocator>::validate_iterator(
