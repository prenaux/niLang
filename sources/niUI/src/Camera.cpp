...
#define CAMUPDATE()                                                   \
  if (mnDirtyFlags && niFlagIsNot(mnDirtyFlags, CAMDIRTY_UPDATING)) { \
    niThis(cCamera)->_UpdateCamera();                                 \
  }
...
#define CAMVIEWDIRTY() mnDirtyFlags |= CAMDIRTY_VIEW;

#define CAMPROJDIRTY() mnDirtyFlags |= CAMDIRTY_PROJ;
...
class cCamera : public ni::ImplRC<ni::iCamera, ni::eImplFlags_Default> {
 ...
 public:
  ///////////////////////////////////////////////
  cCamera()
      : mFrustum(cGraphics::_CreateFrustum())
  {
    ZeroMembers();
  ...
  void _UpdateCamera()
  {
    ...
    if ((mMoveType != eCameraMoveType_SetMatrix) &&
        (niFlagIs(mnDirtyFlags, CAMDIRTY_VIEW) ||
         niFlagIs(mnDirtyFlags, CAMDIRTY_MOVE)))
    {
      if (niFlagIs(mnDirtyFlags, CAMDIRTY_VIEW)) {
        MatrixLookAtLH(mmtxView, mvPos, mvTarget, mvTargetUp);
      }

      if ((mMoveType != eCameraMoveType_None) &&
          niFlagIs(mnDirtyFlags, CAMDIRTY_MOVE))
      {
    ...
    if ((mProjection != eCameraProjectionType_SetMatrix) &&
        niFlagIs(mnDirtyFlags, CAMDIRTY_PROJ))
    {
      const ni::tF32 aspectRatio = GetAbsoluteAspect();
      if (mProjection == ni::eCameraProjectionType_Orthogonal) {
        MatrixOrthoLH(mmtxProj, mfOrthoSize * aspectRatio, mfOrthoSize,
                      mfNearPlane, mfFarPlane);
      }
      else if (mProjection == ni::eCameraProjectionType_Perspective) {
        MatrixPerspectiveFovLH(mmtxProj, mfFov, aspectRatio, mfNearPlane,
                               mfFarPlane);
      }
    }
                                      ...
                                      const ni::sRectf& aRect)
  {
    ...
    if (mProjection == ni::eCameraProjectionType_Orthogonal) {
      ...
      MatrixOrthoLH(mtxP, (tF64)mfOrthoSize * aspectRatio, (tF64)mfOrthoSize,
                    (tF64)mfNearPlane, (tF64)mfNearPlane + 10000.0);
      mtxVP = mtxV * mtxP;
      ni::VecUnproject(pos, ni::Vec3<ni::tF64>(afX, afY, 0.0),
                       (tF64)rect.GetLeft(), (tF64)rect.GetTop(),
                       (tF64)rect.GetWidth(), (tF64)rect.GetHeight(), mtxVP);
      ...
      // recompute perspective projection with safe clip planes for rays...
      MatrixPerspectiveFovLH(mtxP, (tF64)mfFov, aspectRatio, (tF64)mfNearPlane,
  ...
  ///////////////////////////////////////////////
  virtual ni::sVec3f __stdcall GetScreenPosition(const ni::sVec3f& avPos,
                                                 const ni::sRectf& aRect) const
  {
  ...
  ///////////////////////////////////////////////
  static inline ni::tF32 __stdcall _ComputeAbsoluteAspect(const tF32 afAspect,
                                                          const sRectf& aVP)
  {
    return (afAspect > 0)
             ? afAspect
             : ni::FDiv((ni::tF32)aVP.GetWidth(), (ni::tF32)aVP.GetHeight());
  }
  ...
  void __stdcall _SetMoveType(ni::eCameraMoveType aMode)
  {
    ...
    case ni::eCameraMoveType_Fly: niFlagOn(mnMoveMode, CAMMOVE_TARGET); break;
    case ni::eCameraMoveType_FlyFixedTarget: break;
    case ni::eCameraMoveType_Orbit: niFlagOn(mnMoveMode, CAMMOVE_ORBIT); break;
    }
    mvMove = ni::sVec3f::Zero();
    CAMVIEWDIRTY();
    CAMMOVEDIRTY();
