...
struct VGLine : public VGBase {
  ...
  void _DrawMain(iCanvas* apCanvas) niImpl
  {
    ...
    apCanvas->GetVGMaterial()->SetRasterizerStates(
      mbWireframe ? eCompiledStates_RS_Wireframe
...
struct Icons : public VGBase {
  ...
  void _DrawMain(iCanvas* apCanvas) niImpl
  {
    ...
    fontIcons->SetSizeAndResolution(
      Vec2f(32, 32), 32, mpWidget->GetUIContext()->GetContentsScale());
    ...
    sVec2f glyphAdvance =
      fontIcons->GetGlyphPath(mptrPath, gi, pos + Vec2f(0, ascent), 1.0f);

    // Draw the glyph's box
    apCanvas->BlitFill(Rectf(pos.x, pos.y, (tF32)fontIcons->GetResolution(),
                             (tF32)fontIcons->GetResolution()),
                       0xCCCCCC);
    ...
    apCanvas->BlitText(fontIcons, Rectf(5, 5, 0, 0), 0,
                       "Hi \uF002\uF004\uF040!");

    Ptr<iTextObject> textObject = mpWidget->GetGraphics()->CreateTextObject(
      "<font size='32' res='32' color='black'>Hi <font name='fas' color='darkgreen'><g name='magnifying-glass'/>Foo<g name='heart'/></font> Bar!",
      Vec2f(500, 500), mpWidget->GetUIContext()->GetContentsScale());
    textObject->DrawAt(apCanvas, sRectf::Null(), Vec3f(pos.x, pos.y + 120, 0));

    static tBool _once = eTrue;
    if (_once) {
      niDebugFmt(("... gi: %d, res: %s, adv: %s, path: %s", gi,
                  fontIcons->GetResolution(), glyphAdvance,
                  mptrPath->GetSVGPath()));
      _once = eFalse;
    }
