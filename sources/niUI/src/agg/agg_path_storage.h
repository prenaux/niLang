 ...
 public:
  ...
  class vertex_source {
   public:
    ...
    vertex_source(const path_storage& p)
        : m_path(&p)
        , m_vertex_idx(0)
    {
    }
  ...
  void arc_to(agg_real rx, agg_real ry, agg_real angle, bool large_arc_flag,
              bool sweep_flag, agg_real x, agg_real y);

  void arc_rel(agg_real rx, agg_real ry, agg_real angle, bool large_arc_flag,
               bool sweep_flag, agg_real dx, agg_real dy);

  void curve3(agg_real x_ctrl, agg_real y_ctrl, agg_real x_to, agg_real y_to);

  void curve3_rel(agg_real dx_ctrl, agg_real dy_ctrl, agg_real dx_to,
                  agg_real dy_to);
  ...
  void curve4(agg_real x_ctrl1, agg_real y_ctrl1, agg_real x_ctrl2,
              agg_real y_ctrl2, agg_real x_to, agg_real y_to);

  void curve4_rel(agg_real dx_ctrl1, agg_real dy_ctrl1, agg_real dx_ctrl2,
                  agg_real dy_ctrl2, agg_real dx_to, agg_real dy_to);

  void curve4(agg_real x_ctrl2, agg_real y_ctrl2, agg_real x_to, agg_real y_to);

  void curve4_rel(agg_real x_ctrl2, agg_real y_ctrl2, agg_real x_to,
                  agg_real y_to);
  ...
  void add_poly(const agg_real* vertices, unsigned num, bool solid_path = false,
  ...
  unsigned arrange_polygon_orientation(unsigned start,
  ...
  {
    agg_real* pv =
      m_coord_blocks[idx >> block_shift] + ((idx & block_mask) << 1);
    *pv++ = x;
    *pv = y;
  }
