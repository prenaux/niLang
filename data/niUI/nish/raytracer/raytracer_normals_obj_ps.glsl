...
nish_std_PixelOutput niUIGpuFuncs_raytracer_normals_obj_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  if (_tmp_K1) {
    ...
    if (_tmp_N1) {
    ...
    else {
      {
        ...
        vec3 n = niUIGpuFuncs_Lerp_4_Vec3_Vec3_Vec3_Vec3(v0.normal,v1.normal,v2.normal,bary);
        vec3 _tmp_D2 = niUIGpuFuncs_NormalToColor(n);
        color = vec4(_tmp_D2.x,_tmp_D2.y,_tmp_D2.z,1.0);
      }
    }
  }
  else {
    bool _tmp_F2 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_F2) {
  ...
  vec4 _tmp_S2 = color;
  return nish_std_PixelOutput_new(_tmp_S2);
}
