...
niGlobalCommand(Mem, _A("Show the memory usage."))
{
  ...
  o.Format("--- MemStats ---\n"
           ...
           ni::GetLang()->GetFrameNumber(), ms.x, ms.y, ms.x - ms.y, ms.z, ms.w,
           ms.z - ms.w);
  niLog(Info, o.Chars());
  ni_mem_dump_report();
  return eTrue;
...
niGlobalCommand(ListNamespaces, _A("Show the list of the namespaces."))
{
  ...
  for (cConsole::tNamespaceMapIt it =
         static_cast<cConsole*>(apConsole)->mmapNamespaces.begin();
       it != static_cast<cConsole*>(apConsole)->mmapNamespaces.end(); ++it)
  {
...
niGlobalCommand(ListCommands, _A("Show the list of the commands."))
{
  ...
  if (niCommandNumArgs() >= 2) {
    cConsole::sNamespace* pNS =
      static_cast<cConsole*>(apConsole)->GetNamespace(niCommandArg(1).Chars());
    if (!pNS) {
      niLog(Info,
            niFmt(_A("Can't find namespace '%s'"), niCommandArg(1).Chars()));
    }
    else {
      niLog(Info, niFmt(_A("%s:"), niCommandArg(1).Chars()));
      for (cConsole::tCommandMapIt itCmd = pNS->mapCommands.begin();
           itCmd != pNS->mapCommands.end(); ++itCmd)
      {
        iCommandSink* pSink = itCmd->second;
        niLog(Info, niFmt(_A("  %s - %s"), pSink->GetName(),
                          pSink->GetDescription()));
      }
    }
  }
  else {
    cConsole::sNamespace* pNS =
      &static_cast<cConsole*>(apConsole)->mGlobalNamespace;
    niLog(Info, _A("Global:"));
    cConsole::tCommandMapIt itCmd;
    for (itCmd = pNS->mapCommands.begin(); itCmd != pNS->mapCommands.end();
         ++itCmd)
    {
      iCommandSink* pSink = itCmd->second;
      niLog(Info,
            niFmt(_A("  %s - %s"), pSink->GetName(), pSink->GetDescription()));
    }
...
niGlobalCommand(ListVariables, _A("Show the list of the variables."))
{
  ...
  if (niCommandNumArgs() >= 2) {
    ...
    if (!pNS) {
      ...
      niLog(Info, niFmt(_A("%s:"), niCommandArg(1).Chars()));
      for (cConsole::tStringMap::iterator itVar = pNS->mapVars.begin();
           itVar != pNS->mapVars.end(); ++itVar)
      {
        niLog(Info, niFmt(_A("  %s = %s"), itVar->first.Chars(),
                          itVar->second.Chars()));
      }
    }
  }
...
#if niMinFeatures(15)
///////////////////////////////////////////////
niGlobalCommand(ListScriptingHosts,
                _A("Show the list of the registered Scripting Hosts."))
{
...
niGlobalCommand(Set, _A("Set the value of a variable."))
{
  if (niCommandNumArgs() < 3) {
    ...
    niLog(Info, niFmt(_A("    %s variable_name variable_value"),
                      niCommandArg(0).Chars()));
    niLog(Info, _A(""));
  }
  else {
    if (!apConsole->SetVariable(niCommandArg(1).Chars(),
                                niCommandArg(2).GetWithoutBEQuote().Chars()))
      niLog(Info,
            niFmt(_A("Can't set variable '%s'."), niCommandArg(1).Chars()));
  }
...
void RegisterConsoleCommands_Global(iConsole* apConsole)
{
  ...
  apConsole->AddCommand(
    niCreateGlobalCommandInstance(CommandDescription, apConsole));
  ...
  apConsole->AddCommand(
    niCreateGlobalCommandInstance(ListNamespaces, apConsole));
  apConsole->AddCommand(
    niCreateGlobalCommandInstance(ListVariables, apConsole));
#if niMinFeatures(15)
  apConsole->AddCommand(
    niCreateGlobalCommandInstance(ListScriptingHosts, apConsole));
#endif
