...
struct sVGPathStorage {
 ...
 public:
  ...
  void curve4(ni::tF32 x_ctrl1, ni::tF32 y_ctrl1, ni::tF32 x_ctrl2,
              ni::tF32 y_ctrl2, ni::tF32 x_to, ni::tF32 y_to)
  {
...
int font_engine_freetype_base::find_face(const char* face_name) const
{
  unsigned i;
  for (i = 0; i < m_num_faces; ++i) {
    if (strcmp(face_name, m_face_names[i]) == 0)
      return i;
  }
  return -1;
}
                                          ...
                                          ni::iFile* apFile)
{
  ...
  if (idx >= 0) {
    ...
    if (m_num_faces >= m_max_faces) {
      ...
      memcpy(m_face_names, m_face_names + 1, (m_max_faces - 1) * sizeof(char*));
    ...
    if (apFile) {
      ...
      m_last_error = FT_New_Face(mptrLibrary->mLibrary, font_name, face_index,
                                 &m_faces[m_num_faces]);
    }
...
{
  switch (ren_type) {
  case glyph_ren_native_mono: m_glyph_rendering = glyph_ren_native_mono; break;
...
#if niMinFeatures(20)
//------------------------------------------------------------------------
bool font_engine_freetype_base::get_glyph_path(
  unsigned glyph_code, bool flip_y, bool hinting, const trans_affine& mtx,
  ni::iVGPath* apPath, agg_real* advance_x, agg_real* advance_y,
  agg_real* real_width, agg_real* real_height)
{
  ...
  unsigned last_error = FT_Load_Glyph(
    m_cur_face, glyph_index, hinting ? FT_LOAD_DEFAULT : FT_LOAD_NO_HINTING);
      ...
      if (m_last_error == 0) {
        if (m_flag32) {
                                   ...
                                   m_affine, m_path32))
          {
            ...
            m_advance_y = int26p6_to_dbl(m_cur_face->glyph->advance.y);
            m_bearing_x =
              int26p6_to_dbl(m_cur_face->glyph->metrics.horiBearingX);
            m_bearing_y =
              int26p6_to_dbl(m_cur_face->glyph->metrics.vertBearingY);
            m_real_width = int26p6_to_dbl(m_cur_face->glyph->metrics.width);
            m_real_height = int26p6_to_dbl(m_cur_face->glyph->metrics.height);
