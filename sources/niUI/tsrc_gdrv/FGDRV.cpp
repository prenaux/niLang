...
struct FGDRV_WindowHandler : public ImplRC<iMessageHandler> {
  ...
  void __stdcall HandleMessage(const tU32 anMsg, const Var& a,
                               const Var& b) niImpl
  {
    if (!_context)
      return;
    ...
    case eOSWindowMessage_KeyDown:
      TRACE_OSWINDOW(("eOSWindowMessage_KeyDown: %d (%s)\n", a.mU32,
                      niEnumToChars(eKey, a.mU32)));
      if (!_context->OnKeyDown(a.mU32, _debugKeyMod)) {
        switch (a.mU32) {
        case eKey_LControl: niFlagOn(_debugKeyMod, eKeyMod_Control); break;
        case eKey_LAlt: niFlagOn(_debugKeyMod, eKeyMod_Alt); break;
        case eKey_LShift: niFlagOn(_debugKeyMod, eKeyMod_Shift); break;
        case eKey_F:
          wnd->SetFullScreen(
            wnd->GetFullScreen() == eInvalidHandle ? 0 : eInvalidHandle);
          break;
        case eKey_Z: wnd->SetCursorPosition(sVec2i::Zero()); break;
        case eKey_X: wnd->SetCursorPosition(wnd->GetClientSize() / 2); break;
        case eKey_C: wnd->SetCursorPosition(wnd->GetClientSize()); break;
        case eKey_Q: wnd->SetCursor(eOSCursor_None); break;
        case eKey_W: wnd->SetCursor(eOSCursor_Arrow); break;
        case eKey_E: wnd->SetCursor(eOSCursor_Text); break;
        case eKey_K: wnd->SetCursorCapture(!wnd->GetCursorCapture()); break;
        case eKey_Space: {
    ...
    case eOSWindowMessage_KeyUp:
      TRACE_OSWINDOW(("eOSWindowMessage_KeyUp: %d (%s)\n", a.mU32,
                      niEnumToChars(eKey, a.mU32)));
      switch (a.mU32) {
      case eKey_LControl: niFlagOff(_debugKeyMod, eKeyMod_Control); break;
      case eKey_LAlt: niFlagOff(_debugKeyMod, eKeyMod_Alt); break;
      case eKey_LShift: niFlagOff(_debugKeyMod, eKeyMod_Shift); break;
      }
      break;
...
tBool sFGDRV_Base::Start(UnitTest::TestResults& testResults_)
{
  ...
  _mq = niCheckNN(
    _mq, ni::GetOrCreateMessageQueue(ni::ThreadGetCurrentThreadID()), eFalse);

  _windowHandler =
    niCheckNN(_windowHandler, niNew FGDRV_WindowHandler(this), eFalse);
  ...
  niLog(Info,
        niFmt("Start: %s, r: %s, monitorRect: %s", m_testName, r, monitorRect));

  // Create the window
  _window = niCheckNN(_window,
                      ni::GetLang()->CreateWindow(nullptr, m_testName, r, 0,
  ...
  _graphics = niCheckNN(
    _graphics,
    (iGraphics*)niCreateInstance(niUI, Graphics, niVarNull, niVarNull), eFalse);
  niCheck(_graphics->InitializeDriver(_gdrvName), eFalse);

  _graphicsContext = niCheckNN(
    _graphicsContext,
    _graphics->CreateContextForWindow(_window, _A("R8G8B8A8"), _A("D24S8"),
                                      _swapInterval, _backBufferFlags),
    eFalse);
...
tBool sFGDRV_Base::Step(UnitTest::TestResults& testResults_)
{
  ...
  FGDRV_WindowHandler* windowHandler =
    (FGDRV_WindowHandler*)_windowHandler.raw_ptr();
...
{
  _window->SetTitle(
    niFmt("%s (%s)", m_testName, _graphics->GetDriver()->GetName()));
  return eTrue;
}
...
tBool sFGDRV_Base::BeforePaint(UnitTest::TestResults& testResults_)
{
  ...
  this->_graphicsContext->ClearBuffers(this->_clearBuffers, this->_clearColor,
                                       1.0f, 0);
...
tBool sFGDRV_Base::AfterPaint(UnitTest::TestResults& testResults_)
{
  ...
  cString fpsText = niFmt("%s (%s) - %gfps %dms %dalloc %dobj", m_testName,
                          _graphics->GetDriver()->GetName(),
