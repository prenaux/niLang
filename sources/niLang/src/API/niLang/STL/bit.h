...
static inline ni::tU32 countl_zero(T x)
{
  if (x == 0)
    return sizeof(T) * 8;
  static_assert(eastl::is_unsigned<T>::value,
                "countl_zero requires unsigned integer type");
  static_assert(sizeof(T) <= sizeof(ni::tU64),
                "countl_zero type must be <= ni::tU64");
