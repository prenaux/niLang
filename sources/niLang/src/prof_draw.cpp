...
int Prof_get_colors(float factor, float text_color_ret[3],
                    float glow_color_ret[3], float* glow_alpha_ret)
{
...
static void graph_func(int id, int x0, int x1, float* values, void* data)
{
  ...
  while (x1 > x0 && (values[x1 - 1 - x0] < 0.0002f))
    --x1;
...
void Prof_draw_graph(iProfDraw* drawer, float sx, float sy, float x_spacing,
                     float y_spacing)
{
  #ifdef Prof_ENABLED
...
void Prof_draw(iProfDraw* drawer, float sx, float sy, float full_width,
               float height, int precision)
{
  #ifdef Prof_ENABLED
  ...
  for (i = 0; i < NUM_TITLE; ++i) {
    if (pob->title[i]) {
      drawer->Text(sx + 2, sy, pob->title[i],
  ...
  for (j = 1; j < NUM_HEADER; ++j) {
    if (pob->header[j]) {
      drawer->Text(sx + name_width + field_width * (j - 1) + field_width / 2 -
                     drawer->GetTextWidth(pob->header[j]) / 2,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (Prof_get_colors((float)r->heat, text_color, glow_color, &glow_alpha)) {
      const tU32 gc =
        ULColorBuildf(glow_color[0], glow_color[1], glow_color[2], glow_alpha);
      drawer->Text(x + 2, sy - 1, buf, gc);
    }

    const tU32 textColor =
      ULColorBuildf(text_color[0], text_color[1], text_color[2], 1.0f);
    ...
    for (j = 0; j < NUM_VALUES; ++j) {
      if (r->value_flag & (1 << j)) {
        ...
        if (r->indent)
          pad += (int)plus_width;
        drawer->Text(sx + pad + name_width + field_width * j, sy, buf,
                     textColor);
      }
    }
...
// Make sure you're in 2d rendering mode before calling this function
void Prof_draw_text(void* context, float sx, float sy, float full_width,
                    float height, float line_spacing, int precision,
                    void (*begin_rects)(void* context),
                    void (*end_rects)(void* context),
                    void (*draw_rectangle)(void* context, float x0, float y0,
                                           float x1, float y1, sVec4f& avCol),
                    void (*beginText)(void* c), void (*endText)(void* c),
                    void (*printText)(void* c, float x, float y,
                                      const char* str, const sColor4f& avCol),
                    float (*textWidth)(void* c, const char* str))
{
  ...
  if (draw_rectangle) {
    ...
    for (i = 0; i < NUM_TITLE; ++i) {
      if (pob->title[i]) {
        ...
        draw_rectangle(context, header_x0, sy - 2, header_x1,
                       sy + line_spacing + 2, c);
        sy += 1.5f * line_spacing;
        height -= ni::Abs(line_spacing) * 1.5f;
      }
  ...
  if (draw_rectangle) {
    ...
    draw_rectangle(context, sx, sy, sx + full_width, sy + line_spacing,
                   rectCol);
    sy += line_spacing;
    for (i = 0; i < n; i++) {
      float y0, y1;
  ...
  for (j = 1; j < NUM_HEADER; ++j) {
    if (pob->header[j]) {
      printText(context,
                sx + name_width + field_width * (j - 1) + field_width / 2 -
                  textWidth(context, pob->header[j]) / 2,
                sy, pob->header[j], c);
    }
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < NUM_VALUES; ++j) {
      if (r->value_flag & (1 << j)) {
        ...
        if (r->indent)
          pad += (int)plus_width;
        printText(context, sx + pad + name_width + field_width * j, sy, buf, c);
      }
    }
