...
struct sFGpu_Triangle : public sFGpu_Base {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
      ...
      _vaBuffer =
        niCheckNN(_vaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTriangle_VA"), sizeof(tVertexFmt) * 3,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                  eFalse);
      tVertexFmt* verts =
        (tVertexFmt*)_vaBuffer->Lock(0, _vaBuffer->GetSize(), eLock_Discard);
...

    {
      _vertexGpuFun = niCheckNN(
        ...
        _driverGpu->CreateGpuFunction(
          eGpuFunctionType_Vertex, _H("test/nish/gpu/triangle_vs.gpufunc.xml")),
        eFalse);
      _pixelGpuFun = niCheckNN(
        _pixelGpuFun,
        _driverGpu->CreateGpuFunction(
          eGpuFunctionType_Pixel, _H("test/nish/gpu/triangle_ps.gpufunc.xml")),
        eFalse);
    }

    {
      ...
      _pipeline = niCheckNN(
        _pipeline,
        _driverGpu->CreateGpuPipeline(_H("GpuTriangle_Pipeline"), pipelineDesc),
        eFalse);
    }
...
struct sFGpu_TriangleViewport : public sFGpu_Triangle {
  niFn(tBool) OnPaint(UnitTest::TestResults& testResults_) niImpl
  {
    ...
    gpuContext->ClearBuffersRect(eClearBuffersFlags_ColorDepthStencil,
                                 Rectf(0, 0, (tF32)w, (tF32)h), 0xFFFF0000,
                                 1.0f);
    cmdEncoder->SetPipeline(_pipeline);
    cmdEncoder->SetVertexBuffer(_vaBuffer, 0, 0);
    cmdEncoder->Draw(eGraphicsPrimitiveType_TriangleList, 0, 1, 0, 3);
    ...
    gpuContext->ClearBuffersRect(eClearBuffersFlags_ColorDepthStencil,
                                 Rectf(0, 0, (tF32)w, (tF32)h), 0xFF00FF00,
                                 1.0f);
    cmdEncoder->SetPipeline(_pipeline);
    cmdEncoder->SetVertexBuffer(_vaBuffer, 0, 0);
    cmdEncoder->Draw(eGraphicsPrimitiveType_TriangleList, 0, 1, 0, 3);
    ...
    gpuContext->ClearBuffersRect(eClearBuffersFlags_ColorDepthStencil,
                                 Rectf(0, 0, (tF32)w, (tF32)h), 0xFF0000FF,
                                 1.0f);
    cmdEncoder->SetPipeline(_pipeline);
    cmdEncoder->SetVertexBuffer(_vaBuffer, 0, 0);
    cmdEncoder->Draw(eGraphicsPrimitiveType_TriangleList, 0, 1, 0, 3);
    ...
    gpuContext->ClearBuffersRect(eClearBuffersFlags_ColorDepthStencil,
                                 Rectf(0, 0, (tF32)w, (tF32)h), 0xFFFFFFFF,
                                 1.0f);
    cmdEncoder->SetPipeline(_pipeline);
    cmdEncoder->SetVertexBuffer(_vaBuffer, 0, 0);
    cmdEncoder->Draw(eGraphicsPrimitiveType_TriangleList, 0, 1, 0, 3);
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niImpl
  {
...

    {
      _vaBuffer =
        niCheckNN(_vaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuSquare_VA"), sizeof(tVertexFmt) * 4,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                  eFalse);
      tVertexFmt* verts =
        (tVertexFmt*)_vaBuffer->Lock(0, _vaBuffer->GetSize(), eLock_Discard);
    ...
    }
    {
      _iaBuffer =
        niCheckNN(_iaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuSquare_IA"), sizeof(tU32) * 6,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Index),
                  eFalse);
      tU32* inds =
        (tU32*)_iaBuffer->Lock(0, _iaBuffer->GetSize(), eLock_Discard);
...

    {
      ...
      _pipeline = niCheckNN(
        _pipeline,
        _driverGpu->CreateGpuPipeline(_H("GpuSquare_Pipeline"), pipelineDesc),
        eFalse);
    }
  ...
  tBool OnInit(UnitTest::TestResults& testResults_) niOverride
  {
...

    {
      _vaBuffer =
        niCheckNN(_vaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTexture_VA"), sizeof(tVertexFmt) * 4,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                  eFalse);
      tVertexFmt* verts =
        (tVertexFmt*)_vaBuffer->Lock(0, _vaBuffer->GetSize(), eLock_Discard);
      niCheck(verts != nullptr, eFalse);
      verts[0] = {
        { -0.5f, 0.5f, 0.0f }, sVec3f::YAxis(), 0xFFFF0000, { 0.0f, 0.0f }
      }; // Red, TL
      verts[1] = {
        { 0.5f, 0.5f, 0.0f }, sVec3f::YAxis(), 0xFF00FF00, { 1.0f, 0.0f }
      }; // Green, TR
      verts[2] = {
        { 0.5f, -0.5f, 0.0f }, sVec3f::YAxis(), 0xFF0000FF, { 1.0f, 1.0f }
      }; // Blue, BR
      verts[3] = {
        { -0.5f, -0.5f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 0.0f, 1.0f }
      }; // White, BL
      _vaBuffer->Unlock();
    }
    {
      _iaBuffer =
        niCheckNN(_iaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTexture_IA"), sizeof(tU32) * 6,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Index),
                  eFalse);
      tU32* inds =
        (tU32*)_iaBuffer->Lock(0, _iaBuffer->GetSize(), eLock_Discard);
...

    {
      ...
      _pixelGpuFun = niCheckNN(
        _pixelGpuFun,
        _driverGpu->CreateGpuFunction(
          eGpuFunctionType_Pixel, _H("test/nish/gpu/texture_ps.gpufunc.xml")),
        eFalse);
    }
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niImpl
  {
...

    {
      _vaBuffer =
        niCheckNN(_vaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTexAlpha_VA"), sizeof(tVertexFmt) * 4,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                  eFalse);
      tVertexFmt* verts =
        (tVertexFmt*)_vaBuffer->Lock(0, _vaBuffer->GetSize(), eLock_Discard);
      ...
      verts[1] = {
        { 0.5f, 0.5f, 0.0f }, sVec3f::YAxis(), 0xFF00FF00, { 3.0f, 0.0f }
      }; // Green, TR
      verts[2] = {
        { 0.5f, -0.5f, 0.0f }, sVec3f::YAxis(), 0xFF0000FF, { 3.0f, 3.0f }
      }; // Blue, BR
      verts[3] = {
        { -0.5f, -0.5f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 0.0f, 3.0f }
      }; // White, BL
      _vaBuffer->Unlock();
    }
    {
      _iaBuffer =
        niCheckNN(_iaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTexAlpha_IA"), sizeof(tU32) * 6,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Index),
                  eFalse);
      tU32* inds =
        (tU32*)_iaBuffer->Lock(0, _iaBuffer->GetSize(), eLock_Discard);
      ...
      _vertexGpuFun =
        niCheckNN(_vertexGpuFun,
                  _driverGpu->CreateGpuFunction(
                    ...
                    _H("test/nish/gpu/texture_alphatest_vs.gpufunc.xml")),
                  eFalse);
      _pixelGpuFun =
        niCheckNN(_pixelGpuFun,
                  _driverGpu->CreateGpuFunction(
                    eGpuFunctionType_Pixel,
                    _H("test/nish/gpu/texture_alphatest_ps.gpufunc.xml")),
                  eFalse);
    }

    {
      ...
      _pipeline = niCheckNN(
        _pipeline,
        _driverGpu->CreateGpuPipeline(_H("GpuTexAlpha_Pipeline"), pipelineDesc),
        eFalse);
    }
...
struct sFGpu_TexAlphaUBuffer : public sFGpu_TexAlphaBase {
    ...
    _uBuffer =
      niCheckNN(_uBuffer,
                _driverGpu->CreateGpuBufferFromDataRaw(
                  _H("GpuTexAlpha_TestUniforms"), (tPtr)&ubInit,
                  sizeof(TestGpuFuncs_TestUniforms),
                  eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Uniform),
                eFalse);
    return eTrue;
  }
    ...
    niOverride
  {
    ...
    {
      uBuffer->materialColor =
        sVec4f::One() * ((1.0f - _pingpongTime) + 0.1f) * 3.0f;
      uBuffer->alphaRef =
        _pingpongTime -
        0.05f; // 0.05 so that we have a full "no alpha test" mode
...
struct sFGpu_ClearRects : public sFGpu_Base {
  ...
  niFn(tBool) OnPaint(UnitTest::TestResults& testResults_) niImpl
  {
    ...
    niVar gpuCtx =
      niCheckNN(gpuCtx, QueryInterface<iGraphicsContextGpu>(ctx), eFalse);
    ...
    gpuCtx->ClearBuffersRect(
      ...
      Rectf(border, border, (tF32)w - (2 * border), (tF32)h - (2 * border)),
      0xFF333333, 0.0f);

    gpuCtx->ClearBuffersRect(eClearBuffersFlags_ColorDepthStencil,
                             Rectf(50, 50, (tF32)w - 100, (tF32)h - 100),
                             0xFF00FF00, 0.0f);
...
struct sFGpu_RenderTarget : public sFGpu_Texture {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niImpl
  {
    ...
    _rtTex = niCheckNN(_rtTex,
                       _graphics->CreateTexture(_H("rtTex"), eBitmapType_2D,
                                                "R8G8B8A8", 0, 256, 256, 0,
                                                eTextureFlags_RenderTarget),
                       eFalse);
    _rtGC = niCheckNN(_rtGC,
                      _graphics->CreateContextForRenderTargets(
                        _rtTex, nullptr, nullptr, nullptr, nullptr),
                      eFalse);
    return eTrue;
  }
  ...
  {
    {
      ...
      _rtGC->ClearBuffers(eClearBuffersFlags_ColorDepthStencil, 0xFFFF0000,
                          1.0f, 0);
      ...
      rtEncoder->DrawIndexed(eGraphicsPrimitiveType_TriangleList, 0, 1, 0, 0,
                             6);
...
struct sTest_DepthTest {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_,
                     ain<nn<iGraphicsDriverGpu>> aDriverGpu)
  {
      ...
      _vaBuffer =
        niCheckNN(_vaBuffer,
                  aDriverGpu->CreateGpuBuffer(
                    _H("GpuSquare_VA"), sizeof(tVertexFmt) * 12,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                  eFalse);
      tVertexFmt* verts =
        (tVertexFmt*)_vaBuffer->Lock(0, _vaBuffer->GetSize(), eLock_Discard);
    ...
    }
    {
      _iaBuffer =
        niCheckNN(_iaBuffer,
                  aDriverGpu->CreateGpuBuffer(
                    _H("GpuSquare_IA"), sizeof(tU32) * 18,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Index),
                  eFalse);
      tU32* inds =
        (tU32*)_iaBuffer->Lock(0, _iaBuffer->GetSize(), eLock_Discard);
    ...
    {
      NN<iGpuPipelineDesc> pipelineDesc =
        niCheckNN(pipelineDesc, aDriverGpu->CreateGpuPipelineDesc(), eFalse);
      pipelineDesc->SetFVF(tVertexFmt::eFVF);
      pipelineDesc->SetColorFormat(0, eGpuPixelFormat_BGRA8);
      pipelineDesc->SetDepthFormat(eGpuPixelFormat_D32);
      pipelineDesc->SetFunction(eGpuFunctionType_Vertex, _vertexGpuFun);
      ...
      _pipeline = niCheckNN(
        _pipeline,
        aDriverGpu->CreateGpuPipeline(_H("GpuSquare_Pipeline"), pipelineDesc),
        eFalse);
    }
  ...
  niFn(tBool) OnPaint(UnitTest::TestResults& testResults_,
                      ain<nn<iGraphicsContextGpu>> gpuContext)
  {
    ...
    cmdEncoder->SetVertexBuffer(_vaBuffer, 0, 0);
    cmdEncoder->SetIndexBuffer(_iaBuffer, 0, eGpuIndexType_U32);
    cmdEncoder->DrawIndexed(eGraphicsPrimitiveType_TriangleList, 0, 1, 0, 0,
                            18);
    return eTrue;
  }
};
...
struct sFGpu_DepthTest : public sFGpu_Base {
  ...
  niFn(tBool) OnPaint(UnitTest::TestResults& testResults_) niImpl
  {
    QPtr<iGraphicsContextGpu> gpuContext = _graphicsContext;
    niPanicAssert(gpuContext.IsOK());
    CHECK_RET(_testDepthTest.OnPaint(testResults_, gpuContext.non_null()),
              eFalse);
    return eTrue;
  }
};
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niImpl
  {
    ...
    _rtTex = niCheckNN(_rtTex,
                       _graphics->CreateTexture(_H("rtTex"), eBitmapType_2D,
                                                ...
                                                eTextureFlags_RenderTarget),
                       eFalse);
    _dsTex = niCheckNN(_rtTex,
                       _graphics->CreateTexture(_H("dsTex"), eBitmapType_2D,
                                                "D32", 0, 256, 256, 0,
                                                eTextureFlags_DepthStencil),
                       eFalse);
    _rtGC = niCheckNN(_rtGC,
                      _graphics->CreateContextForRenderTargets(
                        _rtTex, nullptr, nullptr, nullptr, _dsTex),
                      eFalse);
    return eTrue;
  }
  ...
  tBool OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Create quad scaled larger to show mips
    {
      _vaBuffer =
        niCheckNN(_vaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTextureMip_VA"), sizeof(tVertexFmt) * 4,
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                  eFalse);
      ...
      niCheck(verts != nullptr, eFalse);
      verts[0] = {
        { -2.0f, 2.0f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 0.0f, 0.0f }
      };
      verts[1] = {
        { 2.0f, 2.0f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 8.0f, 0.0f }
      };
      verts[2] = {
        { 2.0f, -2.0f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 8.0f, 8.0f }
      };
      verts[3] = {
        { -2.0f, -2.0f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 0.0f, 8.0f }
      };
      _vaBuffer->Unlock();
    }
      ...
      _texture =
        niCheckNN(_texture,
                  _graphics->CreateTextureFromRes(
                    _H("test/tex/mipmaps.dds"), nullptr, eTextureFlags_Default),
                  eFalse);
    }
...
struct sFGpu_TextureCube : public sFGpu_Base {
  ...
  tBool OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Create cube vertices (standard skybox cube)
    {
      ...
      _vaBuffer =
        niCheckNN(_vaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTextureCube_VA"), sizeof(kCubeVertices),
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                  eFalse);
    ...
    // Create indices for the cube
    {
      ...
      _iaBuffer =
        niCheckNN(_iaBuffer,
                  _driverGpu->CreateGpuBuffer(
                    _H("GpuTextureCube_IA"), sizeof(kCubeIndices),
                    eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Index),
                  eFalse);
...
#if 1
    {
      ...
      for (tU32 i = 0; i < 6; ++i) {
        faces[i] = (iBitmap2D*)_graphics->LoadBitmapFromRes(
          _H(kFaces[i].filename), nullptr);
        niCheck(niIsOK(faces[i]), eFalse);
      }

      // Create cube with size from first face
      niLet cubeSize = faces[0]->GetWidth();
      Ptr<iBitmapCube> bmpCube =
        _graphics->CreateBitmapCube(cubeSize, "R8G8B8A8");
      ...
      for (tU32 i = 0; i < 6; ++i) {
        ...
        cubeFace->BlitStretch(faces[i], 0, 0, 0, 0, faces[i]->GetWidth(),
                              faces[i]->GetHeight(), cubeFace->GetWidth(),
                              cubeFace->GetHeight());
      }

      _cubemap = AsNN(_graphics->CreateTextureFromBitmap(
        _H("CubeMap"), bmpCube, eTextureFlags_Default));
    }
#else
    {
      ...
      _cubemap = niCheckNN(
        ...
        _graphics->CreateTextureFromRes(
          ...
          _H("test/tex/Beach_cubemap_128.dds"), nullptr, eTextureFlags_Default),
        eFalse);
    }
#endif
    ...
    // Create shaders
    {
      _vertexGpuFun = niCheckNN(
        _vertexGpuFun,
        _driverGpu->CreateGpuFunction(
          eGpuFunctionType_Vertex, _H("test/nish/gpu/skybox_vs.gpufunc.xml")),
        eFalse);
      _pixelGpuFun = niCheckNN(
        _pixelGpuFun,
        _driverGpu->CreateGpuFunction(
          eGpuFunctionType_Pixel, _H("test/nish/gpu/skybox_ps.gpufunc.xml")),
        eFalse);
    }
    ...
    // Create pipeline
    {
      ...
      _pipeline = niCheckNN(_pipeline,
                            _driverGpu->CreateGpuPipeline(
                              _H("GpuTextureCube_Pipeline"), pipelineDesc),
                            eFalse);
    }
  ...
  tBool OnPaint(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    MatrixPerspectiveFovLH(persp, niRadf(110),
                           ni::FDiv((tF32)_graphicsContext->GetWidth(),
