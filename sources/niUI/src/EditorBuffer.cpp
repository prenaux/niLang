...
void EditorBuffer::DeleteSelectionContent(const Selection& sel)
{
  if (sel._start._line == sel._end._line) {
    ...
    if (toremove > 0) {
      _lines.erase(_lines.begin() + startremove,
                   _lines.begin() + (startremove + toremove));
    }
    //erase the end start
...
bool EditorBuffer::GetSelectionData(const Selection& sel, cString& dest) const
{
  ...
  if (sel._start._line == sel._end._line) {
    ...
    dest.appendEx(&line.GetChars()[selStartOffset],
    ...
    dest.appendEx(&start.GetChars()[selStartOffset],
                  start.GetSizeInBytes() - selStartOffset);
...
{
  return GetLineCount()
           ? Position(GetLineCount() - 1, GetLineLen(GetLineCount() - 1))
           : Position(0, 0);
}
...
void EditorBuffer::_NotifyModified()
{
  ...
  if (handler.IsOK()) {
    handler->OnWidgetSink(NULL, eEditorBufferMessage_Modified, niVarNull,
                          niVarNull);
  }
}
