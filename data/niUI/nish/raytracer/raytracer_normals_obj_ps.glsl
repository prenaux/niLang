...
nish_std_PixelOutput niUIGpuFuncs_raytracer_normals_obj_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
nish_std_RayDesc niUIGpuFuncs_MakeRayDesc(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms) {
  ...
  mat4 _tmp_A3 = aUniforms.cameraInvView;
  vec3 origin = vec3(_tmp_A3[3][0],_tmp_A3[3][1],_tmp_A3[3][2]);
  vec3 target = nish_std_Vec3TransformCoord(ndc,aUniforms.cameraInvViewProj);
  vec3 dir = normalize(((target-(origin.xyz)).xyz));
  vec3 _tmp_L3 = origin;
  vec3 _tmp_M3 = dir;
  float _tmp_N3 = 0.001;
  float _tmp_O3 = aUniforms.cameraFarClipPlane;
  return nish_std_RayDesc_new(_tmp_L3, _tmp_M3, _tmp_N3, _tmp_O3);
}
niUIGpuFuncs_IntersectionResult niUIGpuFuncs_TraceRay_6_uint_Vec3_Vec3_float_float_RayInstances(uint aRayFlags, vec3 aOrigin, vec3 aDir, float aMinT, float aMaxT, accelerationStructureEXT aAS) {
  /* mut */ rayQueryEXT rayQuery/*__noinit__*/;
  rayQueryInitializeEXT(rayQuery,aAS,aRayFlags,255,aOrigin,aMinT,aDir,aMaxT);
  while(true) {
    bool _tmp_Y3 = rayQueryProceedEXT(rayQuery);
    if (!(_tmp_Y3)) {
      break;
    }
    uint candidateType = rayQueryGetIntersectionTypeEXT(rayQuery,false);
    bool _tmp_34 = (candidateType == nish_std_RayQueryCandidateIntersectionType_CandidateTriangle);
    if (_tmp_34) {
  ...
  bool _tmp_b4 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_b4) {
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_normals_obj_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_A4 = niUIGpuFuncs__op__eq_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_Triangle);
  if (_tmp_A4) {
    vec3 _tmp_F4 = niUIGpuFuncs_NormalToColor(intersection.objNormal);
    color = vec4(_tmp_F4.x,_tmp_F4.y,_tmp_F4.z,1.0);
  }
  else {
    bool _tmp_I4 = niUIGpuFuncs__op__eq_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_InvalidTriangle);
    if (_tmp_I4) {
      color = vec4(1.0,0.0,1.0,1.0);
    }
    else {
      bool _tmp_Q4 = niUIGpuFuncs__op__eq_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_BoundingVolume);
      if (_tmp_Q4) {
  ...
  vec4 _tmp_45 = color;
  return nish_std_PixelOutput_new(_tmp_45);
}
...
// Pixel Shader main: niUIGpuFuncs_raytracer_normals_obj_ps
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = niUIGpuFuncs_raytracer_normals_obj_ps(aInput, aUniforms, IN_1_aAS);
