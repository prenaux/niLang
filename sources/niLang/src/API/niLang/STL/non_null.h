...
struct is_comparable_to_nullptr<
  T, eastl::enable_if_t<eastl::is_convertible<
       decltype(eastl::declval<T>() != nullptr), bool>::value>>
    : eastl::true_type {};
...
struct non_null {
 ...
 public:
  ...
  template <typename U>
  constexpr non_null(U&& u,
                     eastl::enable_if_t<eastl::is_pointer_v<U>>* = nullptr)
  {
    static_assert(
      false,
      "Trying to move a raw pointer to non_null. Use ni::as_nn(ptr) or astl::as_non_null(ptr) to mark the raw pointer as non null.");
  }

  template <
    typename = eastl::enable_if_t<!eastl::is_same<std::nullptr_t, T>::value>>
  ...
  non_null(const non_null& other)
      : ptr_(other.ptr_)
  {
  }
  non_null(non_null&& other)
  ...
  constexpr eastl::conditional_t<eastl::is_copy_constructible<T>::value, T,
                                 const T&>
  raw_ptr() const
  {
...
auto operator==(const non_null<T>& lhs,
                const non_null<U>& rhs) noexcept(noexcept(lhs.raw_ptr() ==
                                                          rhs.raw_ptr()))
...
static_assert(sizeof(non_null<int*>) == sizeof(int*),
              "non_null<> must be the same size as a pointer.");
