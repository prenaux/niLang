...
//
//----------------------------------------------------------------------------
static Ptr<iGpuBuffer> MakeSquareVB(ain<nn<iGraphicsDriverGpu>> aDriver, const sVec2f& aPos, tF32 afSize = 0.5f) {
  ...
  tVertexCanvas* verts = (tVertexCanvas*)buffer->Lock(0, buffer->GetSize(), eLock_Discard);
  niCheck(verts != nullptr, nullptr);
  verts[0] = {{aPos.x-afSize, aPos.y+afSize, 0.0f}, sVec3f::YAxis(), 0xFFFFFFFF, {0.0f,0.0f}}; // TL
  verts[1] = {{aPos.x+afSize, aPos.y+afSize, 0.0f}, sVec3f::YAxis(), 0xFFFFFFFF, {1.0f,0.0f}}; // TR
  verts[2] = {{aPos.x+afSize, aPos.y-afSize, 0.0f}, sVec3f::YAxis(), 0xFFFFFFFF, {1.0f,1.0f}}; // BR
  verts[3] = {{aPos.x-afSize, aPos.y-afSize, 0.0f}, sVec3f::YAxis(), 0xFFFFFFFF, {0.0f,1.0f}}; // BL
  buffer->Unlock();
