...
struct FixedBase : public ni::cWidgetSinkImpl<> {
  tBool kbTextures;
  tBool mbAnimated;

  TEST_CONSTRUCTOR(FixedBase)
  {
    niDebugFmt(("FixedBase::FixedBase"));
  ...
  tBool __stdcall OnWheel(tF32 afWheel, const sVec2f& avAbsMousePos) niOverride
  {
    if (mbMouseLook && mbCameraInput) {
      mptrCamera->MoveForward(afWheel * 0.2f *
                              ((mpWidget->GetUIContext()->GetInputModifiers() &
  ...
  tBool __stdcall OnKeyUp(eKey aKey, tU32 aKeyMod) niOverride
  {
    ...
    case eKey_Down:
    case eKey_W:
    case eKey_S: mMove.z = 0.0; break;
    case eKey_Left:
    case eKey_Right:
    case eKey_A:
    case eKey_D: mMove.x = 0.0; break;
    case eKey_PgUp:
    case eKey_PgDn:
    case eKey_R:
    case eKey_F:
  ...
  virtual tBool __stdcall LoadEarthTextures()
  {
    ...
    _earthBase = mpWidget->GetGraphics()->CreateTextureFromBitmap(
      _H(fp->GetSourcePath()), mpWidget->GetGraphics()->LoadBitmap(fp),
    ...
    if (!_earthSpec.IsOK())
      return eFalse;
    fp = mpWidget->GetGraphics()->OpenBitmapFile(
      _A("test/tex/earth_clouds_d.jpg"));
    _earthClouds = mpWidget->GetGraphics()->CreateTextureFromBitmap(
      _H(fp->GetSourcePath()), mpWidget->GetGraphics()->LoadBitmap(fp),
      eTextureFlags_Default);
    if (!_earthClouds.IsOK())
      return eFalse;
  ...
  tBool CreateSphere(const sVec3f& avCenter, iTexture* apTex,
  ...
  tBool CreateCube(const sVec3f& avCenter, iTexture* apTex, tBool abCW = eTrue,
                   tBool abAlpha = eFalse, tF32 afSize = 10.0f)
  {
  ...
  tBool InitScene(tBool abBoxed = eFalse)
  {
    ...
    const tBool bMinimalScene =
      !!ni::GetLang()->GetProperty("tests.minimal_scene").Long();

    if (!LoadEarthTextures())
      return eFalse;

    if (!CreateCube(Vec3(0.0f, -130.0f, 100.0f), _rustSteel, eTrue, eFalse,
                    100.0f))
      return eFalse;

    if (!bMinimalScene) {
      if (!CreateCube(Vec3(-25.0f, -20.0f, 100.0f), _rustSteel))
        return eFalse;
      if (!CreateCube(Vec3(-30.0f, -10.0f, 110.0f), _earthClouds))
        return eFalse;
      if (!CreateCube(Vec3(-30.0f, 10.0f, 105.0f), _rustSteel))
        return eFalse;
      if (!CreateCube(Vec3(25.0f, -20.0f, 100.0f), _earthBase))
        return eFalse;
      if (!CreateCube(Vec3(30.0f, -10.0f, 110.0f), _earthBase))
        return eFalse;
    }

    if (!CreateCube(Vec3(30.0f, 10.0f, 105.0f), _earthBase))
      return eFalse;
    if (!CreateCube(Vec3(0.0f, -15.0f, 75.0f), _tree, eTrue, eTrue))
      return eFalse;

    if (abBoxed) {
      if (!CreateCube(Vec3(0.0f, -232.0f, 0.0f), _rustSteel, eFalse, eFalse,
                      500))
        return eFalse;
      if (!CreateSphere(Vec3(-200.0f, -60.0f, -200.0f), _earthBase, 35.0f,
                        eTrue))
        return eFalse;
      if (!CreateSphere(Vec3(200.0f, -60.0f, -200.0f), _earthBase, 35.0f,
                        eTrue))
        return eFalse;
      if (!CreateSphere(Vec3(200.0f, -60.0f, 200.0f), _earthBase, 35.0f, eTrue))
        return eFalse;
      if (!CreateSphere(Vec3(-200.0f, -60.0f, 200.0f), _earthBase, 35.0f,
                        eTrue))
        return eFalse;
...

    {
      ...
      niLoop (j, stacks) {
        niLoop (i, sphereCount) {
          ...
          if (!CreateSphere(p + Vec3(0.0f, 0.0f, 100.0f), _earthBase, 8.0f))
            return eFalse;
        }
      }
    }
...
struct Base : public FixedBase {
  TEST_CONSTRUCTOR_BASE(Base, FixedBase)
  {
  }

  tBool __stdcall OnSinkAttached() niImpl
  {
    CHECK(FixedBase::OnSinkAttached());
    CHECK(InitScene());
    return eTrue;
  }
  ...
  {
    FixedBase::OnPaint(avMousePos, apCanvas);
    return eFalse;
  }
};
TEST_FIXTURE_WIDGET(FFixed, Base);
