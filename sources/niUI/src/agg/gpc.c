...
{
  GPC_REAL x;           /* X coordinate component            */
  GPC_REAL y;           /* Y coordinate component            */
...
typedef struct edge_shape {
  ...
  GPC_REAL xb;                   /* Scanbeam bottom x coordinate      */
  GPC_REAL xt;                   /* Scanbeam top x coordinate         */
  GPC_REAL dx;                   /* Change in x for a unit y increase */
...
{
  GPC_REAL y;             /* Y coordinate at local minimum     */
...
{
  GPC_REAL y;               /* Scanbeam node y value             */
...
{
  GPC_REAL xmin; /* Minimum x coordinate              */
  GPC_REAL ymin; /* Minimum y coordinate              */
  GPC_REAL xmax; /* Maximum x coordinate              */
  GPC_REAL ymax; /* Maximum y coordinate              */
} bbox;
...
static edge_node** bound_list(lmt_node** lmt, GPC_REAL y)
{
...
static void add_to_sbtree(int* entries, sb_tree** sbtree, GPC_REAL y)
{
  if (!*sbtree) {
...
static void build_sbt(int* entries, GPC_REAL* sbt, sb_tree* sbtree)
{
  if (sbtree->less)
      ...
      for (min = 0; min < num_vertices; min++) {
        ...
        if (FWD_MIN(edge_table, min, num_vertices)) {
          ...
          for (i = 0; i < num_edges; i++) {
            ...
            e[i].dx =
              (edge_table[v].vertex.x - e[i].bot.x) / (e[i].top.y - e[i].bot.y);
            e[i].type = type;
            e[i].outp[ABOVE] = NULL;
            e[i].outp[BELOW] = NULL;
            e[i].next = NULL;
...
static void build_intersection_table(it_node** it, edge_node* aet, GPC_REAL dy)
{
  st_node *st, *stp;
  edge_node* edge;
...
static void add_left(polygon_node* p, GPC_REAL x, GPC_REAL y)
{
  vertex_node* nv;
...
static void add_right(polygon_node* p, GPC_REAL x, GPC_REAL y)
{
  vertex_node* nv;
...
static void add_local_min(polygon_node** p, edge_node* edge, GPC_REAL x,
                          GPC_REAL y)
{
...
static void add_vertex(vertex_node** t, GPC_REAL x, GPC_REAL y)
{
  if (!(*t)) {
...
static void new_tristrip(polygon_node** tn, edge_node* edge, GPC_REAL x,
                         GPC_REAL y)
{
  if (!(*tn)) {
...
static bbox* create_contour_bboxes(gpc_polygon* p)
{
  ...
  for (c = 0; c < p->num_contours; c++) {
    ...
    box[c].xmin = GPC_REAL_MAX;
    box[c].ymin = GPC_REAL_MAX;
    box[c].xmax = -GPC_REAL_MAX;
    box[c].ymax = -GPC_REAL_MAX;
...
void gpc_read_polygon(FILE* fp, int read_hole_flags, gpc_polygon* p)
{
  ...
  for (c = 0; c < p->num_contours; c++) {
    ...
    MALLOC(p->contour[c].vertex,
           p->contour[c].num_vertices * sizeof(gpc_vertex), "vertex creation",
           gpc_vertex);
    for (v = 0; v < p->contour[c].num_vertices; v++)
      fscanf(fp, "%f %f", &(p->contour[c].vertex[v].x),
...
{
  sb_tree* sbtree = NULL;
  it_node *it = NULL, *intersect = NULL;
  edge_node *edge = NULL, *prev_edge = NULL, *next_edge = NULL,
            *succ_edge = NULL, *e0 = NULL, *e1 = NULL;
  edge_node *aet = NULL, *c_heap = NULL, *s_heap = NULL;
  lmt_node *lmt = NULL, *local_min;
  polygon_node *out_poly = NULL, *p = NULL, *q = NULL, *poly = NULL,
               *npoly = NULL, *cf = NULL;
  vertex_node *vtx = NULL, *nv = NULL;
  h_state horiz[2];
  ...
  GPC_REAL *sbt = NULL, xb = 0, px = 0, yb = 0, yt = 0, dy = 0, ix = 0, iy = 0;
  ...
  MALLOC(sbt, sbt_entries * sizeof(GPC_REAL), "sbt creation", GPC_REAL);
  ...
  while (scanbeam < sbt_entries) {
    ...
    for (edge = aet; edge; edge = edge->next) {
      ...
      if (exists[CLIP] || exists[SUBJ]) {
        ...
        if (exists[CLIP])
          horiz[CLIP] =
            next_h_state[horiz[CLIP]][((exists[CLIP] - 1) << 1) + parity[CLIP]];
        if (exists[SUBJ])
          horiz[SUBJ] =
            next_h_state[horiz[SUBJ]][((exists[SUBJ] - 1) << 1) + parity[SUBJ]];
                       ...
                       gpc_tristrip* result)
{
  ...
  lmt_node *lmt = NULL, *local_min = NULL;
  polygon_node *tlist = NULL, *tn = NULL, *tnn = NULL, *p = NULL, *q = NULL;
  vertex_node *lt = NULL, *ltn = NULL, *rt = NULL, *rtn = NULL;
  h_state horiz[2];
  vertex_type cft = 0;
  int in[2], exists[2], parity[2] = { LEFT, LEFT };
  int s = 0, v = 0, contributing = 0, search = 0, scanbeam = 0, sbt_entries = 0;
  int vclass = 0, bl = 0, br = 0, tl = 0, tr = 0;
  GPC_REAL *sbt = NULL, xb = 0, px = 0, nx = 0, yb = 0, yt = 0, dy = 0, ix = 0,
           iy = 0;
