    ...
    : mSink(this)
{
  ...
  mpwSplitter = mpWidget->GetUIContext()->CreateWidget(
    _A("Splitter"), mpWidget, sRectf(0, 0, 50, 50), eWidgetStyle_DontSerialize,
    NULL);
  ...
  mpwTree = mpWidget->GetUIContext()->CreateWidget(
    _A("Tree"), ptrSplitter->GetSplitterWidget(0), sRectf(0, 0),
    eWidgetStyle_HoldFocus | eWidgetStyle_DontSerialize,
    _H("ID_PropertyBoxTree"));
...
///////////////////////////////////////////////
void __stdcall cWidgetPropertyBox::sSink::OnDataTableSink_SetName(
  iDataTable* apDT)
{
  if (!mpParent)
    return;
...
///////////////////////////////////////////////
void __stdcall cWidgetPropertyBox::sSink::OnDataTableSink_SetMetadata(
  iDataTable* apDT)
{
  if (!mpParent)
    return;
...
///////////////////////////////////////////////
void __stdcall cWidgetPropertyBox::sSink::OnDataTableSink_AddChild(
  iDataTable* apDT, iDataTable* apChild)
{
  if (!mpParent)
    return;
  iWidgetTreeNode* pNode = mpParent->FindNode(NULL, apDT);
...
///////////////////////////////////////////////
void __stdcall cWidgetPropertyBox::sSink::OnDataTableSink_RemoveChild(
  iDataTable* apDT, iDataTable* apChild)
{
  if (!mpParent)
    return;
...
///////////////////////////////////////////////
void __stdcall cWidgetPropertyBox::sSink::OnDataTableSink_SetProperty(
  iDataTable* apDT, tU32 anProperty)
{
  if (!mpParent || mnIgnoreSetProperty == anProperty)
    return;
  ePropertyEditBoxType ebtypeProperty =
    mpParent->GetPropertyEditBoxType(apDT, anProperty);
  if (ebtypeProperty == ePropertyEditBoxType_Hidden)
    return;
  iWidgetTreeNode* pNode = mpParent->FindNode(NULL, apDT);
  if (!pNode)
    return;
  iWidgetTreeNode* pPropertyNode =
    pNode->GetChildNodeFromName(apDT->GetPropertyName(anProperty));
  if (!pPropertyNode) {
...
///////////////////////////////////////////////
void __stdcall cWidgetPropertyBox::sSink::OnDataTableSink_RemoveProperty(
  iDataTable* apDT, tU32 anProperty)
{
  if (!mpParent)
    return;
...
///////////////////////////////////////////////
ni::Var __stdcall cWidgetPropertyBox::sSink::OnDataTableSink_GetProperty(
  ni::iDataTable*, ni::tU32)
{
  return niVarNull;
}

///////////////////////////////////////////////
tBool __stdcall cWidgetPropertyBox::sSink::OnWidgetSink(iWidget* apWidget,
                                                        tU32 anMsg,
                                                        const Var& aA,
                                                        const Var& aB)
{
  ...
  case eUIMessage_Notify: {
    ...
    case eUIMessage_SetFocus: {
      if (mpParent->mpwTree.ptr() != aA.mpIUnknown) {
        sVec2f vPos = mpParent->mpWidget->GetUIContext()->GetCursorPosition() -
                      mpParent->mpwTree->GetAbsolutePosition();
        mpParent->mpwTree->SendMessage(
          eUIMessage_LeftClickDown,
          vPos - mpParent->mpwTree->GetClientPosition(), vPos);
      }
      break;
    }
  ...
  case eUIMessage_NCPaint: {
    ...
    if (tree.IsOK() && canvas.IsOK()) {
      ...
      if (clippedRect.Intersect(vMousePos)) {
        hoverNode = tree->GetNodeFromPosition(vMousePos +
                                              apWidget->GetAbsolutePosition());
      }
      if (!hoverNode.IsOK()) {
      ...
      if (hoverNode.IsOK()) {
        ...
        sRectf nodeRect =
          hoverNode->GetScrolledNodeRect();              // Tree Client relative
...
///////////////////////////////////////////////
tBool __stdcall cWidgetPropertyBox::OnWidgetSink(iWidget* apWidget, tU32 anMsg,
...
void cWidgetPropertyBox::FillNode(iDataTable* apDT, iWidgetTreeNode* apNode)
{
  ...
  for (tU32 id = 0; id < apDT->GetNumChildren(); ++id) {
    ...
    if (id == mnMaxNumChildren) {
      pNewNode->SetName(
        niFmt(_A("%s %d...%d\n"), c->GetName(), id, apDT->GetNumChildren()));
      pNewNode->SetFlags(pNewNode->GetFlags() |
...
///////////////////////////////////////////////
iWidgetTreeNode* cWidgetPropertyBox::FindNode(iWidgetTreeNode* apNode,
                                              iDataTable* apDT)
{
  iWidgetTreeNode* pNode =
    apNode ? apNode : QPtr<iWidgetTree>(mpwTree)->GetRootNode();
...
///////////////////////////////////////////////
iWidget* cWidgetPropertyBox::CreateWidgetEditBox(ePropertyEditBoxType aType,
                                                 iHString* ahspMD)
{
  ...
  case ePropertyEditBoxType_Text: {
    pEB = pCtx->CreateWidget(
      _A("EditBox"), mpwEditBoxParent, sRectf(0, 0, 10, 22),
      eWidgetStyle_DontSerialize | eWidgetStyle_NotifyParent |
  ...
  case ePropertyEditBoxType_TextML: {
    pEB = pCtx->CreateWidget(
      _A("EditBox"), mpwEditBoxParent, sRectf(0, 0, 10, 102),
      (eWidgetStyle_DontSerialize | eWidgetStyle_NotifyParent |
       eWidgetEditBoxStyle_MultiLine | eWidgetEditBoxStyle_PasteValidate) |
        ((aType == ePropertyEditBoxType_ROTextML)
           ? (tU32)eWidgetEditBoxStyle_ReadOnly
           : (tU32)eWidgetStyle_HoldFocus),
      NULL);
    Ptr<iDataTableReadStack> ptrDT =
      ni::CreateDataTableReadStack(ParseTypeToDT(niHStr(ahspMD)));
  ...
  case ePropertyEditBoxType_Color: {
    pEB =
      pCtx->CreateWidget(_A("ComboBox"), mpwEditBoxParent, sRectf(0, 0, 10, 22),
                         eWidgetStyle_DontSerialize | eWidgetStyle_HoldFocus |
                           eWidgetComboBoxStyle_NoDefaultListBox,
                         NULL);
    Ptr<iWidget> w =
      pCtx->CreateWidget(_A("ColorPicker"), pEB, sRectf(0, 0, 100, 100),
                         eWidgetStyle_DontSerialize | eWidgetStyle_HoldFocus,
                         _H("ComboBox_ColorPicker"));
  ...
  case ePropertyEditBoxType_Font: {
    ...
    Ptr<iWidget> w = mpWidget->GetUIContext()->CreateWidget(
      _A("ListBox"), pEB, sRectf(0, 0, 100, 80),
      eWidgetStyle_Free | eWidgetStyle_NotifyParent | eWidgetStyle_HoldFocus,
  ...
  case ePropertyEditBoxType_Enum: {
    ...
    {
      Ptr<iWidget> wMenu =
        pCtx->CreateWidget(_A("Menu"), pEB, sRectf::Null(), 0, NULL);
  ...
  case ePropertyEditBoxType_Range: {
    pEB = pCtx->CreateWidget(
      _A("ScrollBar"), mpwEditBoxParent, sRectf(0, 0, 10, 22),
      eWidgetStyle_DontSerialize | eWidgetStyle_HoldFocus |
        eWidgetScrollBarStyle_Slider | eWidgetScrollBarStyle_Horz,
      NULL);
    {
      QPtr<iWidgetScrollBar> ptrSB = pEB;
      Ptr<iDataTableReadStack> ptrDT =
        ni::CreateDataTableReadStack(ParseTypeToDT(niHStr(ahspMD)));
      ptrSB->SetScrollRange(
        Vec2<tF32>(ptrDT->GetFloatDefault(_A("min"), 0.0f),
                   ptrDT->GetFloatDefault(_A("max"), 100.0f)));
      ptrSB->SetPageSize(ptrDT->GetFloatDefault(_A("step"), 1.0f));
    }
  ...
  case ePropertyEditBoxType_Bool: {
    pEB = pCtx->CreateWidget(
      _A("Button"), mpwEditBoxParent, sRectf(0, 0, 10, 22),
      eWidgetStyle_DontSerialize | eWidgetStyle_HoldFocus |
        eWidgetButtonStyle_OnOff | eWidgetButtonStyle_CheckBox,
      NULL);
    pEB->SetSkinClass(_H("PropertyBoxButton"));
    break;
  }
  case ePropertyEditBoxType_Hidden:
  case ePropertyEditBoxType_Unknown: break;
  }
  if (niIsOK(pEB)) {
...
void cWidgetPropertyBox::DoSetProperty(iWidget* apWidget)
{
  if (!niIsOK(apWidget))
    return;
  Ptr<iWidgetTreeNode> ptrModifiedNode =
    QPtr<iWidgetTree>(mpwTree)->GetRootNode()->FindNodeFromWidget(apWidget);
...
///////////////////////////////////////////////
ePropertyEditBoxType cWidgetPropertyBox::GetPropertyEditBoxType(
  iDataTable* apDT, tU32 anIndex)
{
  ...
  if (!ni::HStringIsEmpty(hspMD)) {
    ...
    else if (strName.IEq(_A("bool")) || strName.IEq(_A("i8")) ||
             strName.IEq(_A("u8")))
    {
...
ePropertyEditBoxType cWidgetPropertyBox::GetWidgetEditBoxType(iWidget* apWidget)
{
  ...
  if (apWidget->GetClassName() == _H("Button")) {
    ...
    return niFlagIs(apWidget->GetStyle(), eWidgetComboBoxStyle_Multiselect)
...
///////////////////////////////////////////////
iWidget* cWidgetPropertyBox::SetEditBox(iWidgetTreeNode* apTreeNode,
                                        iDataTable* apDT, tU32 anProperty,
                                        ePropertyEditBoxType ebtypeProperty)
{
  ...
  if (ebtypeWidget != ebtypeProperty) {
    ...
    pEB = CreateWidgetEditBox(ebtypeProperty,
                              apDT->GetMetadataFromIndex(anProperty));
    niAssert(niIsOK(pEB));
    ...
    apTreeNode->SetFlags(apTreeNode->GetFlags() &
...
///////////////////////////////////////////////
void cWidgetPropertyBox::SetEditBoxInDataTable(iWidgetTreeNode* apNode,
                                               iDataTable* apDT)
{
  ...
  case ePropertyEditBoxType_Color: {
    sVec4f col = mpWidget->GetGraphics()->GetColor4FromName(
      apNode->GetWidget()->GetText());
  ...
  case ePropertyEditBoxType_Range: {
    strNewVal.Set(
      QPtr<iWidgetScrollBar>(apNode->GetWidget())->GetScrollPosition());
    break;
  }
  case ePropertyEditBoxType_Bool: {
