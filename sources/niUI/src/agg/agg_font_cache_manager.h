...
class font_cache {
 public:
  ...
  //--------------------------------------------------------------------
  glyph_cache* cache_glyph(unsigned glyph_code, unsigned glyph_index,
                           unsigned data_size, glyph_data_type data_type,
                           const rect_i& bounds, agg_real advance_x,
                           agg_real advance_y)
  {
    unsigned msb = (glyph_code >> 8) & 0xFF;
    if (m_glyphs[msb] == 0) {
    ...
    glyph_cache* glyph =
      (glyph_cache*)m_allocator.allocate(sizeof(glyph_cache), sizeof(agg_real));
...
class font_cache_pool {
 public:
  ...
  {
    if (m_cur_font) {
      return m_cur_font->cache_glyph(glyph_code, glyph_index, data_size,
                                     data_type, bounds, advance_x, advance_y);
    }
    return 0;
  }
...
class font_cache_manager {
 public:
  ...
  const glyph_cache* glyph(unsigned glyph_code)
  {
    ...
    if (gl) {
      ...
      if (m_engine.prepare_glyph(glyph_code)) {
        m_prev_glyph = m_last_glyph;
        m_last_glyph = m_fonts.cache_glyph(
          glyph_code, m_engine.glyph_index(), m_engine.data_size(),
          m_engine.data_type(), m_engine.bounds(), m_engine.advance_x(),
          m_engine.advance_y());
  ...
  //--------------------------------------------------------------------
  void init_embedded_adaptors(const glyph_cache* gl, agg_real x, agg_real y,
                              agg_real scale = 1.0)
  {
    if (gl) {
      switch (gl->data_type) {
  ...
  //--------------------------------------------------------------------
  bool add_kerning(agg_real* x, agg_real* y)
  {
