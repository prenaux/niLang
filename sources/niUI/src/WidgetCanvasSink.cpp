...
#pragma niNote( \
    "The filling rules for the renderer is that the last pixel is not drawn")
#pragma niTodo( \
    "Add a possibility to set a iMetrics in the math expression evaluator, so that it supports parsing those automatically.")
...
///////////////////////////////////////////////
tBool cWidgetCanvasSink::OnWidgetSink(ni::iWidget* apWidget, tU32 nMsg,
                                      const Var& aA, const Var& aB)
{
  ...
  case eUIMessage_Layout: {
    ...
    tU32 nNumIt = 0;
    do {
      mpWidget->ComputeAutoLayout(autoLayoutFlags &
  ...
  case eUIMessage_Command: {
    if (IsScrollCanvas()) {
      ...
      if (cmd.IsOK()) {
        if (cmd->GetSender() == mpwScrollH &&
            cmd->GetID() == eWidgetScrollBarCmd_Scrolled)
        {
          UpdateClientPos();
        }
        else if (cmd->GetSender() == mpwScrollV &&
                 cmd->GetID() == eWidgetScrollBarCmd_Scrolled)
        {
          UpdateClientPos();
        }
    ...
    if (hasFillColor || hasBorderColor) {
      ...
      if (c.IsOK()) {
        ...
        if (hasBorderColor) {
          ...
          sRectf left(clientRect.GetTopLeft(),
                      clientRect.GetBottomLeft() +
                        Vec2<tF32>(mvBorder.Left(), 0));
          sRectf right(clientRect.GetTopRight() -
                         Vec2<tF32>(mvBorder.Right(), 0),
                       clientRect.GetBottomRight());
          sRectf top(clientRect.GetTopLeft(),
                     clientRect.GetTopRight() + Vec2<tF32>(0, mvBorder.Top()));
          sRectf bottom(clientRect.GetBottomLeft() -
                          Vec2<tF32>(0, mvBorder.Bottom()),
                        clientRect.GetBottomRight());
          if (mvBorder.Left()) {
...
void cWidgetCanvasSink::_InitializeScrollBars()
{
  if (ScrollH()) {
    if (!mpwScrollH.IsOK()) {
      mpwScrollH = mpWidget->GetUIContext()->CreateWidget(
        _A("ScrollBar"), mpWidget, sRectf(0, 0),
        eWidgetScrollBarStyle_Horz | eWidgetStyle_NCRelative |
          eWidgetStyle_DontSerialize,
        _HC(ID_CanvasScrollH));
      if (mpwScrollH.IsOK()) {
  ...
  if (ScrollV()) {
    if (!mpwScrollV.IsOK()) {
      mpwScrollV = mpWidget->GetUIContext()->CreateWidget(
        _A("ScrollBar"), mpWidget, sRectf(0, 0),
        eWidgetStyle_NCRelative | eWidgetStyle_DontSerialize,
        _HC(ID_CanvasScrollV));
      if (mpwScrollV.IsOK()) {
...
tBool cWidgetCanvasSink::UpdateScrolls()
{
  ...
  if (niFlagIs(mpWidget->GetAutoLayout(), eWidgetAutoLayoutFlags_Size)) {
    ...
    if (vNewSize.x > 0)
      clientSize.x = vNewSize.x;
    if (vNewSize.y > 0)
      clientSize.y = vNewSize.y;
  }
  ...
  if (ScrollH()) {
    ...
    if (ptrScroll.IsOK()) {
      if (bNeedScrollH) {
        ...
        mpwScrollH->SetSize(Vec2<tF32>(
          minSize.x - ((clientSize.y > size.y) ? SCROLLBAR_SIZE_H : 0),
          SCROLLBAR_SIZE_H));
        ptrScroll->SetScrollRange(
          Vec2<tF32>(-mfMarginH,
                     mfMarginH + ((clientSize.x - size.x) +
                                  (bNeedScrollV ? SCROLLBAR_SIZE_H : 0))) /
          STEP_SIZE_H);
      }
      else {
  ...
  if (ScrollV()) {
    ...
    if (ptrScroll.IsOK()) {
      if (bNeedScrollV) {
        ...
        mpwScrollV->SetSize(Vec2<tF32>(
          SCROLLBAR_SIZE_V,
          minSize.y /*-((clientSize.x > size.x)?SCROLLBAR_SIZE:0)*/));
        ptrScroll->SetScrollRange(
          Vec2<tF32>(-mfMarginV,
                     mfMarginV + ((clientSize.y - size.y) +
                                  (bNeedScrollH ? SCROLLBAR_SIZE_V : 0))) /
          STEP_SIZE_V);
      }
      else {
...
void cWidgetCanvasSink::UpdateClientPos()
{
  ...
  if (mpwScrollH.IsOK() && mpwScrollH->GetEnabled()) {
    pos.x =
      -QPtr<iWidgetScrollBar>(mpwScrollH)->GetScrollPosition() * STEP_SIZE_H;
    mpwScrollH->SetPosition(Vec2<tF32>(0, size.y - SCROLLBAR_SIZE_H));
  }
  if (mpwScrollV.IsOK() && mpwScrollV->GetEnabled()) {
    pos.y =
      -QPtr<iWidgetScrollBar>(mpwScrollV)->GetScrollPosition() * STEP_SIZE_V;
