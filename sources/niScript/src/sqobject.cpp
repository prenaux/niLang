...
const SQChar* SQFunctionProto::GetLocal(SQVM* vm, tU32 stackbase, tU32 nseq,
                                        tU32 nop)
{
  ...
  if (nvars >= nseq) {
    for (tU32 i = 0; i < nvars; i++) {
      if (_localvarinfos[i]._start_op <= nop &&
          _localvarinfos[i]._end_op >= nop)
      {
        if (nseq == 0) {
...
#define _CHECK_IO(exp, reason)                \
  {                                           \
    if (!(exp)) {                             \
      v->Raise_MsgError("io error: " reason); \
      return false;                           \
    }                                         \
  }

static inline bool SafeWrite(SQVM* v, ain<nn<ni::iFile>> fp, ni::tPtr dest,
                             tI32 size)
{
...
static inline bool SafeRead(SQVM* v, ain<nn<ni::iFile>> fp, ni::tPtr dest,
                            tI32 size)
{
...
static inline bool ReadAndCheckTag(SQVM* v, ain<nn<ni::iFile>> fp,
                                   ain<tU8> aExpectedTag)
{
...
bool WriteSQObject(SQVM* v, ain<nn<ni::iFile>> fp, ain<SQObjectPtr> o)
{
  ...
  case OT_CLOSURE: {
    ...
    _CHECK_IO(WriteSQFunctionProto(v, fp, _funcproto(_closure(o)->_function)),
              "write closure funcproto");
    break;
  }
  case OT_TABLE: {
    ...
    if (!locked) {
      v->Raise_MsgError(
        "Table already serialized, cyclic structures can't be serialized");
      return false;
    }
    fp->WriteLE32(nsize);
    tU32 nwritten = 0;
    ...
    if (nsize != nwritten) {
      v->Raise_MsgError(
        niFmt("Table size mismatch, nsize is %d but serialized %d fields.",
              nsize, nwritten));
      return eFalse;
    }
    break;
  ...
  case OT_ARRAY: {
    ...
    if (!locked) {
      v->Raise_MsgError(
        "Array already serialized, cyclic structures can't be serialized");
      return false;
    }
    fp->WriteLE32(nsize);
    niLoop (i, nsize) {
  ...
  default: {
    v->Raise_MsgError(niFmt("cannot serialize write a '%s' (%d)",
...
bool ReadSQObject(SQVM* v, ain<nn<ni::iFile>> fp, aout<SQObjectPtr> o)
{
  ...
  case _kTagTypeStringShort: {
    ...
    if (len) {
      ...
      _CHECK_IO(buffer[len - 1] != 0 && buffer[len] == 0,
  ...
  case _kTagTypeClosure: {
    ...
    _CHECK_IO(ReadSQFunctionProto(v, fp, _funcproto(_closure(o)->_function)),
              "read closure");
    break;
  }
  case _kTagTypeTable: {
  ...
  default: {
    v->Raise_MsgError(
      niFmt("cannot serialize read tag '%c' (%d)", readTag, readTag));
    return false;
  }
  }
  return true;
                          ...
                          SQFunctionProto* aProto)
{
  ...
  {
    _CHECK_IO(fp->Write8(_kTagFuncProtoRetAndParams),
              "write tag closure ret params");
    ...
    niLoop (i, nsize) {
      _CHECK_IO(WriteSQObject(v, fp, aProto->_parameters[i]._name),
                "write param name");
      _CHECK_IO(WriteSQObject(v, fp, aProto->_parameters[i]._type),
                "write param type");
    }
  }
...

  {
    ...
    niLoop (i, nsize) {
      _CHECK_IO(fp->Write8(aProto->_outervalues[i]._blocal),
                "write outerval blocal");
      _CHECK_IO(WriteSQObject(v, fp, aProto->_outervalues[i]._src),
                "write outerval src");
    }
  }
...

  {
    ...
    _CHECK_IO(
      fp->WriteRaw(aProto->_lineinfos.data(), sizeof(SQLineInfo) * nsize) ==
        sizeof(SQLineInfo) * nsize,
      "write lineinfos");
  }

  {
    static_assert(sizeof(SQInstruction) == 8);
    _CHECK_IO(fp->Write8(_kTagFuncProtoInstructions),
              "write tag closure instructions");
    const tU32 nsize = (tU32)aProto->_instructions.size();
    _CHECK_IO(fp->WriteLE32(nsize), "write instructions size");
    _CHECK_IO(fp->WriteRaw(aProto->_instructions.data(),
                           sizeof(SQInstruction) * nsize) ==
                sizeof(SQInstruction) * nsize,
              "write instructions");
  }

  {
    _CHECK_IO(fp->Write8(_kTagFuncProtoFunctions),
              "write tag closure functions");
    const tU32 nsize = (tU32)aProto->_functions.size();
    _CHECK_IO(fp->WriteLE32(nsize), "write functions size");
    niLoop (i, nsize) {
      _CHECK_IO(WriteSQFunctionProto(v, fp, _funcproto(aProto->_functions[i])),
                "write function");
    }
  }
                         ...
                         SQFunctionProto* aProto)
{
  ...
  {
    _CHECK_IO(ReadAndCheckTag(v, fp, _kTagFuncProtoRetAndParams),
              "read tag closure ret params");
  ...
  {
    _CHECK_IO(ReadAndCheckTag(v, fp, _kTagFuncProtoLiterals),
              "read tag closure literals");
    niLet nsize = fp->ReadLE32();
    niLoop (i, nsize) {
  ...
  {
    _CHECK_IO(ReadAndCheckTag(v, fp, _kTagFuncProtoOuters),
              "read tag closure outers");
  ...
  {
    _CHECK_IO(ReadAndCheckTag(v, fp, _kTagFuncProtoLocals),
              "read tag closure locals");
  ...
  {
    static_assert(sizeof(SQLineInfo) == 12);
    _CHECK_IO(ReadAndCheckTag(v, fp, _kTagFuncProtoLines),
              "read tag closure lines");
    niLet lineInfosSize = fp->ReadLE32();
    aProto->_lineinfos.resize(lineInfosSize);
    _CHECK_IO(fp->ReadRaw(aProto->_lineinfos.data(),
                          sizeof(SQLineInfo) * lineInfosSize) ==
                sizeof(SQLineInfo) * lineInfosSize,
              "read lineinfos");
  }
  ...
  {
    static_assert(sizeof(SQInstruction) == 8);
    _CHECK_IO(ReadAndCheckTag(v, fp, _kTagFuncProtoInstructions),
              "read tag instructions");
    niLet instructionsSize = fp->ReadLE32();
    aProto->_instructions.resize(instructionsSize);
    _CHECK_IO(fp->ReadRaw(aProto->_instructions.data(),
                          sizeof(SQInstruction) * instructionsSize) ==
                sizeof(SQInstruction) * instructionsSize,
              "read instructions");
  }

  {
    _CHECK_IO(ReadAndCheckTag(v, fp, _kTagFuncProtoFunctions),
              "read tag closure functions");
    ...
    niLoop (i, functionsSize) {
      ...
      _CHECK_IO(ReadSQFunctionProto(v, fp, _funcproto(aProto->_functions[i])),
                "read function");
    }
  }
