...
#pragma niTodo( \
    "The serialization is pretty bad, we cant use it when hand writing the UI definition.")
...
static __forceinline iWidget* _CreateCanvas(iWidget* apParent)
{
  ...
  return apParent->GetUIContext()->CreateWidget(
    _A("Canvas"), apParent, sRectf(0, 0, 0, 0), 0,
    _H(niFmt(_A("SplitterCanvas%d"), _nCounter++)));
}
...
{
  if (anIndex >= GetNumSplitterWidgets())
    return NULL;
...
tBool __stdcall cWidgetSplitter::AddSplitterBefore(tU32 anWidget, tF32 afPos)
{
  ...
  if (mvSplitters.empty()) {
    ...
    tF32 fNextSplitter = ((nNewSplitterIndex + 1) >= mvSplitters.size())
                           ? 1.0f
                           : mvSplitters[nNewSplitterIndex + 1].mfPosition;
    tF32 fPrevSplitter = (nNewSplitterIndex < 1)
                           ? 0.0f
                           : mvSplitters[nNewSplitterIndex - 1].mfPosition;
    mvSplitters[anWidget].mfPosition =
      (fNextSplitter - fPrevSplitter) * afPos + fPrevSplitter;
    iWidget* pNewW = _CreateCanvas(mpWidget);
...
tBool __stdcall cWidgetSplitter::AddSplitterAfter(tU32 anWidget, tF32 afPos)
{
  ...
  if (mvSplitters.empty()) {
    ...
    tF32 fPrevSplitter = (nNewSplitterIndex < 1)
                           ...
                           : mvSplitters[nNewSplitterIndex - 1].mfPosition;
    mvSplitters[anWidget].mfPosition =
      fNextSplitter - ((fNextSplitter - fPrevSplitter) * afPos);
    iWidget* pNewW = _CreateCanvas(mpWidget);
...
{
  if (anA >= GetNumSplitterWidgets())
    return eFalse;
  if (anB >= GetNumSplitterWidgets())
    return eFalse;
...
///////////////////////////////////////////////
void __stdcall cWidgetSplitter::SetSplitterParentDockRectMinimumSize(
  sVec2f avMinSize)
{
...
///////////////////////////////////////////////
tBool __stdcall cWidgetSplitter::OnWidgetSink(iWidget* apWidget, tU32 anMsg,
                                              const Var& aA, const Var& aB)
{
  ...
  case eUIMessage_MouseMove: {
    ...
    if (mnDragging != eInvalidHandle) {
      ...
      if (mnDragging >= DRAGGINGBORDER_FIRST) {
        ...
        niLet absMousePos =
          aB.GetVec2fValue() + mpWidget->GetAbsolutePosition();
        ...
        if (pParent) {
          ...
          rectDockFill +=
            pParent->GetAbsolutePosition() + pParent->GetClientPosition();
        }
        ...
        if (niFlagIs(mpWidget->GetStyle(), eWidgetSplitterStyle_Horizontal)) {
          SetSplitterPosition(mnDragging,
                              ncMousePos.y / mpWidget->GetClientSize().y);
        }
        else {
          SetSplitterPosition(mnDragging,
                              ncMousePos.x / mpWidget->GetClientSize().x);
        }
      }
    }
    break;
  ...
  case eUIMessage_SetCursor: {
    if (!_SplittersEnabled())
      return eFalse;
    if (mnDragging != eInvalidHandle) {
      apWidget->GetUIContext()->SetCursor(
        apWidget->FindSkinCursor(NULL, NULL, _H("Hand")));
      return eTrue;
    }

    if (mnDragging != eInvalidHandle ||
        niFlagIs(mpWidget->GetStyle(), eWidgetSplitterStyle_NoCursorResize))
      return eTrue;
    ...
    if (TestIntersection(ncMousePos, nI, nF)) {
      if (GetIsZoneHorizontal(nI)) {
        switch (nF) {
        case FOLD_UNFOLD:
          apWidget->GetUIContext()->SetCursor(skin.curArrowBiVt);
          break;
        case FOLD_LEFT_TOP:
          apWidget->GetUIContext()->SetCursor(skin.curArrowUp);
          break;
        case FOLD_RIGHT_BOTTOM:
          apWidget->GetUIContext()->SetCursor(skin.curArrowDown);
          break;
        ...
        switch (nF) {
        case FOLD_UNFOLD:
          apWidget->GetUIContext()->SetCursor(skin.curArrowBiHz);
          break;
        case FOLD_LEFT_TOP:
          apWidget->GetUIContext()->SetCursor(skin.curArrowLeft);
          break;
        case FOLD_RIGHT_BOTTOM:
          apWidget->GetUIContext()->SetCursor(skin.curArrowRight);
          break;
  ...
  case eUIMessage_SerializeWidget: {
    ...
    if (niFlagIs(nFlags, eWidgetSerializeFlags_Write)) {
      ...
      ptrDT->SetVec2(_A("splitter_parent_dock_rect_min_size"),
                     mvParentDockRectMinSize);
      ptrDT->SetInt(_A("num_splitters"), mvSplitters.size());
      for (tSplitterVec::iterator itS = mvSplitters.begin();
           itS != mvSplitters.end(); ++itS)
      {
      ...
      for (tWidgetVec::iterator itW = mvWidgets.begin(); itW != mvWidgets.end();
           ++itW)
      {
      ...
      if (!mmapFoldMap.empty()) {
        dt->Push(_A("FoldMap"));
        for (tFoldMap::iterator itF = mmapFoldMap.begin();
             itF != mmapFoldMap.end(); ++itF)
        {
      ...
      mvParentDockRectMinSize =
        ptrDT->GetVec2(_A("splitter_parent_dock_rect_min_size"));
      tU32 nChildTable = 0, i;
      tU32 nNumSplitters = ptrDT->GetInt(_A("num_splitters"));
      for (i = 0; i < nNumSplitters; ++i, ++nChildTable) {
        AddSplitterBack(
          ptrDT->GetChildFromIndex(nChildTable)->GetFloat(_A("position")));
      }
      tU32 nNumSplitterWidgets = ptrDT->GetInt(_A("num_splitter_widgets"));
      for (i = 0; i < nNumSplitterWidgets; ++i, ++nChildTable) {
        mvWidgets[i] = mpWidget->GetChildFromIndex(
          ptrDT->GetChildFromIndex(nChildTable)->GetInt(_A("child_index")));
        mvWidgets[i]->SetID(
          _H(ptrDT->GetChildFromIndex(nChildTable)->GetString(_A("id"))));
      }
      if (dt->PushFail(_A("FoldMap"))) {
        ...
        niLoop (i, dt->GetNumChildren()) {
          ...
          if (ni::StrCmp(dt->GetName(), _A("Fold")) == 0 &&
              dt->GetTop()->GetPropertyIndex(_A("splitter")) !=
                eInvalidHandle &&
...
void cWidgetSplitter::UpdateSplitterRects(tU32 anPivotSplitter)
{
  ...
  if (mnFillerIndex < mvSplitters.size()) {
    ...
    tF32 fSize = ni::Max(
      1.0f, niFlagIs(mpWidget->GetStyle(), eWidgetSplitterStyle_Horizontal)
              ? vSize.y
              : vSize.x);
    ...
    if (niFlagIs(mpWidget->GetStyle(), eWidgetSplitterStyle_Horizontal)) {
      for (itS = mvSplitters.begin(); itS != mvSplitters.end(); ++itS, ++itW) {
        (*itW)->SetRect(
          sRectf(0, fRectTop, vSize.x, itS->mRect.GetTop() - fRectTop));
      ...
      for (itS = mvSplitters.begin(); itS != mvSplitters.end(); ++itS, ++itW) {
        (*itW)->SetRect(
          sRectf(fRectTop, 0, itS->mRect.GetLeft() - fRectTop, vSize.y));
...
{
  skin.curResizeHz =
    mpWidget->FindSkinCursor(NULL, NULL, _H("ResizeHorizontal"));
  ...
  skin.curArrowBiVt =
    mpWidget->FindSkinCursor(NULL, NULL, _H("ArrowBiVertical"));
  skin.curArrowBiHz =
    mpWidget->FindSkinCursor(NULL, NULL, _H("ArrowBiHorizontal"));

  const sColor4f defColor = sColor4f::White();
  skin.colBorder =
    ULColorBuild(mpWidget->FindSkinColor(defColor, NULL, NULL, _H("Border")));
  skin.colFill =
    ULColorBuild(mpWidget->FindSkinColor(defColor, NULL, NULL, _H("Fill")));
  skin.colFoldLeftTop = ULColorBuild(
    mpWidget->FindSkinColor(defColor, NULL, NULL, _H("FoldLeftTop")));
  skin.colFoldRightBottom = ULColorBuild(
    mpWidget->FindSkinColor(defColor, NULL, NULL, _H("FoldRightBottom")));
  skin.colFoldUnfold = ULColorBuild(
    mpWidget->FindSkinColor(defColor, NULL, NULL, _H("FoldUnfold")));
}
  ...
  if (it != mmapFoldMap.end()) {
    ...
    default: {
      ...
      if (abLeftTop) {
        ...
        if (niFlagIs(mpWidget->GetStyle(), eWidgetSplitterStyle_Horizontal)) {
          SetSplitterPosition(anID, (mpWidget->GetSize().y - foldedSize) /
                                      mpWidget->GetSize().y);
        }
        else {
          SetSplitterPosition(anID, (mpWidget->GetSize().x - foldedSize) /
                                      mpWidget->GetSize().x);
        }
      }
      break;
...
///////////////////////////////////////////////
tBool cWidgetSplitter::TestRect(const sVec2f& avMousePos, tU32 anZone,
                                tU32& anFold) const
{
  anFold = FOLD_NONE;
  if (_CanFold()) {
    ...
    else if (_CanFoldLeftTop() &&
             GetZoneRect(anZone, FOLD_LEFT_TOP).Intersect(avMousePos))
    {
      anFold = FOLD_LEFT_TOP;
      return eTrue;
    }
    else if (_CanFoldRightBottom() &&
             GetZoneRect(anZone, FOLD_RIGHT_BOTTOM).Intersect(avMousePos))
    {
...
///////////////////////////////////////////////
tBool cWidgetSplitter::TestIntersection(const sVec2f& avNCMousePos,
                                        tU32& anIntersect, tU32& anFold) const
{
...
sRectf cWidgetSplitter::GetZoneRect(tU32 anZone, tU32 anFold) const
{
  ...
  default:
    ...
    bHorizontal =
      niFlagIs(mpWidget->GetStyle(), eWidgetSplitterStyle_Horizontal);
    break;
  }
  ...
  if (anFold) {
    ...
    if (anFold == FOLD_UNFOLD || _CanFoldOnlyOne()) {
      if (bHorizontal) {
        foldZone.Left() =
          rect.Left() + ((rect.GetWidth() / 2) - (kSplitterFoldHandleSize / 2));
        foldZone.Right() =
          rect.Left() + ((rect.GetWidth() / 2) + (kSplitterFoldHandleSize / 2));
      }
      else {
        foldZone.Top() =
          rect.Top() + ((rect.GetHeight() / 2) - (kSplitterFoldHandleSize / 2));
        foldZone.Bottom() =
          rect.Top() + ((rect.GetHeight() / 2) + (kSplitterFoldHandleSize / 2));
      }
    }
      ...
      if (bHorizontal) {
        foldZone.Left() =
          rect.Left() +
          ((rect.GetWidth() / 2) - (kSplitterFoldHandleSize / 2)) +
          (kSplitterFoldHandleSize / 2);
        foldZone.Right() = foldZone.Left() + (kSplitterFoldHandleSize / 2);
      }
      else {
        foldZone.Top() =
          rect.Top() +
          ((rect.GetHeight() / 2) - (kSplitterFoldHandleSize / 2)) +
          (kSplitterFoldHandleSize / 2);
        foldZone.Bottom() = foldZone.Top() + (kSplitterFoldHandleSize / 2);
      }
    }
...
tBool cWidgetSplitter::GetIsZoneHorizontal(tU32 anZone) const
{
  switch (anZone) {
  case DRAGGINGBORDER_LEFT:
  case DRAGGINGBORDER_RIGHT: return eFalse;
  case DRAGGINGBORDER_TOP:
  case DRAGGINGBORDER_BOTTOM: return eTrue;
  default:
