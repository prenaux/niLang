...
//----------------------------------------------------------------------------
//
// Section: UDP unreliable socket test
//
//----------------------------------------------------------------------------
...
{
  ni::Ptr<ni::iSocket> socket =
    ni::GetLang()->CreateSocket(ni::eSocketProtocol_UDP, NULL);
  CHECK(socket.IsOK());
  CHECK_EQUAL((ni::tU32)ni::eSocketProtocol_UDP,
              (ni::tU32)socket->GetProtocol());
...
{
  ni::Ptr<ni::iRemoteAddressIPv4> ra =
    ni::GetLang()->CreateRemoteAddressIPv4(0, 0);
  ra->SetHost(niFourCC(127,0,0,1));
  ra->SetPort(_GetTestSocketPort());

  ni::Ptr<ni::iSocket> recv =
    ni::GetLang()->CreateSocket(ni::eSocketProtocol_UDP, NULL);
  CHECK(recv.IsOK());
...
TEST_FIXTURE(FSocket_UDP, LoopbackSendRecv)
{
  ...
  // receive data
  {
    ni::Ptr<ni::iRemoteAddressIPv4> raSender =
      ni::GetLang()->CreateRemoteAddressIPv4(0, 0);
    ni::Ptr<ni::iFile> recvData = ni::CreateFileDynamicMemory(256, NULL);
    CHECK(recv->Wait(ni::eSocketWaitFlags_Receive) ==
          ni::eSocketWaitFlags_Receive);
    if (recv->Wait(ni::eSocketWaitFlags_Receive) ==
        ni::eSocketWaitFlags_Receive)
    {
...
//----------------------------------------------------------------------------
//
// Section: TCP reliable socket test
//
//----------------------------------------------------------------------------
...
{
  ni::Ptr<ni::iSocket> socket =
    ni::GetLang()->CreateSocket(ni::eSocketProtocol_TCP, NULL);
  CHECK(socket.IsOK());
  CHECK_EQUAL((ni::tU32)ni::eSocketProtocol_TCP,
              (ni::tU32)socket->GetProtocol());
  CHECK(!!ni::QueryInterface<ni::iUnknown>(socket));
  CHECK(!!ni::QueryInterface<ni::iSocket>(socket));
}
...
struct SimpleTCPServer
    : public ni::ImplRC<ni::iRunnable, ni::eImplFlags_Default> {
  SimpleTCPServer()
      : _error(_H(""))
  {
  }
  ...
  ni::Var __stdcall Run() override
  {
    ...
    ni::Ptr<ni::iSocket> socketConnect =
      ni::GetLang()->CreateSocket(ni::eSocketProtocol_TCP, NULL);
    ...
    while (1) {
      if (socketConnect->Wait(ni::eSocketWaitFlags_Receive) ==
          ni::eSocketWaitFlags_Receive)
      {
    ...
    if (socketClient->Receive(file) <= 0) {
      _error = _H(
        niFmt(_A("Can't receive data, errno: %d"), socketClient->GetErrno()));
      return 0;
    }
    file->SeekSet(0);
    ...
    if (socketClient->Send(file, 0) <= 0) {
      _error =
        _H(niFmt(_A("Can't send data, errno: %d"), socketClient->GetErrno()));
      return 0;
    }
...
struct SimpleTCPClient
  ...
  ni::Var __stdcall Run() override
  {
    ...
    ni::Ptr<ni::iRemoteAddressIPv4> serverAddr =
      ni::GetLang()->CreateRemoteAddressIPv4(niFourCC(127,0,0,1),
                                                      _GetTestSocketPort());
    ...
    if (socket->Receive(file) <= 0) {
      _error =
        _H(niFmt(_A("Can't receive data, errno: %d"), socket->GetErrno()));
      return 0;
    }
...
TEST_FIXTURE(FSocket_TCP, Connect)
{
  ...
  ni::Ptr<ni::iFuture> serverThreadFuture =
    ni::GetConcurrent()->ThreadRun(server);
  ...
  if (ni::HStringIsEmpty(server->_error)) {
    ni::Ptr<ni::iFuture> clientThreadFuture =
      ni::GetConcurrent()->ThreadRun(client);
...
{
  ni::Ptr<ni::iRemoteAddressIPv4> ra =
    ni::GetLang()->CreateRemoteAddressIPv4(0, 44719);
  ni::Ptr<ni::iSocket> ptrSocket =
    ni::GetLang()->CreateSocket(ni::eSocketProtocol_TCP, ra);
  CHECK(ptrSocket.IsOK());
