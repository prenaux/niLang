...
#ifdef _DEBUG
    niDebugFmt(("ScriptVM[%p], detached from thread '%d'",
                (void*)it->second.ptr(), aThreadID));
#endif
...
#ifdef _DEBUG
      niDebugFmt(("ScriptVM[%p], attached to thread '%d'", (void*)newVM.ptr(),
                  aThreadID));
#endif
    }
    return it->second;
...
#if !defined niNoThreads
  ni::ConcurrentSetThreadStartEndCallbacks(_ConcurrentCallbackThreadStart,
...
struct CallbackScript
    : public ImplRC<iCallback, eImplFlags_DontInherit1, iRunnable> {
  WeakPtr<SQSharedState> mSS;
  HSQOBJECT mClosure;
