...
void vec3_static_initialize() {
  ...
  vec3_Red = vec3(1.0,0.0,0.0);
  vec3_Green = vec3(0.0,1.0,0.0);
  vec3_Blue = vec3(0.0,0.0,1.0);
}
...
vec3 nish_std_Vec3Transform_2_Vec3_Matrix4x3(vec3 v, mat4x3 m) {
  vec3 _tmp_i = v;
  return (m * vec4(_tmp_i.x,_tmp_i.y,_tmp_i.z,1.0));
}
vec3 nish_std_Vec3TransformNormal_2_Vec3_Matrix4x3(vec3 v, mat4x3 m) {
  vec3 _tmp_m = v;
  vec3 r = (m * vec4(_tmp_m.x,_tmp_m.y,_tmp_m.z,0.0));
  return (r.xyz);
}
vec3 nish_std_Vec3TransformCoord(vec3 v, mat4 m) {
  vec3 _tmp_s = v;
  vec4 r = (m * vec4(_tmp_s.x,_tmp_s.y,_tmp_s.z,1.0));
  return ((r/r.w).xyz);
}
...
float niUIGpuFuncs_PositionToWorldChecker(vec3 aPos, float aGridSize, vec3 aNormal);
vec3 niUIGpuFuncs_PositionToHeat(vec3 aPos, float aMaxDist);
nish_std_PixelOutput niUIGpuFuncs_raytracer_pos_world_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
bool niUIGpuFuncs_IntersectionResult_SetFromCommittedIntersection(inout /* mut */ niUIGpuFuncs_IntersectionResult r, rayQueryEXT aRayQuery) {
  ...
  bool _tmp_f = (committedType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_f) {
    ...
    bool _tmp_I = (committedType == nish_std_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_I) {
...
void niUIGpuFuncs_IntersectionResult_SetFromTriangleData(inout /* mut */ niUIGpuFuncs_IntersectionResult r, uint aVBIndex, uint aIBIndex, uint aPrimIndex, uint aFirstIndex, uint aBaseVertexIndex, vec2 aIntersectionBaryCentrics, mat4x3 aObjectToWorld, bool aIsFrontFacing) {
  ...
  if (_tmp_P) {
  ...
  else {
    {
      ...
      bool _tmp_y1 = (aIsFrontFacing == false);
      if (_tmp_y1) {
...
nish_std_RayDesc niUIGpuFuncs_MakeRayDesc(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms) {
  ...
  mat4 _tmp_u3 = aUniforms.cameraInvView;
  vec3 origin = vec3(_tmp_u3[3][0],_tmp_u3[3][1],_tmp_u3[3][2]);
  vec3 target = nish_std_Vec3TransformCoord(ndc,aUniforms.cameraInvViewProj);
  vec3 dir = normalize(((target-(origin.xyz)).xyz));
  vec3 _tmp_F3 = origin;
  vec3 _tmp_G3 = dir;
  float _tmp_H3 = 0.001;
  float _tmp_I3 = aUniforms.cameraFarClipPlane;
  return nish_std_RayDesc_new(_tmp_F3, _tmp_G3, _tmp_H3, _tmp_I3);
}
niUIGpuFuncs_IntersectionResult niUIGpuFuncs_TraceRay_6_uint_Vec3_Vec3_float_float_RayInstances(uint aRayFlags, vec3 aOrigin, vec3 aDir, float aMinT, float aMaxT, accelerationStructureEXT aAS) {
  /* mut */ rayQueryEXT rayQuery/*__noinit__*/;
  rayQueryInitializeEXT(rayQuery,aAS,aRayFlags,255,aOrigin,aMinT,aDir,aMaxT);
  while(true) {
    bool _tmp_S3 = rayQueryProceedEXT(rayQuery);
    if (!(_tmp_S3)) {
      break;
    }
    uint candidateType = rayQueryGetIntersectionTypeEXT(rayQuery,false);
    bool _tmp_X3 = (candidateType == nish_std_RayQueryCandidateIntersectionType_CandidateTriangle);
    if (_tmp_X3) {
  ...
  bool _tmp_54 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_54) {
...
float niUIGpuFuncs_PositionToWorldChecker(vec3 aPos, float aGridSize, vec3 aNormal) {
  vec3 gridCoords = floor((aPos/aGridSize));
  vec3 absNormal = abs(aNormal);
  float checker;
  bool _tmp_u4 = ((absNormal.x > absNormal.y) && (absNormal.x > absNormal.z));
  if (_tmp_u4) {
    checker = mod((gridCoords.y + gridCoords.z),2.0);
  }
  else {
    bool _tmp_L4 = (absNormal.y > absNormal.z);
    if (_tmp_L4) {
      checker = mod((gridCoords.x + gridCoords.z),2.0);
    }
    else {
      {
        checker = mod((gridCoords.x + gridCoords.y),2.0);
      }
    }
  }
  return step(0.5, checker);
}
vec3 niUIGpuFuncs_PositionToHeat(vec3 aPos, float aMaxDist) {
  float distance = length(aPos);
  float centerDist = (aMaxDist * 0.5);
  float cycleSize = aMaxDist;
  vec3 centerColor = vec3_Green;
  vec3 bandColor1 = vec3_Red;
  vec3 bandColor2 = vec3_Blue;
  vec3 _tmp_85;
  bool _tmp_95 = (distance < centerDist);
  if (_tmp_95) {
    float t = (distance / centerDist);
    _tmp_85 = mix(centerColor,bandColor1,t);
  }
  else {
    {
      float remainingDist = (distance - centerDist);
      float cyclePos = mod(remainingDist,cycleSize);
      float cycleT = (cyclePos / cycleSize);
      float isRedToBlue = mod(floor((remainingDist / cycleSize)),2.0);
      bool _tmp_x5 = (isRedToBlue < 0.5);
      if (_tmp_x5) {
        _tmp_85 = mix(bandColor1,bandColor2,cycleT);
      }
      else {
        {
          _tmp_85 = mix(bandColor2,bandColor1,cycleT);
        }
      }
    }
  }
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_pos_world_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_K5 = niUIGpuFuncs__op__eq_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_Triangle);
  if (_tmp_K5) {
    vec3 worldNormal = intersection.worldNormal;
    vec3 worldPos = intersection.worldPos;
    vec3 _tmp_T5 = (mix(1.0,0.2,niUIGpuFuncs_PositionToWorldChecker(worldPos,10.0,worldNormal))*niUIGpuFuncs_PositionToHeat(worldPos,200.0));
    color = vec4(_tmp_T5.x,_tmp_T5.y,_tmp_T5.z,1.0);
  }
  else {
    bool _tmp_46 = niUIGpuFuncs__op__eq_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_InvalidTriangle);
    if (_tmp_46) {
      color = vec4(1.0,0.0,1.0,1.0);
    }
    else {
      bool _tmp_c6 = niUIGpuFuncs__op__eq_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_BoundingVolume);
      if (_tmp_c6) {
  ...
  vec4 _tmp_q6 = color;
  return nish_std_PixelOutput_new(_tmp_q6);
}
...
// Pixel Shader main: niUIGpuFuncs_raytracer_pos_world_ps
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = niUIGpuFuncs_raytracer_pos_world_ps(aInput, aUniforms, IN_1_aAS);
