...
EASTDC_API int Stricmp(const T* pString1, const T* pString2)
{
  ...
  while ((c1 = static_cast<T>(std::tolower(*pString1++))) ==
         (c2 = static_cast<T>(std::tolower(*pString2++))))
  {
    if (c1 == 0)
      return 0;
  }
...
template <typename T>
EASTDC_API inline size_t Strlcpy(T* pDestination, const T* pSource,
...
EASTDC_API inline size_t Strlcpy(wchar_t* pDestination, const char* pSource,
                                 size_t nDestCapacity)
{
  return mbstowcs(pDestination, pSource, nDestCapacity);
}

EASTDC_API inline size_t Strlcpy(char* pDestination, const wchar_t* pSource,
                                 size_t nDestCapacity)
{
