...
struct sUTCTM {
  sUTCTM()
  {
    tm_sec = tm_min = tm_hour = tm_mday = tm_mon = tm_year = 0;
  }
  int tm_sec;  /* seconds */
  int tm_min;  /* minutes */
  int tm_hour; /* hours */
...
static TM* SecondsToUTCTM(const Time64_T in_time, struct TM* p)
{
  ...
  p->tm_hour = v_tm_hour;
  p->tm_mon =
    v_tm_mon + 1; // v_tm_mon is computed in base 0, we store in base 1

  niAssert((julian_days_by_month[leap][p->tm_mon - 1] + (int)m) <=
           length_of_year[IS_LEAP(p->tm_year)]);
  niAssert(check_tm(p));
...
static const ni::achar* _kaszMonth[] = { "Jan", "Feb", "Mar", "Apr",
                                         "May", "Jun", "Jul", "Aug",
                                         "Sep", "Oct", "Nov", "Dec" };
static const ni::achar* _kaszMonthFull[] = {
  "January", "February", "March",     "April",   "May",      "June",
  "July",    "August",   "September", "October", "November", "December"
};
static const ni::achar* _kaszWeekday2[] = { "Su", "Mo", "Tu", "We",
                                            "Th", "Fr", "Sa" };
static const ni::achar* _kaszWeekday3[] = { "Sun", "Mon", "Tue", "Wed",
                                            "Thu", "Fri", "Sat" };
static const ni::achar* _kaszWeekdayFull[] = {
...
static void __stdcall _formatDateTime(const iTime* apTime, cString& aFormat,
                                      cString& aRet)
{
...
static bool _StrFormatTimeAToI(const char*& s, int& result, int low, int high,
                               int offset)
{
  bool worked = false;
  const char* end;
...
static const char* StrParseTime(const char* s, const char* format,
                                struct sFullTM* tm)
{
...
class cTime : public ImplRC<iTime, eImplFlags_Default, iToString> {
 public:
  ...
  {
    return niFmt("%d/%02d/%02d-%02d:%02d:%02d", GetYear(), GetMonth(), GetDay(),
