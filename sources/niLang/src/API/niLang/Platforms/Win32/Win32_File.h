...
class cOSWinFile : public ImplRC<iFileBase> {
 ...
 public:
  cOSWinFile(HANDLE ahRead, HANDLE ahWrite, tOSWinFileFlags aFileFlags,
  ...
  tBool __stdcall _CanReadWrite() const
  {
    if (niFlagIs(mFileFlags, eOSWinFileFlags_ConnectNamedPipe)) {
      ...
      niFlagOnIf(niThis(cOSWinFile)->mFileFlags, eInternalFlags_Connected,
                 bConnected);
    }
    return eTrue;
  }
  ...
  tBool __stdcall _SeekFileHandle(tI64 offset, DWORD moveMethod,
                                  tI64* curPos = NULL)
  {
  ...
  HANDLE __stdcall _GetReadHandle() const
  {
    return mhRead;
  }
  HANDLE __stdcall _GetWriteHandle() const
  {
    return mhWrite;
  }
