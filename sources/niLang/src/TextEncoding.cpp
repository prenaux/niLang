...
static void BitsStringWriteCloseMode(iFile* apFile, tI8 mode, tBool bExtIs32)
{
  switch (mode) {
  case 0: {
    apFile->WriteBits8(0, 3);
    break;
  }
  case 1: {
    apFile->WriteBits8(0, 4);
    break;
  }
  case 2: {
    apFile->WriteBits8(0, 6);
    break;
  }
  case 3: {
    apFile->WriteBits16(0, bExtIs32 ? 10 : 9);
    break;
  }
  default: return;
  }
...
static void BitsStringWrite(iFile* apFile, const achar* aaszString,
                            tBool abExtIs32)
{
  ...
  if (niIsStringOK(aaszString)) {
    ...
    for (;;) {
      ...
      else if (c >= 43 && c <= 57) { // mode 1 (10b) - numbers
        if (currentMode != 1) {
          BitsStringWriteCloseMode(apFile, currentMode, abExtIs32);
      ...
      else if (c >= 60 && c <= 122) { // mode 2 (01b) - eng chars
        if (currentMode != 2) {
          BitsStringWriteCloseMode(apFile, currentMode, abExtIs32);
...
static void BitsStringRead(iFile* apFile, ni::cString& strOut,
                           tBool abEndOnNewLine)
{
  tU32 t;
  tI8 currentMode = -1;
    ...
    if (currentMode >= 0) {
      ...
      case 3: { // mode 3 - extended
        if (bExtIs32) {
          ...
          if (apFile->ReadBit() == 0)
            t = apFile->ReadBitsU8(8);
          else
            t = apFile->ReadBitsU16(16);
          if (t == 0)
            currentMode = -1;
          else {
            strOut.appendChar(t);
...
eTextEncodingFormat __stdcall TextEncodingFormatGet(
  iFile* apFile, tBool abSkipHeader, eTextEncodingFormat aDefault)
{
  if (!niIsOK(apFile))
  ...
  if (apFile->ReadRaw(header, 2) == 2) {
    ...
    // UTF16LE
      ...
      if (!abSkipHeader)
        ...
        apFile->SeekSet(
          nPos +
          2); // we have to 'move back' one byte because we read a third byte
...
static inline tU32 _TextEncodingFormatReadCharUTF8(iFile* apFile)
{
  ...
  buf[0] = apFile->Read8();
  int charLen =
    utf8::sequence_length(buf); // squence_length only needs the first byte
  ...
  switch (charLen) {
  case 1: err = utf8::get_sequence_1(it, buf + 1, &cp); break;
  case 2: err = utf8::get_sequence_2(it, buf + 2, &cp); break;
  case 3: err = utf8::get_sequence_3(it, buf + 3, &cp); break;
  case 4: err = utf8::get_sequence_4(it, buf + 4, &cp); break;
  }
...
tU32 __stdcall TextEncodingFormatReadChar(iFile* apFile,
                                          eTextEncodingFormat aFormat)
{
  if (!niIsOK(apFile))
    return 0;
  switch (aFormat) {
  ...
  case eTextEncodingFormat_UTF16LE:
    return _TextEncodingFormatReadCharUTF16LE(apFile);
  case eTextEncodingFormat_UTF16BE:
    return _TextEncodingFormatReadCharUTF16BE(apFile);
  case eTextEncodingFormat_UTF32LE:
    return _TextEncodingFormatReadCharUTF32LE(apFile);
  case eTextEncodingFormat_UTF32BE:
    return _TextEncodingFormatReadCharUTF32BE(apFile);
  default: break;
  }
  return 0;
}
...
tSize __stdcall TextEncodingFormatWriteChar(iFile* apFile,
                                            eTextEncodingFormat aFormat,
                                            tU32 ch)
{
  if (!apFile)
  ...
  case eTextEncodingFormat_UTF8:
    return _TextEncodingFormatWriteCharUTF8(apFile, ch);
  case eTextEncodingFormat_UTF16LE:
    return _TextEncodingFormatWriteCharUTF16LE(apFile, ch);
  case eTextEncodingFormat_UTF16BE:
    return _TextEncodingFormatWriteCharUTF16BE(apFile, ch);
  case eTextEncodingFormat_UTF32LE:
    return _TextEncodingFormatWriteCharUTF32LE(apFile, ch);
  case eTextEncodingFormat_UTF32BE:
    return _TextEncodingFormatWriteCharUTF32BE(apFile, ch);
  default: break;
  }
  return 0;
}
...
///////////////////////////////////////////////
tBool __stdcall TextEncodingFormatWriteHeader(iFile* apFile,
                                              eTextEncodingFormat aFormat)
{
  if (!niIsOK(apFile))
    return eFalse;
  ...
  case eTextEncodingFormat_BitStream:
    apFile->WriteRaw(_kBOM_BITSTREAM, 2);
    break;
  default: bRet = eFalse; break;
  }
  return bRet;
}

///////////////////////////////////////////////
inline tSize __stdcall _TextEncodingFormatReadString(
  iFile* apFile, eTextEncodingFormat aFormat, cString& strOut,
  const tU32 aNewLineStop[2])
{
    ...
    while (1) {                                                              \
      ...
      if (aNewLineStop[0] && c == aNewLineStop[0]) {                         \
        ...
        if (t != 0 && t != aNewLineStop[1]) {                                \
          apFile->SeekSet(                                                   \
            pos); /* rewind one character - for '\r' only case */            \
        }                                                                    \
        break;                                                               \
      }                                                                      \
...
tSize __stdcall TextEncodingFormatReadString(iFile* apFile,
                                             eTextEncodingFormat aFormat,
                                             cString& strOut)
{
...
tSize __stdcall TextEncodingFormatReadStringLine(iFile* apFile,
                                                 eTextEncodingFormat aFormat,
                                                 cString& strOut)
{
...
///////////////////////////////////////////////
tSize __stdcall TextEncodingFormatWriteString(iFile* apFile,
                                              eTextEncodingFormat aFormat,
                                              const achar* aaszStr,
                                              tBool abWriteEndZero)
{
  niAssert(niIsOK(apFile));
  niCheckIsOK(apFile, 0);
...
#define WRITE_LOOP(WRITECHAR, WRITEZERO)                \
  ...
  case eTextEncodingFormat_UTF16LE:
    WRITE_LOOP(_TextEncodingFormatWriteCharUTF16LE, WriteLE16);
    break;
  case eTextEncodingFormat_UTF16BE:
    WRITE_LOOP(_TextEncodingFormatWriteCharUTF16BE, WriteBE16);
    break;
  case eTextEncodingFormat_UTF32LE:
    WRITE_LOOP(_TextEncodingFormatWriteCharUTF32LE, WriteLE32);
    break;
  case eTextEncodingFormat_UTF32BE:
    WRITE_LOOP(_TextEncodingFormatWriteCharUTF32BE, WriteBE32);
    break;
  case eTextEncodingFormat_BitStream: {
    tBool bIsWrittingBitStream = !apFile->IsWriteBitsBegan();
    tI64 nBeg = apFile->Tell();
    if (bIsWrittingBitStream)
