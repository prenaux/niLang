...
struct HelloUIWidget : public cWidgetSinkImpl<> {
  ...
  tBool __stdcall OnSinkAttached() niImpl
  {
    ...
    Ptr<iWidget> w = mpWidget->GetUIContext()->CreateWidget(
      "Button", mpWidget, Vec4f(0, 0, 250, 250), eWidgetStyle_HoldFocus,
      _HC(ID_Button));
    ...
    w = mpWidget->GetUIContext()->CreateWidget(
      "EditBox", mpWidget, Vec4f(0, 0, 350, 100),
  ...
  {
    apCanvas->BlitFill(mpWidget->GetWidgetRect(), 0xFF252525);

    const tU32 blitTextFlags = 0 | eFontFormatFlags_Border |
    ...
    apCanvas->BlitText(
      _monoFont,
      Rectf(20, 20, mpWidget->GetSize().x / 3, mpWidget->GetSize().y - 20), 0,
      _kText);
    apCanvas->Flush();
    return eFalse;
  }
