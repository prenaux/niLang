...
template <typename T>
struct HasSizeAndData<T, void_t<decltype(eastl::size(eastl::declval<T>())),
                                decltype(eastl::data(eastl::declval<T>()))>>
    : eastl::true_type {};
...
struct SubspanExtent
    : eastl::integral_constant<size_t, (Count != dynamic_extent
                                          ? Count
...
class span {
 public:
  ...
  template <size_t N, typename = enable_if_t<(Extent == eastl::dynamic_extent ||
                                              N == Extent)>>
  ...
  using SfinaeForGenericContainers =
    enable_if_t<!is_same_v<Container, span> &&
                !is_same_v<Container, astl::array<value_type>> &&
                !is_array_v<Container> &&
                Internal::HasSizeAndData<Container>::value &&
                is_convertible_v<remove_pointer_t<decltype(eastl::data(
                                   eastl::declval<Container&>()))> (*)[],
                                 element_type (*)[]>>;

  // generic container conversion constructors
  template <typename Container,
            typename = SfinaeForGenericContainers<Container>>
  EA_CONSTEXPR span(Container& cont);

  template <typename Container,
            typename = SfinaeForGenericContainers<const Container>>
  EA_CONSTEXPR span(const Container& cont);

  template <
    typename U, size_t N,
    typename = enable_if_t<(Extent == eastl::dynamic_extent || N == Extent) &&
  ...
  EA_CPP14_CONSTEXPR span<element_type, dynamic_extent> first(
    size_t Count) const;

  template <size_t Count>
  EA_CPP14_CONSTEXPR span<element_type, Count> last() const;
  EA_CPP14_CONSTEXPR span<element_type, dynamic_extent> last(
    size_t Count) const;
  ...
  subspan() const;
  EA_CONSTEXPR span<element_type, dynamic_extent> subspan(
    size_t Offset, size_t Count = dynamic_extent) const;
...
span(T (&)[N]) -> span<T, N>;
template <class T, size_t N>
span(astl::array<T, N>&) -> span<T, N>;
template <class T, size_t N>
span(const astl::array<T, N>&) -> span<const T, N>;
...
EA_CONSTEXPR span<T, Extent>::span() EA_NOEXCEPT
{
  static_assert(
    ...
    "impossible to default construct a span with a fixed Extent different than 0");
}
...
{
  niPanicAssertMsg(
    Extent == dynamic_extent || Extent == mnSize,
    "impossible to create a span with a fixed Extent different than the size of the supplied buffer");
}

template <typename T, ni::tSize Extent>
EA_CONSTEXPR span<T, Extent>::span(pointer pBegin, pointer pEnd)
    : mpData(pBegin)
    , mnSize(static_cast<index_type>(pEnd - pBegin))
{
  niPanicAssertMsg(
    Extent == dynamic_extent || Extent == mnSize,
    "impossible to create a span with a fixed Extent different than the size of the supplied buffer");
...
template <typename T, ni::tSize Extent>
template <size_t N, typename>
EA_CONSTEXPR span<T, Extent>::span(const astl::array<value_type, N>& arr)
  EA_NOEXCEPT : span(arr.data(), arr.size())
{
}
...
EA_CONSTEXPR span<T, Extent>::span(Container& cont)
    : span(static_cast<pointer>(eastl::data(cont)),
           static_cast<index_type>(eastl::size(cont)))
{
}
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::pointer span<T, Extent>::data() const
  EA_NOEXCEPT
{
  return mpData;
}
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::index_type span<T, Extent>::size() const
  EA_NOEXCEPT
{
  return mnSize;
}
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::index_type span<T, Extent>::size_bytes()
  const EA_NOEXCEPT
{
...
{
  niPanicAssertMsg(!empty(), "undefined behavior accessing an empty span");
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::reference span<T, Extent>::operator[](
  index_type idx) const
{
  niPanicAssertMsg(!empty(), "undefined behavior accessing an empty span");
  niPanicAssertMsg(bounds_check(idx),
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::iterator span<T, Extent>::begin() const
  EA_NOEXCEPT
{
  return mpData;
}
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::iterator span<T, Extent>::end() const
  EA_NOEXCEPT
{
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::const_iterator span<T, Extent>::cbegin()
  const EA_NOEXCEPT
{
  return mpData;
}
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::const_iterator span<T, Extent>::cend()
  const EA_NOEXCEPT
{
  return mpData + mnSize;
}
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::reverse_iterator span<
  T, Extent>::rbegin() const EA_NOEXCEPT
{
  return reverse_iterator(mpData + mnSize);
}

template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::reverse_iterator span<T, Extent>::rend()
  const EA_NOEXCEPT
{
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::const_reverse_iterator span<
  T, Extent>::crbegin() const EA_NOEXCEPT
{
...
template <typename T, ni::tSize Extent>
EA_CONSTEXPR typename span<T, Extent>::const_reverse_iterator span<
  T, Extent>::crend() const EA_NOEXCEPT
{
...
template <typename T, ni::tSize Extent>
template <size_t Count>
EA_CPP14_CONSTEXPR span<typename span<T, Extent>::element_type, Count> span<
  T, Extent>::first() const
{
  niPanicAssertMsg(Count <= size(),
                   "undefined behavior accessing out of bounds");
...
{
  niPanicAssertMsg(sz <= size(), "undefined behavior accessing out of bounds");
...
EA_CONSTEXPR span<typename span<T, Extent>::element_type,
                  Internal::SubspanExtent<Extent, Offset, Count>::value>
span<T, Extent>::subspan() const
{
  niPanicAssertMsg(Offset <= size(),
                   "undefined behaviour accessing out of bounds");
  niPanicAssertMsg(Count == dynamic_extent || Count <= (size() - Offset),
                   "undefined behaviour exceeding size of span");

  return { data() + Offset,
           eastl_size_t(Count == dynamic_extent ? size() - Offset : Count) };
}

template <typename T, ni::tSize Extent>
EA_CONSTEXPR span<typename span<T, Extent>::element_type, dynamic_extent> span<
  T, Extent>::subspan(size_t offset, size_t count) const
{
  niPanicAssertMsg(offset <= size(),
                   "undefined behaviour accessing out of bounds");
  niPanicAssertMsg(count == dynamic_extent || count <= (size() - offset),
