...
template <typename T, ni::tSize N>
EA_CONSTEXPR span<const T, N> make_span(const astl::array<T, N>& arr) noexcept
{
  return { arr };
}
...
template <typename ElementType, ni::tSize Extent>
span<const ni::tU8,
     ((Extent == eastl::dynamic_extent) ? eastl::dynamic_extent
...
template <class ElementType, size_t Extent,
          typename eastl::enable_if<!eastl::is_const<ElementType>::value,
                                    int>::type = 0>
span<ni::tU8,
     ((Extent == eastl::dynamic_extent) ? eastl::dynamic_extent
                                        : sizeof(ElementType) * Extent)>
...
template <class T, ni::tSize N, ni::tSize M>
EA_NODISCARD constexpr bool starts_with(span<T, N> data, span<T, M> prefix)
{
...
template <class T, ni::tSize N, ni::tSize M>
EA_NODISCARD constexpr bool ends_with(span<T, N> data, span<T, M> suffix)
{
...
template <class T, ni::tSize N, ni::tSize M>
EA_NODISCARD constexpr bool contains(span<T, N> data, span<T, M> sub)
{
  return eastl::search(data.begin(), data.end(), sub.begin(), sub.end()) !=
         data.end();
}
