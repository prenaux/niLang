...
class font_engine_freetype_base {
 public:
  ...
  bool load_font(ni::sMyFTLibrary* apLib, const char* font_name,
  ...
  {
    return agg_real(agg_real(m_height) / 64.0);
  }
  agg_real width() const
  {
    return agg_real(agg_real(m_width) / 64.0);
  }
  ...
  bool has_path() const;
#if niMinFeatures(20)
  bool get_glyph_path(unsigned glyph_code, bool flip_y, bool hinting,
                      const trans_affine& mtx, ni::iVGPath* apPath,
  ...
  agg_real advance_x() const
  {
    return m_advance_x;
  }
  agg_real advance_y() const
  {
    return m_advance_y;
  }
  agg_real real_width() const
  {
    return m_real_width;
  }
  agg_real real_height() const
  {
    return m_real_height;
  }
  void write_glyph_to(int8u* data) const;
  bool add_kerning(unsigned first, unsigned second, agg_real* x, agg_real* y);
  bool get_kerning(unsigned first, unsigned second, agg_real* x, agg_real* y);

  FT_Face get_face(ni::tU32 anIndex) const
  {
    return m_faces[anIndex];
  }
  FT_Face get_cur_face() const
  {
    return m_cur_face;
  }
