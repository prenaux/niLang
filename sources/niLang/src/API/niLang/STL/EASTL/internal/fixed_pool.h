...
#include "../type_traits.h"
  ...
  #pragma warning(push)
  #pragma warning( \
      disable : 4275) // non dll-interface class used as base for DLL-interface classkey 'identifier'
#endif
...
///
  ...
  #define EASTL_FIXED_POOL_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX           \
      " fixed_pool" // Unless the user overrides something, this is "EASTL fixed_pool".
#endif
  ...
  void* allocate()
  {
    ...
    if (pLink) // If we have space...
    {
      ...
      if (mpNext != mpCapacity) {
        ...
        mpNext = reinterpret_cast<Link*>(reinterpret_cast<char8_t*>(mpNext) +
                                         mnNodeSize);
...
class fixed_pool_with_overflow : public fixed_pool_base {
 public:
  ...
  fixed_pool_with_overflow(void* pMemory,
  ...
  fixed_pool_with_overflow(void* pMemory, size_t memorySize, size_t nodeSize,
      ...
      : mOverflowAllocator(EASTL_FIXED_POOL_DEFAULT_NAME)
  {
    fixed_pool_base::init(pMemory, memorySize, nodeSize, alignment,
                          alignmentOffset);
  ...
  void init(void* pMemory, size_t memorySize, size_t nodeSize, size_t alignment,
  ...
  void* allocate(size_t alignment, size_t alignmentOffset)
  {
    ...
    if (pLink) {
      ...
      if (mpNext != mpCapacity) {
        ...
        p = allocate_memory(mOverflowAllocator, mnNodeSize, alignment,
                            alignmentOffset);
        EASTL_ASSERT_MSG(
          p != nullptr,
 ...
 public:
  ...
  void*
    mpPoolBegin; // Ideally we wouldn't need this member variable. he problem is that the information about the pool buffer and object size is stored in the owning container and we can't have access to it without increasing the amount of code we need and by templating more code. It may turn out that simply storing data here is smaller in the end.
...
///
template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment,
          size_t nodeAlignmentOffset, bool bEnableOverflow,
          typename OverflowAllocator = EASTLAllocatorType>
class fixed_node_allocator {
 public:
  typedef typename type_select<bEnableOverflow,
                               fixed_pool_with_overflow<OverflowAllocator>,
                               fixed_pool>::type pool_type;
  typedef fixed_node_allocator<nodeSize, nodeCount, nodeAlignment,
                               nodeAlignmentOffset, bEnableOverflow,
                               OverflowAllocator>
    this_type;
  typedef OverflowAllocator overflow_allocator_type;
  ...
  enum {
    ...
    kNodesSize =
      nodeCount *
      nodeSize, // Note that the kBufferSize calculation assumes that the compiler sets sizeof(T) to be a multiple alignof(T), and so sizeof(T) is always >= alignof(T).
    kBufferSize = kNodesSize + ((nodeAlignment > 1) ? nodeSize - 1 : 0) +
                  nodeAlignmentOffset,
 ...
 public:
  ...
  fixed_node_allocator(void* pNodeBuffer)
      : mPool(pNodeBuffer, kNodesSize, kNodeSize, kNodeAlignment,
  ...
  fixed_node_allocator(void* pNodeBuffer,
                       const overflow_allocator_type& allocator)
      : mPool(pNodeBuffer, kNodesSize, kNodeSize, kNodeAlignment,
              kNodeAlignmentOffset, allocator)
  {
  }
  ...
  fixed_node_allocator(const this_type& x)
      : mPool(x.mPool.mpNext, kNodesSize, kNodeSize, kNodeAlignment,
              kNodeAlignmentOffset, x.mPool.mOverflowAllocator)
  {
  }
  ...
  {
    mPool.init(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment,
               kNodeAlignmentOffset);
  }
  ...
  void copy_overflow_allocator(
    const this_type&
      x) // This function exists so we can write generic code that works for allocators that do and don't have overflow allocators.
  {
...
template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment,
          size_t nodeAlignmentOffset, typename OverflowAllocator>
class fixed_node_allocator<nodeSize, nodeCount, nodeAlignment,
                           nodeAlignmentOffset, false, OverflowAllocator> {
 public:
 ...
 public:
  ...
  fixed_node_allocator(
    void* pNodeBuffer,
    const overflow_allocator_type& /*allocator*/) // allocator is unused because bEnableOverflow is false in this specialization.
      : mPool(pNodeBuffer, kNodesSize, kNodeSize, kNodeAlignment,
              kNodeAlignmentOffset)
  {
  }
  ...
  fixed_node_allocator(
    const this_type&
      x) // No need to copy the overflow allocator, because bEnableOverflow is false in this specialization.
      : mPool(x.mPool.mpNext, kNodesSize, kNodeSize, kNodeAlignment,
              kNodeAlignmentOffset)
  {
  }
  ...
  void copy_overflow_allocator(
    const this_type&) // This function exists so we can write generic code that works for allocators that do and don't have overflow allocators.
  {
    // We don't have an overflow allocator.
  }
...
inline bool operator==(
  const fixed_node_allocator<nodeSize, nodeCount, nodeAlignment,
                             nodeAlignmentOffset, bEnableOverflow,
                             OverflowAllocator>& a,
  const fixed_node_allocator<nodeSize, nodeCount, nodeAlignment,
                             nodeAlignmentOffset, bEnableOverflow,
...
template <size_t bucketCount, size_t nodeSize, size_t nodeCount,
          ...
          bool bEnableOverflow, typename OverflowAllocator = EASTLAllocatorType>
class fixed_hashtable_allocator {
 public:
  typedef typename type_select<bEnableOverflow,
                               fixed_pool_with_overflow<OverflowAllocator>,
                               fixed_pool>::type pool_type;
  typedef fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount,
                                    nodeAlignment, nodeAlignmentOffset,
  ...
  enum {
    ...
    kNodesSize =
      ...
      nodeSize, // Note that the kBufferSize calculation assumes that the compiler sets sizeof(T) to be a multiple alignof(T), and so sizeof(T) is always >= alignof(T).
    kBufferSize =
      kNodesSize + ((nodeAlignment > 1) ? nodeSize - 1 : 0) +
      nodeAlignmentOffset, // Don't need to include kBucketsSize in this calculation, as fixed_hash_xxx containers have a separate buffer for buckets.
    kNodeAlignment = nodeAlignment,
    kNodeAlignmentOffset = nodeAlignmentOffset,
    kAllocFlagBuckets =
      0x00400000 // Flag to allocator which indicates that we are allocating buckets and not nodes.
  };
 ...
 public:
  ...
  fixed_hashtable_allocator(void* pNodeBuffer)
      : mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment,
              kNodeAlignmentOffset)
      , mpBucketBuffer(NULL)
  {
  ...
  fixed_hashtable_allocator(const this_type& x)
      : mPool(x.mPool.mpHead, kBufferSize, kNodeSize, kNodeAlignment,
              kNodeAlignmentOffset, x.mPool.mOverflowAllocator)
  ...
  void* allocate(size_t n, int flags = 0)
  {
    ...
    EASTL_CT_ASSERT(
      kAllocFlagBuckets ==
      0x00400000); // Currently we expect this to be so, because the hashtable has a copy of this enum.

    if ((flags & kAllocFlagBuckets) ==
        0) // If we are allocating nodes and (probably) not buckets...
    {
      EASTL_ASSERT(n == kNodeSize);
      EA_UNUSED(n);
      return mPool.allocate();
...
template <size_t bucketCount, size_t nodeSize, size_t nodeCount,
          size_t nodeAlignment, size_t nodeAlignmentOffset,
          typename OverflowAllocator>
class fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount, nodeAlignment,
                                nodeAlignmentOffset, false, OverflowAllocator> {
 public:
  typedef fixed_pool pool_type;
  typedef fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount,
                                    nodeAlignment, nodeAlignmentOffset, false,
                                    OverflowAllocator>
    this_type;
  typedef OverflowAllocator overflow_allocator_type;
 ...
 public:
  ...
  fixed_hashtable_allocator(
    void* pNodeBuffer, void* pBucketBuffer,
    const overflow_allocator_type& /*allocator*/) // allocator is unused because bEnableOverflow is false in this specialization.
      : mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment,
              kNodeAlignmentOffset)
      , mpBucketBuffer(pBucketBuffer)
...
inline bool operator==(
  const fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount,
                                  nodeAlignment, nodeAlignmentOffset,
                                  bEnableOverflow, OverflowAllocator>& a,
  const fixed_hashtable_allocator<bucketCount, nodeSize, nodeCount,
                                  nodeAlignment, nodeAlignmentOffset,
                                  bEnableOverflow, OverflowAllocator>& b)
{
  return (&a == &b); // They are only equal if they are the same object.
}
...
class fixed_vector_allocator {
 public:
  typedef fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment,
                                 nodeAlignmentOffset, bEnableOverflow,
                                 OverflowAllocator>
    this_type;
  typedef OverflowAllocator overflow_allocator_type;
 ...
 public:
  ...
  void*
    mpPoolBegin; // To consider: Find some way to make this data unnecessary, without increasing template proliferation.
 ...
 public:
  ...
  {
    if (p != mpPoolBegin)
      mOverflowAllocator.deallocate(p,
                                    n); // Can't do this to our own allocation.
  }
...
template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment,
          size_t nodeAlignmentOffset, typename OverflowAllocator>
class fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment,
                             nodeAlignmentOffset, false, OverflowAllocator> {
 public:
  typedef fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment,
                                 nodeAlignmentOffset, false, OverflowAllocator>
  ...
  {
    EASTL_ASSERT(
      false); // A fixed_vector should not reallocate, else the user has exhausted its space.
    return NULL;
  }
...
template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment,
          ...
          typename OverflowAllocator>
inline bool operator==(
  const fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment,
                               nodeAlignmentOffset, bEnableOverflow,
                               OverflowAllocator>& a,
  const fixed_vector_allocator<nodeSize, nodeCount, nodeAlignment,
                               nodeAlignmentOffset, bEnableOverflow,
...
{
  return fixed_swap_impl<Container, sizeof(Container) >=
                                      EASTL_MAX_STACK_USAGE>::swap(a, b);
}
