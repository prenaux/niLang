...
TheoraVideoClip_Theora::TheoraVideoClip_Theora(ni::iFile* data_source,
...
bool TheoraVideoClip_Theora::_readData()
{
	...
	for (;;)
	{
		...
		int bytes_read = mStream->ReadRaw(buffer, 4096);
		...
		if (bytes_read < 4096)
		{
			...
			{
        mEndOfFile = true;
        // niLog(Info, (mName + " finished playing").c_str());
				return 0;
			}
		}
	...
	for (;;)
	{
		...
		if (ret > 0)
		{
			...
			if (time < mTimer->getTime() && !mRestarted && frame_number % 16 != 0)
			{
				...
				niWarning(mName + ": pre-dropped frame " + str((int) frame_number));
#endif
				++mNumDroppedFrames;
				continue; // drop frame
			}
...
bool TheoraVideoClip_Theora::load(ni::iFile* source)
{
#ifdef _DEBUG
	niWarning("-----");
#endif
    ...
    niError("Couldn't read vorbis headers.");
    return false;
  }
...
#ifdef _DEBUG
	niWarning((_ASTR("width: ") + str(mWidth) + ", height: " + str(mHeight) + ", fps: " + str((int) getFPS())).c_str());
#endif
	...
	const ni::tU64 streamSize = mStream->GetSize();
  ni::tU64 seekPos;
	for (unsigned int i = 1; i <= 50; ++i)
	{
		...
		mStream->SeekSet(seekPos);

		char *buffer = ogg_sync_buffer(&mInfo.OggSyncState, 4096 * i);
		int bytes_read = mStream->ReadRaw(buffer, 4096 * i);
		ogg_sync_wrote(&mInfo.OggSyncState, bytes_read);
      ...
      */
		...
		if (mNumFrames > 0 || streamSize < 4096 * i)
      break;

	}
	if (mNumFrames < 0)
	{
		niWarning("unable to determine file duration!");
	}
	else
	{
		mDuration = mNumFrames / mFPS;
#ifdef _DEBUG
		niWarning((_ASTR("duration: ") + strf(mDuration) + " seconds").c_str());
#endif
	}
		...
		// clip might have audio, it will be ignored
	{
		...
		TheoraAudioInterfaceFactory* audio_factory = GetTheoraAudioInterfaceFactory();
		if (audio_factory)
		{
...
bool TheoraVideoClip_Theora::readTheoraVorbisHeaders()
{
	ogg_packet tempOggPacket;
	bool done = false;
	bool decode_audio = (GetTheoraAudioInterfaceFactory() != NULL);
		   ...
		   (mVorbisStreams && (mVorbisStreams < 3)))
	{
		...
		else
		{
			...
			if (bytes_read == 0) {
				niError("End of file found prematurely");
        return false;
      }
		}
...
{
	int i;
  ni::tU64 seek_min = 0, seek_max = mStream->GetSize();
	long frame;
	ogg_int64_t granule = 0;

	if (targetFrame == 0) mStream->SeekSet(0);
	for (i = (targetFrame == 0) ? 100 : 0; i < 100; ++i)
	{
		ogg_sync_reset(&mInfo.OggSyncState);
		mStream->SeekSet(seek_min / 2 + seek_max / 2); // do a binary search
		memset(&mInfo.OggPage, 0, sizeof(ogg_page));
		ogg_sync_pageseek(&mInfo.OggSyncState, &mInfo.OggPage);
	...
	mStream->SeekSet((seek_min + seek_max) / 2); // do a binary search
	return -1;
}
...
void TheoraVideoClip_Theora::doSeek()
{
...
#ifdef _DEBUG
	//		niWarning(mName + " [seek]: nearest keyframe for frame " + str(mSeekFrame) + " is frame: " + str(frame));
#endif
	seekPage(astl::max(0, frame - 1), 0);
	...
	for (;mSeekFrame != 0;)
	{
		...
		else
		{
			if (!_readData())
			{
				niWarning((mName + " [seek]: fineseeking failed, _readData failed!").c_str());
...
#ifdef _DEBUG
	//	niWarning(mName + " [seek]: fineseeked to frame " + str(frame + 1) + ", requested: " + str(mSeekFrame));
#endif
	if (mAudioInterface)
	{
