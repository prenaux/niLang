...
template <typename T, bool = eastl::is_const<T>::value ||
...
#define EASTL_TYPE_TRAIT_add_volatile_CONFORMANCE \
  1 // add_volatile is conforming.

template <typename T, bool = eastl::is_volatile<T>::value ||
                             eastl::is_reference<T>::value ||
                             eastl::is_function<T>::value>
...
#define EASTL_TYPE_TRAIT_make_signed_CONFORMANCE \
  0 // make_signed is only partially conforming.
...
template <>
struct make_signed<const unsigned short> {
...
template <>
struct make_signed<const unsigned long> {
...
template <>
struct make_signed<const unsigned long long> {
  typedef const signed long long type;
};

#if (                                        \
  defined(CHAR_MAX) && defined(UCHAR_MAX) && \
  (CHAR_MAX ==                               \
   UCHAR_MAX)) // If char is unsigned, we convert char to signed char. However, if char is signed then make_signed returns char itself and not signed char.
template <>
...
struct make_signed<const char> {
  typedef signed char type;
       ...
       (__WCHAR_MAX__ ==         \
        4294967295U)) // If wchar_t is a 32 bit unsigned value...
template <>
struct make_signed<wchar_t> {
  typedef int32_t type;
};
  #elif (defined(__WCHAR_MAX__) && \
         (__WCHAR_MAX__ == 65535)) // If wchar_t is a 16 bit unsigned value...
template <>
struct make_signed<wchar_t> {
  typedef int16_t type;
};
  #elif (defined(__WCHAR_MAX__) && \
         (__WCHAR_MAX__ == 255)) // If wchar_t is an 8 bit unsigned value...
template <>
struct make_signed<wchar_t> {
  typedef int8_t type;
};
...
// To do: This implementation needs to be updated to support C++11 conformance (recognition of enums) and
// to support volatile-qualified types. It will probably be useful to have it fail for unsupported types.
#define EASTL_TYPE_TRAIT_make_unsigned_CONFORMANCE \
  0 // make_unsigned is only partially conforming.
...
template <>
struct make_unsigned<const signed short> {
...
template <>
struct make_unsigned<const signed long> {
...
template <>
struct make_unsigned<const signed long long> {
  typedef const unsigned long long type;
};

#if (        \
  CHAR_MIN < \
  0) // If char is signed, we convert char to unsigned char. However, if char is unsigned then make_unsigned returns char itself and not unsigned char.
template <>
...
struct make_unsigned<const char> {
  ...
  #if (defined(__WCHAR_MAX__) && \
       (__WCHAR_MAX__ !=         \
        4294967295U)) // If wchar_t is a 32 bit signed value...
template <>
struct make_unsigned<wchar_t> {
  typedef uint32_t type;
};
  #elif (defined(__WCHAR_MAX__) && \
         (__WCHAR_MAX__ != 65535)) // If wchar_t is a 16 bit signed value...
template <>
struct make_unsigned<wchar_t> {
  typedef uint16_t type;
};
  #elif (defined(__WCHAR_MAX__) && \
         (__WCHAR_MAX__ != 255)) // If wchar_t is an 8 bit signed value...
template <>
struct make_unsigned<wchar_t> {
...
#define EASTL_TYPE_TRAIT_remove_extent_CONFORMANCE \
  1 // remove_extent is conforming.
...
#define EASTL_TYPE_TRAIT_remove_all_extents_CONFORMANCE \
  1 // remove_all_extents is conforming.
...
#define EASTL_TYPE_TRAIT_aligned_storage_CONFORMANCE \
  1 // aligned_storage is conforming.
...
#elif (EABASE_VERSION_N >= 20040) && \
  !defined(EA_COMPILER_NO_ALIGNAS) // If C++11 alignas is supported...
template <size_t N, size_t Align = EASTL_ALIGN_OF(double)>
struct aligned_storage {
  typedef struct {
  ...
  (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION < 4007)) || \
  defined(                                                       \
    EA_COMPILER_EDG) // At some point GCC fixed their attribute(align) to support non-literals, though it's not clear what version aside from being no later than 4.7 and no earlier than 4.2.
...
#if defined(EA_COMPILER_NO_TEMPLATE_ALIASES)
  #define EASTL_ALIGNED_STORAGE_T(N, Align) \
    typename eastl::aligned_storage_t<N, Align>::type
#else
template <size_t N, size_t Align = EASTL_ALIGN_OF(double)>
...
///////////////////////////////////////////////////////////////////////
  ...
  !EASTL_TYPE_TRAIT_static_max_CONFORMANCE
  #define EASTL_TYPE_TRAIT_aligned_union_CONFORMANCE \
    0 // aligned_union is not conforming, as it supports only a two-member unions.
...
template <size_t minSize, typename Type0, typename Type1 = char,
          typename Type2 = char, typename Type3 = char>
struct aligned_union {
  ...
  static const size_t alignment0 =
    eastl::static_max<EA_ALIGN_OF(Type0), EA_ALIGN_OF(Type1)>::value;
  static const size_t alignment1 =
    eastl::static_max<alignment0, EA_ALIGN_OF(Type2)>::value;
  static const size_t alignment_value =
    eastl::static_max<alignment1, EA_ALIGN_OF(Type3)>::value;
...
using aligned_union_t =
  typename aligned_union<minSize, Type0, Type1, Type2, Type3>::type;
  #endif
#else
  #define EASTL_TYPE_TRAIT_aligned_union_CONFORMANCE \
    1 // aligned_union is conforming.

template <size_t minSize, typename Type0, typename... TypeN>
struct aligned_union {
  static const size_t size =
    eastl::static_max<minSize, sizeof(Type0), sizeof(TypeN)...>::value;
  static const size_t alignment_value =
    eastl::static_max<EA_ALIGN_OF(Type0), EA_ALIGN_OF(TypeN)...>::value;
...
DestType union_cast(SourceType sourceValue)
{
  EASTL_CT_ASSERT(
    ...
    (EA_ALIGN_OF(DestType) ==
     EA_ALIGN_OF(
       SourceType))); // To support differening alignments, we would need to use a memcpy-based solution or find a way to make the two union members align with each other.
