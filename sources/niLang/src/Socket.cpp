...
#ifdef niNoSocket
namespace ni {
ni::iSocket* __stdcall cLang::CreateSocketFromHandle(ni::tInt aSocket,
                                                     ni::iRemoteAddress* apAddr)
{
  niError("Not implemented");
  return NULL;
}
ni::iSocket* __stdcall cLang::CreateSocket(ni::eSocketProtocol aProtocol,
                                           ni::iRemoteAddress* apAddr)
{
  niError("Not implemented");
  return NULL;
...
static const tU32 _knSocketBufferSize = niStackBufferSize;
  ...
  #define CHECKVALID(RET)                                            \
    if (mSocket == INVALID_SOCKET) { /*niError("Invalid Socket.");*/ \
      return RET;                                                    \
    }
...
class cSocket : public ImplRC<ni::iSocket> {
 ...
 public:
  ...
  cSocket(tInt aSocket, iRemoteAddress* apAddr)
  {
    ...
    mProtocol =
      (type == SOCK_DGRAM) ? eSocketProtocol_UDP : eSocketProtocol_TCP;
    if (!mptrAddress.IsOK()) {
      sockaddr_in addr;
  ...
  cSocket(eSocketProtocol aProtocol, iRemoteAddress* apAddr = NULL)
  {
    ...
    case eSocketProtocol_UDP: {
      ...
      if (setsockopt(mSocket, SOL_SOCKET, SO_RCVBUF,
                     (char*)&_knSocketBufferSize, sizeof(int)) == SOCKET_ERROR)
      {
        _UpdateErrno();
  ...
  virtual tBool __stdcall Connect(iRemoteAddress* apAddress)
  {
    ...
    if (!_RemoteToSocketAddress(apAddress, addr)) {
      niError(
        niFmt("Invalid address: %s.", apAddress->GetAddressString().Chars()));
      return eFalse;
    }
  ...
  virtual tBool __stdcall Bind(iRemoteAddress* apAddr)
  {
    ...
    if (!_RemoteToSocketAddress(apAddr, addr)) {
      niError(
        niFmt("Invalid address: %s.", apAddr->GetAddressString().Chars()));
      return eFalse;
    }
  ...
  tI32 __stdcall DoSend(iFile* apFile, tSize anBytes, iRemoteAddress* apAddress)
  {
    ...
    while (toSend > 0) {
      ...
      if (apAddress) {
        sockaddr_in addr;
        if (!_RemoteToSocketAddress(apAddress, addr)) {
          niError(niFmt("Invalid address: %s.",
                        apAddress->GetAddressString().Chars()));
          return -1;
        }
        sent = sendto(mSocket, (char*)&buffer[pos], toSend, 0,
                      (sockaddrAlias*)&addr, sizeof(addr));
        if (sent < 0) {
          _UpdateErrno();
          return -1;
        }
                                     ...
                                     tSize anBufCount)
  {
    ...
    if (apAddress != NULL) {
      Ptr<iRemoteAddressIPv4> addr =
        ni::QueryInterface<iRemoteAddressIPv4>(apAddress);
      if (!addr.IsOK())
        return -1;
  ...
  #else
    if (WSASendTo(mSocket, (LPWSABUF)apBuffers, (DWORD)anBufCount,
                  (LPDWORD)&sentLength, 0,
                  apAddress != NULL ? (struct sockaddr*)&sin : 0,
                  apAddress != NULL ? sizeof(struct sockaddr_in) : 0, NULL,
                  NULL) == SOCKET_ERROR)
    {
      _UpdateErrno();
      return (mLastError == eSocketErrno_WouldBlock) ? 0 : -1;
  ...
  virtual tI32 __stdcall ReceiveFrom(iFile* apFile, iRemoteAddress* apAddress)
  {
    ...
    int ret = recvfrom(mSocket, (char*)&buffer[pos], _knSocketBufferSize, 0,
                       (sockaddrAlias*)&addr, &fromlen);
    if (ret == SOCKET_ERROR) {
      _UpdateErrno();
      return (mLastError == eSocketErrno_WouldBlock) ? 0 : -1;
    }
  ...
  ///////////////////////////////////////////////
  virtual tI32 __stdcall ReceiveBuffers(iRemoteAddress* apAddress,
                                        ...
                                        tSize anBufCount)
  {
    ...
    DWORD flags = 0;
    if (WSARecvFrom(mSocket, (LPWSABUF)apBuffers, (DWORD)anBufCount,
                    (LPDWORD)&recvLength, &flags,
                    apAddress ? (struct sockaddr*)&sin : NULL,
                    apAddress ? &sinLength : NULL, NULL, NULL) == SOCKET_ERROR)
    {
      _UpdateErrno();
      switch (mLastError) {
      case eSocketErrno_WouldBlock:
      case eSocketErrno_ConnectionReset: return 0;
      default: return -1;
      }
    }
                                          ...
                                          tU32 anTimeoutInMs)
  {
    ...
    int selectCount =
      select((int)(mSocket + 1), &readSet, &writeSet, NULL, &timeVal);
    if (selectCount < 0) {
      _UpdateErrno();
      return eSocketWaitFlags_Error;
    }
  ...
  virtual tBool __stdcall SetKeepAlive(tBool aKeepAlive)
  {
    ...
    return setsockopt(mSocket, SOL_SOCKET, SO_KEEPALIVE, (char*)&nKeepAlive,
                      nOptSize) == 0;
  #else
    niError("Not implemented");
    return eFalse;
  ...
  virtual tBool __stdcall GetIsKeepAlive() const
  {
    ...
    socklen_t nOptSize = sizeof(nKeepAlive);
  #if defined niPosix
    if (getsockopt(mSocket, SOL_SOCKET, SO_KEEPALIVE, &nKeepAlive, &nOptSize) !=
        0)
    {
    ...
    if (getsockopt(mSocket, SOL_SOCKET, SO_KEEPALIVE, (char*)&nKeepAlive,
                   &nOptSize) != 0)
    {
      niError("Can't get SO_KEEPALIVE.");
      return eFalse;
  ...
  //////////////////////////////////////////////////////////////
  virtual tBool __stdcall SetKeepAliveParameters(tU64 anTimeMS, tU64 anIntvlMS,
                                                 tU32 anProbes)
  {
  #if defined niOSX
    if (anTimeMS != 0) {
      tU64 nTimeMS = anTimeMS / 1000;
      if (setsockopt(mSocket, IPPROTO_TCP, TCP_KEEPALIVE, &nTimeMS,
                     sizeof(nTimeMS)) != 0)
      {
    ...
    if (anIntvlMS != 0) {
      ...
      if (setsockopt(mSocket, IPPROTO_TCP, TCP_KEEPINTVL, &nIntvl,
                     sizeof(nIntvl)) != 0)
      {
    ...
    if (anProbes != 0) {
      if (setsockopt(mSocket, IPPROTO_TCP, TCP_KEEPCNT, &anProbes,
                     sizeof(anProbes)) != 0)
      {
    ...
    if (anIntvlMS != 0) {
      ...
      if (setsockopt(mSocket, SOL_SOCKET, TCP_KEEPIDLE, &nTimeMS,
                     sizeof(nTimeMS)) != 0)
      {
    ...
    if (anIntvlMS != 0) {
      ...
      if (setsockopt(mSocket, SOL_SOCKET, TCP_KEEPINTVL, &nIntvl,
                     sizeof(nIntvl)) != 0)
      {
        niError("Can't set TCP_KEEPINTVL.");
        return eFalse;
    ...
    if (anProbes != 0) {
      ...
      if (setsockopt(mSocket, SOL_SOCKET, TCP_KEEPCNT, &nProbes,
                     sizeof(nProbes)) != 0)
      {
        niError("Can't set TCP_KEEPCNT.");
        return eFalse;
    ...
    stKeepAlive.onoff = 1;
    if (WSAIoctl(mSocket, SIO_KEEPALIVE_VALS, &stKeepAlive, sizeof(stKeepAlive),
                 NULL, 0, &nRet, NULL, NULL) != 0)
    {
  ...
  virtual tU64 __stdcall GetKeepAliveTime() const
  {
    ...
    if (getsockopt(mSocket, IPPROTO_TCP, TCP_KEEPALIVE, &nTimeMS, &nOptSize) !=
        0)
    {
  ...
  virtual tU64 __stdcall GetKeepAliveInterval() const
  {
    ...
    if (getsockopt(mSocket, IPPROTO_TCP, TCP_KEEPINTVL, &nIntvl, &nOptSize) !=
        0)
    {
  ...
  virtual tBool __stdcall GetReuseAddress() const
  {
    ...
    if (getsockopt(mSocket, SOL_SOCKET, SO_REUSEADDR, (char*)&optval,
                   &nOptSize) != 0)
    {
  ...
  tBool _RemoteToSocketAddress(iRemoteAddress* apAddr, sockaddr_in& dest)
  {
    if (!niIsOK(apAddr))
      return eFalse;
    if (apAddr->GetType() != eRemoteAddressType_IPv4)
      return eFalse;
...
iSocket* __stdcall cLang::CreateSocketFromHandle(ni::tInt aSocket,
                                                 ni::iRemoteAddress* apAddr)
{
  return niNew cSocket(aSocket, apAddr);
}
iSocket* __stdcall cLang::CreateSocket(ni::eSocketProtocol aProtocol,
                                       ni::iRemoteAddress* apAddr)
{
