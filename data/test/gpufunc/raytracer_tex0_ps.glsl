...
TestGpuFuncs_PixelOutput TestGpuFuncs_raytracer_tex0_ps(lib_shader_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
TestGpuFuncs_PixelOutput TestGpuFuncs_raytracer_tex0_ps(lib_shader_PixelInput aInput, TestGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_x1 = (intersectionType == lib_shader_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_x1) {
    ...
    bool _tmp_B2 = (intersectionType == lib_shader_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_B2) {
  ...
  vec4 _tmp_O2 = color;
  return TestGpuFuncs_PixelOutput_new(_tmp_O2);
}
...
void main(void) {
  ...
  TestGpuFuncs_PixelOutput _rval_ = TestGpuFuncs_raytracer_tex0_ps(aInput, aUniforms, IN_1_aAS);
