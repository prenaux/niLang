...
struct sStringIDSerializer : public ImplLocal<iSerializable> {
  sStringIDSerializer(cString& aStringID)
      : mStringID(aStringID)
  {
  }
  ...
  tSize __stdcall Serialize(iFile* apFile, eSerializeMode aMode)
  {
    ...
    case eSerializeMode_WriteRaw: {
      return apFile->WriteStringZEx(eTextEncodingFormat_BitStream,
                                    mStringID.Chars());
    }
    default: break;
    }
...
static iUnknown* _CreateObjectInstance(const achar* aOID, const Var& aVarA,
                                       const Var& aVarB)
{
...
///////////////////////////////////////////////
tBool __stdcall cLang::IsSerializedObject(iFile* apFile, const achar* aID,
                                          tI64* apObjSize)
{
  ...
  if (apFile->ReadLE32() == kfccSerializedObject) {
    ...
    if (apFile->ReadBit()) {
      cString id = AZEROSTR;
      if (sStringIDSerializer(id).Serialize(apFile, eSerializeMode_ReadRaw) !=
          eInvalidHandle)
      {
        if (id.IEq(aID))
          bRes = eTrue;
...
///////////////////////////////////////////////
tI64 __stdcall cLang::ReadSerializationHeader(iFile* apFile, cString* apID,
                                              tI64* apObjSize)
{
  ...
  if (apFile->ReadLE32() == kfccSerializedObject) {
    ...
    apFile->BeginReadBits();
    if (apFile->ReadBit()) {
      if (sStringIDSerializer(*apID).Serialize(
            apFile, eSerializeMode_ReadRaw) == eInvalidHandle)
      {
...
tI64 __stdcall cLang::WriteSerializationHeader(iFile* apFile, const achar* apID,
                                               tI64 anSize)
{
  niCheck(niIsOK(apFile), eInvalidHandle);
  ...
  if (apID && niIsStringOK(apID)) {
    ...
    cString objID = apID;
    if (sStringIDSerializer(objID).Serialize(apFile, eSerializeMode_WriteRaw) ==
        eInvalidHandle)
    {
                                           ...
                                           eSerializeMode aMode, tI64* apSize)
{
  ...
  case eSerializeMode_WriteRaw: {
    ...
    if (aMode != eSerializeMode_WriteRaw) {
      ...
      if (WriteSerializationHeader(apFile,
                                   pSerializable->GetSerializeObjectTypeID(),
                                   nObjSize) == eInvalidHandle)
      {
  ...
  case eSerializeMode_ReadRaw: {
    ...
    if (objID.IsEmpty()) {
      ...
      if (pSerializable == NULL) {
        niError(_A(
          "Object doesnt implement iSerializable and no object ID serialized."));
        apFile->SeekSet(nDestPos);
        return NULL;
      }
    }
      ...
      if (!apObject) {
        ...
        if (!niIsOK(apObject)) {
          niError(
            niFmt(_A("Can't create an instance of the object '%s'."), objID));
          apFile->SeekSet(nDestPos);
          return NULL;
        }
      }
