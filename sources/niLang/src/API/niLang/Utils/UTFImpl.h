...
const ni::tU32 knUTFSurrogateOffset =
  0x10000u - (knUTFLeadSurrogateMin << 10) - knUTFTrailSurrogateMin;
...
{
  return (cp <= knUTFCodePointMax && !utf_is_surrogate(cp) && cp != 0xfffe &&
          cp != 0xffff);
}
...
struct utf8 {
  ...
  static inline eUTFError get_sequence_1(const cchar*& it, const cchar* end,
                                         ni::tU32* code_point)
  {
    if (it != end) {
      if (code_point)
  ...
  static inline eUTFError get_sequence_2(const cchar*& it, const cchar* end,
                                         ni::tU32* code_point)
  {
  ...
  static inline eUTFError get_sequence_3(const cchar*& it, const cchar* end,
                                         ni::tU32* code_point)
  {
    eUTFError ret_code = eUTFError_NotEnoughtRoom;
  ...
  static inline eUTFError get_sequence_4(const cchar*& it, const cchar* end,
                                         ni::tU32* code_point)
  {
    eUTFError ret_code = eUTFError_NotEnoughtRoom;
  ...
  static inline ni::tU32 raw_next(const cchar*& it)
  {
    ...
    case 4:
      ...
      cp += (*it) & 0x3f;
      break;
    default: niUTFAssertUnreachable("Invalid utf sequence_length."); break;
    }
    ++it;
    return cp;
  }
  ...
  static inline cchar* replace_invalid(const cchar* start, const cchar* end,
                                       cchar* out, ni::tU32 replacement)
  {
    while (start != end) {
      ...
      case eUTFError_OK:
        for (const cchar* it = sequence_start; it != start; ++it)
          *out++ = *it;
        break;
      case eUTFError_NotEnoughtRoom: niAssert(0 && "not_enough_room"); break;
      case eUTFError_InvalidLead:
