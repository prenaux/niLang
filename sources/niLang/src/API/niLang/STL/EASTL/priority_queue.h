...
///
  ...
  #define EASTL_PRIORITY_QUEUE_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX               \
      " priority_queue" // Unless the user overrides something, this is "EASTL priority_queue".
#endif

/// EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR
///
#ifndef EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR
  #define EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_PRIORITY_QUEUE_DEFAULT_NAME)
#endif
...
class priority_queue {
 ...
 public
  ...
  container_type
    c; // The C++ standard specifies that you declare a protected member variable of type Container called 'c'.
  compare_type
    comp; // The C++ standard specifies that you declare a protected member variable of type Compare called 'comp'.
 ...
 public:
  ...
  explicit priority_queue(
    ...
    typename eastl::enable_if<
      eastl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
      : c(allocator)
      , comp()
  {
  }
  ...
  template <class Allocator>
  priority_queue(
    const this_type& x, const Allocator& allocator,
    typename eastl::enable_if<
      eastl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
      : c(x.c, allocator)
      , comp(x.comp)
  ...
  template <class Allocator>
  priority_queue(
    this_type&& x, const Allocator& allocator,
    typename eastl::enable_if<
      eastl::uses_allocator<container_type, Allocator>::value>::type* = NULL)
  ...
  priority_queue(
    ...
    const compare_type& compare =
      compare_type()); // C++11 doesn't specify that std::priority_queue has initializer list support.
  ...
  void change(
    size_type
      n); /// Moves the item at the given array index to a new location based on its current priority.
  void remove(size_type n); /// Removes the item at the given array index.

  container_type& get_container();
  const container_type& get_container() const;

  void swap(this_type& x) EA_NOEXCEPT_IF(
    (eastl::is_nothrow_swappable<this_type::container_type>::value &&
...
inline priority_queue<T, Container, Compare>::priority_queue()
    : c()
    , // To consider: use c(EASTL_PRIORITY_QUEUE_DEFAULT_ALLOCATOR) here, though that would add the requirement that the user supplied container support this.
    comp()
{
}
...
template <typename T, typename Container, typename Compare>
inline priority_queue<T, Container, Compare>::priority_queue(
  std::initializer_list<value_type> ilist, const compare_type& compare)
    : c()
    , comp(compare)
{
...
template <typename InputIterator>
inline priority_queue<T, Container, Compare>::priority_queue(
  InputIterator first, InputIterator last, const compare_type& compare)
    : c(first, last)
    , comp(compare)
{
  eastl::make_heap(c.begin(), c.end(), comp);
...
template <typename T, typename Container, typename Compare>
inline typename priority_queue<T, Container, Compare>::size_type priority_queue<
  T, Container, Compare>::size() const
{
  return c.size();
}
...
{
  push(value_type(eastl::forward<Args>(
    args)...)); // The C++11 Standard 23.6.4/1 states that c.emplace is used, but also declares that c doesn't need to have an emplace function.
}
...
{
  value = eastl::move(
    c.front()); // To consider: value = move_if_noexcept_assignable(c.front());
  pop();
}
...
template <typename T, typename Container, typename Compare>
bool operator==(const priority_queue<T, Container, Compare>& a,
...
inline void swap(priority_queue<T, Container, Compare>& a,
                 priority_queue<T, Container, Compare>& b)
  EA_NOEXCEPT_IF((
    eastl::is_nothrow_swappable<
      typename priority_queue<T, Container, Compare>::container_type>::value &&
    eastl::is_nothrow_swappable<
      typename priority_queue<T, Container, Compare>::compare_type>::
