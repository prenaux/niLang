...
#define IS_COMPATIBLE_PXF(PXF) \
  true //(PXF->GetBytesPerPixel() == 4 || PXF->GetBytesPerPixel() == 1)
 ...
 public:
  ...
  virtual tBool __stdcall Copy(iVGImage* apImage)
  {
    ...
    if (apImage->GetHasBitmap() || apImage->GetHasTexture()) {
      ...
      niAssert(niIsOK(dst));
      iBitmap2D* src =
        apImage->GrabBitmap(eVGImageUsage_Source, sRecti::Null());
      niAssert(niIsOK(src));
    ...
    if (aLock == eVGImageUsage_DepthStencil) {
      if (!(mnFlags & VGIMAGEFLAGS_DSCREATED)) {
        ...
        mptrTexture = mptrGraphics->CreateTexture(
          VGIMAGE_NEWDSNAME, eBitmapType_2D, _A("D24S8"), 0, mnWidth, mnHeight,
          0, eTextureFlags_DepthStencil);
      ...
      if (!(mnFlags & VGIMAGEFLAGS_TEXCREATED) ||
          (aLock == eVGImageUsage_Target && !(mnFlags & VGIMAGEFLAGS_TEXRT)))
      {
        mnFlags |= VGIMAGEFLAGS_TEXCREATED;
        if (aLock == eVGImageUsage_Target)
        ...
        mptrTexture = mptrGraphics->CreateTexture(
          VGIMAGE_NEWTEXNAME, eBitmapType_2D, _GetPixelFormat()->GetFormat(), 0,
          mnWidth, mnHeight, 0,
          eTextureFlags_Overlay |
            ((aLock == eVGImageUsage_Target) ? eTextureFlags_RenderTarget
...
///////////////////////////////////////////////
Ptr<iVGImage> __stdcall cGraphics::CreateVGImageFromBitmapAndTexture(
  iBitmap2D* apBitmap, iTexture* apTexture)
{
  ...
  Ptr<iVGImage> img =
    niNew cVGImage(QPtr<iGraphics>(this), apBitmap, apTexture);
...
Ptr<iVGImage> cGraphics::CreateVGImageFromResource(iHString* ahspRes)
{
  ...
  Ptr<iVGImage> img =
    niNew cVGImage(QPtr<iGraphics>(this), QPtr<iBitmap2D>(ptrBmp));
...
Ptr<iVGImage> cGraphics::CreateVGImageFromAnyBitmap(iBitmap2D* apBmp)
{
  ...
  if (!ni::StrEq(apBmp->GetPixelFormat()->GetFormat(), AGG_PIXELFORMAT)) {
    bmp = (iBitmap2D*)bmp->CreateConvertedFormat(
      this->CreatePixelFormat(AGG_PIXELFORMAT));
  }
