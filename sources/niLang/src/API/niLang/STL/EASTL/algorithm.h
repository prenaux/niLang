...
///
template <typename ForwardIterator, typename Compare>
eastl::pair<ForwardIterator, ForwardIterator> minmax_element(
...
{
  return eastl::forward<T>(median_impl<T&&, Compare>(
    eastl::forward<T>(a), eastl::forward<T>(b), eastl::forward<T>(c), compare));
}
                                  ...
                                  EASTL_ITC_NS::random_access_iterator_tag)
{
  return eastl::move(
    first, first + n,
    result); // Take advantage of the optimizations present in the move algorithm.
}
                                  ...
                                  EASTL_ITC_NS::random_access_iterator_tag)
{
  return eastl::copy(
    first, first + n,
    result); // Take advantage of the optimizations present in the copy algorithm.
}
...
struct move_and_copy_backward_helper {
  ...
  static BidirectionalIterator2 move_or_copy_backward(
...
struct move_and_copy_backward_helper<EASTL_ITC_NS::random_access_iterator_tag,
  ...
  {
    typedef
      typename eastl::iterator_traits<BidirectionalIterator1>::difference_type
        difference_type;
...
inline BidirectionalIterator2 move_and_copy_backward_chooser(
  BidirectionalIterator1 first, BidirectionalIterator1 last,
  BidirectionalIterator2 resultEnd)
{
  typedef
    typename eastl::iterator_traits<BidirectionalIterator1>::iterator_category
      IIC;
  typedef
    typename eastl::iterator_traits<BidirectionalIterator2>::iterator_category
      OIC;
  ...
  return eastl::move_and_copy_backward_helper<IIC, isMove, canBeMemmoved>::
    move_or_copy_backward(
      first, last,
      resultEnd); // Need to chose based on the input iterator tag and not the output iterator tag, because containers accept input ranges of iterator types different than self.
}
...
template <bool isMove, typename BidirectionalIterator1,
          typename BidirectionalIterator2>
inline BidirectionalIterator2 move_and_copy_backward_unwrapper(
  BidirectionalIterator1 first, BidirectionalIterator1 last,
  BidirectionalIterator2 resultEnd)
{
  return BidirectionalIterator2(eastl::move_and_copy_backward_chooser<isMove>(
    ...
    eastl::unwrap_iterator(
      resultEnd))); // Have to convert to BidirectionalIterator2 because result.base() could be a T*
}
...
inline typename eastl::iterator_traits<InputIterator>::difference_type count(
...
inline typename eastl::iterator_traits<InputIterator>::difference_type count(
  InputIterator first, InputIterator last, const T& value, Predicate predicate)
{
  typename eastl::iterator_traits<InputIterator>::difference_type result = 0;
...
///
template <typename InputIterator, typename Predicate>
inline typename eastl::iterator_traits<InputIterator>::difference_type count_if(
                          ...
                          const T& value)
{
  while (
    (first != last) &&
    !(*first ==
      value)) // Note that we always express value comparisons in terms of < or ==.
    ++first;
  return first;
}
...
inline ForwardIterator1 find_first_not_of(ForwardIterator1 first1,
                                          ForwardIterator1 last1,
                                          ForwardIterator2 first2,
                                          ForwardIterator2 last2,
                                          BinaryPredicate predicate)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator1>::value_type value_type;
...
inline BidirectionalIterator1 find_last_of(BidirectionalIterator1 first1,
                     ...
                     Generator generator)
{
  for (; first != last;
    ...
    *first =
      generator(); // because the 'last-first' might not be supported by the
} // given iterator.
...
///
template <typename InputIterator1, typename InputIterator2>
EA_CPP14_CONSTEXPR inline bool equal(InputIterator1 first1,
                                     ...
                                     InputIterator2 first2)
{
  for (; first1 != last1; ++first1, ++first2) {
    if (
      !(*first1 ==
        *first2)) // Note that we always express value comparisons in terms of < or ==.
      return false;
  }
  return true;
}
...
#if defined( \
  _MSC_VER) // If using the VC++ compiler (and thus bool is known to be a single byte)...
...
inline eastl::pair<InputIterator1, InputIterator2> mismatch(
  ...
  InputIterator2 first2) // , InputIterator2 last2)
{
  while (
    (first1 != last1) &&
    (*first1 ==
     *first2)) // && (first2 != last2) <- C++ standard mismatch function doesn't check first2/last2.
  {
    ++first1;
    ++first2;
  }
                            ...
                            const T& value)
{
  ...
  DifferenceType d = eastl::distance(
    first,
  ...
  while (d > 0) {
    ...
    DifferenceType d2 =
      d >>
      1; // We use '>>1' here instead of '/2' because MSVC++ for some reason generates significantly worse code for '/2'. Go figure.

    eastl::advance(
      i,
      d2); // This will be efficient for a random access iterator such as an array.
                            ...
                            const T& value)
{
  ...
  while (len > 0) {
    ...
    if (
      !(value <
        *i)) // Note that we always express value comparisons in terms of < or ==.
    {
      first = ++i;
      len -= len2 + 1;
    }
                                 ...
                                 Predicate predicate)
{
  ...
  if (first != last) {
    ForwardIterator i(first);
    return eastl::remove_copy_if<ForwardIterator, ForwardIterator, Predicate>(
                         ...
                         EASTL_ITC_NS::bidirectional_iterator_tag)
{
  for (; (first != last) && (first != --last);
    ...
    eastl::iter_swap(first,
                     last); // generic (bidirectional or otherwise) iterator.
}
...
{
  typedef
    typename eastl::iterator_traits<BidirectionalIterator>::iterator_category
      IC;
                        ...
                        BinaryPredicate predicate)
{
  ...
  if (d2 != 0) {
    ...
    {
      if (eastl::equal<ForwardIterator1, ForwardIterator2, BinaryPredicate>(
            first1, i, first2, predicate))
        return first1;
              ...
              const T& value, EASTL_ITC_NS::forward_iterator_tag)
{
  ...
  if (
    count >
    d1) // ForwardIterator may not be a true iterator but instead something like a pointer.
    return last;
...
search_n_impl(RandomAccessIterator first, RandomAccessIterator last, Size count,
...
ForwardIterator unique(ForwardIterator first, ForwardIterator last)
{
  ...
  if (
    first !=
    last) // We expect that there are duplicated items, else the user wouldn't be calling this function.
  {
    ...
    for (++first; first != last; ++first) {
      if (
        !(*dest ==
          *first)) // Note that we always express value comparisons in terms of < or ==.
        *++dest = *first;
    }
    return ++dest;
  }
...
template <typename BidirectionalIterator1, typename BidirectionalIterator2>
BidirectionalIterator1 find_end_impl(BidirectionalIterator1 first1,
                                     BidirectionalIterator1 last1,
...
{
  typedef
    typename eastl::iterator_traits<ForwardIterator1>::iterator_category IC1;
  typedef
    typename eastl::iterator_traits<ForwardIterator2>::iterator_category IC2;
                               ...
                               EASTL_ITC_NS::forward_iterator_tag)
{
    ...
    last2) // We have to do this check because the search algorithm below will return first1 (and not last1) if the first2/last2 range is empty.
  {
    for (ForwardIterator1 result = last1;;) {
      const ForwardIterator1 resultNext(
        eastl::search<ForwardIterator1, ForwardIterator2, BinaryPredicate>(
    ...
    if (first != --i) // If the range has more than one item...
    {
      for (;;) {
        ...
        if (
          compare(
            *--i,
            *ii)) // Find two consecutive values where the first is less than the second.
        {
          j = last;
          while (!compare(
            *i,
            *--j)) // Find the final value that's greater than the first (it may be equal to the second).
          {
          }
        ...
        if (
          i ==
          first) // There are no two consecutive values where the first is less than the second, meaning the range is in reverse order. The reverse ordered range is always the last permutation.
        {
                                        ...
                                        ForwardIterator last)
{
  ...
  ForwardIterator result = eastl::move(
    eastl::next(first), last,
    ...
    if (middle != last) {
      ...
      return Internal::rotate_helper < IC,
             eastl::is_trivially_move_assignable<value_type>::
                 value || // This is the best way of telling if we can move types via memmove, but without a conforming C++11 compiler it usually returns false.
               eastl::is_pod<value_type>::
                 value || // This is a more conservative way of telling if we can move types via memmove, and most compilers support it, but it doesn't have as full of coverage as is_trivially_move_assignable.
               eastl::is_scalar<value_type>::
                   value > // This is the most conservative means and works with all compilers, but works only for scalars.
