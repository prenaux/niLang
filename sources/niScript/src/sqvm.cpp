...
#pragma niTodo( \
    "DeepClone can get in infinite loop because of some internal cycles, fix this.")
...
SQVM::SQVM(SQSharedState* aSS)
{
  ...
  _emptystring = _H("");
  _execDumpStack =
    !!ni::GetLang()->GetProperty("niScript.ExecDumpStack").Bool();
}
...
SQObjectPtr SQVM::PrintObjVal(const SQObject& o)
{
  ...
  default: return _ss->GetTypeNameObj(o);
  }
}
...
void SQVM::Raise_ObjError(const SQObjectPtr& desc, bool aPrint)
{
  ...
  if (aPrint && (_raiseErrorMode > eScriptRaiseErrorMode_Error)) {
    cString str = AEOL;
    sqFormatError(this, _sqtype(desc) == OT_STRING ? _stringval(desc) : NULL,
                  str, 0);
    niLog(Raw, str.Chars());
  }
}
...
void SQVM::Raise_CompareError(const SQObject& o1, const SQObject& o2)
{
  ...
  Raise_MsgError(niFmt(_A("compare between '%.50s' and '%.50s'"),
                       _stringval(oval1), _stringval(oval2)));
}
...
bool SQVM::ArithMetaMethod(int op, const SQObjectPtr& o1, const SQObjectPtr& o2,
                           SQObjectPtr& dest)
{
  SQMetaMethod mm;
  switch (op) {
  ...
  default: niAssertUnreachable("Unreachable"); return false;
  }
  switch (_sqtype(o1)) {
  case OT_TABLE:
...
bool SQVM::Modulo(const SQObjectPtr& o1, const SQObjectPtr& o2,
                  SQObjectPtr& dest)
{
  if (sq_isnumeric(o1) && sq_isnumeric(o2)) {
    ...
    case OT_USERDATA:
      if (_userdata(o1)->GetDelegate()) {
        Push(o1);
        Push(o2);
        if (CallMetaMethod(_userdata(o1)->GetDelegate(), MT_ARITH_MODULO, 2,
                           dest))
          return true;
      }
      break;
    ...
    VM_ERRORB(niFmt(_A("modulo between '%s' and '%s'"), _ss->GetTypeNameStr(o1),
...
bool SQVM::ObjEq(const SQObjectPtr& o1, const SQObjectPtr& o2, int& result)
{
  if (_sqtype(o1) == _sqtype(o2)) {
    ...
    switch (_sqtype(o1)) {
    case OT_NULL: result = 0; return true;
    case OT_STRING:
...
bool SQVM::ObjCmp(const SQObjectPtr& o1, const SQObjectPtr& o2, int& result)
{
  if (_sqtype(o1) == OT_IUNKNOWN) {
    ...
    case OT_INTEGER: result = CmpByVal(_int(o1), _int(o2)); return true;
    case OT_FLOAT: result = CmpByVal(_float(o1), _float(o2)); return true;
    case OT_TABLE:
      if (_table(o1)->CanCallMetaMethod()) {
        Push(o1);
        Push(o2);
...
bool SQVM::StringCat(const SQObjectPtr& str, const SQObjectPtr& obj,
                     SQObjectPtr& dest)
{
  switch (_sqtype(obj)) {
  case OT_STRING: {
    ...
    case OT_INTEGER:
      ...
      break;
    case OT_NULL: newStr.Format(_A("(null)%s"), _stringval(obj)); break;
    default:
  ...
  case OT_NULL: dest = _H(niFmt("%s(null)", _stringval(str))); break;
  case OT_FLOAT: {
    cString newStr;
    newStr += _stringval(str);
    ...
    if (hsp) {
      ...
      if (ptrToString.IsOK()) {
            ...
            _sqtype(res) == OT_STRING)
        {
          ...
          newStr << niFmt("<%s>@%p", strUnknown.Chars(),
  ...
  case OT_USERDATA:
    if (_userdata(obj)->GetDelegate()) {
      SQObjectPtr res;
      Push(obj);
      if (CallMetaMethod(_userdata(obj)->GetDelegate(), MT_STD_TOSTRING, 1,
                         res))
      {
        if (_sqtype(res) != OT_STRING) {
          VM_ERRORB(niFmt(
            _A(
              "string concatenation between '%s' and userdata, _tostring returned a '%s' not a string."),
            _ss->GetTypeNameStr(str), _ss->GetTypeNameStr(obj),
            _ss->GetTypeNameStr(res)));
        }
        else {
          cString newStr;
          newStr = _stringval(str);
    ...
    VM_ERRORB(niFmt(
      _A("string concatenation between '%s' and userdata without _tostring "),
      _ss->GetTypeNameStr(str), _ss->GetTypeNameStr(obj)));
    break;
  case OT_TABLE: {
    if (_table(obj)->CanCallMetaMethod()) {
      ...
      if (CallMetaMethod(_table(obj), MT_STD_TOSTRING, 1, res)) {
        if (_sqtype(res) != OT_STRING) {
          VM_ERRORB(niFmt(
            _A(
              "string concatenation between '%s' and table, _tostring returned a '%s' not a string."),
            _ss->GetTypeNameStr(str), _ss->GetTypeNameStr(obj),
            _ss->GetTypeNameStr(res)));
        }
        else {
...
void SQVM::TypeOf(const SQObjectPtr& obj1, SQObjectPtr& dest)
{
  ...
  case OT_USERDATA:
    if (_userdata(obj1)->GetDelegate()) {
      Push(obj1);
      if (CallMetaMethod(_userdata(obj1)->GetDelegate(), MT_STD_TYPEOF, 1,
                         dest))
        return;
    }
  }
...
bool SQVM::Init(bool abInitRootTable)
{
  ...
  if (abInitRootTable) {
    ...
    RegisterSQRegFunctions(as_nn(_table(_roottable)),
                           SQSharedState::_base_funcs);
    RegisterSQRegFunctions(as_nn(_table(_roottable)),
                           SQSharedState::_lint_funcs);
    RegisterSQRegFunctions(as_nn(_table(_roottable)),
                           SQSharedState::_automation_funcs);
  }
  else {
    _roottable = _null_;
  }
...
{
  return aType == OT_TABLE || aType == OT_ARRAY || aType == OT_STRING ||
         aType == OT_USERDATA;
}
...
bool SQVM::NewSlot(const SQObjectPtr& self, const SQObjectPtr& key,
                   const SQObjectPtr& val, int opExt)
{
  niLet noNewSlot = [&]() -> bool {
    ...
    VM_ERRORB(niFmt(_A("newslot indexing %s with %s"),
  ...
  case OT_USERDATA: {
    if (_userdata(self)->GetDelegate()) {
      ...
      if (DoGet(self, key, res, NULL, 0)) {
        ...
        Push(key);
        Push(val);
        return CallMetaMethod(_userdata(self)->GetDelegate(),
                              MT_USERDATA_NEWSLOT, 3, res);
      }
    }
    return noNewSlot();
  }
...
bool SQVM::DeleteSlot(const SQObjectPtr& self, const SQObjectPtr& key,
                      SQObjectPtr& res, int opExt)
{
  ...
  case OT_USERDATA:
    if (_userdata(self)->GetDelegate()) {
      ...
      Push(key);
      SQObjectPtr t;
      if (!CallMetaMethod(_userdata(self)->GetDelegate(), MT_USERDATA_DELSLOT,
                          2, t))
      {
  ...
  default:
    ...
    VM_ERRORB(niFmt(_A("attempt to delete a slot from a %s"),
...
bool SQVM::CallNative(SQNativeClosure* nclosure, int nargs, int stackbase,
                      SQObjectPtr& retval)
{
  ...
  {
    VM_ERRORB(niFmt(
      "wrong number of parameters, native closure '%s' (0x%x), expected %d, passed %d",
      nclosure->_name, nclosure, nparamscheck, nargs));
  }
...
bool SQVM::GetIUnknownMetaMethod(iUnknown* o, SQMetaMethod mm,
                                 SQObjectPtr& closure)
{
  if (((cScriptVM*)_foreignptr)
        ->GetAutomation()
        ->Get(this, o, _sq_metamethods[mm], closure, _OPEXT_GET_SAFE))
  {
    return true;
  }
  return false;
...
bool SQVM::CallIUnknownMetaMethod(iUnknown* o, SQMetaMethod mm, int nparams,
                                  SQObjectPtr& outres)
{
  SQObjectPtr closure;
  if (((cScriptVM*)_foreignptr)
...
bool SQVM::CallIUnknownMetaMethod(iUnknown* o, SQObjectPtr& closure,
                                  int nparams, SQObjectPtr& outres)
{
#ifdef LAST_CALLSTACK
  _strLastCallStack.clear();
  _raiseErrorMode = eScriptRaiseErrorMode_Locals;
...
bool SQVM::DoGet(const SQObjectPtr& self, const SQObjectPtr& key,
                 SQObjectPtr& dest, const SQObjectPtr* root, int opExt)
{
  ...
  case OT_TABLE: {
    ...
    if (_table(self)->GetDelegate()) {
      return DoGet(SQObjectPtr(_table(self)->GetDelegate()), key, dest,
                   root ? root : &self, opExt);
    }
  ...
  case OT_IUNKNOWN:
    return ((cScriptVM*)_foreignptr)
      ->GetAutomation()
      ->Get(this, _iunknown(self), key, dest, opExt);
  case OT_ARRAY:
    if (sq_isnumeric(key)) {
  ...
  case OT_STRING:
    if (sq_isnumeric(key)) {
      ...
      return _ddel(*_ss, string)->Get(key, dest);
    break;
  case OT_USERDATA: {
    bool getRetVal = false;
    if (_userdata(self)->GetDelegate()) {
      getRetVal = DoGet(SQObjectPtr(_userdata(self)->GetDelegate()), key, dest,
                        root, opExt | _OPEXT_GET_RAW);
      if (!getRetVal) {
        if (opExt & _OPEXT_GET_RAW)
          return false;
        Push(root ? *root : self);
        Push(key);
        if (!CallMetaMethod(_userdata(self)->GetDelegate(), MT_USERDATA_GET, 2,
                            dest))
        {
          return false;
        }
  ...
  case OT_INTEGER:
  case OT_FLOAT: return _ddel(*_ss, number)->Get(key, dest);
  case OT_CLOSURE:
  case OT_NATIVECLOSURE: return _ddel(*_ss, closure)->Get(key, dest);
  default:
    if (!(opExt & _OPEXT_GET_SAFE)) {
...
bool SQVM::DoSet(const SQObjectPtr& self, const SQObjectPtr& key,
                 const SQObjectPtr& val, int opExt)
{
  switch (_sqtype(self)) {
  case OT_TABLE: {
  ...
  case OT_IUNKNOWN:
    return ((cScriptVM*)_foreignptr)
      ->GetAutomation()
      ->Set(this, _iunknown(self), key, val, opExt);
  case OT_ARRAY:
    if (!sq_isnumeric(key)) {
      VM_ERRORB(niFmt(_A("indexing '%s' with '%s'"), _ss->GetTypeNameStr(self),
                      _ss->GetTypeNameStr(key)));
    }
  ...
  case OT_USERDATA:
    if (_userdata(self)->GetDelegate()) {
      ...
      Push(key);
      Push(val);
      return CallMetaMethod(_userdata(self)->GetDelegate(), MT_USERDATA_SET, 3,
                            t);
    }
    break;
  }
...
bool SQVM::StartCall(SQClosure* closure, int target, int nargs, int stackbase,
                     bool tailcall)
{
  ...
  if ((paramssize - 1) == 1 &&
      _stringobj(func->_parameters[1]._name) == _HC(_args_))
  {
    // niDebugFmt(("...VARARGS..."));
  ...
  */
  ...
  if (((unsigned int)newtop + (func->_stacksize << 1) + outerssize) >
      _stack.size())
  {
...
void SQVM::dumpstack(int stackbase, bool dumpall)
{
  ...
  for (int i = 0; i < size; i++) {
    ...
    case OT_TABLE:
      o << niFmt(_A("TABLE %p[%p]"), _table(obj), _table(obj)->GetDelegate());
      break;
    ...
    case OT_USERDATA:
      o << niFmt(_A("USERDATA %p[%p]"), _userdata(obj),
                 _userdata(obj)->GetDelegate());
      break;
    case OT_IUNKNOWN:
      o << niFmt(_A("IUNKNOWN %p [%d refs]"), (tIntPtr)_iunknown(obj),
                 _iunknown(obj) ? _iunknown(obj)->GetNumRefs() : 0);
      break;
    default: niAssertUnreachable("Unknown object type."); break;
    };
    o << _A("\n");
    ++n;
  }
