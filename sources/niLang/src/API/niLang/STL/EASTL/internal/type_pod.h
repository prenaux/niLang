...
///////////////////////////////////////////////////////////////////////
  ...
  (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) ||  \
   (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_empty)))
  #define EASTL_TYPE_TRAIT_is_empty_CONFORMANCE 1 // is_empty is conforming.

template <typename T>
struct is_empty : public integral_constant<bool, __is_empty(T)> {};
#else
  #define EASTL_TYPE_TRAIT_is_empty_CONFORMANCE \
    1 // is_empty is fully conforming.
...
struct is_empty_helper<T, true>
    : public eastl::integral_constant<bool, sizeof(is_empty_helper_t1<T>) ==
                                              sizeof(is_empty_helper_t2)> {};
...
///////////////////////////////////////////////////////////////////////
  ...
  #define EASTL_TYPE_TRAIT_is_pod_CONFORMANCE \
    1 // is_pod is conforming. Actually as of VS2008 it is apparently not fully conforming, as it flags the following as a non-pod: struct Pod{ Pod(){} };

EA_DISABLE_VC_WARNING(4647)
template <
  typename T> // We check for has_trivial_constructor only because the VC++ is_pod does. Is it due to some compiler bug?
struct is_pod
    : public eastl::integral_constant<
        bool, (__has_trivial_constructor(T) && __is_pod(T) &&
               !eastl::is_hat_type<T>::value) ||
                eastl::is_void<T>::value || eastl::is_scalar<T>::value> {};
EA_RESTORE_VC_WARNING()

#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  (defined(EA_COMPILER_GNUC) ||                         \
   (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_pod)))
  #define EASTL_TYPE_TRAIT_is_pod_CONFORMANCE 1 // is_pod is conforming.

template <typename T>
struct is_pod
    : public eastl::integral_constant<bool, __is_pod(T) ||
                                              eastl::is_void<T>::value ||
                                              eastl::is_scalar<T>::value> {};
#else
  #define EASTL_TYPE_TRAIT_is_pod_CONFORMANCE \
    0 // is_pod is not conforming. Can return false negatives.

template <
  typename T> // There's not much we can do here without some compiler extension.
struct is_pod
    : public eastl::integral_constant<
        bool, eastl::is_void<T>::value ||
                eastl::is_scalar<
                  typename eastl::remove_all_extents<T>::type>::value> {};
#endif
...
#define EASTL_DECLARE_IS_POD(T, isPod)                                         \
  namespace eastl {                                                            \
  template <>                                                                  \
  struct is_pod<T> : public eastl::integral_constant<bool, isPod> {};          \
  template <>                                                                  \
  struct is_pod<const T> : public eastl::integral_constant<bool, isPod> {};    \
  template <>                                                                  \
  struct is_pod<volatile T> : public eastl::integral_constant<bool, isPod> {}; \
  template <>                                                                  \
  struct is_pod<const volatile T>                                              \
      : public eastl::integral_constant<bool, isPod> {};                       \
  }

// Old style macro, for bacwards compatibility:
#define EASTL_DECLARE_POD(T)                    \
  namespace eastl {                             \
  template <>                                   \
...
//
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE &&              \
  ((defined(EA_COMPILER_MSVC) && (_MSC_VER >= 1700)) ||            \
   (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4006)) || \
   (defined(EA_COMPILER_CLANG) &&                                  \
    EA_COMPILER_HAS_FEATURE(is_standard_layout)))
  #define EASTL_TYPE_TRAIT_is_standard_layout_CONFORMANCE \
    1 // is_standard_layout is conforming.

template <typename T>
struct is_standard_layout
    : public eastl::integral_constant<bool, __is_standard_layout(T) ||
                                              eastl::is_void<T>::value ||
                                              eastl::is_scalar<T>::value> {};
#else
  #define EASTL_TYPE_TRAIT_is_standard_layout_CONFORMANCE \
    0 // is_standard_layout is not conforming. Can return false negatives.

template <
  typename T> // There's not much we can do here without some compiler extension.
struct is_standard_layout
    : public eastl::integral_constant<bool, is_void<T>::value ||
                                              is_scalar<T>::value> {};
#endif
...
#define EASTL_DECLARE_IS_STANDARD_LAYOUT(T, isStandardLayout)       \
  ...
  struct is_standard_layout<T>                                      \
      : public eastl::integral_constant<bool, isStandardLayout> {}; \
  template <>                                                       \
  struct is_standard_layout<const T>                                \
      : public eastl::integral_constant<bool, isStandardLayout> {}; \
  template <>                                                       \
  struct is_standard_layout<volatile T>                             \
      : public eastl::integral_constant<bool, isStandardLayout> {}; \
  template <>                                                       \
  struct is_standard_layout<const volatile T>                       \
      : public eastl::integral_constant<bool, isStandardLayout> {}; \
  }

// Old style macro, for bacwards compatibility:
#define EASTL_DECLARE_STANDARD_LAYOUT(T)                    \
  namespace eastl {                                         \
  template <>                                               \
  struct is_standard_layout<T> : public true_type {};       \
  template <>                                               \
  struct is_standard_layout<const T> : public true_type {}; \
  }
...
#define EASTL_DECLARE_HAS_TRIVIAL_CONSTRUCTOR(T, hasTrivialConstructor)  \
  ...
  struct has_trivial_constructor<T>                                      \
      : public eastl::integral_constant<bool, hasTrivialConstructor> {}; \
  }

// Old style macro, for bacwards compatibility:
#define EASTL_DECLARE_TRIVIAL_CONSTRUCTOR(T)                     \
  namespace eastl {                                              \
  template <>                                                    \
...
struct has_trivial_copy
    : public eastl::integral_constant<bool, (__has_trivial_copy(T) ||
                                             eastl::is_pod<T>::value) &&
                                              !eastl::is_volatile<T>::value &&
                                              !eastl::is_hat_type<T>::value> {};
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
...
struct has_trivial_copy
    : public eastl::integral_constant<
        bool,
        (__has_trivial_copy(T) || eastl::is_pod<T>::value) &&
          (!eastl::is_volatile<T>::value && !eastl::is_reference<T>::value)> {};
EA_RESTORE_CLANG_WARNING()
#else
  #define EASTL_TYPE_TRAIT_has_trivial_copy_CONFORMANCE \
    0 // has_trivial_copy is not fully conforming. Can return false negatives.

template <typename T>
struct has_trivial_copy
    : public eastl::integral_constant<bool, eastl::is_pod<T>::value &&
                                              !eastl::is_volatile<T>::value> {};
#endif
...
#define EASTL_DECLARE_HAS_TRIVIAL_COPY(T, hasTrivialCopy)         \
  ...
  struct has_trivial_copy<T>                                      \
      : public eastl::integral_constant<bool, hasTrivialCopy> {}; \
  }

// Old style macro, for bacwards compatibility:
#define EASTL_DECLARE_TRIVIAL_COPY(T)                     \
  namespace eastl {                                       \
  template <>                                             \
...
struct has_trivial_assign
    : public integral_constant<
        bool, (__has_trivial_assign(T) || eastl::is_pod<T>::value) &&
                !eastl::is_const<T>::value && !eastl::is_volatile<T>::value &&
                !eastl::is_hat_type<T>::value> {};
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) ||    \
   defined(EA_COMPILER_CLANG))
...
template <typename T>
struct has_trivial_assign
    : public integral_constant<bool, is_pod<T>::value && !is_const<T>::value &&
                                       !is_volatile<T>::value> {};
#endif
...
#define EASTL_DECLARE_HAS_TRIVIAL_ASSIGN(T, hasTrivialAssign)       \
  ...
  struct has_trivial_assign<T>                                      \
      : public eastl::integral_constant<bool, hasTrivialAssign> {}; \
  }

// Old style macro, for bacwards compatibility:
#define EASTL_DECLARE_TRIVIAL_ASSIGN(T)                     \
  namespace eastl {                                         \
  template <>                                               \
...
#define EASTL_DECLARE_HAS_TRIVIAL_DESTRUCTOR(T, hasTrivialDestructor)   \
  ...
  struct has_trivial_destructor<T>                                      \
      : public eastl::integral_constant<bool, hasTrivialDestructor> {}; \
  }

// Old style macro, for bacwards compatibility:
#define EASTL_DECLARE_TRIVIAL_DESTRUCTOR(T)                     \
  namespace eastl {                                             \
  template <>                                                   \
...
#define EASTL_TYPE_TRAIT_has_trivial_relocate_CONFORMANCE \
  0 // is_pod is not fully conforming. Can return false negatives.

template <typename T>
struct has_trivial_relocate
    : public eastl::bool_constant<eastl::is_pod_v<T> &&
                                  !eastl::is_volatile_v<T>> {};

#define EASTL_DECLARE_TRIVIAL_RELOCATE(T)                     \
  namespace eastl {                                           \
  template <>                                                 \
...
template <
  typename T> // This is mistakenly returning true for an unbounded array of scalar type.
struct has_nothrow_constructor
    : public eastl::integral_constant<
        bool, __has_nothrow_constructor(T) ||
                eastl::is_scalar<
                  typename eastl::remove_all_extents<T>::type>::value ||
                eastl::is_reference<T>::value> {};

#else
  #define EASTL_TYPE_TRAIT_has_nothrow_constructor_CONFORMANCE \
    0 // has_nothrow_constructor is not fully conforming. Can return false negatives.

template <typename T>
struct
  has_nothrow_constructor // To do: Improve this to include other types that can work.
{
  static const bool value =
    eastl::is_scalar<typename eastl::remove_all_extents<T>::type>::value ||
    eastl::is_reference<T>::value;
};
#endif
...
#define EASTL_DECLARE_HAS_NOTHROW_CONSTRUCTOR(T, hasNothrowConstructor)  \
  ...
  struct has_nothrow_constructor<T>                                      \
      : public eastl::integral_constant<bool, hasNothrowConstructor> {}; \
  }
...
struct has_nothrow_copy
    : public eastl::integral_constant<
        bool, __has_nothrow_copy(T) ||
                eastl::is_scalar<
                  typename eastl::remove_all_extents<T>::type>::value ||
                eastl::is_reference<T>::value> {};

#else
  #define EASTL_TYPE_TRAIT_has_nothrow_copy_CONFORMANCE \
    0 // has_nothrow_copy is not fully conforming. Can return false negatives.

template <typename T>
struct
  has_nothrow_copy // To do: Improve this to include other types that can work.
{
  static const bool value =
    eastl::is_scalar<typename eastl::remove_all_extents<T>::type>::value ||
    eastl::is_reference<T>::value;
...
#define EASTL_DECLARE_HAS_NOTHROW_COPY(T, hasNothrowCopy)         \
  ...
  struct has_nothrow_copy<T>                                      \
      : public eastl::integral_constant<bool, hasNothrowCopy> {}; \
  }
...
struct has_nothrow_assign
    : public eastl::integral_constant<
        bool, __has_nothrow_assign(T) ||
                eastl::is_scalar<
                  typename eastl::remove_all_extents<T>::type>::value ||
                eastl::is_reference<T>::value> {};
#else
  #define EASTL_TYPE_TRAIT_has_nothrow_assign_CONFORMANCE \
    0 // has_nothrow_assign is not fully conforming. Can return false negatives.

template <typename T>
struct
  has_nothrow_assign // To do: Improve this to include other types that can work.
{
  static const bool value =
    eastl::is_scalar<typename eastl::remove_all_extents<T>::type>::value ||
    eastl::is_reference<T>::value;
...
#define EASTL_DECLARE_HAS_NOTHROW_ASSIGN(T, hasNothrowAssign)       \
  ...
  struct has_nothrow_assign<T>                                      \
      : public eastl::integral_constant<bool, hasNothrowAssign> {}; \
  }
...
struct has_virtual_destructor
    : public eastl::integral_constant<bool, __has_virtual_destructor(T)> {};
#else
  #define EASTL_TYPE_TRAIT_has_virtual_destructor_CONFORMANCE \
...
#define EASTL_DECLARE_HAS_VIRTUAL_DESTRUCTOR(T, hasVirtualDestructor)   \
  ...
  struct has_virtual_destructor<T>                                      \
      : public eastl::integral_constant<bool, hasVirtualDestructor> {}; \
  template <>                                                           \
  struct has_virtual_destructor<const T>                                \
      : public eastl::integral_constant<bool, hasVirtualDestructor> {}; \
  template <>                                                           \
  struct has_virtual_destructor<volatile T>                             \
      : public eastl::integral_constant<bool, hasVirtualDestructor> {}; \
  template <>                                                           \
  struct has_virtual_destructor<const volatile T>                       \
      : public eastl::integral_constant<bool, hasVirtualDestructor> {}; \
  }
...
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_literal))
  #define EASTL_TYPE_TRAIT_is_literal_type_CONFORMANCE 1

template <typename T>
struct is_literal_type
    : public eastl::integral_constant<bool, __is_literal(T)> {};

#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE &&            \
  ((defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4006)) || \
   (defined(_MSC_VER) && (_MSC_VER >= 1700))) // VS2012+
  #if defined(EA_COMPILER_GNUC) && \
    (!defined(EA_COMPILER_CPP11_ENABLED) || (EA_COMPILER_VERSION < 4007))
    #define EASTL_TYPE_TRAIT_is_literal_type_CONFORMANCE \
      0 // It seems that in this case GCC supports the compiler intrinsic but reports it as false when it's true.
  #else
    #define EASTL_TYPE_TRAIT_is_literal_type_CONFORMANCE 1
  #endif
...
template <typename T>
struct is_literal_type
    : public eastl::integral_constant<bool, __is_literal_type(T)> {};
...
template <
  typename T> // This is not a complete implementation and will be true for only some literal types (the basic ones).
struct is_literal_type
    : public eastl::integral_constant<
        bool, eastl::is_scalar<typename eastl::remove_reference<
                typename eastl::remove_all_extents<T>::type>::type>::value> {};
#endif
...
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  (defined(_MSC_VER) || defined(EA_COMPILER_GNUC) ||  \
   (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_abstract)))
  #define EASTL_TYPE_TRAIT_is_abstract_CONFORMANCE \
    1 // is_abstract is conforming.
...
class is_abstract_helper {
  ...
  static eastl::no_type test(
    T1 (*)
      [1]); // The following: 'typedef SomeAbstractClass (*SomeFunctionType)[1];' is invalid (can't have an array of abstract types) and thus doesn't choose this path.
...
#define EASTL_DECLARE_IS_ABSTRACT(T, isAbstract)                              \
  namespace eastl {                                                           \
  template <>                                                                 \
  struct is_abstract<T> : public eastl::integral_constant<bool, isAbstract> { \
  };                                                                          \
  template <>                                                                 \
  struct is_abstract<const T>                                                 \
      : public eastl::integral_constant<bool, isAbstract> {};                 \
  template <>                                                                 \
  struct is_abstract<volatile T>                                              \
      : public eastl::integral_constant<bool, isAbstract> {};                 \
  template <>                                                                 \
  struct is_abstract<const volatile T>                                        \
      : public eastl::integral_constant<bool, isAbstract> {};                 \
  }
...
///////////////////////////////////////////////////////////////////////
  ...
  ((defined(_MSC_VER) && (_MSC_VER >= 1700)) ||                    \
   (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 5003)) || \
   (defined(EA_COMPILER_CLANG) &&                                  \
    EA_COMPILER_HAS_FEATURE(is_trivially_copyable)))
...
#elif EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  (defined(EA_COMPILER_MSVC) || defined(EA_COMPILER_GNUC))
  #define EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE 1
...
struct is_trivially_copyable
    : public integral_constant<
        ...
        (__has_trivial_copy(T) ||
         eastl::is_pod<typename eastl::remove_all_extents<T>::type>::value) &&
          (!eastl::is_void<T>::value && !eastl::is_volatile<T>::value &&
           !eastl::is_reference<T>::value)> {};
#else
  #define EASTL_TYPE_TRAIT_is_trivially_copyable_CONFORMANCE \
...
#define EASTL_DECLARE_IS_TRIVIALLY_COPYABLE(T, isTriviallyCopyable)    \
  ...
  struct is_trivially_copyable<T>                                      \
      : public eastl::integral_constant<bool, isTriviallyCopyable> {}; \
  template <>                                                          \
  struct is_trivially_copyable<const T>                                \
      : public eastl::integral_constant<bool, isTriviallyCopyable> {}; \
  template <>                                                          \
  struct is_trivially_copyable<volatile T>                             \
      : public eastl::integral_constant<bool, isTriviallyCopyable> {}; \
  template <>                                                          \
  struct is_trivially_copyable<const volatile T>                       \
      : public eastl::integral_constant<bool, isTriviallyCopyable> {}; \
  }
...
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  (defined(_MSC_VER) ||                               \
   (defined(EA_COMPILER_CLANG) && EA_COMPILER_HAS_FEATURE(is_constructible)))
...
template <typename T>
inline typename eastl::remove_reference<T>::type&& move_internal(T&& x)
  EA_NOEXCEPT
{
...
typename first_type_select<
  eastl::true_type,
  decltype(eastl::move_internal(T(eastl::declval<Args>()...)))>::type
is(T&&, Args&&...);
...
struct
  is_constructible_helper_2 // argument_sink will catch all T that is not constructible from the Args and denote false_type
    : public eastl::identity<decltype(is(eastl::declval<T>(),
                                         eastl::declval<Args>()...))>::type {};

template <typename T>
struct is_constructible_helper_2<true, T> : public eastl::is_scalar<T> {};

template <
  typename T,
  typename Arg0> // We handle the case of multiple arguments below (by disallowing them).
struct is_constructible_helper_2<true, T, Arg0>
    : public eastl::identity<decltype(can_construct_scalar_helper<T>::can(
        eastl::declval<Arg0>()))>::type {};
...
struct is_constructible_helper_1
    : public is_constructible_helper_2<eastl::is_scalar<T>::value ||
                                         eastl::is_reference<T>::value,
                                       T, Args...> {};

// Unilaterally dismiss void, abstract, unknown bound arrays, and function types as not constructible.
template <typename T, typename... Args>
struct is_constructible_helper_1<true, T, Args...> : public false_type {};

// is_constructible
template <typename T, typename... Args>
struct is_constructible
    : public is_constructible_helper_1<
        (eastl::is_abstract<
           typename eastl::remove_all_extents<T>::type>::value ||
...
struct is_constructible_helper_2<false, Array[N]>
    : public eastl::is_constructible<
        typename eastl::remove_all_extents<Array>::type> {};
...
#define EASTL_DECLARE_IS_CONSTRUCTIBLE(T, U, isConstructible)      \
  ...
  struct is_constructible<T, U>                                    \
      : public eastl::integral_constant<bool, isConstructible> {}; \
  }
...
///////////////////////////////////////////////////////////////////////
  ...
  #if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
    (defined(EA_COMPILER_CLANG) &&                      \
     EA_COMPILER_HAS_FEATURE(is_trivially_constructible))

template <typename T, typename Arg0 = eastl::unused>
struct is_trivially_constructible : public eastl::false_type {};
...
struct is_trivially_constructible<T, eastl::unused>
    : public eastl::integral_constant<
        bool, eastl::is_constructible<T>::value &&
                eastl::has_trivial_constructor<
                  typename eastl::remove_all_extents<T>::type>::value> {};

template <typename T>
struct is_trivially_constructible<T, T>
    : public eastl::integral_constant<bool,
                                      eastl::is_constructible<T>::value &&
                                        eastl::has_trivial_copy<T>::value> {};
...
struct is_trivially_constructible
    : public eastl::integral_constant<
        bool, eastl::is_constructible<
                T, Args...>::value&& __is_trivially_constructible(T, Args...)> {
};

  #else

    #define EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE \
      0 // This is 0 but in fact it will work for most real-world cases due to the has_trivial_constructor specialization below.
...
#define EASTL_DECLARE_IS_TRIVIALLY_CONSTRUCTIBLE(T, isTriviallyConstructible) \
  ...
  template <>                                                                 \
  struct is_trivially_constructible<T>                                        \
      : public eastl::integral_constant<bool, isTriviallyConstructible> {};   \
  template <>                                                                 \
  struct is_trivially_constructible<const T>                                  \
      : public eastl::integral_constant<bool, isTriviallyConstructible> {};   \
  template <>                                                                 \
  struct is_trivially_constructible<volatile T>                               \
      : public eastl::integral_constant<bool, isTriviallyConstructible> {};   \
  template <>                                                                 \
  struct is_trivially_constructible<const volatile T>                         \
      : public eastl::integral_constant<bool, isTriviallyConstructible> {};   \
  }

#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_trivially_constructible_v =
...
#define EASTL_TYPE_TRAIT_is_trivially_default_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_trivially_default_constructible_v =
...
#define EASTL_TYPE_TRAIT_is_trivial_CONFORMANCE                        \
  ((EASTL_TYPE_TRAIT_is_trivially_default_constructible_CONFORMANCE && \
...
struct is_trivial_helper
    : public eastl::integral_constant<
        bool, eastl::is_trivially_copyable<T>::value &&
                eastl::is_trivially_default_constructible<T>::value> {};

template <typename T>
struct is_trivial_helper<true, T> : public false_type {};

template <typename T>
struct is_trivial
    : public is_trivial_helper<
        (EA_ALIGN_OF(T) > EA_PLATFORM_MIN_MALLOC_ALIGNMENT), T>::type {};
#else
...
template <typename T, typename... Args>
struct is_nothrow_constructible : public eastl::false_type {};

template <typename T>
struct is_nothrow_constructible<T>
    : public eastl::integral_constant<
        bool, eastl::has_nothrow_constructor<T>::value> {};
...
struct is_nothrow_constructible<T, T&&>
    ...
    #define EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE \
      0 // GCC up to v4.7's noexcept is broken and fails to generate true for the case of compiler-generated constructors.
  #else
    #define EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE \
...
struct is_nothrow_constructible_helper<true, T, Args...>
    : public eastl::integral_constant<
        bool,
        is_nothrow_constructible_helper_noexcept_wrapper<T, Args...>::value> {};

template <typename T, typename Arg>
struct is_nothrow_constructible_helper<true, T, Arg>
    : public eastl::integral_constant<bool,
                                      noexcept(T(eastl::declval<Arg>()))> {};
...
struct is_nothrow_constructible
    : public eastl::is_nothrow_constructible_helper<
        eastl::is_constructible<T, Args...>::value, T, Args...> {};

template <typename T, size_t N>
struct is_nothrow_constructible<T[N]>
    : public eastl::is_nothrow_constructible_helper<
        eastl::is_constructible<T>::value, T> {};
#endif
...
#define EASTL_DECLARE_IS_NOTHROW_CONSTRUCTIBLE(T, isNothrowConstructible) \
  ...
  template <>                                                             \
  struct is_nothrow_constructible<T>                                      \
      : public eastl::integral_constant<bool, isNothrowConstructible> {}; \
  }

#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T, typename... Args>
EA_CONSTEXPR bool is_nothrow_constructible_v =
...
#define EASTL_TYPE_TRAIT_is_default_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE

template <typename T>
struct is_default_constructible : public eastl::is_constructible<T> {};

#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_default_constructible_v =
...
#define EASTL_TYPE_TRAIT_is_copy_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE

template <typename T>
struct is_copy_constructible
    : public eastl::is_constructible<
        T, typename eastl::add_lvalue_reference<
             typename eastl::add_const<T>::type>::type> {};
...
#define EASTL_TYPE_TRAIT_is_trivially_copy_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_trivially_copy_constructible_v =
...
#define EASTL_TYPE_TRAIT_is_nothrow_copy_constructible_CONFORMANCE \
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_nothrow_copy_constructible_v =
...
#define EASTL_TYPE_TRAIT_is_move_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_constructible_CONFORMANCE

template <typename T>
struct is_move_constructible
    : public eastl::is_constructible<
        T, typename eastl::add_rvalue_reference<T>::type> {};
...
#define EASTL_TYPE_TRAIT_is_trivially_move_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_trivially_constructible_CONFORMANCE
...
#define EASTL_DECLARE_IS_TRIVIALLY_MOVE_CONSTRUCTIBLE(                        \
  T, isTrivallyMoveConstructible)                                             \
  namespace eastl {                                                           \
  template <>                                                                 \
  struct is_trivially_move_constructible<T>                                   \
      : public eastl::integral_constant<bool, isTriviallyMoveConstructible> { \
  };                                                                          \
  }

#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_trivially_move_constructible_v =
...
struct is_assignable_helper {
  ...
  static decltype(eastl::declval<T1>() = eastl::declval<U1>(),
                  eastl::yes_type())
  is(int);
...
template <typename T, typename U>
struct is_assignable : public eastl::integral_constant<
                         bool, eastl::is_assignable_helper<T, U>::value> {};
...
#define EASTL_DECLARE_IS_ASSIGNABLE(T, U, isAssignable)         \
  ...
  struct is_assignable<T, U>                                    \
      : public eastl::integral_constant<bool, isAssignable> {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_lvalue_assignable_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE

template <typename T, typename U>
struct is_lvalue_assignable
    : public eastl::is_assignable<typename eastl::add_lvalue_reference<T>::type,
                                  typename eastl::add_lvalue_reference<
                                    typename eastl::add_const<U>::type>::type> {
};

#define EASTL_DECLARE_IS_LVALUE_ASSIGNABLE(T, U, isLvalueAssignable)  \
  namespace eastl {                                                   \
  template <>                                                         \
  struct is_lvalue_assignable<T, U>                                   \
      : public eastl::integral_constant<bool, isLvalueAssignable> {}; \
  }
...
#if EASTL_COMPILER_INTRINSIC_TYPE_TRAITS_AVAILABLE && \
  (defined(EA_COMPILER_CLANG) &&                      \
   EA_COMPILER_HAS_FEATURE(is_trivially_assignable))
...
struct is_trivially_assignable
  ...
  #define EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE \
    EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
...
template <typename T, typename U>
struct is_trivially_assignable_helper<true, T, U>
    : eastl::integral_constant<bool, __is_trivially_assignable(T, U)> {};
...
struct is_trivially_assignable
    : eastl::integral_constant<
        bool, is_trivially_assignable_helper<eastl::is_assignable<T, U>::value,
                                             T, U>::value> {};
...
struct is_trivially_assignable
    : eastl::integral_constant<
        ...
        eastl::is_assignable<T, U>::value &&
          (eastl::is_pod<typename eastl::remove_reference<T>::type>::value ||
           __has_trivial_assign(typename eastl::remove_reference<T>::type))> {};
#else
...
template <typename T, typename U>
struct is_trivially_assignable : public eastl::false_type {};
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T, class U>
EA_CONSTEXPR bool is_trivially_assignable_v =
...
#define EASTL_DECLARE_IS_TRIVIALLY_ASSIGNABLE(T, U, isTriviallyAssignable) \
  ...
  struct is_trivially_assignable<T, U>                                     \
      : public eastl::integral_constant<bool, isTriviallyAssignable> {};   \
  }
...
struct is_nothrow_assignable
  ...
  defined(                                \
    __EDG_VERSION__) // EDG mis-compiles the conforming code below and so must be placed here.
  #define EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE 0

template <typename T, typename U>
struct is_nothrow_assignable : public false_type {};
...
struct is_nothrow_assignable<T&, T>
    : public eastl::integral_constant<bool,
                                      eastl::has_nothrow_assign<T>::value> {};
...
struct is_nothrow_assignable_helper<
  true, T,
  U> // Set to true if the assignment (same as is_assignable) cannot generate an exception.
    : public eastl::integral_constant<bool, noexcept(eastl::declval<T>() =
                                                       eastl::declval<U>())> {};

template <typename T, typename U>
struct is_nothrow_assignable : public eastl::is_nothrow_assignable_helper<
                                 eastl::is_assignable<T, U>::value, T, U> {};
#endif

#define EASTL_DECLARE_IS_NOTHROW_ASSIGNABLE(T, isNothrowAssignable)    \
  namespace eastl {                                                    \
  template <>                                                          \
  struct is_nothrow_assignable<T>                                      \
      : public eastl::integral_constant<bool, isNothrowAssignable> {}; \
  template <>                                                          \
  struct is_nothrow_assignable<const T>                                \
      : public eastl::integral_constant<bool, isNothrowAssignable> {}; \
  template <>                                                          \
  struct is_nothrow_assignable<volatile T>                             \
      : public eastl::integral_constant<bool, isNothrowAssignable> {}; \
  template <>                                                          \
  struct is_nothrow_assignable<const volatile T>                       \
      : public eastl::integral_constant<bool, isNothrowAssignable> {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_copy_assignable_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
...
#define EASTL_TYPE_TRAIT_is_trivially_copy_assignable_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE

#if EASTL_TYPE_TRAIT_is_trivially_copy_assignable_CONFORMANCE
template <typename T>
struct is_trivially_copy_assignable
    : public eastl::is_trivially_assignable<
        typename eastl::add_lvalue_reference<T>::type,
        typename eastl::add_lvalue_reference<
          typename eastl::add_const<T>::type>::type> {};
#else
template <typename T>
struct is_trivially_copy_assignable
    : public integral_constant<
        bool, eastl::is_scalar<T>::value || eastl::is_pod<T>::value ||
                eastl::is_trivially_assignable<
                  typename eastl::add_lvalue_reference<T>::type,
                  typename eastl::add_lvalue_reference<
                    typename eastl::add_const<T>::type>::type>::value> {};
#endif

#define EASTL_DECLARE_IS_TRIVIALLY_COPY_ASSIGNABLE(T,                         \
                                                   isTriviallyCopyAssignable) \
  namespace eastl {                                                           \
  template <>                                                                 \
  struct is_trivially_copy_assignable<T>                                      \
      : public eastl::integral_constant<bool, isTriviallyCopyAssignable> {};  \
  }

#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_trivially_copy_assignable_v =
...
#define EASTL_TYPE_TRAIT_is_nothrow_copy_assignable_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE

template <typename T>
struct is_nothrow_copy_assignable
    : public eastl::is_nothrow_assignable<
        typename eastl::add_lvalue_reference<T>::type,
        typename eastl::add_lvalue_reference<
          typename eastl::add_const<T>::type>::type> {};
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_nothrow_copy_assignable_v =
...
#define EASTL_TYPE_TRAIT_is_move_assignable_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE

template <typename T>
struct is_move_assignable : public eastl::is_assignable<
                              typename eastl::add_lvalue_reference<T>::type,
...
#define EASTL_DECLARE_IS_MOVE_ASSIGNABLE(T, isMoveAssignable)       \
  ...
  struct is_move_assignable<T>                                      \
      : public eastl::integral_constant<bool, isMoveAssignable> {}; \
  template <>                                                       \
  struct is_move_assignable<const T>                                \
      : public eastl::integral_constant<bool, isMoveAssignable> {}; \
  template <>                                                       \
  struct is_move_assignable<volatile T>                             \
      : public eastl::integral_constant<bool, isMoveAssignable> {}; \
  template <>                                                       \
  struct is_move_assignable<const volatile T>                       \
      : public eastl::integral_constant<bool, isMoveAssignable> {}; \
  }
...
#define EASTL_TYPE_TRAIT_is_trivially_move_assignable_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_trivially_assignable_CONFORMANCE
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_trivially_move_assignable_v =
...
#define EASTL_TYPE_TRAIT_is_nothrow_move_assignable_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_nothrow_assignable_CONFORMANCE
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_nothrow_move_assignable_v =
...
///////////////////////////////////////////////////////////////////////
  ...
  defined(EA_COMPILER_NO_FUNCTION_TEMPLATE_DEFAULT_ARGS) || \
  defined(_MSC_VER) ||                                      \
  defined(                                                  \
    __EDG_VERSION__) // VS2012 and EDG mis-compile the conforming code below and so must be placed here.
...
struct is_destructible
    : public eastl::integral_constant<
        bool, !eastl::is_array_of_unknown_bounds<T>::value &&
                !eastl::is_void<T>::value && !eastl::is_function<T>::value &&
...
template <
  typename T,
  typename U = decltype(eastl::declval<eastl::destructible_test_helper<T>>()
                          .~destructible_test_helper<T>())>
eastl::true_type destructible_test_function(int);

template <
  typename T,
  bool = eastl::is_array_of_unknown_bounds<T>::
           value || // Exclude these types from being considered destructible.
         eastl::is_void<T>::value ||
         eastl::is_function<T>::value || eastl::is_abstract<T>::value>
struct is_destructible_helper
    : public eastl::identity<decltype(eastl::destructible_test_function<T>(
        0))>::type {
}; // Need to wrap decltype with identity because some compilers otherwise don't like the bare decltype usage.

template <typename T>
struct is_destructible_helper<T, true> : public eastl::false_type {};
...
#define EASTL_DECLARE_IS_DESTRUCTIBLE(T, isDestructible)          \
  ...
  struct is_destructible<T>                                       \
      : public eastl::integral_constant<bool, isDestructible> {}; \
  template <>                                                     \
  struct is_destructible<const T>                                 \
      : public eastl::integral_constant<bool, isDestructible> {}; \
  template <>                                                     \
  struct is_destructible<volatile T>                              \
      : public eastl::integral_constant<bool, isDestructible> {}; \
  template <>                                                     \
  struct is_destructible<const volatile T>                        \
      : public eastl::integral_constant<bool, isDestructible> {}; \
  }
...
struct
  is_trivially_destructible // Can't use just __has_trivial_destructor(T) because some compilers give it slightly different meaning, and are just plain broken, such as VC++'s __has_trivial_destructor, which says false for fundamental types.
    : public integral_constant<
        bool,
        eastl::is_destructible<T>::value &&
          ((__has_trivial_destructor(T) && !eastl::is_hat_type<T>::value) ||
           eastl::is_scalar<
             typename eastl::remove_all_extents<T>::type>::value)> {};
EA_RESTORE_CLANG_WARNING()
...
struct is_trivially_destructible_helper
    : public integral_constant<bool, (eastl::is_pod<T>::value ||
                                      eastl::is_scalar<T>::value ||
                                      eastl::is_reference<T>::value) &&
                                       !eastl::is_void<T>::value> {};
...
template <typename T>
struct is_trivially_destructible
    : public eastl::is_trivially_destructible_helper<
        typename eastl::remove_all_extents<T>::type> {};
#endif
...
#define EASTL_DECLARE_IS_TRIVIALLY_DESTRUCTIBLE(T, isTriviallyDestructible) \
  ...
  struct is_trivially_destructible<T>                                       \
      : public eastl::integral_constant<bool, isTriviallyDestructible> {};  \
  template <>                                                               \
  struct is_trivially_destructible<const T>                                 \
      : public eastl::integral_constant<bool, isTriviallyDestructible> {};  \
  template <>                                                               \
  struct is_trivially_destructible<volatile T>                              \
      : public eastl::integral_constant<bool, isTriviallyDestructible> {};  \
  template <>                                                               \
  struct is_trivially_destructible<const volatile T>                        \
      : public eastl::integral_constant<bool, isTriviallyDestructible> {};  \
  }

#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_trivially_destructible_v =
...
///////////////////////////////////////////////////////////////////////
  ...
  #define EASTL_TYPE_TRAIT_is_nothrow_destructible_CONFORMANCE \
    ((_MSC_VER >= 1900)                                        \
       ? 1                                                     \
       : 0) // VS2013 (1800) doesn't support noexcept and so can't support all usage of this properly (in particular default exception specifications defined in [C++11 Standard, 15.4 paragraph 14].
...
template <typename T>
struct is_nothrow_destructible
    : public eastl::is_nothrow_destructible_helper<
        typename eastl::remove_all_extents<T>::type> {};

#else
  #if defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION < 4008)
    #define EASTL_TYPE_TRAIT_is_nothrow_destructible_CONFORMANCE \
      0 // GCC up to v4.7's noexcept is broken and fails to generate true for the case of compiler-generated destructors.
  #else
    #define EASTL_TYPE_TRAIT_is_nothrow_destructible_CONFORMANCE \
      EASTL_TYPE_TRAIT_is_destructible_CONFORMANCE
  #endif
...
template <typename T>
struct is_nothrow_destructible_helper<T, false> : public eastl::false_type {};

template <typename T>
struct is_nothrow_destructible_helper<
  T,
  true> // If the expression T::~T is a noexcept expression then it's nothrow.
    : public eastl::integral_constant<
        bool, is_nothrow_destructible_helper_noexcept_wrapper<T>::value> {};

template <typename T>
struct
  is_nothrow_destructible // A type needs to at least be destructible before it could be nothrow destructible.
    : public eastl::is_nothrow_destructible_helper<
        T, eastl::is_destructible<T>::value> {};

template <
  typename T,
  size_t
    N> // An array is nothrow destructible if its element type is nothrow destructible.
struct is_nothrow_destructible<
  T[N]> // To consider: Replace this with a remove_all_extents pathway.
    : public eastl::is_nothrow_destructible<T> {};

template <typename T>
struct is_nothrow_destructible<
  T&> // A reference type cannot throw while being destructed. It's just a reference.
    : public eastl::true_type {};

template <typename T>
struct is_nothrow_destructible<
  T&&> // An rvalue reference type cannot throw while being destructed.
    : public eastl::true_type {};
...
#define EASTL_DECLARE_IS_NOTHROW_DESTRUCTIBLE(T, isNoThrowDestructible) \
  ...
  struct is_nothrow_destructible<T> {                                   \
    static const bool value = isNoThrowDestructible;                    \
  };                                                                    \
  template <>                                                           \
  struct is_nothrow_destructible<const T> {                             \
    static const bool value = isNoThrowDestructible;                    \
  };                                                                    \
  template <>                                                           \
  struct is_nothrow_destructible<volatile T> {                          \
    static const bool value = isNoThrowDestructible;                    \
  };                                                                    \
  template <>                                                           \
  struct is_nothrow_destructible<const volatile T> {                    \
    static const bool value = isNoThrowDestructible;                    \
  };                                                                    \
  }
...
//
///////////////////////////////////////////////////////////////////////
#define EASTL_TYPE_TRAIT_is_nothrow_default_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE
...
#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_nothrow_default_constructible_v =
...
//
///////////////////////////////////////////////////////////////////////
#define EASTL_TYPE_TRAIT_is_nothrow_move_constructible_CONFORMANCE \
  EASTL_TYPE_TRAIT_is_nothrow_constructible_CONFORMANCE

template <typename T>
struct is_nothrow_move_constructible
    : public eastl::is_nothrow_constructible<
        T, typename eastl::add_rvalue_reference<T>::type> {};

#if EASTL_VARIABLE_TEMPLATES_ENABLED
template <class T>
EA_CONSTEXPR bool is_nothrow_move_constructible_v =
