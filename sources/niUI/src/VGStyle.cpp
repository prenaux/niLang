...
class cVGStyle : public ImplRC<iVGStyle> {
 public:
  ...
  tBool __stdcall Copy(const iVGStyle* apStyle)
  {
    ...
    top().mfRasterizerApproximationScale =
      apStyle->GetRasterizerApproximationScale();
    top().mfTesselatorApproximationScale =
      apStyle->GetTesselatorApproximationScale();
  ...
  {
    SET(StrokeColor);
    top().mStrokePaint->SetColor(
      Vec4f(avColor.x, avColor.y, avColor.z, GetStrokeOpacity()));
  }
  ...
  {
    SET(FillColor);
    top().mFillPaint->SetColor(
      Vec4f(avColor.x, avColor.y, avColor.z, GetFillOpacity()));
  }
  ...
  void __stdcall SetRasterizerApproximationScale(
    tF32 afRasterizerApproximationScale)
  {
  ...
  void __stdcall SetTesselatorApproximationScale(
    tF32 afTesselatorApproximationScale)
  {
