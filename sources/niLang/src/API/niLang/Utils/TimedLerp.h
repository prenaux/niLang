...
struct sTimedLerp {
  static inline void GetIndices(const ContainerT& avKeys, TimeT afTime,
                                tU32& aA, tU32& aB, TimeT& afFactor)
  {
    ...
    TimeT fFrameTime =
      GetKeyTimeT::GetKeyTime(avKeys[1]) - GetKeyTimeT::GetKeyTime(avKeys[0]);
    TimeT fCount = afTime;
    tU32 i;
    for (i = 0; i <= ulNumKey; ++i) {
      fCount -= fabsf(GetKeyTimeT::GetKeyTime(avKeys[(i + 1) % ulNumKey]) -
                      GetKeyTimeT::GetKeyTime(avKeys[i % ulNumKey]));
      if (fCount <= 0.0f)
        break;
    }
    ...
    if (aA != aB) {
      fFrameTime = GetKeyTimeT::GetKeyTime(avKeys[aB]) -
                   GetKeyTimeT::GetKeyTime(avKeys[aA]);
