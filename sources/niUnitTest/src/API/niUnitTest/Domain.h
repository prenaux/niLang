...
bool Check(ni::tI32 aStart, ni::tI32 aEnd, GEN& aGenerator,
           PREDICATE& aPredicate, TestResults& results,
           ...
           int const line)
{
  for (ni::tI32 i = aStart; i <= aEnd; ++i) {
    ...
    if (!aPredicate(a)) {
      ni::cString stream;
      stream << "Domain::Check[" << aStart << "," << aEnd << "] at [" << i
             << "] predicate failed with [" << a << "]";
...
template <typename T, typename GEN, typename PREDICATE>
bool Check2(ni::tI32 aStart, ni::tI32 aEnd, GEN& aGenerator,
            PREDICATE& aPredicate, TestResults& results,
            char const* const testName, char const* const filename,
            int const line)
{
  for (ni::tI32 i = aStart; i <= aEnd; ++i) {
    ...
    if (!aPredicate(a, b)) {
      ni::cString stream;
      stream << "Domain::Check2[" << aStart << "," << aEnd << "] at [" << i
             << "] predicate failed with [" << a << "] & [" << b << "]";
      results.OnTestFailure(filename, line, testName, stream.GetText());
      return false;
    }
  }
...
#define CHECK_DOMAIN_(CHECK, START, END, TYPE, GENERATOR, PREDICATE)          \
  niTry () {                                                                  \
    UnitTest::Domain::CHECK<TYPE>(START, END, GENERATOR, PREDICATE,           \
                                  testResults_, m_testName, __FILE__,         \
                                  __LINE__);                                  \
  }                                                                           \
  TEST_CATCH_ASSERT_EXCEPTION("Assert exception in CHECK_DOMAIN(" #GENERATOR  \
                              ", " #PREDICATE ")")                            \
  TEST_CATCH_ALL_EXCEPTIONS("Unhandled exception in CHECK_DOMAIN(" #GENERATOR \
                            ", " #PREDICATE ")")
