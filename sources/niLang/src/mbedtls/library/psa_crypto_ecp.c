    ...
    mbedtls_ecp_keypair **p_ecp )
{
    ...
    // Pierre: Rewritten this way to work around an internal compiler error in MSVC 15 x86
    int type__is_public_key = PSA_KEY_TYPE_IS_PUBLIC_KEY( type );
    int type__ecc_family = PSA_KEY_TYPE_ECC_GET_FAMILY( type );
    if( type__is_public_key &&
        type__ecc_family != PSA_ECC_FAMILY_MONTGOMERY )
    {
