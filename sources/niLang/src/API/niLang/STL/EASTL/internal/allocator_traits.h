...
struct has_value_type {
 private:
  ...
  template <class U>
  static eastl::yes_type test(typename U::value_type* = 0);
...
static eastl::no_type test(...);
template <class U>
static eastl::yes_type test(typename U::pointer* = 0);
} // namespace has_pointer_type_imp

template <class T>
struct has_pointer_type
    : public integral_constant<bool, sizeof(has_pointer_type_imp::test<T>(0)) ==
...
template <class T, class D>
struct pointer_type {
  typedef typename PointerTypeInternal::pointer_type<
    T, typename remove_reference<D>::type>::type type;
};
...
struct has_const_pointer {
 private:
  ...
  static eastl::no_type test(...);
  template <class U>
  static eastl::yes_type test(typename U::const_pointer* = 0);
...
#else
  typedef
    typename pointer_traits<Pointer>::template rebind<const T>::other type;
#endif
};
...
struct has_void_pointer {
 private:
  ...
  static eastl::no_type test(...);
  template <class U>
  static eastl::yes_type test(typename U::void_pointer* = 0);
...
struct has_const_void_pointer {
 private:
  ...
  static eastl::no_type test(...);
  template <class U>
  static eastl::yes_type test(typename U::const_void_pointer* = 0);
...
template <class Pointer, class Alloc,
          bool = has_const_void_pointer<Alloc>::value>
...
#else
  typedef
    typename pointer_traits<Pointer>::template rebind<const void>::other type;
#endif
};
...
struct has_size_type {
 private:
  ...
  static eastl::no_type test(...);
  template <class U>
  static char test(typename U::size_type* = 0);
...
decltype(eastl::declval<Alloc>().construct(eastl::declval<T*>(),
                                           eastl::declval<Args>()...),
         eastl::true_type())
has_construct_test(Alloc&& a, T* p, Args&&... args);

template <class Alloc, class Pointer, class... Args>
eastl::false_type has_construct_test(const Alloc& a, Pointer&& p,
                                     Args&&... args);
...
struct has_construct
    : public eastl::integral_constant<
        bool,
        eastl::is_same<decltype(has_construct_test(eastl::declval<Alloc>(),
                                                   eastl::declval<Pointer>(),
                                                   eastl::declval<Args>()...)),
                       eastl::true_type>::value> {};

// has_destroy
template <class Alloc, class Pointer>
auto has_destroy_test(Alloc&& a, Pointer&& p)
  -> decltype(a.destroy(p), eastl::true_type());
...
struct has_destroy
    : public eastl::integral_constant<
        bool, is_same<decltype(has_destroy_test(eastl::declval<Alloc>(),
                                                eastl::declval<Pointer>())),
                      eastl::true_type>::value> {};
...
struct has_max_size
    : public eastl::integral_constant<
        bool, is_same<decltype(has_max_size_test(eastl::declval<Alloc&>())),
                      eastl::true_type>::value> {};
...
struct allocator_traits {
  ...
  typedef
    typename Internal::pointer_type<value_type, allocator_type>::type pointer;
  typedef
    typename Internal::const_pointer<value_type, pointer, allocator_type>::type
      const_pointer;
  typedef
    typename Internal::void_pointer<pointer, allocator_type>::type void_pointer;
  typedef typename Internal::const_void_pointer<pointer, allocator_type>::type
    const_void_pointer;
  typedef typename Internal::alloc_traits_difference_type<
    allocator_type, pointer>::type difference_type;
  typedef typename Internal::size_type<allocator_type, difference_type>::type
    size_type;
  ...
  } // additional parenthesis disables the windows max macro from expanding.
  ...
  static void deallocate(allocator_type& a, pointer p, size_type n) EA_NOEXCEPT
  {
    a.deallocate(p, n);
  }

  template <class T, class... Args>
  static void internal_construct(eastl::true_type, allocator_type& a, T* p,
                                 Args&&... args)
  {
  ...
  template <class T, class... Args>
  static void internal_construct(false_type, allocator_type&, T* p,
                                 Args&&... args)
  {
  ...
  {
    internal_construct(Internal::has_construct<allocator_type, T*, Args...>(),
                       a, p, eastl::forward<Args>(args)...);
  }
