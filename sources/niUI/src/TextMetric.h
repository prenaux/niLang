...
struct sDisplayGlyph {
  ...
  tF32 Compute(const cFont* apFont, const tF32 afFontSizeDivByRes,
               const tF32 afX, const tF32 afY, const tU32 anPrevChar,
               const tU32 anChar);

  ///////////////////////////////////////////////
  __forceinline void PushToCanvas(ni::iCanvas* apCanvas, const cFont* apFont,
                                  ...
                                  const sVec3f& avPos) const
  {
    ...
    if (!apCanvas->GetMaterial() ||
        ...
        apFont->mStates.mptrMaterial->GetChannelTexture(
          eMaterialChannel_Base) != ogTex)
    {
      apCanvas->Flush();
      apFont->mStates.mptrMaterial->SetChannelTexture(eMaterialChannel_Base,
                                                      ogTex);
      apCanvas->SetMaterial(apFont->mStates.mptrMaterial);
    }

    const sVec2f tlPos = { displayPos.x + avPos.x, displayPos.y + avPos.y };
    const sVec2f brPos = { displayPos.z + avPos.x, displayPos.w + avPos.y };
    const sVec2f tlTexCoo = { displayTexCoo.x, displayTexCoo.y };
    const sVec2f brTexCoo = { displayTexCoo.z, displayTexCoo.w };

    apCanvas->RectTA(tlPos, brPos, tlTexCoo, brTexCoo, avPos.z, aColFront);
  }
};

//----------------------------------------------------------------------------
//
// Section: TextLineMetric
//
//----------------------------------------------------------------------------
...
static __forceinline void TextLineMetric_PushToCanvas(
  const sTextLineMetric* apTM, ni::iCanvas* apCanvas, const iFont* apFont,
  const tU32 aColFront, const sVec3f& avPos,
...
///////////////////////////////////////////////
static __forceinline sRectf TextLineMetric_ComputeRect(const iFont* apFont,
...
static __forceinline sRectf TextLineMetric_ComputeRect(const iFont* apFont,
                                                       ...
                                                       const tBool abKerning)
{
  return TextLineMetric_ComputeRect(apFont, aStr.Chars(), aStr.size(),
                                    abKerning);
}
...
niExportFuncCPP(sRectf) TextLayout_Compute(
  ...
  // back in there
  tFontFormatFlags* aFlags, astl::vector<sTextLineMetric>& aMetrics);
...
static __forceinline void TextLayout_PushToCanvas(
  const sTextLineMetric* apTextLineMetrics, const tU32 anNumTM,
  ni::iCanvas* apCanvas, const iFont* apFont, const tU32 aColFront,
  const sVec3f& avPos, const sRectf* apRectClipH, const sRectf* apRectClipV)
{
  if (apRectClipV) {
    ...
    for (; i < anNumTM; ++i, ++tm) {
      if ((tm->rect.w + avPos.y) > b)
        break;
      TextLineMetric_PushToCanvas(tm, apCanvas, apFont, aColFront, avPos,
                                  apRectClipH);
    }
  }
  else {
    niLoop (i, anNumTM) {
      TextLineMetric_PushToCanvas(&apTextLineMetrics[i], apCanvas, apFont,
                                  aColFront, avPos, apRectClipH);
    }
  }
}
