...
nish_std_VertexOutput niUIGpuFuncs_fixed_pa_vs(nish_std_VertexPA aInput, niUIGpuFuncs_FixedUniforms aUniforms);
...
nish_std_VertexOutput niUIGpuFuncs_fixed_pa_vs(nish_std_VertexPA aInput, niUIGpuFuncs_FixedUniforms aUniforms) {
  ...
  vec3 _tmp_9 = vec3_YAxis;
  vec4 _tmp_a = (aInput.color*aUniforms.materialColor);
  vec2 _tmp_f = vec2_Zero;
  return nish_std_VertexOutput_new(_tmp_5, _tmp_9, _tmp_a, _tmp_f);
}
// MODULE END niUIGpuFuncs

// Vertex Shader main: niUIGpuFuncs_fixed_pa_vs
...
void main(void) {
  ...
  nish_std_VertexOutput _rval_ = niUIGpuFuncs_fixed_pa_vs(aInput, aUniforms);
