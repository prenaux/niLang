...
niExportFuncCPP(const cString&) ni_stack_get_current(cString& aOutput,
                                                     void* apExp, int skip)
{
  niUnused(apExp);
  ...
  #if defined USE_BACKTRACE_SYMBOLS || defined USE_BACKTRACE_DLADDR
  {
    ...
    char** symbols = backtrace_symbols(callstack, nFrames);
    {
      ...
      for (int i = skip + 1; i < nFrames; ++i) {
        _StackCatEntry(aOutput, i - skip, nullptr, 0, symbols[i], nullptr, 0,
    ...
    aOutput << "--- BACKTRACE DLARR STACKTRACE ---\n";
    {
      for (int i = skip + 1; i < nFrames; ++i) {
        ...
        if (dladdr(callstack[i], &info)) {
          ...
          _StackCatEntry(aOutput, i - skip, nullptr, 0,
                         status == 0 ? demangled : info.dli_sname, nullptr,
                         (tIntPtr)callstack[i], STACK_INCLUDE_CODE_SNIPPETS);
          free(demangled);
        }
        else {
  ...
  #ifdef USE_BOOST_STACKTRACE
  {
    ...
    for (int i = skip + 1; i < st.size(); ++i) {
      auto& frame = st[i];
      _StackCatEntry(aOutput, i - skip, frame.source_file().c_str(),
                     frame.source_line(), frame.name().c_str(), nullptr,
                     (tIntPtr)frame.address(), STACK_INCLUDE_CODE_SNIPPETS);
    }
  ...
  #ifdef USE_BACKWARD
  {
    ...
    for (int i = skip + 1; i < st.size(); ++i) {
      ...
      _StackCatEntry(aOutput, i - skip, trace.source.filename.c_str(),
                     trace.source.line, trace.object_function.c_str(), nullptr,
                     (tIntPtr)trace.addr, STACK_INCLUDE_CODE_SNIPPETS);
    }
