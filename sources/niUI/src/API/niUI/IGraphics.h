...
struct iGraphics : public iUnknown {
  ...
  virtual tU32 __stdcall FVFGetTexCooDim(tFVF anFVF,
                                         tU32 anTexCooIndex) const = 0;
  ...
  virtual iBitmapFormat* __stdcall CreateBitmapFormat(
    eBitmapType aType, const achar* aaszFormat, tU32 anNumMipMaps, tU32 anWidth,
    tU32 anHeight, tU32 anDepth) = 0;
  //! Create a new bitmap  format.
  virtual iBitmapFormat* __stdcall CreateBitmapFormatEx(
  ...
  virtual iBitmap2D* __stdcall CreateBitmap2DEx(tU32 nW, tU32 nH,
                                                iPixelFormat* pFmt) = 0;
  //! Create a 2D bitmap.
  virtual iBitmap2D* __stdcall CreateBitmap2D(tU32 nW, tU32 nH,
                                              const achar* aszPixFmt) = 0;

  //! Create a 2D bitmap from the specified memory.
  //! \remark the given iPixelFormat instance will be release if the creation failed and when
  //!     the bitmap is released.
  virtual iBitmap2D* __stdcall CreateBitmap2DMemoryEx(tU32 nW, tU32 nH,
                                                      iPixelFormat* pFmt,
                                                      tU32 anPitch,
                                                      tPtr ptrAddr,
                                                      tBool bFreeAddr) = 0;
  //! Create a 2D bitmap from the specified memory.
  virtual iBitmap2D* __stdcall CreateBitmap2DMemory(tU32 nW, tU32 nH,
                                                    const achar* aszPixFmt,
                                                    tU32 anPitch, tPtr ptrAddr,
                                                    tBool bFreeAddr) = 0;

  //! Create a Cube bitmap.
  virtual iBitmapCube* __stdcall CreateBitmapCubeEx(tU32 ulSize,
                                                    iPixelFormat* pFmt) = 0;
  //! Create a Cube bitmap.
  virtual iBitmapCube* __stdcall CreateBitmapCube(tU32 ulSize,
                                                  const achar* aszPixFmt) = 0;

  //! Create a 3D bitmap.
  virtual iBitmap3D* __stdcall CreateBitmap3DEx(tU32 anW, tU32 anH, tU32 anD,
                                                iPixelFormat* apFmt) = 0;
  //! Create a 3D bitmap.
  virtual iBitmap3D* __stdcall CreateBitmap3D(tU32 anW, tU32 anH, tU32 anD,
                                              const achar* aaszPixFmt) = 0;
  //! Create a 3D bitmap from the specified memory.
  virtual iBitmap3D* __stdcall CreateBitmap3DMemoryEx(
    tU32 anW, tU32 anH, tU32 anD, iPixelFormat* apFmt, tU32 anRowPitch,
    tU32 anSlicePitch, tPtr aptrAddr, tBool abFreeAddr) = 0;
  //! Create a 3D bitmap from the specified memory.
  virtual iBitmap3D* __stdcall CreateBitmap3DMemory(
    tU32 anW, tU32 anH, tU32 anD, const achar* aszPixFmt, tU32 anRowPitch,
    tU32 anSlicePitch, tPtr aptrAddr, tBool abFreeAddr) = 0;

  //! Load a bitmap from a file.
  virtual iBitmapBase* __stdcall LoadBitmap(iFile* pFile) = 0;
  //! Load a bitmap from a resource.
  virtual iBitmapBase* __stdcall LoadBitmapFromRes(iHString* ahspRes,
                                                   iHString* ahspBasePath) = 0;
  //! Load a bitmap from a file with the specified format.
  virtual iBitmapBase* __stdcall LoadBitmapEx(const achar* aaszFormat,
                                              iFile* pFile) = 0;

  //! Save a bitmap in a file.
  virtual tBool __stdcall SaveBitmap(const achar* aszFile, iBitmapBase* pBmp,
                                     tU32 ulCompression = 0) = 0;

  //! Save a bitmap in the specified format in a file.
  virtual tBool __stdcall SaveBitmapEx(const achar* aaszFormat, iFile* apFile,
                                       iBitmapBase* apBmp,
                                       tU32 anCompression) = 0;

  //! Find the path/url matching the specified resource and basepath.
  virtual cString __stdcall URLFindBitmapFilePath(const achar* aszRes,
                                                  const achar* aszBasePath) = 0;
  ...
  //!     methods are used.
  virtual iFile* __stdcall OpenBitmapFile(const achar* aszRes,
                                          const achar* aszBasePath = NULL) = 0;
  ...
  virtual tI64 __stdcall ComputeBitmapDiff(const iBitmap2D* apImgA,
                                           const iBitmap2D* apImgB,
                                           iBitmap2D* apOutput = NULL,
                                           tF64 afThreshold = 0.1,
                                           tBool abIgnoreAA = eTrue) = 0;
  //! @}
  ...
  //!     will be invalidated.
  virtual iGeometry* __stdcall CreateGeometryPolygonal(
    tU32 aulNumVertices, tU32 aulNumIndices, tGeometryCreateFlags aFlags,
    tFVF aFVF) = 0;
  //! Create a polygonal geometry from an exisiting vertex array and index array.
  virtual iGeometry* __stdcall CreateGeometryPolygonalEx(iVertexArray* apVA,
                                                         iIndexArray* apIA) = 0;
  ...
  virtual iGeometryModifier* __stdcall CreateGeometryModifier(
    const achar* aszName, iGeometry* apGeometry, iUnknown* apInitData) = 0;

  //! Create a cube polygonal geometry.
  virtual iGeometry* __stdcall CreateGeometryPolygonalCube(
    tGeometryCreateFlags aFlags, tFVF aFVF, const sVec3f& avCenter,
    tF32 afWidth, tBool abCW, tU32 aulColor, const sMatrixf& amtxUV) = 0;
  //! Create a plane polygonal geometry.
  virtual iGeometry* __stdcall CreateGeometryPolygonalPlane(
    tGeometryCreateFlags aFlags, tFVF aFVF, ePlaneType aPlaneType,
    const sVec3f& avCenter, tF32 afWidth, tF32 afHeight, tU32 aulNumDiv,
    tBool abCW, tU32 aulColor, const sMatrixf& amtxUV) = 0;
  //! Create a cylinder polygonal geometry.
  virtual iGeometry* __stdcall CreateGeometryPolygonalCylinder(
    tGeometryCreateFlags aFlags, tFVF aFVF, tF32 baseRadius, tF32 topRadius,
    tF32 height, tI32 slices, tI32 stacks, tBool abCW, tU32 aulColor,
    const sMatrixf& amtxUV, tBool abCap, tBool abCentered) = 0;
  //! Create a sphere polygonal geometry.
  virtual iGeometry* __stdcall CreateGeometryPolygonalSphere(
    tGeometryCreateFlags aFlags, tFVF aFVF, tF32 radius, tI32 slices,
    tI32 stacks, tBool abCW, tU32 aulColor, const sMatrixf& amtxUV) = 0;
  //! Create a geosphere polygonal geometry.
  virtual iGeometry* __stdcall CreateGeometryPolygonalGeosphere(
    tGeometryCreateFlags aFlags, tFVF aFVF, tF32 radius, tI32 slices,
    tBool abCW, tU32 aulColor, const sMatrixf& amtxUV) = 0;
  //! Create a disk sweep polygonal geometry.
  virtual iGeometry* __stdcall CreateGeometryPolygonalDiskSweep(
    tGeometryCreateFlags aFlags, tFVF aFVF, tF32 innerRadius, tF32 outerRadius,
    tI32 slices, tI32 stacks, tF32 startAngle, tF32 sweepAngle, tBool abCW,
    tU32 aulColor, const sMatrixf& amtxUV) = 0;
  //! Update a disk polygonal geometry.
  virtual tBool __stdcall UpdateGeometryPolygonalDiskSweep(
    iGeometry* apGeom, tF32 innerRadius, tF32 outerRadius, tI32 slices,
    tI32 stacks, tF32 startAngle, tF32 sweepAngle, tBool abCW, tU32 aulColor,
    const sMatrixf& amtxUV) = 0;
  //! Create a disk polygonal geometry.
  virtual iGeometry* __stdcall CreateGeometryPolygonalDisk(
    tGeometryCreateFlags aFlags, tFVF aFVF, tF32 innerRadius, tF32 outerRadius,
    tI32 slices, tI32 stacks, tBool abCW, tU32 aulColor,
    const sMatrixf& amtxUV) = 0;
  //! @}
  ...
  //! {Property}
  virtual iDeviceResourceManager* __stdcall GetGenericDeviceResourceManager()
    const = 0;
  //! @}
  ...
  //! {Property}
  virtual iDeviceResourceManager* __stdcall GetTextureDeviceResourceManager()
    const = 0;
  ...
  virtual iTexture* __stdcall CreateTextureFromBitmap(iHString* ahspName,
                                                      iBitmapBase* apBitmap,
                                                      tTextureFlags aFlags) = 0;
  //! Create a new texture from the specified resource.
  virtual iTexture* __stdcall CreateTextureFromRes(iHString* ahspRes,
  ...
  virtual tBool __stdcall BlitBitmapToTexture(
    ...
    eTextureBlitFlags aFlags = eTextureBlitFlags_None) = 0;
  ...
  virtual tBool __stdcall BlitBitmap3DToTexture(
    iBitmap3D* apSrc, iTexture* apDest, tU32 anDestLevel,
  ...
  virtual tBool __stdcall BlitTextureToBitmap3D(
    iTexture* apSrc, tU32 anSrcLevel, iBitmap3D* apDest,
    const sVec3i& aSrcMin = sVec3i::Zero(),
    const sVec3i& aDestMin = sVec3i::Zero(),
    const sVec3i& avSize = sVec3i::Zero(),
    eTextureBlitFlags aFlags = eTextureBlitFlags_None) = 0;
  ...
  virtual iShaderConstants* __stdcall CreateShaderConstants(
    tU32 anMaxRegisters) const = 0;
  //! Serialize shader constants in the specified datatable.
  virtual tBool __stdcall SerializeShaderConstants(iShaderConstants* apConsts,
                                                   iDataTable* apDT,
                                                   tSerializeFlags aFlags) = 0;

  //! Get the shader device resource manager.
  //! {Property}
  virtual iDeviceResourceManager* __stdcall GetShaderDeviceResourceManager()
    const = 0;
  ...
  //! {Property}
  virtual iSamplerStates* __stdcall GetCompiledSamplerStates(
    tIntPtr aHandle) const = 0;
  //! Get the specified compiled rasterizer states.
  //! {Property}
  virtual iRasterizerStates* __stdcall GetCompiledRasterizerStates(
    tIntPtr aHandle) const = 0;
  //! Get the specified compiled depth stencil states.
  //! {Property}
  virtual iDepthStencilStates* __stdcall GetCompiledDepthStencilStates(
    tIntPtr aHandle) const = 0;
  //! @}
  ...
  virtual tF32 __stdcall ColorLuminanceEx(
    const sVec3f& aColor, const sVec3f& avLuminanceDistribution) = 0;
  //! Correct the gamma of a color.
  virtual sVec3f __stdcall ColorGammaCorrect(const sVec3f& aC,
                                             tF32 afFactor) = 0;
  ...
  virtual sVec4f __stdcall ColorConvert(eColorSpace aSource, eColorSpace aDest,
                                        const sVec4f& aColor) = 0;
  ...
  virtual sVec4f __stdcall ColorConvertEx(eColorSpace aSource,
                                          eColorSpace aDest,
                                          const sVec4f& aColor,
                                          const sVec4f& aXYZRef) = 0;
#endif
  ...
  virtual const achar* __stdcall FindColorName(
    const sColor4f& aColor) const = 0;
  ...
  virtual sVec4f __stdcall GetColor4FromNameA(iHString* ahspName,
                                              tF32 afAlpha) const = 0;
  //! Get a color from it's name.
  //! {Property}
  //! \remark Supported syntaxes, case insensitive, in order of priority:<br/>
  ...
  //! \remark See the W3C CSS2 specifications.
  virtual sVec4f __stdcall GetCssColor4FromNameA(iHString* ahspName,
                                                 tF32 afAlpha) const = 0;
  //! @}
  ...
  virtual void __stdcall UDPointsSphereHammersley(
    tVec3fCVec* apResults) const = 0;
  //! Generates hammersley base p1 points on a unit sphere.
  virtual void __stdcall UDPointsSphereHammersleyEx(tVec3fCVec* apResults,
                                                    tI32 p1) const = 0;
  //! Generates halton p1=2,p2  points on a unit sphere.
  virtual void __stdcall UDPointsSphereHalton(tVec3fCVec* apResults,
                                              tI32 p2 = 3) const = 0;
  //! Generates halton p1,p2  points on a unit sphere.
  virtual void __stdcall UDPointsSphereHaltonEx(tVec3fCVec* apResults, tI32 p1,
                                                tI32 p2) const = 0;
  //! Generates random points on a unit hemisphere.
  virtual void __stdcall UDPointsHemisphereRandom(
    tVec3fCVec* apResults) const = 0;
  //! Generates hammersley base2 points on a unit hemisphere.
  virtual void __stdcall UDPointsHemisphereHammersley(
    tVec3fCVec* apResults) const = 0;
  //! Generates hammersley base p1 points on a unit hemisphere.
  virtual void __stdcall UDPointsHemisphereHammersleyEx(tVec3fCVec* apResults,
                                                        tI32 p1) const = 0;
  //! Generates halton p1=2,p2  points on a unit hemisphere.
  virtual void __stdcall UDPointsHemisphereHalton(tVec3fCVec* apResults,
                                                  tI32 p2 = 3) const = 0;
  //! Generates halton p1,p2  points on a unit hemisphere.
  virtual void __stdcall UDPointsHemisphereHaltonEx(tVec3fCVec* apResults,
                                                    tI32 p1, tI32 p2) const = 0;
  ...
  virtual void __stdcall UDPointsPlaneHammersley(
    tVec2fCVec* apResults) const = 0;
  //! Generate hammersley base p1 points on a unit plane.
  virtual void __stdcall UDPointsPlaneHammersleyEx(tVec2fCVec* apResults,
                                                   tI32 p1) const = 0;
  //! Generate halton p1=2,p2 points on a unit plane.
  virtual void __stdcall UDPointsPlaneHalton(tVec2fCVec* apResults,
                                             tI32 p2 = 3) const = 0;
  //! Generate halton p1,p2 points on a unit plane.
  virtual void __stdcall UDPointsPlaneHaltonEx(tVec2fCVec* apResults, tI32 p1,
                                               tI32 p2) const = 0;
  //! @}
  ...
  virtual tF32 __stdcall DampedSpringGetDampingRatio(const tF32 Ks,
                                                     const tF32 Kd) const = 0;
  //! Compute the value of Kd for the specified Ks that will result in a spring with the specified damping ratio.
  virtual tF32 __stdcall DampedSpringComputeKdFromDampingRatio(
    const tF32 Ks, const tF32 E) const = 0;
  //! Computes the acceleration of a 1D damped spring system.
  //! \see ni::iSystem::DampedSpringAcceleration4
  virtual tF32 __stdcall DampedSpringAcceleration1(tF32 D, tF32 V,
                                                   const tF32 Ks,
                                                   const tF32 Kd) const = 0;
  //! Computes the acceleration of a 1D damped spring system.
  //! \see ni::iSystem::DampedSpringAcceleration4
  virtual sVec2f __stdcall DampedSpringAcceleration2(const sVec2f& D,
                                                     const sVec2f& V,
                                                     const tF32 Ks,
                                                     const tF32 Kd) const = 0;
  //! Computes the acceleration of a 1D damped spring system.
  //! \see ni::iSystem::DampedSpringAcceleration4
  virtual sVec3f __stdcall DampedSpringAcceleration3(const sVec3f& D,
                                                     const sVec3f& V,
                                                     const tF32 Ks,
                                                     const tF32 Kd) const = 0;
  ...
  //!        the acceleration.
  virtual sVec4f __stdcall DampedSpringAcceleration4(const sVec4f& D,
                                                     const sVec4f& V,
                                                     const tF32 Ks,
                                                     const tF32 Kd) const = 0;
  //! Creates a 1D damped spring.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpring1* __stdcall CreateDampedSpring1(
    const tF32 Ks, const tF32 Kd) const = 0;
  //! Creates a 2D damped spring.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpring2* __stdcall CreateDampedSpring2(
    const tF32 Ks, const tF32 Kd) const = 0;
  //! Creates a 3D damped spring.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpring3* __stdcall CreateDampedSpring3(
    const tF32 Ks, const tF32 Kd) const = 0;
  //! Creates a 4D damped spring.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpring4* __stdcall CreateDampedSpring4(
    const tF32 Ks, const tF32 Kd) const = 0;
  //! Creates a 1D damped spring position.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpringPosition1* __stdcall CreateDampedSpringPosition1(
    const tF32 Ks, const tF32 Kd) const = 0;
  //! Creates a 2D damped spring position.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpringPosition2* __stdcall CreateDampedSpringPosition2(
    const tF32 Ks, const tF32 Kd) const = 0;
  //! Creates a 3D damped spring position.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpringPosition3* __stdcall CreateDampedSpringPosition3(
    const tF32 Ks, const tF32 Kd) const = 0;
  //! Creates a 4D damped spring position.
  //! \remark If Ks and Kd are zero a critically damped spring with Ks=10 is set.
  virtual ni::iDampedSpringPosition4* __stdcall CreateDampedSpringPosition4(
    const tF32 Ks, const tF32 Kd) const = 0;
#endif
  //! @}
  ...
  //! \remark Returns a font instance that can be used directly.
  virtual iFont* __stdcall LoadFontEx(iHString* ahspName,
                                      tFontLoadFlags aFlags) = 0;
  ...
  //!         and create a new base font object. The prefered method
  //!         to load/create fonts is to use the LoadFont/Ex() functions.
  virtual iFont* __stdcall CreateFontFromBitmap(iHString* ahspName,
                                                iBitmap2D* pBmp) = 0;
  ...
  virtual tBool __stdcall RegisterSystemFont(iHString* ahspName,
  ...
  virtual iFont* __stdcall FindFont(iHString* ahspName,
                                    tFontLoadFlags mode) const = 0;
  ...
  virtual iImage* __stdcall CreateImageFromBitmapAndTexture(
    iBitmap2D* apBitmap, iTexture* apTexture) = 0;
  //! Create a new image.
  virtual iImage* __stdcall CreateImageEx(tU32 anWidth, tU32 anHeight,
                                          iBitmap2D* apBmp, iTexture* apTex,
                                          iTexture* apDepthStencil) = 0;

  //! Create an image map.
  virtual iImageMap* __stdcall CreateImageMap(const achar* aaszBaseName,
                                              const achar* aaszPxf) = 0;

  //! Create a graphics context that uses the specified Image as main render target and depth buffer.
  virtual iGraphicsContext* __stdcall CreateImageGraphicsContext(
    iImage* apImage) = 0;
  ...
  virtual iCanvas* __stdcall CreateCanvas(iGraphicsContext* apContext,
                                          iMaterial* apResetMaterial) = 0;
  //! @}
  ...
  virtual Ptr<iTextObject> __stdcall CreateTextObject(
    const achar* aaszText, const sVec2f& avSize,
  ...
  virtual iVideoDecoder* __stdcall CreateVideoDecoder(iHString* ahspName,
                                                      iFile* apFile,
                                                      tU32 aFlags) = 0;
  //! @}
  ...
  virtual Ptr<iVGImage> __stdcall CreateVGImage(tU32 anWidth,
                                                tU32 anHeight) = 0;
  virtual Ptr<iVGImage> __stdcall CreateVGImageFromBitmap(
    iBitmap2D* apBitmap) = 0;
  virtual Ptr<iVGImage> __stdcall CreateVGImageFromTexture(
    iTexture* apTexture) = 0;
  virtual Ptr<iVGImage> __stdcall CreateVGImageFromBitmapAndTexture(
    iBitmap2D* apBitmap, iTexture* apTexture) = 0;
  virtual Ptr<iVGImage> __stdcall CreateVGImageFromFile(iFile* apFile) = 0;
  virtual Ptr<iVGImage> __stdcall CreateVGImageFromResource(
    iHString* ahspRes) = 0;
  virtual Ptr<iVGGradientTable> __stdcall CreateVGGradientTable(
    tU32 anSize) = 0;
  virtual Ptr<iVGGradientTable> __stdcall CreateVGGradientTableTwoColors(
    const sColor4f& acolStart, const sColor4f& acolEnd) = 0;
  virtual Ptr<iVGGradientTable> __stdcall CreateVGGradientTableStops(
    const tF32CVec* apOffsets, const tVec4fCVec* apColors) = 0;
  virtual Ptr<iVGPaint> __stdcall CreateVGPaint(eVGPaintType aType) = 0;
  virtual Ptr<iVGPaint> __stdcall CreateVGPaintSolid(
    const sColor4f& aColor) = 0;
  virtual Ptr<iVGPaintImage> __stdcall CreateVGPaintImage(
    iVGImage* apImage, eVGImageFilter aFilter, eVGWrapType aWrap) = 0;
  virtual Ptr<iVGPaintGradient> __stdcall CreateVGPaintGradient(
    eVGGradientType aType, iVGGradientTable* apGradientTable) = 0;
  virtual Ptr<iVGPaintGradient> __stdcall CreateVGPaintGradientTwoColors(
    eVGGradientType aType, const sColor4f& acolStart,
    const sColor4f& acolEnd) = 0;
  virtual Ptr<iVGPaintGradient> __stdcall CreateVGPaintGradientStops(
    eVGGradientType aType, const tF32CVec* apOffsets,
...
static inline tBool HasShaderProfile(T* apGraphics, S aUnit,
