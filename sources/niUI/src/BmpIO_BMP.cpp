...
static int read_image(iFile* f, iBitmap2D* bmp, NI_BITMAPINFOHEADER* infoheader)
{
  ...
  for (i = 0; i < (int)infoheader->biHeight; i++) {
    ...
    case 24:
      read_24bit_line(infoheader->biWidth, f, bmp,
                      infoheader->biHeight - i - 1);
      break;

    case 32:
      read_32bit_line(infoheader->biWidth, f, bmp,
                      infoheader->biHeight - i - 1);
      break;
...
///////////////////////////////////////////////
static void read_RLE8_compressed_image(iFile* pFile, iBitmap2D* pBmp,
  ...
  while (eopicflag == 0) {
    ...
    while ((eolflag == 0) && (eopicflag == 0)) {
      ...
      if (count > 0) {
        ...
        switch (val) {
        case 0: eolflag = 1; break;

        case 1: eopicflag = 1; break;
...
struct BitmapLoader_BMP : public ImplRC<iBitmapLoader> {
                                            ...
                                            iFile* pFile) niImpl
  {
    ...
    if (infoheader.biBitCount == 24) {
      ...
      niError(niFmt(_A("Can't load bitmap of '%d' bit count."),
    ...
    ptrBmp = apGraphics->CreateBitmap2DEx(
      ...
      apGraphics->CreatePixelFormat(strPixelFormat.Chars()));
    ...
    default: {
      niError(niFmt("Can't read BMP image, unknown compression '%d'.",
