...
#define FFMPEG_DEF_TIMEOUT 3000000 // Default timeout in nano seconds
...
#else
#define FFMPEG_LOG_LEVEL AV_LOG_ERROR
#endif
...
static int show_codecs(void *optctx, const char *opt, const char *arg)
{
	const AVCodecDescriptor **codecs;
...
VideoStream_FFmpeg::VideoStream_FFmpeg(ni::iFile* data_source,
...
VideoStream_FFmpeg::~VideoStream_FFmpeg()
{
	unload();
}

bool VideoStream_FFmpeg::load(ni::iFile* source)
{
  ...
  if (ni::StrEndsWith(source->GetSourcePath(),".ipcam")) {
    ...
    mstrUrl = dt->GetString("url");
    if (mstrUrl.empty()) {
      niWarning("ffmpeg: no url in ipcam datatable.");
      return false;
    }

    mFPS = dt->GetIntDefault("FPS", 10);
    if (dt->GetBoolDefault("swap_rgb",ni::eFalse)) {
      this->setOutputMode(TH_BGRX);
    }
    // init network for ipcam
    timeout = dt->GetInt("timeout_in_ms") * 1000;
  }
  ...
  if (avformat_network_init() != 0) {
    niWarning("ffmpeg: avformat network init failed!");
    return false;
  }


  timeout = timeout < FFMPEG_DEF_TIMEOUT ? FFMPEG_DEF_TIMEOUT : timeout;
  AVDictionary *dict = NULL;
  av_dict_set(&dict, "stimeout", niFmt("%d",timeout), 0); // Timeout in nano seconds
	if ((err = avformat_open_input(&mFormatContext, mstrUrl.c_str(), NULL, &dict)) != 0)
	{
	...
	mDuration = mFormatContext->duration / (float)AV_TIME_BASE;
  if (!mstrUrl.empty() || mDuration < 0) {
    mDuration = 0;
  }
...
void VideoStream_FFmpeg::unload()
{
	if (mAvioContext)
	{
...
bool VideoStream_FFmpeg::_readData()
{
	return 1;
}

bool VideoStream_FFmpeg::decodeNextFrame()
{
    ...
    if (ret < 0) {
      ...
      unload();
      niError(niFmt("frame end with error code %d",ret));
      return false;
    }
...
void VideoStream_FFmpeg::decodedAudioCheck()
{
...
float VideoStream_FFmpeg::decodeAudio()
{
	return -1;
}

void VideoStream_FFmpeg::doSeek()
{

}

void VideoStream_FFmpeg::_restart()
{
