...
#pragma niTodo( \
    "Make a template version of cPixelFormatStd to replace the mpBase thing...")

//////////////////////////////////////////////////////////////////////////////////////////////
// 8 bits pixel format

///////////////////////////////////////////////
tPtr cPixelFormatStd::s8::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                       tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s8::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                      tF32 a) const
{
...
{
  if (ulRSigned)
    col.x = tF32(tI32(((*((tU8*)(pC)) >> ulRLShift) & ulRMask) << ulRRShift)) /
            2147483648.0f;
  else
    col.x = tF32((*((tU8*)(pC)) >> ulRLShift) & ulRMask) / fRMul;

  if (ulGSigned)
    col.y = tF32(tI32(((*((tU8*)(pC)) >> ulGLShift) & ulGMask) << ulGRShift)) /
            2147483648.0f;
  else
    col.y = tF32((*((tU8*)(pC)) >> ulGLShift) & ulGMask) / fGMul;

  if (ulBSigned)
    col.z = tF32(tI32(((*((tU8*)(pC)) >> ulBLShift) & ulBMask) << ulBRShift)) /
            2147483648.0f;
  else
    col.z = tF32((*((tU8*)(pC)) >> ulBLShift) & ulBMask) / fBMul;

  if (ulASigned)
    col.w = tF32(tI32(((*((tU8*)(pC)) >> ulALShift) & ulAMask) << ulARShift)) /
            2147483648.0f;
  else
    col.w = tF32((*((tU8*)(pC)) >> ulALShift) & ulAMask) / fAMul;
}
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s16::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                        tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s16::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                       tF32 a) const
{
...
{
  if (ulRSigned)
    col.x = tF32(tI32(((*((tU16*)(pC)) >> ulRLShift) & ulRMask) << ulRRShift)) /
            2147483648.0f;
  else
    col.x = tF32((*((tU16*)(pC)) >> ulRLShift) & ulRMask) / fRMul;

  if (ulGSigned)
    col.y = tF32(tI32(((*((tU16*)(pC)) >> ulGLShift) & ulGMask) << ulGRShift)) /
            2147483648.0f;
  else
    col.y = tF32((*((tU16*)(pC)) >> ulGLShift) & ulGMask) / fGMul;

  if (ulBSigned)
    col.z = tF32(tI32(((*((tU16*)(pC)) >> ulBLShift) & ulBMask) << ulBRShift)) /
            2147483648.0f;
  else
    col.z = tF32((*((tU16*)(pC)) >> ulBLShift) & ulBMask) / fBMul;

  if (ulASigned)
    col.w = tF32(tI32(((*((tU16*)(pC)) >> ulALShift) & ulAMask) << ulARShift)) /
            2147483648.0f;
  else
    col.w = tF32((*((tU16*)(pC)) >> ulALShift) & ulAMask) / fAMul;
}
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s24::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                        tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s24::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                       tF32 a) const
{
...
void cPixelFormatStd::s24::UnpackPixelf(tPtr pC, sColor4f& col) const
{
  ...
  if (ulRSigned)
    col.x =
      tF32(tI32(((ulV >> ulRLShift) & ulRMask) << ulRRShift)) / 2147483648.0f;
  else
    col.x = tF32((ulV >> ulRLShift) & ulRMask) / fRMul;

  if (ulGSigned)
    col.y =
      tF32(tI32(((ulV >> ulGLShift) & ulGMask) << ulGRShift)) / 2147483648.0f;
  else
    col.y = tF32((ulV >> ulGLShift) & ulGMask) / fGMul;

  if (ulBSigned)
    col.z =
      tF32(tI32(((ulV >> ulBLShift) & ulBMask) << ulBRShift)) / 2147483648.0f;
  else
    col.z = tF32((ulV >> ulBLShift) & ulBMask) / fBMul;

  if (ulASigned)
    col.w =
      tF32(tI32(((ulV >> ulALShift) & ulAMask) << ulARShift)) / 2147483648.0f;
  else
    col.w = tF32((ulV >> ulALShift) & ulAMask) / fAMul;
}
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s32::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                        tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s32::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                       tF32 a) const
{
...
{
  if (ulRSigned)
    col.x = tF32(tI32(((*((tU32*)(pC)) >> ulRLShift) & ulRMask) << ulRRShift)) /
            2147483648.0f;
  else
    col.x = tF32((*((tU32*)(pC)) >> ulRLShift) & ulRMask) / fRMul;

  if (ulGSigned)
    col.y = tF32(tI32(((*((tU32*)(pC)) >> ulGLShift) & ulGMask) << ulGRShift)) /
            2147483648.0f;
  else
    col.y = tF32((*((tU32*)(pC)) >> ulGLShift) & ulGMask) / fGMul;

  if (ulBSigned)
    col.z = tF32(tI32(((*((tU32*)(pC)) >> ulBLShift) & ulBMask) << ulBRShift)) /
            2147483648.0f;
  else
    col.z = tF32((*((tU32*)(pC)) >> ulBLShift) & ulBMask) / fBMul;

  if (ulASigned)
    col.w = tF32(tI32(((*((tU32*)(pC)) >> ulALShift) & ulAMask) << ulARShift)) /
            2147483648.0f;
  else
    col.w = tF32((*((tU32*)(pC)) >> ulALShift) & ulAMask) / fAMul;
}
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s40::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                        tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s40::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                       tF32 a) const
{
...
void cPixelFormatStd::s40::UnpackPixelf(tPtr pC, sColor4f& col) const
{
  ...
  if (ulRSigned)
    col.x =
      tF32(tI64(((ulV >> ulRLShift) & qRMask) << ulRRShift)) / 2147483648.0f;

  if (ulGSigned)
    col.y =
      tF32(tI64(((ulV >> ulGLShift) & qGMask) << ulGRShift)) / 2147483648.0f;
  else
    col.y = tF32(tI64((ulV >> ulGLShift) & qGMask)) / fGMul;

  if (ulBSigned)
    col.z =
      tF32(tI64(((ulV >> ulBLShift) & qBMask) << ulBRShift)) / 2147483648.0f;
  else
    col.z = tF32(tI64((ulV >> ulBLShift) & qBMask)) / fBMul;

  if (ulASigned)
    col.w =
      tF32(tI64(((ulV >> ulALShift) & qAMask) << ulARShift)) / 2147483648.0f;
  else
    col.w = tF32(tI64((ulV >> ulALShift) & qAMask)) / fAMul;
}
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s48::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                        tU32 a) const
{
  tU64 col = ((((a >> ulARShift) & qAMask) << ulALShift) |
              (((r >> ulRRShift) & qRMask) << ulRLShift) |
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s48::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                       tF32 a) const
{
  tU64 col = ((tU64(a * fAMul) & qAMask) << ulALShift) |
             ((tU64(r * fRMul) & qRMask) << ulRLShift) |
...
void cPixelFormatStd::s48::UnpackPixelf(tPtr pC, sColor4f& col) const
{
  ...
  if (ulRSigned)
    ...
    col.x = tF32(tI64((ulV >> ulRLShift) & qRMask)) / fRMul;
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s56::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                        tU32 a) const
{
  tU64 col = ((((a >> ulARShift) & qAMask) << ulALShift) |
              (((r >> ulRRShift) & qRMask) << ulRLShift) |
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s56::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                       tF32 a) const
{
  tU64 col = ((tU64(a * fAMul) & qAMask) << ulALShift) |
             ((tU64(r * fRMul) & qRMask) << ulRLShift) |
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s64::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                        tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::s64::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                       tF32 a) const
{
...
{
  if (ulASigned)
    col.w = tF32(tI32(((*((tU64*)pC) >> ulALShift) & qAMask) << ulARShift)) /
            2147483648.0f;
  else
    col.w = tF32((*((tI64*)(pC)) >> ulALShift) & qAMask) / fAMul;

  if (ulRSigned)
    col.x = tF32(tI32(((*((tU64*)pC) >> ulRLShift) & qRMask) << ulRRShift)) /
            2147483648.0f;
  else
    col.x = tF32((*((tI64*)(pC)) >> ulRLShift) & qRMask) / fRMul;

  if (ulGSigned)
    col.y = tF32(tI32(((*((tU64*)pC) >> ulGLShift) & qGMask) << ulGRShift)) /
            2147483648.0f;
  else
    col.y = tF32((*((tI64*)(pC)) >> ulGLShift) & qGMask) / fGMul;

  if (ulBSigned)
    col.z = tF32(tI32(((*((tU64*)pC) >> ulBLShift) & qBMask) << ulBRShift)) /
            2147483648.0f;
  else
    col.z = tF32((*((tI64*)(pC)) >> ulBLShift) & qBMask) / fBMul;
}
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::sFloat::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                           tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::sFloat::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                          tF32 a) const
{
  if (niFlagIs(RFlags, PXFFLOAT_HASCHANNEL)) {
    if (niFlagIs(RFlags, PXFFLOAT_16BITS))
...
{
  col.x = niFlagIsNot(RFlags, PXFFLOAT_HASCHANNEL)
            ? 0
            : tU32(ni::Clamp<tF32>(
                     niFlagIs(RFlags, PXFFLOAT_16BITS)
                       ? (tF32) * ((tF16*)(pColor + PXFFLOAT_GETINDEX(RFlags)))
                   ...
                   65535.0f)
                << 16;
  col.y = niFlagIsNot(GFlags, PXFFLOAT_HASCHANNEL)
            ? 0
            : tU32(ni::Clamp<tF32>(
                     niFlagIs(GFlags, PXFFLOAT_16BITS)
                       ? (tF32) * ((tF16*)(pColor + PXFFLOAT_GETINDEX(GFlags)))
                   ...
                   65535.0f)
                << 16;
  col.z = niFlagIsNot(BFlags, PXFFLOAT_HASCHANNEL)
            ? 0
            : tU32(ni::Clamp<tF32>(
                     niFlagIs(BFlags, PXFFLOAT_16BITS)
                       ? (tF32) * ((tF16*)(pColor + PXFFLOAT_GETINDEX(BFlags)))
                   ...
                   65535.0f)
                << 16;
  col.w = niFlagIsNot(AFlags, PXFFLOAT_HASCHANNEL)
            ? 0
            : tU32(ni::Clamp<tF32>(
                     niFlagIs(AFlags, PXFFLOAT_16BITS)
                       ? (tF32) * ((tF16*)(pColor + PXFFLOAT_GETINDEX(AFlags)))
...
cPixelFormatStd::cPixelFormatStd(const achar* aszFormat)
{
  ...
  mCaps = ePixelFormatCaps(
    ePixelFormatCaps_BuildPixel | ePixelFormatCaps_UnpackPixel |
    ePixelFormatCaps_Blit | ePixelFormatCaps_BlitMirrorLeftRight |
    ePixelFormatCaps_BlitMirrorUpDown | ePixelFormatCaps_BlitStretch);
  ...
  if (*p == 'F') {
    ...
    for (i = 0; i < 4 && *p; ++i) {
      ...
      if (*pChannel != 0) {
        niError(niFmt(_A("Channel '%c' already defined, format '%s'."), *p,
                      aszFormat));
        niDelete pFloatFmt;
        return;
      }
      ...
      if (nNumBits != 16 && nNumBits != 32) {
        niError(niFmt(
          _A("Invalid number of bits '%d' after channel '%c', format '%s'."),
          nNumBits, channel, aszFormat));
        niDelete pFloatFmt;
        return;
      }
      *pChannel |= PXFFLOAT_HASCHANNEL | PXFFLOAT_BUILDINDEX(nCurByte) |
                   ((nNumBits == 16) ? PXFFLOAT_16BITS : 0);
    ...
    if (*p != 0) {
      niError(
        niFmt(_A("Expected end of format '%s', from '%s'."), aszFormat, p - 1));
      niDelete pFloatFmt;
      return;
    }
    ...
    while (1) {
      if (!*p || !ni::StrIsDigit(*p)) {
        if (strCurrentDigit.IsNotEmpty()) {
          ...
          case _A('Q'):
          case _A('X'):
          case _A('S'): lComp = 0; break;
          case _A('R'):
          case _A('W'):
          case _A('L'):
          case _A('P'):
          case _A('D'): lComp = 1; break;
          case _A('G'):
          case _A('V'): lComp = 2; break;
          case _A('B'):
          case _A('U'): lComp = 3; break;
          default: lComp = -1;
          }
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::BuildPixelul(tPtr pOut, tU32 r, tU32 g, tU32 b,
                                   tU32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::BuildPixelf(tPtr pOut, tF32 r, tF32 g, tF32 b,
                                  tF32 a) const
{
...
///////////////////////////////////////////////
tPtr cPixelFormatStd::BeginUnpackPixels(tPtr pSurface, tU32 ulPitch, tU32 ulX,
                                        tU32 ulY, tU32 ulW, tU32 ulH)
{
  return pSurface;
}
...
static __forceinline void dither_for_4444(const int r, const int g,
...
static __forceinline tU32 _ShouldDither(const iPixelFormat* apSrcFmt,
                                        const iPixelFormat* apDstFmt)
{
...
static __forceinline void _DitherColor12(sColor4ub& col, tU32 x, tU32 y)
{
  ...
  dither_for_4444(col.x, col.y, col.z /*,col.w*/, x, y, returned_r, returned_g,
                  returned_b /*,returned_a*/);
...
static __forceinline void _DitherColor(tU32 anBpp, sColor4ub& col, tU32 x,
                                       tU32 y)
{
  switch (anBpp) {
...
tBool cPixelFormatStd::Blit(tPtr pDst, tU32 ulDestPitch, tU32 dx, tU32 dy,
                            tPtr pSrc, tU32 ulSrcPitch, iPixelFormat* pSrcFmt,
                            tU32 sx, tU32 sy, tU32 w, tU32 h,
                            ePixelFormatBlit blitFlags) const
{
  ...
  if (blitFlags == ePixelFormatBlit_Normal) {
      ...
      if ((pSrcFmt->GetCaps() & ePixelFormatCaps_Signed)) {
        if (!(mCaps & ePixelFormatCaps_Signed)) {
          ...
          if (pSrc) {
            for (y = 0; y < h; ++y) {
              pubSrc =
                ((tU8*)(pSrc)) + ((y + sy) * ulSrcPitch) + (sx * ulSrcBpp);
              pubDst = ((tU8*)(pDst)) + ((y + dy) * ulDestPitch) +
                       (dx * mulBytesPerPixel);
              for (x = 0; x < w; ++x) {
                sColor4f c = pSrcFmt->UnpackPixelf(tPtr(pubSrc));
                BuildPixelus(tPtr(pubDst), tU16(c.x * 32767.0f + 32768.0f),
          ...
          if (ditherBpp) {
            ...
            for (y = 0; y < h; ++y) {
              ...
              for (x = 0; x < w; ++x) {
                ...
                BuildPixelus(tPtr(pubDst), (tU16)c.x, (tU16)c.y, (tU16)c.z,
                             (tU16)c.w);
                pubSrc += ulSrcBpp;
                pubDst += mulBytesPerPixel;
              }
    ...
    if (niFlagTest(blitFlags, ePixelFormatBlit_MirrorLeftRight |
      ...
      if (pSrc) {
        ...
        if (ditherBpp) {
          for (y = 0, yd = dy + h - 1; y < h; ++y, --yd) {
            ...
            pubDst = ((tU8*)(pDst)) + (yd * ulDestPitch) +
                     (dx * mulBytesPerPixel) + (w * mulBytesPerPixel) -
                     mulBytesPerPixel;
            for (x = 0; x < w; ++x) {
              sColor4ub c = pSrcFmt->UnpackPixelub(tPtr(pubSrc));
              _DitherColor(ditherBpp, c, x + sx, y + sy);
          ...
          if (ditherBpp) {
            for (y = 0, yd = dy + h - 1; y < h; ++y, --yd) {
              pubSrc =
                ((tU8*)(pSrc)) + ((y + sy) * ulSrcPitch) + (sx * ulSrcBpp);
              pubDst =
                ((tU8*)(pDst)) + (yd * ulDestPitch) + (dx * mulBytesPerPixel);
              for (x = 0; x < w; ++x) {
                sColor4ub c = pSrcFmt->UnpackPixelub(tPtr(pubSrc));
                _DitherColor(ditherBpp, c, x + sx, y + sy);
                BuildPixelub(tPtr(pubDst), c.x, c.y, c.z, c.w);
...
///////////////////////////////////////////////
tBool cPixelFormatStd::BlitStretch(tPtr pDst, tU32 ulDestPitch,
                                   tU32 ulDestWidth, tU32 ulDestHeight, tU32 dx,
                                   tU32 dy, tU32 dw, tU32 dh, tPtr pSrc,
                                   tU32 ulSrcPitch, iPixelFormat* pSrcFmt,
                                   tU32 sx, tU32 sy, tU32 sw, tU32 sh) const
{
  if (!(pSrcFmt->GetCaps() & ePixelFormatCaps_UnpackPixel))
    return eFalse;
  ...
  // no scaling
  if (sw == dw && sh == dh) {
    return Blit(pDst, ulDestPitch, dx, dy, pSrc, ulSrcPitch, pSrcFmt, sx, sy,
                sw, sh, ePixelFormatBlit_Normal);
  }

  // half scaling, most likely creating mipmaps, use a bilinear filter
#pragma niTodo("This should go in the blit flags")
  if (!(sw & 1) && !(sh & 1) && dw == sw / 2 && dh == sh / 2 && dx == 0 &&
      dy == 0 && sx == 0 && sy == 0)
  { // Want to create an inferior mip map level...
    ...
    if (pSrc) {
      ...
      for (y = 0; y < dh; ++y) {
        for (x = 0; x < dw; ++x) {
          pCol =
            ((tU8*)pSrc) + (ulSrcPitch * (y << 1)) + ((x << 1) * ulSrcByPP);
          ...
          BuildPixelub(
            tPtr(((tU8*)(pDst)) + (ulDestPitch * y) + (x * mulBytesPerPixel)),
            (ca.x + cb.x + cc.x + cd.x) >> 2, (ca.y + cb.y + cc.y + cd.y) >> 2,
            (ca.z + cb.z + cc.z + cd.z) >> 2, (ca.w + cb.w + cc.w + cd.w) >> 2);
        }
      }
    ...
    if (pSrc) {
      ...
      if (ditherBpp) {
        for (; x < dyend; ++x, y += yinc) {
          ...
          for (tU32 ditherX = si_dx; d < dend;
               d += GetBytesPerPixel(), s += xinc, ++ditherX)
          {
