...
///
  ...
  #define EASTL_LRUCACHE_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX         \
      " lru_cache" // Unless the user overrides something, this is "EASTL lru_cache".
#endif

/// EASTL_LRUCACHE_DEFAULT_ALLOCATOR
///
#ifndef EASTL_LRUCACHE_DEFAULT_ALLOCATOR
  #define EASTL_LRUCACHE_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_LRUCACHE_DEFAULT_NAME)
#endif
...
template <typename Key, typename Value, typename Allocator = EASTLAllocatorType,
          ...
          typename map_type = eastl::unordered_map<
            Key, eastl::pair<Value, typename list_type::iterator>,
            eastl::hash<Key>, eastl::equal_to<Key>, Allocator>>
class lru_cache {
 public:
  using key_type = Key;
  ...
  using this_type =
    lru_cache<key_type, value_type, Allocator, list_type, map_type>;
  ...
  void emplace(const key_type& k, Args&&... args)
  {
    ...
    m_list.push_front(k);
    m_map.emplace(
      k, data_container_type(eastl::forward<Args>(args)..., m_list.begin()));
  }
  ...
  {
    m_map.set_allocator(allocator);
    m_list.set_allocator(allocator);
  }
