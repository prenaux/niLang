...
struct sMetalWindowSink : public ImplRC<iMessageHandler> {
  ...
  virtual tBool Init()
  {
    ...
    dispatch_data_t data =
      dispatch_data_create(_allShaders_DATA, _allShaders_DATA_SIZE, NULL, NULL);
  ...
  virtual void __stdcall HandleMessage(const tU32 anMsg, const Var& a,
                                       const Var& b)
  {
    switch (anMsg) {
    ...
    case eOSWindowMessage_KeyDown:
      ...
      case eKey_F:
        _w->SetFullScreen(
          _w->GetFullScreen() == eInvalidHandle ? 0 : eInvalidHandle);
        break;
      case eKey_Z: _w->SetCursorPosition(sVec2i::Zero()); break;
      case eKey_X: _w->SetCursorPosition(_w->GetClientSize() / 2); break;
      case eKey_C: _w->SetCursorPosition(_w->GetClientSize()); break;
      case eKey_Q: _w->SetCursor(eOSCursor_None); break;
      case eKey_W: _w->SetCursor(eOSCursor_Arrow); break;
      case eKey_E: _w->SetCursor(eOSCursor_Text); break;
      case eKey_K: _w->SetCursorCapture(!_w->GetCursorCapture()); break;
      }
      break;
    ...
    case eOSWindowMessage_MouseMove:
      ...
      niDebugFmt((
        ...
        a.mV2L[0], a.mV2L[1], b.mV2L[0], b.mV2L[1], _w->GetContentsScale()));
  #endif
      break;
...
TEST_FIXTURE(FOSWindowMetal, Clear)
{
  ...
  struct sMetalClear_MetalWindowSink : public sMetalWindowSink {
    sMetalClear_MetalWindowSink(iOSWindow* w)
        : sMetalWindowSink(w)
    {
    }
    virtual void Draw()
    {
      id<MTLRenderCommandEncoder> commandEncoder =
        (__bridge id<MTLRenderCommandEncoder>)
          _metalAPI->NewRenderCommandEncoder(Vec4<tF64>(0, 1, 1, 1), 1.0f, 0);
...
TEST_FIXTURE(FOSWindowMetal, Triangle)
{
  ...
  struct sMetalTriangle_MetalWindowSink : public sMetalWindowSink {
    ...
    sMetalTriangle_MetalWindowSink(iOSWindow* w)
        : sMetalWindowSink(w)
    {
    }
    ...
    tBool Init() override
    {
      ...
      vertices[0].pos = { 0.0, 0.5, 0 };
      vertices[0].colora = ULColorBuildf(1, 0, 0, 1);
      vertices[1].pos = { -0.5, -0.5, 0 };
      vertices[1].colora = ULColorBuildf(0, 1, 0, 1);
      vertices[2].pos = { 0.5, -0.5, 0 };
      vertices[2].colora = ULColorBuildf(0, 0, 1, 1);
      _vertexBuffer =
      ...
      dispatch_data_t data = dispatch_data_create(
        _allShaders_DATA, _allShaders_DATA_SIZE, NULL, NULL);
      id<MTLLibrary> library = [device newLibraryWithData:data error:&error];
      if (!library) {
        NSLog(@"Error occurred when compiling shader library: %@", error);
        return eFalse;
