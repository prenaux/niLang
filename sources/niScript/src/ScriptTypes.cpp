...
///////////////////////////////////////////////
niExportFunc(void) sqa_setdebugname(HSQUIRRELVM v, int idx,
...
niExportFunc(int) sqa_pushInterfaceDef(HSQUIRRELVM v,
                                       const sInterfaceDef* apInterfaceDef)
{
  ...
  //  niAssert(niIsOK(pVM));
  sq_pushud(*pVM, niNew sScriptTypeInterfaceDef(*v->_ss, apInterfaceDef));
  return SQ_OK;
}

niExportFunc(int) sqa_getInterfaceDef(HSQUIRRELVM v, int idx,
                                      const sInterfaceDef** appInterfaceDef)
{
  sScriptTypeInterfaceDef* pV = sqa_getud<sScriptTypeInterfaceDef>(v, idx);
  if (!pV)
    return SQ_ERROR;
  if (appInterfaceDef)
    *appInterfaceDef = pV->pInterfaceDef;
  return SQ_OK;
}
...
static int method_parameterName(HSQUIRRELVM v)
{
  ...
  if (!SQ_SUCCEEDED(sq_getint(v, 2, &idx)))
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "method_parameterName, index to lookup in the method parameters '%s' has to be an integer."),
        idx, methodDef->maszName, methodDef->mnNumParameters));
  ...
  if ((tU32)idx > methodDef->mnNumParameters)
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "method_parameterName, index '%d' to get in the method parameters '%s' is out of range (size:%d)."),
        idx, methodDef->maszName, methodDef->mnNumParameters));
...
static int method_parameterTypeFlags(HSQUIRRELVM v)
{
  ...
  if (!SQ_SUCCEEDED(sq_getint(v, 2, &idx)))
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "method_parameterValue, index to lookup in the method parameters '%s' has to be an integer."),
        idx, methodDef->maszName, methodDef->mnNumParameters));
  ...
  if ((tU32)idx > methodDef->mnNumParameters)
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "method_parameterValue, index '%d' to get in the method parameters '%s' is out of range (size:%d)."),
        idx, methodDef->maszName, methodDef->mnNumParameters));
...
niExportFunc(int) sqa_pushMethodDef(HSQUIRRELVM v,
                                    const sInterfaceDef* apInterfaceDef,
                                    const sMethodDef* apMethDef)
{
  niAssert(apMethDef != NULL);
  cScriptVM* pVM = reinterpret_cast<cScriptVM*>(sq_getforeignptr(v));
  //  niAssert(niIsOK(pVM));
  sq_pushud(*pVM,
            niNew sScriptTypeMethodDef(*v->_ss, apInterfaceDef, apMethDef));
  return SQ_OK;
}

niExportFunc(int) sqa_getMethodDef(HSQUIRRELVM v, int idx,
                                   const sInterfaceDef** appInterfaceDef,
                                   const sMethodDef** appMethDef)
{
  sScriptTypeMethodDef* pV = sqa_getud<sScriptTypeMethodDef>(v, idx);
  if (!pV)
    return SQ_ERROR;
  if (appMethDef)
    *appMethDef = pV->pMethodDef;
  if (appInterfaceDef)
    *appInterfaceDef = pV->pInterfaceDef;
  return SQ_OK;
}
...
int sqa_pushPropertyDef(HSQUIRRELVM v, const sInterfaceDef* apInterfaceDef,
                        const sMethodDef* apSetMethDef,
                        const sMethodDef* apGetMethDef)
{
  ...
  sScriptTypePropertyDef* ud =
    niNew sScriptTypePropertyDef(apInterfaceDef, apSetMethDef, apGetMethDef);
  sq_pushud(v, ud);
  return SQ_OK;
}

int sqa_getPropertyDef(HSQUIRRELVM v, int idx,
                       const sInterfaceDef** appInterfaceDef,
                       const sMethodDef** appSetMethDef,
                       const sMethodDef** appGetMethDef)
{
  sScriptTypePropertyDef* pV = sqa_getud<sScriptTypePropertyDef>(v, idx);
  if (!pV)
    return SQ_ERROR;
  if (appSetMethDef)
    *appSetMethDef = pV->pSetMethodDef;
  if (appGetMethDef)
    *appGetMethDef = pV->pGetMethodDef;
  if (appInterfaceDef)
    *appInterfaceDef = pV->pInterfaceDef;
  return SQ_OK;
}
...
///////////////////////////////////////////////
int sqa_pushIndexedProperty(HSQUIRRELVM v, iUnknown* apObject,
                            const sScriptTypePropertyDef* apProp)
{
  niAssert(niIsOK(apObject));
  sScriptTypeIndexedProperty* ud =
    niNew sScriptTypeIndexedProperty(*v->_ss, apObject, apProp);
  sq_pushud(v, ud);
  return SQ_OK;
}

///////////////////////////////////////////////
int sqa_getIndexedProperty(HSQUIRRELVM v, int idx, iUnknown** appObject,
                           const sScriptTypePropertyDef** appProp)
{
  sScriptTypeIndexedProperty* pV =
    sqa_getud<sScriptTypeIndexedProperty>(v, idx);
  if (!pV)
    return SQ_ERROR;
  if (appObject)
    *appObject = pV->pObject;
  if (appProp)
    *appProp = pV->pProp;
  return SQ_OK;
}
...
niExportFuncCPP(int) sqa_pushErrorCode(HSQUIRRELVM v,
                                       ain<nn<iHString>> ahspKind,
                                       ain<tChars> aDesc)
{
  cScriptVM* pVM = reinterpret_cast<cScriptVM*>(sq_getforeignptr(v));
  //  niAssert(niIsOK(pVM));
...
niExportFuncCPP(int) sqa_getResolvedType(HSQUIRRELVM v, int idx,
...
{
  const sEnumDef* pV;
  if (!SQ_SUCCEEDED(sqa_getEnumDef(v, 1, &pV)))
    return sq_throwerror(v, _A("Can't get enum definition."));
...
static int enum_newslot(HSQUIRRELVM v)
{
  ...
  return sq_throwerror(
    v,
    niFmt(_A("Can't call _newslot on enumeration table '%s'."), pV->maszName));
}
...
static int enum_delslot(HSQUIRRELVM v)
{
  ...
  return sq_throwerror(
    v,
    niFmt(_A("Can't call _delslot on enumeration table '%s'."), pV->maszName));
}
...
static int enum_set(HSQUIRRELVM v)
{
  ...
  if (!SQ_SUCCEEDED(sqa_getEnumDef(v, 1, &pV)))
    return sq_throwerror(v, _A("Can't get enum definition."));
  return sq_throwerror(
    v, niFmt(_A("Can't call _set on enumeration table '%s'."), pV->maszName));
}
...
static int enum_get(HSQUIRRELVM v)
{
  ...
  if (sq_type(obj) == OT_STRING) {
    ...
    if (!valTable->Get(obj, val)) {
      return sq_throwerror(v, niFmt(_A("Can't find key '%s' in enum '%s'."),
                                    _stringhval(obj), pV->pEnumDef->maszName));
    }
    v->Push(val);
  }
    ...
    if (!valTable->GetKey(val, key)) {
      return sq_throwerror(v, niFmt(_A("Can't find key '%d' in enum '%s'."),
                                    val, pV->pEnumDef->maszName));
    }
    v->Push(key);
  }
  else {
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "Lookup in the enumeration table '%s' has to be a string or a number."),
...
static int enum_elementName(HSQUIRRELVM v)
{
  ...
  if (!SQ_SUCCEEDED(sq_getint(v, 2, &idx)))
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "enum_elementName, index to lookup in the enumeration table '%s' has to be an integer."),
        idx, enumDef->maszName, enumDef->mnNumValues));
  ...
  if ((tU32)idx > enumDef->mnNumValues)
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "enum_elementName, index '%d' to get in the enumeration table '%s' is out of range (size:%d)."),
        idx, enumDef->maszName, enumDef->mnNumValues));
...
static int enum_elementValue(HSQUIRRELVM v)
{
  ...
  if (!SQ_SUCCEEDED(sq_getint(v, 2, &idx)))
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "enum_elementValue, index to lookup in the enumeration table '%s' has to be an integer."),
        idx, enumDef->maszName, enumDef->mnNumValues));
  ...
  if ((tU32)idx > enumDef->mnNumValues)
    return sq_throwerror(
      ...
      niFmt(
        _A(
          "enum_elementValue, index '%d' to get in the enumeration table '%s' is out of range (size:%d)."),
        idx, enumDef->maszName, enumDef->mnNumValues));
...
int sqa_pushEnumDef(HSQUIRRELVM v, const sEnumDef* apEnumDef)
{
  ...
  sScriptTypeEnumDef* ud = niNew sScriptTypeEnumDef(*v->_ss, apEnumDef);
  sq_pushud(v, ud);
  return SQ_OK;
}
...
niExportFunc(int) sqa_pushIUnknown(HSQUIRRELVM v, iUnknown* apClass)
{
  return reinterpret_cast<cScriptVM*>(sq_getforeignptr(v))
    ->GetAutomation()
    ->PushIUnknown(v, apClass);
}

///////////////////////////////////////////////
niExportFunc(int) sqa_getIUnknown(HSQUIRRELVM v, int idx,
                                  iUnknown** appIUnknown, const tUUID& aIID)
{
  return reinterpret_cast<cScriptVM*>(sq_getforeignptr(v))
    ->GetAutomation()
...
struct mathop_Neg {
  ...
  #define GET_MATHUD(NAME, INDEX)                           \
    SQUserData* ud##NAME;                                   \
    if (!SQ_SUCCEEDED(sq_getuserdata(v, INDEX, &ud##NAME))) \
      return SQ_ERROR;                                      \
    int type##NAME = udThis->GetType();                     \
    niUnused(type##NAME);                                   \
...
int __cdecl mathop_binary(HSQUIRRELVM v)
{
      ...
      (typeThis != typeOp))
  {
    return sq_throwerror(
      v, niFmt(
           _A("mathop_binary: This '%s' and Op '%s' are not compatible types."),
           v->_ss->GetTypeNameStr(typeThis), v->_ss->GetTypeNameStr(typeOp)));
  }
  ...
  default: niAssertUnreachable("Unreachable code."); break;
  }
...
int __cdecl mathop_cmp(HSQUIRRELVM v)
{
  ...
  #define GET_MATHUD_VECOP(NAME, INDEX) GET_MATHUD_TYPE(T, NAME, INDEX)

  #define VECOP_VARS1(FUNCNAME) GET_MATHUD_VECOP(This, 1);
  ...
  #define VECOP_VARS(FUNCNAME)                                              \
    ...
    if (typeThis != typeOp) {                                               \
      return sq_throwerror(                                                 \
        v, niFmt(_A(#FUNCNAME)                                              \
                   _A(": This '%s' and Op '%s' are not compatible types."), \
                 v->_ss->GetTypeNameStr(typeThis),                          \
                 v->_ss->GetTypeNameStr(typeOp)));                          \
    }
...
int __cdecl vecop_cross(HSQUIRRELVM v)
{
  ...
  switch (sizeof(T)) {
  case sizeof(sVec2f):
    sq_pushf32(v, (pThis->x * pOp->y) - (pThis->y * pOp->x));
    break;
  case sizeof(sVec3f):
    sqa_pushvec3f(v, (sVec3f&)VecCross(vec, (sVec3f&)*pThis, (sVec3f&)*pOp));
    break;
  case sizeof(sVec4f):
    sqa_pushvec3f(v, (sVec3f&)VecCross(vec, (sVec3f&)*pThis, (sVec3f&)*pOp));
    break;
...
int __cdecl vecop_transform(HSQUIRRELVM v)
{
  ...
  if (typeOp != eScriptType_Matrix)
    return sq_throwerror(
      v, niFmt(_A("vecop_transform: The operand must be a matrix.")));
...
int __cdecl vecop_transform_coord(HSQUIRRELVM v)
{
  ...
  if (typeOp != eScriptType_Matrix)
    return sq_throwerror(
      v, niFmt(_A("vecop_transform_coord: The operand must be a matrix.")));
  ...
  default: {
    return sq_throwerror(
      v, niFmt(_A("vecop_transform_coord: Unknown this type.")));
  }
  }
...
int __cdecl vecop_transform_normal(HSQUIRRELVM v)
{
  ...
  if (typeOp != eScriptType_Matrix)
    return sq_throwerror(
      v, niFmt(_A("vecop_transform_normal: The operand must be a matrix.")));
...
int __cdecl rect_intersect(HSQUIRRELVM v)
{
  ...
  SQUserData* pOp;
  if (!SQ_SUCCEEDED(sq_getuserdata(v, 2, &pOp)))
    return SQ_ERROR;
  ...
  default:
    return sq_throwerror(
      v,
      niFmt(_A(
        "rect_intersect: The operand must be a vector2 or vector4 or a rectangle.")));
  }
...
int __cdecl rect_frameCenter(HSQUIRRELVM v)
{
  ...
  tF32 l, r, t, b;
  if (!SQ_SUCCEEDED(sq_getf32(v, 2, &l)))
    return sq_throwerror(
      v, _A("rect_frameCenter: left parameter must be a number."));
  if (!SQ_SUCCEEDED(sq_getf32(v, 3, &r)))
    return sq_throwerror(
      v, _A("rect_frameCenter: right parameter must be a number."));
  if (!SQ_SUCCEEDED(sq_getf32(v, 4, &t)))
    return sq_throwerror(
      v, _A("rect_frameCenter: top parameter must be a number."));
  if (!SQ_SUCCEEDED(sq_getf32(v, 5, &b)))
    return sq_throwerror(
      v, _A("rect_frameCenter: bottom parameter must be a number."));
...
int __cdecl rect_frameLeftBorder(HSQUIRRELVM v)
{
  ...
  if (!pFrame)
    return sq_throwerror(
      v, niFmt(_A(
           "rect_frameLeftBorder: The parameter must be a rectangle/vec4.")));
...
int __cdecl rect_frameRightBorder(HSQUIRRELVM v)
{
  ...
  if (!pFrame)
    return sq_throwerror(
      v, niFmt(_A(
           "rect_frameRightBorder: The parameter must be a rectangle/vec4.")));
  const sRectf& frameRect = (sRectf&)pFrame->_val;
...
int __cdecl rect_frameTopBorder(HSQUIRRELVM v)
{
  ...
  if (!pFrame)
    return sq_throwerror(
      v, niFmt(
           _A("rect_frameTopBorder: The parameter must be a rectangle/vec4.")));
  const sRectf& frameRect = (sRectf&)pFrame->_val;
...
int __cdecl rect_frameBottomBorder(HSQUIRRELVM v)
{
  ...
  if (!pFrame)
    return sq_throwerror(
      v, niFmt(_A(
           "rect_frameBottomBorder: The parameter must be a rectangle/vec4.")));
  const sRectf& frameRect = (sRectf&)pFrame->_val;
...
static int vec2f_set(HSQUIRRELVM v)
{
  ...
  if (type == OT_INTEGER) {
    ...
    sq_getint(v, -2, &idx);
    if (idx < 0 || idx > 1)
      return sq_throwerror(v, _A("vec2 set, index out of range."));
    ...
    return sq_throwerror(
      v, _A("vec2 set, can index 2d vector only with string and integer."));
  }
  return 0;
}
...
static int vec2f_get(HSQUIRRELVM v)
{
  ...
  if (type == OT_INTEGER) {
    ...
    sq_getint(v, -1, &idx);
    if (idx < 0 || idx > 1)
      return sq_throwerror(v, _A("vec2f_get, index out of range."));
    ...
    return sq_throwerror(v,
                         niFmt(_A("vec2f_get, invalid member '%s'."), aaszStr));
  }

  return sq_throwerror(
    v, _A("vec2f_get, can index 2d vector only with string and integer."));
}
...
{
  sVec2f vf = { (float)aV.x, (float)aV.y };
...
{
  sVec2f vf;
  if (!SQ_SUCCEEDED(sqa_getvec2f(v, idx, &vf)))
    return SQ_ERROR;
  aV->x = (long)vf.x;
  aV->y = (long)vf.y;
  return SQ_OK;
...
int vec2f_constructor(HSQUIRRELVM v)
{
  ...
  case 1: {
    ...
    default: {
      return sq_throwerror(
        v,
        _A("vec2 constructor error, copy constructor parameter invalid type."));
    }
    }
    break;
  }
  ...
  default:
    return sq_throwerror(
      v, niFmt(_A("vec2 constructor error, invalid number of parameters '%d'."),
               top));
  }
  sqa_pushvec2f(v, val);
  return 1;
...
static int vec3f_set(HSQUIRRELVM v)
{
  ...
  if (type == OT_INTEGER) {
    ...
    sq_getint(v, -2, &idx);
    if (idx < 0 || idx > 2)
      return sq_throwerror(v, _A("vec3 set, index out of range."));
    sq_getf32(v, -1, &vals[idx]);
  }
  else if (type == OT_STRING) {
    SQInt idx;
    ...
    return sq_throwerror(
      v, _A("vec3 set, can index 3d vector only with string and integer."));
  }
  return 0;
}
...
static int vec3f_get(HSQUIRRELVM v)
{
  ...
  if (type == OT_INTEGER) {
    ...
    sq_getint(v, -1, &idx);
    if (idx < 0 || idx > 2)
      return sq_throwerror(v, _A("vec3f_get, index out of range."));
    sq_pushf32(v, vals[idx]);
    return 1;
  }
  else if (type == OT_STRING) {
    ...
    return sq_throwerror(v,
                         niFmt(_A("vec3f_get, invalid member '%s'."), aaszStr));
  }
  else {
    return sq_throwerror(
      v, _A("vec3f_get, can index 3d vector only with string and integer."));
  }
}
...
{
  sVec3f vf;
  if (!SQ_SUCCEEDED(sqa_getvec3f(v, idx, &vf)))
    return SQ_ERROR;
  aV->x = (long)vf.x;
  aV->y = (long)vf.y;
  aV->z = (long)vf.z;
...
int vec3f_constructor(HSQUIRRELVM v)
{
  ...
  case 1: {
    ...
    default: {
      return sq_throwerror(
        v,
        _A("vec3 constructor error, copy constructor parameter invalid type."));
    }
    }
    break;
  }
  ...
  default:
    return sq_throwerror(
      v, niFmt(_A("vec3 constructor error, invalid number of parameters '%d'."),
               top));
  }
  sqa_pushvec3f(v, val);
  return 1;
...
static int vec4f_set(HSQUIRRELVM v)
{
  ...
  if (type == OT_INTEGER) {
    ...
    sq_getint(v, -2, &idx);
    if (idx < 0 || idx > 3)
      return sq_throwerror(v, _A("vec4 set, index out of range."));
    sq_getf32(v, -1, &vals[idx]);
  }
  else if (type == OT_STRING) {
    SQInt idx;
    ...
    if (aaszStr[1] == 0) {
      ...
      if (ni::StrEq(aaszStr, _A("width"))) {
        ...
        if (SQ_FAILED(sqa_getvec2f(v, -1, &vec)))
          return sq_throwerror(v,
                               niFmt(_A("vec4 set, size is a vec2."), aaszStr));
        ...
        if (SQ_FAILED(sqa_getvec2f(v, -1, &vec)))
          return sq_throwerror(
            v, niFmt(_A("vec4 set, top_left is a vec2."), aaszStr));
        ...
        if (SQ_FAILED(sqa_getvec2f(v, -1, &vec)))
          return sq_throwerror(
            v, niFmt(_A("vec4 set, top_right is a vec2."), aaszStr));
        ...
        if (SQ_FAILED(sqa_getvec2f(v, -1, &vec)))
          return sq_throwerror(
            v, niFmt(_A("vec4 set, bottom_left is a vec2."), aaszStr));
        ...
        if (SQ_FAILED(sqa_getvec2f(v, -1, &vec)))
          return sq_throwerror(
            v, niFmt(_A("vec4 set, bottom_right is a vec2."), aaszStr));
        ...
        return sq_throwerror(
          v, niFmt(_A("vec4 set, unknown member '%s'."), aaszStr));
    }
    sq_getf32(v, -1, &vals[idx]);
  }
  else {
    return sq_throwerror(
      v, _A("vec4 set, can index 3d vector only with string and integer."));
  }
  return 0;
}
...
static int vec4f_get(HSQUIRRELVM v)
{
  ...
  if (type == OT_INTEGER) {
    ...
    sq_getint(v, -1, &idx);
    if (idx < 0 || idx > 3)
      return sq_throwerror(v, _A("vec4f_get, index out of range."));
    sq_pushf32(v, vals[idx]);
    return 1;
  }
  else if (type == OT_STRING) {
    ...
    if (ni::StrEq(aaszStr, _A("width"))) {
      ...
      sqa_pushvec2f(
        v, static_cast<const sRectf&>(vals).GetCorner(eRectCorners_TopLeft));
      return 1;
    }
    else if (ni::StrEq(aaszStr, _A("top_right"))) {
      sqa_pushvec2f(
        v, static_cast<const sRectf&>(vals).GetCorner(eRectCorners_TopRight));
      return 1;
    }
    else if (ni::StrEq(aaszStr, _A("bottom_left"))) {
      sqa_pushvec2f(
        v, static_cast<const sRectf&>(vals).GetCorner(eRectCorners_BottomLeft));
      return 1;
    }
    else if (ni::StrEq(aaszStr, _A("bottom_right"))) {
      sqa_pushvec2f(v, static_cast<const sRectf&>(vals).GetCorner(
    ...
    return sq_throwerror(v,
                         niFmt(_A("vec4f_get, invalid member '%s'."), aaszStr));
  }

  return sq_throwerror(
    v, _A("vec4f_get, can index 4d vector only with string and integer."));
}
...
{
  sVec4f vf;
  if (!SQ_SUCCEEDED(sqa_getvec4f(v, idx, &vf)))
    return SQ_ERROR;
  aV->x = (long)vf.x;
  aV->y = (long)vf.y;
  aV->z = (long)vf.z;
...
static int vector4base_constructor(HSQUIRRELVM v, sVec4f& val)
{
  ...
  case 1: {
    ...
    default: {
      return sq_throwerror(
        v,
        _A(
          "vector4base_constructor, copy constructor parameter invalid type."));
    }
    }
    break;
  }
  ...
  default:
    return sq_throwerror(
      v,
      niFmt(_A("vector4base_constructor, invalid number of parameters '%d'."),
            top));
  }
  return 1;
}
  ...
  if (type == OT_STRING) {
    ...
    if (*p++ != _A('_')) {
      ...
      if (ni::StrEq(aaszStr, _A("right"))) {
        if (!SQ_SUCCEEDED(sqa_getvec3f(v, -1, &val)))
          return sq_throwerror(
            v, _A("matrixf_set, invalid type, right is a vector3."));
        MatrixSetRight(vals, val);
      }
      else if (ni::StrEq(aaszStr, _A("up"))) {
        if (!SQ_SUCCEEDED(sqa_getvec3f(v, -1, &val)))
          return sq_throwerror(
            v, _A("matrixf_set, invalid type, up is a vector3."));
        MatrixSetUp(vals, val);
      }
      else if (ni::StrEq(aaszStr, _A("forward"))) {
        if (!SQ_SUCCEEDED(sqa_getvec3f(v, -1, &val)))
          return sq_throwerror(
            v, _A("matrixf_set, invalid type, forward is a vector3."));
        MatrixSetForward(vals, val);
      }
      else if (ni::StrEq(aaszStr, _A("translation"))) {
        if (!SQ_SUCCEEDED(sqa_getvec3f(v, -1, &val)))
          return sq_throwerror(
            v, _A("matrixf_set, invalid type, translation is a vector3."));
        MatrixSetTranslation(vals, val);
      }
      else
        return sq_throwerror(
          v, niFmt(_A("matrixf_set, unknown member '%s'."), aaszStr));
    }
    else {
      switch (*p++) {
      case '1': ridx = 0; break;
      ...
      default:
        return sq_throwerror(
          v, niFmt(_A("matrix set, invalid row in member '%s'."), aaszStr));
      }
      switch (*p) {
      case '1': cidx = 0; break;
      case '2': cidx = 1; break;
      ...
      default:
        return sq_throwerror(
          v, niFmt(_A("matrix set, invalid column in member '%s'."), aaszStr));
      }
    ...
    sq_getint(v, -2, &idx);
    if (idx < 0 || idx > 15)
      return sq_throwerror(v, _A("matrix get, index out of range."));
  ...
  if (type == OT_STRING) {
    ...
    if (*p++ != _A('_')) {
      if (ni::StrEq(aaszStr, _A("right"))) {
        ...
        return sq_throwerror(
          v, niFmt(_A("matrix set, unknown member '%s'."), aaszStr));
    }
    else {
      switch (*p++) {
      case '1': ridx = 0; break;
    ...
    return sq_throwerror(v,
                         _A("matrix get, can index matrix only with string."));
  }
  return 1;
}
...
static int matrixf_multiply(HSQUIRRELVM v)
{
  ...
  if (!pOp)
    return sq_throwerror(
      v, _A("matrixf_multiply, right operand can be only a matrix."));
...
int matrixf_constructor(HSQUIRRELVM v)
{
  ...
  default:
    return sq_throwerror(
      v,
      niFmt(_A("matrix constructor error, invalid number of parameters '%d'."),
            top));
  }
...
int uuid_constructor(HSQUIRRELVM v)
{
  ...
  case 1: {
    ...
    default: {
      return sq_throwerror(
        v, _A("uuid_constructor, copy constructor parameter invalid type."));
    }
    }
    break;
  }
  default:
    return sq_throwerror(
      v,
      niFmt(_A("uuid_constructor, invalid number of parameters '%d'."), top));
  }
  sqa_pushUUID(v, val);
  return 1;
}
...
SQRegFunction SQSharedState::_enum_default_delegate_funcz[] = {
  { "_typeof", enum_typeof, 1, NULL, _HC(typestr_string) },
  { "_set", enum_set, 0, NULL },
  { "_get", enum_get, 2, NULL },
  { "_delslot", enum_delslot, 0, NULL },
  { "_newslot", enum_newslot, 0, NULL },
  { "gettable", enum_gettable, 1, NULL, _HC(typestr_table) },
  { "enumName", enum_enumName, 1, NULL, _HC(typestr_string) },
  { "numElements", enum_numElements, 1, NULL, _HC(typestr_int) },
  { "elementName", enum_elementName, 2, NULL, _HC(typestr_string) },
  { "elementValue", enum_elementValue, 2, NULL, _HC(typestr_int) },
  { 0, 0 }
};

SQRegFunction SQSharedState::_vec2f_default_delegate_funcz[] = {
  { "_tostring", vec2f_tostring, 1, NULL, _HC(typestr_string) },
  { "_typeof", vec2f_typeof, 1, NULL, _HC(typestr_string) },
  { "_set", vec2f_set, 3, NULL },
  { "_get", vec2f_get, 2, NULL },
  { "_add", mathop_binary<mathop_Add>, 2, NULL, _HC(Vec2) },
  { "_sub", mathop_binary<mathop_Sub>, 2, NULL, _HC(Vec2) },
  { "_mul", mathop_binary<mathop_Mul>, 2, NULL, _HC(Vec2) },
  { "_div", mathop_binary<mathop_Div>, 2, NULL, _HC(Vec2) },
  { "normalize", vecop_normalize<sVec2f>, 1, NULL, _HC(Vec2) },
  { "Normalize", vecop_normalize<sVec2f>, 1, NULL, _HC(Vec2) },
  { "isnormal", vecop_isnormal<sVec2f>, 1, NULL, _HC(typestr_bool) },
  { "IsNormal", vecop_isnormal<sVec2f>, 1, NULL, _HC(typestr_bool) },
  { "dot", vecop_dot<sVec2f>, 2, NULL, _HC(typestr_float) },
  { "Dot", vecop_dot<sVec2f>, 2, NULL, _HC(typestr_float) },
  { "length", vecop_length<sVec2f>, 1, NULL, _HC(typestr_float) },
  { "Length", vecop_length<sVec2f>, 1, NULL, _HC(typestr_float) },
  { "lengthsq", vecop_lengthsq<sVec2f>, 1, NULL, _HC(typestr_float) },
  { "LengthSq", vecop_lengthsq<sVec2f>, 1, NULL, _HC(typestr_float) },
  { "cross", vecop_cross<sVec2f>, 2, NULL, _HC(Vec2) },
  { "Cross", vecop_cross<sVec2f>, 2, NULL, _HC(Vec2) },
  { "abs", vecop_abs<sVec2f>, 1, NULL, _HC(Vec2) },
  { "Abs", vecop_abs<sVec2f>, 1, NULL, _HC(Vec2) },
  { "min", vecop_min<sVec2f>, 2, NULL, _HC(Vec2) },
  { "Min", vecop_min<sVec2f>, 2, NULL, _HC(Vec2) },
  { "max", vecop_max<sVec2f>, 2, NULL, _HC(Vec2) },
  { "Max", vecop_max<sVec2f>, 2, NULL, _HC(Vec2) },
  { "transform", vecop_transform<sVec2f>, 2, NULL, _HC(Vec4) },
  { "Transform", vecop_transform<sVec2f>, 2, NULL, _HC(Vec4) },
  { "transform_coord", vecop_transform_coord<sVec2f>, 2, NULL, _HC(Vec2) },
  { "TransformCoord", vecop_transform_coord<sVec2f>, 2, NULL, _HC(Vec2) },
  { "transform_normal", vecop_transform_normal<sVec2f>, 2, NULL, _HC(Vec2) },
...
SQRegFunction SQSharedState::_vec3f_default_delegate_funcz[] = {
  { "_tostring", vec3f_tostring, 1, NULL, _HC(typestr_string) },
  { "_typeof", vec3f_typeof, 1, NULL, _HC(typestr_string) },
  { "_set", vec3f_set, 3, NULL },
  { "_get", vec3f_get, 2, NULL },
  { "_add", mathop_binary<mathop_Add>, 2, NULL, _HC(Vec3) },
  { "_sub", mathop_binary<mathop_Sub>, 2, NULL, _HC(Vec3) },
  { "_mul", mathop_binary<mathop_Mul>, 2, NULL, _HC(Vec3) },
  { "_div", mathop_binary<mathop_Div>, 2, NULL, _HC(Vec3) },
  { "normalize", vecop_normalize<sVec3f>, 1, NULL, _HC(Vec3) },
  { "Normalize", vecop_normalize<sVec3f>, 1, NULL, _HC(Vec3) },
  { "isnormal", vecop_isnormal<sVec3f>, 1, NULL, _HC(typestr_bool) },
  { "IsNormal", vecop_isnormal<sVec3f>, 1, NULL, _HC(typestr_bool) },
  { "dot", vecop_dot<sVec3f>, 2, NULL, _HC(typestr_float) },
  { "Dot", vecop_dot<sVec3f>, 2, NULL, _HC(typestr_float) },
  { "length", vecop_length<sVec3f>, 1, NULL, _HC(typestr_float) },
  { "Length", vecop_length<sVec3f>, 1, NULL, _HC(typestr_float) },
  { "lengthsq", vecop_lengthsq<sVec3f>, 1, NULL, _HC(typestr_float) },
  { "LengthSq", vecop_lengthsq<sVec3f>, 1, NULL, _HC(typestr_float) },
  { "cross", vecop_cross<sVec3f>, 2, NULL, _HC(Vec3) },
  { "Cross", vecop_cross<sVec3f>, 2, NULL, _HC(Vec3) },
  { "abs", vecop_abs<sVec3f>, 1, NULL, _HC(Vec3) },
  { "Abs", vecop_abs<sVec3f>, 1, NULL, _HC(Vec3) },
  { "min", vecop_min<sVec3f>, 2, NULL, _HC(Vec3) },
  { "Min", vecop_min<sVec3f>, 2, NULL, _HC(Vec3) },
  { "max", vecop_max<sVec3f>, 2, NULL, _HC(Vec3) },
  { "Max", vecop_max<sVec3f>, 2, NULL, _HC(Vec3) },
  { "transform", vecop_transform<sVec3f>, 2, NULL, _HC(Vec3) },
  { "Transform", vecop_transform<sVec3f>, 2, NULL, _HC(Vec3) },
  { "transform_coord", vecop_transform_coord<sVec3f>, 2, NULL, _HC(Vec4) },
  { "TransformCoord", vecop_transform_coord<sVec3f>, 2, NULL, _HC(Vec4) },
  { "transform_normal", vecop_transform_normal<sVec3f>, 2, NULL, _HC(Vec3) },
  { "TransformNormal", vecop_transform_normal<sVec3f>, 2, NULL, _HC(Vec3) },
  { "_modulo", mathop_binary<mathop_Mod>, 1, NULL, _HC(Vec3) },
...
SQRegFunction SQSharedState::_vec4f_default_delegate_funcz[] = {
  { "_tostring", vec4f_tostring, 1, NULL, _HC(typestr_string) },
  { "_typeof", vec4f_typeof, 1, NULL, _HC(typestr_string) },
  { "_set", vec4f_set, 3, NULL },
  { "_get", vec4f_get, 2, NULL },
  { "_add", mathop_binary<mathop_Add>, 2, NULL, _HC(Vec4) },
  { "_sub", mathop_binary<mathop_Sub>, 2, NULL, _HC(Vec4) },
  { "_mul", mathop_binary<mathop_Mul>, 2, NULL, _HC(Vec4) },
  { "_div", mathop_binary<mathop_Div>, 2, NULL, _HC(Vec4) },
  { "Normalize", vecop_normalize<sVec4f>, 1, NULL, _HC(Vec4) },
  { "normalize", vecop_normalize<sVec4f>, 1, NULL, _HC(Vec4) },
  { "isnormal", vecop_isnormal<sVec4f>, 1, NULL, _HC(typestr_bool) },
  { "IsNormal", vecop_isnormal<sVec4f>, 1, NULL, _HC(typestr_bool) },
  { "dot", vecop_dot<sVec4f>, 2, NULL, _HC(typestr_float) },
  { "Dot", vecop_dot<sVec4f>, 2, NULL, _HC(typestr_float) },
  { "length", vecop_length<sVec4f>, 1, NULL, _HC(typestr_float) },
  { "Length", vecop_length<sVec4f>, 1, NULL, _HC(typestr_float) },
  { "lengthsq", vecop_lengthsq<sVec4f>, 1, NULL, _HC(typestr_float) },
  { "LengthSq", vecop_lengthsq<sVec4f>, 1, NULL, _HC(typestr_float) },
  { "cross", vecop_cross<sVec4f>, 2, NULL, _HC(Vec4) },
  { "Cross", vecop_cross<sVec4f>, 2, NULL, _HC(Vec4) },
  { "abs", vecop_abs<sVec4f>, 1, NULL, _HC(Vec4) },
  { "Abs", vecop_abs<sVec4f>, 1, NULL, _HC(Vec4) },
  { "min", vecop_min<sVec4f>, 2, NULL, _HC(Vec4) },
  { "Min", vecop_min<sVec4f>, 2, NULL, _HC(Vec4) },
  { "max", vecop_max<sVec4f>, 2, NULL, _HC(Vec4) },
  { "Max", vecop_max<sVec4f>, 2, NULL, _HC(Vec4) },
  { "transform", vecop_transform<sVec4f>, 2, NULL, _HC(Vec4) },
  { "Transform", vecop_transform<sVec4f>, 2, NULL, _HC(Vec4) },
  { "transform_coord", vecop_transform_coord<sVec4f>, 2, NULL, _HC(Vec4) },
  { "TransformCoord", vecop_transform_coord<sVec4f>, 2, NULL, _HC(Vec4) },
  { "transform_normal", vecop_transform_normal<sVec4f>, 2, NULL, _HC(Vec4) },
  { "TransformNormal", vecop_transform_normal<sVec4f>, 2, NULL, _HC(Vec4) },
  { "_modulo", mathop_binary<mathop_Mod>, 1, NULL, _HC(Vec4) },
  { "_unm", mathop_unary<mathop_Neg>, 1, NULL, _HC(Vec4) },
  { "toint", vec4f_toint, 1, NULL, _HC(typestr_int) },
  { "ToInt", vec4f_toint, 1, NULL, _HC(typestr_int) },
  { "intersect", rect_intersect, 2, NULL, _HC(typestr_bool) },
  { "Intersect", rect_intersect, 2, NULL, _HC(typestr_bool) },
  { "frame_center", rect_frameCenter, 5, NULL, _HC(Vec4) },
  { "ComputeFrameCenter", rect_frameCenter, 5, NULL, _HC(Vec4) },
  { "frame_left_border", rect_frameLeftBorder, 2, NULL, _HC(typestr_float) },
  { "GetFrameLeftBorder", rect_frameLeftBorder, 2, NULL, _HC(typestr_float) },
  { "frame_right_border", rect_frameRightBorder, 2, NULL, _HC(typestr_float) },
  { "GetFrameRightBorder", rect_frameRightBorder, 2, NULL, _HC(typestr_float) },
  { "frame_top_border", rect_frameTopBorder, 2, NULL, _HC(typestr_float) },
  { "GetFrameTopBorder", rect_frameTopBorder, 2, NULL, _HC(typestr_float) },
...
SQRegFunction SQSharedState::_matrixf_default_delegate_funcz[] = {
  { "_tostring", matrixf_tostring, 1, NULL, _HC(typestr_string) },
  { "_typeof", matrixf_typeof, 1, NULL, _HC(typestr_string) },
  { "_set", matrixf_set, 3, NULL },
  { "_get", matrixf_get, 2, NULL },
  { "_add", mathop_binary<mathop_Add>, 2, NULL, _HC(Matrix) },
  { "_sub", mathop_binary<mathop_Sub>, 2, NULL, _HC(Matrix) },
  { "_mul", matrixf_multiply, 2, NULL, _HC(Matrix) },
  { "_modulo", mathop_binary<mathop_Mod>, 2, NULL, _HC(Matrix) },
  { "isidentity", matrixf_isidentity, 1, NULL, _HC(typestr_bool) },
  { "IsIdentity", matrixf_isidentity, 1, NULL, _HC(typestr_bool) },
  { "isnormal", matrixf_isnormal, 1, NULL, _HC(typestr_bool) },
  { "IsNormal", matrixf_isnormal, 1, NULL, _HC(typestr_bool) },
  { "isorthonormal", matrixf_isorthonormal, 1, NULL, _HC(typestr_bool) },
  { "IsOrthonormal", matrixf_isorthonormal, 1, NULL, _HC(typestr_bool) },
  { "isorthogonal", matrixf_isorthogonal, 1, NULL, _HC(typestr_bool) },
  { "IsOrthogonal", matrixf_isorthogonal, 1, NULL, _HC(typestr_bool) },
  { "determinant", matrixf_determinant, 1, NULL, _HC(typestr_float) },
  { "Determinant", matrixf_determinant, 1, NULL, _HC(typestr_float) },
  { "inverse", matrixf_inverse, 1, NULL, _HC(Matrix) },
  { "Inverse", matrixf_inverse, 1, NULL, _HC(Matrix) },
  { "transform_inverse", matrixf_transform_inverse, 1, NULL, _HC(Matrix) },
  { "TransformInverse", matrixf_transform_inverse, 1, NULL, _HC(Matrix) },
  { "transpose", matrixf_transpose, 1, NULL, _HC(Matrix) },
  { "Transpose", matrixf_transpose, 1, NULL, _HC(Matrix) },
  { "transposerot", matrixf_transposerot, 1, NULL, _HC(Matrix) },
  { "TransposeRot", matrixf_transposerot, 1, NULL, _HC(Matrix) },
  { "multiply", matrixf_multiply, 2, NULL, _HC(Matrix) },
  { "Multiply", matrixf_multiply, 2, NULL, _HC(Matrix) },
  { "_unm", mathop_unary<mathop_Neg>, 1, NULL, _HC(Matrix) },
  { 0, 0 }
};
...
{
  SQUserData* ud;
  if (!SQ_SUCCEEDED(sq_getuserdata(v, idx, &ud)))
    return NULL;
  if (ud->GetType() != type)
    return NULL;
  return (void*)ud;
