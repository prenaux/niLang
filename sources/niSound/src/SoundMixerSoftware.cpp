...
class cSoundMixerSoftware : public ImplRC<iSoundMixer, eImplFlags_Default,
 ...
 public:
  ...
  tBool __stdcall Initialize(eSoundFormat aFormat, tU32 anFreq,
                             iSoundDriverBuffer* apBuffer, tU32 anNumChannels);
  ...
  niLoop (i, mnNumChannels) {
    if (chan[i].used && !chan[i].pause) {
      ...
      if (chan[i].bits == 16) {
        if (chan[i].stereo) {
          chan[i].used =
            chan[i].buffer->ReadRaw(settings.transfert,
          ...
          chan[i].used = chan[i].buffer->ReadRaw(
            settings.transfert,
            (settings.nsamples * chan[i].frequency / settings.frequency + 1) *
              2 * 1,
            chan[i].loop);
          niLoop (j, settings.nsamples) {
...
{
  chan[nchan].speed =
    (tI32)(NFIXED * (float)chan[nchan].frequency / (float)settings.frequency);
}
...
void cSoundMixerSoftware::MixComputeVolume(tU32 nchan)
{
  ...
  if (settings.reversestereo) {
    chan[nchan].resvolr =
      (((256 - chan[nchan].pan) + (256 - chan[nchan].samplepan)) *
       chan[nchan].resvol);
    chan[nchan].resvoll =
      ((chan[nchan].pan + chan[nchan].samplepan) * chan[nchan].resvol);
  }
  else {
    chan[nchan].resvoll =
      (((256 - chan[nchan].pan) + (256 - chan[nchan].samplepan)) *
...
tBool cSoundMixerSoftware::Initialize(eSoundFormat aFormat, tU32 anFreq,
                                      ...
                                      tU32 anNumChannels)
{
  ...
  case eSoundFormat_Stereo16:
    ...
    break;
  default:
    niError(niFmt(_A("Invalid format for the mixer, %d bits, %d channels."),
                  SoundFormatGetBits(aFormat),
                  SoundFormatGetChannels(aFormat)));
    return eFalse;
  }
  ...
  const tU32 transfertBufferSize =
    4 * apBuffer->GetSize() * (MAX_FREQUENCY / settings.frequency) + 4;
...
///////////////////////////////////////////////
tBool __stdcall cSoundMixerSoftware::SetChannelBuffer(tU32 nchan,
...
///////////////////////////////////////////////
void __stdcall cSoundMixerSoftware::OnSoundDriverBufferDataSink(void* ptr,
                                                                tU32 size)
{
  switch (settings.format) {
  case eSoundFormat_Mono8: MixDecode8BitsMono(ptr, size); break;
  case eSoundFormat_Mono16: MixDecode16BitsMono(ptr, size); break;
  case eSoundFormat_Stereo8: MixDecode8BitsStereo(ptr, size); break;
  case eSoundFormat_Stereo16: MixDecode16BitsStereo(ptr, size); break;
  }
}
