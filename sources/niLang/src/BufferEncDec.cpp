...
struct FileBufferEncoder : public ImplRC<iFileBase> {
  FileBufferEncoder(iFileBase* apFile, iBufferEncoder* apEncoder,
                    tSize anBufferSize)
  {
  ...
  tBool _FlushBufferToFile()
  {
    ...
    if (!outputSize)
      return eFalse;
    tU32 t;
    t = niSwapLE32(_encoderMarker);
    _fp->WriteRaw(&t, 4);
    t = niSwapLE32(bufSize);
    _fp->WriteRaw(&t, 4);
    t = niSwapLE32(outputSize);
    _fp->WriteRaw(&t, 4);
...
struct FileBufferDecoder : public ImplRC<iFileBase> {
  ...
  FileBufferDecoder(iFileBase* apFile, iBufferDecoder* apDecoder,
                    tSize aDecodedSize)
  {
    niAssert(niIsOK(apFile));
  ...
  tSize _ReadBufferFromFile()
  {
    ...
    tSize d = _decoder->DecodeBuffer(&_buffer[0], _buffer.size(),
                                     &_packedBuffer[0], packedSize);
    _buffer.resize(d);
    return d;
  }
...
niExportFunc(iFileBase*) CreateFileBaseBufferEncoder(iFileBase* apBase,
                                                     iBufferEncoder* apEnc)
{
  niCheck(niIsOK(apBase), NULL);
  Ptr<iBufferEncoder> enc = apEnc;
  niCheck(enc.IsOK(), NULL);
  Ptr<iFileBase> fpBase =
    niNew FileBufferEncoder(apBase, enc, _knEncoderBufferSize);
  return fpBase.GetRawAndSetNull();
}

niExportFunc(iFileBase*) CreateFileBaseBufferDecoder(iFileBase* apBase,
...
niExportFunc(iFile*) CreateFileBufferEncoder(iFileBase* apBase,
                                             iBufferEncoder* apEnc)
{
  niCheck(niIsOK(apBase), NULL);
  Ptr<iBufferEncoder> enc = apEnc;
...
niExportFunc(iFile*) CreateFileBufferDecoder(iFileBase* apBase,
                                             iBufferDecoder* apDec,
                                             tSize aDecSize)
{
  niCheck(niIsOK(apBase), NULL);
...
niExportFunc(tSize) FileBufferEncode(iBufferEncoder* apEnc, iFile* apSrc,
...
niExportFunc(tSize) FileBufferDecode(iBufferDecoder* apDec, iFile* apSrc,
                                     tSize anSrcSize, iFile* apDest,
                                     tSize anDestSize)
{
  if (anSrcSize == 0)
  ...
  niCheckIsOK(apDest, 0);
  Ptr<iFileBase> dec =
    niNew FileBufferDecoder(apSrc->GetFileBase(), apDec, anSrcSize);
