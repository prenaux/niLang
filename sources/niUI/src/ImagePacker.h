...
class cImagePacker : public ImplRC<iUnknown> {
 ...
 public:
  cImagePacker(iGraphics* apGraphics, tU32 anWidth, tU32 anHeight,
  ...
  sRecti __stdcall InsertBitmap(iBitmap2D* apSrc, tBool abFiltered,
                                const int aPadding)
  {
    return _DoInsert(sRecti(0, 0, apSrc->GetWidth(), apSrc->GetHeight()), apSrc,
                     abFiltered, aPadding);
  }
 ...
 private:
  sRecti _DoInsert(const sRecti& aRect, iBitmap2D* apSrc, tBool abFiltered,
                   const int aPadding);
  ...
  bool BoxFit(const sRecti& aRectSrc, const sRecti& aRectDest)
  {
    ...
    if ((aRectSrc.GetWidth() > aRectDest.GetWidth()) ||
        ...
        (aRectSrc.x + aRectSrc.GetWidth() >
         aRectDest.x + aRectDest.GetWidth()) ||
        (aRectSrc.y + aRectSrc.GetHeight() >
         aRectDest.y + aRectDest.GetHeight()))
      return false;
