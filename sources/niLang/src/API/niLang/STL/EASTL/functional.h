...
{
  return compare(a, b) ==
         compare(b, a); // We want the not equal comparison results to be equal.
}
...
struct str_less : public binary_function<T, T, bool> {
  bool operator()(T a, T b) const
  {
    while (
      static_cast<
        typename make_unsigned<typename remove_pointer<T>::type>::type>(*a) ==
      static_cast<
        typename make_unsigned<typename remove_pointer<T>::type>::type>(*b))
    {
      if (*a == 0)
        return (*b != 0);
      ++a;
...
struct equal_to_2 : public binary_function<T, U, bool> {
  ...
  EA_CPP14_CONSTEXPR bool operator()(const U& b, const T& a)
    const // If you are getting a 'operator() already defined' error related to on this line while compiling a
  {
    return b == a;
  } // hashtable class (e.g. hash_map), it's likely that you are using hashtable::find_as when you should
...
class unary_negate
 ...
 public:
  ...
  EA_CPP14_CONSTEXPR bool operator()(
    const typename Predicate::argument_type& a) const
  {
    return !mPredicate(a);
  }
};
...
template <typename Predicate>
inline EA_CPP14_CONSTEXPR unary_negate<Predicate> not1(
...
template <typename Predicate>
inline EA_CPP14_CONSTEXPR binary_negate<Predicate> not2(
  const Predicate& predicate)
{
...
struct unary_compose : public unary_function<typename Operation2::argument_type,
 ...
 public:
  ...
  typename Operation1::result_type operator()(
    typename Operation2::argument_type& x) const
  {
    return op1(op2(x));
  }
};
...
template <typename Result, typename T, typename Argument>
inline mem_fun1_t<Result, T, Argument> mem_fun(
...
template <typename Result, typename T, typename Argument>
inline const_mem_fun1_t<Result, T, Argument> mem_fun(
...
template <typename Result, typename T, typename Argument>
inline mem_fun1_ref_t<Result, T, Argument> mem_fun_ref(
  Result (T::*MemberFunction)(Argument))
{
...
template <typename Result, typename T, typename Argument>
inline const_mem_fun1_ref_t<Result, T, Argument> mem_fun_ref(
  Result (T::*MemberFunction)(Argument) const)
{
...
struct not_fn_ret {
  ...
  template <class... Args>
  auto operator()(
    Args&&... args) & -> decltype(!eastl::
                                    declval<eastl::invoke_result_t<
                                      eastl::decay_t<F>&, Args...>>())
  {
  ...
  auto operator()(Args&&... args)
    const& -> decltype(!eastl::declval<eastl::invoke_result_t<
                         eastl::decay_t<F> const&, Args...>>())
  {
    return !eastl::invoke(mDecayF, eastl::forward<Args>(args)...);
  }
...
template <typename T>
struct hash<
  T*> // Note that we use the pointer as-is and don't divide by sizeof(T*). This is because the table is of a prime size and this division doesn't benefit distribution.
{
  size_t operator()(T* p) const
  {
...
struct hash<char8_t*> {
  size_t operator()(const char8_t* p) const
  {
    uint32_t c,
      result =
        2166136261U; // FNV1 hash. Perhaps the best string hash. Intentionally uint32_t instead of size_t, so the behavior is the same regardless of size.
...
struct hash<const char8_t*> {
  size_t operator()(const char8_t* p) const
  {
    uint32_t c,
      result =
        2166136261U; // Intentionally uint32_t instead of size_t, so the behavior is the same regardless of size.
...
struct mem_hash {
  size_t operator()(const T* mem, size_t size, size_t prev = 2166136261U) const
  {
    uint32_t result = (uint32_t)
      prev; // Intentionally uint32_t instead of size_t, so the behavior is the same
            // regardless of size.
    while (size--)
...
struct mem_hash<uint64_t> {
  size_t operator()(const uint64_t* mem, size_t size,
                    size_t prev = 2166136261U) const
  {
