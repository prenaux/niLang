...
#if !defined(EA_HAVE_EXTENSIONS_FEATURE) && \
  !defined(EA_NO_HAVE_EXTENSIONS_FEATURE)
...
// GCC libstdc++
#if !defined(EA_HAVE_LIBSTDCPP_LIBRARY) && \
  !defined(EA_NO_HAVE_LIBSTDCPP_LIBRARY)
...
// Clang libc++
      ...
      #define EA_HAVE_LIBCPP_LIBRARY \
        1 // We could also #include <ciso646> and check if defined(_LIBCPP_VERSION).
    #endif
  #endif
...
// #include <sys/time.h>
  ...
  #if !defined(EA_PLATFORM_MICROSOFT) && \
    !defined(_CPPLIB_VER)        /* _CPPLIB_VER indicates Dinkumware. */
...
// #include <ptrace.h>
  ...
  #if defined(EA_PLATFORM_UNIX) && !defined(__CYGWIN__) && \
    (defined(EA_PLATFORM_DESKTOP) || defined(EA_PLATFORM_SERVER))
...
// #include <sys/stat.h>
  ...
  #if (defined(EA_PLATFORM_UNIX) &&                                     \
       !(defined(EA_PLATFORM_SONY) && defined(EA_PLATFORM_CONSOLE))) || \
...
// #include <wchar.h>
  ...
  #if defined(EA_PLATFORM_DESKTOP) && defined(EA_PLATFORM_UNIX) && \
    defined(EA_PLATFORM_SONY) && defined(EA_PLATFORM_APPLE)
...
// #include <array>, <forward_list>, <ununordered_set>, <unordered_map>
  ...
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && \
    (_CPPLIB_VER >= 520) // Dinkumware. VS2010+
    ...
    (EA_COMPILER_VERSION >=                                            \
     4004) // Actually GCC 4.3 supports array and unordered_
    #define EA_HAVE_CPP11_CONTAINERS 1
...
// #include <atomic>
  ...
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && \
    (_CPPLIB_VER >= 540) // Dinkumware. VS2012+
...
// #include <condition_variable>
  ...
  !defined(EA_NO_HAVE_CPP11_CONDITION_VARIABLE)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && \
    (_CPPLIB_VER >= 540) // Dinkumware. VS2012+
...
// #include <type_traits>
  ...
  !defined(EA_NO_HAVE_CPP11_TYPE_TRAITS)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && \
    (_CPPLIB_VER >= 540) // Dinkumware. VS2012+
    ...
    defined(EA_HAVE_LIBSTDCPP_LIBRARY) && defined(EA_COMPILER_GNUC) && \
    (EA_COMPILER_VERSION >=                                            \
     4007) // Prior versions of libstdc++ have incomplete support for C++11 type traits.
    #define EA_HAVE_CPP11_TYPE_TRAITS 1
  #elif defined(EA_HAVE_LIBCPP_LIBRARY) && (_LIBCPP_VERSION >= 1)
    #define EA_HAVE_CPP11_TYPE_TRAITS 1
  #else
...
// #include <scoped_allocator>
  ...
  !defined(EA_NO_HAVE_CPP11_SCOPED_ALLOCATOR)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && \
    (_CPPLIB_VER >= 540) // Dinkumware. VS2012+
...
// #include <initializer_list>
  ...
  !defined(EA_NO_HAVE_CPP11_INITIALIZER_LIST)
    ...
    #define EA_HAVE_CPP11_INITIALIZER_LIST 1
  #elif defined(EA_COMPILER_CPP11_ENABLED) &&                        \
    defined(EA_HAVE_LIBCPP_LIBRARY) && defined(EA_COMPILER_CLANG) && \
    (EA_COMPILER_VERSION >= 301) &&                                  \
    !defined(EA_COMPILER_NO_INITIALIZER_LISTS) && !defined(EA_PLATFORM_APPLE)
    #define EA_HAVE_CPP11_INITIALIZER_LIST 1
  #elif defined(EA_COMPILER_CPP11_ENABLED) &&                          \
...
// #include <system_error>
  ...
  !defined(EA_NO_HAVE_CPP11_SYSTEM_ERROR)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && (_CPPLIB_VER >= 520) && \
...
/* EA_HAVE_XXX_DECL */
...
#if !defined(EA_HAVE_gettimeofday_DECL) && \
  !defined(EA_NO_HAVE_gettimeofday_DECL)
  #if defined( \
  ...
  #if !defined(EA_PLATFORM_MICROSOFT)
    #define EA_HAVE_strcasecmp_DECL \
      1 /* This is found as stricmp when not found as strcasecmp */
    ...
    #define EA_HAVE_mmap_DECL \
      1 /* mmap functionality varies significantly between systems. */
  #else
  ...
  #define EA_HAVE_fopen_DECL 1 /* C FILE functionality such as fopen */
    ...
    #define EA_HAVE_ISNAN(x) \
      __builtin_isnan(       \
        x) /* There are a number of standard libraries for Android and it's hard to tell them apart, so just go with builtins */
  ...
  #if (defined(EA_PLATFORM_UNIX) && !defined(EA_PLATFORM_SONY)) || \
...
#if !defined(EA_HAVE_pthread_getattr_np_DECL) && \
  !defined(EA_NO_HAVE_pthread_getattr_np_DECL)
...
// <arpa/inet.h> inet_ntop()
  ...
  #if (defined(EA_PLATFORM_UNIX) || defined(EA_PLATFORM_POSIX)) && \
    !defined(EA_PLATFORM_SONY) && !defined(CS_UNDEFINED_STRING)
    ...
    #define EA_HAVE_inet_pton_IMPL \
      1 /* it identifies strictly the <arpa/inet.h> inet_ntop and inet_pton functions. For example, Microsoft has InetNtop in <Ws2tcpip.h> */
  #else
...
// <time.h> clock_gettime()
  ...
  !defined(EA_NO_HAVE_clock_gettime_IMPL)
  #if defined(EA_PLATFORM_LINUX) || defined(__CYGWIN__) || \
    (defined(_POSIX_TIMERS) && (_POSIX_TIMERS > 0)) ||     \
    (defined(EA_PLATFORM_POSIX) && defined(_CPPLIB_VER) /*Dinkumware*/)
    #define EA_HAVE_clock_gettime_IMPL \
      1 /* You need to link the 'rt' library to get this */
  #else
  ...
  #if (defined(EA_PLATFORM_DESKTOP) || defined(EA_PLATFORM_UNIX)) && \
    !defined(EA_PLATFORM_ANDROID) && !defined(EA_PLATFORM_WINRT)
    #define EA_HAVE_getcwd_IMPL \
      1 /* With Microsoft compilers you may need to use _getcwd, as they have deprecated getcwd. And in any case it's present at <direct.h> */
  #else
...
// EA_HAVE specifically refers to std::nullptr_t from the standard libraries.
    ...
    // VS2010+ with its default Dinkumware standard library.
      ...
      defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY)
      ...
      #if (__GLIBCXX__ >= 20110325) && (__GLIBCXX__ != 20120702) && \
        (__GLIBCXX__ != 20110428)
        #define EA_HAVE_nullptr_t_IMPL 1
      #else
...
// <exception> std::terminate
  ...
  !defined(EA_NO_HAVE_std_terminate_IMPL)
...
// <iterator>: std::begin, std::end, std::prev, std::next, std::move_iterator.
  ...
  !defined(EA_NO_HAVE_CPP11_ITERATOR_IMPL)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && (_CPPLIB_VER >= 520) && \
    !(defined(_HAS_CPP0X) && _HAS_CPP0X) // Dinkumware. VS2010+
...
// <memory>: std::weak_ptr, std::shared_ptr, std::unique_ptr, std::bad_weak_ptr, std::owner_less
  ...
  !defined(EA_NO_HAVE_CPP11_SMART_POINTER_IMPL)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && (_CPPLIB_VER >= 520) && \
    !(defined(_HAS_CPP0X) && _HAS_CPP0X) // Dinkumware. VS2010+
...
// <functional>: std::function, std::mem_fn, std::bad_function_call, std::is_bind_expression, std::is_placeholder, std::reference_wrapper, std::hash, std::bind, std::ref, std::cref.
  ...
  !defined(EA_NO_HAVE_CPP11_FUNCTIONAL_IMPL)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && (_CPPLIB_VER >= 520) && \
    !(defined(_HAS_CPP0X) && _HAS_CPP0X) // Dinkumware. VS2010+
...
// <exception> std::current_exception, std::rethrow_exception, std::exception_ptr, std::make_exception_ptr
  ...
  !defined(EA_NO_HAVE_CPP11_EXCEPTION_IMPL)
  #if defined(EA_HAVE_DINKUMWARE_CPP_LIBRARY) && (_CPPLIB_VER >= 520) && \
    !(defined(_HAS_CPP0X) && _HAS_CPP0X) // Dinkumware. VS2010+
