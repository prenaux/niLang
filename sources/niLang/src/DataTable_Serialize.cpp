...
static tSize _WriteDataTableRaw(iFile* apFile, iDataTable* apTable)
{
  ...
  for (i = 0; i < apTable->GetNumProperties(); ++i) {
    ...
    apFile->WriteStringZ(
      HStringGetStringEmpty(apTable->GetMetadataFromIndex(i)));
  }
  apFile->WriteLE32(apTable->GetNumChildren());

  for (i = 0; i < apTable->GetNumChildren(); ++i) {
    if (_WriteDataTableRaw(apFile, apTable->GetChildFromIndex(i)) ==
        eInvalidHandle)
    {
      niError(niFmt(_A("Can't write child data table '%s' (%d)."),
                    apTable->GetChildFromIndex(i)->GetName(), i));
      return eInvalidHandle;
    }
  }
...
static tSize _ReadDataTableRaw(iFile* apFile, iDataTable* apTable)
{
  ...
  if (ver != knDataTableSerializeVersionRaw) {
    niError(niFmt(_A("Invalid binary data table version '%s', expected '%s'."),
                  niFmtVersion(ver),
                  niFmtVersion(knDataTableSerializeVersionRaw)));
    return eInvalidHandle;
  }
  ...
  for (i = 0; i < nNumChildrenTables; ++i) {
    ...
    if (_ReadDataTableRaw(apFile, ptrDT) == eInvalidHandle) {
      niError(niFmt(_A("Can't read child data table '%s' (%d)."),
                    ptrDT->GetName(), i));
      return eInvalidHandle;
    }
...
static tSize _WriteDataTableRawZip(iFile* apFile, iDataTable* apTable)
{
  ...
  Ptr<iFile> fpZip = ni::GetZip()->CreateFileZipBufferEncoder(
    apFile->GetFileBase(), _knZipCompressionMode);
...
static tSize _ReadDataTableRawZip(iFile* apFile, iDataTable* apTable)
{
  ...
  if (ver != knDataTableSerializeVersionRawZip) {
    niError(niFmt(_A("Invalid binary data table version '%s', expected '%s'."),
                  niFmtVersion(ver),
                  niFmtVersion(knDataTableSerializeVersionRawZip)));
    return eInvalidHandle;
  }
  Ptr<iFile> fpZip =
    ni::GetZip()->CreateFileZipBufferDecoder(apFile->GetFileBase(), 2048);
...
static const sDataTableSerializeVersion _Vers[] = {
  { _A("dtb"), knDataTableSerializeVersionRaw, _WriteDataTableRaw,
    _ReadDataTableRaw },
  { _A("dtz"), knDataTableSerializeVersionRawZip, _WriteDataTableRawZip,
    _ReadDataTableRawZip },
};
...
static const sDataTableSerializeVersion* DataTableSerialize_GetVer(
  const ni::tU32 anVer)
{
  niLoop (i, niCountOf(_Vers)) {
    if (anVer == _Vers[i].ver)
                               ...
                               const tU32 anVersion)
{
  ...
  if (!v) {
    niError(niFmt(_A("Invalid binary data table version '%s'."),
                  niFmtVersion(anVersion)));
    return eInvalidHandle;
  }
  ...
  if (ret == eInvalidHandle) {
    niError(niFmt(_A("Can't write datatable with version '%s'."),
                  niFmtVersion(anVersion)));
    return eInvalidHandle;
  }
  return ret;
...
tSize DataTableSerialize_Read(iFile* apFile, iDataTable* apTable)
{
  ...
  if (!v) {
    niError(
      niFmt(_A("Invalid binary data table version '%s'."), niFmtVersion(ver)));
    return eInvalidHandle;
  }
  tSize ret = v->read(apFile, apTable);
  if (ret == eInvalidHandle) {
    niError(
      niFmt(_A("Can't read datatable with version '%s'."), niFmtVersion(ver)));
    return eInvalidHandle;
  }
  return ret;
}
...
static tSize _XMLWriteString(iFile* apFile, const achar* p)
{
  ...
  for (;;) {
    ...
    if (!c)
      break;
    switch (c) {
    case '<': w += apFile->WriteString(_A("&lt;")); break;
    case '>': w += apFile->WriteString(_A("&gt;")); break;
    case '&': w += apFile->WriteString(_A("&amp;")); break;
    case '"': w += apFile->WriteString(_A("&quot;")); break;
    case '\'': w += apFile->WriteString(_A("&apos;")); break;
    case '\r':
    case '\n': w += apFile->WriteString(niFmt(_A("&#x%X;"), c)); break;
    default: w += apFile->WriteChar(c); break;
    }
  }
  return w;
}
...
static tSize DataTableSerialize_WriteXML(iFile* apFile, iDataTable* apTable,
                                         const int indent)
{
  ...
  if ((apTable->GetNumChildren() == 0) && cdataIndex == eInvalidHandle &&
      innerIndex == eInvalidHandle)
  {
    ...
    niLoop (i, apTable->GetNumChildren()) {
      w += DataTableSerialize_WriteXML(apFile, apTable->GetChildFromIndex(i),
                                       nextIndent);
    }
    if (cdataIndex != eInvalidHandle) {
      ...
      if (hasChildren || propNL)
        w += _XMLWriteIndent(apFile, indent);
      w += _XMLWriteString(apFile,
                           apTable->GetStringFromIndex(innerIndex).Chars());
      if (hasChildren || propNL)
        w += _XMLWriteIndent(apFile, indent);
    }
...
struct sXMLParserSinkDT : public ImplRC<iXmlParserSink> {
  ...
  virtual tBool __stdcall OnXmlParserSink_Node(
    eXmlParserNodeType type, const ni::achar* aNameOrData) niOverride
  {
    switch (type) {
  ...
  virtual tBool __stdcall OnXmlParserSink_Attribute(
    const achar* aName, const achar* aValue) niOverride
  {
    cString val;
    ...
    if ((nameEnd - aName) >= 2 && StrNCmp(nameEnd - 2, _A("__"), 2) == 0) {
      ...
      _dt.top()->SetMetadata(name.Chars(), _H(val.Chars()));
    }
    else {
...
struct sJsonParserSinkDT : public ImplRC<ni::iJsonParserSink> {
  ...
  void __stdcall OnJsonParserSink_Error(const achar* aaszReason, tU32 anLine,
                                        tU32 anCol) override
  {
  ...
  __forceinline void _SetValue(eJsonType aType, const Var&& aValue)
  {
    if (_isArray.top()) {
      switch (aType) {
      case eJsonType_Null: _dt->PushNew("jnull"); break;
      case eJsonType_True:
      case eJsonType_False: _dt->PushNew("jbool"); break;
      case eJsonType_Number: _dt->PushNew("jnum"); break;
      case eJsonType_String: _dt->PushNew("jstr"); break;
      default: _dt->PushNew("junk"); break;
      }
  ...
  void __stdcall OnJsonParserSink_Value(eJsonType aType,
                                        const achar* aValue) override
  {
    switch (aType) {
    case eJsonType_Name: {
      _currentName = aValue;
    ...
    default: {
      niWarning(niFmt("Unknown '%s' property '%s' = '%s'.",
...
static tSize DataTableSerialize_ReadJSON(iFile* apFile,
                                         astl::non_null<iDataTable*> apTable)
{
  Nonnull<sJsonParserSinkDT> jsonParser =
    ni::MakeNonnull<sJsonParserSinkDT>(apTable);
  tI64 pos = apFile->Tell();
...
///////////////////////////////////////////////
tBool __stdcall cLang::SerializeDataTable(const achar* aaszType,
                                          ...
                                          iDataTable* apTable, iFile* apFile)
{
  ...
  if (strFileType.IEq(_A("binary")) || strFileType.IEq(_A("bin"))) {
  ...
  else {
    {
      ...
      tBool isXML = eFalse;
      if (strFileType.IEq(_A("xml")) || strFileType.IEq(_A("xml-raw")) ||
      ...
      if (isXML) {
        if (niFlagIs(aMode, eSerializeFlags_Read)) {
          ...
          if (DataTableSerialize_WriteXML(
                apFile, apTable, strFileType.IEq(_A("xml-stream")) ? -1 : 0) ==
              eInvalidHandle)
          {
...

    {
      ...
      tBool isJSON = eFalse;
      if (strFileType.IEq(_A("json")) || strFileType.IEq(_A("json-raw")) ||
      ...
      if (isJSON) {
        if (niFlagIs(aMode, eSerializeFlags_Read)) {
          if (DataTableSerialize_ReadJSON(apFile, astl::as_non_null(apTable)) ==
              eInvalidHandle)
          {
          ...
          if (!DataTableSerialize_WriteJSON(apFile, apTable,
                                            strFileType.IEq(_A("json-stream"))))
          {
    ...
    niError(
      niFmt(_A("Unknown datatable file type '%s'."), strFileType.Chars()));
    return eFalse;
  }
}
