...
FARPROC WINAPI GetModuleProcAddress(LPCTSTR moduleName, LPCSTR lpProcName)
{
  ...
  #ifndef niMSVC
static BOOL WINAPI GetProcessExePath(DWORD anPID, WCHAR* apDestBuffer,
                                     DWORD anDestBufferSize)
{
  return FALSE;
}
  #else
static BOOL InPlaceTranslateDevicePathToDrivePath(WCHAR* pszFilename,
                                                  DWORD anFileNameMaxSize);
typedef BOOL(WINAPI* tpfnQueryFullProcessImageName)(HANDLE hProcess,
                                                    DWORD dwFlags,
                                                    LPWSTR lpExeName,
                                                    PDWORD lpdwSize);
typedef BOOL(WINAPI* tpfnGetProcessImageFileName)(HANDLE hProcess,
                                     ...
                                     DWORD anDestBufferSize)
{
  ...
  *apDestBuffer = 0;
  ...
  pfnQueryFullProcessImageName =
    (tpfnQueryFullProcessImageName)GetModuleProcAddress(
      _T("kernel32"), "QueryFullProcessImageNameW");
  if (pfnQueryFullProcessImageName) {
    ...
    hProcess =
      OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, anPID);
    ...
    if (!*apDestBuffer) {
      ...
      if (hProcess) {
        ...
        pfnGetProcessImageFileName =
          (tpfnGetProcessImageFileName)GetModuleProcAddress(
            _T("psapi"),
    #ifdef _UNICODE
        ...
        if (pfnGetProcessImageFileName) {
          __try
          {
            pfnGetProcessImageFileName(hProcess, apDestBuffer,
                                       anDestBufferSize);
            InPlaceTranslateDevicePathToDrivePath(apDestBuffer,
                                                  anDestBufferSize);
          } __finally
          {
          }
      ...
      if (::QueryDosDeviceW(szDrive, szName, MAX_PATH)) {
        ...
        if (uNameLen < MAX_PATH) {
          ...
          if (bFound) {
            ...
            StringCchPrintfW(szTempFile, _MAX_PATH, L"%s%s", szDrive,
                             pszFilename + uNameLen);
            StringCchCopyNW(pszFilename, anFileNameMaxSize, szTempFile,
                            wcslen(szTempFile));
          }
        }
      }
...
{
  return OpenProcess(SYNCHRONIZE | PROCESS_DUP_HANDLE | PROCESS_TERMINATE |
...
// Helper for GetProcId()
static bool _GetProcIdViaNtQueryInformationProcess(base::ProcessHandle process,
                                                   DWORD* id)
{
               ...
               ni::tOSProcessSpawnFlags aSpawnFlags)
{
  ...
  if (niFlagIs(aSpawnFlags, ni::eOSProcessSpawnFlags_SandBox)) {
    ...
    if (!::CreateProcessAsUserW(
          hNewToken, NULL, wCmdLine.begin(), NULL, NULL, FALSE, createFlags,
          (LPVOID)apEnv, niStringIsOK(aaszWorkDir) ? wWorkDir.begin() : NULL,
...
Cleanup:
  ...
  if (!process_info.hProcess) {
    if (!::CreateProcessW(NULL, // application name
                          ...
                          niStringIsOK(aaszWorkDir) ? wWorkDir.begin()
...
bool DidProcessCrash(ProcessHandle handle)
{
  ...
  if (exitcode == PROCESS_END_NORMAL_TERMINATON ||
      ...
      exitcode == 0x40010004)
  { // Debugger terminated process/end session.
    return false;
  }
...
bool NamedProcessIterator::IncludeEntry()
{
  ...
  return (executable_name_.empty() ||
         ...
         (!filter_ ||
          filter_->Includes(entry_.th32ProcessID, entry_.th32ParentProcessID));
}
...
ni::tIntPtr GetParentProcessFromPid(ni::tIntPtr dwPID)
{
  ...
  pfnNtQueryInformationProcess =
    (tpfnNtQueryInformationProcess)GetModuleProcAddress(
  ...
  ULONG ulRetLen = 0;
  NTSTATUS ntStatus =
    pfnNtQueryInformationProcess(hProcess, ProcessBasicInformation, (void*)&pbi,
                            ...
                            int wait_milliseconds, const ProcessFilter* filter)
{
  ...
  for (;;) {
    ...
    if (!entry)
      break;
    DWORD remaining_wait = eastl::max(
      0, wait_milliseconds - static_cast<int>(GetTickCount() - start_time));
...
{
  bool retval =
    ::WaitForSingleObject(handle, wait_milliseconds) == WAIT_OBJECT_0;
  return retval;
}
