...
struct blender_gray {
  ...
  static AGG_INLINE void blend_pix(value_type* p, unsigned cv, unsigned alpha,
                                   unsigned cover = 0)
  {
    *p = (value_type)((((cv - calc_type(*p)) * alpha) +
                       (calc_type(*p) << base_shift)) >>
                      base_shift);
  }
};
...
struct blender_gray_pre {
  ...
  {
    *p =
      (value_type)(((*p * (color_type::base_mask - alpha)) >> base_shift) + cv);
  }
};
...
class pixfmt_alpha_blend_gray {
 ...
 public:
  ...
  {
    copy_or_blend_pix((value_type*)m_rbuf->row_ptr(x, y, 1) + x * Step + Offset,
                      c, cover);
  }
  ...
  void blend_hline(int x, int y, unsigned len, const color_type& c, int8u cover)
  {
    if (c.a) {
      value_type* p =
        (value_type*)m_rbuf->row_ptr(x, y, len) + x * Step + Offset;
  ...
  void for_each_pixel(Function f)
  {
    ...
    for (y = 0; y < height(); ++y) {
      ...
      if (r.ptr) {
        ...
        value_type* p =
          (value_type*)m_rbuf->row_ptr(r.x1, y, len) + r.x1 * Step + Offset;
...
typedef pixfmt_alpha_blend_gray<blender_gray16_pre, rendering_buffer>
  pixfmt_gray16_pre; //----pixfmt_gray16_pre
} // namespace agg
