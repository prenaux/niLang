...
class cDataTableReadStack
    : public ni::ImplRC<ni::iDataTableReadStack, ni::eImplFlags_Default> {
 ...
 public:
  ...
  tBool __stdcall PushEx(const achar* aaszName, const achar* aaszProp,
                         const achar* aaszVal);
  tBool __stdcall PushFailEx(const achar* aaszName, const achar* aaszProp,
                             const achar* aaszVal);
  tBool __stdcall Pop();
  tBool __stdcall RemoveProperty(const achar* aaszName);
  cString __stdcall GetString(const achar* aaszName) const;
  cString __stdcall GetStringDefault(const achar* aaszName,
                                     const achar* v) const;
  Ptr<iHString> __stdcall GetHString(const achar* aaszName) const;
  Ptr<iHString> __stdcall GetHStringDefault(const achar* aaszName,
                                            iHString* v) const;
  ...
  sMatrixf __stdcall GetMatrixDefault(const achar* aaszName,
                                      const sMatrixf& v) const;
  iUnknown* __stdcall GetIUnknown(const achar* aaszName) const;
  iUnknown* __stdcall GetIUnknownDefault(const achar* aaszName,
                                         iHString* v) const;
  tU32 __stdcall GetEnum(const achar* aaszName, const sEnumDef* apEnumDef,
                         tEnumToStringFlags aFlags) const;
  tU32 __stdcall GetEnumDefault(const achar* aaszName,
                                const sEnumDef* apEnumDef,
                                tEnumToStringFlags aFlags, tU32 v) const;
