...
class renderer_raster_htext_solid {
 public:
  ...
  void render_text(agg_real x, agg_real y, const CharT* str, bool flip = false)
  {
    glyph_rect r;
    while (*str) {
      ...
      if (r.x2 >= r.x1) {
        ...
        if (flip) {
          for (i = r.y1; i <= r.y2; i++) {
            m_ren->blend_solid_hspan(r.x1, i, (r.x2 - r.x1 + 1), m_color,
                                     m_glyph->span(r.y2 - i));
          }
        }
        else {
  ...
  void render_text(agg_real x, agg_real y, const CharT* str, bool flip = false)
  {
    ...
    while (*str) {
      ...
      if (r.x2 >= r.x1) {
        ...
        if (flip) {
          for (i = r.y1; i <= r.y2; i++) {
            m_ren->blend_solid_vspan(i, r.x1, (r.x2 - r.x1 + 1), m_color,
                                     m_glyph->span(i - r.y1));
          }
        }
        else {
  ...
  void render_text(agg_real x, agg_real y, const CharT* str, bool flip = false)
  {
    ...
    while (*str) {
      ...
      if (r.x2 >= r.x1) {
        ...
        if (flip) {
          for (i = r.y1; i <= r.y2; i++) {
            m_ren->render(scanline_single_span(r.x1, i, (r.x2 - r.x1 + 1),
                                               m_glyph->span(r.y2 - i)));
          }
        }
        else {
