...
#if defined __JSCC__ || defined ANDROID || defined niIOS || defined niQNX
...
#  define CTX_GL(FN) ctx->FN
#  define CTX_HAVE_OPENGL_2 ctx->have_opengl_2
...
#  include <GL/gl.h>
#  include "../../../niUI/src/GL/ni_glext.h"
...
struct MOJOSHADER_glProgram
{
  ...
  ni::tI32 vertex_attrib_loc[MOJOSHADER_USAGE_TOTAL][16];
};

#if !defined USE_GL_LIB

#  ifndef APIENTRYP
#    define APIENTRYP KHRONOS_APIENTRY
#  endif
...
{
  snprintf(error_buffer, sizeof (error_buffer), "MojoError: %s", str);
  niError(str);
} // set_error
...
{
  if (val)
    CTX_GL(glEnable)(state);
  else
    CTX_GL(glDisable)(state);
} // toggle_gl_state
...
static int impl_GLSL_MaxUniforms(MOJOSHADER_shaderType shader_type)
{
  ...
  CTX_GL(glGetIntegerv)(pname, &val);
  return (int) val;
} // impl_GLSL_MaxUniforms


static int impl_GLSL_CompileShader(const MOJOSHADER_parseData *shaderPD, GLuint *s)
{
  GLint ok = 0;
  const GLenum shader_type = glsl_shader_type(shaderPD->shader_type);

  MOJOSHADER_TRACE_PREPROCESS(("--- MOJOSHADER TO PREPROCESS ---\n%s",shaderPD->output));
  ...
  const char* code = preprocessed.Chars();
  const GLint codelen = (GLint)preprocessed.size();
  MOJOSHADER_TRACE_COMPILE(("--- MOJOSHADER TO COMPILE ---\n%s",preprocessed));

#  if !defined USE_GL_LIB
  if (ctx->have_opengl_2)
#  endif
  {
    const GLuint shader = CTX_GL(glCreateShader)(shader_type);
    CTX_GL(glShaderSource)(shader, 1, (const GLchar**) &code, &codelen);

    CTX_GL(glCompileShader)(shader);
    CTX_GL(glGetShaderiv)(shader, GL_COMPILE_STATUS, &ok);
    if (!ok)
    {
      GLsizei len = 0;
      CTX_GL(glGetInfoLogARB)(shader, sizeof (error_buffer), &len,
                              (GLchar *) error_buffer);
      CTX_GL(glDeleteShader)(shader);
      *s = 0;
      return 0;
    } // if
...
#  if !defined USE_GL_LIB
  ...
  {
    const GLhandleARB shader = CTX_GL(glCreateShaderObjectARB)(shader_type);
    assert(sizeof (shader) == sizeof (*s));  // not always true on OS X!
    CTX_GL(glShaderSourceARB)(shader, 1,
                              (const GLcharARB **) &code, &codelen);

    CTX_GL(glCompileShaderARB)(shader);
    CTX_GL(glGetObjectParameterivARB)(shader,GL_OBJECT_COMPILE_STATUS_ARB,&ok);
    if (!ok)
    {
      GLsizei len = 0;
      CTX_GL(glGetInfoLogARB)(shader, sizeof (error_buffer), &len,
                              (GLcharARB *) error_buffer);
      CTX_GL(glDeleteObjectARB)(shader);
      *s = 0;
      return 0;
    } // if
...
#  if !defined USE_GL_LIB
    ...
    CTX_GL(glDeleteObjectARB)((GLhandleARB) shader);
#  endif
...
#  if !defined USE_GL_LIB
  if (ctx->have_opengl_2)
#  endif
    CTX_GL(glDeleteProgram)(program);
#  if !defined USE_GL_LIB
  else
    CTX_GL(glDeleteObjectARB)((GLhandleARB) program);
#  endif
...
#  if !defined USE_GL_LIB
      ctx->have_opengl_2 ?
#  endif
      CTX_GL(glGetUniformLocation)(program->handle, name)
#  if !defined USE_GL_LIB
      : CTX_GL(glGetUniformLocationARB)((GLhandleARB) program->handle, name)
#  endif
      ;
} // glsl_uniform_loc
...
#  endif
  {
    return CTX_GL(glGetAttribLocation)(program->handle,
                                       (const GLchar *) a[idx].name);
  } // if

#  if !defined USE_GL_LIB
  return CTX_GL(glGetAttribLocationARB)((GLhandleARB) program->handle,
...
#  endif
  {
    const GLuint program = CTX_GL(glCreateProgram)();

    if (vshader != NULL) CTX_GL(glAttachShader)(program, vshader->handle);
    if (pshader != NULL) CTX_GL(glAttachShader)(program, pshader->handle);

    CTX_GL(glLinkProgram)(program);

    CTX_GL(glGetProgramiv)(program, GL_LINK_STATUS, &ok);
    if (!ok)
    {
      GLsizei len = 0;
      CTX_GL(glGetProgramInfoLog)(program, sizeof (error_buffer),
...
#  if !defined USE_GL_LIB
  ...
  {
    const GLhandleARB program = CTX_GL(glCreateProgramObjectARB)();
    assert(sizeof(program) == sizeof(GLuint));  // not always true on OS X!

    if (vshader != NULL)
      CTX_GL(glAttachObjectARB)(program, (GLhandleARB) vshader->handle);

    if (pshader != NULL)
      CTX_GL(glAttachObjectARB)(program, (GLhandleARB) pshader->handle);

    CTX_GL(glLinkProgramARB)(program);

    CTX_GL(glGetObjectParameterivARB)(program, GL_OBJECT_LINK_STATUS_ARB, &ok);
    if (!ok)
    {
      GLsizei len = 0;
      CTX_GL(glGetInfoLogARB)(program, sizeof (error_buffer),
                              &len, (GLcharARB *) error_buffer);
      CTX_GL(glDeleteObjectARB)(program);
      return 0;
    } // if
...
#  if !defined USE_GL_LIB
  if (ctx->have_opengl_2)
#  endif
    CTX_GL(glUseProgram)(program ? program->handle : 0);
#  if !defined USE_GL_LIB
  else
    CTX_GL(glUseProgramObjectARB)((GLhandleARB) (program ? program->handle : 0));
#  endif
                                        ...
                                        const GLfloat *f)
{
  ...
  if (loc >= 0)   // not optimized out?
    CTX_GL(glUniform4fv)(loc, u->array_count, f);
...
static void impl_GLSL_PushUniforms(void)
{
  ...
  {
    CTX_GL(glUniform4fv)(program->vs_float4_loc,
  ...
  {
    CTX_GL(glUniform4iv)(program->vs_int4_loc,
  ...
  {
    CTX_GL(glUniform1iv)(program->vs_bool_loc,
  ...
  {
    CTX_GL(glUniform4fv)(program->ps_float4_loc,
  ...
  {
    CTX_GL(glUniform4iv)(program->ps_int4_loc,
  ...
  {
    CTX_GL(glUniform1iv)(program->ps_bool_loc,
...
{
  CTX_GL(glUniform1i)(loc, sampler);
...
static int impl_ARB1_MaxUniforms(MOJOSHADER_shaderType shader_type)
{
  ...
  CTX_GL(glGetProgramivARB)(program_type, GL_MAX_PROGRAM_PARAMETERS_ARB, &retval);
...
static int impl_ARB1_CompileShader(const MOJOSHADER_parseData *pd, GLuint *s)
{
  ...
  GLuint shader = 0;
  CTX_GL(glGenProgramsARB)(1, &shader);

  CTX_GL(glGetError)();  // flush any existing error state.
  CTX_GL(glBindProgramARB)(shader_type, shader);
  CTX_GL(glProgramStringARB)(shader_type, GL_PROGRAM_FORMAT_ASCII_ARB,
                             shaderlen, pd->output);

  if (CTX_GL(glGetError)() == GL_INVALID_OPERATION)
  {
    GLint pos = 0;
    CTX_GL(glGetIntegerv)(GL_PROGRAM_ERROR_POSITION_ARB, &pos);
    const GLubyte *errstr = CTX_GL(glGetString)(GL_PROGRAM_ERROR_STRING_ARB);
    snprintf(error_buffer, sizeof (error_buffer),
             "ARB1 compile error at position %d: %s",
             (int) pos, (const char *) errstr);
    CTX_GL(glBindProgramARB)(shader_type, 0);
    CTX_GL(glDeleteProgramsARB)(1, &shader);
    *s = 0;
    return 0;
  } // if
...
static void impl_ARB1_UseProgram(MOJOSHADER_glProgram *program)
{
  ...
  CTX_GL(glBindProgramARB)(GL_VERTEX_PROGRAM_ARB, vhandle);
  CTX_GL(glBindProgramARB)(GL_FRAGMENT_PROGRAM_ARB, phandle);
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if (type == MOJOSHADER_UNIFORM_FLOAT)
    {
      ...
      for (i = 0; i < size; i++, srcf += 4, loc++)
        CTX_GL(glProgramLocalParameter4fvARB)(arb_shader_type, loc, srcf);
    } // if
    else if (type == MOJOSHADER_UNIFORM_INT)
    {
      ...
      if (ctx->have_GL_NV_gpu_program4)
      {
        ...
        for (i = 0; i < size; i++, srci += 4, loc++)
          CTX_GL(glProgramLocalParameterI4ivNV)(arb_shader_type, loc, srci);
      } // if
      else
#endif
      {
        for (i = 0; i < size; i++, srci += 4, loc++)
        {
          ...
          CTX_GL(glProgramLocalParameter4fvARB)(arb_shader_type, loc, fv);
        } // for
      } // else
    } // else if
    else if (type == MOJOSHADER_UNIFORM_BOOL)
    {
      ...
      if (ctx->have_GL_NV_gpu_program4)
      {
        ...
        for (i = 0; i < size; i++, srcb++, loc++)
        {
          ...
          CTX_GL(glProgramLocalParameterI4ivNV)(arb_shader_type, loc, iv);
        } // for
      } // if
      else
#endif
...
#  endif
  {
    ...
    CTX_GL(glGetError)();  // flush any existing error state.
    const char *str = (const char *) CTX_GL(glGetString)(enumval);
    if (CTX_GL(glGetError)() == GL_INVALID_ENUM)
      str = NULL;
#  ifdef USE_GLES2_LIB
    niUnused(str);
    ctx->glsl_major = 1;
...
static void load_extensions(MOJOSHADER_glGetProcAddress lookup, void *d)
{
...

  {
    ...
    ctx->opengl_minor = 0;
#else
    const char *str = (const char *) CTX_GL(glGetString)(GL_VERSION);
    parse_opengl_version_str(str, &ctx->opengl_major, &ctx->opengl_minor);
#endif
    extlist = (const char *) CTX_GL(glGetString)(GL_EXTENSIONS);
  } // else
...
#define MUST_HAVE_GLSL(p, maj, min)                                 \
  ...
  {
    MUST_HAVE_GLSL(MOJOSHADER_PROFILE_GLSL, 1, 0);
  } // else if
#endif
...
static inline GLenum opengl_attr_type(const MOJOSHADER_attributeType type)
{
  switch (type) {
    ...
    case MOJOSHADER_ATTRIBUTE_UNSIGNED_BYTE: return GL_UNSIGNED_BYTE;
    case MOJOSHADER_ATTRIBUTE_SHORT: return GL_SHORT;
    case MOJOSHADER_ATTRIBUTE_UNSIGNED_SHORT: return GL_UNSIGNED_SHORT;
    case MOJOSHADER_ATTRIBUTE_INT: return GL_INT;
    case MOJOSHADER_ATTRIBUTE_UNSIGNED_INT: return GL_UNSIGNED_INT;
...
void MOJOSHADER_glPushUniforms(void) {
  MOJOSHADER_glProgram *program = ctx->bound_program;
  if (program == NULL)
    return;  // nothing to do.
