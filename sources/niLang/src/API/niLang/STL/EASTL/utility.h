...
#include "internal/piecewise_construct_t.h"
  ...
  #pragma warning( \
      disable : 4217) // Member template functions cannot be used for copy-assignment or copy-construction.
  #pragma warning( \
      disable : 4512) // 'class' : assignment operator could not be generated.  // This disabling would best be put elsewhere.
#endif
...
inline void swap(T& a, T& b)
  EA_NOEXCEPT_IF(eastl::is_nothrow_move_constructible<T>::value&&
                   eastl::is_nothrow_move_assignable<T>::value)
{
  T temp(EASTL_MOVE(
    a)); // EASTL_MOVE uses EASTL::move when available, else is a no-op.
  a = EASTL_MOVE(b);
  b = EASTL_MOVE(temp);
}
...
template <typename>
struct is_swappable : public eastl::false_type {};
#else
...
struct is_swappable
    : public integral_constant<
        bool, !eastl::is_same<decltype(swap(eastl::declval<T&>(),
                                            eastl::declval<T&>())),
                              eastl::unused>::value> {
...
struct is_nothrow_swappable_helper_noexcept_wrapper {
  const static bool value =
    noexcept(swap(eastl::declval<T&>(), eastl::declval<T&>()));
};
...
template <typename T>
struct is_nothrow_swappable_helper<T, false> : public eastl::false_type {};

template <typename T>
struct is_nothrow_swappable
    : public eastl::is_nothrow_swappable_helper<T,
...
template <typename T, typename U,
          bool OneTypeIsVoid =
            (eastl::is_void<T>::value || eastl::is_void<U>::value)>
struct is_swappable_with_helper {
  // Don't prefix swap with eastl:: as we want to allow user-defined swaps via argument-dependent lookup.
  static const bool value =
    !eastl::is_same<decltype(swap(eastl::declval<T>(), eastl::declval<U>())),
                    eastl::unused>::value &&
    !eastl::is_same<decltype(swap(eastl::declval<U>(), eastl::declval<T>())),
                    eastl::unused>::value;
};
...
struct is_nothrow_swappable_with_helper {
  // Don't prefix swap with eastl:: as we want to allow user-defined swaps via argument-dependent lookup.
  static const bool value =
    noexcept(swap(eastl::declval<T>(), eastl::declval<U>()))&& noexcept(
      swap(eastl::declval<U>(), eastl::declval<T>()));
};
...
struct is_nothrow_swappable_with
    : public eastl::bool_constant<
        is_nothrow_swappable_with_helper<T, U>::value> {};
...
struct iter_swap_impl<true> {
  ...
  static void iter_swap(ForwardIterator1 a, ForwardIterator2 b)
  {
    swap(
      *a,
      *b); // Don't prefix swap with eastl:: as we want to allow user-defined swaps via argument-dependent lookup.
  }
};
...
{
  typedef
    typename eastl::iterator_traits<ForwardIterator1>::value_type value_type_a;
  typedef
    typename eastl::iterator_traits<ForwardIterator2>::value_type value_type_b;
  typedef
    typename eastl::iterator_traits<ForwardIterator1>::reference reference_a;
  typedef
    typename eastl::iterator_traits<ForwardIterator2>::reference reference_b;

  eastl::iter_swap_impl<eastl::type_and<
    eastl::is_same<value_type_a, value_type_b>::value,
    eastl::is_same<value_type_a&, reference_a>::value,
    eastl::is_same<value_type_b&, reference_b>::value>::value>::iter_swap(a, b);
}
                                    ...
                                    ForwardIterator2 first2)
{
  for (; first1 != last1; ++first1, ++first2)
    iter_swap(
      first1,
      first2); // Don't prefix swap with eastl:: as we want to allow user-defined swaps via argument-dependent lookup.
  return first2;
}
...
inline void swap(T (&a)[N], T (&b)[N])
  EA_NOEXCEPT_IF(eastl::is_nothrow_swappable<T>::value)
{
...
struct pair {
  ...
  template <typename TT2 = T2, typename = eastl::enable_if_t<
                                 eastl::is_default_constructible_v<TT2>>>
...
// https://bugs.llvm.org/show_bug.cgi?id=38374
  ...
  using single_pair_ctor_sfinae =
    eastl::enable_if_t<eastl::is_default_constructible_v<T>>;
#else
  template <typename>
...
// error: with 'eastl::pair<T1, T2>::pair(const T1&) [with T1 = const int&; T2 = const int&]'
  ...
  template <typename TT1 = T1, typename TT2 = T2,
            class = eastl::enable_if_t<eastl::is_copy_constructible_v<TT1> &&
                                       eastl::is_copy_constructible_v<TT2>>>
  ...
  template <typename U, typename V,
            class = eastl::enable_if_t<eastl::is_convertible_v<const U&, T1> &&
                                       eastl::is_convertible_v<const V&, T2>>>
  ...
  template <typename U, typename V,
            typename = eastl::enable_if_t<eastl::is_convertible_v<U, T1> &&
                                          eastl::is_convertible_v<V, T2>>>
  ...
  template <typename V,
            typename = eastl::enable_if_t<eastl::is_convertible_v<V, T2>>>
  ...
  template <class... Args1, class... Args2,
            typename = eastl::enable_if_t<
              eastl::is_constructible_v<first_type, Args1&&...> &&
              eastl::is_constructible_v<second_type, Args2&&...>>>
  pair(eastl::piecewise_construct_t pwc, eastl::tuple<Args1...> first_args,
       eastl::tuple<Args2...> second_args)
      : pair(pwc, eastl::move(first_args), eastl::move(second_args),
 ...
 private:
  ...
  pair(eastl::piecewise_construct_t, eastl::tuple<Args1...> first_args,
       eastl::tuple<Args2...> second_args, eastl::index_sequence<I1...>,
 ...
 public:
  pair& operator=(const pair& p)
    EA_NOEXCEPT_IF(eastl::is_nothrow_copy_assignable_v<T1>&&
                     eastl::is_nothrow_copy_assignable_v<T2>)
  {
    first = p.first;
    second = p.second;
    return *this;
  ...
  pair& operator=(pair&& p)
    EA_NOEXCEPT_IF(eastl::is_nothrow_move_assignable_v<T1>&&
                     eastl::is_nothrow_move_assignable_v<T2>)
  {
  ...
  void swap(pair& p) EA_NOEXCEPT_IF(
    eastl::is_nothrow_swappable_v<T1>&& eastl::is_nothrow_swappable_v<T2>)
  {
...
template <typename T>
struct
  use_self // : public unary_function<T, T> // Perhaps we want to make it a subclass of unary_function.
{
...
template <typename Pair>
struct
  use_second // : public unary_function<Pair, typename Pair::second_type> // Perhaps we want to make it a subclass of unary_function.
{
  typedef Pair argument_type;
...
///
template <typename T1, typename T2>
EA_CPP14_CONSTEXPR inline pair<typename eastl::remove_reference_wrapper<
                                 typename eastl::decay<T1>::type>::type,
                               typename eastl::remove_reference_wrapper<
                                 typename eastl::decay<T2>::type>::type>
make_pair(T1&& a, T2&& b)
{
  typedef typename eastl::remove_reference_wrapper<
    typename eastl::decay<T1>::type>::type T1Type;
  typedef typename eastl::remove_reference_wrapper<
    typename eastl::decay<T2>::type>::type T2Type;

  return eastl::pair<T1Type, T2Type>(eastl::forward<T1>(a),
                                     eastl::forward<T2>(b));
}
...
EA_CPP14_CONSTEXPR inline pair<T1, T2> make_pair(
  ...
  typename eastl::enable_if<!eastl::is_array<T1>::value &&
                            !eastl::is_array<T2>::value>::type* = 0)
{
