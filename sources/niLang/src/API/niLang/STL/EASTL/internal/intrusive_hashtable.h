...
struct intrusive_hashtable_iterator_base {
 ...
 protected:
  ...
  template <typename V>
  friend bool operator==(const intrusive_hashtable_iterator_base<V>&,
                         const intrusive_hashtable_iterator_base<V>&);
...
///
template <typename Value, bool bConst>
struct intrusive_hashtable_iterator
    : public intrusive_hashtable_iterator_base<Value> {
 public:
...
struct
  use_intrusive_key // : public unary_function<T, T> // Perhaps we want to make it a subclass of unary_function.
{
...
class intrusive_hashtable {
 public:
  typedef intrusive_hashtable<Key, Value, Hash, Equal, bucketCount,
                              bConstIterators, bUniqueKeys>
    this_type;
  typedef Key key_type;
  typedef Value value_type;
  ...
  typedef
    typename type_select<bUniqueKeys, pair<iterator, bool>, iterator>::type
      insert_return_type;
  typedef typename type_select<bConstIterators, eastl::use_self<Value>,
                               eastl::use_intrusive_key<Value, key_type>>::type
    extract_key;
 ...
 protected:
  node_type* mBucketArray
    [kBucketCount +
     1]; // '+1' because we have an end bucket which is non-NULL so iterators always stop on it.
  size_type mnElementCount;
  Hash
    mHash; // To do: Use base class optimization to make this go away when it is of zero size.
  Equal
    mEqual; // To do: Use base class optimization to make this go away when it is of zero size.
 ...
 public:
  ...
  size_type bucket_count() const
    EA_NOEXCEPT // This function is unnecessary, as the user can directly reference
  {
    return kBucketCount;
 ...
 public:
  ...
  iterator remove(
    value_type&
      value); // Removes by value instead of by iterator. This is an O(1) operation, due to this hashtable being 'intrusive'.
 ...
 protected:
  eastl::pair<iterator, bool> DoInsertValue(
    value_type&, true_type); // true_type means bUniqueKeys is true.
  iterator DoInsertValue(value_type&,
                         false_type); // false_type means bUniqueKeys is false.

  node_type* DoFindNode(node_type* pNode, const key_type& k) const;

  template <typename U, typename BinaryPredicate>
  node_type* DoFindNode(node_type* pNode, const U& u,
                        BinaryPredicate predicate) const;
...
template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM,
          bool bU>
inline intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::intrusive_hashtable(
  const H& h, const Eq& eq)
    : mnElementCount(0)
    , mHash(h)
    , mEqual(eq)
{
...
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::find(const key_type& k)
{
  ...
  return pNode ? iterator(pNode, mBucketArray + n)
...
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::find(const key_type& k) const
{
  ...
  node_type* const pNode = DoFindNode(mBucketArray[n], k);
  return pNode
           ? const_iterator(pNode, const_cast<node_type**>(mBucketArray) + n)
           : const_iterator(const_cast<node_type**>(mBucketArray) +
                            kBucketCount);
}
...
template <typename U, typename UHash, typename BinaryPredicate>
inline typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::const_iterator
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::find_as(
...
template <typename H, typename U>
inline typename H::const_iterator intrusive_hashtable_find(const H& hashTable,
                                                           const U& u)
{
  return hashTable.find_as(u, eastl::hash<U>(),
                           eastl::equal_to_2<const typename H::key_type, U>());
...
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::count(const key_type& k) const
{
  ...
  for (node_type* pNode = mBucketArray[n]; pNode;
...
eastl::pair<
  ...
  typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::const_iterator>
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::equal_range(
  const key_type& k) const
{
  const size_type n = (size_type)(mHash(k) % kBucketCount);
...
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::DoFindNode(
  node_type* pNode, const key_type& k) const
{
  extract_key extractKey; // extract_key is empty and thus this ctor is a no-op.
...
eastl::pair<typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::iterator,
            bool>
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::DoInsertValue(
  value_type& value, true_type) // true_type means bUniqueKeys is true.
{
  ...
  if (pNode == NULL) {
    ...
    return eastl::pair<iterator, bool>(iterator(&value, mBucketArray + n),
                                       true);
  }
...
typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::iterator
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::DoInsertValue(
  value_type& value, false_type) // false_type means bUniqueKeys is false.
{
  extract_key extractKey; // extract_key is empty and thus this ctor is a no-op.
  const size_type n = (size_type)(mHash(extractKey(value)) % kBucketCount);
...
template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM,
          bool bU>
template <typename InputIterator>
inline void intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::insert(
...
template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM,
          bool bU>
inline typename intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::iterator
intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::erase(const_iterator first,
                                                    const_iterator last)
{
  while (first != last)
    first = erase(first);
...
template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM,
          bool bU>
int intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::validate_iterator(
