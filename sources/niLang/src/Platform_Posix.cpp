...
static ni::tBool _find_ci_path(ni::achar* r, const ni::achar* path,
                               const ni::achar* context)
{
  // return ni::eFalse;
  static int _count = 0;
  _TraceUnixCiPath(("Looking for case insensitive path %d (%s) '%s'.", _count++,
                    context, path));
...
struct sCIPath {
  ...
  ni::tBool ResolvePath(ni::achar* r, const ni::achar* path,
                        const ni::achar* context)
  {
...
niExportFunc(int) unix_findfirst(const ni::achar* pattern, sUnixFFBlk* aFFBlk)
{
  ...
  *unix_ff_get_filename(ffinfo.dirname) = 0;
  ...
  ni::StrZCpy(ffinfo.pattern, niCountOf(ffinfo.pattern) - 1,
  ...
  if (ffinfo.dir == 0) {
    ...
    if (_GetCIPath().ResolvePath(ffinfo.dirname, ffinfo.dirname,
                                 "unix_findfirst"))
    {
      TRACE_UNIX_FF(
        ("... Try opendir after unix_casepath: '%s'", ffinfo.dirname));
      ffinfo.dir = opendir(ffinfo.dirname);
    }
  #endif
    if (ffinfo.dir == 0) {
...
niExportFunc(ni::tBool) unix_filetime(const char* filename, time_t* apATime,
                                      time_t* apMTime, time_t* apCTime)
{
  #ifdef USE_READDIR64
...
niExportFunc(int) unix_findnext(sUnixFFBlk* aFFBlk)
{
  ...
  while (1) {
    ...
    ni::StrZCat(ffinfo.filename,
                niCountOf(ffinfo.filename) - ni::StrSize(ffinfo.filename) - 1,
                tempName.Chars());
    ...
    if ((tempName[0] == '.') && ((tempName[1] != '.') || (tempName[2] != 0)))
...
niExportFunc(FILE*) afopen(const ni::achar* file, const ni::achar* mode,
...
niExportFunc(int) FdOpen(const ni::achar* path, ni::cString* apPathOnDisk,
                         int mode, int pmode)
{
