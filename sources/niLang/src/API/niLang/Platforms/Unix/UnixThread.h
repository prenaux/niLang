...
typedef struct _EVENT_OBJECT {
  ...
  _EVENT_OBJECT(bool _is_set, bool _is_manual_reset)
      : is_set(_is_set)
      , is_pulse(false)
      , is_manual_reset(_is_manual_reset)
  {
...
inline DWORD WaitForSingleObject(HANDLE hObject, DWORD dwTime)
{
  ...
  printf("Unix::WaitForSingleObject for %p: %d, %d\n", hObject,
         lpObject->is_set, lpObject->is_pulse);
  fflush(stdout);
#endif
...
tryagain:
      retval =
        pthread_cond_timedwait(&lpObject->cond, &lpObject->lock, &abstime);
      switch (retval) {
      case EINTR: goto tryagain; break;
      case ETIMEDOUT:
        // time out
        retval = WAIT_TIMEOUT;
        break;
