...
TheoraVideoClip_AVFoundation::TheoraVideoClip_AVFoundation(ni::iFile* data_source,
...
bool TheoraVideoClip_AVFoundation::decodeNextFrame()
{
	...
	if (status == AVAssetReaderStatusFailed)
	{
		...
		niWarning("AVAssetReader reading failed, restarting...");
		...
		if (status == AVAssetReaderStatusFailed)
		{
			niWarning("AVAssetReader restart failed!");
			return 0;
		}
		niWarning("AVAssetReader restart succeeded!");
	}
	...
	if (status == AVAssetReaderStatusReading)
	{
		...
		while ((sampleBuffer = [mOutput copyNextSampleBuffer]))
		{
			...
			if (frame->mTimeToDisplay < mTimer->getTime() && !mRestarted && mFrameNumber % 16 != 0)
			{
				...
				niWarning(mName + ": pre-dropped frame " + str(mFrameNumber - 1));
#endif
				++mNumDisplayedFrames;
				++mNumDroppedFrames;
    ...
    unload();
    mEndOfFile = true;
    // niWarning(mName + " finished playing");
		return 0;
	}
...
bool TheoraVideoClip_AVFoundation::load(ni::iFile* apSource)
{
	mStream = source;
	mFrameNumber = 0;
	mEndOfFile = false;
	astl::string filename = apSource->GetSourcePath();
	...
	if ([tracks count] == 0) {
		niError(niFmt("AVFoundation Unable to open video file: %s.", apSource->GetSourcePath()));
    return false;
	}
	...
	else if (!mLoaded)
	{
		...
		niWarning("duration: " + strf(mDuration) + " seconds\n-----");
	}
#endif
...
{
#if _DEBUG
	niWarning(mName + " [seek]: seeking to frame " + str(mSeekFrame));
#endif
	int frame;
