...
float calculate_average_cache_miss_ratio(
  const astl::vector<unsigned int>& tri_indices, unsigned int cache_size = 14);
/// reorder the triangles, trying to minimize number of cache_misses
/// per triangle (using Tom Forsyth's super simple method.)
bool optimize_vertex_cache_order(astl::vector<unsigned int>& tri_indices,
                                 unsigned int cache_size = 32);

bool optimize_vertex_cache_order(astl::vector<unsigned int>& tri_indices,
                                 unsigned int cache_size)
{
  if ((tri_indices.size() < 3) || (tri_indices.size() % 3 != 0) ||
      (cache_size < 4))
    return false;
  ...
  astl::vector<float> cache_score;
  cache_score.resize(cache_size + 3, 0.75);
  astl::vector<int> cache_idx;
  cache_idx.resize(cache_size + 3, -1);
  astl::vector<int> grow_cache_idx;
  grow_cache_idx.resize(cache_size + 3, -1);
  ...
  astl::vector<tri_data> t;
  t.resize(num_triangles);
  astl::vector<vert_data> v;
  ...
  for (unsigned int i = 0; i < num_triangles; ++i) {
    t[i].score =
      v[t[i].verts[0]].score + v[t[i].verts[1]].score + v[t[i].verts[2]].score;
  ...
  while (tris_left > 0) {
    ...
    if (best_idx < 0) {
      ...
      niAssertMsg(
        0,
        _A(
          "Your stupid triangle re-organizing code isn't working, Jonathan!!"));
      return false;
    }
    else {
      //  add in this tri
      ...
      for (unsigned int i = 0; i < cache_size + 3; ++i)
        if (cache_idx[i] >= 0) {
          ...
          float new_score =
            cache_score[i] +
            (tF32)(2.0 * ni::Pow((tF64)v[idx].remaining_tris.size(), -0.5));
          v[idx].score = new_score;
          //  now update all remaining linked triangles!
          for (astl::set<unsigned int>::iterator it =
                 v[idx].remaining_tris.begin();
               it != v[idx].remaining_tris.end(); ++it)
          {
...
float calculate_average_cache_miss_ratio(
  const astl::vector<unsigned int>& tri_indices, unsigned int cache_size)
{
  if (tri_indices.size() <= cache_size)
    return -1.0;
  astl::vector<int> cache_idx;
  cache_idx.resize(cache_size, -1);
  int cache_ptr = 0;
  int cache_misses = 0;
...
struct CacheOptimize {
  ...
  CacheOptimize(const tIndex* aSrc, const tU32 aSrcCount,
                tU32 aMaxVertexCacheSize = 32, tF32 aCacheDecayPower = 1.5f,
                tF32 aLastTriScore = 0.75f, tF32 aValBoostScale = 2.0f,
                tF32 aValBoostPower = 0.5f)
  ...
  void InitFirstPass()
  {
    ...
    niLoop (i, _srcCount / 3) {
      ...
      tU32 b = _src[i * 3 + 1];
      tU32 c = _src[i * 3 + 2];
      if (a < _baseVertexIndex) {
        _baseVertexIndex = a;
      }
      if (a > maxVertexIndex) {
        maxVertexIndex = a;
      }
      if (b < _baseVertexIndex) {
        _baseVertexIndex = b;
      }
      if (b > maxVertexIndex) {
        maxVertexIndex = b;
      }
      if (c < _baseVertexIndex) {
        _baseVertexIndex = c;
      }
      if (c > maxVertexIndex) {
        maxVertexIndex = c;
...
///////////////////////////////////////////////
cGeometryPolygonal::cGeometryPolygonal(iGraphics* pGraphics,
                                       tU32 aulNumVertices, tU32 aulNumIndices,
                                       ...
                                       const cFVFDescription& aFVFDesc)
{
  ...
  mptrVA =
    mptrGraphics->CreateVertexArray(aulNumVertices, aFVFDesc.GetFVF(), VAUsage);
  if (!mptrVA.IsOK()) {
    niError(_A("Can't create renderer vertex array."));
    return;
  }
  mptrIA =
    mptrGraphics->CreateIndexArray(eGraphicsPrimitiveType_TriangleList,
                                   aulNumIndices, aulNumVertices, IAUsage);
  if (!mptrIA.IsOK()) {
...
///////////////////////////////////////////////
cGeometryPolygonal::cGeometryPolygonal(iGraphics* pGraphics, iVertexArray* apVA,
                                       iIndexArray* apIA)
{
  ZeroMembers();
...
void cGeometryPolygonal::SetDuplicationIndices(tU32 ulOriginalVertices,
                                               tU32 ulIndices,
                                               const tU32* pIndices)
{
  if (!mptrGraphics.IsOK())
  ...
  if (mvSubsets.empty()) {
    ...
    niLoop (j, mptrIA->GetNumIndices() / 3) {
      ...
      if (a >= numVerts || b >= numVerts || c >= numVerts) {
        niWarning(
          niFmt(_A("Invalid index in face[%d](%d,%d,%d)."), j, a, b, c));
        a = b = c = 0;
      }
      face->ulA = a;
      face->ulB = b;
    ...
    niLoop (i, mvSubsets.size()) {
      ...
      if (sub) {
        ...
        if (sub->GetFirstIndex() >= mptrIA->GetNumIndices()) {
          niWarning(
            niFmt(_A("Invalid subset[%d], first index (%d) out of range (%d)."),
                  i, sub->GetFirstIndex(), mptrIA->GetNumIndices()));
          continue;
        }
        if (sub->GetFirstIndex() + sub->GetNumIndices() >
            mptrIA->GetNumIndices())
        {
          niWarning(
            niFmt(_A("Invalid subset[%d], end index (%d) out of range (%d)."),
                  i, sub->GetFirstIndex() + sub->GetNumIndices(),
                  mptrIA->GetNumIndices()));
          continue;
        }
        niLoop (j, sub->GetNumIndices() / 3) {
          ...
          if (a >= numVerts || b >= numVerts || c >= numVerts) {
            niWarning(
              niFmt(_A("Invalid index in subset[%d], face[%d](%d,%d,%d)."), i,
                    j, a, b, c));
            a = b = c = 0;
          }
          face->ulA = a;
...
tBool __stdcall cGeometryPolygonal::Generate(eGeometryGenerate aGenerate,
                                             tF32 fEpsilon)
{
  if (!mptrGraphics.IsOK())
    return eFalse;
  ...
  if (fvfDesc.HasPosition()) {
    ...
    {
      cFVFVertex A(&stm.GetFVFDescription()), B(&stm.GetFVFDescription()),
        C(&stm.GetFVFDescription());
      for (tFaceVecIt itFace = vFaces.begin(); itFace != vFaces.end(); ++itFace)
      {
        ...
        PlaneNormalize(PlaneFromPoints(itFace->Plane, A.Position(),
    ...
    if (niFlagTest(aGenerate, eGeometryGenerate_FlatNormals) ||
        niFlagTest(aGenerate, eGeometryGenerate_Normals))
    {
      _GenerateNormals(stm, vFaces,
                       niFlagTest(aGenerate, eGeometryGenerate_FlatNormals));
    }
  }
  mptrVA->Unlock();
...
tU32 __stdcall cGeometryPolygonal::GetSubsetIndex(tU32 aulID) const
{
  if (mptrGraphics.IsOK()) {
    tU32 ulCount = 0;
    for (tGeometrySubsetVec::const_iterator itSub = mvSubsets.begin();
         itSub != mvSubsets.end(); ++itSub, ++ulCount)
    {
...
///////////////////////////////////////////////
//! Add a subset.
iGeometrySubset* __stdcall cGeometryPolygonal::AddSubset(tU32 anID,
                                                         tU32 anFirstIndex,
                                                         tU32 anNumIndices,
                                                         tU32 anMaterial)
{
...
iGeometry* __stdcall cGeometryPolygonal::Clone(tGeometryCreateFlags aFlags,
                                               tFVF aFVF)
{
  if (!mptrGraphics.IsOK())
    return NULL;
  ...
  if (!_GetVAUsage(aFlags, NULL)) {
    ...
    case eArrayUsage_DynamicReadWrite:
      aFlags |= eGeometryCreateFlags_VADynamicReadWrite;
      break;
    case eArrayUsage_Static: aFlags |= eGeometryCreateFlags_VAStatic; break;
    case eArrayUsage_SystemMemory:
      aFlags |= eGeometryCreateFlags_VASystemMemory;
      break;
    }
  }
  ...
  if (!_GetIAUsage(aFlags, NULL)) {
    ...
    case eArrayUsage_DynamicReadWrite:
      aFlags |= eGeometryCreateFlags_IADynamicReadWrite;
      break;
    case eArrayUsage_Static: aFlags |= eGeometryCreateFlags_IAStatic; break;
    case eArrayUsage_SystemMemory:
      aFlags |= eGeometryCreateFlags_IASystemMemory;
      break;
    }
  }

  Ptr<cGeometryPolygonal> ptrGeom = niNew cGeometryPolygonal(
    mptrGraphics, mptrVA->GetNumVertices(), mptrIA->GetNumIndices(), aFlags,
  ...
  dstStream.SetBase(ptrGeom->GetVertexArray()->Lock(0, 0, eLock_Normal),
                    ptrGeom->GetVertexArray()->GetNumVertices());
  if (!dstStream.IsOK()) {
  ...
  tIndex* pDstIndices =
    (tIndex*)ptrGeom->GetIndexArray()->Lock(0, 0, eLock_Normal);
  ...
  for (tU32 i = 0; i < GetNumSubsets(); ++i) {
    ...
    ptrGeom->AddSubset(pSubset->GetID(), pSubset->GetFirstIndex(),
                       pSubset->GetNumIndices(), pSubset->GetMaterial());
  }
...
///////////////////////////////////////////////
tBool __stdcall cGeometryPolygonal::SetDrawOp(iDrawOperation* apDrawOp,
                                              tU32 aulSubsetIdx)
{
...
///////////////////////////////////////////////
void cGeometryPolygonal::_GenerateAdjacencyArrays(cFVFStream& astmVerts,
                                                  tFaceVec& avFaces,
                                                  tF32 afEpsilon)
{
  ...
  for (tFaceVecIt itFaceA = avFaces.begin(); itFaceA != avFaces.end();
       ++itFaceA)
  {
    ...
    for (i = 0; i < 3; ++i) {
      ...
      for (tFaceVecIt itFaceB = avFaces.begin(); itFaceB != avFaces.end();
           ++itFaceB)
      {
        if (itFaceA == itFaceB)
          continue;
...
///////////////////////////////////////////////
void cGeometryPolygonal::_GenerateNormals(cFVFStream& astmVerts,
                                          tFaceVec& avFaces, tBool bFlat)
{
  const cFVFDescription& fvfDesc = astmVerts.GetFVFDescription();
  ...
  cFVFVertex A(&astmVerts.GetFVFDescription()),
    B(&astmVerts.GetFVFDescription()), C(&astmVerts.GetFVFDescription());
  tFaceVecIt itFace;
...
tBool __stdcall cGeometryPolygonal::Optimize(tGeometryOptimizeFlags aFlags)
{
  ...
  TRACE_GEOMETRY((
    _A(
      "*** Begining of geometry optimization (%d verts, %d inds, %d subsets) ***"),
    mptrVA->GetNumVertices(), mptrIA->GetNumIndices(), mvSubsets.size()));
  ...
  if (!mvSubsets.empty()) {
    ...
    for (tGeometrySubsetVec::iterator itSubset = mvSubsets.begin();
         itSubset != mvSubsets.end(); ++itSubset)
    {
      tU32 nFirstIdx = ~0;
      tU32 nNumIndices = 0;
  ...
  */
  TRACE_GEOMETRY((
    _A(
      "*** End of geometry optimization (%d verts, %d tris, %d subsets, %.5f acmr, %.5f gacmr) ***"),
    mptrVA->GetNumVertices(), mptrIA->GetNumIndices() / 3, mvSubsets.size(),
    acmr, gacmr));
  return eTrue;
}

///////////////////////////////////////////////
tU32 cGeometryPolygonal::_OptimizeRemoveDegenerates(
  tGeometryOptimizeFlags aFlags, tFaceVec& avFaces)
{
  ...
  for (tFaceVecIt itFace = avFaces.begin(); itFace != avFaces.end(); ++itFace) {
    ...
    if (TriangleIsDegenerate(A.Position(), B.Position(), C.Position()) ||
        TriangleAreaAndNormal(triNormal, A.Position(), B.Position(),
                              C.Position()) < niEpsilon9)
    {
      ++nNumDegenerate;
    }
    else {
...
///////////////////////////////////////////////
tU32 cGeometryPolygonal::_OptimizeUnusedVertices(tGeometryOptimizeFlags aFlags,
                                                 tFaceVec& avFaces)
{
  tU32 nNumUnused = 0;
  tU32Vec vVAStat;
  ...
  Ptr<iVertexArray> ptrNewVA =
    mptrGraphics->CreateVertexArray(mptrVA->GetNumVertices() - nNumUnused,
                                    mptrVA->GetFVF(), mptrVA->GetUsage());
  if (!niIsOK(ptrNewVA)) {
...
///////////////////////////////////////////////
tU32 cGeometryPolygonal::_OptimizeDuplicateVertices(
  tGeometryOptimizeFlags aFlags, tFaceVec& avFaces)
{
...
#define HASH(VEC) ((tU16)(HashVec3(VEC.ptr()) & 0xFFF))
  ...
  for (tU32 i = 0; i < 0x1000; ++i) {
    ...
    for (tU32VecIt itPA = vHash[i].begin(); itPA != vHash[i].end(); ++itPA) {
      nCountA = *itPA;
      if (vDuplicate[nCountA] != eInvalidHandle)
        continue;
      stmA.SetCurrentIndex(nCountA);
      cFVFVertex& vertA = stmA.Vertex();
      for (tU32VecIt itPB = itPA + 1; itPB != vHash[i].end(); ++itPB) {
        tU32 nCountB = *itPB;
        if (vDuplicate[nCountB] != eInvalidHandle)
          continue;
        stmB.SetCurrentIndex(nCountB);
        cFVFVertex& vertB = stmB.Vertex();
        if (fvf.HasPosition() && (vertA.Position() != vertB.Position()))
          continue;
        if (fvf.HasNormal() && (vertA.Normal() != vertB.Normal()))
          continue;
        if (fvf.HasColorA() && (vertA.ColorA() != vertB.ColorA()))
          continue;
        ...
        if (ulNumTexCoos) {
          ...
          for (tU32 i = 0; i < ulNumTexCoos; ++i) {
            ...
            switch (ulTexCooDim) {
            case 4:
              if (vertA.TexCoo4(i) != vertB.TexCoo4(i))
                bSameTexCoo = eFalse;
              break;
            case 3:
              if (vertA.TexCoo3(i) != vertB.TexCoo3(i))
                bSameTexCoo = eFalse;
              break;
            case 2:
              if (vertA.TexCoo2(i) != vertB.TexCoo2(i))
                bSameTexCoo = eFalse;
              break;
            }
            if (!bSameTexCoo)
  ...
  // reassign the indices
  for (tFaceVecIt itFace = avFaces.begin(); itFace != avFaces.end(); ++itFace) {
    if (vDuplicate[itFace->ulA] != eInvalidHandle)
      itFace->ulA = vDuplicate[itFace->ulA];
    if (vDuplicate[itFace->ulB] != eInvalidHandle)
      itFace->ulB = vDuplicate[itFace->ulB];
    if (vDuplicate[itFace->ulC] != eInvalidHandle)
      itFace->ulC = vDuplicate[itFace->ulC];
  }
...
///////////////////////////////////////////////
tBool cGeometryPolygonal::_OptimizeRebuildIndexArray(
  tGeometryOptimizeFlags aFlags, tFaceVec& avFaces)
{
  eArrayUsage usage = mptrIA->GetUsage();
  mptrIA = NULL;
  mptrIA = mptrGraphics->CreateIndexArray(eGraphicsPrimitiveType_TriangleList,
                                          (tU32)avFaces.size() * 3,
...
///////////////////////////////////////////////
iGeometry* __stdcall cGraphics::CreateGeometryPolygonal(
  tU32 aulNumVertices, tU32 aulNumIndices, tGeometryCreateFlags aFlags,
  tFVF aFVF)
{
  return niNew cGeometryPolygonal(this, aulNumVertices, aulNumIndices, aFlags,
                                  cFVFDescription(aFVF));
}

///////////////////////////////////////////////
iGeometry* __stdcall cGraphics::CreateGeometryPolygonalEx(iVertexArray* apVA,
                                                          iIndexArray* apIA)
{
...
iGeometry* cGraphics::CreateGeometryPolygonalCube(
  tGeometryCreateFlags aFlags, tFVF aFVF, const sVec3f& avCenter, tF32 afWidth,
...
#define SET(vx, vy, vz)                \
  if (abCW) {                          \
    verts.Vertex().Normal().x = vx;    \
    verts.Vertex().Normal().y = vy;    \
    verts.Vertex().Normal().z = vz;    \
  }                                    \
  else {                               \
    verts.Vertex().Normal().x = -(vx); \
    verts.Vertex().Normal().y = -(vy); \
...
#define SET(vx, vy)                                                          \
  {                                                                          \
    ...
    SET(1.0f, 0.0f);
    SET(1.0f, 1.0f);
    SET(0.0f, 1.0f); // right plane UV's
    SET(0.0f, 0.0f);
    SET(1.0f, 0.0f);
    SET(1.0f, 1.0f);
    SET(0.0f, 1.0f); // left plane UV's
    SET(1.0f, 0.0f);
    SET(1.0f, 1.0f);
    SET(0.0f, 1.0f);
    SET(0.0f, 0.0f); // top plane UV's
    SET(0.0f, 1.0f);
    SET(0.0f, 0.0f);
    SET(1.0f, 0.0f);
    SET(1.0f, 1.0f); // bottom plane UV's
    SET(0.0f, 0.0f);
    SET(1.0f, 0.0f);
    SET(1.0f, 1.0f);
    SET(0.0f, 1.0f); // front plane UV's
    SET(0.0f, 0.0f);
    SET(1.0f, 0.0f);
    SET(1.0f, 1.0f);
    SET(0.0f, 1.0f); // back plane UV's
#undef SET
  }
...
iGeometry* cGraphics::CreateGeometryPolygonalPlane(
  ...
  tBool abCW, tU32 aulColor, const sMatrixf& amtxUV)
{
  ...
  iGeometry* pGeom =
    CreateGeometryPolygonal(ulNumVertices, ulNumIndices, aFlags, aFVF);
  if (!niIsOK(pGeom)) {
    niError(_A("Can't create geometry."));
    return NULL;
  }
  ...
  cFVFDescription fvf(pGeom->GetVertexArray()->GetFVF());
  cFVFStream verts(&fvf, pGeom->GetVertexArray()->Lock(0, 0, eLock_Normal),
                   ulNumVertices);
...
#define SET(vx, vy, vz)                          \
  ...
  verts.Next();
    ...
    if (aPlaneType == ePlaneType_XZ) {
      for (i = 0; i <= aulNumDiv; ++i) {
        for (j = 0; j <= aulNumDiv; ++j) {
          SET((-1.0f + (float(j) * fDelta)) * sizeX, 0.0f,
              (-1.0f + (float(i) * fDelta)) * sizeY); // EDMOND
        }
      }
    }
      ...
      for (i = 0; i <= aulNumDiv; ++i) {
        for (j = 0; j <= aulNumDiv; ++j) {
          SET(0.0f, (-1.0f + (float(i) * fDelta)) * sizeY,
              (-1.0f + (float(j) * fDelta)) * sizeX); // EDMOND
        }
      }
    }
    else //if (aPlaneType == ePlaneType_XY)
    {
      for (i = 0; i <= aulNumDiv; ++i) {
        for (j = 0; j <= aulNumDiv; ++j) {
          SET((-1.0f + (float(j) * fDelta)) * sizeX,
              (-1.0f + (float(i) * fDelta)) * sizeY, 0.0f); // EDMOND
        }
      }
    }
...
iGeometry* __stdcall cGraphics::CreateGeometryPolygonalCylinder(
  ...
  const sMatrixf& amtxUV, tBool abCap, tBool abCentered)
{
  slices = ni::Clamp(slices, 3, (tI32)_knMaxGeomSubDiv);
  stacks = ni::Clamp(stacks, 1, (tI32)_knMaxGeomSubDiv);
  return QuadricCylinder(this, aFlags, aFVF, baseRadius, topRadius, height,
                         slices, stacks, abCW, aulColor, amtxUV, abCap,
                         abCentered);
}

///////////////////////////////////////////////
iGeometry* __stdcall cGraphics::CreateGeometryPolygonalSphere(
  tGeometryCreateFlags aFlags, tFVF aFVF, tF32 radius, tI32 slices, tI32 stacks,
  tBool abCW, tU32 aulColor, const sMatrixf& amtxUV)
{
  slices = ni::Clamp(slices, 3, (tI32)_knMaxGeomSubDiv);
  stacks = ni::Clamp(stacks, 3, (tI32)_knMaxGeomSubDiv);
  return QuadricSphere(this, aFlags, aFVF, radius, slices, stacks, abCW,
                       aulColor, amtxUV);
}

///////////////////////////////////////////////
iGeometry* __stdcall cGraphics::CreateGeometryPolygonalGeosphere(
  tGeometryCreateFlags aFlags, tFVF aFVF, tF32 radius, tI32 slices, tBool abCW,
  tU32 aulColor, const sMatrixf& amtxUV)
{
  slices = ni::Clamp(slices, 2, (tI32)_knMaxGeomSubDiv);
  return GeoSphere(this, aFlags, aFVF, 0, eFalse, radius, slices, abCW,
                   aulColor, amtxUV);
}

///////////////////////////////////////////////
iGeometry* __stdcall cGraphics::CreateGeometryPolygonalDiskSweep(
  ...
  tU32 aulColor, const sMatrixf& amtxUV)
{
  slices = ni::Clamp(slices, 2, (tI32)_knMaxGeomSubDiv);
  stacks = ni::Clamp(stacks, 1, (tI32)_knMaxGeomSubDiv);
  return QuadricDiskSweep(this, aFlags, aFVF, innerRadius, outerRadius, slices,
                          stacks, startAngle, sweepAngle, abCW, aulColor,
                          amtxUV);
}

///////////////////////////////////////////////
tBool __stdcall cGraphics::UpdateGeometryPolygonalDiskSweep(
  ...
  const sMatrixf& amtxUV)
{
  slices = ni::Clamp(slices, 2, (tI32)_knMaxGeomSubDiv);
  stacks = ni::Clamp(stacks, 1, (tI32)_knMaxGeomSubDiv);
  return QuadricDiskSweepEx(apGeom, innerRadius, outerRadius, slices, stacks,
                            startAngle, sweepAngle, abCW, aulColor, amtxUV);
}

///////////////////////////////////////////////
iGeometry* __stdcall cGraphics::CreateGeometryPolygonalDisk(
  tGeometryCreateFlags aFlags, tFVF aFVF, tF32 innerRadius, tF32 outerRadius,
  tI32 slices, tI32 stacks, tBool abCW, tU32 aulColor, const sMatrixf& amtxUV)
{
  slices = ni::Clamp(slices, 2, (tI32)_knMaxGeomSubDiv);
  stacks = ni::Clamp(stacks, 1, (tI32)_knMaxGeomSubDiv);
  return QuadricDisk(this, aFlags, aFVF, innerRadius, outerRadius, slices,
                     stacks, abCW, aulColor, amtxUV);
}
