...
EA_RESTORE_ALL_VC_WARNINGS()
  ...
  #pragma warning(push)
  #pragma warning( \
      disable : 4625) // copy constructor could not be generated because a base class copy constructor is inaccessible or deleted.
  #pragma warning( \
      disable : 4626) // assignment operator could not be generated because a base class assignment operator is inaccessible or deleted.
  #pragma warning( \
...
extern "C" long __stdcall _InterlockedCompareExchange(long volatile* Dest,
...
extern "C" long _InterlockedCompareExchange(long volatile* Dest, long Exchange,
                                            long Comp);
    #pragma intrinsic(_InterlockedCompareExchange)
  #endif
#endif
...
inline int32_t atomic_decrement(int32_t* p32) EA_NOEXCEPT
{
  ...
  return _InterlockedDecrement(
    (volatile long*)
      p32); // volatile long cast is OK because int32_t == long on Microsoft platforms.
#elif defined(EA_COMPILER_GNUC)
  int32_t result;
  __asm__ __volatile__("lock; xaddl %0, %1"
                       : "=r"(result), "=m"(*p32)
...
inline bool atomic_compare_and_swap(int32_t* p32, int32_t newValue,
                                    int32_t condition)
{
#if defined(EA_COMPILER_CLANG) || \
  (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4003))
  ...
  return ((int32_t)_InterlockedCompareExchange(
            (volatile long*)p32, (long)newValue, (long)condition) == condition);
#elif defined(EA_COMPILER_GNUC)
...
class EASTL_API mutex {
 ...
 protected:
  ...
  uint64_t
    mMutexBuffer[40 /
                 sizeof(uint64_t)]; // CRITICAL_SECTION is 40 bytes on Win64.
    #elif defined(_WIN32)
  uint32_t
    mMutexBuffer[24 /
                 sizeof(uint32_t)]; // CRITICAL_SECTION is 24 bytes on Win32.
    #endif
  #elif defined(EA_PLATFORM_POSIX)
