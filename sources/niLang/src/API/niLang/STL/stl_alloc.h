...
#define ASTL_RAW_ALLOCATOR_IMPL(NAME)                                        \
  struct raw_allocator_##NAME {                                              \
    ...
    static void* aligned_allocate(size_t n, size_t alignment,                \
                                  size_t alignmentOffset, int /*flags*/ = 0) \
    {                                                                        \
...
class astl_allocator {
 public:
  ...
  astl_allocator& operator=(const astl_allocator&)
  {
    return *this;
  }

  bool operator==(const astl_allocator&)
  {
    return true;
  }
                 ...
                 int flags = 0)
  {
    return reinterpret_cast<pointer>(
      raw_allocator::aligned_allocate(n, alignment, alignmentOffset, flags));
  }
...
bool operator==(const astl_allocator<T, _RawAlloc>&,
                const astl_allocator<U, _RawAlloc>&)
{
  return true;
}
template <class T, class U, class _RawAlloc>
