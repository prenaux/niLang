...
struct array {
 ...
 public:
  ...
  void swap(this_type& x)
    EA_NOEXCEPT_IF(eastl::is_nothrow_swappable<value_type>::value);
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::iterator array<T, N>::begin()
  EA_NOEXCEPT
{
  return &mValue[0];
}
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::const_iterator array<
  T, N>::begin() const EA_NOEXCEPT
{
  return &mValue[0];
}
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::const_iterator array<
  T, N>::cbegin() const EA_NOEXCEPT
{
  return &mValue[0];
}

template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::iterator array<T, N>::end()
  EA_NOEXCEPT
{
  return &mValue[N];
}
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::const_iterator array<
  T, N>::cend() const EA_NOEXCEPT
{
  return &mValue[N];
}

template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::reverse_iterator array<
  T, N>::rbegin() EA_NOEXCEPT
{
  return reverse_iterator(&mValue[N]);
}

template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::const_reverse_iterator array<
  T, N>::rbegin() const EA_NOEXCEPT
{
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::const_reverse_iterator array<
  T, N>::crend() const EA_NOEXCEPT
{
  return const_reverse_iterator(static_cast<const_iterator>(&mValue[0]));
}

template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::size_type array<T, N>::size()
  const EA_NOEXCEPT
{
  return (size_type)N;
}
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::size_type array<
  T, N>::max_size() const EA_NOEXCEPT
{
  return (size_type)N;
}
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::size_type array<
  T, N>::size_bytes() const EA_NOEXCEPT
{
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::reference array<
  T, N>::operator[](size_type i)
{
  return mValue[i];
}

template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::const_reference array<
  T, N>::operator[](size_type i) const
{
  return mValue[i];
}

template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::reference array<T, N>::front()
{
  return mValue[0];
}
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::reference array<T, N>::back()
{
  return mValue[N - 1];
}
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::const_reference array<T, N>::at(
  size_type i) const
{
#if EASTL_EXCEPTIONS_ENABLED
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline typename array<T, N>::reference array<T, N>::at(
  size_type i)
{
#if EASTL_EXCEPTIONS_ENABLED
  if (EASTL_UNLIKELY(i >= N))
...
template <typename T, size_t N>
EA_CPP14_CONSTEXPR inline bool operator==(const array<T, N>& a,
                                          const array<T, N>& b)
{
...
template <typename T, size_t N>
inline synth_three_way_result<T> operator<=>(const array<T, N>& a,
                                             const array<T, N>& b)
{
  return eastl::lexicographical_compare_three_way(
    &a.mValue[0], &a.mValue[N], &b.mValue[0], &b.mValue[N], synth_three_way{});
}
#else
...
EA_CPP14_CONSTEXPR inline bool operator<(const array<T, N>& a,
                                         const array<T, N>& b)
{
  return eastl::lexicographical_compare(&a.mValue[0], &a.mValue[N],
                                        &b.mValue[0], &b.mValue[N]);
}
...
EA_CPP14_CONSTEXPR inline bool operator>(const array<T, N>& a,
                                         const array<T, N>& b)
{
  return eastl::lexicographical_compare(&b.mValue[0], &b.mValue[N],
                                        &a.mValue[0], &a.mValue[N]);
}
...
{
  static_assert(eastl::is_constructible_v<T, T&>,
                "element type T must be copy-initializable");
  static_assert(!eastl::is_array_v<T>,
...
EA_CONSTEXPR eastl::array<eastl::remove_cv_t<T>, N> to_array(T (&&a)[N])
{
  static_assert(eastl::is_move_constructible_v<T>,
...
template <typename T, size_t N>
class std::tuple_size<::eastl::array<T, N>>
    : public ::eastl::integral_constant<size_t, N> {};
