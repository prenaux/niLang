...
TestGpuFuncs_PixelOutput TestGpuFuncs_VertexOutput_fixed_tex_alphatest_ps(TestGpuFuncs_VertexOutput aInput, TestGpuFuncs_TestUniforms aUniforms, texture2D aTexture, sampler aSS);

// FunctionFwd: TestGpuFuncs
void TestGpuFuncs_alphaTest(float aAlphaValue, float aAlphaRef);
...
TestGpuFuncs_PixelOutput TestGpuFuncs_VertexOutput_fixed_tex_alphatest_ps(TestGpuFuncs_VertexOutput aInput, TestGpuFuncs_TestUniforms aUniforms, texture2D aTexture, sampler aSS) {
  ...
  TestGpuFuncs_alphaTest(texColor.w,aUniforms.alphaRef);
  vec4 _tmp_a = (texColor*aInput.color);
  return TestGpuFuncs_PixelOutput_new(_tmp_a);
}

// Function: TestGpuFuncs
void TestGpuFuncs_alphaTest(float aAlphaValue, float aAlphaRef) {
  bool _tmp_e = (aAlphaValue < aAlphaRef);
  if (_tmp_e) {
    discard;
  }
}

// Pixel Shader main: TestGpuFuncs_VertexOutput_fixed_tex_alphatest_ps
...
void main(void) {
  ...
  TestGpuFuncs_PixelOutput _rval_ = TestGpuFuncs_VertexOutput_fixed_tex_alphatest_ps(aInput, aUniforms, IN_1_aTexture, IN_1_aSS);
