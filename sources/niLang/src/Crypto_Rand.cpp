...
tInt _RandMakeChunk(mbedtls_ctr_drbg_context* ctx, unsigned char* buffer,
                    tSize length)
{
  ...
  if (residue) {
    tInt ret = mbedtls_ctr_drbg_random(ctx, buffer + length - residue, residue);
    if (ret != 0)
      return ret;
  }
...
struct cCryptoRand : public ImplRC<iCryptoRand> {
  ...
  {
    return (mbedtls_ctr_drbg_reseed(&_ctx, custom, length) == 0) ? eTrue
                                                                 : eFalse;
  }
