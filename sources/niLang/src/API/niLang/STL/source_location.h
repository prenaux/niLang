...
struct source_location {
 private:
  ...
  explicit niConstExpr source_location(
    const int line = 0, const int column = 0,
 ...
 public:
  ...
  inline static niConstExpr source_location
  current(const int line = (int)__builtin_LINE(),
  ...
  inline static niConstExpr source_location
  current(const int line = (int)__builtin_LINE(), const int column = 0,
          const char* file_name = __builtin_FILE(),
          const char* function_name = __builtin_FUNCTION()) EA_NOEXCEPT
  #else
    #error "source_location is not supported on this platform."
  inline static niConstExpr source_location
  current(const int line = 0, const int column = 0,
...
#define ASTL_SOURCE_LOCATION_PARAM_WITH_DEFAULT \
  astl::source_location aSrcLoc = astl::source_location::current()
#define ASTL_SOURCE_LOCATION_PARAM_SOLO astl::source_location aSrcLoc
#define ASTL_SOURCE_LOCATION_ARG_SOLO aSrcLoc
#define ASTL_SOURCE_LOCATION_ARG_CALL \
  aSrcLoc.file_name(), aSrcLoc.line(), aSrcLoc.function_name()
