...
nish_std_PixelOutput TestGpuFuncs_triangle_rayquery_ps(nish_std_PixelInput aInput, TestGpuFuncs_TestRayUniforms aUniforms, accelerationStructureEXT aAS);
...
nish_std_PixelOutput TestGpuFuncs_triangle_rayquery_ps(nish_std_PixelInput aInput, TestGpuFuncs_TestRayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec3 _tmp_J = nish_std_UIntToHashColor(uint(rayQueryGetIntersectionInstanceCustomIndexEXT(rayQuery,true)));
  vec4 instanceColor = vec4(_tmp_J.x,_tmp_J.y,_tmp_J.z,1.0);
  vec4 _tmp_O;
  bool _tmp_P = (intersectionType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_P) {
    _tmp_O = instanceColor;
  }
  else {
    bool _tmp_S = (intersectionType == nish_std_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_S) {
  ...
  return nish_std_PixelOutput_new(_tmp_O);
}
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = TestGpuFuncs_triangle_rayquery_ps(aInput, aUniforms, IN_1_aAS);
