...
sVec3<T> __stdcall cFrustumImpl<T, BASE>::GetUp() const
{
  ...
  sVec3<T> r;
  VecCross(r, mvPlanes[eFrustumPlane_Right].GetNormal(),
...
sVec3<T> __stdcall cFrustumImpl<T, BASE>::GetRight() const
{
  ...
  sVec3<T> r;
  VecCross(r, mvPlanes[eFrustumPlane_Bottom].GetNormal(),
...
{
  if (mvPlanes.size() == ulNumPlane)
    return;
  if (ulNumPlane == 0)
    mvPlanes.clear();
  else
    mvPlanes.resize(ulNumPlane);
}
...
template <typename T, typename BASE>
void cFrustumImpl<T, BASE>::AddPlanes(tU32 aulNumPlane,
                                      const sVec4<T>* apPlanes)
{
  if (!aulNumPlane)
    return;
...
template <typename T, typename BASE>
void cFrustumImpl<T, BASE>::SetPlanes(tU32 aulNumPlane,
                                      const sVec4<T>* apPlanes)
{
...
//! Cull an AABB.
template <typename T, typename BASE>
eCullCode cFrustumImpl<T, BASE>::CullAABB(const sVec3f& vMin,
                                          const sVec3f& vMax) const
{
  if (mvPlanes.empty())
...
template <typename T, typename BASE>
tBool cFrustumImpl<T, BASE>::IntersectAABB(const sVec3f& vMin,
                                           const sVec3f& vMax) const
{
  if (mvPlanes.empty())
  ...
  for (tPlaneVecCIt it = mvPlanes.begin(); it != mvPlanes.end(); ++it) {
    if (it->x > 0.0f) {
      if (it->y > 0.0f) {
        if (it->z > 0.0f)
          nearpoint.Set(vMin.x, vMin.y, vMin.z);
        else
          nearpoint.Set(vMin.x, vMin.y, vMax.z);
      }
      else {
        if (it->z > 0.0f)
          nearpoint.Set(vMin.x, vMax.y, vMin.z);
        else
          nearpoint.Set(vMin.x, vMax.y, vMax.z);
      }
    }
    else {
      if (it->y > 0.0f) {
        if (it->z > 0.0f)
          nearpoint.Set(vMax.x, vMin.y, vMin.z);
        else
          nearpoint.Set(vMax.x, vMin.y, vMax.z);
      }
      else {
        if (it->z > 0.0f)
          nearpoint.Set(vMax.x, vMax.y, vMin.z);
        else
          nearpoint.Set(vMax.x, vMax.y, vMax.z);
      }
    }
...
//! Cull a sphere.
template <typename T, typename BASE>
eCullCode cFrustumImpl<T, BASE>::CullSphere(const sVec3f& avCenter,
                                            tF32 afRadius) const
{
  if (mvPlanes.empty())
    return eCullCode_In; // No planes in the frustum means that it's an infinie (360 degres frustum)
...
template <typename T, typename BASE>
tBool cFrustumImpl<T, BASE>::IntersectSphere(const sVec3f& avCenter,
                                             tF32 afRadius) const
{
