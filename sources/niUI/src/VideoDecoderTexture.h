...
struct sVideoDecoderTexture
    : public ImplAggregate<iTexture, eImplFlags_DontInherit1,
                           ni::iDeviceResource> {
  ...
  sVideoDecoderTexture(iGraphics* apGraphics, iHString* ahspName,
                       iVideoDecoder* apDecoder, iTexture* apTexture)
  {
  ...
  iUnknown* __stdcall QueryInterface(const tUUID& aIID)
  {
    ...
    if (aIID == niGetInterfaceUUID(iGLTexture))
      return mptrTexture.IsOK() ? mptrTexture->QueryInterface(aIID)
                                : mptrTexture.ptr();
  ...
  virtual iDeviceResource* __stdcall Bind(iUnknown* apDevice)
  {
    ...
    d->Update(ni::eTrue,
              d->GetUpdateOnBind() ? ni::GetLang()->GetFrameTime() : 0);
