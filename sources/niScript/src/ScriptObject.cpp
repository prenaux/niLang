...
///////////////////////////////////////////////
cScriptObject::cScriptObject(cScriptVM* apVM, int idx, int numpop,
                             tBool abHoldRef)
    : mbHoldRef(abHoldRef)
{
  ...
  if (gScriptObjectDebugID_Counter == 0)
    gScriptObjectDebugID_Counter = 1;
...
{
  if (!IsOK())
    return eScriptObjectType_Unknown;
  return mType;
}
...
tBool __stdcall cScriptObject::Get(int idx, int numpop)
{
  ...
  case eScriptType_NativeClosure: {
    ...
    sq_getclosureinfo(*mpVM, idx, (tU32*)&mnNumParameters,
                      (tU32*)&mnNumFreeVars);
...
{
  #pragma niTodo( \
      "Add a parameter to specify if it's required to enumerate delegate's objects")
  #pragma niTodo( \
      "Add a parameter to specify if it's the hidden (starting with __) keys should be enumerated")
  ...
  while (SQ_SUCCEEDED(sq_getdelegate(v, -1))) {
    ...
    while (SQ_SUCCEEDED(sq_next(v, -2))) {
      const SQChar* aszKey = NULL;
      if (SQ_SUCCEEDED(sq_getstring(v, -2, &aszKey)) &&
          StrNCmp(aszKey, _A("__"), 2) == 0)
      {
        sq_pop(v, 2); // pop key and value
        continue;
