...
template <typename KEYT, typename ITEMT,
          ASTL_TMPL_PARAM(KEYCMP, eastl::less<KEYT>)>
class IndexedMap {
  typedef KEYT tKey;
  typedef ITEMT tItem;

 public:
  typedef astl::map<KEYT, tU32, KEYCMP,
                    ASTL_PAIR_ALLOCATOR(const KEYT, tU32, indexed_map)>
    tKeyIdxMap;
 ...
 public:
  ...
  {
    if (anIndex >= mVec.size())
      return eFalse;
    if (mVec[anIndex].itMap == mMap.end())
      return eFalse;
    return eTrue;
  }
  ...
  tItem& __stdcall operator[](tU32 nIndex)
  {
    return GetItem(nIndex);
  }
  //! Return a constant reference of the item with the given index.
  const tItem& __stdcall operator[](tU32 nIndex) const
  {
    return GetItem(nIndex);
  }
  //! Return a reference of the item with the given name.
  tItem& __stdcall operator[](const tKey& aKey)
  {
    return GetItem(GetKeyIndex(aKey));
  }
  //! Return a reference of the item with the given name.
  const tItem& __stdcall operator[](const tKey& aKey) const
  {
    return GetItem(GetKeyIndex(aKey));
  }
