...
//---------------------------------------------------------------line_coord
struct line_coord {
  AGG_INLINE static int conv(agg_real x)
  {
...
void inline fix_degenerate_bisectrix_start(const line_parameters& lp, int* x,
                                           int* y)
{
  int d = iround((agg_real(*x - lp.x2) * agg_real(lp.y2 - lp.y1) -
                  agg_real(*y - lp.y2) * agg_real(lp.x2 - lp.x1)) /
                 lp.len);
...
void inline fix_degenerate_bisectrix_end(const line_parameters& lp, int* x,
                                         int* y)
{
  int d = iround((agg_real(*x - lp.x2) * agg_real(lp.y2 - lp.y1) -
                  agg_real(*y - lp.y2) * agg_real(lp.x2 - lp.x1)) /
