...
#define Check3D(CHAN, RET)                               \
  if (CHAN < mnFirst3DChannel || CHAN > mnLast3DChannel) \
    return RET;
...
class cSoundMixerSoftware3D : public ImplRC<iSoundMixer3D, eImplFlags_Default> {
 ...
 public:
  ...
  ///////////////////////////////////////////////
  tBool Initialize(iSoundMixer* apBase, tU32 anNum3DChannels,
  ...
  //--------------------------------------------------------------------------
  void Compute3D(
    tI32& anOutVolume, // volume, is a scale of the current absolute volume
    tI32& anOutPan,    // panning...
    tF32& afOutPitch,  // pitch, scale of the frequency
    const sChannel& c, const sListener& l,
  ...
  virtual tBool __stdcall UpdateMixer3D()
  {
    ...
    for (tU32 i = mnFirst3DChannel; i <= mnLast3DChannel; ++i) {
      ...
      if (c.mode != eSoundMode_Normal2D && mptrBase->GetIsChannelBusy(i)) {
        tI32 vol, pan;
        tF32 pitch;
        Compute3D(vol, pan, pitch, c, mListener,
