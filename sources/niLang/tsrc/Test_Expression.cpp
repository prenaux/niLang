...
TEST_FIXTURE(FExpression, String1)
{
  ...
  Ptr<iExpression> expr =
    ptrCtx->CreateExpression("\"hello foo\" + ' bar yey'");
...
TEST_FIXTURE(FExpression, String5)
{
  ...
  Ptr<iExpression> expr = ptrCtx->CreateExpression(
    "\"hello foo\" + ` bar yey 123 + 456 `yep` ` ? time");
  CHECK_RETURN_IF_FAILED(expr.IsOK());
...
TEST_FIXTURE(FExpression, StringFormat_Err)
{
  ...
  CHECK_EQUAL(_ASTR(""),
              ni::GetLang()->Eval("format('foo%.2f',)")->GetString());
}

TEST_FIXTURE(FExpression, StringFormat)
{
  CHECK_EQUAL(_ASTR("str:"),
              ni::GetLang()->Eval("'str:'+format()")->GetString());
  CHECK_EQUAL(
    _ASTR("str:foo123.78"),
    ni::GetLang()->Eval("'str:'+format('foo%.2f',123.781)")->GetString());
  CHECK_EQUAL(_ASTR("str:foo%.2f"),
              ni::GetLang()->Eval("'str:'+format('foo%.2f')")->GetString());
  CHECK_EQUAL(
    _ASTR("str:foo123bar"),
    ni::GetLang()->Eval("'str:'+format('foo%d%d%dbar',1,2,3)")->GetString());

  CHECK_EQUAL(_ASTR("str:N/A"),
              ni::GetLang()->Eval("'str:'+format('%$d',-123)")->GetString());
  CHECK_EQUAL(_ASTR("str:N/A"),
              ni::GetLang()->Eval("'str:'+format('%$D',-14325)")->GetString());
  CHECK_EQUAL(_ASTR("str:N/A"),
              ni::GetLang()->Eval("'str:'+format('%$f',-1.123)")->GetString());
  CHECK_EQUAL(_ASTR("str:N/A"),
              ni::GetLang()->Eval("'str:'+format('%$g',-1.123)")->GetString());

  CHECK_EQUAL(_ASTR("str:yes"),
              ni::GetLang()->Eval("'str:'+format('%y','true')")->GetString());
  CHECK_EQUAL(_ASTR("str:on"),
              ni::GetLang()->Eval("'str:'+format('%z','yes')")->GetString());

  CHECK_EQUAL(_ASTR("str:yes"),
              ni::GetLang()->Eval("'str:'+format('%y',-1)")->GetString());
  CHECK_EQUAL(_ASTR("str:no"),
              ni::GetLang()->Eval("'str:'+format('%y',0)")->GetString());
  CHECK_EQUAL(_ASTR("str:yes"),
              ni::GetLang()->Eval("'str:'+format('%y',1)")->GetString());
  CHECK_EQUAL(_ASTR("str:on"),
              ni::GetLang()->Eval("'str:'+format('%z',-1)")->GetString());
  CHECK_EQUAL(_ASTR("str:off"),
              ni::GetLang()->Eval("'str:'+format('%z',0)")->GetString());
  CHECK_EQUAL(_ASTR("str:on"),
              ni::GetLang()->Eval("'str:'+format('%z',1)")->GetString());
  ...
  CHECK_EQUAL(
    _ASTR("str:foo=bar"),
    ni::GetLang()->Eval("'str:'+format('%s=%s','foo','bar')")->GetString());
}
...
TEST_FIXTURE(FExpression, Protocol)
{
  ...
  {
    Ptr<iExpression> expr =
      ptrCtx->CreateExpression("value://three + value://three");
    CHECK_RETURN_IF_FAILED(expr.IsOK());
    {
      tU32 v = (tU32)expr->GetEvalResult()->GetFloat();
      niDebugFmt((_A("V: %s"), v));
  ...
  {
    Ptr<iExpression> expr =
      ptrCtx->CreateExpression("sqrt(value://alpha) + sqrt(value://alpha)");
    CHECK_RETURN_IF_FAILED(expr.IsOK());
    {
      tU32 v = (tU32)expr->GetEvalResult()->GetFloat();
      niDebugFmt((_A("V: %s"), v));
...
TEST_FIXTURE(FExpression, RGB)
{
  ...
  CHECK_EQUAL(eTrue,
              niFlagIs(result->GetFlags(), eExpressionVariableFlags_Color));
  ...
  v =
    ptrCtx
      ->Eval(_A(
        "rgb(80.39215686274509803921568627451%,36.078431372549019607843137254902%,36.078431372549019607843137254902%)"))
      ->GetVec3();
  CHECK_CLOSE(Vec3<tF32>(0.8039215686274509803921568627451f,
...
TEST_FIXTURE(FExpression, Flags)
{
  ...
  niLoop (i, 1000) {
    tU32 v = (tU32)ni::GetLang()
               ->Eval("eType.IUnknown|eTypeFlags.Pointer")
               ->GetFloat();
    // niDebugFmt((_A("V: %d"),v));
    CHECK_EQUAL(eType_IUnknownPtr, v);
    if (eType_IUnknownPtr != v)
      break;
  }

  {
    tU32 v =
      ni::GetLang()->StringToEnum("Constant|Pointer", niFlagsExpr(eTypeFlags));
    niDebugFmt((_A("V: %d"), v));
...
TEST_FIXTURE(FExpression, DigestHex)
{
  cString md5 =
    ni::GetLang()
      ->Eval(
        "DigestHex('C:/Documents and Settings/Owner/My Documents/abcddefghiàéèçù€µ°你好αβ/Gaya/test你好.txt','md5')")
      ->GetString();
  ...
  cString sha1 =
    ni::GetLang()
      ->Eval(
        "DigestHex('C:/Documents and Settings/Owner/My Documents/abcddefghiàéèçù€µ°你好αβ/Gaya/test你好.txt','sha1')")
      ->GetString();
...
{
  cString r1 = ni::GetLang()
                 ->Eval("KDFCrypt('12345', '$2a$08$VXQoSyZF288.82vVBGuEN.')")
                 ->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(
    _ASTR("$2a$08$VXQoSyZF288.82vVBGuEN.1NasswyF6K7sDZxyH/xGuLP4BEg26yK"), r1);

  cString s1 = ni::GetLang()->Eval("KDFGenSaltBlowfish(4)")->GetString();
  TEST_DEBUGFMT("s1: %s", s1);
  CHECK(s1.StartsWith("$2a$04$"));

  cString s2 = ni::GetLang()->Eval("KDFGenSaltBlowfish(8)")->GetString();
  TEST_DEBUGFMT("s2: %s", s2);
  CHECK(s2.StartsWith("$2a$08$"));

  cString s3 = ni::GetLang()->Eval("KDFGenSaltBlowfish(8)")->GetString();
...
{
  tInt r1 = (tInt)ni::GetLang()
              ->Eval("StrLen('$2a$08$VXQoSyZF288.82vVBGuEN.')")
              ->GetFloat();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(29, r1);
}
...
{
  // empty string
  cString r0 = ni::GetLang()
                 ->Eval("StrSlice('$2a$08$VXQoSyZF288.82vVBGuEN.',7,0)")
                 ->GetString();
  ...
  cString r1 = ni::GetLang()
                 ->Eval("StrSlice('$2a$08$VXQoSyZF288.82vVBGuEN.',7,-1)")
                 ->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(_ASTR("VXQoSyZF288.82vVBGuEN"), r1);

  // trim the last 3 characters
  cString r2 = ni::GetLang()
                 ->Eval("StrSlice('$2a$08$VXQoSyZF288.82vVBGuEN.',7,-3)")
                 ->GetString();
  TEST_DEBUGFMT("r2: %s", r2);
  CHECK_EQUAL(_ASTR("VXQoSyZF288.82vVBGu"), r2);

  // whole string
  cString r3 = ni::GetLang()
                 ->Eval("StrSlice('$2a$08$VXQoSyZF288.82vVBGuEN.',0)")
                 ->GetString();
  TEST_DEBUGFMT("r3: %s", r3);
  CHECK_EQUAL(_ASTR("$2a$08$VXQoSyZF288.82vVBGuEN."), r3);

  cString r4 = ni::GetLang()
                 ->Eval("StrSlice('$2a$08$VXQoSyZF288.82vVBGuEN.',0,1000)")
                 ->GetString();
  TEST_DEBUGFMT("r4: %s", r4);
  CHECK_EQUAL(_ASTR("$2a$08$VXQoSyZF288.82vVBGuEN."), r4);

  cString r5 =
    ni::GetLang()->Eval("StrSlice('Hello World',0,100)")->GetString();
  TEST_DEBUGFMT("r5: %s", r5);
  CHECK_EQUAL(_ASTR("Hello World"), r5);

  cString r6 =
    ni::GetLang()->Eval("StrSlice('Hello World',6,100)")->GetString();
...
{
  cString r1 = ni::GetLang()
                 ->Eval("StrMid('$2a$08$VXQoSyZF288.82vVBGuEN.',7,5)")
                 ->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(_ASTR("VXQoS"), r1);

  cString r2 = ni::GetLang()
                 ->Eval("StrMid('$2a$08$VXQoSyZF288.82vVBGuEN.',7,5000)")
                 ->GetString();
  TEST_DEBUGFMT("r2: %s", r2);
...
{
  cString r1 = ni::GetLang()
                 ->Eval("StrLeft('$2a$08$VXQoSyZF288.82vVBGuEN.',7)")
                 ->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
...
{
  cString r1 = ni::GetLang()
                 ->Eval("StrRight('$2a$08$VXQoSyZF288.82vVBGuEN.',4)")
                 ->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
...
{
  cString r1 = ni::GetLang()
                 ->Eval("StrBefore('$2a$08$VXQoSyZF288.82vVBGuEN.','.')")
                 ->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(_ASTR("$2a$08$VXQoSyZF288"), r1);

  cString r2 = ni::GetLang()
                 ->Eval("StrRBefore('$2a$08$VXQoSyZF288.82vVBGuEN.','.')")
                 ->GetString();
  TEST_DEBUGFMT("r2: %s", r2);
  ...
  cString r3 =
    ni::GetLang()
      ->Eval("StrBefore(StrRAfter('$2a$08$VXQoSyZF288.82vVBGuEN.','$'),'.')")
      ->GetString();
  TEST_DEBUGFMT("r3: %s", r3);
  CHECK_EQUAL(_ASTR("VXQoSyZF288"), r3);

  cString r4 = ni::GetLang()
                 ->Eval("StrAfter('$2a$08$VXQoSyZF288.82vVBGuEN.','.')")
                 ->GetString();
  TEST_DEBUGFMT("r4: %s", r4);
...
{
  tInt r1 = (tInt)ni::GetLang()
              ->Eval("StrFind('$2a$08$VXQoSyZF288.82vVBGuEN.','$08')")
              ->GetFloat();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(3, r1);

  tInt r2 = (tInt)ni::GetLang()
              ->Eval("StrFind('$2a$08$VXQoSyZF288.82vVBGuEN.','$')")
              ->GetFloat();
  TEST_DEBUGFMT("r2: %s", r2);
  CHECK_EQUAL(0, r2);

  tInt r3 = (tInt)ni::GetLang()
              ->Eval("StrRFind('$2a$08$VXQoSyZF288.82vVBGuEN.','$')")
              ->GetFloat();
  TEST_DEBUGFMT("r3: %s", r3);
  CHECK_EQUAL(6, r3);

  tInt r4 = (tInt)ni::GetLang()
              ->Eval("StrRFind('$2a$08$VXQoSyZF288.82vVBGuEN.','marcel')")
              ->GetFloat();
  TEST_DEBUGFMT("r4: %s", r4);
  CHECK_EQUAL(-1, r4);
}
...
{
  cString r1 = ni::GetLang()
                 ->Eval("StrToLower('$2a$08$VXQoSyZF288.82vVBGuEN.')")
                 ->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(_ASTR("$2a$08$vxqosyzf288.82vvbguen."), r1);

  cString r2 = ni::GetLang()
                 ->Eval("StrToUpper('$2a$08$VXQoSyZF288.82vVBGuEN.')")
                 ->GetString();
  TEST_DEBUGFMT("r2: %s", r2);
  CHECK_EQUAL(_ASTR("$2A$08$VXQOSYZF288.82VVBGUEN."), r2);
}
...
{
  cString r1 =
    ni::GetLang()->Eval("StrTrim('  Hello   World    ')")->GetString();
  TEST_DEBUGFMT("r1: %s", r1);
  CHECK_EQUAL(_ASTR("Hello   World"), r1);

  cString r2 =
    ni::GetLang()->Eval("StrNormalize('  Hello   World    ')")->GetString();
  TEST_DEBUGFMT("r2: %s", r2);
...
TEST_FIXTURE(FExpression, Set)
{
  ...
  {
    const tF64 v = ni::GetLang()
                     ->Eval("Set('x',123,'y',2000,Get('x')+Get('y'))")
                     ->GetFloat();
  ...
  {
    const tF64 v = ni::GetLang()
                     ->Eval("Set('x',456,'y',3000,Get('x')+Get('y'))")
                     ->GetFloat();
...
TEST_FIXTURE(FExpression, IfSet)
{
  ...
  {
    const achar* exprStr =
      "Set('i', iota, If(Get('i') > 4, '>4', Get('i') == 4, 'is4', Get('i') == 3, 'is3', '<=2'))";
    Ptr<iExpression> expr = ctx->CreateExpression(exprStr);
    {
      const tF64 v = ctx->Eval("iota")->GetFloat();
      niDebugFmt(("iota = %g", v));
...
{
  const cString v =
    ni::GetLang()->Eval("StrAfter('Container-02C-G1-FHD01','-')")->GetString();
...
TEST_FIXTURE(FExpression, Quat)
{
  ...
  Ptr<iExpression> expr =
    ptrCtx->CreateExpression("QuatMul(QuatIdentity(),QuatIdentity())");
  CHECK_RETURN_IF_FAILED(expr.IsOK());
  ...
  CHECK_EQUAL(eTrue,
              niFlagIs(result->GetFlags(), eExpressionVariableFlags_Quat));
}
