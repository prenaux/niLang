...
class line_image_scale {
 public:
  ...
  line_image_scale(const Source& src, agg_real height)
      : m_source(src)
      , m_height(height)
  ...
  {
    agg_real src_y = (y + 0.5) * m_scale - 0.5;
...
class line_image_pattern {
 public:
  ...
  void create(const Source& src)
  {
    ...
    m_buf.attach(&m_data[0], m_width + m_dilation * 2,
                 m_height + m_dilation * 2, m_width + m_dilation * 2);
    unsigned x, y;
    color_type* d1;
    color_type* d2;
 ...
 private:
  ...
  const line_image_pattern<filter_type>& operator=(
...
class line_image_pattern_pow2 : public line_image_pattern<Filter> {
 public:
  ...
  void pixel(color_type* p, int x, int y) const
  {
    base_type::m_filter->pixel_high_res(base_type::m_buf.rows(), p,
...
class distance_interpolator4 {
 public:
  ...
  distance_interpolator4(int x1, int y1, int x2, int y2, int sx, int sy, int ex,
                         int ey, int len, agg_real scale, int x, int y)
      : m_dx(x2 - x1)
      , m_dy(y2 - y1)
...
class line_interpolator_image {
 public:
  ...
  //---------------------------------------------------------------------
  line_interpolator_image(renderer_type& ren, const line_parameters& lp, int sx,
                          int sy, int ex, int ey, int pattern_start,
                          agg_real scale_x)
      : m_lp(lp)
 ...
 private:
  ...
  const line_interpolator_image<Renderer>& operator=(
...
class renderer_outline_image {
 public:
  ...
  //---------------------------------------------------------------------
  void start_x(agg_real s)
  {
    m_start = iround(s * line_subpixel_scale);
  }
  agg_real start_x() const
  {
    return agg_real(m_start) / line_subpixel_scale;
  }
  ...
  agg_real width() const
  {
    return agg_real(subpixel_width()) / line_subpixel_scale;
  }
  ...
  void line3_no_clip(const line_parameters& lp, int sx, int sy, int ex, int ey)
  {
    ...
    line_interpolator_image<self_type> li(*this, lp, sx, sy, ex, ey, m_start,
                                          m_scale_x);
    if (li.vertical()) {
      while (li.step_ver())
        ;
