...
///
  ...
  #define EASTL_TEMP_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX     \
      " temp" // Unless the user overrides something, this is "EASTL temp".
#endif
...
///
template <typename T>
eastl::pair<T*, ptrdiff_t> get_temporary_buffer(
  ptrdiff_t n, size_t alignment = 1, size_t alignmentOffset = 0,
  const char* pName = EASTL_TEMP_DEFAULT_NAME)
{
  EASTLAllocatorType allocator(*EASTLAllocatorDefault(), pName);
  return eastl::pair<T*, ptrdiff_t>(
    static_cast<T*>(
      EASTLAllocAligned(allocator, n * sizeof(T), alignment, alignmentOffset)),
    n);
}
...
class late_constructed {
 public:
  ...
  using value_type = T;
  using storage_type =
    eastl::aligned_storage_t<sizeof(value_type),
                             eastl::alignment_of_v<value_type>>;

  late_constructed()
    EA_NOEXCEPT // In the case of the late_constructed instance being at global scope, we rely on the
      : mpValue(nullptr)
  {
 ...
 protected:
  storage_type
    mStorage; // Declared first because it may have aligment requirements, and it would be more space-efficient if it was first.
  value_type* mpValue;
};
...
class late_constructed<T, false> : public late_constructed<T, true> {
 public:
  ...
  {
    EASTL_ASSERT(base_type::mpValue);
...
class raw_storage_iterator : public iterator<EASTL_ITC_NS::output_iterator_tag,
                                             void, void, void, void> {
 protected:
...
struct uninitialized_relocate_impl {
  ...
  static ForwardIteratorDest do_move_start(ForwardIterator first,
  ...
  template <typename ForwardIterator, typename ForwardIteratorDest>
  static ForwardIteratorDest do_move_commit(ForwardIterator first,
                                            ForwardIterator last,
                                            ForwardIteratorDest dest) //throw()
  {
    typedef
      typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
    for (; first != last; ++first, ++dest)
  ...
  template <typename ForwardIterator, typename ForwardIteratorDest>
  static ForwardIteratorDest do_move_abort(ForwardIterator first,
                                           ForwardIterator last,
                                           ForwardIteratorDest dest) //throw()
  {
    typedef
...
///
template <typename ForwardIterator, typename ForwardIteratorDest>
inline ForwardIteratorDest uninitialized_relocate_start(
  ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
{
  typedef
  ...
  typedef typename eastl::iterator_traits<ForwardIterator>::value_type
    value_type_input;
  typedef typename eastl::iterator_traits<ForwardIteratorDest>::value_type
    value_type_output;

  const bool bHasTrivialMove =
    type_and<has_trivial_relocate<value_type_input>::value,
  ...
  return Internal::uninitialized_relocate_impl<bHasTrivialMove,
                                               IC>::do_move_start(first, last,
                                                                  dest);
}

template <typename ForwardIterator, typename ForwardIteratorDest>
inline ForwardIteratorDest uninitialized_relocate_commit(
  ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
  ...
  return Internal::uninitialized_relocate_impl<bHasTrivialMove,
                                               IC>::do_move_commit(first, last,
                                                                   dest);
}

template <typename ForwardIterator, typename ForwardIteratorDest>
inline ForwardIteratorDest uninitialized_relocate_abort(
  ForwardIterator first, ForwardIterator last, ForwardIteratorDest dest)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::iterator_category IC;
...
///
template <typename ForwardIterator, typename ForwardIteratorDest>
inline ForwardIteratorDest uninitialized_relocate(ForwardIterator first,
                                                  ForwardIterator last,
                                                  ForwardIteratorDest dest)
{
...
inline ForwardIterator uninitialized_copy_impl(InputIterator first,
                                               InputIterator last,
                                               ForwardIterator dest, true_type)
{
  return eastl::copy(first, last,
                     dest); // The copy() in turn will use memcpy for POD types.
}

template <typename InputIterator, typename ForwardIterator>
inline ForwardIterator uninitialized_copy_impl(InputIterator first,
                                               InputIterator last,
                                               ForwardIterator dest, false_type)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
  ...
  try {
    ...
    for (; first != last; ++first, ++currentDest)
      ::new (static_cast<void*>(eastl::addressof(*currentDest)))
        value_type(*first);
#if EASTL_EXCEPTIONS_ENABLED
  } catch (...) {
                                          ...
                                          ForwardIterator result)
{
  ...
  return Internal::uninitialized_copy_impl(first, last, result,
                                           eastl::is_trivial<value_type>());
}
...
struct uninitialized_copy_n_impl {
  static ForwardIterator impl(InputIterator first, Count n,
...
template <typename InputIterator, typename Count, typename ForwardIterator>
struct uninitialized_copy_n_impl<InputIterator, Count, ForwardIterator,
                                 EASTL_ITC_NS::random_access_iterator_tag> {
  static inline ForwardIterator impl(InputIterator first, Count n,
                                     ForwardIterator dest)
  {
...
template <typename InputIterator, typename Count, typename ForwardIterator>
inline ForwardIterator uninitialized_copy_n(InputIterator first, Count n,
                                            ForwardIterator dest)
{
  typedef typename eastl::iterator_traits<InputIterator>::iterator_category IC;
  return Internal::uninitialized_copy_n_impl<InputIterator, Count,
                                             ForwardIterator, IC>::impl(first,
                                                                        n,
                                                                        dest);
}
...
{
  typedef
    typename eastl::iterator_traits<generic_iterator<Result, void>>::value_type
      value_type;
  const generic_iterator<Result, void> i(Internal::uninitialized_copy_impl(
    eastl::generic_iterator<First, void>(
      first), // generic_iterator makes a pointer act like an iterator.
...
namespace Internal {
template <typename InputIterator, typename ForwardIterator>
inline ForwardIterator uninitialized_move_impl(InputIterator first,
                                               InputIterator last,
                                               ForwardIterator dest, true_type)
{
  return eastl::copy(
    first, last,
    dest); // The copy() in turn will use memcpy for is_trivially_copy_assignable (e.g. POD) types.
}
...
#endif
    for (; first != last; ++first, ++currentDest)
      ::new ((void*)eastl::addressof(*currentDest)) value_type(eastl::move(
        *first)); // If value_type has a move constructor then it will be used here.
#if EASTL_EXCEPTIONS_ENABLED
  } catch (...) {
...
inline Result uninitialized_move_ptr(First first, Last last, Result dest)
{
  typedef
    typename eastl::iterator_traits<generic_iterator<Result, void>>::value_type
      value_type;
  const generic_iterator<Result, void> i(Internal::uninitialized_move_impl(
    eastl::generic_iterator<First, void>(
      first), // generic_iterator makes a pointer act like an iterator.
    eastl::generic_iterator<Last, void>(last),
    eastl::generic_iterator<Result, void>(dest),
    eastl::is_trivially_copy_assignable<
      value_type>())); // is_trivially_copy_assignable identifies if copy assignment would be as valid as move assignment, which means we have the opportunity to memcpy/memmove optimization.
  return i.base();
}
...
inline ForwardIterator uninitialized_move(InputIterator first,
                                          ...
                                          ForwardIterator dest)
{
  return eastl::uninitialized_copy(eastl::make_move_iterator(first),
                                   eastl::make_move_iterator(last), dest);
}
...
///
template <typename InputIterator, typename ForwardIterator>
inline ForwardIterator uninitialized_move_if_noexcept(InputIterator first,
                                                      InputIterator last,
                                                      ForwardIterator dest)
{
  return eastl::uninitialized_copy(eastl::make_move_if_noexcept_iterator(first),
                                   eastl::make_move_if_noexcept_iterator(last),
                                   dest);
}
...
///
template <typename First, typename Last, typename Result>
inline Result uninitialized_move_ptr_if_noexcept(First first, Last last,
                                                 Result dest)
{
#if EASTL_EXCEPTIONS_ENABLED
...
///
template <typename InputIterator, typename Count, typename ForwardIterator>
inline ForwardIterator uninitialized_move_n(InputIterator first, Count n,
                                            ForwardIterator dest)
{
...
inline void uninitialized_default_fill_n_impl(ForwardIterator first, Count n,
                                              false_type)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
...
inline void uninitialized_default_fill_n(ForwardIterator first, Count n)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
  Internal::uninitialized_default_fill_n_impl(first, n,
...
///
template <typename ForwardIterator>
inline void uninitialized_default_construct(ForwardIterator first,
                                            ForwardIterator last)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
...
///
template <typename ForwardIterator, typename Count>
inline ForwardIterator uninitialized_default_construct_n(ForwardIterator first,
                                                         Count n)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
                               ...
                               const T& value)
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
  Internal::uninitialized_fill_impl(
    first, last, value, eastl::is_trivially_copy_assignable<value_type>());
}
...
inline void uninitialized_fill_ptr(T* first, T* last, const T& value)
{
  typedef typename eastl::iterator_traits<
    eastl::generic_iterator<T*, void>>::value_type value_type;
  Internal::uninitialized_fill_impl(
    eastl::generic_iterator<T*, void>(first),
...
namespace Internal {
template <typename ForwardIterator, typename Count, typename T>
inline void uninitialized_fill_n_impl(ForwardIterator first, Count n,
                                      const T& value, true_type)
{
...
inline void uninitialized_fill_n(ForwardIterator first, Count n, const T& value)
{
  ...
  Internal::uninitialized_fill_n_impl(
    first, n, value, eastl::is_trivially_copy_assignable<value_type>());
}
...
{
  typedef
    typename eastl::iterator_traits<generic_iterator<T*, void>>::value_type
      value_type;
  Internal::uninitialized_fill_n_impl(
    eastl::generic_iterator<T*, void>(first), n, value,
    eastl::is_trivially_copy_assignable<value_type>());
}
...
//
template <typename ForwardIterator>
inline void destruct_impl(
  ForwardIterator /*first*/, ForwardIterator /*last*/,
  true_type) // true means the type has a trivial destructor.
{
...
inline void destruct_impl(
  ForwardIterator first, ForwardIterator last,
  false_type) // false means the type has a significant destructor.
{
  typedef
    typename eastl::iterator_traits<ForwardIterator>::value_type value_type;
...
inline void* align(size_t alignment, size_t size, void*& ptr, size_t& space)
{
  if (space >= size) {
    ...
    if (
      (space - size) >=
      offset) // Have to implement this in terms of subtraction instead of addition in order to handle possible overflow.
    {
      ptr = ptrAligned;
      space -= offset;
...
inline void* align_advance(size_t alignment, size_t size, void* ptr,
                           size_t space, void** ptrAdvanced = NULL,
...
template <typename T, typename Allocator,
          bool = has_allocator_type_helper<T>::value>
struct uses_allocator_impl
    : public integral_constant<
        bool,
        eastl::is_convertible<Allocator, typename T::allocator_type>::value> {};
...
template <typename Pointer>
struct
  has_element_type // has_element_type<T>::value is true if T has an element_type member typedef.
{
 private:
  template <typename U>
  static eastl::no_type test(...);
  template <typename U>
  static eastl::yes_type test(typename U::element_type* = 0);
...
template <typename Pointer>
struct
  has_difference_type // has_difference_type<T>::value is true if T has an difference_type member typedef.
{
 private:
  template <typename U>
  static eastl::no_type test(...);
  template <typename U>
  static eastl::yes_type test(typename U::difference_type* = 0);

 public:
  static const bool value =
    sizeof((test<Pointer>(0))) == sizeof(eastl::yes_type);
};
...
struct pointer_traits {
  ...
  typedef
    typename Internal::pointer_difference_type<pointer>::type difference_type;
 ...
 public:
  static pointer pointer_to(
    typename eastl::conditional<eastl::is_void<element_type>::value, void,
                                element_type>::type&
      r) // 20.6.3.2: if element_type is (possibly cv-qualified) void, the type of r is unspecified; otherwise, it is T&.
  {
...
struct pointer_traits<T*> {
 ...
 public:
  static pointer pointer_to(
    typename eastl::conditional<eastl::is_void<element_type>::value, void,
                                element_type>::type& r) EA_NOEXCEPT
  {
