...
struct iCommandSink : public iUnknown {
  ...
  virtual tBool __stdcall OnRun(const tStringCVec* avArgs,
                                iConsole* apConsole) = 0;
};
...
#define niDeclareCommand(ParentType, Namespace, Name, Desc)                \
  class cCommandSink_##Namespace##Name : public ImplRC<ni::iCommandSink> { \
   public:                                                                 \
    cCommandSink_##Namespace##Name(ParentType* pParent)                    \
    {                                                                      \
      mpParent = pParent;                                                  \
    }                                                                      \
    const ni::achar* __stdcall GetName() const                             \
    {                                                                      \
      return _A(#Name);                                                    \
    }                                                                      \
    const ni::achar* __stdcall GetNamespace() const                        \
    {                                                                      \
      return _A(#Namespace);                                               \
    }                                                                      \
    const ni::achar* __stdcall GetDescription() const                      \
    {                                                                      \
      return Desc;                                                         \
    }                                                                      \
    ni::tBool __stdcall OnRun(const ni::tStringCVec* avArgs,               \
                              ni::iConsole* apConsole);                    \
    ParentType* __stdcall GetParent()                                      \
    {                                                                      \
      return mpParent;                                                     \
    }                                                                      \
    const ParentType* __stdcall GetParent() const                          \
    {                                                                      \
      return mpParent;                                                     \
    }                                                                      \
                                                                           \
...
#define niDeclareGlobalCommand(Name, Desc)                      \
  class cCommandSink_##Name : public ImplRC<ni::iCommandSink> { \
   public:                                                      \
    cCommandSink_##Name(ni::iConsole* pParent)                  \
    {                                                           \
      mpParent = pParent;                                       \
    }                                                           \
    const ni::achar* __stdcall GetName() const                  \
    ...
    ni::tBool __stdcall OnRun(const ni::tStringCVec* avArgs,    \
                              ni::iConsole* apConsole);         \
    ni::iConsole* __stdcall GetParent()                         \
    {                                                           \
      return mpParent;                                          \
    }                                                           \
    const ni::iConsole* __stdcall GetParent() const             \
    {                                                           \
      return mpParent;                                          \
    }                                                           \
                                                                \
...
#define niImplementCommand(Namespace, Name)                  \
  ni::tBool __stdcall cCommandSink_##Namespace##Name::OnRun( \
    const ni::tStringCVec* avArgs, ni::iConsole* apConsole)

//! Macro that help to declare the run command.
#define niImplementGlobalCommand(Name)            \
  ni::tBool __stdcall cCommandSink_##Name::OnRun( \
    const ni::tStringCVec* avArgs, ni::iConsole* apConsole)
...
#define niCommandArg(IDX) \
  ((avArgs != NULL && IDX < avArgs->size()) ? (*avArgs)[IDX] : ni::cString())
...
struct iConsoleSink : public iUnknown {
  ...
  virtual void __stdcall OnConsoleSink_NamespaceVariableChanged(
    const achar* aNamespace, const achar* aaszName, const achar* aaszValue) = 0;
  //! Called before a console command is ran.
  //! \return false to disallow the command run
  virtual tBool __stdcall OnConsoleSink_BeforeRunCommand(
    const achar* aaszCmd) = 0;
  //! Called after a console command is ran.
  virtual void __stdcall OnConsoleSink_AfterRunCommand(
    const achar* aaszCmd) = 0;
  //! Called before a script command is ran.
  //! \return false to disallow the script run
  virtual tBool __stdcall OnConsoleSink_BeforeRunScript(
    iScriptingHost* apHost, const achar* aaszCmd) = 0;
  //! Called after a script command is ran.
  virtual void __stdcall OnConsoleSink_AfterRunScript(iScriptingHost* apHost,
                                                      const achar* aaszCmd) = 0;
};
...
struct iConsole : public iUnknown {
  ...
  //!     else eTrue.
  virtual tBool __stdcall AddVariable(const achar* aszName,
                                      const achar* aszValue = NULL) = 0;
  //! Remove a variable.
  ...
  virtual tBool __stdcall SetVariable(const achar* aszName,
                                      const achar* aszValue) = 0;
  ...
  virtual cString __stdcall GetCommandDescription(
    const achar* aszCommand) const = 0;
  //! Run a command.
  ...
  virtual cString __stdcall CompleteCommandLine(const achar* aaszCmd,
                                                tBool abNext = eTrue) = 0;
