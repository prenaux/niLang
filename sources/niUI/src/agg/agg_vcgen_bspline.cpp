...
//------------------------------------------------------------------------
void vcgen_bspline::add_vertex(agg_real x, agg_real y, unsigned cmd)
{
  m_status = initial;
  if (is_move_to(cmd)) {
...
void vcgen_bspline::rewind(unsigned)
{
  ...
  if (m_status == initial && m_src_vertices.size() > 2) {
    if (m_closed) {
      ...
      m_spline_x.add_point(0.0,
                           m_src_vertices.prev(m_src_vertices.size() - 3).x);
      m_spline_y.add_point(0.0,
                           m_src_vertices.prev(m_src_vertices.size() - 3).y);
...
//------------------------------------------------------------------------
unsigned vcgen_bspline::vertex(agg_real* x, agg_real* y)
{
