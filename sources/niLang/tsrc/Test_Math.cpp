...
TEST_FIXTURE(FMath, ProbRaw)
{
  ...
  ProbSampleBuildAliasMethodArrays(probs->data(), probs->size(), amq->data(),
                                   ama->data());
  ...
  ProbSampleAliasMethod(r->data(), knPickCount, amq->data(), ama->data(),
                        probs->size(), [&]() { return ni::RandFloat(&prng); });
  ...
  niLoop (i, eProbability_Last) {
    ...
    niDebugFmt(
      ("... eProbability[%d]: %d, %g%%", i, pickedCount[i], pickedPct[i]));
  }
