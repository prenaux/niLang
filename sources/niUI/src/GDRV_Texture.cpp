...
///////////////////////////////////////////////
tBool __stdcall cGraphics::CheckTextureFormat(iBitmapFormat* apFormat,
...
///////////////////////////////////////////////
iDeviceResourceManager* __stdcall cGraphics::GetTextureDeviceResourceManager()
  const
{
...
///////////////////////////////////////////////
iTexture* __stdcall cGraphics::CreateTexture(iHString* ahspName,
                                             eBitmapType aType,
...
{
  CHECKDRIVER(NULL);
  return mptrDrv->CreateTexture(ahspName, aType, aaszFormat, anNumMipMaps,
                                anWidth, anHeight, anDepth, aFlags);
}

///////////////////////////////////////////////
iTexture* __stdcall cGraphics::CreateTextureFromBitmap(iHString* ahspName,
                                                       ...
                                                       tTextureFlags aFlags)
{
...

  {
    ...
    if (ptrTex.IsOK()) {
      if (ptrTex->GetType() != ptrBmp->GetType()) {
        niError(
          niFmt(_A("Texture '%s' already created, but not of the same type."),
                ahspName));
        return NULL;
      }
      else {
        niWarning(
          niFmt(_A("Texture '%s' already created, not reloaded."), ahspName));
  ...
  if (!apBitmap->GetNumMipMaps()) {
    ...
    niWarning(niFmt(
      ...
      apBitmap->GetNumMipMaps(), ahspName,
      apBitmap->GetPixelFormat()->GetFormat(), apBitmap->GetWidth(),
      apBitmap->GetHeight(), apBitmap->GetDepth()));
  }
  ...
  if (!niFlagIs(aFlags, eTextureFlags_Overlay) && !apBitmap->GetNumMipMaps()) {
    niWarning(niFmt("Automatically computing mipmaps for '%s' (%dx%dx%d %s).",
                    ahspName, ptrBmp->GetWidth(), ptrBmp->GetHeight(),
                    ptrBmp->GetDepth(), ptrBmp->GetPixelFormat()->GetFormat()));
  ...
  Ptr<iTexture> ptrTex = CreateTexture(
    ahspName, ptrBmp->GetType(), ptrBmp->GetPixelFormat()->GetFormat(),
    ptrBmp->GetNumMipMaps(), ptrBmp->GetWidth(), ptrBmp->GetHeight(),
    ptrBmp->GetDepth(), aFlags);
  ...
  case eBitmapType_Cube: {
    ...
    for (tU32 i = 0; i < 6; ++i) {
      ...
      if (!BlitBitmapToTexture(ptrBmpCube->GetFace((eBitmapCubeFace)i),
                               ptrFaceTex, eInvalidHandle))
      {
        niError(
          niFmt(_A("Can't blit the bitmap to the texture face '%d' !."), i));
        return NULL;
      }
    }
    break;
...
///////////////////////////////////////////////
iTexture* __stdcall cGraphics::CreateTextureFromRes(iHString* ahspRes,
                                                    ...
                                                    tTextureFlags aFlags)
{
          ...
          if (ptrFP->GetTime(eFileTime_LastWrite, ptrFileTime)) {
            if (ptrPrevTime->Compare(ptrFileTime) != 0) {
              niLog(
                Info,
                niFmt(
                  "Already loaded texture '%s' but it has changed, it will be reloaded.",
                  ahspRes));
              Ptr<iDeviceResourceManager> texResMan =
                this->GetTextureDeviceResourceManager();
  ...
  if (Video_FileNameIsVideo(niHStr(ahspRes))) {
    ...
    if (!this->IsVideoFile(fp)) {
      niWarning(
        niFmt(_A("File '%s' is not a supported video format !"), ahspRes));
      return NULL;
    }

    Ptr<iVideoDecoder> ptrDecoder =
      this->CreateVideoDecoder(ahspRes, fp, eVideoDecoderFlags_TargetTexture);
    if (!niIsOK(ptrDecoder)) {
      niWarning(
        niFmt(_A("Creation of the video decoder for '%s' failed !"), ahspRes));
      return NULL;
    }
    ...
    if (!ptrTex.IsOK()) {
      niWarning(niFmt(
        _A(
          "Retrieving the initial target of the video decoder for '%s' failed !"),
        ahspRes));
      return NULL;
    }
#else
    ...
    if (!ptrTex.IsOK()) {
      niError(
        niFmt("Can't create texture from bitmap resource '%s'.", ahspRes));
      return NULL;
    }
...
///////////////////////////////////////////////
tBool __stdcall cGraphics::BlitBitmapToTexture(
  ...
  const sRecti& aDestRect, eTextureBlitFlags aFlags)
{
  ...
  tBool bRet = eTrue;
  bRet = mptrDrv->BlitBitmapToTexture(
    apSrc, apDest, (anDestLevel == eInvalidHandle) ? 0 : anDestLevel, srcRect,
    dstRect, aFlags);
      ...
      (nNumMips && !niFlagIs(apDest->GetFlags(), eTextureFlags_AutoGenMipMaps)))
  {
    for (tU32 i = 0; i < nNumMips; ++i) {
      ...
      BlitBitmapToTexture((iBitmap2D*)apSrc->GetMipMap(i), apDest, i + 1,
                          srcRect, dstRect, aFlags);
    }
  }
...
///////////////////////////////////////////////
tBool __stdcall cGraphics::BlitTextureToBitmap(iTexture* apSrc, tU32 anSrcLevel,
                                               iBitmap2D* apDest,
                                               ...
                                               eTextureBlitFlags aFlags)
{
  ...
  tBool bRet = eTrue;
  bRet = mptrDrv->BlitTextureToBitmap(
    apSrc, (anSrcLevel == eInvalidHandle) ? 0 : anSrcLevel, apDest, srcRect,
    dstRect, aFlags);
  if (bRet && anSrcLevel == eInvalidHandle && nNumMips) {
    for (tU32 i = 0; i < nNumMips; ++i) {
      srcRect = _NextMip(srcRect);
      dstRect = _NextMip(dstRect);
      BlitTextureToBitmap(apSrc, i + 1, (iBitmap2D*)apDest->GetMipMap(i),
                          srcRect, dstRect, aFlags);
    }
  }
  return bRet;
...
///////////////////////////////////////////////
tBool __stdcall cGraphics::BlitTextureToTexture(
  ...
  const sRecti& aSrcRect, const sRecti& aDestRect, eTextureBlitFlags aFlags)
{
  ...
  bRet = mptrDrv->BlitTextureToTexture(apSrc, bBlitAll ? 0 : anSrcLevel, apDest,
                                       bBlitAll ? 0 : anDestLevel, srcRect,
                                       dstRect, aFlags);
      ...
      !niFlagIs(apDest->GetFlags(), eTextureFlags_AutoGenMipMaps))
  {
    ...
    for (tU32 i = 0; i < nNumMips; ++i) {
      srcRect = _NextMip(srcRect);
      dstRect = _NextMip(dstRect);
      BlitTextureToTexture(apSrc, i + 1, apDest, i + 1, srcRect, dstRect,
                           aFlags);
    }
  }
  return bRet;
...
///////////////////////////////////////////////
tBool __stdcall cGraphics::BlitBitmap3DToTexture(
  ...
  const sVec3i& aDestMin, const sVec3i& avSize, eTextureBlitFlags aFlags)
{
  ...
  bRet = mptrDrv->BlitBitmap3DToTexture(
    apSrc, apDest, bBlitAll ? 0 : anDestLevel, srcP, dstP, blitSize, aFlags);
  if (bRet && bBlitAll && nNumMips &&
      !niFlagIs(apDest->GetFlags(), eTextureFlags_AutoGenMipMaps))
  {
    for (tU32 i = 0; i < nNumMips; ++i) {
...
///////////////////////////////////////////////
tBool __stdcall cGraphics::BlitTextureToBitmap3D(
  ...
  const sVec3i& aDestMin, const sVec3i& avSize, eTextureBlitFlags aFlags)
{
  ...
  bRet = mptrDrv->BlitTextureToBitmap3D(apSrc, bBlitAll ? 0 : anSrcLevel,
                                        apDest, srcP, dstP, blitSize, aFlags);
  if (bRet && bBlitAll && nNumMips) {
    for (tU32 i = 0; i < nNumMips; ++i) {
      srcP = _NextMip(srcP);
      dstP = _NextMip(dstP);
      blitSize = _NextMip(blitSize);
      BlitTextureToBitmap3D(apSrc, i + 1, apDest->GetMipMap(i), srcP, dstP,
                            blitSize, aFlags);
    }
  }
  return bRet;
...
iBitmapBase* __stdcall cGraphics::CreateBitmapFromTexture(iTexture* apSrc)
{
  ...
  case eBitmapType_2D: {
    QPtr<iBitmap2D> bmp2d = CreateBitmap2DEx(
      apSrc->GetWidth(), apSrc->GetHeight(), apSrc->GetPixelFormat()->Clone());
  ...
  case eBitmapType_3D: {
    QPtr<iBitmap3D> bmp3d =
      CreateBitmap3DEx(apSrc->GetWidth(), apSrc->GetHeight(), apSrc->GetDepth(),
                       apSrc->GetPixelFormat()->Clone());
  ...
  case eBitmapType_Cube: {
    QPtr<iBitmapCube> bmpCube =
      CreateBitmapCubeEx(apSrc->GetWidth(), apSrc->GetPixelFormat()->Clone());
