...
int CaptureClass::scanMediaTypes(unsigned int aWidth, unsigned int aHeight)
{
	...
	while (nativeTypeErrorCode == S_OK && besterror)
	{
      ...
      niDebugFmt((
        "webcam mode[%d], res: %d x %d, guid: %x:%x:%x:%x, bestfit: %d, besterror: %d\n",
        count, width, height,
        nativeGuid.Data1, nativeGuid.Data2, nativeGuid.Data3, nativeGuid.Data4,
        (bestfit == count), besterror));
		}
...
HRESULT CaptureClass::initCapture(int aDevice, int aPreferredMode)
{
	mWhoAmI = aDevice;
  ...
  ChooseDeviceParam param = { 0 };
  {
	...
	if ((signed)param.mCount > aDevice)
	{
		...
		int preferredmode = aPreferredMode;
    if (preferredmode < 0) {
