...
class segmented_vector {
 public:
  ...
  void erase_unsorted(segment_type& segment,
                      typename segment_type::iterator it);
...
inline const segment<T, Count, Allocator>* segment<
  T, Count, Allocator>::next_segment() const
{
...
template <typename T, size_t Count, typename Allocator>
inline segment<T, Count, Allocator>* segment<T, Count,
                                             Allocator>::next_segment()
{
  if (mPrev & kIsLastSegment)
    return 0;
  else
...
template <typename T, size_t Count, typename Allocator>
inline typename segment<T, Count, Allocator>::const_iterator segment<
  T, Count, Allocator>::begin() const
{
  return mData;
}

template <typename T, size_t Count, typename Allocator>
inline typename segment<T, Count, Allocator>::iterator segment<
  T, Count, Allocator>::begin()
{
  return mData;
}
...
template <typename T, size_t Count, typename Allocator>
inline typename segment<T, Count, Allocator>::iterator segment<T, Count,
                                                               Allocator>::end()
{
  if (mPrev & kIsLastSegment)
    return mData + mSize;
...
template <typename T, size_t Count, typename Allocator>
T* segmented_vector_iterator<T, Count, Allocator>::operator->() const
{
  return mCurrent;
}
...
template <typename T, size_t Count, typename Allocator>
segmented_vector_iterator<T, Count, Allocator>& segmented_vector_iterator<
  T, Count, Allocator>::operator++()
{
  ++mCurrent;
...
template <typename T, size_t Count, typename Allocator>
segmented_vector_iterator<T, Count, Allocator> segmented_vector_iterator<
  T, Count, Allocator>::operator++(int)
{
  this_type i(*this);
  return ++i;
}
...
template <typename T, size_t Count, typename Allocator>
inline T& segmented_vector<T, Count, Allocator>::front()
{
  return mFirstSegment->mData[0];
}

template <typename T, size_t Count, typename Allocator>
inline T& segmented_vector<T, Count, Allocator>::back()
{
...
template <typename T, size_t Count, typename Allocator>
inline bool segmented_vector<T, Count, Allocator>::empty() const
{
...
inline void segmented_vector<T, Count, Allocator>::clear()
{
  if (segment_type* segment = mFirstSegment) {
    ...
    for (T *i = segment->mData, *e = segment->mData + segment->mSize; i != e;
         ++i)
      i->~T();
    EASTLFree(mAllocator, segment, sizeof(segment_type));
    mFirstSegment = 0;
...
template <typename T, size_t Count, typename Allocator>
inline T& segmented_vector<T, Count, Allocator>::push_back()
{
  return *(new (DoPushBack()) T());
}

template <typename T, size_t Count, typename Allocator>
inline T& segmented_vector<T, Count, Allocator>::push_back(const T& value)
{
  return *(new (DoPushBack()) T(value));
}

template <typename T, size_t Count, typename Allocator>
inline void* segmented_vector<T, Count, Allocator>::push_back_uninitialized()
{
  return DoPushBack();
}

template <typename T, size_t Count, typename Allocator>
inline void segmented_vector<T, Count, Allocator>::pop_back()
{
  segment_type* lastSegment = mLastSegment;
  ...
  if (!lastSegment->mSize) {
    --mSegmentCount;
    mLastSegment =
      (segment_type*)(lastSegment->mPrev & (~segment_type::kIsLastSegment));
...
template <typename T, size_t Count, typename Allocator>
inline void segmented_vector<T, Count, Allocator>::erase_unsorted(
  segment_type& segment, typename segment_type::iterator it)
{
  EA_UNUSED(segment);
...
template <typename T, size_t Count, typename Allocator>
void segmented_vector<T, Count, Allocator>::swap(this_type& other)
{
...
template <typename T, size_t Count, typename Allocator>
segment<T, Count, Allocator>* segmented_vector<
  T, Count, Allocator>::DoAllocSegment(segment_type* prevSegment)
{
  ++mSegmentCount;
  segment_type* segment = (segment_type*)allocate_memory(
    mAllocator, sizeof(segment_type), EASTL_ALIGN_OF(segment_type), 0);
...
template <typename T, size_t Count, typename Allocator>
inline void* segmented_vector<T, Count, Allocator>::DoPushBack()
{
  if (segment_type* segment = mLastSegment) {
...
inline bool operator==(
  const segmented_vector_iterator<const T, Count, Allocator>& a,
  const segmented_vector_iterator<const T, Count, Allocator>& b)
{
...
template <typename T, size_t Count, typename Allocator>
inline bool operator==(const segmented_vector_iterator<T, Count, Allocator>& a,
                       const segmented_vector_iterator<T, Count, Allocator>& b)
{
  return a.mCurrent == b.mCurrent;
}
