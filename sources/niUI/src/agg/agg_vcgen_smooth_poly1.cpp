...
//------------------------------------------------------------------------
void vcgen_smooth_poly1::add_vertex(agg_real x, agg_real y, unsigned cmd)
{
  m_status = initial;
  if (is_move_to(cmd)) {
...
//------------------------------------------------------------------------
void vcgen_smooth_poly1::calculate(const vertex_dist& v0, const vertex_dist& v1,
                                   const vertex_dist& v2, const vertex_dist& v3)
{

  agg_real k1 = v0.dist / (v0.dist + v1.dist);
  agg_real k2 = v1.dist / (v1.dist + v2.dist);

  agg_real xm1 = v0.x + (v2.x - v0.x) * k1;
  agg_real ym1 = v0.y + (v2.y - v0.y) * k1;
  agg_real xm2 = v1.x + (v3.x - v1.x) * k2;
  agg_real ym2 = v1.y + (v3.y - v1.y) * k2;
...
//------------------------------------------------------------------------
unsigned vcgen_smooth_poly1::vertex(agg_real* x, agg_real* y)
{
  unsigned cmd = path_cmd_line_to;
  while (!is_stop(cmd)) {
    switch (m_status) {
    case initial: rewind(0);
    ...
    case end_poly: m_status = stop; return path_cmd_end_poly | m_closed;

    case stop: return path_cmd_stop;
    }
  }
  return cmd;
}
