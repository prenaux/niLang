...
struct sScrollBarThumbMetrics {
  ...
  sScrollBarThumbMetrics(cScrollBarWidget* apScrollBar, const tF32 w,
                         const tF32 h)
  {
    ...
    if (!bSlider) {
      ...
      tsize = ni::Clamp(
        ni::FDiv(realsize, ((apScrollBar->mfMax - apScrollBar->mfMin) /
                            apScrollBar->mfPageSize) +
                             1.0f),
      ...
      realsize = bHorz ? w : h;
      tsize = bHorz ? apScrollBar->mptrThumb->GetWidgetRect().GetWidth()
...
class cThumbButton : public ImplRC<iWidgetSink> {
 public:
  cThumbButton(cScrollBarWidget* parent)
  {
    mpParent = parent;
    mbDragging = eFalse;
  }
...
tBool cScrollBarWidget::OnWidgetSink(iWidget* apWidget, tU32 nMsg,
                                     const Var& varParam0, const Var& varParam1)
{
  niGuardObject((iWidgetSink*)this);
  switch (nMsg) {
  ...
  case eUIMessage_Paint: {
    ...
    if (isSlider) {
      ...
      if (c) {
        ...
        c->BlitText(
          pFont, apWidget->GetRect(),
          eFontFormatFlags_CenterV | eFontFormatFlags_CenterH |
  ...
  if (!thumbSize.bHorz) {
    ...
    if (!thumbSize.bSlider) {
      if (mButtons.HasButtons()) {
        ...
        mptrThumb->SetRect(
          sRectf(0, w + (fScrollPos * thumbSize.unit), w, thumbSize.tsize));
      }
      else {
        mptrThumb->SetRect(
          sRectf(0, fScrollPos * thumbSize.unit, w, thumbSize.tsize));
      }
    }
    else {
      tF32 tw = mptrThumb->GetWidgetRect().GetWidth();
      sRectf rect = mptrThumb->GetWidgetRect();
      mptrThumb->SetRect(Rectf((w * 0.5f) - (tw * 0.5f),
                               (thumbSize.realsize - thumbSize.tsize) -
    ...
    if (!thumbSize.bSlider) {
      if (mButtons.HasButtons()) {
        ...
        mptrThumb->SetRect(
          sRectf(h + (fScrollPos * thumbSize.unit), 0, thumbSize.tsize, h));
      }
      else {
        mptrThumb->SetRect(
          sRectf(fScrollPos * thumbSize.unit, 0, thumbSize.tsize, h));
      }
    }
    else {
      tF32 th = mptrThumb->GetWidgetRect().GetHeight();
      sRectf rect = mptrThumb->GetWidgetRect();
      mptrThumb->SetRect(sRectf(fScrollPos * thumbSize.unit,
                                (h * 0.5f) - (th * 0.5f), rect.GetWidth(),
                                rect.GetHeight()));
    }
  }
}
...
{
  const sScrollBarThumbMetrics thumbSize(this,
                                         mpWidget->GetWidgetRect().GetWidth(),
                                         mpWidget->GetWidgetRect().GetHeight());
  if (thumbSize.bHorz) {
    SetScrollPosition(
      ComputeRoundedPosition(mfScrollPos - (pos.x / thumbSize.unit)));
  }
  else {
    SetScrollPosition(
      ComputeRoundedPosition(mfScrollPos - (pos.y / thumbSize.unit)));
  }
}
...
void cScrollBarWidget::SetScrollRange(const sVec2f& avRange)
{
  ...
  if (mfMin == mfMax) {
    mfMax +=
      1.0f; // <!> Range cannot be zero, removing this will break the scroll bar is subtle ways.
    if (mptrThumb.IsOK()) {
...
void cScrollBarWidget::UpdateStyle()
{
  ...
  skin.back = mpWidget->FindSkinElement(
    hspSkinClass, NULL, bHorz ? _H("BackgroundHorz") : _H("BackgroundVert"));
  ...
  if (!isSlider) {
    if (hasButtons) {
      mButtons.up = pUICtx->CreateWidget(
        ...
        eWidgetButtonStyle_Sticky | eWidgetButtonStyle_IconFit |
      ...
      mButtons.up->SetSkinClass(bHorz ? _H("ScrollLeft") : _H("ScrollUp"));
      {
        ...
        if (btUp.IsOK()) {
          btUp->SetIcon(mpWidget->FindSkinElement(
            NULL, NULL, bHorz ? _H("ArrowLeft") : _H("ArrowUp")));
        }
      }
      ...
      mButtons.down->SetSkinClass(bHorz ? _H("ScrollRight") : _H("ScrollDown"));
      {
        ...
        if (btDown.IsOK()) {
          btDown->SetIcon(mpWidget->FindSkinElement(
            NULL, NULL, bHorz ? _H("ArrowRight") : _H("ArrowDown")));
        }
      }
    }
    ...
    if (btn.IsOK()) {
      ...
      if (style & eWidgetScrollBarStyle_Right) {
        ...
        bitmap =
          mpWidget->FindSkinElement(_H("ThumbBottom"), NULL, _H("Frame"));
      ...
      mptrThumb->SetRect(
        sRectf(0, 0, bitmap->GetSize().x, bitmap->GetSize().y));
    }
  }
...
tF32 cScrollBarWidget::ComputeRoundedPosition(tF32 afNewPos) const
{
  ...
  {
    newval =
      (ni::Sign(newval) * ni::Floor(ni::Abs(newval / mfPageSize) + 0.5f)) *
      mfPageSize;
  }
  return newval;
}
...
void cScrollBarWidget::_DoScroll(tU32 anFlags)
{
  ...
  //     else
  {
    ...
    moveUnits = size / numThumbs /
                ((anFlags & SCROLL_PAGE) ? TIMER_SCROLL_PAGE_SPLITS
                                         : TIMER_SCROLL_SPLITS);
  }
  if (anFlags & SCROLL_UP) {
    if (mfScrollStopAt != niMaxF32 &&
        ((mfScrollStopAt >= thumbStart && mfScrollStopAt <= thumbEnd) ||
...
void cScrollBarWidget::_StartScroll(tU32 anFlags, tF32 afStopAt)
{
  ...
  if ((anFlags & SCROLL_UP) || (anFlags & SCROLL_DOWN)) {
    ...
    mpWidget->SetTimer(anFlags, (anFlags & SCROLL_PAGE) ? TIMER_SCROLL_PAGE_SECS
                                                        : TIMER_SCROLL_SECS);
    _DoScroll(anFlags);
  }
}
