...
///////////////////////////////////////////////
void __stdcall cWidgetForm::ListInterfaces(iMutableCollection* apLst,
                                           tU32 anFlags) const
{
...
///////////////////////////////////////////////
tBool __stdcall cWidgetForm::OnWidgetSink(iWidget* apWidget, tU32 anMsg,
                                          const Var& aA, const Var& aB)
{
  ...
  case eUIMessage_Submit: { // Dont propagate the submit to parents further
    return eTrue;
  };
  ...
  case eUIMessage_Undocked: {
    ...
    if (bDocked != mbIsDocked) {
      ...
      if (bDocked) {
        if (!mnDrag)
          PushBeforeDock();
        mpWidget->SetStyle(mpWidget->GetStyle() &
                           ~(eWidgetStyle_Free | eWidgetStyle_NoClip));
        mpWidget->SetAutoLayout(mpWidget->GetAutoLayout() &
  ...
  case eUIMessage_NCPaint: {
    ...
    if (mbIsDocked) {
      ...
      if (frame && c && font) {
        c->BlitOverlayFrame(mpWidget->GetWidgetRect(), frame, mFormFrameFlags);
        if (!niFlagIs(mFormFrameFlags, eRectFrameFlags_Center)) {
          const sColor4f& bgColor = skin.backgroundColor;
          if (bgColor.w > 0) {
            const sRectf clearRect =
              frame->ComputeFrameCenter(mpWidget->GetWidgetRect());
            c->BlitFillAlpha(clearRect, ULColorBuild(bgColor));
          }
        }
        c->BlitText(font, skin.titleTextFrame, eFontFormatFlags_CenterV,
  ...
  case eUIMessage_MouseMove: {
    if (mnDrag) {
      ...
      if (mResizeArea == eResizeArea_None) {
        ...
        switch (mResizeArea) {
        case eResizeArea_TopLeft: absRect.SetTopLeft(mousepos); break;
        case eResizeArea_TopRight: absRect.SetTopRight(mousepos); break;
        case eResizeArea_BottomLeft: absRect.SetBottomLeft(mousepos); break;
        case eResizeArea_BottomRight: absRect.SetBottomRight(mousepos); break;
        case eResizeArea_Left: absRect.SetLeft(mousepos.x); break;
        case eResizeArea_Right: absRect.SetRight(mousepos.x); break;
        case eResizeArea_Top: absRect.SetTop(mousepos.y); break;
        case eResizeArea_Bottom: absRect.SetBottom(mousepos.y); break;
        case eResizeArea_Title:
        case eResizeArea_None:
          // Do nothing
          break;
        }
        absRect.SetLeft(ni::Min(absRect.x, mBeforeDock.mRect.z -
                                             mpWidget->GetMinimumSize().x));
        absRect.SetTop(ni::Min(absRect.y, mBeforeDock.mRect.w -
                                            mpWidget->GetMinimumSize().y));
  ...
  case eUIMessage_SetCursor: {
    ...
    // top left corner
    if (sRectf(rectAbs.GetCorner(eRectCorners_TopLeft) +
                 Vec2(-(mfResizeBorderArea / 2), -(mfResizeBorderArea / 2)),
               mfResizeBorderArea, mfResizeBorderArea)
          .Intersect(vAbsMousePos))
    {
    ...
    // top right corner
               ...
               rectAbs.GetCorner(eRectCorners_TopRight) +
                 Vec2(-(mfResizeBorderArea / 2), -(mfResizeBorderArea / 2)),
               mfResizeBorderArea, mfResizeBorderArea)
               .Intersect(vAbsMousePos))
    {
    ...
    // bottom left corner
               ...
               rectAbs.GetCorner(eRectCorners_BottomLeft) +
                 Vec2(-(mfResizeBorderArea / 2), -(mfResizeBorderArea / 2)),
               mfResizeBorderArea, mfResizeBorderArea)
               .Intersect(vAbsMousePos))
    {
      if (niFlagIs(mpWidget->GetStyle(),
    ...
    // bottom right corner
               ...
               rectAbs.GetCorner(eRectCorners_BottomRight) +
                 Vec2(-(mfResizeBorderArea / 2), -(mfResizeBorderArea / 2)),
               mfResizeBorderArea, mfResizeBorderArea)
               .Intersect(vAbsMousePos))
    {
    ...
    // left edge
    else if (sRectf(rectAbs.GetLeft() - (mfResizeBorderArea / 2),
                    rectAbs.GetTop(), mfResizeBorderArea, rectAbs.GetHeight())
               .Intersect(vAbsMousePos))
    {
    ...
    // right edge
    else if (sRectf(rectAbs.GetRight() - (mfResizeBorderArea / 2),
                    rectAbs.GetTop(), mfResizeBorderArea, rectAbs.GetHeight())
               .Intersect(vAbsMousePos))
    {
    ...
    // top edge
    else if (sRectf(rectAbs.GetLeft(),
                    rectAbs.GetTop() - (mfResizeBorderArea / 2),
                    rectAbs.GetWidth(), mfResizeBorderArea)
               .Intersect(vAbsMousePos))
    {
    ...
    // bottom edge
                    ...
                    rectAbs.GetBottom() - (mfResizeBorderArea / 2),
                    rectAbs.GetWidth(), mfResizeBorderArea)
               .Intersect(vAbsMousePos))
    {
    ...
    if (dt.IsOK()) {
      ...
      if (niFlagIs(nFlags, eWidgetSerializeFlags_Read)) {
        ...
        if (dtBeforeDock.IsOK()) {
          ...
          mBeforeDock.mDockStyle =
            (eWidgetDockStyle)dtBeforeDock->GetInt(_A("dock_style"));
          ...
          mBeforeDock.mZOrder =
            (eWidgetZOrder)dtBeforeDock->GetInt(_A("zorder"));
        }
      }
      else {
...
{
  return !mpWidget->GetExclusive() && mpWidget->GetParent() &&
...
void cWidgetForm::UpdateStyle()
{
  ...
  skin.inactiveFrame =
    mpWidget->FindSkinElement(NULL, _H("Inactive"), _H("Frame"));
  skin.inactiveFont =
    mpWidget->FindSkinFont(NULL, _H("Inactive"), _H("Default"));
  ...
  {
    Ptr<iOverlay> ptrTitleFrame =
      mpWidget->FindSkinElement(NULL, NULL, _H("TitleFrame"));
  ...
  {
    Ptr<iOverlay> ptrTitleTextFrame =
      mpWidget->FindSkinElement(NULL, NULL, _H("TitleTextFrame"));
  ...
  skin.titleHeight = ni::Max(
    skin.titleHeight - (skin.titleFrame.GetTop() + skin.titleFrame.GetBottom()),
    5);
  ...
  if (niFlagIs(mpWidget->GetStyle(), eWidgetFormStyle_CloseButton)) {
    Ptr<iOverlay> ptrTitleFrame =
      mpWidget->FindSkinElement(NULL, NULL, _H("TitleFrame"));
    if (!mptrButtonClose.IsOK()) {
      mptrButtonClose = mpWidget->GetUIContext()->CreateWidget(
        _A("Button"), mpWidget, sRectf(0, 0, 10, 10),
        eWidgetStyle_NCRelative | eWidgetStyle_DontSerialize,
    ...
    mptrButtonClose->SetMargin(Vec4<tF32>(0, skin.titleFrame.GetTop(),
...
///////////////////////////////////////////////
cWidgetForm::eResizeArea cWidgetForm::FindResizeArea(
  const sVec2f& aNCMousePos) const
{
  ...
  // title
  if (sRectf(rectAbs.GetCorner(eRectCorners_TopLeft) + Vec2<tF32>(1, 1),
             rectAbs.GetWidth() - 2.0f, font->GetHeight() + 2)
        .Intersect(vAbsMousePos))
  {
  ...
  // top left corner
             ...
             rectAbs.GetCorner(eRectCorners_TopLeft) +
               Vec2<tF32>(-(mfResizeBorderArea / 2), -(mfResizeBorderArea / 2)),
             mfResizeBorderArea, mfResizeBorderArea)
             .Intersect(vAbsMousePos))
  {
...
void cWidgetForm::_HandleDragDock(tBool abEnd, const sVec2f* apMousePos)
{
  ...
  if (!abEnd) {
    if (bInputCanDock && (!(mnDrag & DRAG_DOCK_BEGAN)) &&
        (mnDrag & DRAG_DOCK) && (mResizeArea == eResizeArea_None) && CanDock())
    {
      mnDrag |= DRAG_DOCK_BEGAN;
      ni::SendMessage(mpWidget->GetParent(),
                      eWidgetDockingManagerMessage_BeginMove, niVarNull,
                      niVarNull);
    }
  }
  ...
  if (apMousePos) {
    if (mnDrag & DRAG_DOCK_BEGAN) {
      ni::SendMessage(mpWidget->GetParent(), eWidgetDockingManagerMessage_Move,
                      *apMousePos, niVarNull);
    }
  }
  ...
  if (abEnd) {
    if (mnDrag & DRAG_DOCK_BEGAN) {
      if (mResizeArea == eResizeArea_None && CanDock()) {
        ni::SendMessage(mpWidget->GetParent(),
                        eWidgetDockingManagerMessage_EndMove, mpWidget,
                        niVarNull);
      }
    }
    mnDrag &= ~DRAG_DOCK_BEGAN;
