...
static MTLVertexDescriptor* _CreateMetalVertDescForFVFAndGpuVertexAttibutes(
  ain<tU32> aBufferIndex, ain<tFVF> aFVF,
  ...
  for (const auto& attr : aVertAttrs) {
    ...
    default: {
      if ((aFVF & attr._fvf) != attr._fvf) {
        niError(niFmt(
          "Vertex attribute FVF mismatch: expected to have '%s' (0x%x) in FVF '%s' (0x%x)",
    ...
    niLet handleTexCoo = [&](const tU32 i) -> tBool {
      ...
      switch (dim) {
      case 1: attrDesc.format = MTLVertexFormatFloat; break;
      case 2: attrDesc.format = MTLVertexFormatFloat2; break;
      case 3: attrDesc.format = MTLVertexFormatFloat3; break;
      case 4: attrDesc.format = MTLVertexFormatFloat4; break;
      default: {
        niError(niFmt(
          "Unsupported vertex attribute '%s' (%d) texcoo size '%d' fvf: %s (0x%x)",
          attr._name, attr._location, dim, ni::FVFToShortString(attr._fvf),
          attr._fvf));
        return eFalse;
      }
      }
    ...
    default: handledFvf = eFalse; break;
    }
    if (!handledFvf) {
      niError(niFmt("Unsupported vertex attribute '%s' (%d) fvf: %s (0x%x)",
                    attr._name, attr._location, ni::FVFToShortString(attr._fvf),
                    attr._fvf));
      return nil;
    }
...
{
  switch (aFuncType) {
  case MTLFunctionTypeVertex: return eGpuFunctionType_Vertex;
  case MTLFunctionTypeFragment: return eGpuFunctionType_Pixel;
  case MTLFunctionTypeKernel: return eGpuFunctionType_Compute;
  default:
...
struct sMetalFunction
  ...
  {
    _hspName = ahspPath;
    _datatable = niCheckNN(
      _datatable,
      GpuFunctionDT_Load(niHStr(ahspPath), _GetGpuFunctionTarget(), &_bindType),
      eFalse);
    ...
    id<MTLLibrary> lib = [aDevice newLibraryWithSource:nsSource
                                               options:compileOptions
                                                 error:&error];
    if (!lib) {
      niError(niFmt("MetalGpuFunction '%s': Couldn't create MTLLibrary: %s",
                    _hspName, error.localizedDescription.UTF8String));
      return eFalse;
    }
    ...
    if (!_mtlFunction) {
      niError(niFmt("MetalGpuFunction '%s': Couldn't find MTLFunction '%s'.",
...
{
  switch (aFmt) {
  case eGpuPixelFormat_None: return MTLPixelFormatInvalid;
  case eGpuPixelFormat_BGRA8: return MTLPixelFormatBGRA8Unorm;
  case eGpuPixelFormat_RGBA8: return MTLPixelFormatRGBA8Unorm;
  case eGpuPixelFormat_RGBA16F: return MTLPixelFormatRGBA16Float;
  case eGpuPixelFormat_R16F: return MTLPixelFormatR16Float;
  case eGpuPixelFormat_R32F: return MTLPixelFormatR32Float;
  case eGpuPixelFormat_D32: return MTLPixelFormatDepth32Float;
  case eGpuPixelFormat_D16: return MTLPixelFormatDepth16Unorm;
...
static const MTLCompareFunction _toMTLCompareFunction[] = {
  MTLCompareFunctionNever,        MTLCompareFunctionEqual,
  MTLCompareFunctionNotEqual,     MTLCompareFunctionLess,
...
static const MTLStencilOperation _toMTLStencilOp[] = {
  ...
  MTLStencilOperationReplace,        MTLStencilOperationIncrementWrap,
  MTLStencilOperationDecrementWrap,  MTLStencilOperationIncrementClamp,
...
tBool _MTLVertexAttributeToGpuVertexAttribute(aout<sGpuVertexAttribute> aOut,
                                              MTLVertexAttribute* attr)
{
...
struct sMetalPipeline
  ...
  tBool _CreateMTLPipeline(id<MTLDevice> aDevice, iHString* ahspName,
                           ...
                           id<MTLDepthStencilState> aDS)
  {
    ...
    // Vertex gpu function
    {
      sMetalFunction* vertexFunction =
        (sMetalFunction*)_desc->GetFunction(eGpuFunctionType_Vertex);
      ...
      {
        NSArray<MTLVertexAttribute*>* attributes =
          vertexFunction->_mtlFunction.vertexAttributes;
        for (MTLVertexAttribute* attr in attributes) {
          ...
          if (!_MTLVertexAttributeToGpuVertexAttribute(va, attr)) {
            niError(niFmt(
              "Can't create pipeline, cant validate vertex attribute [%d]: %s",
      ...
      if (attrs.empty()) {
        niError(niFmt(
          "Can't create pipeline, gpu vertex function '%s' doesn't have vertex attributes.",
          vertexFunction->GetDeviceResourceName()));
        return eFalse;
      }
      pipelineDesc.vertexDescriptor =
        _CreateMetalVertDescForFVFAndGpuVertexAttibutes(
    ...
    // Pixel gpu function
    {
      sMetalFunction* pixelFunction =
        (sMetalFunction*)_desc->GetFunction(eGpuFunctionType_Pixel);
    ...
    // Color attachments
    {
      ...
      if (apRS) {
        _fillMode = apRS->GetWireframe() ? MTLTriangleFillModeLines
                                         : MTLTriangleFillModeFill;
        _cullMode = _toMTLCullMode[apRS->GetCullingMode()];
        niLet colorWriteMask = apRS->GetColorWriteMask();
        switch (colorWriteMask) {
        case eColorWriteMask_None: ca.writeMask = MTLColorWriteMaskNone; break;
        case eColorWriteMask_All: ca.writeMask = MTLColorWriteMaskAll; break;
        default: {
    ...
    pipelineDesc.depthAttachmentPixelFormat =
      _GetMTLPixelFormat(_desc->GetDepthFormat());
    niCheck(_desc->GetDepthFormat() == eGpuPixelFormat_None ||
    ...
    NSError* error = nil;
    _mtlPipeline = [aDevice newRenderPipelineStateWithDescriptor:pipelineDesc
                                                           error:&error];
    if (!_mtlPipeline) {
      niError(niFmt("Cant create pipeline state: %s",
...
//----------------------------------------------------------------------------
struct sMetalBuffer
    : public ni::ImplRC<iGpuBuffer, eImplFlags_DontInherit1, iDeviceResource> {
  ...
  tBool _CreateMTLBuffer(id<MTLDevice> aDevice, iHString* ahspName,
                         ...
                         tGpuBufferUsageFlags aUsage)
  {
    ...
    if (apInitialData) {
      ...
      _mtlBuffer = [aDevice newBufferWithLength:anSize options:resOptions];
    }
  ...
  virtual tBool __stdcall Unlock() niImpl
  {
    if (!GetIsLocked())
      return eFalse;
  #if !defined niIOS
    if (_memMode == eGpuBufferMemoryMode_Managed && _lockMode != eLock_ReadOnly)
    {
      [_mtlBuffer didModifyRange:NSMakeRange(_lockOffset, _lockSize)];
    }
  #endif
...
struct cMetalTexture
  ...
  cMetalTexture(id<MTLDevice> aDevice, iGraphics* apGraphics,
                iHString* ahspName, eBitmapType aType, tU32 anW, tU32 anH,
                tU32 anD, tU32 anMM, iPixelFormat* apPxf, tTextureFlags anFlags)
      ...
      , mnFlags(anFlags)
  {
    ...
    if (!texPxf.IsOK()) {
      niError(niFmt("Can't get compatible gpu pixel format for '%s'.",
                    apPxf->GetFormat()));
      return;
    }

    if (niFlagIsNot(anFlags, eTextureFlags_Surface)) {
      METAL_TRACE(("## METAL-TEXTURE [%s] fmt:%s w:%s h:%s", ahspName,
                   apPxf->GetFormat(), mnW, mnH));
      ...
      switch (aType) {
      case eBitmapType_2D: desc.textureType = MTLTextureType2D; break;
      case eBitmapType_Cube: desc.textureType = MTLTextureTypeCube; break;
      case eBitmapType_3D:
        desc.textureType = MTLTextureType3D;
        desc.depth = anD;
        break;
      default: niError("Invalid bitmap format"); break;
      }
      ...
      if (aType == eBitmapType_Cube) {
        niLoop (i, 6) {
          _subTexs[i] = niNew cMetalTexture(
            aDevice, mpGraphics, ahspName, eBitmapType_2D, mnW, mnH, mnD, mnMM,
            texPxf, eTextureFlags_SubTexture | eTextureFlags_Surface);
          _subTexs[i]->_tex = _tex;
          _subTexs[i]->mnFace = i;
          _subTexs[i]->mptrBitmap =
            QPtr<iBitmapCube>(mptrBitmap)->GetFace((eBitmapCubeFace)i);
        }
      }
    }
  ...
  void UploadTexture(Ptr<iPixelFormat> aFormat, const iBitmap2D* apBmpLevel,
                     const tU32 anLevel, const sRecti& aDestRect)
  {
    const tU32 bpp = aFormat->GetBytesPerPixel();
    METAL_TRACE(("Bitmap Format[%s] bpp:%s Texture format[%s]",
                 aFormat->GetFormat(), bpp, mptrPxf->GetFormat()));
    const tU32 bpr = apBmpLevel->GetPitch();
    const tU32 startOffset = (aDestRect.y * bpr) + (aDestRect.x * bpp);
    auto region = MTLRegionMake2D(aDestRect.x, aDestRect.y,
  ...
  iBitmap2D* GetBitmap()
  {
    ...
    if (!bmp.IsOK()) {
      ...
      [_tex getBytes:bmp->GetData()
         bytesPerRow:bmp->GetPitch()
          fromRegion:MTLRegionMake2D(0, 0, mnW, mnH)
         mipmapLevel:mnMM];
    }
...
static iGraphicsContext* New_MetalContextWindow(
  struct cMetalGraphicsDriver* apParent, const tU32 aFrameMaxInFlight,
  iOSWindow* apWindow, iOSXMetalAPI* apAPI, const achar* aaszBBPxf,
  const achar* aaszDSPxf, tU32 anSwapInterval, tTextureFlags anBBFlags);
static iGraphicsContextRT* New_MetalContextRT(cMetalGraphicsDriver* apParent,
                                              const tU32 aFrameMaxInFlight,
                                              iTexture* apRT, iTexture* apDS);

struct cMetalGraphicsDriver
    : public ImplRC<iGraphicsDriver, eImplFlags_Default, iGraphicsDriverGpu> {
  iGraphics* mpGraphics;
  ...
  id<MTLCommandQueue> _GetCommandQueue()
  {
    if (mMetalCommandQueue == nil) {
      ...
      niWarning(
        "No command queue explicitly set by a window context, creating our own.");
    }
    ...
    tU32 anSwapInterval, tTextureFlags aBackBufferFlags)
  {
    ...
    if (!metalAPI.IsOK()) {
      ...
      metalAPI =
        osxMetalCreateAPIForWindow((__bridge void*)mMetalDevice, apWindow);
      if (!metalAPI.IsOK()) {
    ...
    mMetalCommandQueue =
      (__bridge id<MTLCommandQueue>)metalAPI->GetCommandQueue();
...

    {
      ...
      if (mtkView) {
        mtkView.depthStencilPixelFormat =
          _GetMTLPixelFormat(_GetClosestGpuPixelFormatForDS(aaszDSFormat));
        if (anSwapInterval) {
    ...
    Ptr<iGraphicsContext> ctx = New_MetalContextWindow(
      this, knMetalMaxFrameInFlight, apWindow, metalAPI, aaszBBFormat,
      aaszDSFormat, anSwapInterval, aBackBufferFlags);
    if (!niIsOK(ctx))
      return NULL;

    if (mFixedPipelines.raw_ptr() == nullptr) {
      mFixedPipelines =
        niCheckNN(mFixedPipelines, CreateFixedGpuPipelines(this), nullptr);
    }
  ...
  virtual tInt __stdcall GetCaps(eGraphicsCaps aCaps) const niImpl
  {
    ...
    case eGraphicsCaps_IRay: return 0;
    }
  }
                                             ...
                                             tTextureFlags aFlags)
  {
    ...
    NN<iPixelFormat> bmpFormat =
      niCheckNN(bmpFormat, _GetIPixelFormat(mpGraphics, gpufmt), eFalse);
                                            ...
                                            tTextureFlags aFlags)
  {
    ...
    return niNew cMetalTexture(mMetalDevice, mpGraphics, ahspName, aType,
                               anWidth, anHeight, anDepth, anNumMipMaps, pxf,
                               aFlags);
  }
    ...
    const sRecti& aSrcRect, const sRecti& aDestRect, eTextureBlitFlags aFlags)
  {
    ...
    texBmp->BlitStretch(apSrc, aSrcRect.x, aSrcRect.y, aDestRect.x, aDestRect.y,
                        ...
                        aDestRect.GetWidth(), aDestRect.GetHeight());
    if (tex->_tex) {
      tex->UploadTexture(texBmp->GetPixelFormat(), texBmp, anDestLevel,
                         aDestRect);
    }
    METAL_TRACE(("BlitBitmapToTexture %s %s %s", aSrcRect, aDestRect,
                                              ...
                                              eTextureBlitFlags aFlags)
  {
    ...
    apDest->BlitStretch(texBmp, aSrcRect.x, aSrcRect.y, aDestRect.x,
                        ...
                        aDestRect.GetWidth(), aDestRect.GetHeight());
    METAL_TRACE(("BlitTextureToBitmap %s %s %s", aSrcRect, aDestRect,
    ...
    const sRecti& aSrcRect, const sRecti& aDestRect, eTextureBlitFlags aFlags)
  {
    ...
    dTexBmp->BlitStretch(sTexBmp, aSrcRect.x, aSrcRect.y, aDestRect.x,
                         aDestRect.y, aSrcRect.GetWidth(), aSrcRect.GetHeight(),
                         aDestRect.GetWidth(), aDestRect.GetHeight());
    METAL_TRACE(("BlitTextureToTexture %s %s %s", aSrcRect, aDestRect,
                 apSrc->GetDeviceResourceName()));
    return eTrue;
  }
  ...
  /////////////////////////////////////////////
  id<MTLSamplerState> _ssCompiled[(eCompiledStates_SS_SmoothWhiteBorder -
  ...
  #define INIT_COMPILED_SAMPLER_STATES(STATE, FILTER, WRAP)                   \
    {                                                                         \
      const tU32 ssKey =                                                      \
        niFourCC(eSamplerFilter_##FILTER, eSamplerWrap_##WRAP, eSamplerWrap_##WRAP, eSamplerWrap_##WRAP);                   \
      _toMTLSamplerFilter(desc, eSamplerFilter_##FILTER);                     \
      desc.sAddressMode = desc.tAddressMode = desc.rAddressMode =             \
        _toMTLSamplerAddress[eSamplerWrap_##WRAP];                            \
      desc.borderColor = MTLSamplerBorderColorOpaqueWhite;                    \
      id<MTLSamplerState> ss =                                                \
        [mMetalDevice newSamplerStateWithDescriptor:desc];                    \
      _ssCompiled[eCompiledStates_##STATE - eCompiledStates_SS_PointRepeat] = \
        ss;                                                                   \
    }
  ...
  void _InitCompiledDepthStencilStates()
  {
...

    {
      desc.depthCompareFunction =
        _toMTLCompareFunction[eGraphicsCompare_Always];
    ...
    {
      desc.depthCompareFunction =
        _toMTLCompareFunction[eGraphicsCompare_LessEqual];
      desc.depthWriteEnabled = YES;
      _dsDepthTestAndWrite =
        [mMetalDevice newDepthStencilStateWithDescriptor:desc];
    ...
    tGpuBufferUsageFlags aUsage) niImpl
  {
    ...
    niCheck(buffer->_CreateMTLBuffer(mMetalDevice, ahspName, nullptr, anSize,
                                     aMemMode, aUsage),
            nullptr);
    return buffer;
  }
    ...
    tGpuBufferUsageFlags aUsage) niImpl
  {
    ...
    niLet buffer = MakeNN<sMetalBuffer>();
    niCheck(buffer->_CreateMTLBuffer(mMetalDevice, ahspName, apData, anSize,
                                     aMemMode, aUsage),
            nullptr);
    return buffer;
  }
  ...
  virtual Ptr<iGpuFunction> __stdcall CreateGpuFunction(
    eGpuFunctionType aType, iHString* ahspPath) niImpl
  {
    ...
    if (func->GetFunctionType() != aType) {
      ...
      niError(niFmt(
        ...
        ahspPath, niEnumToChars(eGpuFunctionType, aType),
    ...
    iHString* ahspName, const iGpuPipelineDesc* apDesc) niImpl
  {
    ...
    niLet rs =
      mpGraphics->GetCompiledRasterizerStates(apDesc->GetRasterizerStates());
...
niExportFunc(iUnknown*) New_GraphicsDriver_Metal(const Var& avarA,
                                                 const Var& avarB)
{
...
struct sMetalCommandEncoder : public ImplRC<iGpuCommandEncoder> {
  ...
  sMetalCommandEncoder(ain<nn<cMetalGraphicsDriver>> aDriver,
  ...
  void _BeginFrame(id<MTLRenderCommandEncoder> aNewEncoder, tU32 aCurrentFrame,
                   ...
                   ain<sRecti> aViewport, ain<sRecti> aScissor)
  {
    ...
    _frames[_currentFrame]->StartFrame(ni::GetLang()->GetFrameNumber(),
                                       _currentFrame);
    _cache = sCache();
                                         ...
                                         tU32 anBinding) niImpl
  {
    ...
    sMetalBuffer* buffer = (sMetalBuffer*)apBuffer;
    [_cmdEncoder setVertexBuffer:buffer->_mtlBuffer
                          offset:anOffset
                                          ...
                                          tU32 anBinding) niImpl
  {
    ...
    [_cmdEncoder setVertexBuffer:buffer->_mtlBuffer
                          offset:anOffset
                         atIndex:anBinding];
    [_cmdEncoder setFragmentBuffer:buffer->_mtlBuffer
                            offset:anOffset
                           atIndex:anBinding];
  }
  ...
  {
    [_cmdEncoder setFragmentSamplerState:_driver->_GetMTLSamplerState(ahSS)
                                 atIndex:anBinding];
  }

  virtual void __stdcall SetPolygonOffset(const sVec2f& avOffset) niImpl
  {
    [_cmdEncoder setDepthBias:avOffset.x slopeScale:avOffset.y clamp:0.0f];
  }
  ...
  {
    [_cmdEncoder setStencilFrontReferenceValue:aMask backReferenceValue:aMask];
  }
...
struct cMetalContextBase
  ...
  cMetalContextBase(cMetalGraphicsDriver* apParent,
                    const tU32 aFrameMaxInFlight)
      : tGraphicsContextBase(apParent->GetGraphics())
      , mFrameMaxInFlight(aFrameMaxInFlight)
      , mCmdEncoder(
          MakeNN<sMetalCommandEncoder>(as_nn(apParent), aFrameMaxInFlight))
  {
    mpParent = apParent;
  ...
  inline tBool _BeginFrame()
  {
    ...
    mCmdEncoder->_BeginFrame(_NewRenderCommandEncoder(), mnCurrentFrame,
                             this->GetWidth(), this->GetHeight(), mrectViewport,
                             mrectScissor);
                                           ...
                                           tF32 afZ)
  {
    ...
    if (doCapture) {
      niLet clearParams = Vec4<tI32>(aFlags, anColor, ftoul(afZ), -1);
      if (!mpParent->mptrDOCapture->BeginCaptureDrawOp(this, nullptr,
                                                       clearParams))
        return eTrue;
    }
    ...
    return mpParent->mFixedPipelines->ClearRect(mCmdEncoder, pixelSize, aFlags,
                                                aRect, anColor, afZ);
  }
  ...
  virtual tBool __stdcall DrawOperation(iDrawOperation* apDrawOp)
  {
    ...
    {
      if (doCapture) {
        mpParent->mptrDOCapture->EndCaptureDrawOp(this, apDrawOp,
                                                  sVec4i::Zero());
      }
    };
    if (doCapture) {
      if (!mpParent->mptrDOCapture->BeginCaptureDrawOp(this, apDrawOp,
                                                       sVec4i::Zero()))
        return eTrue;
    }
    ...
    iGpuFunction* funcVertex =
      mpParent->mFixedPipelines->GetFixedGpuFuncVertex(fvf);
    iGpuFunction* funcPixel =
      mpParent->mFixedPipelines->GetFixedGpuFuncPixel(*pDOMatDesc);
    const tFixedGpuPipelineId rpId = GetFixedGpuPipelineId(
      mRT0Format, mDSFormat, fvf, _GetBlendMode(pDOMatDesc),
    ...
    if (rpId != cmdStateCache._lastPipeline) {
      sMetalPipeline* pipeline =
        (sMetalPipeline*)mpParent->mFixedPipelines
          ->GetRenderPipeline(mpParent, rpId, funcVertex, funcPixel)
          .raw_ptr();
      if (!pipeline) {
...

    {
      ...
      mCmdEncoder->StreamUniformBuffer((tPtr)&fixedUniforms,
...
struct cMetalContextWindow : public cMetalContextBase {
  ...
  cMetalContextWindow(cMetalGraphicsDriver* apParent,
                      const tU32 aFrameMaxInFlight, iOSWindow* apWindow,
                      iOSXMetalAPI* apAPI, const achar* aaszBBPxf,
                      const achar* aaszDSPxf, tU32 anSwapInterval,
  ...
  virtual id<MTLRenderCommandEncoder> __stdcall _NewRenderCommandEncoder()
  {
    ...
    return (__bridge id<MTLRenderCommandEncoder>)
      mptrMetalAPI->NewRenderCommandEncoder(sVec4d::Zero(), 1.0f, 0);
  }
  ...
  tBool __stdcall _DoResizeContext()
  {
    ...
    mptrRT[0] = niNew cMetalTexture(
      mpParent->mMetalDevice, g, _H("Metal_MainRT"), eBitmapType_2D, w, h, 0, 0,
      mptrBBPxf, eTextureFlags_RenderTarget | eTextureFlags_Surface);

    ni::SafeInvalidate(mptrDS.ptr());
    mptrDS = niNew cMetalTexture(
      mpParent->mMetalDevice, g, _H("Metal_MainDS"), eBitmapType_2D, w, h, 0, 0,
      mptrDSPxf, eTextureFlags_DepthStencil | eTextureFlags_Surface);
    ...
    niLog(
      ...
      niFmt(
        ...
        (tIntPtr)this, w, h, mptrRT[0]->GetPixelFormat()->GetFormat(),
...
static iGraphicsContext* New_MetalContextWindow(
  cMetalGraphicsDriver* apParent, const tU32 aFrameMaxInFlight,
  iOSWindow* apWindow, iOSXMetalAPI* apAPI, const achar* aaszBBPxf,
  const achar* aaszDSPxf, tU32 anSwapInterval, tTextureFlags anBBFlags)
{
  return niNew cMetalContextWindow(apParent, aFrameMaxInFlight, apWindow, apAPI,
                                   aaszBBPxf, aaszDSPxf, anSwapInterval,
                                   anBBFlags);
}
...
struct cMetalContextRT : public cMetalContextBase {
  ...
  cMetalContextRT(cMetalGraphicsDriver* apParent, const tU32 aFrameMaxInFlight,
                  iTexture* apRT, iTexture* apDS)
      : cMetalContextBase(apParent, aFrameMaxInFlight)
  {
    mptrRT[0] = apRT;
  ...
  virtual id<MTLRenderCommandEncoder> __stdcall _NewRenderCommandEncoder()
  {
    ...
    _commandEncoder =
      [_commandBuffer renderCommandEncoderWithDescriptor:passDesc];
  ...
  virtual tBool __stdcall _EndCommandEncoder()
  {
    ...
    if (_commandBuffer) {
      ...
      id<MTLBlitCommandEncoder> blitEncoder =
        [_commandBuffer blitCommandEncoder];
      cMetalTexture* rtTex = (cMetalTexture*)mptrRT[0].ptr();
  ...
  virtual tBool __stdcall ChangeRenderTarget(tU32 anIndex,
                                             iTexture* apRT) niImpl
  {
    niCheck(_commandEncoder == NULL,
            eFalse); // can't change the RT while rendering is happening
    return cMetalContextBase::ChangeRenderTarget(anIndex, apRT);
  }
  virtual tBool __stdcall ChangeDepthStencil(iTexture* apDS) niImpl
  {
    niCheck(_commandEncoder == NULL,
            eFalse); // can't change the DS while rendering is happening
