...
#define EASTL_MOVE(x) eastl::move(x)
  ...
  #define EASTL_MOVE_INLINE(x) \
    static_cast<typename eastl::remove_reference<decltype(x)>::type&&>(x)
#elif defined(__GNUC__)
  #define EASTL_MOVE_INLINE(x) \
    static_cast<typename eastl::remove_reference<__typeof__(x)>::type&&>(x)
#else
  #define EASTL_MOVE_INLINE(x) eastl::move(x)
#endif

#define EASTL_FORWARD(T, x) eastl::forward<T>(x)
#define EASTL_FORWARD_INLINE(T, x) \
  eastl::forward<T>(               \
    x) // Need to investigate how to properly make a macro for this. (eastl::is_reference<T>::value ? static_cast<T&&>(static_cast<T&>(x)) : static_cast<T&&>(x))
...
#define EASTL_MOVE_BACKWARD_RANGE(first, last, resultEnd) \
  eastl::move_backward(first, last, resultEnd)
...
//
template <typename T>
EA_CPP14_CONSTEXPR T&& forward(typename eastl::remove_reference<T>::type& x)
  EA_NOEXCEPT
{
...
//
template <typename T>
EA_CPP14_CONSTEXPR typename eastl::remove_reference<T>::type&& move(T&& x)
  EA_NOEXCEPT
{
...
template <typename T>
EA_CPP14_CONSTEXPR
  typename eastl::conditional<!eastl::is_nothrow_move_constructible<T>::value &&
...
#else
template <typename T>
EA_CPP14_CONSTEXPR T&& move_if_noexcept(T& x) EA_NOEXCEPT
{
  return eastl::move(x);
}
#endif
