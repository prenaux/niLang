...
class StrBreakIt
    : public eastl::iterator<eastl::forward_iterator_tag, ni::StrCharIt> {
 ...
 public:
  explicit StrBreakIt(const StrCharIt& aCharIt,
                      const BREAKER& aBreaker = BREAKER())
      : _whole(aCharIt)
      , _cur(aCharIt.start(), aCharIt.start())
      , _breaker(aBreaker)
  {
    _Init();
  }
...
struct StrBreakChr {
  ...
  StrBreakChr(const StrBreakChr& aBreaker)
      : lastSeparator(AZEROSTR)
  {
    splitters = aBreaker.splitters;
  }
  StrBreakChr(const astl::vector<tU32>& aSplitters)
      : lastSeparator(AZEROSTR)
  {
    splitters = aSplitters;
  }
  StrBreakChr(const achar* aaszSplitters)
      : lastSeparator(AZEROSTR)
  {
