...
cImagePacker::cImagePacker(iGraphics* apGraphics, tU32 anWidth, tU32 anHeight,
                           tBool abUseMipMaps, const char* aaszBaseName,
                           const char* aaszPxf)
{
  mlMinHole = 5;
  mbIsFull = false;
  ...
  mnNumMipMaps =
    abUseMipMaps ? ni::ComputeNumPow2Levels(anWidth >> 1, anHeight >> 1) : 0;

  Ptr<iTexture> ptrTex = apGraphics->CreateTexture(
    _H(niFmt(_A("%s_%p"), aaszBaseName, (void*)this)), eBitmapType_2D, aaszPxf,
    mnNumMipMaps, anWidth, anHeight, 0, eTextureFlags_Overlay);
  niCheck(ptrTex.IsOK(), ;);

  Ptr<iBitmap2D> ptrBmp = apGraphics->CreateBitmap2D(
    anWidth, anHeight, ptrTex->GetPixelFormat()->GetFormat());
  niCheck(ptrBmp.IsOK(), ;);
  if (mnNumMipMaps) {
...
{
  Ptr<iBitmap2D> bmp = _GetTargetBitmap(
    sRecti(aNewPos.x, aNewPos.y, apBmp->GetWidth(), apBmp->GetHeight()));
  bmp->Blit(apBmp, 0, 0, aNewPos.x, aNewPos.y, apBmp->GetWidth(),
            apBmp->GetHeight());
  ...
  if (nNumMips) {
    ...
    niLoop (i, nNumMips) {
      ...
      if (niIsOK(d)) {
        d->Blit(src, 0, 0, pos.x, pos.y, src->GetWidth(), src->GetHeight());
      }
      srcPrev = src;
    }
  }
...
void cImagePacker::DrawBitmap(iBitmap2D* apBmp, const sVec2i& aDestPos,
                              tBool abFiltered)
{
  if (abFiltered) {
    for (int i = -1; i <= 1; ++i) {
...
sRecti cImagePacker::_DoInsert(const sRecti& aRect, iBitmap2D* apSrc,
                               tBool abFiltered, const int aPadding)
{
  ...
  for (it = lstNodes.begin(); it != lstNodes.end(); ++it) {
    ...
    if (pData->mlHandle < 0) {
      ...
      const sRecti NewRect = sRecti(pData->mRect.x, pData->mRect.y,
      ...
      if (BoxFit(NewRect, pData->mRect)) {
        ...
        // If there is still space left, make new nodes.
          ...
          //Upper
          UpperNode = mRects.InsertAt(cBitmapRect(NewRect.x, NewRect.y,
                                                  pData->mRect.GetWidth(),
                                                  NewRect.GetHeight(), -2),
                                      TopNode, eBinTreeNode_Left);

          //Lower
          mRects.InsertAt(
            cBitmapRect(NewRect.x, NewRect.y + NewRect.GetHeight(),
                        pData->mRect.GetWidth(),
                        pData->mRect.GetHeight() - NewRect.GetHeight(), -3),
            TopNode, eBinTreeNode_Right);
          ...
          mRects.InsertAt(cBitmapRect(NewRect.x, NewRect.y, NewRect.GetWidth(),
                                      NewRect.GetHeight(), 2),
                          UpperNode, eBinTreeNode_Left);

          //Lower split, this is empty
          mRects.InsertAt(
            cBitmapRect(NewRect.x + NewRect.GetWidth(), NewRect.y,
                        pData->mRect.GetWidth() - NewRect.GetWidth(),
                        NewRect.GetHeight(), -4),
            UpperNode, eBinTreeNode_Right);
        }

        if (apSrc) {
          DrawBitmap(apSrc, Vec2i(NewRect.x + aPadding, NewRect.y + aPadding),
                     abFiltered);
        }

        mvRects.push_back(sRecti(NewRect.x + aPadding, NewRect.y + aPadding,
