...
class line_profile_aa
{
 public:
  ...
  //---------------------------------------------------------------------
  void min_width(agg_real w) { m_min_width = w; }
  void smoother_width(agg_real w) { m_smoother_width = w; }
  ...
  template<class GammaF> void gamma(const GammaF& gamma_function)
  {
    ...
    for(i = 0; i < aa_scale; i++)
    {
      m_gamma[i] = value_type(
          uround(gamma_function(agg_real(i) / aa_mask) * aa_mask));
    }
  }
  ...
  //---------------------------------------------------------------------
  agg_real min_width() const { return m_min_width; }
  agg_real smoother_width() const { return m_smoother_width; }
