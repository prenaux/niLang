...
struct FBlitStretch {
  ...
  void ResampleAndSave(const iBitmap2D* apSrc, const achar* aaszPxfmt,
                       const achar* aaszPath, const tU32 anOutWidth,
                       const tU32 anOutHeight)
  {
    Ptr<iBitmap2D> outBmp =
      graphics->CreateBitmap2D(anOutWidth, anOutHeight, aaszPxfmt);
    sVec4f c = { 1.0f, 0.0f, 0.0f, 1.0f };
    outBmp->Clearf(c);
    outBmp->BlitStretch(apSrc, 0, 0, 0, 0, apSrc->GetWidth(),
                        apSrc->GetHeight(), anOutWidth, anOutHeight);
...
TEST_FIXTURE(FBlitStretch, BasicAtZeroZeroRGBA)
{
  ...
  graphics->SaveBitmap(TEST_FILEPATH(niFmt("%s_Basic.png", m_testName)),
                       outBmp);
}
...
TEST_FIXTURE(FBlitStretch, BasicAtZeroZeroRGB)
{
  ...
  Ptr<iBitmap2D> outBmp = graphics->CreateBitmap2D(1024, 713, "B8G8R8A8");
  outBmp->BlitStretch(bmp, 0, 0, 0, 0, 512, 512, 1024, 713);
  graphics->SaveBitmap(TEST_FILEPATH(niFmt("%s_Basic.jpg", m_testName)),
                       outBmp);
}
...
{
  CHECK_RETURN_IF_FAILED(LoadTestBitmapLogo());
  ResampleAndSave(bmp, _A("B8G8R8"), TEST_FILEPATH(niFmt("%s.png", m_testName)),
                  (tU32)((tF32)bmp->GetWidth() * DOWNSAMPLE_X),
                  (tU32)((tF32)bmp->GetHeight() * DOWNSAMPLE_Y));
}
...
{
  CHECK_RETURN_IF_FAILED(LoadTestBitmapLogo());
  ResampleAndSave(bmp, _A("R8G8B8"), TEST_FILEPATH(niFmt("%s.png", m_testName)),
                  (tU32)((tF32)bmp->GetWidth() * DOWNSAMPLE_X),
                  (tU32)((tF32)bmp->GetHeight() * DOWNSAMPLE_Y));
}
...
TEST_FIXTURE(FBlitStretch, MipMapLogo)
{
  ...
  niLoop (i, bmp->GetNumMipMaps() + 1) {
    ...
    graphics->SaveBitmap(TEST_FILEPATH(niFmt("%s_l%02d.png", m_testName, i)),
                         l);
  }
}
...
{
  CHECK_RETURN_IF_FAILED(LoadTestBitmapA());
  bmp = bmp->CreateResized(ni::GetNextPow2(bmp->GetWidth()),
                           ni::GetNextPow2(bmp->GetHeight()));
  CHECK(bmp->CreateMipMaps(0, eTrue));
  niLoop (i, bmp->GetNumMipMaps() + 1) {
    Ptr<iBitmap2D> l = bmp->GetLevel(i);
    graphics->SaveBitmap(TEST_FILEPATH(niFmt("%s_l%02d.png", m_testName, i)),
...
TEST_FIXTURE(FBlitStretch, CleanupRGBA_DoubleBGR_24)
{
  ...
  Ptr<iBitmap2D> cleanedUpBmp = graphics->CreateBitmap2DEx(
    bmp->GetWidth(), bmp->GetHeight(), bmp->GetPixelFormat());
  ...
  CHECK(cleanedUpBmp->BlitAlpha(
    bmp, 0, 0, 0, 0, bmp->GetWidth(), bmp->GetHeight(), ePixelFormatBlit_Normal,
    sColor4f::One(), sColor4f::One(), eBlendMode_Translucent));
