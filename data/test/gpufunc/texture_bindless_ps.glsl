...
TestGpuFuncs_PixelOutput TestGpuFuncs_VertexOutput_texture_bindless_ps(TestGpuFuncs_VertexOutput aInput, sampler aSS);
...
TestGpuFuncs_PixelOutput TestGpuFuncs_VertexOutput_texture_bindless_ps(TestGpuFuncs_VertexOutput aInput, sampler aSS) {
  vec4 texColor = texture(sampler2D(nil_builtin_AllTextures2D[0],aSS),aInput.tex0);
  vec4 _tmp_7 = (texColor*aInput.color);
  return TestGpuFuncs_PixelOutput_new(_tmp_7);
}
...
void main(void) {
  ...
  TestGpuFuncs_PixelOutput _rval_ = TestGpuFuncs_VertexOutput_texture_bindless_ps(aInput, IN_1_aSS);
