...
#endif
#define MIN_ALLOC_ALIGNMENT \
  MEM_MAX(VECTORMATH_MIN_ALIGN, EA_PLATFORM_MIN_MALLOC_ALIGNMENT)
...
struct sBumpAllocator {
  ...
  void* alloc_aligned(size_t size, size_t alignment)
  {
    ...
    while (true) {
      ...
      char* aligned_ptr =
        (char*)(((uintptr_t)old_current + alignment - 1) & ~(alignment - 1));
      char* new_current = aligned_ptr + size;

      if (new_current > end) {
        __sync_lock();
        if (old_current != current.load(std::memory_order_relaxed))
          continue;
  ...
  #else
    if (!ptr)
      return alloc_aligned(new_size, alignment);
...
#define CHECK_ZERO_MEMORY()                          \
  niDebugAssert(((tU32*)_memZero._buffer)[0] == 0 && \
                ((tU32*)_memZero._buffer)[1] == 0)

static __forceinline void* _internal_aligned_malloc(size_t size,
                                                    size_t alignment,
                                                    const char* f, int l,
                                                    const char* sf)
{
...
static __forceinline void* _internal_malloc(size_t size, const char* f, int l,
                                            const char* sf)
{
                                            ...
                                            const char* sf)
{
  ...
  #if defined USE_MEMORY_TRACKING
  void* ptr =
    mmgrAllocator(f, l, sf, m_alloc_malloc, MIN_ALLOC_ALIGNMENT, size);
  #elif defined USE_NEDMALLOC
...
static __forceinline void* _internal_realloc(void* ptr, size_t size,
                                             const char* f, int l,
                                             const char* sf)
{
  niUnused(f);
...
static __forceinline void _internal_free(void* ptr, const char* f, int l,
                                         const char* sf)
{
  niUnused(f);
  niUnused(l);
...
niExportFunc(void*) ni_malloc(size_t size, const char* f, int l, const char* sf)
{
  ...
  {
    niDebugFmt(
      ("... ni_malloc: size: %d, f: %s, l: %d, sf: %s", size, f, l, sf));
  }
#endif
  return r;
}
                                      ...
                                      const char* f, int l, const char* sf)
{
  ...
  {
    niDebugFmt(
      ("... ni_aligned_malloc: size: %d, alignment: %d, f: %s, l: %d, sf: %s",
...
niExportFunc(void*) ni_realloc(void* ptr, size_t size, const char* f, int l,
                               const char* sf)
{
  CHECK_ZERO_MEMORY();
...
niExportFunc(void) ni_free(void* ptr, const char* f, int l, const char* sf)
{
  ...
  {
    niDebugFmt(
      ("... ni_free: ptr: %p, f: %s, l: %d, sf: %s", (tIntPtr)ptr, f, l, sf));
  }
#endif
}
...
struct sWeakPtrImpl : public iUnknown {
 ...
 public:
  sWeakPtrImpl(iUnknown* apObserved)
      : mpObserved(apObserved)
      , mprotected_nNumRefs(0)
  {
#ifdef COUNT_WEAK_PTR
  ...
  void __stdcall ListInterfaces(iMutableCollection* apLst,
                                tU32 anListFlags) const niOverride
  {
    __sync_local_ptr(iUnknown, pObserved);
    iUnknown* o = pObserved.ptr();
    if (o) {
  ...
  iUnknown* Deref()
  {
    ...
    if (o) {
      if (o->GetNumRefs() <= 0) {
#ifdef _DEBUG
        niLog(Debug, niFmt("Deref: Trying to revive %p with weak pointer.",
                           (tIntPtr)o));
#endif
        return nullptr;
      }
...
#define MEMORY_GET_HEADER(apObjectPtr) \
  ...
  #define OBJECT_MEMORY_DESTRUCT(apObjectPtr)                                     \
    if (apObjectPtr->mpWeakPtr) {                                                 \
      niDebugFmt((                                                                \
        "... Invalidating WeakPtr %p for object %p ; %d weak pointers in total.", \
        (tIntPtr)basePtr->mpWeakPtr,                                              \
        (tIntPtr)basePtr->mpWeakPtr->GetObservedIntPtr(), WEAK_PTR_COUNT));       \
...
niExportFunc(void*) ni_object_alloc(size_t anSize, const achar* file, int line,
                                    const achar* fun)
{
...
niExportFunc(void) ni_object_free(void* apObjectPtr, const char* file, int line,
                                  const char* fun)
{
  CHECK_ZERO_MEMORY();
#ifdef COUNT_ALLOCS
...
niExportFunc(iUnknown*) ni_object_get_weak_ptr(iUnknown* apObject)
{
  ...
  iUnknown* pBaseObject =
    apObject ? apObject->QueryInterface(niGetInterfaceUUID(iUnknown)) : nullptr;
  if (!pBaseObject)
    return nullptr;
  ...
  if (!basePtr->mpWeakPtr) {
    ...
    niDebugFmt(("... New WeakPtr %p for object %p ; %d weak pointers in total.",
                (tIntPtr)basePtr->mpWeakPtr,
                (tIntPtr)basePtr->mpWeakPtr->mpObjectMemory, WEAK_PTR_COUNT));
#endif
  }
...
niExportFunc(iUnknown*) ni_object_deref_weak_ptr(iUnknown* apWeakPtr)
{
  ...
  niDebugAssert(apWeakPtr->QueryInterface(niGetInterfaceUUID(iWeakPtr)) !=
                nullptr);
#endif
