...
struct get_type_index<I, T, Head, Types...> {
  static const int value =
    is_same_v<T, Head> ? I : get_type_index<I + 1, T, Types...>::value;
};
...
struct type_count<T, H, Types...> {
  static const int value =
    (is_same_v<T, H> ? 1 : 0) + type_count<T, Types...>::value;
};
...
struct unique_type_list : public unique_type_list<Ts...> {
  ...
  typedef decltype(types(
    declval<typename unique_type_list<Ts...>::type>())) type;
};
...
EA_DISABLE_VC_WARNING(
  4242 4244) // conversion from 'T' to 'U', possible loss of data.
template <typename T, typename OverloadSet,
          typename ResultT = decltype(declval<OverloadSet>()(declval<T>()))>
...
using overload_resolution_t =
  typename overload_resolution<decay_t<T>, OverloadSet>::type;
