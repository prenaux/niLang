...
template <class T>
tBool TriangleIntersectRay(const T* orig, const T* dir, const T* vert0,
                           const T* vert1, const T* vert2, T* outt = NULL,
                           T* outu = NULL, T* outv = NULL,
                           T epsilon = niEpsilon5);

template <typename T>
sVec3<T>& TriangleBaryCentric(sVec3<T>& vOut, const sVec3<T>& A,
                              const sVec3<T>& B, const sVec3<T>& C,
                              const sVec3<T>& P);

template <class T>
tBool TriangleIntersectRayCull(const T* orig, const T* dir, const T* vert0,
                               const T* vert1, const T* vert2, T* t, T* u, T* v,
                               T epsilon = niEpsilon5);

template <typename T>
inline tF32 TriangleAreaAndNormal(sVec3<T>& avN, const sVec3<T>& avA,
                                  const sVec3<T>& avB, const sVec3<T>& avC);

template <typename T>
inline tBool TriangleIsDegenerate(const sVec3<T>& avA, const sVec3<T>& avB,
                                  const sVec3<T>& avC);

template <typename T>
inline tBool TriangleIntersectAABB(const sVec3<T>& A, const sVec3<T>& B,
                                   const sVec3<T>& C, const sVec3<T>& vAABBMin,
                                ...
                                T epsilon)
{
  ...
  if (fabsf(du0) < epsilon)
    du0 = 0.0;
  if (fabsf(du1) < epsilon)
    du1 = 0.0;
  if (fabsf(du2) < epsilon)
    du2 = 0.0;
  ...
  if (du0du1 > T(0) &&
      du0du2 > T(0)) // same sign on all of them + not equal 0 ?
  ...
  if (fabsf(dv0) < epsilon)
    dv0 = 0.0;
  if (fabsf(dv1) < epsilon)
    dv1 = 0.0;
  if (fabsf(dv2) < epsilon)
    dv2 = 0.0;
  ...
  if (dv0dv1 > T(0) &&
      dv0dv2 > T(0)) // same sign on all of them + not equal 0 ?
    return eFalse;   // no intersection occurs
...
template <class T>
tBool TriangleIntersectRay(const T* orig, const T* dir, const T* vert0,
                           const T* vert1, const T* vert2, T* outt, T* outu,
                           T* outv, T epsilon)
{
