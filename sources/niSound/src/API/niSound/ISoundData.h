...
#define niSoundFormatBuild(BITS, CHANNELS, EXTRA)             \
  ((tU32)((tU32(BITS & 0xFF) | (tU32(CHANNELS & 0xFF) << 8) | \
           (tU32(EXTRA & 0xFFFF) << 16))))
...
{
  return SoundFormatBuild(anBits, SoundFormatGetChannels(anFormat),
                          SoundFormatGetExtra(anFormat));
}
inline tU32 SoundFormatSetChannels(tU32 anFormat, tU8 anChannels)
{
  return SoundFormatBuild(SoundFormatGetBits(anFormat), anChannels,
                          SoundFormatGetExtra(anFormat));
}
inline tU32 SoundFormatSetExtra(tU32 anFormat, tU16 anExtra)
{
  return SoundFormatBuild(SoundFormatGetBits(anFormat),
                          SoundFormatGetChannels(anFormat), anExtra);
}
