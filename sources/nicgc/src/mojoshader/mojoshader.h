...
#define MOJOSHADER_TRACE_PREPROCESS //niDebugFmt
#define MOJOSHADER_TRACE_COMPILE //niDebugFmt

#if defined niAndroid || defined niIOS || defined niJSCC || defined niQNX
#define MOJOSHADER_GLSL_GLES
#elif defined niWindows || defined niOSX || defined niLinux
#define MOJOSHADER_GLSL_DESKTOP
#else
...
#define MOJOSHADER_CHANGESET "???"
  ...
  typedef enum
  {
    ...
    MOJOSHADER_ATTRIBUTE_UNSIGNED_BYTE,
    MOJOSHADER_ATTRIBUTE_SHORT,
    MOJOSHADER_ATTRIBUTE_UNSIGNED_SHORT,
    MOJOSHADER_ATTRIBUTE_INT,
    MOJOSHADER_ATTRIBUTE_UNSIGNED_INT,
...
#define MOJOSHADER_PROFILE_METAL "metal"
  ...
  // Set whether the GLSL translator will unpack the MOJOSHADER_USAGE_COLOR
  // attribute using unpackColor4
  // \remark This is used when the shader is translated, normally its set only
  //         once dependening on the device capabilities, but if not care must
  //         be taken to ensure that this flag is set correctly before the
  //         translation occurs (usually before MOJOSHADER_glCompileShader).
  void MOJOSHADER_GLSL_SetFloatColor(bool abUseFloatColor);
  // Get whether the GLSL translator will unpack the MOJOSHADER_USAGE_COLOR attribute using unpackColor4
  bool MOJOSHADER_GLSL_GetFloatColor();

  // OES_standard_derivatives
  void MOJOSHADER_GLSL_SetStandardDerivatives(bool abStandardDerivatives);
  bool MOJOSHADER_GLSL_GetStandardDerivatives();

  // texture2D/Cube/3DLod is supported in the fragment shader
  enum eMOJOSHADERTextureLod {
    eMOJOSHADERTextureLod_None = 0,
    eMOJOSHADERTextureLod_ARB = 1, // 'regular' textureTYPELod call
    eMOJOSHADERTextureLod_EXT = 2  // adds 'EXT' to textureTYPELod for fun, so it becomes textureTYPELodEXT
  };
  void MOJOSHADER_GLSL_SetTextureLod(eMOJOSHADERTextureLod aTextureLod);
