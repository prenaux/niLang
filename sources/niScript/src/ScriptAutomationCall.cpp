...
niExportFunc(int) sqa_pushvar(HSQUIRRELVM v, const Var& aVar)
{
  ...
  case eType_I8: {
    v->Push((SQInt)aVar.GetI8());
    return SQ_OK;
  }
  case eType_U8: {
    v->Push((SQInt)aVar.GetU8());
    return SQ_OK;
  }
  case eType_I16: {
    v->Push((SQInt)aVar.GetI16());
    return SQ_OK;
  }
  case eType_U16: {
    v->Push((SQInt)aVar.GetU16());
    return SQ_OK;
  }
  case eType_I32: {
    v->Push((SQInt)aVar.GetI32());
    return SQ_OK;
  }
  case eType_U32: {
    v->Push((SQInt)aVar.GetU32());
    return SQ_OK;
  }
  case eType_I64: {
    v->Push((SQInt)(aVar.GetI64() & 0xFFFFFFFF));
    return SQ_OK;
  }
  case eType_U64: {
    v->Push((SQInt)(aVar.GetU64() & 0xFFFFFFFF));
    return SQ_OK;
  }
  case eType_F32: {
    v->Push((SQFloat)aVar.GetF32());
    return SQ_OK;
  }
  case eType_F64: {
    v->Push((SQFloat)aVar.GetF64());
    return SQ_OK;
  }
  case eType_UUID: {
  ...
  case eType_AChar: {
    ...
    sq_pushstring(v,
                  _H(aVar.GetACharConstPointer() ? aVar.GetACharConstPointer()
  ...
  default: {
    niSQThrowError(
      niFmt(_A("Variant type '%d' is not handled."), niType(aVar.GetType())));
  }
  }
}
...
///////////////////////////////////////////////
niExportFunc(int) sqa_getvar_astype(HSQUIRRELVM v, int idx, Var* apVar,
                                    const tType type)
{
  switch (niType(type)) {
  case eType_Null: {
    if (sqa_getscripttype(v, idx) != eScriptType_Null)
      niSQThrowError(
        niFmt(_A("Stack position '%d' isn't a null object."), idx));
    apVar->SetNull();
    return SQ_OK;
  }
  case eType_I8: {
  ...
  case eType_I16: {
    ...
    if (!SQ_SUCCEEDED(sq_getint(v, idx, &val)))
      return sq_throwerror(
        v, niFmt(_A("Can't get I16 at stack position '%d'."), idx));
    *apVar = (tI16)val;
    return SQ_OK;
  }
  case eType_U16: {
  ...
  case eType_Matrixf: {
    ...
    if (!SQ_SUCCEEDED(sqa_getmatrixf(v, idx, &val)))
      niSQThrowError(
        niFmt(_A("Can't get matrixf at stack position '%d'."), idx));
    *apVar = val;
    return SQ_OK;
  }
  case eType_String: {
    ...
    if (!SQ_SUCCEEDED(sq_getstring(v, idx, &aszStr)))
      niSQThrowError(
        niFmt(_A("Can't get CString at stack position '%d'."), idx));
  ...
  case eType_AChar: {
    if (!niFlagIs(type, ((tU32)(eTypeFlags_Constant | eTypeFlags_Pointer)))) {
      ...
      *apVar = (tI32)n;
      ...
      if (!SQ_SUCCEEDED(sq_getstring(v, idx, &aszStr)))
        niSQThrowError(
          niFmt(_A("Can't get const cchar* at stack position '%d'."), idx));
      *apVar = aszStr;
      return SQ_OK;
    }
  }
  case eType_IUnknown: {
    ...
    if (!SQ_SUCCEEDED(sqa_getIUnknown(v, idx, &pIUnk, kuuidZero)))
      niSQThrowError(
        niFmt(_A("Can't get IUnknown at stack position '%d'."), idx));
  ...
  case eType_Variant: {
    if (!SQ_SUCCEEDED(sqa_getvar(v, idx, apVar)))
      niSQThrowError(
        niFmt(_A("Can't get variant at stack position '%d'."), idx));
    return SQ_OK;
  }
  case eType_UUID: {
    tUUID uuid;
...
niExportFunc(int) sqa_getvar(HSQUIRRELVM v, int idx, Var* apVar)
{
  ...
  case eScriptType_UserData: niSQThrowError(_A("Invalid script type."));
...
struct sVMCallWrapper {
  ...
  inline tBool Init(HSQUIRRELVM vm, const ni::achar* aaszCallerName,
                    ...
                    const sInterfaceDef* apInterfaceDef)
  {
    ...
    if (_vmcall == NULL)
      return WriteError(_A("method has no xcall wrapper."));
    return eTrue;
  }
  ...
  inline tBool DoCall(iUnknown* apInst, tInt aStackBase, tU32 numPassed)
  {
    ...
    if (numPassed != _meth->mnNumParameters) {
      return WriteError(
        niFmt("Invalid number of parameters, expected '%s' but got '%s'.",
              _meth->mnNumParameters, numPassed));
    }
    const ni::tU32 numParams = ni::Min(numPassed, _meth->mnNumParameters);
    if (_meth->mpParameters) {
      niLoop (i, numParams) {
        if (!SQ_SUCCEEDED(sqa_getvar_astype(_vm, i + aStackBase, &params[i],
                                            _meth->mpParameters[i].mType)))
        {
          return WriteError(
            niFmt("Can't get param '%d', expected '%s' but got '%s'.", i,
  ...
  {
    _err =
      niFmt(_A("%s: %s::%s/%d: "), _caller, _intf ? _intf->maszName : _A(""),
            _meth->maszName, _meth->mnNumParameters);
    if (aaszErr)
      _err << aaszErr;
    return eFalse;
  }
...
//======================================================================
  ...
  #define GET_IPUD(NAME, INDEX)                        \
    Ptr<sScriptTypeIndexedProperty> ptr##NAME =        \
      sqa_getud<sScriptTypeIndexedProperty>(v, INDEX); \
    if (!ptr##NAME.IsOK())                             \
      return SQ_ERROR;
...
int indexedproperty_set(HSQUIRRELVM v)
{
  ...
  if (pMethodDef == NULL)
    return sq_throwerror(v,
                         niFmt(_A("indexedproperty_set, %s::%s, no setter."),
...
int indexedproperty_get(HSQUIRRELVM v)
{
  ...
  if (pMethodDef == NULL)
    return sq_throwerror(v,
                         niFmt(_A("indexedproperty_get, %s::%s, no getter."),
                               pInterfaceDef ? pInterfaceDef->maszName : _A(""),
                               ptrV->pProp->GetName().Chars()));
...
bool SQVM::CallMethodDef(sScriptTypeMethodDef* meth, int nargs, int stackbase,
                         SQObjectPtr& retval)
{
  niAssert(nargs > 0);
  ...
  if (!xcall.Init(v, __FUNCTION__, pMethodDef, pInterfaceDef)) {
    VM_ERRORB(niFmt(_A("Call to %s::%s/%d failed. No VMCall Wrapper."),
                    pInterfaceDef ? pInterfaceDef->maszName : _A(""),
                    pMethodDef->maszName, pMethodDef->mnNumParameters));
  }
  ...
  #else
    if (!SQ_SUCCEEDED(pAutomation->GetIUnknown(this, 1, &pStackInst,
                                               *pInterfaceDef->mUUID)) ||
        !pStackInst)
    {
  ...
  #else
      if (!SQ_SUCCEEDED(
            pAutomation->GetIUnknown(this, 1, &pStackInst, kuuidZero)) ||
          !pStackInst)
      {
...
///////////////////////////////////////////////
bool cScriptAutomation::Get(HSQUIRRELVM v, iUnknown* apObj,
                            ...
                            int opExt)
{
  ...
  if (sq_type(key) == OT_INTEGER) {
    ...
    if (index < 0 || index >= (SQInt)pCollection->GetSize()) {
      if (!(opExt & _OPEXT_GET_SAFE)) {
        niWarning(niFmt("Get[int], index %d out of range [0-%d].", index,
                        pCollection->GetSize() - 1));
      }
      niSqUnGuard(v);
      return false;
    }
  ...
  case OT_USERDATA: {
    ...
    case eScriptType_PropertyDef: {
      ...
      if (pMethodDef == NULL || pMethodDef->mnNumParameters > 0) {
        ...
        if (!pObject) {
          if (!(opExt & _OPEXT_GET_SAFE)) {
            niWarning(niFmt(
              "Can't query interface '%s' of '%p' when getting property '%s'.",
    ...
    if (!(opExt & _OPEXT_GET_SAFE)) {
      niWarning(
        niFmt("'%s': Not a method or property getter.", _stringhval(key)));
    }
    niSqUnGuard(v);
    return false;
  }
...
///////////////////////////////////////////////
bool cScriptAutomation::Set(HSQUIRRELVM v, iUnknown* apObj,
