...
TEST_FIXTURE(FFont, IconFontBase)
{
  ...
  niDebugFmt(
    ("... font %s loaded, %s", font->GetName(), font->GetFamilyName()));
  const tBool isFontAwesomeFontFamily =
    _ASTR(niHStr(font->GetFamilyName())).icontains("Font Awesome");
  ...
  niDebugFmt(("... font %s has %d glyphs", font->GetName(), glyphs->size()));
  {
    ...
    for (auto& glyph : *glyphs) {
      niDebugFmt(("... glyph: index: %d, codepoint: 0x%x, name: %s",
                  glyph.first, glyph.second, font->GetGlyphName(glyph.first)));
#if !defined _DEBUG
      if (c++ > 10)
        break;
#endif
...
TEST_FIXTURE(FFont, OSFonts)
{
  ...
  for (auto& osFont : *osFonts) {
    niDebugFmt(("... osFont: %s = %s", osFont.first, osFont.second));
  }
}
...
struct DrawTextIcons : public TestCanvasWidgetSink {
  ...
  niFn(tBool) OnSinkAttached() niImpl
  {
    ...
    niLet tex =
      niCheckNN(tex,
                graphics->CreateTextureFromBitmap(_H(fp->GetSourcePath()), bmp,
    ...
    _iconFont->SetSizeAndResolution(
      Vec2f(64, 64), 64, mpWidget->GetUIContext()->GetContentsScale());

    _iconFont->SetColor(0xFF0000FF);
    niLet glyphIndexSearchIcon =
      _iconFont->GetGlyphIndexFromName("magnifying-glass");
    _glyphSearch = niCheckNN(
      _glyphSearch, _iconFont->GetGlyphOverlay(glyphIndexSearchIcon), eFalse);

    _iconFont->SetColor(0xFFFF0000);
    _glyphHeart = niCheckNN(
      _glyphHeart,
      _iconFont->GetGlyphOverlay(_iconFont->GetGlyphIndexFromName("heart")),
      eFalse);
  ...
  void PaintTest(iCanvas* apCanvas) niImpl
  {
    ...
    apCanvas->BlitOverlay(Rectf(5, 5, _bgPic->GetSize().x, _bgPic->GetSize().y),
                          _bgPic);

    apCanvas->BlitOverlay(Rectf(_bgPic->GetSize().x - _glyphSearch->GetSize().x,
                                5, _glyphSearch->GetSize().x,
                                _glyphSearch->GetSize().y),
                          _glyphSearch);

    apCanvas->BlitOverlay(
      Rectf((_bgPic->GetSize().x - _glyphHeart->GetSize().x) / 2.0f,
