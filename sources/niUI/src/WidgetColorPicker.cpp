 ...
 public:
  ...
  virtual void PickColor(const sVec2f& avPos)
  {
    ...
    case DRAGGING_HUE: {
      sColor3f hsv = GetHSV();
      hsv.x =
        ni::ClampZeroOne((avPos.x - mrectH.GetLeft()) / mrectH.GetWidth());
    ...
    case DRAGGING_SV: {
      sColor3f hsv = GetHSV();
      hsv.y =
        ni::ClampZeroOne((avPos.x - mrectSV.GetLeft()) / mrectSV.GetWidth());
      hsv.z = 1.0f - ni::ClampZeroOne((avPos.y - mrectSV.GetTop()) /
                                      mrectSV.GetHeight());
      ...
      if (hsv.y == 0.0f)
        ...
        hsv.y = 1.0f - clampEpsilon;
      if (hsv.z == 0.0f)
        hsv.z = clampEpsilon;
      else if (hsv.z == 1.0f)
        hsv.z = 1.0f - clampEpsilon;
      _SetHSV(hsv, mnDragging);
      break;
    }
    case DRAGGING_ALPHA: {
      sColor4f rgba = GetRGBA();
      rgba.w = 1.0f - ni::ClampZeroOne((avPos.y - mrectA.GetTop()) /
                                       mrectA.GetHeight());
      _SetRGBA(rgba, mnDragging);
      break;
    }
    case DRAGGING_B: {
      tF32 b = mfMaxBrightness * ni::ClampZeroOne((avPos.x - mrectB.GetLeft()) /
                                                  mrectB.GetWidth());
  ...
  ///////////////////////////////////////////////
  virtual tBool __stdcall OnMouseMove(const sVec2f& avPos,
                                      const sVec2f& avNCPos)
  {
    PickColor(avPos);
    return eFalse;
  }
  virtual tBool __stdcall OnNCMouseMove(const sVec2f& avPos,
                                        const sVec2f& avNCPos)
  {
  ...
  ///////////////////////////////////////////////
  virtual tBool __stdcall OnNCLeftClickDown(const sVec2f& avPos,
                                            const sVec2f& avNCPos)
  {
    return OnLeftClickDown(avPos, avNCPos);
  }
  virtual tBool __stdcall OnLeftClickDown(const sVec2f& avPos,
                                          const sVec2f& avNCPos)
  {
    mnDragging = DRAGGING_NO;
  ...
  virtual tBool __stdcall OnLeftClickUp(const sVec2f& avPos,
                                        const sVec2f& avNCPos)
  {
    if (mnDragging) {
  ...
  virtual tBool __stdcall OnNCLeftClickUp(const sVec2f& avPos,
                                          const sVec2f& avNCPos)
  {
  ...
  virtual tBool __stdcall OnNCPaint(const sVec2f& avMousePos, iCanvas* apCanvas)
  {
    ...
    //                sVec2f::Zero(),
    //                mpWidget->GetSize());
    apCanvas->BlitOverlayFrame(mpWidget->GetWidgetRect(), skin.frame,
                               eRectFrameFlags_All);
    return eTrue;
  }
  ...
  virtual tBool __stdcall OnPaint(const sVec2f& avMousePos, iCanvas* apCanvas)
  {
    ...
    sColor4f colBrightRGBA =
      ColorConvert_HSV_RGB(Vec4<tF32>(colHSV.x, 1, 1, 1));
    sColor4f colRGBA = GetRGBA();

    // draw the hue
    apCanvas->Flush();
    apCanvas->SetMaterial(NULL);
    apCanvas->GetMaterial()->SetChannelTexture(
      eMaterialChannel_Base,
      GetTextureHue(apCanvas->GetGraphicsContext()->GetGraphics()));
    //      apCanvas->Rect(mrectH.GetTopLeft(),
    //              mrectH.GetBottomRight(),
    //            0.0f);
    apCanvas->Rect(sVec2f::Zero(), mpWidget->GetClientRect().GetSize(), 0.0f);
    apCanvas->Flush();
    ...
    // draw the saturation/brightness
    {
      ...
      apCanvas->RectTA2(mrectSV.GetTopLeft(), mrectSV.GetBottomRight(),
                        sVec2f::Zero(), sVec2f::Zero(), 0.0f,
                        ULColorBuild(colTL), ULColorBuild(colTR),
                        ULColorBuild(colBR), ULColorBuild(colBL));
    }
    ...
    rect = mrectRGB;
    apCanvas->RectA(rect.GetTopLeft(), rect.GetBottomRight(), 0.0f,
    ...
    rect = mrectA;
    apCanvas->RectTA2(
      rect.GetTopLeft(), rect.GetBottomRight(), sVec2f::Zero(), sVec2f::Zero(),
      0.0f, ULColorBuild(sColor4f::White()), ULColorBuild(sColor4f::White()),
      ULColorBuild(sColor4f::Black()), ULColorBuild(sColor4f::Black()));
    ...
    if (niFlagIs(mpWidget->GetStyle(), eWidgetColorPickerStyle_Brightness)) {
      ...
      rect = mrectB;
      sRectf rectGradA =
        sRectf(rect.GetLeft(), rect.GetTop(),
               rect.GetWidth() * (1.0f / mfMaxBrightness), rect.GetHeight());
      ...
      apCanvas->RectTA2(rectGradA.GetTopLeft(), rectGradA.GetBottomRight(),
                        sVec2f::Zero(), sVec2f::Zero(), 0.0f,
                        ULColorBuild(sColor4f::Black()), ULColorBuild(colRGBA),
                        ULColorBuild(colRGBA), ULColorBuild(sColor4f::Black()));
      apCanvas->RectTA2(rectGradB.GetTopLeft(), rectGradB.GetBottomRight(),
                        sVec2f::Zero(), sVec2f::Zero(), 0.0f,
                        ULColorBuild(colRGBA), ULColorBuild(sColor4f::White()),
                        ULColorBuild(sColor4f::White()), ULColorBuild(colRGBA));
    }
    ...
    apCanvas->SetColorA(ULColorBuild(
      (sColor3f::One() - (sColor3f&)colBrightRGBA), _kfMarkerAlpha));
    apCanvas->Rect(Vec2<tF32>(xH - 1, mrectH.GetTop()),
                   Vec2<tF32>(xH + 1, mrectH.GetBottom()), 0.0f);
    ...
    apCanvas->SetColorA(ULColorBuild(
      (sColor3f::One() - (sColor3f&)colBrightRGBA), _kfMarkerAlpha));
    apCanvas->Rect(Vec2<tF32>(xS - 1, mrectSV.GetTop()),
                   Vec2<tF32>(xS + 1, mrectSV.GetBottom()), 0.0f);
    apCanvas->Rect(Vec2<tF32>(mrectSV.GetLeft(), xV - 1),
                   Vec2<tF32>(mrectSV.GetRight(), xV + 1), 0.0f);

    // draw the alpha line
    apCanvas->SetColorA(_knAlphaSelectionLine);
    tF32 xA = ni::Lerp(mrectA.GetBottom(), mrectA.GetTop(), mHSVA.w);
    apCanvas->Rect(Vec2<tF32>(mrectA.GetLeft(), xA - 1),
                   Vec2<tF32>(mrectA.GetRight(), xA + 1), 0.0f);

    if (niFlagIs(mpWidget->GetStyle(), eWidgetColorPickerStyle_Brightness)) {
      // draw the brightness line
      apCanvas->SetColorA(_knAlphaSelectionLine);
      tF32 xB = ni::Lerp(mrectB.GetLeft(), mrectB.GetRight(),
                         mfBrightness / mfMaxBrightness);
      apCanvas->Rect(Vec2<tF32>(xB - 1, mrectB.GetTop()),
                     Vec2<tF32>(xB + 1, mrectB.GetBottom()), 0.0f);
    }
  ...
  virtual ni::iTexture* GetTextureHue(iGraphics* apGraphics)
  {
    ...
    for (tU32 x = 0; x < ptrBmp->GetWidth(); ++x) {
      sColor4f col =
        ColorConvert_HSV_RGB(Vec4<tF32>(tF32(x) * fInvWidth, 1, 1, 1));
      ptrBmp->PutPixelf(x, 0, col);
      ptrBmp->PutPixelf(x, 1, col);
    }

    mptrTextureHue = apGraphics->CreateTextureFromBitmap(
      _H("ColorPicker_Hue"), ptrBmp, eTextureFlags_Default);
    return mptrTextureHue;
  }
  ...
  #define NewLine(H) \
    ...
    mrectSV =
      sRectf(x, y, (clW * _kfRectSVRatioSize) - _kfControlsHSpacing, svH);
  ...
  void __stdcall _SetRGBA(const sColor4f& avRGBA, tU32 anColorPickerState)
  {
    sColor4f colRGBA;
    ni::VecMaximize(
      (sColor4f&)colRGBA, (sColor4f&)sColor4f::Zero(),
  ...
  ///////////////////////////////////////////////
  virtual void __stdcall _SetBrightness(const tF32 afBrightness,
                                        tU32 anColorPickerState)
  {
  ...
  {
    sColor4f colRGBA = GetRGBA();
    niThis(cWidgetColorPicker)->mhspExpr =
      _H(niFmt(_A(":RGBA(%g,%g,%g,%g)"), ni::ClampZeroOne(colRGBA.x),
               ni::ClampZeroOne(colRGBA.y), ni::ClampZeroOne(colRGBA.z),
