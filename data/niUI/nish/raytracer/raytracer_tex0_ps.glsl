...
bool niUIGpuFuncs_IntersectionResult_SetFromCommittedIntersection(inout /* mut */ niUIGpuFuncs_IntersectionResult r, rayQueryEXT aRayQuery) {
  ...
  bool _tmp_p = (committedType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_p) {
    ...
    bool _tmp_S = (committedType == nish_std_RayQueryIntersectionType_CommittedBoundingVolume);
    if (_tmp_S) {
...
void niUIGpuFuncs_IntersectionResult_SetFromTriangleData(inout /* mut */ niUIGpuFuncs_IntersectionResult r, uint aVBIndex, uint aIBIndex, uint aPrimIndex, uint aFirstIndex, uint aBaseVertexIndex, vec2 aIntersectionBaryCentrics, mat4x3 aObjectToWorld, bool aIsFrontFacing) {
  ...
  if (_tmp_Z) {
  ...
  else {
    {
      ...
      bool _tmp_I1 = (aIsFrontFacing == false);
      if (_tmp_I1) {
...
nish_std_RayDesc niUIGpuFuncs_MakeRayDesc(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms) {
  ...
  mat4 _tmp_E3 = aUniforms.cameraInvView;
  vec3 origin = vec3(_tmp_E3[3][0],_tmp_E3[3][1],_tmp_E3[3][2]);
  vec3 target = nish_std_Vec3TransformCoord(ndc,aUniforms.cameraInvViewProj);
  vec3 dir = normalize(((target-(origin.xyz)).xyz));
  vec3 _tmp_P3 = origin;
  vec3 _tmp_Q3 = dir;
  float _tmp_R3 = 0.001;
  float _tmp_S3 = aUniforms.cameraFarClipPlane;
  return nish_std_RayDesc_new(_tmp_P3, _tmp_Q3, _tmp_R3, _tmp_S3);
}
niUIGpuFuncs_IntersectionResult niUIGpuFuncs_TraceRay_6_uint_Vec3_Vec3_float_float_RayInstances(uint aRayFlags, vec3 aOrigin, vec3 aDir, float aMinT, float aMaxT, accelerationStructureEXT aAS) {
  /* mut */ rayQueryEXT rayQuery/*__noinit__*/;
  rayQueryInitializeEXT(rayQuery,aAS,aRayFlags,255,aOrigin,aMinT,aDir,aMaxT);
  while(true) {
    bool _tmp_24 = rayQueryProceedEXT(rayQuery);
    if (!(_tmp_24)) {
      break;
    }
    uint candidateType = rayQueryGetIntersectionTypeEXT(rayQuery,false);
    bool _tmp_74 = (candidateType == nish_std_RayQueryCandidateIntersectionType_CandidateTriangle);
    if (_tmp_74) {
  ...
  bool _tmp_f4 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_f4) {
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_tex0_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_F4 = niUIGpuFuncs__op__eq_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_Triangle);
  if (_tmp_F4) {
    uint primIndex = intersection.primIndex;
    ...
    vec3 bary = intersection.bary;
    vec2 uv;
    bool _tmp_i5 = (bary.x > 0.85);
    if (_tmp_i5) {
      uv = v0.tex0;
    }
    else {
      bool _tmp_n5 = (bary.y > 0.85);
      if (_tmp_n5) {
        uv = v1.tex0;
      }
      else {
        bool _tmp_s5 = (bary.z > 0.85);
        if (_tmp_s5) {
          uv = v2.tex0;
        }
        else {
    ...
    bool _tmp_E5 = niUIGpuFuncs__op__gte_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_Triangle);
    if (_tmp_E5) {
      color = vec4(intersection.uv.x,intersection.uv.y,0.0,1.0);
    }
    else {
      bool _tmp_Q5 = niUIGpuFuncs__op__lt_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_None);
      if (_tmp_Q5) {
  ...
  vec4 _tmp_46 = color;
  return nish_std_PixelOutput_new(_tmp_46);
}
