...
struct sFRayPipeline_Base : public sFGDRV_Base {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Setup display quad
    _displayVABuffer =
      niCheckNN(_displayVABuffer,
                _driverGpu->CreateGpuBuffer(
                  _H("RayDisplay_VA"), sizeof(tVertexCanvas) * 4,
                  eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Vertex),
                eFalse);
    tVertexCanvas* verts = (tVertexCanvas*)_displayVABuffer->Lock(
      0, _displayVABuffer->GetSize(), eLock_Discard);
    niCheck(verts != nullptr, eFalse);
    verts[0] = {
      { -0.8f, 0.8f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 0.0f, 0.0f }
    }; // TL
    verts[1] = {
      { 0.8f, 0.8f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 1.0f, 0.0f }
    }; // TR
    verts[2] = {
      { 0.8f, -0.8f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 1.0f, 1.0f }
    }; // BR
    verts[3] = {
      { -0.8f, -0.8f, 0.0f }, sVec3f::YAxis(), 0xFFFFFFFF, { 0.0f, 1.0f }
    }; // BL
    _displayVABuffer->Unlock();

    _displayIABuffer =
      niCheckNN(_displayIABuffer,
                _driverGpu->CreateGpuBuffer(
                  _H("RayDisplay_IA"), sizeof(tU32) * 6,
                  eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_Index),
                eFalse);
    tU32* inds = (tU32*)_displayIABuffer->Lock(0, _displayIABuffer->GetSize(),
                                               eLock_Discard);
    ...
    _displayVertexGpuFun = niCheckNN(
      _displayVertexGpuFun,
      _driverGpu->CreateGpuFunction(eGpuFunctionType_Vertex,
                                    _H("test/nish/gpu/texture_vs.gpufunc.xml")),
      eFalse);
    _displayPixelGpuFun = niCheckNN(
      _displayPixelGpuFun,
      _driverGpu->CreateGpuFunction(eGpuFunctionType_Pixel,
                                    _H("test/nish/gpu/texture_ps.gpufunc.xml")),
      eFalse);
    ...
    _displayPipeline = niCheckNN(
      _displayPipeline,
      _driverGpu->CreateGpuPipeline(_H("RayDisplay_Pipeline"), pipelineDesc),
      eFalse);
...
struct sFRayPipeline_Triangle : public sFRayPipeline_Base {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    {
      _rayGenFun =
        niCheckNN(_rayGenFun,
                  _driverGpu->CreateGpuFunction(
                    eGpuFunctionType_RayGeneration,
                    _H("test/nish/raypipeline/triangle_rgen.gpufunc.xml")),
                  eFalse);

      _rayMissFun =
        niCheckNN(_rayMissFun,
                  _driverGpu->CreateGpuFunction(
                    eGpuFunctionType_RayMiss,
                    _H("test/nish/raypipeline/triangle_rmiss.gpufunc.xml")),
                  eFalse);

      _rayHitFun =
        niCheckNN(_rayHitFun,
                  _driverGpu->CreateGpuFunction(
                    eGpuFunctionType_RayClosestHit,
                    _H("test/nish/raypipeline/triangle_rchit.gpufunc.xml")),
                  eFalse);
    }
    ...
    {
      _rayFuncTable =
        niCheckNN(_rayFuncTable, _driverRay->CreateRayFunctionTable(), eFalse);
      ...
      niLet hitGroupId = _rayFuncTable->AddHitGroup(
        _H("triangle"), eRayFunctionGroupType_Triangles, _rayHitFun,
      ...
      _rayPipeline = niCheckNN(_rayPipeline,
                               _driverRay->CreateRayPipeline(
                                 _H("RayTriangle_Pipeline"), _rayFuncTable),
                               eFalse);
    }
    ...
    // Create acceleration structure
    {
      ...
      // Add a triangle
      {
        niLet prDesc = niCheckNN(prDesc,
                                 _driverRay->CreateRayTrianglePrimitivesDesc(
                                   _H("RayTrianglePrimitivesDesc_Triangle")),
                                 eFalse);

        niLet triangleVB =
          MakeTriVB(_driverGpu, ++_numTriVB, 1.0f, Vec3f(0, 0, 0.3f));
        ...
        niLet primitiveAS =
          niCheckNN(primitiveAS,
                    buildEncoder->BuildRayTrianglePrimitives(
                      _H("RayTrianglePrimitives_Triangle"), prDesc),
                    eFalse);

        niCheck(instDesc->AddInstance(primitiveAS,
      ...
      _instanceAS = niCheckNN(
        _instanceAS,
        buildEncoder->BuildRayInstances(_H("RayInstances_Triangle"), instDesc),
        eFalse);
    }
    ...
    {
      _rayOutputImage =
        niCheckNN(_rayOutputImage,
                  _graphics->CreateTexture(_H("rayOutputImage"), eBitmapType_2D,
                                           "R8G8B8A8", 0, 256, 256, 0,
  ...
  niFn(tBool) OnPaint(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    NN<iRayCommandEncoder> rayEncoder =
      AsNN(QueryInterface<iRayCommandEncoder>(gpuEncoder));
    rayEncoder->SetRayInstances(_instanceAS);
    rayEncoder->SetRayOutputImage(_rayOutputImage);
    rayEncoder->SetRayPipeline(_rayPipeline);
    rayEncoder->DispatchRays(_rayOutputImage->GetWidth(),
                             _rayOutputImage->GetHeight(), 1);
...
struct sFRayPipeline_Quad : public sFRayPipeline_Base {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Create ray tracing pipeline
    {
      ...
      niLet hitGroupId = _rayFuncTable->AddHitGroup(
        _H("quad"), eRayFunctionGroupType_Triangles, _rayHitFun,
        nullptr, // No any-hit shader
        nullptr  // No intersection shader (using built-in quad intersection)
      );
      niCheck(hitGroupId != eInvalidHandle, eFalse);

      _rayPipeline = niCheckNN(
        _rayPipeline,
    ...
    // Create acceleration structure
    {
...

      {
        niLet prDesc = niCheckNN(prDesc,
                                 _driverRay->CreateRayTrianglePrimitivesDesc(
                                   _H("RayTrianglePrimitivesDesc_Quad")),
                                 eFalse);

        niLet quadVB =
          MakeQuadVB(_driverGpu, ++_numQuadVB, 1.0f, Vec3f(0, 0, 0.3f));
        niLet quadIB = MakeQuadIB(_driverGpu, ++_numQuadIB, 1);
        niCheck(prDesc->AddTrianglesIndexed(quadVB, 0, sizeof(tVertexTri), 4,
      ...
      _instanceAS = niCheckNN(
        _instanceAS,
        buildEncoder->BuildRayInstances(_H("RayInstances_Quad"), instDesc),
        eFalse);
    }
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Create acceleration structure
    {
...

      {
        ...
        {
          niLet triangleVB = MakeTriVB(_driverGpu, ++_numTriVB, 0.5f,
                                       Vec3f(-0.25f, 0.35f, 0.3f));
          niCheck(prDesc->AddTriangles(triangleVB, 0, sizeof(tVertexTri), 3,
                                       sMatrixf::Identity(),
                                       eRayPrimitiveFlags_Opaque, 0),
                  eFalse);
...

        {
          ...
          niCheck(prDesc->AddTrianglesIndexed(
                    triangleVB, 0, sizeof(tVertexTri), 3, triangleIB, 0,
                    eGpuIndexType_U32, 3, sMatrixf::Identity(),
                    eRayPrimitiveFlags_Opaque, 0),
                  eFalse);
        }
        ...
        {
          niLet quadVB =
            MakeQuadVB(_driverGpu, ++_numQuadVB, 0.5f, Vec3f(0.25f, 0, 0.3f));
          niLet quadIB = MakeQuadIB(_driverGpu, ++_numQuadIB, 1);
          niCheck(prDesc->AddTrianglesIndexed(quadVB, 0, sizeof(tVertexTri), 4,
                                              quadIB, 0, eGpuIndexType_U32, 6,
                                              sMatrixf::Identity(),
...
struct sFRayPipeline_InstancesBase : public sFRayPipeline_Base {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Create ray tracing shaders
    {
      ...
      _rayHitFun = niCheckNN(_rayHitFun,
                             _driverGpu->CreateGpuFunction(
                               eGpuFunctionType_RayClosestHit, _rchitPath),
                             eFalse);
    }
    ...
    // Create acceleration structure
    {
      _rayBuildEncoder = niCheckNN(_rayBuildEncoder,
                                   _driverRay->CreateRayBuildEncoder(), eFalse);
      ...
      niLet triangleVB =
        MakeTriVB(_driverGpu, ++_numTriVB, 0.5f, sVec3f::Zero());
      niCheck(prDesc->AddTriangles(triangleVB, 0, sizeof(tVertexTri), 3,
                                   sMatrixf::Identity(),
                                   eRayPrimitiveFlags_Opaque, 0),
              eFalse);
      niLet primitiveAS =
        niCheckNN(primitiveAS,
                  _rayBuildEncoder->BuildRayTrianglePrimitives(
                    _H("RayTrianglePrimitives_Triangle"), prDesc),
                  eFalse);
      ...
      niLoop (i, _instances.size()) {
        ...
        niCheck(_instancesDesc->AddInstance(primitiveAS,
                                            mtx,    // Transform
  ...
  niFn(tBool) OnPaint(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    niLet instanceAS = niCheckNN(
      instanceAS, _rayBuildEncoder->BuildRayInstances(nullptr, _instancesDesc),
      eFalse);
...
struct sFRayPipeline_InstancesIndex : public sFRayPipeline_InstancesBase {
  sFRayPipeline_InstancesIndex()
      : sFRayPipeline_InstancesBase(
          _H("test/nish/raypipeline/triangle_instanceindex_rchit.gpufunc.xml"))
  {
  }
};
...
struct sFRayPipeline_InstancesId : public sFRayPipeline_InstancesBase {
  sFRayPipeline_InstancesId()
      : sFRayPipeline_InstancesBase(
          _H("test/nish/raypipeline/triangle_instanceid_rchit.gpufunc.xml"))
  {
  }
};
...
struct sFRayPipeline_InstancesBary : public sFRayPipeline_InstancesBase {
  sFRayPipeline_InstancesBary()
      : sFRayPipeline_InstancesBase(
          _H("test/nish/raypipeline/triangle_bary_rchit.gpufunc.xml"))
  {
  }
};
...
struct sFRayPipeline_IntSphere : public sFRayPipeline_Base {
  ...
  NN<iGpuFunction> _rayMissFun = niDeferredInit(NN<iGpuFunction>);
  NN<iGpuFunction> _rayHitFun = niDeferredInit(NN<iGpuFunction>);
  NN<iGpuFunction> _rayIntFun =
    niDeferredInit(NN<iGpuFunction>); // Intersection shader
  NN<iRayFunctionTable> _rayFuncTable = niDeferredInit(NN<iRayFunctionTable>);
  NN<iRayPipeline> _rayPipeline = niDeferredInit(NN<iRayPipeline>);
  NN<iTexture> _rayOutputImage = niDeferredInit(NN<iTexture>);
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Create ray tracing shaders
    {
      ...
      _rayIntFun =
        niCheckNN(_rayIntFun,
                  _driverGpu->CreateGpuFunction(
                    eGpuFunctionType_RayIntersection,
                    _H("test/nish/raypipeline/sphere_rint.gpufunc.xml")),
                  eFalse);
    }
    ...
    // Create acceleration structure
    {
      ...
      {
        niLet prDesc = niCheckNN(prDesc,
                                 _driverRay->CreateRayProceduralPrimitivesDesc(
                                   _H("RayTrianglePrimitivesDesc_Sphere")),
                                 eFalse);

        niLet aabb =
          cAABBf(Vec3f(-0.5f, -0.5f, -0.5f), Vec3f(0.5f, 0.5f, 0.5f));

        niLet aabbBuffer = niCheckNN(
          aabbBuffer,
          _driverGpu->CreateGpuBufferFromDataRaw(
            _H("SphereAABB"), (tPtr)&aabb, sizeof(cAABBf),
            eGpuBufferMemoryMode_Shared, eGpuBufferUsageFlags_RayBuildInput),
          eFalse);

        niCheck(prDesc->AddAABBs(aabbBuffer, 0, sizeof(cAABBf), 1,
                                 sMatrixf::Identity(),
                                 eRayPrimitiveFlags_Opaque, 0),
                eFalse);
        ...
        niLet primitiveAS =
          niCheckNN(primitiveAS,
                    buildEncoder->BuildRayProceduralPrimitives(
                      _H("RayTrianglePrimitives_Sphere"), prDesc),
                    eFalse);
      ...
      _instanceAS = niCheckNN(
        _instanceAS,
        buildEncoder->BuildRayInstances(_H("RayInstances_Sphere"), instDesc),
        eFalse);
    }
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    // Create acceleration structure
    {
      ...
      // Create a procedural AABB for the sphere
      {
        niLet prDesc = niCheckNN(prDesc,
                                 _driverRay->CreateRayProceduralPrimitivesDesc(
                                   _H("RayProceduralPrimitivesDesc_Sphere")),
                                 eFalse);
        ...
        niCheck(prDesc->AddAABBs(aabbBuffer, 0, sizeof(cAABBf), 1,
                                 sMatrixf::Identity(),
                                 eRayPrimitiveFlags_Opaque, _hitGroupSpheres),
                eFalse);
