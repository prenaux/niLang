...
class DC {
 public:
  ...
  HFONT CreateAndSelectFont(const TCHAR* pszFace, UINT iSize,
                            INT iWeight = FW_NORMAL)
  {
    HFONT hFont = ::CreateFontA(-(int)iSize, 0, 0, 0, iWeight, 0, 0, 0, 0, 0, 0,
  ...
  tBool StretchBlitToDC(int bmp_w, int bmp_h, int bmp_bpp, BYTE* bmp_pixels,
                        int src_x, int src_y, int src_w, int src_h, int dest_x,
                        int dest_y, int dest_w, int dest_h)
  {
    if (!bmp_pixels)
      return eFalse;
    ...
    if (StretchDIBits(mhDC, dest_x, dest_y, dest_w, dest_h, src_x,
                      bmp_h - src_y - src_h, src_w, src_h, bmp_pixels, &bi,
  ...
  tBool BlitToDC(int bmp_w, int bmp_h, int bmp_bpp, BYTE* bmp_pixels, int src_x,
                 int src_y, int dest_x, int dest_y, int w, int h)
  {
    return StretchBlitToDC(bmp_w, bmp_h, bmp_bpp, bmp_pixels, src_x, src_y, w,
                           h, dest_x, dest_y, w, h);
  }
