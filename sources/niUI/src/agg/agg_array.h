...
void pod_vector<T>::deserialize(const int8u* data, unsigned byte_size)
{
  ...
  allocate(byte_size);
  if (byte_size)
    memcpy(m_array, data, byte_size * sizeof(T));
}
...
//---------------------------------------------------------------pod_bvector
// A simple class template to store Plain Old Data, similar to astl::deque
...
//------------------------------------------------------------------------
template <class T, unsigned S>
const pod_bvector<T, S>& pod_bvector<T, S>::operator=(
...
void pod_bvector<T, S>::allocate_block(unsigned nb)
{
  if (nb >= m_max_blocks) {
    T** new_blocks =
      pod_allocator<T*>::allocate(m_max_blocks + m_block_ptr_inc);
