...
static tBool _LogMessageCallback(tLogFlags type, const tF64 afTime,
                                 ...
                                 const char* msg)
{
  ...
  if (mt.IsOK()) {
    ...
    cString final;
    ni_log_format_message(final, type, file, line, func, msg, afTime,
                          _fPrevTime);
    _fPrevTime = afTime;
...
///////////////////////////////////////////////
static sPropertyBool _logSystemInfo("log.system_info",
#ifdef niEmbedded
                                    eTrue
#else
                                     eFalse
  ...
  niRunOnce
  {
...
#if niMinFeatures(15)
    {
      ...
      if (pProcMan) {
        ...
        str << "Exe Path: " << pProcMan->GetCurrentProcess()->GetExePath()
            << AEOL;
        str << "Command Line: "
            << pProcMan->GetCurrentProcess()->GetCommandLine() << AEOL;
      }
    }
#endif
...
void cLang::_Construct()
{
...
#ifdef niJNI
  {
    ...
    envPATH = ni::GetLang()->GetProperty("ni.dirs.bin") + envPATHSep + envPATH;
...
void __stdcall cLang::Invalidate()
{
  if (mptrSystemMessageHandlers.IsOK()) {
    ni::SendMessages(mptrSystemMessageHandlers, eSystemMessage_Exit, niVarNull,
                     niVarNull);
  }
  if (mptrSystemMessageHandlers.IsOK()) {
...
tBool __stdcall cLang::JsonParseString(const cString& aString,
                                       iJsonParserSink* apSink)
{
  return ni::JsonParseString(aString, apSink);
}
iJsonWriter* __stdcall cLang::CreateJsonSinkWriter(iJsonWriterSink* apSink,
                                                   tBool abPrettyPrint)
{
  return ni::CreateJsonSinkWriter(apSink, abPrettyPrint);
}
iJsonWriter* __stdcall cLang::CreateJsonFileWriter(iFile* apFile,
                                                   tBool abPrettyPrint)
{
...
///////////////////////////////////////////////
#if niMinFeatures(20)
ni::tBool __stdcall cLang::XmlParseFile(ni::iFile* apFile,
                                        ni::iXmlParserSink* apSink)
{
  return ni::XmlParseFile(apFile, apSink);
}
ni::tBool __stdcall cLang::XmlParseString(const ni::cString& aString,
                                          ni::iXmlParserSink* apSink)
{
...
tI32 __stdcall cLang::StartPath(const achar* aaszURL)
{
...
#elif defined niWinDesktop
  {
    const ni::tBool bIsURL =
      strURL.StartsWithI("http://") || strURL.StartsWithI("https://") ||
      strURL.StartsWithI("ftps://") || strURL.StartsWithI("ftp://");
    if (!bIsURL) {
    ...
    tIntPtr ret = (tIntPtr)ShellExecute(
      NULL, "open", path.GetPath().Chars(), NULL,
...
///////////////////////////////////////////////
iDataTableWriteStack* __stdcall cLang::CreateDataTableWriteStack(
  iDataTable* apDT)
{
...
///////////////////////////////////////////////
iDataTableWriteStack* __stdcall cLang::CreateDataTableWriteStackFromName(
