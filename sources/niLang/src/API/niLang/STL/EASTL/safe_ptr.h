...
class safe_object {
 public:
  bool is_unreferenced()
    const; /// Returns true if there are zero references (by a smart_ptr) to this object (mpSafePtrList is NULL).
  bool has_unique_reference()
    const; /// Returns true if there is at most one reference (by a smart_ptr) to us.
 ...
 protected:
  ...
  void
  clear_references(); /// Forcibly removes any references (by smart_ptrs) to this object. All safe_ptr mpObject values are set to NULL.

 private:
  friend class safe_ptr_base;

  void add(
    safe_ptr_base* pBase) const; /// Link pBase into my list of safe pointers.
  void remove(
    safe_ptr_base* pBase) const; /// Unlink pBase from my list of safe pointers.
...
class safe_ptr_base {
 public:
  bool unique()
    const; /// Returns true if there are no other smart pointers pointing to our object except us. True if mpObject is NUll
  bool empty() const; /// Returns true if mpObject is NULL.
  void reset(
    const safe_object* pObject); /// Make this point to pObject and enlist.
...
class safe_ptr : public safe_ptr_base {
 ...
 public:
  ...
  bool operator==(const this_type& safePtr)
    const; /// Returns true if safePtr points to the same object as this.
 ...
 public:
  ...
  typedef T* (this_type::*bool_)()
    const; /// Allows for a more portable version of testing an instance of this class as a bool.
  operator bool_()
    const // A bug in the CodeWarrior compiler forces us to implement this inline instead of below.
  {
    if (mpObject)
...
inline void eastl::safe_object::remove(safe_ptr_base* pBase) const
{
  ...
  if (pBase == mpSafePtrList)
    ...
    for (safe_ptr_base *pPrev = mpSafePtrList,
                       *pCurrent = mpSafePtrList->mpNext;
         pCurrent; pPrev = pCurrent, pCurrent = pCurrent->mpNext)
    {
      if (pCurrent == pBase) {
...
{
  return (mpNext == NULL) &&
         ((mpObject == NULL) || (mpObject->mpSafePtrList == this));
}
...
template <class T>
inline typename eastl::safe_ptr<T>::this_type& eastl::safe_ptr<T>::operator=(
  const this_type& safePtr)
{
  if (this != &safePtr)
...
template <class T>
inline bool operator<(const eastl::safe_ptr<T>& safePtrA,
                      const eastl::safe_ptr<T>& safePtrB)
{
