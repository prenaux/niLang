...
typedef sTimedLerp<tF32CVec, tF32, sTimedLerp_GetKeyTimeStaticCast<tF32, tF32>>
  tTimedLerp;
...
class gradient_reflect_polymorphic_wrapper
    : public ImplRC<iAGGGradientPolymorphicWrapperBase> {
 public:
  gradient_reflect_polymorphic_wrapper()
...
template <class GradientF>
class gradient_repeat_polymorphic_wrapper
    : public ImplRC<iAGGGradientPolymorphicWrapperBase> {
 public:
  gradient_repeat_polymorphic_wrapper()
      : m_adaptor(m_gradient)
  {
  }
  virtual int calculate(int x, int y, int d) const
...
template <class GradientF>
class gradient_clamp_polymorphic_wrapper
    : public ImplRC<iAGGGradientPolymorphicWrapperBase> {
 public:
  gradient_clamp_polymorphic_wrapper()
      : m_adaptor(m_gradient)
  {
  }
  virtual int calculate(int x, int y, int d) const
...
iAGGGradientPolymorphicWrapperBase* GetNewGradientWrapperBase(
  eVGGradientType aType, eVGWrapType aWrap)
{
  #define SELWRAP(TYPE)                                          \
  ...
  switch (aType) {
  default:
  case eVGGradientType_Linear: SELWRAP(agg::gradient_x); break;
  case eVGGradientType_Radial: SELWRAP(agg::gradient_radial); break;
  case eVGGradientType_Cross: SELWRAP(agg::gradient_xy); break;
  case eVGGradientType_SqrtCross: SELWRAP(agg::gradient_sqrt_xy); break;
  case eVGGradientType_Conic: SELWRAP(agg::gradient_conic); break;
  case eVGGradientType_Diamond: SELWRAP(agg::gradient_diamond); break;
  }
...
class cVGGradientTable : public ImplRC<iVGGradientTable> {
 public:
  ...
  tBool __stdcall SetColorRange(tU32 anStart, tU32 anEnd,
                                const sColor4f& avColor)
  {
    if (anStart >= mnTableSize)
      return eFalse;
    if (anEnd >= mnTableSize)
      anEnd = mnTableSize - 1;
  ...
  void __stdcall GenerateTwoColors(const sColor4f& aStartColor,
                                   const sColor4f& aEndColor)
  {
    sColor4f colorT;
    for (tU32 i = 0; i < mnTableSize; ++i) {
      tF32 f = tF32(i) / tF32(mnTableSize);
      mTable[i] =
        AGGColorFromColor4f(ni::VecLerp(colorT, aStartColor, aEndColor, f));
    }
  }

  tBool __stdcall GenerateStops(const tF32CVec* apOffsets,
                                const tVec4fCVec* apColors)
  {
    ...
    if (apOffsets->GetSize() == 1) {
      ...
      for (tU32 i = 0; i < mnTableSize; ++i) {
        ...
        mTable[i] = AGGColorFromColor4f(
          ni::VecLerp(colorT, (*apColors)[a], (*apColors)[b], t));
      }
    }
    return ni::eTrue;
  }

  iVGImage* __stdcall CreateImage(eVGGradientType aType, eVGWrapType aWrapType,
                                  const iVGTransform* apTransform, tU32 anWidth,
                                  tU32 anHeight, tI32 anD1, tI32 anD2)
  {
    Ptr<iAGGGradientPolymorphicWrapperBase> ptrGrad =
      GetNewGradientWrapperBase(aType, aWrapType);
    if (!niIsOK(ptrGrad)) {
    ...
    Ptr<iBitmap2D> ptrBmp =
      ptrImage->GrabBitmap(eVGImageUsage_Target, sRecti::Null());
    niAssert(ptrBmp.IsOK());

    agg::rendering_buffer buf(ptrBmp->GetData(), ptrBmp->GetWidth(),
                              ptrBmp->GetHeight(), ptrBmp->GetPitch(),
    ...
    tAGGGradientSpanGen span_gen(inter, *ptrGrad, t, agg_real(anD1),
                                 agg_real(anD2));
  ...
  tAGGColorType operator[](unsigned v) const
  {
    return mTable[v];
  }
...
{
  return sAGGGradientTable(
    niStaticCast(const cVGGradientTable*, apTable)->mnTableSize,
...
class cVGPaintSolid : public ImplRC<iVGPaint> {
 public:
  ...
  {
    if (apPaint->GetType() != eVGPaintType_Solid)
      return eFalse;
...
//----------------------------------------------------------------------------
class cVGPaintImage
    : public ImplRC<iVGPaintImage, eImplFlags_DontInherit1, iVGPaint> {
 public:
  cVGPaintImage(iGraphics* apGraphics, iVGImage* apImage = NULL,
                const sColor4f& avColor = sColor4f::White())
  {
    mptrGraphics = apGraphics;
    mColor = avColor;
    mptrImage =
      apImage ? Ptr<iVGImage>(apImage) : apGraphics->CreateVGImage(4, 4);
    mRect =
      Rectf(0, 0, (tF32)mptrImage->GetWidth(), (tF32)mptrImage->GetHeight());
  ...
  tBool __stdcall Copy(const iVGPaint* apPaint)
  {
    if (apPaint->GetType() != eVGPaintType_Image)
      return eFalse;
...
//
//----------------------------------------------------------------------------
class cVGPaintGradient
    : public ImplRC<iVGPaintGradient, eImplFlags_DontInherit1, iVGPaint> {
 public:
  cVGPaintGradient(iGraphics* apGraphics,
                   const sColor4f& avColor = sColor4f::White(),
                   iVGGradientTable* apTable = NULL)
  {
    mptrGraphics = apGraphics;
    mColor = avColor;
  ...
  tBool __stdcall Copy(const iVGPaint* apPaint)
  {
    if (apPaint->GetType() != eVGPaintType_Gradient)
      return eFalse;
    const cVGPaintGradient* pSrc =
      niStaticCast(const cVGPaintGradient*, apPaint);
...
///////////////////////////////////////////////
Ptr<iVGGradientTable> __stdcall cGraphics::CreateVGGradientTableTwoColors(
...
///////////////////////////////////////////////
Ptr<iVGGradientTable> __stdcall cGraphics::CreateVGGradientTableStops(
...
{
  switch (aType) {
  case eVGPaintType_Solid: return niNew cVGPaintSolid();
  case eVGPaintType_Gradient:
    return niNew cVGPaintGradient(QPtr<iGraphics>(this));
  case eVGPaintType_Image: return niNew cVGPaintImage(QPtr<iGraphics>(this));
  }
  return NULL;
}
...
///////////////////////////////////////////////
Ptr<iVGPaintImage> __stdcall cGraphics::CreateVGPaintImage(
  iVGImage* apImage, eVGImageFilter aFilter, eVGWrapType aWrap)
{
  Ptr<iVGPaintImage> ptrPaint =
    niNew cVGPaintImage(QPtr<iGraphics>(this), apImage);
...
///////////////////////////////////////////////
Ptr<iVGPaintGradient> __stdcall cGraphics::CreateVGPaintGradient(
  eVGGradientType aType, iVGGradientTable* apGradientTable)
{
  Ptr<iVGPaintGradient> ptrPaint = niNew cVGPaintGradient(
    QPtr<iGraphics>(this), sColor4f::White(), apGradientTable);
  ptrPaint->SetGradientType(aType);
  return ptrPaint.GetRawAndSetNull();
}

///////////////////////////////////////////////
Ptr<iVGPaintGradient> __stdcall cGraphics::CreateVGPaintGradientTwoColors(
  eVGGradientType aType, const sColor4f& acolStart, const sColor4f& acolEnd)
{
  Ptr<iVGPaintGradient> ptrPaint =
    niNew cVGPaintGradient(QPtr<iGraphics>(this));
  ptrPaint->SetGradientType(aType);
...
///////////////////////////////////////////////
Ptr<iVGPaintGradient> __stdcall cGraphics::CreateVGPaintGradientStops(
  eVGGradientType aType, const tF32CVec* apOffsets, const tVec4fCVec* apColors)
{
  Ptr<iVGPaintGradient> ptrPaint =
    niNew cVGPaintGradient(QPtr<iGraphics>(this));
  ptrPaint->SetGradientType(aType);
