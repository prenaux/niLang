...
tBool __stdcall cVGPath::Translate(const sVec2f& avTranslate)
{
  ...
  agg::trans_affine trans =
    agg::trans_affine_translation(avTranslate.x, avTranslate.y);
...
{
  if (!niIsOK(apTransform))
    return eFalse;
  agg::path_storage path = m_storage;
  agg::conv_transform<agg::path_storage> transformed(
    path, AGGGetTransform(apTransform));
  m_storage.remove_all();
  m_storage.add_path(transformed);
  return eTrue;
}
...
tBool __stdcall cVGPath::TransformAlongOnePath(const iVGPath* apPath,
                                               ...
                                               tF32 afCurvesApproxScale)
{
  ...
  typedef agg::conv_transform<conv_segm_type, agg::trans_single_path>
    conv_trans_type;
...
tBool __stdcall cVGPath::TransformAlongTwoPaths(
  ...
  tF32 afCurvesApproxScale)
{
  ...
  typedef agg::conv_curve<agg::path_storage> conv_curve_type;
  typedef agg::conv_segmentator<conv_curve_type> conv_segm_type;
  typedef agg::conv_transform<conv_segm_type, agg::trans_double_path>
    conv_trans_type;
...
{
  m_storage.arrange_orientations_all_paths(abCW ? (agg::path_flags_cw)
                                                : (agg::path_flags_ccw));
}
...
sRectf __stdcall cVGPath::GetBoundingRect(const iVGTransform* apTransform) const
{
  ...
  if (apTransform)
    transform = AGGGetTransform(apTransform);
  agg::conv_transform<agg::path_storage> trans((agg::path_storage&)m_storage,
                                               transform);
...
struct sRenderTesselated {
  ...
  {
    tesselatePath.tesselate_path(
      vs, abStroke ? eTrue : (apStyle->GetFillEvenOdd() ? eTrue : eFalse), 0);
    pPathRenderer->AddPathPolygons(tesselatePath.m_tesselator, apStyle,
                                   abStroke);
  }
};

tBool __stdcall cVGPath::RenderTesselated(iVGPathTesselatedRenderer* apPath,
                                          ...
                                          const iVGStyle* apStyle) const
{
  ...
  niConstCast(cVGPath*, this)
    ->RenderPath(render, apTransform, apStyle, sVec2f::Zero());
  return eTrue;
}
  ...
  while (tok.Next()) {
    ...
    case 'a':
      ...
      if (tok.HasError())
        break;
      if (cmd == 'a') {
        ArcToRel(arg[0], arg[1], niRadf(arg[2]), arg[3] == 0 ? false : true,
                 arg[4] == 0 ? false : true, arg[5], arg[6]);
      }
      else {
        ArcTo(arg[0], arg[1], niRadf(arg[2]), arg[3] == 0 ? false : true,
              arg[4] == 0 ? false : true, arg[5], arg[6]);
      }
      break;
