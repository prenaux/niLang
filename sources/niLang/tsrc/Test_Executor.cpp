...
TEST_FIXTURE(FExecutor, FuncPtr)
{
  struct _Local {
    ...
    static Var run()
    {
      ...
      niDebugFmt(("ThreadPool, Runned FuncPtr [%d] on thread '0x%x'.",
                  counter().Get(), ni::ThreadGetCurrentThreadID()));
      return 0;
    };
  };
  ...
  Ptr<iExecutor> exec =
    ni::GetConcurrent()->CreateExecutorThreadPool(_knFExecutorThreadCount);
  niLoop (i, kNumRun) {
  ...
  CHECK_EQUAL(1, counter->GetNumRefs());
  {
    ...
    auto runMe = [=]() -> int {
      ...
      niDebugFmt(("ThreadPool, Runned Lambda [%d] (%d) on thread '0x%x'.",
                  counter->Get(), counter->GetNumRefs(),
...
TEST_FIXTURE(FExecutor, TaskStealing)
{
  ...
  // should go in queue 0
    ...
    niSubmit_ (executor, &taskThread1, &taskSeq1, &taskSeqCounter) {
  ...
  // should go in queue 1
    ...
    niSubmit_ (executor, done, &taskThread2, &taskSeq2, &taskSeqCounter) {
  ...
  // should go in queue 0
    ...
    niSubmit_ (executor, done, &taskThread3, &taskSeq3, &taskSeqCounter) {
  ...
  // should go in queue 1
    ...
    niSubmit_ (executor, done, &taskThread4, &taskSeq4, &taskSeqCounter) {
  ...
  // should go in queue 0
    ...
    niSubmit_ (executor, done, &taskThread5, &taskSeq5, &taskSeqCounter) {
