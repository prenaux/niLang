...
template<class VertexConsumer> class math_stroke
{
 public:
  ...
  void miter_limit(agg_real ml) { m_miter_limit = ml; }
  void miter_limit_theta(agg_real t);
  void inner_miter_limit(agg_real ml) { m_inner_miter_limit = ml; }
  void approximation_scale(agg_real as) { m_approx_scale = as; }

  agg_real width() const { return m_width * 2.0; }
  agg_real miter_limit() const { return m_miter_limit; }
  agg_real inner_miter_limit() const { return m_inner_miter_limit; }
  agg_real approximation_scale() const { return m_approx_scale; }
 ...
 private:
  void calc_arc(VertexConsumer& out_vertices,
                ...
                agg_real dx1, agg_real dy1,
                agg_real dx2, agg_real dy2);
...
//-----------------------------------------------------------------------
template<class VC> void math_stroke<VC>::width(agg_real w)
{
  m_width = w * 0.5;
  if(m_width < 0)
  {
...
//-----------------------------------------------------------------------
template<class VC> void math_stroke<VC>::miter_limit_theta(agg_real t)
{
                               ...
                               agg_real len)
{
  ...
  {
    agg_real da = acos(m_width_abs / (m_width_abs + 0.125 / m_approx_scale)) * 2;
    agg_real a1;
    int i;
    int n = int(pi / da);
