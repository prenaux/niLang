...
///
template <typename T, size_t bucketCount, typename Hash = eastl::hash<T>,
          typename Equal = eastl::equal_to<T>>
class intrusive_hash_set
    : public intrusive_hashtable<T, T, Hash, Equal, bucketCount, true, true> {
 public:
  typedef intrusive_hashtable<T, T, Hash, Equal, bucketCount, true, true>
    base_type;
...
class intrusive_hash_multiset
    : public intrusive_hashtable<T, T, Hash, Equal, bucketCount, true, false> {
 public:
  typedef intrusive_hashtable<T, T, Hash, Equal, bucketCount, true, false>
    base_type;
  typedef intrusive_hash_multiset<T, bucketCount, Hash, Equal> this_type;

 public:
  explicit intrusive_hash_multiset(const Hash& h = Hash(),
