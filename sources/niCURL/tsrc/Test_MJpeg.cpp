...
tBool __stdcall ReadMjpegPart(const char* aTestName, iGraphics* apGraphics,
                              iFile* apJpeg, tU32 anPartId)
{
  static cString _basePath =
    UnitTest::GetTestOutputFilePath(aTestName, "test_curl_mjpeg");
  cString path;
  tBool bRet = eFalse;
  ...
  Ptr<iFile> fpDump =
    ni::GetRootFS()->FileOpen(path.Chars(), eFileOpenMode_Write);
  if (fpDump.IsOK()) {
    apJpeg->SeekSet(0);
...
TEST_FIXTURE(FCURLMJpeg, IPCam)
{
  ...
  Ptr<iRunnable> runnable = _curl->URLGetMultiPart(
    ni::MessageHandler([=, this, &futureValue, &numPartReceived,
                        &numJpegDecoded](tU32 anMsg, const Var& A,
                                         const Var& B) {
      futureValue = B;
      switch (anMsg) {
      ...
      case eCURLMessage_ReceivedPart: {
        ...
        ++numPartReceived;
        if (ReadMjpegPart(m_testName, _graphics, VarQueryInterface<iFile>(A),
                          numJpegDecoded))
        {
          ++numJpegDecoded;
        }
