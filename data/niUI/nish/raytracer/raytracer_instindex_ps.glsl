...
nish_std_PixelOutput niUIGpuFuncs_raytracer_instindex_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
nish_std_RayDesc niUIGpuFuncs_MakeRayDesc(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms) {
  ...
  mat4 _tmp_z3 = aUniforms.cameraInvView;
  vec3 origin = vec3(_tmp_z3[3][0],_tmp_z3[3][1],_tmp_z3[3][2]);
  vec3 target = nish_std_Vec3TransformCoord(ndc,aUniforms.cameraInvViewProj);
  vec3 dir = normalize(((target-(origin.xyz)).xyz));
  vec3 _tmp_K3 = origin;
  vec3 _tmp_L3 = dir;
  float _tmp_M3 = 0.001;
  float _tmp_N3 = aUniforms.cameraFarClipPlane;
  return nish_std_RayDesc_new(_tmp_K3, _tmp_L3, _tmp_M3, _tmp_N3);
}
niUIGpuFuncs_IntersectionResult niUIGpuFuncs_TraceRay_6_uint_Vec3_Vec3_float_float_RayInstances(uint aRayFlags, vec3 aOrigin, vec3 aDir, float aMinT, float aMaxT, accelerationStructureEXT aAS) {
  /* mut */ rayQueryEXT rayQuery/*__noinit__*/;
  rayQueryInitializeEXT(rayQuery,aAS,aRayFlags,255,aOrigin,aMinT,aDir,aMaxT);
  while(true) {
    bool _tmp_X3 = rayQueryProceedEXT(rayQuery);
    if (!(_tmp_X3)) {
      break;
    }
    uint candidateType = rayQueryGetIntersectionTypeEXT(rayQuery,false);
    bool _tmp_24 = (candidateType == nish_std_RayQueryCandidateIntersectionType_CandidateTriangle);
    if (_tmp_24) {
  ...
  bool _tmp_a4 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_a4) {
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_instindex_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  vec4 color;
  bool _tmp_z4 = niUIGpuFuncs__op__gte_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_Triangle);
  if (_tmp_z4) {
    vec3 _tmp_E4 = nish_std_UIntToHashColor(intersection.userInstIndex);
    color = vec4(_tmp_E4.x,_tmp_E4.y,_tmp_E4.z,1.0);
  }
  else {
    bool _tmp_H4 = niUIGpuFuncs__op__lt_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_None);
    if (_tmp_H4) {
  ...
  vec4 _tmp_V4 = color;
  return nish_std_PixelOutput_new(_tmp_V4);
}
...
// Pixel Shader main: niUIGpuFuncs_raytracer_instindex_ps
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = niUIGpuFuncs_raytracer_instindex_ps(aInput, aUniforms, IN_1_aAS);
