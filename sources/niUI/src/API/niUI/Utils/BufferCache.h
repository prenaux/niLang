...
struct BufferCache {
  ...
  inline BufferCache(const tU32 anReserve)
      : mnReserve(anReserve)
  {
  ...
  inline void Add(const T* apEls, tU32 anNumEls)
  {
    if (mnCurrentEl + anNumEls >= mvEls.size()) {
      mvEls.resize(
        ni::AlignOffset<tU32>(mnReserve, (tU32)mvEls.size() + anNumEls));
    }
...
struct BufferCacheVertex
    : public BufferCache<TVERTEX, BufferCacheVertex<TVERTEX>>,
      public ImplRC<iUnknown> {
  ...
  {
    mptrVA = mptrGraphics->CreateVertexArray(anNumEls, tVertex::eFVF,
                                             eArrayUsage_Dynamic);
...
struct BufferCacheIndex : public BufferCache<tU32, BufferCacheIndex>,
                          public ImplRC<iUnknown> {
  ...
  inline tBool _Init(const tU32 anNumEls)
  {
    mptrIA = mptrGraphics->CreateIndexArray(eGraphicsPrimitiveType_TriangleList,
                                            anNumEls, ~0, eArrayUsage_Dynamic);
    TRACE_BUFFER_CACHE(
      ("... BufferCacheIndex[%p]::Init: %d indices.", (tIntPtr)this, anNumEls));
    return mptrIA.IsOK();
  }
