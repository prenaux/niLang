...
struct cWidgetLabel
    : public ImplRC<iWidgetSink, eImplFlags_Default, iWidgetLabel> {
                               ...
                               const Var& aB)
  {
    ...
    case eUIMessage_Layout: {
      if (niFlagIs(aA.mU32, eWidgetAutoLayoutFlags_Size)) {
        ...
        rect = apWidget->GetFont()->ComputeTextSize(
          rect, niHStr(apWidget->GetLocalizedText()), mFontFormatFlags);
        const sVec2f newSize = {
          (canComputeW
             ? rect.GetWidth() + (apWidget->GetFont()->GetCharWidth(' ') / 2.0f)
             : apWidget->GetSize().x),
          (canComputeH ? rect.GetHeight() : apWidget->GetSize().y)
        };
    ...
    case eUIMessage_Paint: {
      ...
      if (c) {
        ...
        c->BlitText(font, rect, mFontFormatFlags,
    ...
    case eUIMessage_SerializeWidget: {
      ...
      if (ptrDT.IsOK()) {
        ...
        if (nFlags & eWidgetSerializeFlags_Write) {
          ptrDT->SetEnum(_A("font_format_flags"), niFlagsExpr(eFontFormatFlags),
                         mFontFormatFlags);
          if (nFlags & eSerializeFlags_TypeInfoMetadata) {
            ptrDT->SetMetadata(_A("font_format_flags"),
                               _H("flags[*eFontFormatFlags]"));
          }
        }
        else if (nFlags & eWidgetSerializeFlags_Read) {
          mFontFormatFlags = ptrDT->GetEnumDefault(
            "font_format_flags", niFlagsExpr(eFontFormatFlags),
            mFontFormatFlags);
