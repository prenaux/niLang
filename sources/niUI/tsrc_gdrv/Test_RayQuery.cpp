...
struct sFRayQuery_Triangle : public sFRayQuery_Base {
  ...
  niFn(tBool) OnInit(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    {
      _triangleRayQueryFun =
        niCheckNN(_triangleRayQueryFun,
                  _driverGpu->CreateGpuFunction(
                    eGpuFunctionType_Pixel,
                    _H("test/nish/rayquery/triangle_rayquery_ps.gpufunc.xml")),
                  eFalse);
    ...
    // Create acceleration structure
    {
        ...
        niLet prDesc =
          niCheckNN(prDesc,
                    _driverRay->CreateRayTrianglePrimitivesDesc(
                      HFmt("%s_RayTrianglePrimitivesDesc", m_testName)),
                    eFalse);
      ...
      _instanceAS = niCheckNN(_instanceAS,
    ...
    // Recreate the display pipeline with our shader
    {
      ...
      _displayPipeline =
        niCheckNN(_displayPipeline,
                  _driverGpu->CreateGpuPipeline(
                    HFmt("%s_DisplayPipeline", m_testName), pipelineDesc),
                  eFalse);
    }
  ...
  niFn(tBool) OnPaint(UnitTest::TestResults& testResults_) niOverride
  {
    ...
    sMatrixf camView =
      MatrixLookAtLH(Vec3f(0, 0, -1.0f), Vec3f(0, 0, 0), Vec3f(0, 1, 0));
    ...
    // Create ray tracing shaders
    {
      _triangleRayQueryFun =
        niCheckNN(_triangleRayQueryFun,
                  _driverGpu->CreateGpuFunction(
                    eGpuFunctionType_Pixel,
                    _H("test/nish/rayquery/sphere_rayquery_ps.gpufunc.xml")),
                  eFalse);
      CHECK_EQUAL(eGpuFunctionType_Pixel,
                  _triangleRayQueryFun->GetFunctionType());
      CHECK_EQUAL(eGpuFunctionBindType_FixedRayInstances,
      ...
      // Create a procedural AABB for the sphere
      {
        niLet prDesc =
          niCheckNN(prDesc,
                    _driverRay->CreateRayProceduralPrimitivesDesc(
                      HFmt("%s_RayProceduralPrimitivesDesc", m_testName)),
                    eFalse);
        ...
        niLet aabbBuffer = niCheckNN(
          ...
          _driverGpu->CreateGpuBufferFromDataRaw(
            HFmt("%s_SphereAABB", m_testName), (tPtr)&aabb, sizeof(cAABBf),
        ...
        niLet primitiveAS =
          niCheckNN(primitiveAS,
                    buildEncoder->BuildRayProceduralPrimitives(
                      HFmt("%s_RayProceduralPrimitives", m_testName), prDesc),
                    eFalse);
