...
/*
  ...
  Status square colors:
    ...
    Unknown = pink (1,0,1)
*/
struct cVideoDecoderIpcam
    : public ni::ImplRC<ni::iVideoDecoder, ni::eImplFlags_Default> {
  ...
  ///////////////////////////////////////////////
  cVideoDecoderIpcam(iGraphics* apGraphics, iHString* ahspName, iFile* apFile,
                     tVideoDecoderFlags aFlags)
      : mFlags(aFlags)
      ...
      , mbUpdateOnBind(eTrue)
  {
    ...
    if (!ni::GetLang()->SerializeDataTable("xml", ni::eSerializeMode_Read, dt,
                                           apFile))
    {
    ...
    mptrCURL->SetRequestTimeoutInSecs(dt->GetIntDefault("request_timeout", 0));

    mptrTargetBitmap =
      mptrGraphics->CreateBitmap2D(mnWidth, mnHeight, IPCAM_PIXEL_FORMAT);
    ...
    if (niFlagIs(mFlags, eVideoDecoderFlags_TargetTexture)) {
      Ptr<iTexture> ptrTex = mptrGraphics->CreateTexture(
        NULL, eBitmapType_2D, mptrTargetBitmap->GetPixelFormat()->GetFormat(),
        0, mnWidth, mnHeight, 0, eTextureFlags_Dynamic | eTextureFlags_Overlay);
      if (!ptrTex.IsOK()) {
      ...
      mpTargetTexture =
        niNew sVideoDecoderTexture(apGraphics, ahspName, this, ptrTex);
    }
    ...
    niLog(Info, niFmt("Initialized ipcam: %s, width: %d, height: %d.", mstrURL,
                      mnWidth, mnHeight));
  }
  ...
  virtual tBool __stdcall Update(tBool abUpdateTarget, tF32 afFrameTime)
  {
        ...
        niFlagIs(mFlags, eVideoDecoderFlags_TargetTexture))
    {
      ...
      mptrGraphics->BlitBitmapToTexture(mptrTargetBitmap,
                                        mpTargetTexture->mptrTexture, 0);
      ...
      if (mptrFuture.IsOK() && mptrFuture->GetIsDone()) {
        ...
        niExec (IO, _this) {
          ...
          if (videoDecoder.IsOK()) {
            ni::SleepMs(videoDecoder->mptrCURL->GetConnectionTimeoutInSecs() *
                        1000);
            niLog(Info, niFmt("Trying to reconnect to ipcam '%s'.",
          ...
          return eTrue;
               ...
               (ni::TimerInSeconds() - mfTargetLastUpdateTime) >
                 (tF64)(mptrCURL->GetConnectionTimeoutInSecs()))
      {
        // Timed out...
        _StopDecoding();
      }
  ...
  void _StartDecoding(eIpcamStatus aConnStatus)
  {
    ...
    Ptr<iRunnable> runnable = mptrCURL->URLGetMultiPart(
      ni::MessageHandler(
        [_this](tU32 anMsg, const Var& A, const Var& B) {
          QPtr<cVideoDecoderIpcam> videoDecoder = _this;
          if (!videoDecoder.IsOK())
            return;
          ...
          case eCURLMessage_ReceivedPart: {
            ...
            IPCAM_TRACE(
              ("... Received Part: %d.", videoDecoder->mnNumPartDecoded));
              ...
              if (videoDecoder->mnNumJpegDecoded == 1) {
                niLog(
                  ...
                  niFmt("First mjpeg frame decoded with size %dx%d from '%s'.",
                        bmpDecoded->GetWidth(), bmpDecoded->GetHeight(),
                        videoDecoder->mstrURL));
              }
          ...
          case eCURLMessage_Failed: {
            niLog(Info,
                  niFmt("Ipcam '%s' Failed: %s", videoDecoder->mstrURL, A));
            videoDecoder->_UpdateStatus(eIpcamStatus_Stopped);
            break;
          }
          case eCURLMessage_Completed: {
            niLog(Info,
                  niFmt("Ipcam '%s' Completed: %s", videoDecoder->mstrURL, A));
            videoDecoder->_UpdateStatus(eIpcamStatus_Stopped);
            break;
          }
          default: {
...
iVideoDecoder* CreateVideoDecoder_Ipcam(iGraphics* apGraphics,
                                        iHString* ahspName, iFile* apFile,
                                        tVideoDecoderFlags aFlags)
{
