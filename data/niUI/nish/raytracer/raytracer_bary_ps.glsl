...
nish_std_PixelOutput niUIGpuFuncs_raytracer_bary_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  if (_tmp_z4) {
    vec3 _tmp_E4 = intersection.bary;
    color = vec4(_tmp_E4.x,_tmp_E4.y,_tmp_E4.z,1.0);
  }
  else {
    bool _tmp_G4 = niUIGpuFuncs__op__lt_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_None);
    if (_tmp_G4) {
  ...
  vec4 _tmp_U4 = color;
  return nish_std_PixelOutput_new(_tmp_U4);
}
