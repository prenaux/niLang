...
class distance_interpolator00 {
 public:
  ...
  distance_interpolator00(int xc, int yc, int x1, int y1, int x2, int y2, int x,
                          int y)
...
class distance_interpolator1 {
 public:
  ...
  distance_interpolator1(int x1, int y1, int x2, int y2, int x, int y)
      ...
      , m_dy(y2 - y1)
      , m_dist(
          iround(agg_real(x + line_subpixel_scale / 2 - x2) * agg_real(m_dy) -
                 agg_real(y + line_subpixel_scale / 2 - y2) * agg_real(m_dx)))
  {
...
class distance_interpolator2 {
 public:
  ...
  distance_interpolator2(int x1, int y1, int x2, int y2, int sx, int sy, int x,
                         int y)
      : m_dx(x2 - x1)
      , m_dy(y2 - y1)
      ...
      m_dist_start(
        (line_mr(x + line_subpixel_scale / 2) - line_mr(sx)) * m_dy_start -
  ...
  distance_interpolator2(int x1, int y1, int x2, int y2, int ex, int ey, int x,
                         int y, int)
      : m_dx(x2 - x1)
      , m_dy(y2 - y1)
      ...
      m_dist_start(
        (line_mr(x + line_subpixel_scale / 2) - line_mr(ex)) * m_dy_start -
...
class distance_interpolator3 {
 public:
  ...
  distance_interpolator3(int x1, int y1, int x2, int y2, int sx, int sy, int ex,
                         int ey, int x, int y)
      : m_dx(x2 - x1)
      , m_dy(y2 - y1)
      , m_dx_start(line_mr(sx) - line_mr(x1))
      ...
      m_dist_end(
        (line_mr(x + line_subpixel_scale / 2) - line_mr(ex)) * m_dy_end -
...
class line_interpolator_aa_base {
 ...
 private:
  ...
  const line_interpolator_aa_base<Renderer>& operator=(
...
class line_interpolator_aa0 : public line_interpolator_aa_base<Renderer> {
 public:
  ...
  line_interpolator_aa0(renderer_type& ren, const line_parameters& lp)
      ...
      , m_di(lp.x1, lp.y1, lp.x2, lp.y2, lp.x1 & ~line_subpixel_mask,
  ...
  bool step_ver()
  {
    ...
    *p1++ = (cover_type)base_type::m_ren.cover(s1);
    ...
    base_type::m_ren.blend_solid_hspan(base_type::m_x - dx + 1, base_type::m_y,
                                       unsigned(p1 - p0), p0);
    return ++base_type::m_step < base_type::m_count;
  }
 ...
 private:
  ...
  const line_interpolator_aa0<Renderer>& operator=(
...
class line_interpolator_aa1 : public line_interpolator_aa_base<Renderer> {
 public:
  ...
  //---------------------------------------------------------------------
  line_interpolator_aa1(renderer_type& ren, const line_parameters& lp, int sx,
                        int sy)
      : line_interpolator_aa_base<Renderer>(ren, lp)
      , m_di(lp.x1, lp.y1, lp.x2, lp.y2, sx, sy, lp.x1 & ~line_subpixel_mask,
             lp.y1 & ~line_subpixel_mask)
  {
    int dist1_start;
    int dist2_start;
    ...
    if (lp.vertical) {
      do {
        ...
        base_type::m_x =
          (base_type::m_lp->x1 + base_type::m_li.y()) >> line_subpixel_shift;
      ...
      do {
        ...
        base_type::m_y =
          (base_type::m_lp->y1 + base_type::m_li.y()) >> line_subpixel_shift;
 ...
 private:
  ...
  const line_interpolator_aa1<Renderer>& operator=(
...
class line_interpolator_aa2 : public line_interpolator_aa_base<Renderer> {
 public:
  ...
  //---------------------------------------------------------------------
  line_interpolator_aa2(renderer_type& ren, const line_parameters& lp, int ex,
                        int ey)
      : line_interpolator_aa_base<Renderer>(ren, lp)
      , m_di(lp.x1, lp.y1, lp.x2, lp.y2, ex, ey, lp.x1 & ~line_subpixel_mask,
             lp.y1 & ~line_subpixel_mask, 0)
  {
    base_type::m_li.adjust_forward();
 ...
 private:
  ...
  const line_interpolator_aa2<Renderer>& operator=(
...
class line_interpolator_aa3 : public line_interpolator_aa_base<Renderer> {
 public:
  ...
  //---------------------------------------------------------------------
  line_interpolator_aa3(renderer_type& ren, const line_parameters& lp, int sx,
 ...
 private:
  ...
  const line_interpolator_aa3<Renderer>& operator=(
...
class line_profile_aa {
 public:
  ...
  line_profile_aa(agg_real w, const GammaF& gamma_function)
      : m_subpixel_width(0)
      , m_min_width(1.0)
      , m_smoother_width(1.0)
  {
  ...
  //---------------------------------------------------------------------
  void min_width(agg_real w)
  {
    m_min_width = w;
  }
  void smoother_width(agg_real w)
  {
    m_smoother_width = w;
  }
  ...
  void gamma(const GammaF& gamma_function)
  {
    ...
    for (i = 0; i < aa_scale; i++) {
      m_gamma[i] =
        value_type(uround(gamma_function(agg_real(i) / aa_mask) * aa_mask));
    }
  }
  ...
  //---------------------------------------------------------------------
  agg_real min_width() const
  {
    return m_min_width;
  }
  agg_real smoother_width() const
  {
 ...
 private:
  ...
  value_type* profile(agg_real w);
  void set(agg_real center_width, agg_real smoother_width);

  //---------------------------------------------------------------------
  pod_array<value_type> m_profile;
  value_type m_gamma[aa_scale];
  int m_subpixel_width;
  agg_real m_min_width;
  agg_real m_smoother_width;
};
...
class renderer_outline_aa {
 public:
  ...
  //-------------------------------------------------------------------------
  template <class Cmp>
  void semidot_hline(Cmp cmp, int xc1, int yc1, int xc2, int yc2, int x1,
                     int y1, int x2)
  {
    cover_type
      covers[line_interpolator_aa_base<self_type>::max_half_width * 2 + 4];
  ...
  //-------------------------------------------------------------------------
  void pie_hline(int xc, int yc, int xp1, int yp1, int xp2, int yp2, int xh1,
                 int yh1, int xh2)
  {
  ...
  void line1_no_clip(const line_parameters& lp, int sx, int sy)
  {
    if (lp.len > line_max_length) {
      ...
      line1_no_clip(lp2, lp1.x2 + (lp1.y2 - lp1.y1),
  ...
  void line2_no_clip(const line_parameters& lp, int ex, int ey)
  {
    if (lp.len > line_max_length) {
      line_parameters lp1, lp2;
      lp.divide(lp1, lp2);
      line2_no_clip(lp1, lp1.x2 + (lp1.y2 - lp1.y1),
                    lp1.y2 - (lp1.x2 - lp1.x1));
