...
enum iterator_status_flag {
  isf_none =
    0x00, /// This is called none and not called invalid because it is not strictly the opposite of invalid.
  isf_valid =
    0x01, /// The iterator is valid, which means it is in the range of [begin, end].
  isf_current =
...
struct iterator_traits<T*> {
  typedef EASTL_ITC_NS::random_access_iterator_tag
    iterator_category; // To consider: Change this to contiguous_iterator_tag for the case that
  typedef T
    value_type; //              EASTL_ITC_NS is "eastl" instead of "std".
...
///     is_iterator_wrapper(astl::array<char>*)::value                               => false
...
class is_iterator_wrapper {
  ...
  static eastl::yes_type test(
    typename U::wrapped_iterator_type*,
    typename eastl::enable_if<eastl::is_class<U>::value>::type* = 0);

 public:
  EA_DISABLE_VC_WARNING(6334)
  static const bool value =
    (sizeof(test<Iterator>(NULL)) == sizeof(eastl::yes_type));
...
template <typename Iterator>
inline typename is_iterator_wrapper_helper<
  Iterator, eastl::is_iterator_wrapper<Iterator>::value>::iterator_type
unwrap_iterator(Iterator it)
{
  return eastl::is_iterator_wrapper_helper<
    Iterator, eastl::is_iterator_wrapper<Iterator>::value>::get_base(it);
}
...
class reverse_iterator
    : public iterator<
        typename eastl::iterator_traits<Iterator>::iterator_category,
 ...
 public:
  ...
  typedef iterator_type
    wrapped_iterator_type; // This is not in the C++ Standard; it's used by use to identify it as a wrapping iterator type.
  ...
  typedef
    typename eastl::iterator_traits<Iterator>::difference_type difference_type;
 ...
 public:
  ...
  template <typename U>
  EA_CPP14_CONSTEXPR reverse_iterator<Iterator>& operator=(
    const reverse_iterator<U>& ri)
  {
    mIterator = ri.base();
    return *this;
  }
...
EA_CPP14_CONSTEXPR inline bool operator==(const reverse_iterator<Iterator1>& a,
...
template <typename Iterator>
EA_CPP14_CONSTEXPR inline reverse_iterator<Iterator> operator+(
...
///
template <typename T>
struct is_reverse_iterator : public eastl::false_type {};
...
template <typename Iterator,
          typename IteratorType = typename eastl::conditional<
            eastl::is_nothrow_move_constructible<
              typename eastl::iterator_traits<Iterator>::value_type>::value ||
...
///
template <typename T>
struct is_move_iterator : public eastl::false_type {};
...
///
template <typename Iterator>
inline typename eastl::is_iterator_wrapper_helper<
  Iterator, eastl::is_move_iterator<Iterator>::value>::iterator_type
unwrap_move_iterator(Iterator it)
{
  return eastl::is_iterator_wrapper_helper<
    Iterator, eastl::is_move_iterator<Iterator>::value>::get_base(it);
}
...
///
template <typename Container>
class insert_iterator : public iterator<EASTL_ITC_NS::output_iterator_tag, void,
                                        void, void, void> {
 public:
  typedef Container container_type;
...
inline eastl::insert_iterator<Container> inserter(Container& x, Iterator i)
{
...
///
template <typename T>
struct is_insert_iterator : public eastl::false_type {};
...
template <typename InputIterator>
inline typename eastl::iterator_traits<InputIterator>::difference_type distance(
...
struct advance_bi_impl {
  ...
  static void advance_impl(
    ...
    Distance n) // Specialization for unsigned distance type.
  {
    while (n--)
      ++i;
  }
...
struct advance_bi_impl<true> {
  ...
  static void advance_impl(
    BidirectionalIterator& i,
    Distance n) // Specialization for signed distance type.
  {
    if (n > 0) {
      while (n--)
        ++i;
...
inline InputIterator next(
  InputIterator it,
  typename eastl::iterator_traits<InputIterator>::difference_type n = 1)
{
  eastl::advance(it, n);
  return it;
}
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto begin(const Container& container)
  -> decltype(container.begin())
{
  return container.begin();
}
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto cbegin(const Container& container)
  -> decltype(container.begin())
{
  return container.begin();
}
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto end(const Container& container)
  -> decltype(container.end())
{
  return container.end();
}
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto cend(const Container& container)
  -> decltype(container.end())
{
  return container.end();
}
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto rbegin(Container& container)
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto rbegin(const Container& container)
  -> decltype(container.rbegin())
{
  return container.rbegin();
}
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto rend(Container& container)
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto rend(const Container& container)
  -> decltype(container.rend())
{
  return container.rend();
}
...
template <typename Container>
EA_CPP14_CONSTEXPR inline auto crbegin(const Container& container)
  -> decltype(eastl::rbegin(container))
{
  return container.rbegin();
}

template <typename Container>
EA_CPP14_CONSTEXPR inline auto crend(const Container& container)
  -> decltype(eastl::rend(container))
{
  return container.rend();
}
...
template <typename T, size_t arraySize>
EA_CPP14_CONSTEXPR inline reverse_iterator<T*> rbegin(
  T (&arrayObject)[arraySize])
{
...
template <typename E>
EA_CPP14_CONSTEXPR inline reverse_iterator<const E*> rbegin(
...
template <typename E>
EA_CPP14_CONSTEXPR inline reverse_iterator<const E*> rend(
  std::initializer_list<E> ilist)
{
