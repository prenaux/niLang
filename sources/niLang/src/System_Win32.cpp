...
typedef BOOL(WINAPI* tpfnGetPointerType)(UINT32 pointerId,
...
static bool _InitializeFingerHandler(HWND hWnd, bool abEnablePressAndHold)
{
  if (!_pfnGetPointerType) {
    ...
    if (hDLL) {
      ...
      _pfnGetPointerType =
        (tpfnGetPointerType)::GetProcAddress(hDLL, "GetPointerType");
      if (_pfnGetPointerType) {
        TRACE_POINTER(
          ("... _GetPointerType: user32.dll: found GetPointerType"));
      }
      else {
        TRACE_POINTER(
          ("... _GetPointerType: user32.dll: NOT FOUND GetPointerType"));
      }
      ::FreeLibrary(hDLL);
    }
  }
...
struct sFingerHandler {
  ...
  void FingerDown(tMessageHandlerSinkLst* apMsgHandlers, tU32 anId,
                  const sVec2i& avPos)
  {
    {
      ...
      if (checkIt != mMapFingers.end()) {
        niWarning(
          niFmt("FingerDown: Pointer with id '%s' already down.", anId));
        return;
      }
    }

    tFingerMap::iterator it =
      astl::upsert(mMapFingers, anId, sFinger((tU32)mMapFingers.size()));
    if (apMsgHandlers) {
      const sFinger& finger = it->second;
      ni::SendMessages(apMsgHandlers, eOSWindowMessage_FingerDown,
                       finger.mIndex,
  ...
  void FingerUp(tMessageHandlerSinkLst* apMsgHandlers, tU32 anId,
                const sVec2i& avPos)
  {
    ...
    if (apMsgHandlers) {
      const sFinger& finger = it->second;
      ni::SendMessages(apMsgHandlers, eOSWindowMessage_FingerUp, finger.mIndex,
                       Vec3f((tF32)avPos.x, (tF32)avPos.y, 1.0f));
    }
    mMapFingers.erase(it);
  }

  void FingerMove(tMessageHandlerSinkLst* apMsgHandlers, tU32 anId,
                  const sVec2i& avPos)
  {
    tFingerMap::iterator it = mMapFingers.find(anId);
    if (it == mMapFingers.end()) {
  ...
  void _DoFingerMove(tMessageHandlerSinkLst* apMsgHandlers,
                     tFingerMap::iterator it, const sVec2i& avPos)
  {
    sFinger& finger = it->second;
    if (apMsgHandlers) {
      if (finger.mPrevPos != avPos) {
        ni::SendMessages(apMsgHandlers, eOSWindowMessage_FingerMove,
                         finger.mIndex,
                         Vec3f((tF32)avPos.x, (tF32)avPos.y, 1.0f));
        if (finger.mPrevPos.x != eInvalidHandle) {
          const sVec2i vRelMove = avPos - finger.mPrevPos;
          ni::SendMessages(apMsgHandlers, eOSWindowMessage_FingerRelativeMove,
                           finger.mIndex,
...
niCAssert(MY_WM_UNSTICK < 0x7FFF);
  ...
  #define MY_WM_IME_FINALIZESTRING \
    WM_USER + 2205                         // wParam: FinalizeString(abSend)
...
struct sWindowsStaticManager {
  ...
  sWindowsStaticManager()
  {
    ...
    if (ULColorGetA(_knWindowClearColor) != 0) {
      mbrushClear = ::CreateSolidBrush(RGB(ULColorGetR(_knWindowClearColor),
    ...
    miconLogo =
      (HICON)::LoadImage(::GetModuleHandle(0), MAKEINTRESOURCE(101), IMAGE_ICON,
                         mnIconLogoWidth, mnIconLogoHeight, LR_LOADTRANSPARENT);
  ...
  void Activate(tU32& anKeyMod)
  {
    ...
    niFlagOnIf(anKeyMod, eKeyMod_Shift, (GetKeyState(VK_SHIFT) & 0x8000));
    niFlagOnIf(anKeyMod, eKeyMod_NumLock, (GetKeyState(VK_NUMLOCK) & 0x8000));
    niFlagOnIf(anKeyMod, eKeyMod_CapsLock, (GetKeyState(VK_CAPITAL) & 0x8000));
    niFlagOnIf(anKeyMod, eKeyMod_ScrollLock, (GetKeyState(VK_SCROLL) & 0x8000));
  }
  ...
  ni::eKey TranslateKey(ni::tU32 wParam, ni::tU32 lParam, ni::tBool abDown)
  {
    ...
    case VK_CONTROL:
      if (lParam & EXTENDED_KEYMASK)
        ...
        wParam = VK_LCONTROL;
      break;
    case VK_SHIFT: wParam = VK_LSHIFT;
  #if 0
...
static const WCHAR* _wszDefaultWindowClass = L"TSWindowClass";
static LRESULT CALLBACK _StaticWndProc(HWND hWnd, UINT message, WPARAM wParam,
                                       LPARAM lParam);

  #pragma niNote( \
      "For IME to work properly the _StaticWndProc has to be wired when we register the window class, not later... otherwise it doesn't initialize properly")
...
static const achar* _GetLastErrorMessage()
{
  ...
  #define MY_WINAPI_INIT(HDLL, NAME)                        \
    ...
    TRACE_SET_DPI_AWARE(                                    \
      ("... WINAPI_INIT Loaded " #NAME ": %p", (tIntPtr)_pfn##NAME));
...
{
  if (h == DPI_AWARENESS_CONTEXT_UNAWARE) {
    return "DPI_AWARENESS_CONTEXT_UNAWARE";
  }
  else if (h == DPI_AWARENESS_CONTEXT_SYSTEM_AWARE) {
    return "DPI_AWARENESS_CONTEXT_SYSTEM_AWARE";
  }
  else if (h == DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE) {
    return "DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE";
  }
  else if (h == DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2) {
    return "DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2";
  }
  else if (h == DPI_AWARENESS_CONTEXT_UNAWARE_GDISCALED) {
    return "DPI_AWARENESS_CONTEXT_UNAWARE_GDISCALED";
  }
  else if (h == (HANDLE)eInvalidHandle) {
    return "DPI_AWARENESS_CONTEXT_INVALIDHANDLE";
  }
...
// shcore.dll
MY_WINAPI_FUNCPTR(HRESULT, SetProcessDpiAwareness,
                  (MY_PROCESS_DPI_AWARENESS value));
// shcore.dll
MY_WINAPI_FUNCPTR(HRESULT, GetScaleFactorForMonitor,
                  (HMONITOR hMon, DWORD* pScale)); // Win8 only...
// user32.dll
MY_WINAPI_FUNCPTR(HRESULT, SetProcessDPIAware, (VOID));
// user32.dll
MY_WINAPI_FUNCPTR(HANDLE, GetThreadDpiAwarenessContext, ());
// user32.dll
MY_WINAPI_FUNCPTR(MY_DPI_AWARENESS, GetAwarenessFromDpiAwarenessContext,
                  (HANDLE value));
// user32.dll
...
static void _SetThreadDpiAwareness()
{
  ...
  if (_pfnSetThreadDpiAwarenessContext) {
    awareness = _pfnSetThreadDpiAwarenessContext(
      DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
  }
  TRACE_SET_DPI_AWARE(
    ("... _SetThreadDpiAwareness: threadId: %p, awareness: %s, error: %s",
     (tU32)::GetCurrentThreadId(), _GetDpiAwarenessContextString(awareness),
...
static tBool _SetDPIAware()
{
  ...
  TRACE_SET_DPI_AWARE(
    ("... _SetDPIAware: hDLLShcore: %p", (tIntPtr)_hDLLShcore));

  HMODULE _hDLLUser32 = ::LoadLibraryW(L"user32.dll");
  TRACE_SET_DPI_AWARE(
    ("... _SetDPIAware: hDLLUser32: %p", (tIntPtr)_hDLLUser32));
  ...
  if (_pfnSetProcessDpiAwarenessContext &&
      _pfnSetProcessDpiAwarenessContext(
        DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2))
  {
    TRACE_SET_DPI_AWARE(
      ("... _SetDPIAware: SetProcessDpiAwarenessContext: succeeded"));
  }
  else {
    TRACE_SET_DPI_AWARE(
      ("... _SetDPIAware: SetProcessDpiAwarenessContext: failed: %s",
       _GetLastErrorMessage()));
  }

  if (_pfnSetProcessDpiAwareness &&
      (_pfnSetProcessDpiAwareness(MY_PROCESS_PER_MONITOR_DPI_AWARE) == S_OK))
  {
    TRACE_SET_DPI_AWARE(
      ("... _SetDPIAware: SetProcessDpiAwareness: succeeded"));
  }
  else {
    TRACE_SET_DPI_AWARE(("... _SetDPIAware: SetProcessDpiAwareness: failed: %s",
                         _GetLastErrorMessage()));
  }
  ...
  if (_pfnSetProcessDPIAware && _pfnSetProcessDPIAware()) {
    ...
    TRACE_SET_DPI_AWARE(("... _SetDPIAware: SetProcessDPIAware: failed: %s",
                         _GetLastErrorMessage()));
  }
...
static UINT _GetDpiForWindow(HWND hWnd)
{
  ...
  if (_pfnGetAwarenessFromDpiAwarenessContext &&
      _pfnGetThreadDpiAwarenessContext && _pfnGetDpiForSystem &&
      _pfnGetDpiForWindow)
  {
    // Determine the DPI to use, according to the DPI awareness mode
    MY_DPI_AWARENESS dpiAwareness = _pfnGetAwarenessFromDpiAwarenessContext(
...
static inline void _GetWindowStyle(tOSWindowStyleFlags aStyle, LONG_PTR& style,
                                   LONG_PTR& exstyle)
{
...
static HWND _CreateWindow(HWND ahWndParent, tOSWindowStyleFlags aStyle)
{
  ...
  HWND hWnd = niWin32API(CreateWindowEx)(exstyle, _wszDefaultWindowClass,
                                         L"Default", style, -1000, -1000, 0, 0,
...
class cThreadedWindow : public ni::ImplRC<ni::iUnknown> {
 public:
  cThreadedWindow(cOSWindowWindows* apOwner, HWND ahWndParent,
  ...
  virtual tU32 __stdcall Run()
  {
    ...
    while (1) {
      ...
      if (bRet == -1) {
        ...
        SignalMessage();
        switch (msg.message) {
        case WM_DESTROY: doBreak = eTrue; break;
        }
        {
...
//! Description
class cOSWindowWindows
    : public ni::ImplRC<ni::iOSWindow, ni::eImplFlags_Default,
                        ni::iOSWindowWindows> {
 ...
 public:
  ...
  void __stdcall SetClientAreaWindow(tIntPtr aHandle)
  {
    ...
    if (::IsWindow((HWND)mhClientAreaWnd)) {
      if (::GetWindowLong((HWND)mhClientAreaWnd, GWL_STYLE) & WS_CHILD) {
        ...
        ::SetWindowPos((HWND)mhClientAreaWnd, NULL, rect.GetLeft(),
  ...
  void _UpdateContentsScale(const char* aReason, UINT dpi)
  {
    ...
    mfContentsScale =
      _GetContentsScaleFromDpi(dpi ? dpi : _GetDpiForWindow(mHandle));
    TRACE_SET_DPI_AWARE((
      "... _UpdateContentsScale (%s): %p, contentsScale: %g, wasContentsScale: %g",
      aReason, (tIntPtr)mHandle, mfContentsScale, wasContentsScale));
  ...
  ///////////////////////////////////////////////
  tBool _SetWindowHandle(HWND aHWND, tBool abOwnHandle, tBool abSetWindowProc,
                         cThreadedWindow* apWindowThread)
  {
    if (!::IsWindow(aHWND))
      return eFalse;
  ...
  #ifdef niMSVC
    #pragma warning( \
        disable : 4244) //  conversion from 'LONG_PTR' to 'LONG', possible loss of data --- SetWindowLongPtr isnt declared to work warning-free with the Win64 checks
  #endif
      niWin32API(SetWindowLongPtr)(mHandle, GWLP_USERDATA, (LONG_PTR)(this));
      ((LONG_PTR&)mPrevWindowProc) =
        niWin32API(GetWindowLongPtr)(mHandle, GWLP_WNDPROC);
      if (mPrevWindowProc != _StaticWndProc) {
        // wire in _StaticWndProc
        niWin32API(SetWindowLongPtr)(mHandle, GWLP_WNDPROC,
                                     (LONG_PTR)(_StaticWndProc));
      }
      else {
    ...
    if (1) {
      ...
      Rid[0].dwFlags =
        0; //RIDEV_INPUTSINK; // Must be zero otherwise the function fails on some versions of windows
  ...
  virtual tOSWindowShowFlags __stdcall GetShow() const
  {
    ...
    switch (wp.showCmd) {
    case SW_SHOW:
    case SW_SHOWNORMAL: cmd |= eOSWindowShowFlags_Show; break;
    ...
    case SW_SHOWMAXIMIZED: // SW_MAXIMIZE has the same value...
      ...
      break;
    case SW_SHOWNOACTIVATE: cmd |= eOSWindowShowFlags_ShowNoActivate; break;
    case SW_HIDE: cmd |= eOSWindowShowFlags_Hide; break;
    case SW_RESTORE: cmd |= eOSWindowShowFlags_Restore; break;
    case SW_MINIMIZE: cmd |= eOSWindowShowFlags_Minimize; break;
    }
    return cmd;
  }
  ...
  virtual void __stdcall SetRect(const sRecti& aRect)
  {
    ...
    SetWindowPos(mHandle, NULL, newRect.GetLeft(), newRect.GetTop(),
  ...
  virtual void __stdcall SetClientSize(const sVec2i& avSize)
  {
    ...
    ::SetWindowPos(mHandle, NULL, windowRect.left, windowRect.top,
  ...
  void __stdcall _DoClear()
  {
    ...
    if (mStaticManager.miconLogo) {
      ::DrawIconEx(dc.GetDC(),
                   rect.GetLeft() +
                     ((rect.GetWidth() - mStaticManager.mnIconLogoWidth) / 2),
                   rect.GetTop() +
                     ((rect.GetHeight() - mStaticManager.mnIconLogoHeight) / 2),
                   mStaticManager.miconLogo, mStaticManager.mnIconLogoWidth,
                   mStaticManager.mnIconLogoHeight, 0, NULL,
                   DI_NORMAL | DI_COMPAT);
    }
  }
  ...
  {
    switch (aCursor) {
    case eOSCursor_None: mhCursor = NULL; break;
    case eOSCursor_Wait: mhCursor = mStaticManager.mcurWait; break;
    case eOSCursor_Hand: mhCursor = mStaticManager.mcurHelp; break;
    case eOSCursor_Text: mhCursor = mStaticManager.mcurText; break;
    case eOSCursor_Help: mhCursor = mStaticManager.mcurHelp; break;
    case eOSCursor_ResizeHz: mhCursor = mStaticManager.mcurResizeHz; break;
    case eOSCursor_ResizeVt: mhCursor = mStaticManager.mcurResizeVt; break;
    case eOSCursor_ResizeDiag: mhCursor = mStaticManager.mcurResizeDiag; break;
    ...
    default:
    case eOSCursor_Arrow: mhCursor = mStaticManager.mcurArrow; break;
    }
  ...
  ///////////////////////////////////////////////
  tBool __stdcall InitCustomCursor(tIntPtr aID, tU32 anWidth, tU32 anHeight,
                                   tU32 anPivotX, tU32 anPivotY,
                                   const tU32* apData)
  {
  ...
  tBool __stdcall SetFullScreen(tU32 anMonitor)
  {
    ...
    if (mFullScreen.setOnMonitor != eInvalidHandle) {
      ...
      ::SetWindowPos(mHandle, NULL, mFullScreen.window_rect.GetLeft(),
  ...
  virtual void __stdcall SetRefreshTimer(tF32 afRefreshTimer)
  {
    if (afRefreshTimer <= 0) {
      ...
      ::SetTimer(mHandle, TIMERID_REFRESH, (tU32)(mfRefreshTimer * 1000.0f),
                 NULL);
    }
  }
  ...
  ///////////////////////////////////////////////
  ni::tU32 __stdcall WindowsTranslateKey(ni::tU32 wParam, ni::tU32 lParam,
                                         ni::tBool abDown)
  {
 ...
 public:
  ...
  LRESULT(CALLBACK* mPrevWindowProc)
  ...
  tBool _PostMessage(eOSWindowMessage aMsg, const Var& avarA = niVarNull,
  ...
  ///////////////////////////////////////////////
  tIntPtr __stdcall WndProc(tIntPtr hWnd, tU32 message, tIntPtr wParam,
                            tIntPtr lParam)
  {
    tIntPtr sinkRet = 0;
    niCallSinkRetTest_(OSWindowWindowsSink, mlstSinks, WndProc,
                       (hWnd, message, wParam, lParam), sinkRet, sinkRet == 0);
    if (sinkRet != 0)
      return sinkRet;
    ...
    case WM_DESTROY: {
      ...
      niWindowsOSPlatformTrace(niFmt(
        _A("=== WINMSG === WM_DESTROY - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
    ...
    case WM_CLOSE: {
      niWindowsOSPlatformTrace(
        niFmt(_A("=== WINMSG === WM_CLOSE - wParam:%d:%d:%d - lParam:%d:%d:%d"),
              wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
              HIWORD(lParam)));
    ...
    case WM_SHOWWINDOW: {
      niWindowsOSPlatformTrace(niFmt(
        _A(
          "=== WINMSG === WM_SHOWWINDOW[%p] - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        mHandle, wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
    ...
    case WM_MOVE: {
      ...
      if (hasClientAreaWindow) {
        if (::GetWindowLong(ca, GWL_STYLE) & WS_CHILD) {
          ...
          ::GetClientRect(mHandle, (LPRECT)&rect);
          rect.MoveTo(sVec2i::Zero());
          ::SetWindowPos(ca, NULL, rect.GetLeft(), rect.GetTop(),
    ...
    case WM_ACTIVATE: {
      niWindowsOSPlatformTrace(niFmt(
        _A("=== WINMSG === WM_ACTIVATE - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
    ...
    case WM_ENTERMENULOOP: {
      niWindowsOSPlatformTrace(niFmt(
        _A(
          "=== WINMSG === WM_ENTERMENULOOP - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
    ...
    case WM_EXITMENULOOP: {
      niWindowsOSPlatformTrace(niFmt(
        _A(
          "=== WINMSG === WM_EXITMENULOOP - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
    ...
    case WM_INITMENUPOPUP: {
      niWindowsOSPlatformTrace(niFmt(
        _A(
          "=== WINMSG === WM_INITMENUPOPUP - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
      return 0;
    }
    ...
    case WM_MENUSELECT: {
      niWindowsOSPlatformTrace(niFmt(
        _A("=== WINMSG === WM_MENUSELECT - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
      return 0;
    }
    ...
    case WM_POWERBROADCAST: {
      niWindowsOSPlatformTrace(niFmt(
        _A(
          "=== WINMSG === WM_POWERBROADCAST - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
    ...
    case WM_SYSCOMMAND: {
      if (_kbDisableScreenSaver && ((wParam & 0xfff0) == SC_MONITORPOWER ||
    ...
    case WM_SYSKEYDOWN: {
      if (mStaticManager.AcceptKey(wParam, lParam)) {
        niWindowsOSPlatformTrace(niFmt(
          _A(
            "=== WINMSG === WM_SYSKEYDOWN - wParam:%d:%d:%d - lParam:%d:%d:%d"),
          wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
          HIWORD(lParam)));

        {
          tU32 key =
            mStaticManager.TranslateKey((tU32)wParam, (tU32)lParam, eTrue);
    ...
    case WM_KEYDOWN: {
      if (mStaticManager.AcceptKey(wParam, lParam)) {
        niWindowsOSPlatformTrace(niFmt(
          _A("=== WINMSG === WM_KEYDOWN - wParam:%d:%d:%d - lParam:%d:%d:%d"),
          wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
          HIWORD(lParam)));
        tU32 key =
          mStaticManager.TranslateKey((tU32)wParam, (tU32)lParam, eTrue);
    ...
    case WM_SYSKEYUP: {
      niWindowsOSPlatformTrace(niFmt(
        _A("=== WINMSG === WM_SYSKEYUP - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
      tU32 key =
        mStaticManager.TranslateKey((tU32)wParam, (tU32)lParam, eFalse);
    ...
    case WM_KEYUP: {
      niWindowsOSPlatformTrace(
        niFmt(_A("=== WINMSG === WM_KEYUP - wParam:%d:%d:%d - lParam:%d:%d:%d"),
              wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
              HIWORD(lParam)));
      tU32 key =
        mStaticManager.TranslateKey((tU32)wParam, (tU32)lParam, eFalse);
    ...
    case WM_CHAR: {
      niWindowsOSPlatformTrace(
        niFmt(_A("=== WINMSG === WM_CHAR - wParam:%d:%d:%d - lParam:%d:%d:%d"),
              wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
              HIWORD(lParam)));
      tU32 charCode = (tU32)wParam;
      _PostMessage(eOSWindowMessage_KeyChar, charCode,
                   mStaticManager.mnCharPreviousKey);
      mStaticManager.mnCharPreviousKey = 0;
      break;
    }
    ...
    case WM_IME_CHAR: {
      niWindowsOSPlatformTrace(niFmt(
        _A("=== WINMSG === WM_IME_CHAR - wParam:%d:%d:%d - lParam:%d:%d:%d"),
        wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
        HIWORD(lParam)));
      tU32 charCode = (tU32)wParam;
      mStaticManager.mnCharPreviousKey = 0;
    ...
    case WM_LBUTTONDOWN: {
      if (_CanSendMouseMessage()) {
        niWindowsOSPlatformTrace(niFmt(
          _A(
            "=== WINMSG === WM_LBUTTONDOWN - wParam:%d:%d:%d - lParam:%d:%d:%d"),
          wParam, LOWORD(wParam), HIWORD(wParam), lParam, LOWORD(lParam),
          HIWORD(lParam)));
        this->SetFocus();
        mStaticManager.mbMouseDown[ePointerButton_Left] = eTrue;
        _SetCursorCapture(CURSOR_CAPTURE_L, eTrue);
        _PostMessage(eOSWindowMessage_MouseButtonDown,
    ...
    case WM_LBUTTONUP: {
      if (_CanSendMouseMessage()) {
        ...
        _PostMessage(eOSWindowMessage_MouseButtonUp,
                     (ni::tU32)ePointerButton_Left);
    ...
    case WM_LBUTTONDBLCLK: {
      if (_CanSendMouseMessage()) {
        ...
        _PostMessage(eOSWindowMessage_MouseButtonDown,
                     (ni::tU32)ePointerButton_Left);
        _PostMessage(eOSWindowMessage_MouseButtonDoubleClick,
                     (ni::tU32)ePointerButton_Left);
      }
      break;
    }
            ...
            if (raw->header.dwType == RIM_TYPEMOUSE) {
              if (_CanSendMouseMessage()) {
                if (niFlagIs(raw->data.mouse.usFlags, MOUSE_MOVE_ABSOLUTE)) {
                  const ni::tBool isVirtualDesktop =
                    niFlagIs(raw->data.mouse.usFlags, MOUSE_VIRTUAL_DESKTOP);

                  const ni::tI32 width = GetSystemMetrics(
                    isVirtualDesktop ? SM_CXVIRTUALSCREEN : SM_CXSCREEN);
                  const ni::tI32 height = GetSystemMetrics(
                    isVirtualDesktop ? SM_CYVIRTUALSCREEN : SM_CYSCREEN);

                  const ni::tI32 absoluteX =
                    ni::tI32((raw->data.mouse.lLastX / 65535.0f) * width);
                  const ni::tI32 absoluteY =
                    ni::tI32((raw->data.mouse.lLastY / 65535.0f) * height);
                ...
                else if (raw->data.mouse.lLastX != 0 ||
    ...
    case WM_POINTERUPDATE: {
      ...
      if ((!_pfnGetPointerType ||
           _pfnGetPointerType(pointerId, &pointerType)) &&
          (pointerType == PT_TOUCH || pointerType == PT_PEN))
      {
        ...
        TRACE_POINTER(("=== WINMSG === WM_POINTER%s[%d:%d]: %d,%d",
                       (message == WM_POINTERDOWN)
                         ? "DOWN"
                         : ((message == WM_POINTERUP) ? "UP" : "UPDATE"),
                       (tU32)pointerType, GET_POINTERID_WPARAM(wParam), p.x,
                       p.y));
        switch (message) {
        case WM_POINTERDOWN: {
    ...
    if (mPrevWindowProc) {
      return niWin32API(CallWindowProc)(mPrevWindowProc, (HWND)hWnd, message,
                                        wParam, lParam);
    }
    else {
...
//
//----------------------------------------------------------------------------
static BOOL CALLBACK _MonitorEnumProc(HMONITOR hMonitor, HDC hdcMonitor,
                                         ...
                                         tOSWindowStyleFlags aStyle)
{
  ...
  Ptr<cOSWindowWindows> ptrWindow =
    niNew cOSWindowWindows(down_cast<iOSWindow*>(apParent));
  if (niFlagIsNot(aCreate, eOSWindowCreateFlags_NoThread)) {
    Ptr<cThreadedWindow> ptrWindowThread =
      niNew cThreadedWindow(ptrWindow, parent, aStyle);
    ...
    if (!ptrWindow->_SetWindowHandle(ptrWindowThread->GetHWND(), eTrue, eTrue,
                                     ptrWindowThread))
    {
  ...
  if (caParent) {
    ...
    ::SetParent((HWND)ptrWindow->GetHandle(), (HWND)caParent->GetHandle());
    ...
    ::SetWindowPos(ca, NULL, rect.GetLeft(), rect.GetTop(), rect.GetWidth(),
                   rect.GetHeight(),
                   SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOZORDER);
    rect.Inflate(-sVec2i::One());
    ::SetWindowPos(ca, NULL, rect.GetLeft(), rect.GetTop(), rect.GetWidth(),
                                           ...
                                           tOSWindowCreateFlags aCreate)
{
  ...
  if (!ptrWindow->_SetWindowHandle(
        ...
        niFlagIs(aCreate, eOSWindowCreateFlags_SetWindowProc), NULL))
  {
    niError(_A("Can't setup the window's handle."));
                                                ...
                                                tOSMessageBoxFlags aFlags)
{
  ...
  case IDABORT:
  case IDCANCEL:
  case IDNO: return eOSMessageBoxReturn_No;
  #ifdef IDCONTINUE
  case IDCONTINUE:
  #endif
  #ifdef IDRETRY
  ...
  case IDIGNORE:
  case IDOK:
  case IDYES: return eOSMessageBoxReturn_Yes;
  }
...
struct sOpenFileNameDialog {
  ...
  sOpenFileNameDialog(iOSWindow* aParent, const achar* aTitle,
...
///////////////////////////////////////////////
INT CALLBACK _BrowseForFolderCallbackProc(HWND hwnd, UINT uMsg, LPARAM lp,
                                          LPARAM pData)
{
...
ni::tU32 __stdcall cLang::GetNumGameCtrls() const
{
  ...
  case GAMECTRL_XINPUT: return XInputGameCtrls_GetNumGameCtrls();
  }
  return 0;
}
iGameCtrl* __stdcall cLang::GetGameCtrl(tU32 anIndex) const
{
  _InitGameCtrls();
  switch (_knGameCtrlType) {
  case GAMECTRL_XINPUT: return XInputGameCtrls_GetGameCtrl(anIndex);
  }
  return NULL;
}
                                      ...
                                      LPRECT lprcMonitor, LPARAM dwData)
{
  ...
  if (GetMonitorInfo(hMonitor, &monInfos)) {
    ...
    niFlagOnIf(mon.mFlags, eOSMonitorFlags_Primary,
               (monInfos.dwFlags & MONITORINFOF_PRIMARY));
