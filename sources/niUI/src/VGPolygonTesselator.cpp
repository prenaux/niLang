...
tBool __stdcall cVGPolygonTesselator::SubmitContour()
{
  ...
  tessAddContour(mTess, 2, mvPolyVerts.data(), sizeof(sVec2f),
                 mvPolyVerts.size());
  #endif
  ++mnNumContour;
...
tBool __stdcall cVGPolygonTesselator::EndPolygon()
{
  ...
  if (mGPCPoly.num_contours) {
    ...
    for (tI32 ts = 0; ts < tris.num_strips; ++ts) {
      ...
      mvFinalVerts->reserve(mvFinalVerts->size() +
                            (3 * (verts.num_vertices - 2)));
  ...
  if (mTess) {
    if (mnNumContour) {
      ...
      if (tessTesselate(mTess, TESS_WINDING_POSITIVE, TESS_BOUNDARY_CONTOURS, 0,
                        0, 0))
      {
        ...
        if (tessTesselate(mTess, TESS_WINDING_POSITIVE, TESS_POLYGONS, nvp, 2,
                          0))
        {
          const float* verts = tessGetVertices(mTess);
          // const int* vinds = tessGetVertexIndices(mTess);
          ...
          for (int i = 0; i < nelems; ++i) {
            ...
            for (int j = 0; j < nvp && p[j] != TESS_UNDEF; ++j) {
              mvFinalVerts->push_back(
                Vec2f(verts[p[j] * 2], verts[p[j] * 2 + 1]));
            }
          }
        }
      }
