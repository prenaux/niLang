...
struct Complex : public ni::sVec2<T> {
  ...
  inline static void _div(const T& ar, const T& ai, const T& br, const T& bi,
                          T& rr, T& ri)
  {
    T tr = br >= 0 ? br : -br;
    T ti = bi >= 0 ? bi : -bi;
...
template <typename T>
inline Complex<T> cx(T re, T im = T(0))
{
  Complex<T> c;
  c.re() = re;
  c.im() = im;
...
template <typename T>
inline Complex<T> cx(const ni::sVec2<T>& c)
{
  cx<T>(c.re(), c.im());
}
template <typename T>
inline Complex<T> re(T v)
{
  return cx<T>(v, T(0));
}
template <typename T>
inline Complex<T> im(T v)
{
  return cx<T>(T(0), v);
}
template <typename T>
inline T re(const Complex<T>& v)
{
  return v.re();
}
template <typename T>
inline T im(const Complex<T>& v)
{
  return v.im();
}
template <>
inline Complex<tF32> Num<Complex<tF32>>(const tF64 a)
{
  return cx<tF32>((tF32)a);
}
template <>
inline Complex<tF64> Num<Complex<tF64>>(const tF64 a)
{
  return cx<tF64>((tF64)a);
}
...
inline Complexf ref(tF32 v)
{
  return re<tF32>(v);
}
inline Complexf imf(tF32 v)
{
  return im<tF32>(v);
}
//! tF64 complex number
