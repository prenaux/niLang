...
struct iCanvas : public iUnknown {
  ...
  //! \remark Destination is relative to the current viewport.
  virtual tBool __stdcall BlitFillAlpha(const sRectf& aDestRect,
                                        tU32 aColor) = 0;

  //! \remark Destination is relative to the current viewport.
  virtual tBool __stdcall BlitStretch(const sRectf& aDestRect, iTexture* apSrc,
                                      const sRectf& aSrcRect) = 0;
  //! \remark Destination is relative to the current viewport.
  ...
  virtual tBool __stdcall BlitLineAlpha(const sVec2f& aStart,
                                        const sVec2f& aEnd, tU32 aColor) = 0;
  //! \remark Destination is relative to the current viewport.
  virtual tBool __stdcall BlitRect(const sRectf& aDestRect, tU32 aColor) = 0;
  //! \remark Destination is relative to the current viewport.
  virtual tBool __stdcall BlitRectAlpha(const sRectf& aDestRect,
                                        tU32 aColor) = 0;
  ...
  virtual tBool __stdcall Rect(const sVec2f& avTL, const sVec2f& avBR,
                               tF32 afZ) = 0;
  ...
  virtual tBool __stdcall RectTA2(const sVec2f& avTL, const sVec2f& avBR,
                                  ...
                                  tU32 anBRColA, tU32 anBLColA) = 0;

  //! Push an immediate mode quad with 3d positions, normals, texture coordinates, color A and color B.
  virtual tBool __stdcall QuadEx(const sVec3f& avTL, const sVec3f& avTLN,
                                 const sVec2f& avTLTex, tU32 anTLA,
                                 const sVec3f& avTR, const sVec3f& avTRN,
                                 const sVec2f& avTRTex, tU32 anTRA,
                                 const sVec3f& avBR, const sVec3f& avBRN,
                                 const sVec2f& avBRTex, tU32 anBRA,
                                 const sVec3f& avBL, const sVec3f& avBLN,
                                 const sVec2f& avBLTex, tU32 anBLA) = 0;
  //! Push an immediate mode quad with 3d positions, texture coordinates, color A and color B.
  virtual tBool __stdcall Quad(const sVec3f& avTL, const sVec3f& avTR,
                               const sVec3f& avBR, const sVec3f& avBL) = 0;
  //! Push an immediate mode quad with 3d positions, texture coordinates, color A and color B.
  virtual tBool __stdcall QuadA(const sVec3f& avTL, const sVec3f& avTR,
  ...
  //! \param anNumColumns
  //! \param anNumRows
  virtual void __stdcall LineGridXY(ni::tF32 aX, ni::tF32 aY,
                                    ni::tU32 anNumColumns,
                                    ni::tU32 anNumRows) = 0;
  ...
  //! \param anNumColumns
  //! \param anNumRows
  virtual void __stdcall LineGridXZ(ni::tF32 aX, ni::tF32 aZ,
                                    ni::tU32 anNumColumns,
                                    ni::tU32 anNumRows) = 0;

  //! Draw a circle.
  //! \remark Uses the current line parameters.
  virtual tBool __stdcall WireframeCircle(
    const sVec3f& avCenter, tF32 afRadius, tU32 aulNumDiv = 32,
    ePlaneType aPlane = ePlaneType_XY) = 0;
  //! Draw an ellipse.
  //! \remark Uses the current line parameters.
  virtual tBool __stdcall WireframeEllipse(
    const sVec3f& avCenter, const sVec2f& avRadius, tU32 aulNumDiv = 32,
    ePlaneType aPlane = ePlaneType_XY) = 0;
  //! Draw wireframe AABB.
  //! \remark Uses the current line parameters.
  virtual tBool __stdcall WireframeAABB(const sVec3f& avMin,
                                        const sVec3f& avMax) = 0;
  ...
  //! {Property}
  virtual iVGTransform* __stdcall GetVGTransform(
    eVGTransform aTransform) const = 0;
  //! Draw a VG path.
