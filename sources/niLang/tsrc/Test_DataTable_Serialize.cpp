...
TEST_FIXTURE(FDTSerialize, Base64)
{
  ...
  {
    Ptr<iDataTable> ptrDT =
      CreateDataTableFromXML(_dtXML.c_str(), _dtXML.size());
    CHECK(ptrDT.IsOK());
    compressedString = DataTableToCompressedString(ptrDT);
    CHECK(!compressedString.IsEmpty());
    niDebugFmt(("... COMPRESSED STRING: (%d)\n<{%s}>", compressedString.size(),
                compressedString));
    CHECK_EQUAL(_dtXML_Base64Compressed.size(), compressedString.size());
    CHECK_EQUAL(compressedString, _dtXML_Base64Compressed);
  }
  {
    Ptr<iDataTable> ptrDT =
      CreateDataTableFromCompressedString(compressedString.Chars());
    CHECK(ptrDT.IsOK());
  ...
  {
    compressedString =
      "hwcAAHjavZZZZZVrT9swFIb/iuVPoJVgJ81NaisxLhISaAiqMfYlMulpa82NI9spMPHjd5wGemEX TRr70tqvj31eP/ZxBqeVk+6JlEpYW4kFDOlJs1g8UbLqnD66Y7H4ZO6lGwszA0dHg4/CApkZ 3dSvwVMlZnZIKYFK3CuYDCmnh6PB2IjKTrVZEKVLoYqFcEY+Dull+793wIIk7bMo7B2EAYvi JIUDxvIeC9IoS1nYYz0MYUmS8CxBNc/jPIxXGkuzpL8KSHFmHrZyzngWRb12YR5mMQawXuh/ +D4lFj3gnj5DGe3xHm+1zvl5NQcj3ZW20kldfXneEe52ha8vwrV2wgt+v8dzqSYGqtHgLVbE CEZscG2h7uAsu6DO1TUoXHsJf8mVBVmWRmmEyBBn1E+zzCPpJyyPE88iQKBR7mFnLdY8zfs+ OGRhGnMPDYM5w6aHytM8554/rspzP3zAwzCIs7DXD9Isiv35RTEOh/8Jc0uJTPVySPsxJdq4 uS6s/I5ZOWPIGIQpSiXrolai8io62JXgA2MJ6qaxrlkUZaOUrGZtrC5L1VjMtq36NDNdCTWk rD0GuQAchXXLrpvdQZ/AVDTKUYJpn/w8Yp2ui4c5VIUBC67VlNZ12zCwBGNXB01qzFzUGn9e x20N3aB1wrjCSZ8k9Rdk0nUySsrG4C18HQ2QEa6LQnsfsU7R/OGG58P1Tg5XNxcb68v8rvtk 77zPHZ9nmAvMS2Fdal9cv7bMXyzzd7K8Y/hn7lbakI51U87Z86VuLFzA1D1faVk5MBe6/AYT Sm5BzubuTCrU2ivLgpCSG6h8JS3xTIcU+0dlCQqrx9cSCmxb8W9RN3FTvhSPXo0oGRs5Qzen iOJp3G4BV7hBbLdIDX2D6C7vaHBU+qkd9vOqbtyVdOWckgnYclv5HbCNl4+49hPkvzNr2EsJ DwW+h7PVkxPu4fvGIh4F4b4//rcu7sTDloe2/48dsP2tEvtjtf0A80hu6Q==";
    Ptr<iDataTable> ptrDT =
      CreateDataTableFromCompressedString(compressedString.Chars());
    // Expect failure
    CHECK(!ptrDT.IsOK());
...
{
  Ptr<iDataTable> ptrDT = CreateDataTableFromXML(_dtXML_Prop_with_space.c_str(),
                                                 _dtXML_Prop_with_space.size());
  CHECK(ptrDT.IsOK());
