...
///
  ...
  #define EASTL_HASH_SET_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX         \
      " hash_set" // Unless the user overrides something, this is "EASTL hash_set".
#endif
...
///
  ...
  #define EASTL_HASH_MULTISET_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX              \
      " hash_multiset" // Unless the user overrides something, this is "EASTL hash_multiset".
#endif

/// EASTL_HASH_SET_DEFAULT_ALLOCATOR
///
#ifndef EASTL_HASH_SET_DEFAULT_ALLOCATOR
  #define EASTL_HASH_SET_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_HASH_SET_DEFAULT_NAME)
#endif

/// EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR
///
#ifndef EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR
  #define EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_HASH_MULTISET_DEFAULT_NAME)
#endif
...
///
template <typename Value, typename Hash = eastl::hash<Value>,
          typename Predicate = eastl::equal_to<Value>,
          typename Allocator = EASTLAllocatorType, bool bCacheHashCode = false>
class hash_set
    : public hashtable<Value, Value, Allocator, eastl::use_self<Value>,
 ...
 public:
  ...
  explicit hash_set(
    const allocator_type& allocator = EASTL_HASH_SET_DEFAULT_ALLOCATOR)
      : base_type(0, Hash(), mod_range_hashing(), default_ranged_hash(),
                  Predicate(), eastl::use_self<Value>(), allocator)
  {
    // Empty
  }
  ...
  explicit hash_set(
    ...
    const allocator_type& allocator = EASTL_HASH_SET_DEFAULT_ALLOCATOR)
      : base_type(nBucketCount, hashFunction, mod_range_hashing(),
                  default_ranged_hash(), predicate, eastl::use_self<Value>(),
                  allocator)
  {
    // Empty
  }
  ...
  hash_set(FowardIterator first, FowardIterator last,
...
class hash_multiset
 ...
 public:
  typedef hashtable<Value, Value, Allocator, eastl::use_self<Value>, Predicate,
                    ...
                    prime_rehash_policy, bCacheHashCode, false, false>
    base_type;
  typedef hash_multiset<Value, Hash, Predicate, Allocator, bCacheHashCode>
    this_type;
  typedef typename base_type::size_type size_type;
  typedef typename base_type::value_type value_type;
  typedef typename base_type::allocator_type allocator_type;
 ...
 public:
  ...
  explicit hash_multiset(
    const allocator_type& allocator = EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR)
      : base_type(0, Hash(), mod_range_hashing(), default_ranged_hash(),
                  Predicate(), eastl::use_self<Value>(), allocator)
  {
    // Empty
  ...
  template <typename FowardIterator>
  hash_multiset(
    FowardIterator first, FowardIterator last, size_type nBucketCount = 0,
    const Hash& hashFunction = Hash(), const Predicate& predicate = Predicate(),
    const allocator_type& allocator = EASTL_HASH_MULTISET_DEFAULT_ALLOCATOR)
      : base_type(first, last, nBucketCount, hashFunction, mod_range_hashing(),
                  default_ranged_hash(), predicate, eastl::use_self<Value>(),
...
{
  typedef typename hash_set<Value, Hash, Predicate, Allocator,
       ...
       ai != aiEnd; ++ai)
  {
    ...
    if (
      (bi == biEnd) ||
      !(*ai ==
        *bi)) // We have to compare values in addition to making sure the lookups succeeded. This is because the lookup is done via the user-supplised Predicate
...
{
  typedef typename hash_multiset<Value, Hash, Predicate, Allocator,
                                 bCacheHashCode>::const_iterator const_iterator;
       ...
       ai = aRange.second) // For each element in a...
  {
    aRange = a.equal_range(
      *ai); // Get the range of elements in a that are equal to ai.
    bRange = b.equal_range(
      *ai); // Get the range of elements in b that are equal to ai.
    ...
    if (aDistance == 1) {
      if (
        !(*aRange.first ==
          *bRange
             .first)) // We have to compare values in addition to making sure the distance (element count) was equal. This is because the lookup is done via the user-supplised Predicate
