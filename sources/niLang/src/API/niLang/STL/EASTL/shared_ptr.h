...
///
  ...
  #define EASTL_SHARED_PTR_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX           \
      " shared_ptr" // Unless the user overrides something, this is "EASTL shared_ptr".
#endif

/// EASTL_SHARED_PTR_DEFAULT_ALLOCATOR
///
#ifndef EASTL_SHARED_PTR_DEFAULT_ALLOCATOR
  #define EASTL_SHARED_PTR_DEFAULT_ALLOCATOR \
    EASTLAllocatorType(EASTL_SHARED_PTR_DEFAULT_NAME)
#endif
...
struct ref_count_sp {
  int32_t
    mRefCount; /// Reference count on the contained pointer. Starts as 1 by default.
  int32_t
    mWeakRefCount; /// Reference count on contained pointer plus this ref_count_sp object itself. Starts as 1 by default.
...
inline ref_count_sp::ref_count_sp(int32_t refCount,
                                  int32_t weakRefCount) EA_NOEXCEPT
    : mRefCount(refCount),
      mWeakRefCount(weakRefCount)
{
}
...
{
  for (int32_t refCountTemp = mRefCount; refCountTemp != 0;
       refCountTemp = mRefCount)
  {
    if (Internal::atomic_compare_and_swap(&mRefCount, refCountTemp + 1,
                                          refCountTemp))
    {
      Internal::atomic_increment(&mWeakRefCount);
      return this;
...
class ref_count_sp_t : public ref_count_sp {
 public:
  ...
  ref_count_sp_t(value_type value, deleter_type deleter,
                 allocator_type allocator)
      : ref_count_sp()
      , mValue(value)
      , mDeleter(eastl::move(deleter))
...
class ref_count_sp_t_inst : public ref_count_sp {
 public:
  ...
  typedef Allocator allocator_type;
  typedef
    typename aligned_storage<sizeof(T), eastl::alignment_of<T>::value>::type
      storage_type;
...
void do_enable_shared_from_this(
  const ref_count_sp* pRefCount,
  const enable_shared_from_this<T>* pEnableSharedFromThis, const U* pValue)
{
  if (pEnableSharedFromThis)
    pEnableSharedFromThis->mWeakPtr.assign(
      const_cast<U*>(pValue), const_cast<ref_count_sp*>(pRefCount));
}
...
class shared_ptr {
 public:
  ...
  typedef typename shared_ptr_traits<T>::reference_type
    reference_type; // This defines what a reference to a T is. It's always simply T&, except for the case where T is void, whereby the reference is also just void.
 ...
 protected:
  ...
  ref_count_sp*
    mpRefCount; /// Base pointer to Reference count for owned pointer and the owned pointer.

 public:
  /// Initializes and "empty" shared_ptr.
  /// Postcondition: use_count() == zero and get() == 0
  shared_ptr() EA_NOEXCEPT : mpValue(NULL), mpRefCount(NULL)
  {
  ...
  template <typename U>
  explicit shared_ptr(
    U* pValue, typename eastl::enable_if<
      ...
      , mpRefCount(NULL) // alloc_internal will set this.
  {
    ...
    // custom version.
    alloc_internal(
      pValue, default_allocator_type(),
      default_delete<
        U>()); // Problem: We want to be able to use default_deleter_type() instead of
  ...
  shared_ptr(std::nullptr_t) EA_NOEXCEPT : mpValue(NULL), mpRefCount(NULL)
  {
    // Intentionally leaving mpRefCount as NULL. Can't allocate here due to noexcept.
  }
  ...
  shared_ptr(U* pValue, Deleter deleter,
  ...
  explicit shared_ptr(
    U* pValue, Deleter deleter, const Allocator& allocator,
    typename eastl::enable_if<
      eastl::is_convertible<U*, element_type*>::value>::type* = 0)
      : mpValue(NULL)
      , mpRefCount(NULL) // alloc_internal will set this.
  ...
  shared_ptr(const shared_ptr<U>& sharedPtr,
             typename eastl::enable_if<
               eastl::is_convertible<U*, element_type*>::value>::type* = 0)
    EA_NOEXCEPT : mpValue(sharedPtr.mpValue),
                  mpRefCount(sharedPtr.mpRefCount)
  {
    if (mpRefCount)
      mpRefCount->addref();
  ...
  template <typename U, typename Deleter>
  shared_ptr(
    unique_ptr<U, Deleter>&& uniquePtr,
    typename eastl::enable_if<
      !eastl::is_array<U>::value && !is_lvalue_reference<Deleter>::value &&
      eastl::is_convertible<U*, element_type*>::value>::type* = 0)
      : mpValue(NULL)
      , mpRefCount(NULL)
  {
    alloc_internal(uniquePtr.release(), default_allocator_type(),
                   uniquePtr.get_deleter());
  }
  ...
  template <typename U, typename Deleter, typename Allocator>
  shared_ptr(
    unique_ptr<U, Deleter>&& uniquePtr, const Allocator& allocator,
    typename eastl::enable_if<
      !eastl::is_array<U>::value && !is_lvalue_reference<Deleter>::value &&
      eastl::is_convertible<U*, element_type*>::value>::type* = 0)
      : mpValue(NULL)
    ...
    typename eastl::enable_if<
      ...
      , mpRefCount(
          weakPtr.mpRefCount
            ...
            : weakPtr
                .mpRefCount) // mpRefCount->lock() addref's the return value for us.
  {
    if (!mpRefCount) {
  ...
  // unique_ptr operator=
  template <typename U, typename Deleter>
  typename eastl::enable_if<!eastl::is_array<U>::value &&
                              eastl::is_convertible<U*, element_type*>::value,
                            this_type&>::type
  ...
  {
#if EASTL_RTTI_ENABLED
    return mpRefCount ? static_cast<Deleter*>(mpRefCount->get_deleter(
                          typeid(typename remove_cv<Deleter>::type)))
                      : NULL;
#else
 ...
 protected:
  ...
  void alloc_internal(U* pValue, Allocator allocator, Deleter deleter)
  {
    ...
    try {
      ...
      if (!pMemory)
        throw std::bad_alloc();
      mpRefCount = ::new (pMemory)
        ref_count_type(pValue, eastl::move(deleter), eastl::move(allocator));
      mpValue = pValue;
      do_enable_shared_from_this(mpRefCount, pValue, pValue);
    } catch (...) // The exception would usually be std::bad_alloc.
    {
      deleter(
        pValue); // 20.7.2.2.1 p7: If an exception is thrown, delete p is called.
    ...
    if (pMemory) {
      ...
      deleter(
        pValue); // We act the same as we do above with exceptions enabled.
    }
#endif
  }
...
template <typename T>
inline typename shared_ptr<T>::element_type* get_pointer(
  const shared_ptr<T>& sharedPtr) EA_NOEXCEPT
{
...
template <typename T, typename U>
inline bool operator==(const shared_ptr<T>& a,
                       const shared_ptr<U>& b) EA_NOEXCEPT
{
                      ...
                      const shared_ptr<U>& b) EA_NOEXCEPT
{
  ...
  typedef typename eastl::common_type<T*, U*>::type
    CPointer; // We currently need to make these temporary variables, as otherwise clang complains about CPointer being int*&&&.
  CPointer pT =
    a.get(); // I wonder if there's something wrong with our common_type type trait implementation.
  CPointer pU =
    b.get(); // "in instantiation of function template specialization 'eastl::operator<<int, int>, no known conversion from 'element_type *' (aka 'int *') to 'int *&&&' for 1st argument"
  return less<CPointer>()(
    pT,
    pU); // It looks like common_type is making CPointer be (e.g.) int*&& instead of int*, though the problem may be in how less<> deals with that.
}
...
template <typename T, typename U>
inline shared_ptr<T> reinterpret_pointer_cast(shared_ptr<U> const& sharedPtr)
  EA_NOEXCEPT
{
...
///                 where w is the return value.
template <typename T, typename U>
inline shared_ptr<T> static_pointer_cast(const shared_ptr<U>& sharedPtr)
  EA_NOEXCEPT
{
...
template <typename T,
          typename U> // Retained for support for pre-C++11 shared_ptr.
inline shared_ptr<T> static_shared_pointer_cast(const shared_ptr<U>& sharedPtr)
  EA_NOEXCEPT
{
...
///                 where w is the return value.
template <typename T, typename U>
inline shared_ptr<T> const_pointer_cast(const shared_ptr<U>& sharedPtr)
  EA_NOEXCEPT
{
...
template <typename T,
          typename U> // Retained for support for pre-C++11 shared_ptr.
inline shared_ptr<T> const_shared_pointer_cast(const shared_ptr<U>& sharedPtr)
  EA_NOEXCEPT
{
...
///
template <typename T, typename U>
inline shared_ptr<T> dynamic_pointer_cast(const shared_ptr<U>& sharedPtr)
  EA_NOEXCEPT
{
...
template <typename T,
          typename U> // Retained for support for pre-C++11 shared_ptr.
inline typename eastl::enable_if<
  !eastl::is_array<T>::value && !eastl::is_array<U>::value, shared_ptr<T>>::type
...
template <typename T>
void allocate_shared_helper(eastl::shared_ptr<T>& sharedPtr,
                            ref_count_sp* pRefCount, T* pValue)
{
...
shared_ptr<T> allocate_shared(const Allocator& allocator, Args&&... args)
{
  ...
  shared_ptr<T> ret;
  void* const pMemory =
    EASTLAlloc(const_cast<Allocator&>(allocator), sizeof(ref_count_type));
  if (pMemory) {
    ref_count_type* pRefCount =
      ::new (pMemory) ref_count_type(allocator, eastl::forward<Args>(args)...);
...
shared_ptr<T> make_shared(Args&&... args)
{
  ...
  return eastl::allocate_shared<T>(EASTL_SHARED_PTR_DEFAULT_ALLOCATOR,
...
template <typename T>
inline shared_ptr<T> atomic_load_explicit(const shared_ptr<T>* pSharedPtr,
...
template <typename T>
inline void atomic_store_explicit(shared_ptr<T>* pSharedPtrA,
                                  shared_ptr<T> sharedPtrB,
                                  ... /*std::memory_order memoryOrder*/)
{
...
template <typename T>
shared_ptr<T> atomic_exchange(shared_ptr<T>* pSharedPtrA,
                              shared_ptr<T> sharedPtrB)
{
  Internal::shared_ptr_auto_mutex autoMutex(pSharedPtrA);
  pSharedPtrA->swap(sharedPtrB);
...
template <typename T>
inline shared_ptr<T> atomic_exchange_explicit(
  shared_ptr<T>* pSharedPtrA, shared_ptr<T> sharedPtrB,
  ... /*std::memory_order memoryOrder*/)
{
...
template <typename T>
bool atomic_compare_exchange_strong(shared_ptr<T>* pSharedPtr,
                                    shared_ptr<T>* pSharedPtrCondition,
                                    shared_ptr<T> sharedPtrNew)
{
  Internal::shared_ptr_auto_mutex autoMutex(pSharedPtr);
...
template <typename T>
inline bool atomic_compare_exchange_weak(shared_ptr<T>* pSharedPtr,
                                         shared_ptr<T>* pSharedPtrCondition,
                                         shared_ptr<T> sharedPtrNew)
{
...
template <
  typename T> // Returns true if pSharedPtr was equivalent to *pSharedPtrCondition.
inline bool atomic_compare_exchange_strong_explicit(
  shared_ptr<T>* pSharedPtr, shared_ptr<T>* pSharedPtrCondition,
  shared_ptr<T> sharedPtrNew,
  ... /*memory_order memoryOrderSuccess, memory_order memoryOrderFailure*/)
{
  return atomic_compare_exchange_strong(pSharedPtr, pSharedPtrCondition,
                                        sharedPtrNew);
}
...
///
  ...
  #define EASTL_WEAK_PTR_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX         \
      " weak_ptr" // Unless the user overrides something, this is "EASTL weak_ptr".
#endif

/// EASTL_WEAK_PTR_DEFAULT_ALLOCATOR
///
#ifndef EASTL_WEAK_PTR_DEFAULT_ALLOCATOR
  #define EASTL_WEAK_PTR_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_WEAK_PTR_DEFAULT_NAME)
#endif
...
class weak_ptr {
 ...
 public:
  /// weak_ptr
  weak_ptr() EA_NOEXCEPT : mpValue(NULL), mpRefCount(NULL)
  {
  }
  ...
  weak_ptr(this_type&& weakPtr) EA_NOEXCEPT : mpValue(weakPtr.mpValue),
                                              mpRefCount(weakPtr.mpRefCount)
  {
  ...
  template <typename U>
  weak_ptr(const weak_ptr<U>& weakPtr,
           typename eastl::enable_if<
             eastl::is_convertible<U*, element_type*>::value>::type* = 0)
    EA_NOEXCEPT : mpValue(weakPtr.mpValue),
                  mpRefCount(weakPtr.mpRefCount)
  {
    if (mpRefCount)
      mpRefCount->weak_addref();
  ...
  {
    if (mpRefCount !=
        sharedPtr.mpRefCount) // This check encompasses assignment to self.
    {
  ...
  {
    if (mpRefCount !=
        weakPtr.mpRefCount) // This check encompasses assignment to self.
    {
      // Release old reference
      if (mpRefCount)
        mpRefCount->weak_release();
