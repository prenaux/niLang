...
class unique_pointer_type {
 ...
 public:
  typedef decltype(test<typename eastl::remove_reference<Deleter>::type>(
    0)) type;
};
...
template <
  ...
  bool = eastl::is_same<
    typename eastl::remove_cv<typename pointer_traits<P1>::element_type>::type,
    typename eastl::remove_cv<
...
}; // P1's underlying type is not the same as P2's, so it can't be converted, even if P2 refers to a subclass of P1. Parent == Child, but Parent[] != Child[]

template <typename P1, typename P2,
          bool = eastl::is_scalar<P1>::value && !eastl::is_pointer<P1>::value>
...
template <
  typename Base,
  typename Derived> // This returns true if Derived is unrelated to Base. That's a wrong answer, but is better for us than returning false for compilers that don't support is_base_of.
struct is_derived
    : public eastl::integral_constant<
        bool,
        !eastl::is_same<typename eastl::remove_cv<Base>::type,
...
struct is_safe_array_conversion
    : public eastl::integral_constant<
        bool, eastl::is_convertible<U_pointer, T_pointer>::value &&
                eastl::is_array<U>::value &&
                (!eastl::is_pointer<U_pointer>::value ||
                 !is_pointer<T_pointer>::value ||
                 !Internal::is_derived<
                   T, typename eastl::remove_extent<U>::type>::value)> {};
...
struct default_delete {
  ...
  (EA_COMPILER_VERSION <=        \
   4006) // GCC prior to 4.7 has a bug with noexcept here.
  ...
  template <
    typename U> // Enable if T* can be constructed with U* (i.e. U* is convertible to T*).
  default_delete(
    const default_delete<U>&,
    typename eastl::enable_if<is_convertible<U*, T*>::value>::type* = 0)
    EA_NOEXCEPT
  {
  }
...
struct default_delete<T[]> // Specialization for arrays.
{
  ...
  template <
    typename U> // This ctor is enabled if T is equal to or a base of U, and if U is less or equal const/volatile-qualified than T.
  default_delete(const default_delete<U[]>&,
                 typename eastl::enable_if<
                   Internal::is_array_cv_convertible<U*, T*>::value>::type* = 0)
    EA_NOEXCEPT
  {
  }
