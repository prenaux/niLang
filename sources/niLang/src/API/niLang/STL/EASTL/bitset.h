...
///
#if !defined(__GNUC__) || \
  (__GNUC__ >= 3) // GCC 2.x can't handle the simpler declaration below.
  #define BITSET_WORD_COUNT(nBitCount, WordType) \
    (nBitCount == 0 ? 1 : ((nBitCount - 1) / (8 * sizeof(WordType)) + 1))
#else
  #define BITSET_WORD_COUNT(nBitCount, WordType) \
    ((nBitCount - 1) / (8 * sizeof(WordType)) + 1)
#endif
...
///
#if defined(__GNUC__) && (EA_COMPILER_VERSION > 4007) && \
  defined(EA_PLATFORM_ANDROID) // Earlier than GCC 4.7
...
template <
  size_t NW,
  typename WordType> // Templated on the number of words used to hold the bitset and the word type.
struct BitsetBase {
 ...
 public:
  ...
  BitsetBase(
    uint32_t
      value); // This exists only for compatibility with std::bitset, which has a 'long' constructor.
  ...
  size_type DoFindLast()
    const; // Returns NW * kBitsPerWord (the bit count) if no bits are set.
  size_type DoFindPrev(size_type last_find)
    const; // Returns NW * kBitsPerWord (the bit count) if no bits are set.
...
struct BitsetBase<1, WordType> {
 ...
 public:
  word_type mWord
    [1]; // Defined as an array of 1 so that bitset can treat this BitsetBase like others.
 ...
 public:
  ...
  size_type DoFindLast()
    const; // Returns 1 * kBitsPerWord (the bit count) if no bits are set.
  size_type DoFindPrev(size_type last_find)
    const; // Returns 1 * kBitsPerWord (the bit count) if no bits are set.
...
struct BitsetBase<2, WordType> {
 ...
 public:
  ...
  size_type DoFindLast()
    const; // Returns 2 * kBitsPerWord (the bit count) if no bits are set.
  size_type DoFindPrev(size_type last_find)
    const; // Returns 2 * kBitsPerWord (the bit count) if no bits are set.
...
class bitset : private BitsetBase<BITSET_WORD_COUNT(N, WordType), WordType> {
 public:
  ...
  enum {
    ...
    kWordSize = sizeof(
      word_type), // The size of individual words the bitset uses to hold the bits.
    kWordCount = BITSET_WORD_COUNT(
      N,
 ...
 public:
  ...
  class reference {
   ...
   public:
    ...
    bool operator~() const;
    operator bool()
      const // Defined inline because CodeWarrior fails to be able to compile it outside.
    {
      return (*mpBitWord & (static_cast<word_type>(1)
                            << (mnBitIndex & kBitsPerWordMask))) != 0;
    }
...
inline uint32_t GetFirstBit(
  uint16_t
    x) // To do: Update this to use VC++ _BitScanForward, _BitScanForward64; GCC __builtin_ctz, __builtin_ctzl. VC++ __lzcnt16, __lzcnt, __lzcnt64 requires recent CPUs (2013+) and probably can't be used. http://en.wikipedia.org/wiki/Haswell_%28microarchitecture%29#New_features
{
  if (x) {
    uint32_t n = 1;
...
inline uint32_t GetLastBit(eastl_uint128_t x)
{
  if (x) {
    ...
    eastl_uint128_t mask(UINT64_C(
      0xFFFFFFFF00000000)); // There doesn't seem to exist compiler support for INT128_C() by any compiler. EAStdC's int128_t supports it though.
    mask <<= 64;
...
inline void BitsetBase<NW, WordType>::operator<<=(size_type n)
{
  ...
  if (nWordShift) {
    for (int i = (int)(NW - 1); i >= 0; --i)
      mWord[i] =
        (nWordShift <= (size_type)i) ? mWord[i - nWordShift] : (word_type)0;
  }

  if (n &= kBitsPerWordMask) {
    for (size_t i = (NW - 1); i > 0; --i)
      mWord[i] =
        (word_type)((mWord[i] << n) | (mWord[i - 1] >> (kBitsPerWord - n)));
    mWord[0] <<= n;
  }
...
inline void BitsetBase<NW, WordType>::operator>>=(size_type n)
{
  ...
  if (nWordShift) {
    for (size_t i = 0; i < NW; ++i)
      mWord[i] =
        ((nWordShift < (NW - i)) ? mWord[i + nWordShift] : (word_type)0);
  }
...
{
  if (value)
    mWord[i >> kBitsPerWordShift] |=
      (static_cast<word_type>(1) << (i & kBitsPerWordMask));
  else
    mWord[i >> kBitsPerWordShift] &=
      ~(static_cast<word_type>(1) << (i & kBitsPerWordMask));
}
...
template <size_t NW, typename WordType>
inline typename BitsetBase<NW, WordType>::size_type BitsetBase<
  NW, WordType>::count() const
{
  size_type n = 0;

  for (size_t i = 0; i < NW; i++) {
#if defined(__GNUC__) && (((__GNUC__ * 100) + __GNUC_MINOR__) >= 304) && \
  !defined(EA_PLATFORM_ANDROID) // GCC 3.4 or later
  #if (EA_PLATFORM_WORD_SIZE == 4)
    ...
    n += BitsetCountBits(
      mWord[i]); // GCC 2.x compiler inexplicably blows up on the code below.
#else
...
inline void BitsetBase<NW, WordType>::from_uint64(uint64_t value)
{
  ...
  EASTL_CT_ASSERT(
    NW >
    2); // We can assume this because we have specializations of BitsetBase for <1> and <2>.
  //if(NW > 1) // NW is a template constant, but it would be a little messy to take advantage of it's const-ness.
  mWord[1] = static_cast<word_type>(value >> 32);
#else
  mWord[0] = static_cast<word_type>(value);
...
#endif
  return (unsigned long)mWord
    [0]; // Todo: We need to deal with the case whereby sizeof(word_type) < sizeof(unsigned long)
}
...
template <size_t NW, typename WordType>
inline typename BitsetBase<NW, WordType>::word_type& BitsetBase<
  NW, WordType>::DoGetWord(size_type i)
{
...
template <size_t NW, typename WordType>
inline typename BitsetBase<NW, WordType>::word_type BitsetBase<
  NW, WordType>::DoGetWord(size_type i) const
{
  return mWord[i >> kBitsPerWordShift];
}
...
template <size_t NW, typename WordType>
inline typename BitsetBase<NW, WordType>::size_type BitsetBase<
  NW, WordType>::DoFindNext(size_type last_find) const
{
  ...
  if (word_index < NW) {
    ...
    word_type this_word =
      mWord[word_index] & (~static_cast<word_type>(0) << bit_index);
...
template <size_t NW, typename WordType>
inline typename BitsetBase<NW, WordType>::size_type BitsetBase<
  NW, WordType>::DoFindPrev(size_type last_find) const
{
  if (last_find > 0) {
    // Set initial state based on last find.
    size_type word_index =
      static_cast<size_type>(last_find >> kBitsPerWordShift);
    size_type bit_index = static_cast<size_type>(last_find & kBitsPerWordMask);

    // Mask off subsequent bits of the word so our search becomes a "find last".
    word_type mask =
      (~static_cast<word_type>(0) >> (kBitsPerWord - 1 - bit_index)) >>
...
template <typename WordType>
inline typename BitsetBase<1, WordType>::size_type BitsetBase<
  1, WordType>::count() const
{
#if defined(__GNUC__) && (((__GNUC__ * 100) + __GNUC_MINOR__) >= 304) && \
  !defined(EA_PLATFORM_ANDROID) // GCC 3.4 or later
  #if (EA_PLATFORM_WORD_SIZE == 4)
  ...
  #endif
#elif defined(__GNUC__) && (__GNUC__ < 3)
  return BitsetCountBits(
    mWord[0]); // GCC 2.x compiler inexplicably blows up on the code below.
#else
  size_type n = 0;
...
#if (EA_PLATFORM_WORD_SIZE == 4)
  EASTL_ASSERT(value <= 0xffffffff);
  mWord[0] = static_cast<word_type>(
    value); // This potentially loses data, but that's what the user is requesting.
#else
  mWord[0] = static_cast<word_type>(value);
#endif
}
...
#if EASTL_EXCEPTIONS_ENABLED
  #if (                            \
    (EA_PLATFORM_WORD_SIZE > 4) && \
    defined(                       \
      EA_PLATFORM_MICROSOFT)) // If we are using 64 bit words but ulong is less than 64 bits... Microsoft platforms alone use a 32 bit long under 64 bit platforms.
...
template <typename WordType>
inline typename BitsetBase<1, WordType>::word_type& BitsetBase<
  1, WordType>::DoGetWord(size_type)
{
  return mWord[0];
}
...
template <typename WordType>
inline typename BitsetBase<1, WordType>::word_type BitsetBase<
  1, WordType>::DoGetWord(size_type) const
{
  return mWord[0];
}
...
template <typename WordType>
inline typename BitsetBase<1, WordType>::size_type BitsetBase<
  1, WordType>::DoFindPrev(size_type last_find) const
{
  if (last_find > 0) {
    // Mask off previous bits of word so our search becomes a "find first".
    const word_type this_word =
      mWord[0] & ((~static_cast<word_type>(0)) >> (kBitsPerWord - last_find));
...
inline void BitsetBase<2, WordType>::operator<<=(size_type n)
{
  ...
  {
    if (EASTL_UNLIKELY(
          n >= kBitsPerWord)) // parent expected to handle high bits and n >= 64
    {
      mWord[1] = mWord[0];
      mWord[0] = 0;
      n -= kBitsPerWord;
    ...
    mWord[1] =
      (mWord[1] << n) |
      (mWord[0] >> (kBitsPerWord - n)); // Intentionally use | instead of +.
    mWord[0] <<= n;
    // We let the parent class turn off any upper bits.
  }
}
...
inline void BitsetBase<2, WordType>::operator>>=(size_type n)
{
  if (n) // to avoid a shift by kBitsPerWord, which is undefined
  {
    ...
    mWord[0] =
      (mWord[0] >> n) |
      (mWord[1] << (kBitsPerWord - n)); // Intentionally use | instead of +.
    mWord[1] >>= n;
  }
}
...
template <typename WordType>
inline typename BitsetBase<2, WordType>::size_type BitsetBase<
  2, WordType>::count() const
{
#if defined(__GNUC__) && \
  (((__GNUC__ * 100) + __GNUC_MINOR__) >= 304) // GCC 3.4 or later
  #if (EA_PLATFORM_WORD_SIZE == 4)
  return (size_type)__builtin_popcountl(mWord[0]) +
         (size_type)__builtin_popcountl(mWord[1]);
  #else
  return (size_type)__builtin_popcountll(mWord[0]) +
         (size_type)__builtin_popcountll(mWord[1]);
  #endif
...
template <typename WordType>
inline typename BitsetBase<2, WordType>::word_type& BitsetBase<
  2, WordType>::DoGetWord(size_type i)
{
  return mWord[i >> kBitsPerWordShift];
}
...
template <typename WordType>
inline typename BitsetBase<2, WordType>::word_type BitsetBase<
  2, WordType>::DoGetWord(size_type i) const
{
  return mWord[i >> kBitsPerWordShift];
}
...
template <typename WordType>
inline typename BitsetBase<2, WordType>::size_type BitsetBase<
  2, WordType>::DoFindNext(size_type last_find) const
{
  ...
  if (++last_find < (size_type)kBitsPerWord) {
    ...
    // Mask off previous bits of word so our search becomes a "find first".
    word_type this_word =
      mWord[1] & ((~static_cast<word_type>(0)) << last_find);
...
template <typename WordType>
inline typename BitsetBase<2, WordType>::size_type BitsetBase<
  2, WordType>::DoFindPrev(size_type last_find) const
{
  ...
  if (last_find > (size_type)kBitsPerWord) {
    ...
    // Mask off previous bits of word so our search becomes a "find first".
    word_type this_word =
      mWord[1] & ((~static_cast<word_type>(0)) >> (kBitsPerWord - last_find));
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::reference& bitset<
  N, WordType>::reference::operator=(bool value)
{
  if (value)
    *mpBitWord |=
      (static_cast<word_type>(1) << (mnBitIndex & kBitsPerWordMask));
  else
    *mpBitWord &=
      ~(static_cast<word_type>(1) << (mnBitIndex & kBitsPerWordMask));
  return *this;
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::reference& bitset<
  N, WordType>::reference::operator=(const reference& x)
{
  if (*x.mpBitWord &
      (static_cast<word_type>(1) << (x.mnBitIndex & kBitsPerWordMask)))
    *mpBitWord |=
      (static_cast<word_type>(1) << (mnBitIndex & kBitsPerWordMask));
  else
    *mpBitWord &=
...
{
  return (*mpBitWord &
          (static_cast<word_type>(1) << (mnBitIndex & kBitsPerWordMask))) == 0;
}
    ...
    : base_type(value)
{
  if (
    ...
    mWord[kWordCount - 1] &=
      ~(static_cast<word_type>(~static_cast<word_type>(0))
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::operator&=(
  const this_type& x)
{
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::this_type& bitset<
  N, WordType>::operator<<=(size_type n)
{
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::set()
{
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::reset()
{
  base_type::reset();
  return *this;
}
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::this_type& bitset<N, WordType>::flip()
{
  EA_DISABLE_VC_WARNING(6313)
  base_type::flip();
  if (
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::reference bitset<N, WordType>::operator[](
  size_type i)
{
#if EASTL_ASSERT_ENABLED
  if (EASTL_UNLIKELY(!(i < N)))
...
inline bool bitset<N, WordType>::operator[](size_type i) const
{
  ...
  return (DoGetWord(i) &
          (static_cast<word_type>(1) << (i & kBitsPerWordMask))) != 0;
}

template <size_t N, typename WordType>
inline const typename bitset<N, WordType>::word_type* bitset<
  N, WordType>::data() const
{
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::size_type bitset<N, WordType>::size() const
{
  return (size_type)N;
}
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_first()
  const
{
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_next(
  size_type last_find) const
{
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_last()
  const
{
...
template <size_t N, typename WordType>
inline typename bitset<N, WordType>::size_type bitset<N, WordType>::find_prev(
  size_type last_find) const
{
...
template <size_t N, typename WordType>
inline bitset<N, WordType> operator&(const bitset<N, WordType>& a,
