...
//===========================================================================
  ...
  #ifdef TSGL_DESKTOP
int tsglCreateContext(tsglContext** appCtx, iOSWindow* apWindow,
                      tU32 anColorBits, tU32 anDepthBits, tU32 anStencilBits,
                      tU32 anAA, tU32 anSwapInterval)
{
    #ifdef __APPLE__
  ...
  if (!apWindow) {
    ...
    ctx->mhWnd =
      CreateWindowA("STATIC", "", WS_POPUP | WS_DISABLED, -32000, -32000, 0, 0,
    ...
    if (bs.Create()) {
      ...
      if (_wglGetExtensionsStringARB) {
        const char* extensions =
          (const char*)_wglGetExtensionsStringARB(ctx->mhDC);
  ...
  if (!pf && _wglChoosePixelFormatARB) {
    ...
    // try AA
    if (anAA) {
      int attrsAA[] = { WGL_DRAW_TO_WINDOW_ARB,
                        GL_TRUE,
                        ...
                        0 };
      BOOL result =
        _wglChoosePixelFormatARB(ctx->mhDC, attrsAA, NULL, 1, &pf, &numFormats);
    ...
    // without AA
    if (!pf) {
      int attrs[] = { WGL_SAMPLE_BUFFERS_ARB,
                      0,
                      WGL_SAMPLES_ARB,
                      0,
                      WGL_SUPPORT_OPENGL_ARB,
      ...
      do {
        BOOL result =
          _wglChoosePixelFormatARB(ctx->mhDC, attrs, NULL, 1, &pf, &numFormats);
        if (!result || !numFormats) {
          attrs[3] >>= 1;
    ...
    if (!DescribePixelFormat(ctx->mhDC, pf, sizeof(PIXELFORMATDESCRIPTOR),
                             &pfd))
    {
      memset(&pfd, 0, sizeof(pfd));
      pf = 0;
...
int tsglCreateContext(tsglContext** appCtx, void* aOSWndHandle,
                      ...
                      const int* apConfig)
{
      ...
      #ifdef _WIN32
  {
    ...
    ctx->eglDisplay = _eglGetDisplay(ctx->displayHDC);
      ...
      #else
  ctx->eglDisplay = _eglGetDisplay(
    EGL_DEFAULT_DISPLAY); /* display can be null on some platforms */
      #endif
    #endif
  ...
  printf("- Android Window specs: %d*%d (def: %d*%d) format=%d\n",
         ctx->displayWindow->width, ctx->displayWindow->height, aDefaultWidth,
         aDefaultHeight, ctx->displayWindow->format);
  ...
  if (!_eglChooseConfig(ctx->eglDisplay, apConfig, &ctx->eglConfig, 1,
                        &numConfigs))
  {
    TSGL_FREE(ctx, sizeof(*ctx));
  ...
  ctx->eglSurface =
    _eglCreateWindowSurface(ctx->eglDisplay, ctx->eglConfig,
      #ifdef _WIN32
                            ctx->displayHWND,
      #endif
      #ifdef ANDROID
  ...
  /// Create the context
  {
    ...
    int attrib_list[] = { EGL_CONTEXT_CLIENT_VERSION, 1, EGL_NONE };
      #endif
    ctx->eglContext = _eglCreateContext(ctx->eglDisplay, ctx->eglConfig,
...
void tsglMakeCurrent(tsglContext* apCtx)
{
  ...
  if (apCtx->mpWindow) {
    ...
    #ifdef niMSVC
  try {
    _eglMakeCurrent(apCtx->eglDisplay, apCtx->eglSurface, apCtx->eglSurface,
                    apCtx->eglContext);
  } catch (...) {
  }
    #else
...
NI_DLL_BEGIN_LOADER(opengl, TSGL_DLL_NAME);
  ...
  #define TSGL_CORE_PROC(RET, FUNC, PARAMS) \
    _##FUNC = &FUNC;                        \
    ++_dllLoader._numLoaded._static
#else
  #define TSGL_CORE_PROC(RET, FUNC, PARAMS) \
    _##FUNC = (tpfn_##FUNC)_dllLoader.LoadProc(#FUNC)
#endif

#ifdef __TSGL_STATIC_EXT__
  #define TSGL_EXT_PROC(RET, FUNC, PARAMS) \
    _##FUNC = &FUNC;                       \
    ++_dllLoader._numLoaded._static
  #define TSGL_OPT_EXT_PROC(RET, FUNC, PARAMS) \
    _##FUNC = &FUNC;                           \
    ++_dllLoader._numLoaded._static
#else
  #define TSGL_EXT_PROC(RET, FUNC, PARAMS)            \
    _##FUNC = (tpfn_##FUNC)_dllLoader.LoadProcCustom( \
      #FUNC, tsglGetExtProcAddress, eFalse)
  #define TSGL_OPT_EXT_PROC(RET, FUNC, PARAMS)        \
    _##FUNC = (tpfn_##FUNC)_dllLoader.LoadProcCustom( \
      #FUNC, tsglGetExtProcAddress, eTrue)
#endif
