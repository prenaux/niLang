...
struct AppConfig {
  ...
  {
    ni::tF32 windowContentsScale =
      apWindow ? apWindow->GetContentsScale() : 1.0f;
    return niFloatIsZero(this->contentsScale) ? windowContentsScale
                                              : this->contentsScale;
  }
};
...
extern ni::tBool AppStartup(astl::non_null<AppContext*> apContext,
                            ni::iOSWindow* apWindow, ni::iRunnable* apOnStarted,
...
extern ni::tBool AppGenericStartup(astl::non_null<AppContext*> apContext,
                                   const ni::achar* aaszTitle, ni::tI32 anWidth,
                                   ni::tI32 anHeight,
                                   ni::iRunnable* apOnStarted,
                                   ni::iRunnable* apOnShutdown);

//! Should be called when the app's window is resized.
extern void AppGenericResize(astl::non_null<AppContext*> apContext, ni::tI32 w,
                             ni::tI32 h);
//! Sends keyboard event to the generic window.
extern void AppGenericKey(astl::non_null<AppContext*> apContext, ni::eKey aKey,
                          ni::tBool abIsDown);
//! Sends text input to the generic window.
extern void AppGenericInputString(astl::non_null<AppContext*> apContext,
...
extern void AppGenericFingerMove(astl::non_null<AppContext*> apContext,
                                 ni::tI32 fingerId, ni::tF32 x, ni::tF32 y,
                                 ni::tF32 pressure);
...
extern void AppGenericFingerRelativeMove(astl::non_null<AppContext*> apContext,
                                         ni::tI32 fingerId, ni::tF32 rx,
                                         ni::tF32 ry, ni::tF32 pressure);
...
extern void AppGenericMouseWheel(astl::non_null<AppContext*> apContext,
                                 ni::tF32 delta);
//! Sends a pinch gesture to the generic window.
extern void AppGenericPinch(astl::non_null<AppContext*> apContext,
                            ni::tF32 scale, const ni::eGestureState aState);
#endif

#if defined niAppLib_Native
//! Creates a native window and then calls AppStartup with the window and the
//! specified OnStarted runnable.
ni::tBool AppNativeStartup(astl::non_null<AppContext*> apContext,
                           const ni::achar* aaszTitle, ni::tI32 anWidth,
                           ni::tI32 anHeight, ni::iRunnable* apOnStarted,
                           ni::iRunnable* apOnShutdown);
