...
BOOL ccNode::SetName(const char *theName)
{
  ...
  if (name) {
    niFree(name); // Get rid of the old one first, if any.
    name = NULL;
  }
  ...
  {
    Len = strlen(theName);
    name = (char*)niMalloc(sizeof(char)*(Len+1));
...
unsigned long CalcHash(const char *String)
{
  ...
  while(*String != 0)
  {
    Hash = (Hash << 4) + ni::StrToUpper(*String++);
