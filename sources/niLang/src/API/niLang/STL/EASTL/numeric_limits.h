...
#define EASTL_LIMITS_MIN(T) \
  ((EASTL_LIMITS_IS_SIGNED(T) ? EASTL_LIMITS_MIN_S(T) : EASTL_LIMITS_MIN_U(T)))
...
#define EASTL_LIMITS_MAX(T) \
  ((EASTL_LIMITS_IS_SIGNED(T) ? EASTL_LIMITS_MAX_S(T) : EASTL_LIMITS_MAX_U(T)))
...
enum float_denorm_style {
  denorm_indeterminate =
    -1, /// It cannot be determined whether or not the type allows denormalized values.
...
class numeric_limits<bool> {
 public:
  ...
  static constexpr bool traps =
    true; // Should this be true or false? Given that it's implemented in hardware as an integer type, we use true.
...
class numeric_limits<signed long long> {
 public:
  ...
  (defined(EA_COMPILER_GNUC) ||        \
   defined(__clang__)) // If __int128_t/__uint128_t is supported...
...
class numeric_limits<float> {
 public:
  ...
  static constexpr value_type round_error()
  {
    ...
    _MSVC_STL_UPDATE >= 202206L // If using a recent version of MSVC's STL...
  static constexpr value_type infinity()
  {
    return __builtin_huge_valf();
  }
