...
{
  ni::Ptr<iFile> fp =
    ni::CreateFileMemory((tPtr)aString.Chars(), aString.size(), eFalse, NULL);
  ...
  virtual tBool __stdcall OnWrite(eJsonType aType, const achar* aaszString)
  {
    if (_prettyPrint && _indent) {
      if (aType == eJsonType_ObjectEnd || aType == eJsonType_ArrayEnd) {
        if (!(aType == eJsonType_ObjectEnd &&
              _prevType == eJsonType_ObjectBegin) &&
...
{
  return _TokenIsNumberSign() || _TokenIsNumberDigit() || _TokenIsNumberDot();
}
tBool sJsonParser::_TokenIsNumberPart() const
{
  return _TokenIsNumberSign() || _TokenIsNumberDigit() || _TokenIsNumberExp() ||
         _TokenIsNumberDot();
}
  ...
  if (_tok == '\\') {
    ...
    case 'u': {
      ...
      niLoop (i, 4) {
        if (!_NextToken()) {
          OnError(niFmt(
            "Unexpected end of stream when parsing unicode char [%d].", i));
          return eFalse;
        }
        StrSetCharX(&p, _tok);
      }
...
tBool sJsonParser::_ParseValue()
{
  ...
  if (_TokenIsNumberBegining()) {
    ...
    default: {
      if (_TokenIsEndOfValue()) {
        OnError(
          niFmt(_A("Unexpected end of value character '%c' (%d)"), _tok, _tok));
        return eFalse;
      }
      ...
      *p = 0;
      if (!_TokenIsSpace() && !_TokenIsEndOfValue()) {
        OnError(niFmt(
          _A(
            "Expected space or 'end of value' after atom value, got '%c' (%d)."),
          _tok, _tok));
        return eFalse;
      }
...
tBool sJsonParser::_ParsePair()
{
  ...
  default:
    OnError(niFmt(
      _A(
        "Expected either end of object '}' or member or pair separator (',' or ':'), got '%c' (%d)."),
      _tok, _tok));
    return eFalse;
  }
...
tBool sJsonWriter::ArrayEnd()
{
  ...
  if (!_TypeIsArray()) {
    OnError(
      niFmt(_A("Expected array type, found '%c' (%d)."), _Type(), _Type()));
    return eFalse;
  }
...
tBool sJsonWriter::ObjectEnd()
{
  ...
  if (!_TypeIsObject()) {
    OnError(
      niFmt(_A("Expected object type, found '%c' (%d)."), _Type(), _Type()));
    return eFalse;
  }
...
tBool sJsonWriter::_Item(eJsonType aType, const achar* aValue)
{
  if (_TypeIsArray()) {
    ...
    OnError(niFmt(_A("Expected array or value type, found '%c' (%d)."), _Type(),
                  _Type()));
    return eFalse;
  }
  if (aType >= eJsonType_ObjectBegin) {
    OnError(
      _A("Nested arrays and objects can't be added in a single operation."));
    return eFalse;
  }
  _TypePush(',');
  _WriteValue(aType, aValue);
...
achar sJsonWriter::_Type() const
{
  return _typeStack.top();
}
achar sJsonWriter::_TypeIsArray() const
{
  return _Type() == '[';
}
achar sJsonWriter::_TypeIsObject() const
{
  return _Type() == '{';
}
achar sJsonWriter::_TypeIsName() const
{
  return _Type() == ':';
}
achar sJsonWriter::_TypeIsValue() const
{
  return _Type() == ',';
}
achar sJsonWriter::_TypeIsPair() const
{
  return _TypeIsName() || _TypeIsValue();
}
void sJsonWriter::_TypePush(achar c)
{
  _typeStack.push(c);
}
achar sJsonWriter::_TypePop()
{
  achar c = _Type();
  _typeStack.pop();
  return c;
...
niExportFunc(ni::tBool) JsonParseStringToDataTable(const cString& aString,
                                                   ni::iDataTable* apDT)
{
  niCheckIsOK(apDT, ni::eFalse);
  _JsonParseToDataTableSink sinkParser(apDT);
