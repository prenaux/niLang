...
static __forceinline tGlyphHash TTFGlyphIndexHash(tU64 GLYPHINDEX, tU64 SIZE,
                                                  tBool DISTANCEFIELD)
{
  return ((GLYPHINDEX)
            ? (((DISTANCEFIELD) ? TTFGlyphFlags_DistanceField : 0ULL) |
               ((SIZE) << 32ULL) | (GLYPHINDEX))
            : 0ULL);
}

// this is a good compromise between quality and generation time
const int SDF_scaler = 4;
static tU8 get_SDF_radial(const tU8* fontmap, int w, int h, int x, int y,
                          int max_radius);

static cFontTTF* _GetFallback(const cFontTTF* apThis, cGraphics* apGraphics)
{
  cFont* pFallback = apGraphics->_GetFallbackFont();
  if ((pFallback) && (pFallback->mptrTTF.IsOK()) &&
      (pFallback->mptrTTF.ptr() != apThis))
  {
...
static cString GetTTFFileFontName(sMyFTLibrary* apLibrary, iFile* apFile)
{
  ...
  if (pFace == NULL) {
    niError(
      niFmt(_A("Can't load font from file '%s'."), apFile->GetSourcePath()));
    return AZEROSTR;
  }
...
static void GetTTFFontVMetrics(FT_Face aFace, int* height, int* ascent,
                               int* descent)
{
  *height = aFace->height;
...
static tBool SetTTFPixelSize(FT_Face aFace, tU32 anPixelSize,
                             tU32* apLastKerningPixelSize)
{
  ...
  FT_Error ftError = FT_Set_Pixel_Sizes(
    aFace, 0,
    (FT_UInt)((float)anPixelSize * (float)aFace->units_per_EM /
              (float)(aFace->ascender - aFace->descender)));
  if (ftError)
    return eFalse;
  return eTrue;
}

static Ptr<iBitmap2D> RenderGlyphIndexBitmap(iGraphics* apGraphics,
                                             FT_Face aFace, tU32 glyphIndex,
                                             tU32 anPixelSize, float* advance,
                                             sVec2f* lead, sVec2i* dim,
                                             tF32 ascent)
{
  ...
  FT_Error ftError =
    FT_Load_Glyph(aFace, glyphIndex, FT_LOAD_RENDER | FT_LOAD_NO_AUTOHINT);
  if (ftError) {
    // try without any hinting...
    ftError =
      FT_Load_Glyph(aFace, glyphIndex,
  ...
  *advance = (float)ftGlyph->advance.x / 64.0f;
  ...
  if (ftGlyph->bitmap.width > 0) {
    bmp = apGraphics->CreateBitmap2D(ftGlyph->bitmap.width,
                                     ftGlyph->bitmap.rows, "A8");
    memcpy(bmp->GetData(), ftGlyph->bitmap.buffer,
...
#ifdef TTF_KERNING
static int GetTTFGlyphKernAdvance(FT_Face aFace, tU32 anPixelSize,
                                  tU32* apLastKerningPixelSize, int glyph1,
                                  int glyph2)
{
...
static bool FTOutlineToPath(const FT_Outline& outline, bool flip_y,
                            const agg::trans_affine& mtx, iVGPath* path)
{
...
///////////////////////////////////////////////
static bool GetTTFGlyphPath(FT_Face face, unsigned glyph_index,
                            tU32 anPixelSize, bool flip_y,
                            const agg::trans_affine& mtx, ni::iVGPath* apPath)
{
  ...
  if (error != 0) {
    niError(niFmt("Can't load glyph '%d': %d: %s.", (tI32)glyph_index,
                  (tI32)error, MyFT_GetErrorString(error)));
    return false;
  }
  ...
  if (error != 0) {
    niError(niFmt("Can't get glyph '%d': %d: %s.", (tI32)glyph_index,
                  (tI32)error, MyFT_GetErrorString(error)));
    return false;
  }
...
///////////////////////////////////////////////
cFontTTF::cFontTTF(cGraphics* apGraphics, sMyFTLibrary* apLibrary,
                   iFile* apFile)
    : mnImageMapPageSize(
        ni::Min(TTF_PAGE_SIZE,
                apGraphics->GetDriverCaps(eGraphicsCaps_Texture2DMaxSize)))
{
  ...
  niLog(
    Info,
    niFmt(
      "Created TTF Font: '%s' (%s): isFixed: %s, hasKerning: %s, ucs2CharMap: %s, height: %d, ascent: %d, descent: %d.",
      this->GetName(), apFile->GetSourcePath(),
      this->GetIsFixedWidth() ? "yes" : "no",
      this->GetHasKerning() ? "yes" : "no", foundUCS2CharMap ? "yes" : "no",
  ...
  for (int n = 0; n < mFace->num_charmaps; ++n) {
    ...
    niLog(Info, niFmt("Font '%s' charmap%s '%s', %d, %d.",
                      apFile->GetSourcePath(), selected ? " [SELECTED]" : "",
                      _ASTR("").SetFourCC(niSwapBE32(charmap->encoding)),
...
///////////////////////////////////////////////
tBool cFontTTF::GetCharSize(sVec2f& avOut, tU32 anChar, int anSize,
                            tF32 afRealSize, tBool abDistanceField)
{
  const sFontGlyph* glyph =
    CacheCodepoint(anChar, anSize, abDistanceField, NULL);
  if (!glyph) {
    return eFalse;
  }
  avOut.x = glyph->dim.x;
...
///////////////////////////////////////////////
tF32 cFontTTF::GetCharWidth(tU32 anChar, int anSize, tF32 afRealSize,
                            tBool abDistanceField)
{
  sVec2f v = { 0, 0 };
...
///////////////////////////////////////////////
tF32 cFontTTF::GetCharHeight(tU32 anChar, int anSize, tF32 afRealSize,
                             tBool abDistanceField)
{
  sVec2f v = { 0, 0 };
  GetCharSize(v, anChar, anSize, afRealSize, abDistanceField);
...
///////////////////////////////////////////////
tBool cFontTTF::RenderGlyph(sFontGlyph* baseGlyph, tU32 anBaseSize,
                            tBool abDistanceField)
{
  ...
  if (abDistanceField) {
    ...
    sFontGlyph* glyph =
      CacheGlyphIndex(baseGlyph->glyphIndex, anBaseSize * SDF_scaler, eFalse);
    niAssert(glyph);
    if (!glyph) {
      return eFalse;
    }

    renderedBmp = mpGraphics->CreateBitmap2D(
      glyph->GetWidth() / SDF_scaler, glyph->GetHeight() / SDF_scaler, "A8");
    ...
    for (int j = 0; j < h; ++j) {
      ...
      memcpy(smooth_buf.data() + (SDF_scaler * 2 + ((j + SDF_scaler * 2) * sw)),
             row, w);
    }
    ...
    for (int j = 0; j < h; ++j) {
      ...
      for (int i = 0; i < w; ++i) {
        const tU8 dist =
          get_SDF_radial(smooth_buf.data(), sw, sh, SDF_scaler * 2 + i,
                         SDF_scaler * 2 + j, 2 * SDF_scaler);
        row[i] = dist;
      }
    }
    renderedBmp->BlitStretch(glyph->bmp, 0, 0, 0, 0, glyph->bmp->GetWidth(),
                             glyph->bmp->GetHeight(), renderedBmp->GetWidth(),
                             renderedBmp->GetHeight());

    if (_dumpGlyphs.get()) {
      mpGraphics->SaveBitmap(niFmt("%s/%s_%d_char_%d_df0.png",
                                   ni::GetProperty("ni.dirs.logs"), GetName(),
                                   anBaseSize, baseGlyph->glyphIndex),
                             glyph->bmp, 0);
    }
...

  {
    Ptr<iBitmap2D> bmp = mpGraphics->CreateBitmap2D(
      ...
      renderedBmp->GetHeight() + (TTF_GLYPH_PADDING * 2), IMAGE_PIXELFORMAT);
    bmp->Clear(NULL);
    ...
    niLoop (j, renderedBmp->GetHeight()) {
      tU8* dst = bmp->GetData() + ((j + destYOffset) * bmp->GetPitch()) +
                 (TTF_GLYPH_PADDING * 4);
    ...
    if (_dumpGlyphs.get()) {
      mpGraphics->SaveBitmap(niFmt("%s/%s_%d_char_%d_r.png",
                                   ni::GetProperty("ni.dirs.logs"), GetName(),
                                   anBaseSize, baseGlyph->glyphIndex),
                             renderedBmp, 0);
      mpGraphics->SaveBitmap(niFmt("%s/%s_%d_char_%d.png",
                                   ni::GetProperty("ni.dirs.logs"), GetName(),
                                   anBaseSize, baseGlyph->glyphIndex),
                             bmp, 0);
    }
...
///////////////////////////////////////////////
const sFontGlyph* cFontTTF::RenderGlyphIndex(tU32 anGlyphIndex, tU32 anBaseSize,
                                             tBool abDistanceField)
{
  anBaseSize = ni::Min(anBaseSize, GetMaxCharSize());
  sFontGlyph* glyph =
    CacheGlyphIndex(anGlyphIndex, anBaseSize, abDistanceField);
  if (!glyph)
    return NULL;
...
///////////////////////////////////////////////
const sFontGlyph* cFontTTF::RenderCodepoint(tU32 anCodepoint, tU32 anBaseSize,
                                            tBool abDistanceField)
{
  anBaseSize = ni::Min(anBaseSize, GetMaxCharSize());
  cFontTTF* pRenderFont;
  sFontGlyph* glyph =
    CacheCodepoint(anCodepoint, anBaseSize, abDistanceField, &pRenderFont);
  if (!glyph)
    return NULL;
...
tI32 cFontTTF::GetKerning(tU32 anPixelSize, tU32 anPrevChar, tU32 anChar) const
{
  ...
  return GetTTFGlyphKernAdvance(mFace, anPixelSize, &mnLastKerningPixelSize,
                                anPrevChar, anChar);
#else
  return 0;
#endif
}
tF32 cFontTTF::GetAdvanceGlyphIndex(tU32 anPixelSize, tBool abDistanceField,
                                    tU32 anGlyphIndex) const
{
  const sFontGlyph* g = niThis(cFontTTF)->CacheGlyphIndex(
    anGlyphIndex, anPixelSize, abDistanceField);
  if (!g) {
    return 0;
  }
  return g->xadv;
}
tF32 cFontTTF::GetAdvanceCodepoint(tU32 anPixelSize, tBool abDistanceField,
                                   tU32 anCodepoint) const
{
  const sFontGlyph* g = niThis(cFontTTF)->CacheCodepoint(
    anCodepoint, anPixelSize, abDistanceField, NULL);
  if (!g) {
    return 0;
  }
  return g->xadv;
...
///////////////////////////////////////////////
sFontGlyph* cFontTTF::CacheGlyphIndex(tU32 anGlyphIndex, tU32 anSize,
                                      tBool abDistanceField)
{
  ...
  if (it == mhmapGlyphCache.end()) {
    ...
    glyph.bmp = RenderGlyphIndexBitmap(mpGraphics, mFace, anGlyphIndex, anSize,
                                       &glyph.xadv, &glyph.lead, &glyph.dim,
                                       GetAscender(anSize));
...
///////////////////////////////////////////////
sFontGlyph* cFontTTF::CacheCodepoint(tU32 anCodepoint, tU32 anSize,
                                     tBool abDistanceField,
                                     cFontTTF** appRenderFont)
{
  {
  ...
  if (pFallback) {
    ...
    if (glyphIndex) {
      sFontGlyph* gl =
        pFallback->CacheGlyphIndex(glyphIndex, anSize, abDistanceField);
      if (gl) {
        if (appRenderFont)
...
///////////////////////////////////////////////
tBool cFontTTF::UncacheGlyphIndex(tU32 anGlyphIndex, tU32 anSize,
                                  tBool abDistanceField)
{
  tGlyphHash hash = TTFGlyphIndexHash(anGlyphIndex, anSize, abDistanceField);
  tGlyphCacheHMap::iterator it = mhmapGlyphCache.find(hash);
...
///////////////////////////////////////////////
sVec2f __stdcall cFontTTF::GetGlyphPath(iVGPath* apPath, tU32 anGlyphIndex,
                                        tU32 anSize, const sVec2f& avOffset,
                          ...
                          int max_radius)
{
  ...
  tU8 v = fontmap[x + y * w];
  for (int radius = 1; (radius <= max_radius) && (radius * radius < d2);
       ++radius)
  {
    int line, lo, hi;
    //  north
