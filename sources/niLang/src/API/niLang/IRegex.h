...
struct iPCRE : public iRegex {
  ...
  virtual cString __stdcall Sub(const achar* aaszString,
                                ...
                                tBool abDoDollarSub) = 0;
  //! @}
};

//! Creates a new PCRE regular expression object
niExportFunc(iPCRE*) CreatePCRE(const achar* aRegex = NULL,
...
niExportFunc(ni::iRegex*) CreateFilePatternRegex(
  const ni::achar* aaszPattern, const ni::achar* aaszSep = _A("|"));

niExportFunc(ni::iUnknown*) New_niLang_FilePatternRegex(const ni::Var&,
                                                        const ni::Var&);
