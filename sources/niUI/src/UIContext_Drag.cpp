...
///////////////////////////////////////////////
tBool cUIContext::BeginFingerDrag(tU32 anFinger, cWidget* apWidget,
                                  const sVec3f& avStartPos,
                                  const sVec3f& avMousePos)
{
  CHECK_FINGER(anFinger, eFalse);
  sFinger& f = GET_FINGER(anFinger);

  if (ni::VecDistance((sVec2f&)f.mvDragStart, f.GetPosition()) <
      f.mnDragStartDistance)
    return eFalse;
  ...
  if (apWidget->mClientRect.Intersect(vNCMousePos)) {
    ...
    apWidget->SendMessage(ni::eUIMessage_NCDragFingerBegin, anFinger,
                          avStartPos);
    if (isPrimaryFinger) {
      const sVec2f a = { avStartPos.x, avStartPos.y };
  ...
  _UIDragTrace(niFmt(_A("BEGIN DRAG FINGER[%d]: [%s], (%.2f,%.2f)\n"), anFinger,
                     niHStr(apWidget->GetID()), avStartPos.x, avStartPos.y));

  // Skip if widget is not a DragSource
  niCheckSilent(niFlagIs(apWidget->GetStyle(), eWidgetStyle_DragSource),
                eFalse);
...
///////////////////////////////////////////////
void cUIContext::EndFingerDrag(tU32 anFinger, const sVec3f& avAbsPos,
                               tBool abCancel)
{
...

  {
    ...
    {
      dragWidgetMT->SendMessage(ni::eUIMessage_DragFingerEnd, anFinger,
                                avAbsPos);
      if (isPrimaryFinger) {
      ...
      dragWidgetMT->SendMessage(ni::eUIMessage_NCDragFingerEnd, anFinger,
                                avAbsPos);
      if (isPrimaryFinger) {
        const sVec2f a = { avAbsPos.x, avAbsPos.y };
    ...
    _UIDragTrace(niFmt(_A("END DRAG FINGER[%d] (%.2f,%.2f), TO [%s]\n"),
                       anFinger, avAbsPos.x, avAbsPos.y,
                       dragWidgetMT->GetID()));
  }
...
///////////////////////////////////////////////
void __stdcall cUIContext::SetDragFingerStartDistance(tU32 anFinger,
