...
class cVGPath : public ni::ImplRC<iVGPath> {
 public:
  ...
  typedef agg::conv_transform<curved_smooth_stroked>
    curved_smooth_stroked_trans;
  ...
  typedef agg::conv_transform<curved_smooth_dash_stroked>
    curved_smooth_dash_stroked_trans;
  ...
  typedef agg::conv_stroke<curved_trans_smooth_dash>
    curved_trans_smooth_dash_stroked;
  ...
  virtual tBool __stdcall AddVertex(const sVec2f& avVertex,
                                    eVGPathCommand aCommand)
  {
    m_storage.add_vertex(avVertex.x, avVertex.y,
                         (agg::path_commands_e)aCommand);
    return eTrue;
  }
  ...
  void __stdcall Curve4K(tF32 x2, tF32 y2, tF32 k)
  {
    ...
    tF32 x1 = prev.x;
    tF32 y1 = prev.y;
    Curve4(x1 - (y2 - y1) * k, y1 + (x2 - x1) * k, x2 + (y2 - y1) * k,
           y2 - (x2 - x1) * k, x2, y2);
  }
  void __stdcall Curve4KRel(tF32 x2, tF32 y2, tF32 k)
  {
    ...
    tF32 x1 = prev.x;
    tF32 y1 = prev.y;
    Curve4Rel(x1 - (y2 - y1) * k, y1 + (x2 - x1) * k, x2 + (y2 - y1) * k,
              y2 - (x2 - x1) * k, x2, y2);
  }
  // Hermite curve
  void __stdcall Hermite(tF32 x2, tF32 y2, tF32 x3, tF32 y3, tF32 x4, tF32 y4)
  {
    ...
    tF32 x1 = prev.x;
    tF32 y1 = prev.y;
    return Curve4((3 * x1 + x3) / 3, (3 * y1 + y3) / 3, (3 * x2 - x4) / 3,
                  (3 * y2 - y4) / 3, x2, y2);
  }
  void __stdcall HermiteRel(tF32 x2, tF32 y2, tF32 x3, tF32 y3, tF32 x4,
                            tF32 y4)
  {
    // Trans. matrix Hermite to Bezier
    //
    ...
    tF32 x1 = prev.x;
    tF32 y1 = prev.y;
    return Curve4Rel((3 * x1 + x3) / 3, (3 * y1 + y3) / 3, (3 * x2 - x4) / 3,
                     (3 * y2 - y4) / 3, x2, y2);
  }
  // UBSpline
  void __stdcall UBSpline(tF32 x2, tF32 y2, tF32 x3, tF32 y3, tF32 x4, tF32 y4)
  {
    ...
    tF32 x1 = prev.x;
    tF32 y1 = prev.y;
    MoveTo((x1 + 4 * x2 + x3) / 6, (y1 + 4 * y2 + y3) / 6);
    return Curve4((4 * x2 + 2 * x3) / 6, (4 * y2 + 2 * y3) / 6,
                  (2 * x2 + 4 * x3) / 6, (2 * y2 + 4 * y3) / 6,
                  (x2 + 4 * x3 + x4) / 6, (y2 + 4 * y3 + y4) / 6);
  }
  void __stdcall UBSplineRel(tF32 x2, tF32 y2, tF32 x3, tF32 y3, tF32 x4,
                             tF32 y4)
  {
    // Trans. matrix Uniform BSpline to Bezier
    //
    ...
    tF32 x1 = prev.x;
    tF32 y1 = prev.y;
    MoveToRel((x1 + 4 * x2 + x3) / 6, (y1 + 4 * y2 + y3) / 6);
    return Curve4Rel((4 * x2 + 2 * x3) / 6, (4 * y2 + 2 * y3) / 6,
                     (2 * x2 + 4 * x3) / 6, (2 * y2 + 4 * y3) / 6,
                     (x2 + 4 * x3 + x4) / 6, (y2 + 4 * y3 + y4) / 6);
  }
  // Catrom
  void __stdcall Catrom(tF32 x2, tF32 y2, tF32 x3, tF32 y3, tF32 x4, tF32 y4)
  {
    ...
    tF32 y1 = prev.y;
    MoveTo(x2, y2);
    return Curve4((-x1 + 6 * x2 + x3) / 6, (-y1 + 6 * y2 + y3) / 6,
  ...
  void __stdcall CatromRel(tF32 x2, tF32 y2, tF32 x3, tF32 y3, tF32 x4, tF32 y4)
  {
    ...
    tF32 y1 = prev.y;
    MoveToRel(x2, y2);
    return Curve4Rel((-x1 + 6 * x2 + x3) / 6, (-y1 + 6 * y2 + y3) / 6,
                     (x2 + 6 * x3 - x4) / 6, (y2 + 6 * y3 - y4) / 6, x3, y3);
  }
  // A
  void __stdcall ArcTo(tF32 rx, tF32 ry, tF32 angle, tBool large_arc_flag,
                       tBool sweep_flag, tF32 x, tF32 y)
  {
    m_storage.arc_to(rx, ry, angle, !!large_arc_flag, !!sweep_flag, x, y);
  }
  // a
  void __stdcall ArcToRel(tF32 rx, tF32 ry, tF32 angle, tBool large_arc_flag,
                          tBool sweep_flag, tF32 x, tF32 y)
  {
  ...
  void __stdcall RoundedRect(tF32 x, tF32 y, tF32 width, tF32 height, tF32 rx,
                             tF32 ry)
  {
    if (rx * 2 > width)
      rx = width / 2;
  ...
  void __stdcall RoundedRectCentered(tF32 cx, tF32 cy, tF32 width, tF32 height,
                                     tF32 rx, tF32 ry)
  {
  ...
  void __stdcall Star(tF32 xc, tF32 yc, tF32 r1, tF32 r2, tU32 n,
                      tF32 start_angle = 0.0)
  {
    tU32 i;
    for (i = 0; i < n; i++) {
      ...
      if (i & 1) {
        ...
        if (i)
          LineTo(xc + dx * r2, yc + dy * r2);
        else
          MoveTo(xc + dx * r2, yc + dy * r2);
      }
    }
    ClosePolygon();
  }
  ...
  tBool __stdcall TransformAlongOnePath(const iVGPath* apPath,
                                        tBool abPreserveXScale,
                                        tF32 afBaseLength,
                                        tF32 afSegmentApproxScale,
                                        tF32 afCurvesApproxScale);
  // Transform the path against two other paths
  tBool __stdcall TransformAlongTwoPaths(const iVGPath* apPath1,
                                         const iVGPath* apPath2,
                                         tBool abPreserveXScale,
                                         tF32 afBaseHeight, tF32 afBaseLength,
                                         tF32 afSegmentApproxScale,
                                         tF32 afCurvesApproxScale);
  ...
  void RenderPath(AGGRas& aRas, const iVGTransform* apPathTransform,
                  const iVGStyle* apStyle, const sVec2f& avOffset)
  {
    ...
    if (apPathTransform)
      trans = AGGGetTransform(apPathTransform);
      ...
      if (apStyle->GetNumDashes()) {
        ...
        if (apStyle->GetSmooth() > 0.0f) {
          if (apStyle->GetStrokeTransformed()) {
            ...
            curved_trans_smooth_dash_stroked trans_smooth_dash_stroked(
              trans_smooth_dash);
            ...
            trans_smooth_dash_stroked.line_join(
              (agg::line_join_e)apStyle->GetLineJoin());
            trans_smooth_dash_stroked.line_cap(
              (agg::line_cap_e)apStyle->GetLineCap());
            trans_smooth_dash_stroked.miter_limit(apStyle->GetMiterLimit());
            trans_smooth_dash_stroked.inner_join(
              (agg::inner_join_e)apStyle->GetInnerJoin());
            ...
            curved_smooth_dash_stroked_trans smooth_dash_stroked_trans(
              smooth_dash_stroked, trans);
            ...
            smooth_dash_stroked.line_join(
              (agg::line_join_e)apStyle->GetLineJoin());
            smooth_dash_stroked.line_cap(
              (agg::line_cap_e)apStyle->GetLineCap());
            smooth_dash_stroked.miter_limit(apStyle->GetMiterLimit());
            smooth_dash_stroked.inner_join(
              (agg::inner_join_e)apStyle->GetInnerJoin());
        ...
        if (apStyle->GetSmooth() > 0.0f) {
          if (apStyle->GetStrokeTransformed()) {
            ...
            trans_smooth_stroked.line_join(
              (agg::line_join_e)apStyle->GetLineJoin());
            trans_smooth_stroked.line_cap(
              (agg::line_cap_e)apStyle->GetLineCap());
            trans_smooth_stroked.miter_limit(apStyle->GetMiterLimit());
            trans_smooth_stroked.inner_join(
              (agg::inner_join_e)apStyle->GetInnerJoin());
            ...
            curved_smooth_stroked_trans smooth_stroked_trans(smooth_stroked,
                                                             trans);
  ...
  tBool __stdcall Text(iFont* apFont, const sVec2f& avPos,
  ...
  tBool __stdcall TextAlongOnePath(iFont* apFont, const achar* aaszText,
                                   const iVGPath* apPath,
                                   tBool abPreserveXScale, tF32 afBaseLength,
                                   tF32 afSegmentApproxScale,
                                   tF32 afCurvesApproxScale)
  {
    if (!niIsOK(apFont))
    ...
    while (!itText.is_end()) {
      ...
      ptrGlyphPath->TransformAlongOnePath(apPath, abPreserveXScale,
                                          afBaseLength, afSegmentApproxScale,
                                          afCurvesApproxScale);
    ...
    tF32 afBaseLength, tF32 afSegmentApproxScale, tF32 afCurvesApproxScale)
  {
    ...
    while (!itText.is_end()) {
      ...
      ptrGlyphPath->TransformAlongTwoPaths(
        apPath1, apPath2, eTrue, afBaseHeight, afBaseLength,
        afSegmentApproxScale, afCurvesApproxScale);
      this->AddPath(ptrGlyphPath);
    }
  ...
  void __stdcall SpeechBubbleRect(const tF32 x, const tF32 y, const tF32 w,
                                  const tF32 h, const tF32 rx, const tF32 ry,
                                  const tF32 tx, const tF32 ty,
                                  const tF32 arrowGap,
                                  const tF32 arrowLen) niImpl
  {
    ...
    int edge = -1;
    // bottom edge?
    if ((tx >= x && tx <= x2 && ty >= y2) || (ty >= (y2 + testArrowLen))) {
      edge = 2;
    }
    // top edge?
    else if ((tx >= x && tx <= x2 && ty <= y) || (ty <= (y - testArrowLen))) {
      edge = 0;
    }
    // left edge?
    else if ((ty >= y && ty <= y2 && tx <= x) || (tx <= (x - testArrowLen))) {
      edge = 3;
    }
    // right edge?
    else if ((ty >= y && ty <= y2 && tx >= x2) || (tx >= (x2 + testArrowLen))) {
      edge = 1;
    }
