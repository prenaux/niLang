...
static scope_guard<TReleaseFun> make_scope_guard(TAcquireFun&& afunAcquire,
                                                 TReleaseFun&& afunRelease)
{
  auto scope =
    astl::scope_guard<TReleaseFun>(eastl::forward<TReleaseFun>(afunRelease));
  afunAcquire();
  return scope;
}
...
struct scope_guard_on_exit {
  ...
  #define ASTL_SCOPE_EXIT                                                    \
    auto ASTL_ANONYMOUS_VARIABLE(__scope_exit) = astl::scope_guard_on_exit() \
                                                 << [&]()
