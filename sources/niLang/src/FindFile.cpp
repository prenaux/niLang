...
niExportFunc(tBool) FindFile_First(sFindFile* apFF, const achar* aaszPath)
{
  ...
  if (path.IsDirectory()) {
    cString& dir = const_cast<cString&>(path.GetDirectory());
    dir
      .Remove(); // remove the trailing slash that findfirst doesnt handle as we want
    apFF->mHandle = (unix_findfirst(dir.Chars(), &apFF->mFD) == 0) ? 0 : -1;
  }
  else {
    apFF->mHandle =
      (unix_findfirst(path.GetPath().Chars(), &apFF->mFD) == 0) ? 0 : -1;
  }
#endif
...
niExportFunc(tFileAttrFlags) FindFile_FileAttribs(const sFindFile* apFF)
{
  ...
  if (attrs & _A_SUBDIR)
    nFlags |= eFileAttrFlags_Directory;
  else
    nFlags |= eFileAttrFlags_File;
  if (attrs & _A_ARCH)
    nFlags |= eFileAttrFlags_Archive;
  if (attrs & _A_HIDDEN)
    nFlags |= eFileAttrFlags_Hidden;
  if (attrs & _A_RDONLY)
    nFlags |= eFileAttrFlags_ReadOnly;
  if (attrs & _A_SYSTEM)
    nFlags |= eFileAttrFlags_System;
  return nFlags;
}
