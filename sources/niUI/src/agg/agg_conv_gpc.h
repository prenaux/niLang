...
template<class VSA, class VSB> class conv_gpc
{
 ...
 private:
  ...
  void free_gpc_data();
  void start_contour();
  void add_vertex(agg_real x, agg_real y);
  void end_contour(unsigned orientation);
  void make_polygon(gpc_polygon& p);
  void start_extracting();
  bool next_contour();
  bool next_vertex(agg_real* x, agg_real* y);
  ...
  template<class VS> void add(VS& src, gpc_polygon& p)
  {
    unsigned cmd;
    agg_real x, y;
    agg_real start_x = 0.0;
    agg_real start_y = 0.0;
    bool line_to = false;
...
//------------------------------------------------------------------------
template<class VSA, class VSB>
inline void conv_gpc<VSA, VSB>::add_vertex(agg_real x, agg_real y)
{
  gpc_vertex v;
  v.x = x;
  v.y = y;
...
//------------------------------------------------------------------------
template<class VSA, class VSB>
inline bool conv_gpc<VSA, VSB>::next_vertex(agg_real* x, agg_real* y)
{
...
//------------------------------------------------------------------------
template<class VSA, class VSB>
unsigned conv_gpc<VSA, VSB>::vertex(agg_real* x, agg_real* y)
{
