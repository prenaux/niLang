...
static const sMaterialDesc& _GetDefaultMaterial()
{
  ...
  if (!_pDefaultMaterial) {
    ...
    _pDefaultMaterial->mChannels[eMaterialChannel_Base].mColor =
      sColor4f::One();
    _pDefaultMaterial->mChannels[eMaterialChannel_Emissive].mColor =
      sColor4f::One();
    _pDefaultMaterial->mChannels[eMaterialChannel_Ambient].mColor =
      sColor4f::One();
    _pDefaultMaterial->mChannels[eMaterialChannel_Bump].mColor =
      Vec4f(0.5f, 1, 1, 0.5f);
    _pDefaultMaterial->mChannels[eMaterialChannel_Specular].mColor =
      Vec4f(0.5f, 0.5f, 0.5f, 0.5f);
    _pDefaultMaterial->mChannels[eMaterialChannel_Refraction].mColor =
      Vec4f(0.5f, 1, 1, 0.5f);
    _pDefaultMaterial->mChannels[eMaterialChannel_Environment].mColor =
      Vec4f(1, 1, 1, 0.3f);
  }
...
//----------------------------------------------------------------------------
//
// Section: cMaterial implementation.
//
//----------------------------------------------------------------------------

class cMaterial : public ni::cUserdata<ni::ImplRC<ni::iMaterial>>,
 ...
 public:
  ...
  {
    return mChannels[eMaterialChannel_Base].mTexture.IsOK()
             ? mChannels[eMaterialChannel_Base].mTexture->GetWidth()
             : 16;
  }
  tU32 __stdcall GetHeight() const
  {
    return mChannels[eMaterialChannel_Base].mTexture.IsOK()
             ? mChannels[eMaterialChannel_Base].mTexture->GetHeight()
             : 16;
  }
  tU32 __stdcall GetDepth() const
  {
    return mChannels[eMaterialChannel_Base].mTexture.IsOK()
             ? mChannels[eMaterialChannel_Base].mTexture->GetDepth()
             : 16;
  }
  ...
  tBool __stdcall SetChannelTexture(ni::eMaterialChannel aChannel,
                                    iTexture* apTexture);
  iTexture* __stdcall GetChannelTexture(ni::eMaterialChannel aChannel) const;
  tBool __stdcall SetChannelColor(ni::eMaterialChannel aChannel,
                                  const sColor4f& aColor);
  const sColor4f& __stdcall GetChannelColor(
    ni::eMaterialChannel aChannel) const;
  tBool __stdcall SetChannelSamplerStates(eMaterialChannel aChannel,
                                          tIntPtr aHandle);
  tIntPtr __stdcall GetChannelSamplerStates(eMaterialChannel aChannel) const;

  tBool __stdcall CopyChannel(eMaterialChannel aDestChannel,
                              ...
                              eMaterialChannel aSrcChannel)
  {
    ...
    const sMaterialDesc* pSrcMat =
      (const sMaterialDesc*)apSource->GetDescStructPtr();
  ...
  tBool __stdcall Serialize(eSerializeMode aMode, iDataTable* apDT,
                                       ...
                                       iHString* ahspExpr)
  {
    ...
    if (HStringIsNotEmpty(ahspExpr)) {
      mptrExpressions[aExpr] =
        ni::GetLang()->GetExpressionContext()->CreateExpression(
          niHStr(ahspExpr));
    }
    else {
  ...
  virtual sMatrixf __stdcall GetExpressionValueMatrix(
    eMaterialExpression aExpr) const
  {
...
{
  if (aChannel >= eMaterialChannel_Last)
    return eFalse;
  return niIsOK(mChannels[aChannel].mTexture);
}

///////////////////////////////////////////////
tBool __stdcall cMaterial::SetChannelTexture(eMaterialChannel aChannel,
                                             iTexture* apTexture)
{
  if (aChannel >= eMaterialChannel_Last)
    return eFalse;
  if (mChannels[aChannel].mTexture == apTexture)
    return eTrue;
...
///////////////////////////////////////////////
iTexture* __stdcall cMaterial::GetChannelTexture(
  eMaterialChannel aChannel) const
{
  if (aChannel >= eMaterialChannel_Last)
    return NULL;
...
///////////////////////////////////////////////
tBool __stdcall cMaterial::SetChannelColor(eMaterialChannel aChannel,
                                           const sColor4f& aColor)
{
  if (aChannel >= eMaterialChannel_Last)
    return eFalse;
...
///////////////////////////////////////////////
const sColor4f& __stdcall cMaterial::GetChannelColor(
  eMaterialChannel aChannel) const
{
  if (aChannel >= eMaterialChannel_Last)
...
///////////////////////////////////////////////
tBool __stdcall cMaterial::SetChannelSamplerStates(eMaterialChannel aChannel,
                                                   tIntPtr aHandle)
{
  if (aChannel >= eMaterialChannel_Last)
    return eFalse;
...
///////////////////////////////////////////////
tIntPtr __stdcall cMaterial::GetChannelSamplerStates(
  eMaterialChannel aChannel) const
{
  if (aChannel >= eMaterialChannel_Last)
    return 0;
...
///////////////////////////////////////////////
tBool cMaterial::SerializeWrite(iDataTableWriteStack* apDT,
                                iHString* ahspBasePath)
{
  if (!mptrGraphics.IsOK())
    return eFalse;
  ...
  if (mhDS) {
    apDT->SetEnum(_A("depth_stencil_states"), niEnumExpr(eCompiledStates),
                  mhDS);
  }
  ...
  niLoop (i, eMaterialChannel_Last) {
    ...
    if (memcmp(&mChannels[c], &defaultMaterial.mChannels[c],
...
///////////////////////////////////////////////
tBool cMaterial::SerializeRead(iDataTableReadStack* apDT,
                               iHString* ahspBasePath)
{
  if (!mptrGraphics.IsOK())
    return eFalse;
  ...
  SetFlags(
    apDT->GetEnumDefault(_A("flags"), niFlagsExpr(eMaterialFlags), GetFlags()));
  SetBlendMode((eBlendMode)apDT->GetEnumDefault(
    _A("blend_mode"), niEnumExpr(eBlendMode), GetBlendMode()));
  SetPolygonOffset(
    apDT->GetVec2Default(_A("polygon_offset"), GetPolygonOffset()));
  ...
  if (apDT->PushFail(_A("DepthstencilStates"))) {
    ...
    SetDepthStencilStates((eCompiledStates)apDT->GetEnumDefault(
      "depth_stencil_states", niEnumExpr(eCompiledStates),
    ...
    niLoop (i, apDT->GetNumChildren()) {
      ...
      if (c < eMaterialChannel_Last) {
        ...
        // base properties
        {
          ...
          SetChannelColor(
            c, apDT->GetVec4Default(_A("color"), GetChannelColor(c)));
        ...
        // texture
        {
          ...
          if (!HStringIsEmpty(hspTex)) {
            ptrTex = mptrGraphics->CreateTextureFromRes(hspTex, ahspBasePath,
  ...
  if (apDT->PushFail("Shaders")) {
    niLoop (i, apDT->GetNumChildren()) {
      apDT->PushChild(i);
      eShaderUnit u = (eShaderUnit)ni::GetLang()->StringToEnum(
        apDT->GetStringDefault("unit", NULL).Chars(), niEnumExpr(eShaderUnit));
      if (u < eShaderUnit_Last) {
        Ptr<iShader> ptrShader = mShaders[i];
        tHStringPtr hspPath = apDT->GetHStringDefault(
          "path", ptrShader.IsOK() ? ptrShader->GetDeviceResourceName()
                                   : _H("").raw_ptr());
...
///////////////////////////////////////////////
tBool __stdcall cMaterial::Serialize(eSerializeMode aMode, iDataTable* apDT,
                                     iHString* ahspBasePath)
{
