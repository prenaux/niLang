  ...
  if (HStringIsNotEmpty(mpWidget->GetText())) {
    ...
    if (niFlagIs(mpWidget->GetStyle(), eWidgetGroupStyle_Fold)) {
      ...
      if (folded) {
        ...
        apCanvas->BlitOverlay(sRectf(2 + rect.Left(), 6, size.x, size.y),
                              folded);
...

    {
      ...
      const sRectf txtrect = apCanvas->BlitText(
        font, sRectf(textoff, 4), 0, niHStr(mpWidget->GetLocalizedText()));
      if (false) {
        const tF32 toplen =
          rect.GetWidth() -
          (textoff + (didDrawFoldButton ? -4 : 7) + txtrect.GetWidth());
        apCanvas->BlitOverlayFrame(sRectf(textoff + txtrect.GetWidth() + 6,
                                          rect.y + 2, toplen, frameSize.y1()),
                                   frame, eRectFrameFlags_TopEdge);
      }
    }
  }
  else {
...
///////////////////////////////////////////////
tBool __stdcall cWidgetGroup::OnWidgetSink(iWidget* apWidget, tU32 anMsg,
                                           const Var& avarA, const Var& avarB)
{
  niGuardObject((iWidgetSink*)this);
  switch (anMsg) {
  case eUIMessage_SkinChanged: _UpdateSkin(); break;
  ...
  case eUIMessage_Padding: ComputeClientRect(); return eTrue;
...
void cWidgetGroup::_UpdateSkin()
{
  ...
  skin.colEvenGroup =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("EvenGroup")));
  skin.colOddGroup =
    ULColorBuild(mpWidget->FindSkinColor(color, NULL, NULL, _H("OddGroup")));
}

///////////////////////////////////////////////
void __stdcall cWidgetGroup::SetFolded(tBool abFolded)
{
  if (abFolded == mbFolded)
    return;
  mbFolded = abFolded;
  ...
  if (mbFolded) {
    ...
    const tF32 newHeight = frame->GetFrame().y + frame->GetFrame().w +
                           mpWidget->GetFont()->GetHeight() +
                           _knFontHeightMarginCollapsed;
