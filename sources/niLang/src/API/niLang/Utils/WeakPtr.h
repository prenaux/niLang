...
struct WeakPtr {
 ...
 public:
  ...
  WeakPtr(const T* other)
  {
    this->_Set(other);
  }
  WeakPtr(const Ptr<T>& other)
  {
    this->_Set(other.raw_ptr());
  }
  WeakPtr(const QPtr<T>& other)
  {
    this->_Set(other.raw_ptr());
  }
  WeakPtr(const Nonnull<T>& other)
  {
    this->_Set(other.raw_ptr());
  }
 ...
 private:
  __forceinline void _Set(const T* apObject)
  {
    mWeakPtrObject =
      apObject
        ? ni_object_get_weak_ptr((niTypename T::IUnknownBaseType*)apObject)
        : NULL;
  }
...
template <class T, class U>
inline bool operator==(WeakPtr<T> const& a, WeakPtr<U> const& b)
{
