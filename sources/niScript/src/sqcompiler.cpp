...
struct SQCompiler {
  ...
  eCompileResult _Expect(sCompileErrors& aErrors, int* toks, int num,
                         int* ptheTok, SQObjectPtr* pRet)
  {
    SQObjectPtr ret;
    int thetok = -1;
    int expectedI;
    ...
    if (expectedI == num) {
      ...
      COMPILER_ERROR(niFmt("Expected: %s got %d (%s)", str.Chars(), _token,
                           _stringval(_lex.Tok2Str(_token))));
    }
    if (ptheTok)
      *ptheTok = thetok;
    switch (thetok) {
    case TK_IDENTIFIER: ret = _H(_lex._svalue); break;
    case TK_STRING_LITERAL:
      ret = _H(cString(_lex._svalue, _lex._longstr.size() - 1));
      break;
    case TK_INTEGER: ret = _lex._nvalue; break;
    case TK_FLOAT: ret = _lex._fvalue; break;
    }
    if (pRet)
      *pRet = ret;
    return eCompileResult_OK;
  ...
  tBool Compile(sCompileErrors& aErrors, SQObjectPtr& o, int* apPos)
  {
    ...
    SQFuncState funcstate(SQFunctionProto::Create(), NULL, _lex._sourceName,
    ...
    _fs->AddLineInfos(_lex.GetLastTokenLineCol(), getGenerateLineInfo(), true);
  ...
  eCompileResult Statement(sCompileErrors& aErrors, int* apPos)
  {
    ...
    case TK_CONTINUE:
      ...
      COMPILE_CHECK(Lex(aErrors));
      break;
    case TK_FUNCTION: COMPILE_CHECK(FunctionStatement(aErrors, apPos)); break;
    case '{': {
      int stacksize = _fs->GetStackSize();
      COMPILE_CHECK(Lex(aErrors));
    ...
    case TK_TRY: COMPILE_CHECK(TryCatchStatement(aErrors, apPos)); break;
    case TK_THROW:
      COMPILE_CHECK(Lex(aErrors));
      COMPILE_CHECK(CommaExpr(aErrors, apPos));
  ...
  eCompileResult Expression(sCompileErrors& aErrors, ExpState* apExpState,
                            int* apPos)
  {
    PushExpState();
    ...
    case TK_BWXOREQ: {
      ...
      case TK_MULEQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_MULEQ);
        }
        else {
          Emit2ArgsOP(_OP_MULEQ, -1);
        }
        break;
      case TK_MODULOEQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_MODULOEQ);
        }
        else {
          Emit2ArgsOP(_OP_MODULOEQ, -1);
        }
        break;
      case TK_DIVEQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_DIVEQ);
        }
        else {
          Emit2ArgsOP(_OP_DIVEQ, -1);
        }
        break;
      case TK_SHIFTLEQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_SHIFTLEQ);
        }
        else {
          Emit2ArgsOP(_OP_SHIFTLEQ, -1);
        }
        break;
      case TK_SHIFTREQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_SHIFTREQ);
        }
        else {
          Emit2ArgsOP(_OP_SHIFTREQ, -1);
        }
        break;
      case TK_USHIFTREQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_USHIFTREQ);
        }
        else {
          Emit2ArgsOP(_OP_USHIFTREQ, -1);
        }
        break;
      case TK_BWANDEQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_BWANDEQ);
        }
        else {
          Emit2ArgsOP(_OP_BWANDEQ, -1);
        }
        break;
      case TK_BWOREQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_BWOREQ);
        }
        else {
          Emit2ArgsOP(_OP_BWOREQ, -1);
        }
        break;
      case TK_BWXOREQ:
        if (ds == DEREF_FIELD) {
          EmitDerefOp(_OP_BWXOREQ);
        }
        else {
          Emit2ArgsOP(_OP_BWXOREQ, -1);
        }
        break;
      }
  ...
  eCompileResult BIN_EXP(sCompileErrors& aErrors, SQOpcode op, int* apPos,
                         eCompileResult (SQCompiler::*f)(sCompileErrors&,
                                                         int* apPos))
  {
    COMPILE_CHECK(Lex(aErrors));
  ...
  eCompileResult LogicalAndExp(sCompileErrors& aErrors, int* apPos)
  {
    ...
    for (;;) {
      ...
      case TK_IN:
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_EXISTS, apPos, &SQCompiler::BitwiseOrExp));
        break;
  ...
  eCompileResult BitwiseOrExp(sCompileErrors& aErrors, int* apPos)
  {
    ...
    for (;;) {
      if (_token == '|') {
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_BWOR, apPos, &SQCompiler::BitwiseXorExp));
      }
      else
        return eCompileResult_OK;
    }
  ...
  eCompileResult BitwiseXorExp(sCompileErrors& aErrors, int* apPos)
  {
    ...
    for (;;) {
      if (_token == '^') {
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_BWXOR, apPos, &SQCompiler::BitwiseAndExp));
      }
      else
        return eCompileResult_OK;
    }
  ...
  eCompileResult CompExp(sCompileErrors& aErrors, int* apPos)
  {
    ...
    for (;;) {
      ...
      case TK_SPACESHIP:
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_SPACESHIP, apPos, &SQCompiler::ShiftExp));
        break;
      case TK_EQ:
  ...
  eCompileResult ShiftExp(sCompileErrors& aErrors, int* apPos)
  {
    ...
    for (;;) {
      ...
      case TK_USHIFTR:
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_USHIFTR, apPos, &SQCompiler::PlusExp));
        break;
      case TK_SHIFTL:
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_SHIFTL, apPos, &SQCompiler::PlusExp));
        break;
      case TK_SHIFTR:
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_SHIFTR, apPos, &SQCompiler::PlusExp));
        break;
      default: return eCompileResult_OK;
      }
    }
  ...
  eCompileResult MultExp(sCompileErrors& aErrors, int* apPos)
  {
    ...
    for (;;) {
      ...
      case '*':
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_MUL, apPos, &SQCompiler::PrefixedExpr));
        break;
      case '/':
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_DIV, apPos, &SQCompiler::PrefixedExpr));
        break;
      case '%':
        COMPILE_CHECK(
          BIN_EXP(aErrors, _OP_MODULO, apPos, &SQCompiler::PrefixedExpr));
        break;
      default: return eCompileResult_OK;
      }
    }
  ...
  eCompileResult GetExpr(sCompileErrors& aErrors, int& pos,
                         bool needGetIfNotDot = false)
  {
    SQObjectPtr idx;
    COMPILE_CHECK(Lex(aErrors));
    {
  ...
  eCompileResult FuncCallExpr(sCompileErrors& aErrors, const int pos,
                              const bool closeWithParen, int* apPos)
  {
  ...
  eCompileResult PrefixedExpr(sCompileErrors& aErrors, int* apPos)
  {
    ...
    for (;;) {
      ...
      case TK_PLUSPLUS: {
        if (_exst._deref != DEREF_NO_DEREF && !IsEndOfStatement()) {
          ...
          if (pos < 0) {
            ...
            // if _derefstate != DEREF_NO_DEREF && DEREF_FIELD so is the index of a local
            int src = _fs->PopTarget();
            _fs->AddInstruction(tok == TK_MINUSMINUS ? _OP_PDEC : _OP_PINC,
                                _fs->PushTarget(), src, 0, 0xFF);
          }
          if (_exst._opExt) {
            _fs->SetTopInstructionOpExt(_exst._opExt);
            _exst._opExt = 0;
      ...
      default: goto _done_PrefixedExprEx;
      }
    }
_done_PrefixedExprEx:
  ...
  eCompileResult Factor(sCompileErrors& aErrors, int* apPos)
  {
    ...
    case TK_INTEGER:
      _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(),
                          _fs->GetNumericConstant(_lex._nvalue));
      COMPILE_CHECK(Lex(aErrors));
      break;
    case TK_TRUE:
      _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(),
                          _fs->GetNumericConstant(1));
      COMPILE_CHECK(Lex(aErrors));
      break;
    case TK_FALSE:
      _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(),
                          _fs->GetNumericConstant(0));
      COMPILE_CHECK(Lex(aErrors));
      break;
    case TK_INVALID:
      _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(),
                          _fs->GetNumericConstant((SQInt)eInvalidHandle));
      COMPILE_CHECK(Lex(aErrors));
      break;
    case TK_FLOAT:
      _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(),
                          _fs->GetNumericConstant(_lex._fvalue));
      COMPILE_CHECK(Lex(aErrors));
      break;
    case '[': {
    ...
    case '{': {
      ...
      while (_token != '}') {
        ...
        int key = _fs->PopTarget();
        int table =
          _fs->TopTarget(); //<<BECAUSE OF THIS NO COMMON EMIT FUNC IS POSSIBLE
    ...
    default: {
      if (_token == TK_SEXP_START_COMMA) {
        ...
        COMPILER_ERROR(
          niFmt("expression expected, got (%d:%c)", _token, _token));
      }
    }
    }
  ...
  bool NeedGet(bool bIsDot)
  {
    ...
    return _token != '=' && _token != TK_PLUSPLUS && _token != TK_MINUSMINUS &&
           _token != TK_PLUSEQ && _token != TK_MINUSEQ && _token != TK_MULEQ &&
           _token != TK_DIVEQ && _token != TK_MODULOEQ &&
           _token != TK_SHIFTLEQ && _token != TK_SHIFTREQ &&
           _token != TK_USHIFTREQ && _token != TK_BWANDEQ &&
           _token != TK_BWOREQ && _token != TK_BWXOREQ && _token != '(' &&
           _token != TK_NEWSLOT &&
           ((!_exst._delete) ||
            (_exst._delete && (_token == '.' || _token == '[')));
  }

  eCompileResult FunctionCallArgs(sCompileErrors& aErrors, bool closeWithParen,
                                  int* apPos)
  {
    int nargs = 1; // this
    while (1) {
  ...
  eCompileResult DoWhileStatement(sCompileErrors& aErrors, int* apPos)
  {
    ...
    COMPILE_CHECK(CommaExpr(aErrors, apPos));
    COMPILE_CHECK(Expect(aErrors, ')', NULL));
    _fs->AddInstruction(_OP_JNZ, _fs->PopTarget(),
                        jzpos - _fs->GetCurrentPos() - 1);
  ...
  eCompileResult TryCatchStatement(sCompileErrors& aErrors, int* apPos)
  {
    ...
    if (_fs->GetBreakTargets()->size())
      _fs->GetBreakTargets()->back()++;
    if (_fs->GetContinueTargets()->size())
      _fs->GetContinueTargets()->back()++;
  ...
  eCompileResult PrefixIncDec(sCompileErrors& aErrors, int token, int* apPos)
  {
    ...
    if (es._deref == DEREF_FIELD) {
      ...
      int src = _fs->PopTarget();
      _fs->AddInstruction(token == TK_PLUSPLUS ? _OP_INC : _OP_DEC,
                          _fs->PushTarget(), src, 0, 0xFF);
    }
    if (opExt) {
      _fs->SetTopInstructionOpExt(opExt);
  ...
  eCompileResult ParseType(sCompileErrors& aErrors, SQObjectPtr& aTypeDef,
                           tBool* abCanOnlyBeAType)
  {
    if (abCanOnlyBeAType) {
  ...
  eCompileResult CreateFunction(sCompileErrors& aErrors, SQObjectPtr name,
                                bool hasArgList, int* apPos)
  {
    SQFuncState funcstate(SQFunctionProto::Create(), _fs, _lex._sourceName,
                          _lex.GetLastTokenLineCol());
    ...
    _fs = &funcstate;
    COMPILE_CHECK(Statement(aErrors, apPos));
    funcstate.AddLineInfos(_lex.GetLastTokenLineCol(), getGenerateLineInfo(),
                           true);
...
tBool CompileString(ain<nn<iHString>> ahspSourceName,
                    ain<tChars> aaszSourceCode, aout<sCompileErrors> aErrors,
