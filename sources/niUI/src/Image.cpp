...
class cImage : public ImplRC<iImage> {
 public:
  cImage(iGraphics* apGraphics, tU32 anWidth, tU32 anHeight, iBitmap2D* apBmp,
         iTexture* apTex, iTexture* apDepthStencil)
  {
    _Init(apGraphics, anWidth, anHeight, apBmp, apTex, apDepthStencil);
  }

  void _Init(iGraphics* apGraphics, tU32 anWidth, tU32 anHeight,
             iBitmap2D* apBmp, iTexture* apTex, iTexture* apDepthStencil)
  {
    mnFlags = 0;
    mnWidth = anWidth;
    mnHeight = anHeight;
    ...
    if (niIsOK(apTex)) {
      ...
      if (!mnWidth || !mnHeight) {
        ...
        niError(
          niFmt("Image invalid texture size, expected %dx%d but is %dx%d.",
    ...
    if (niIsOK(apDepthStencil)) {
      ...
      else if (apDepthStencil->GetWidth() != mnWidth ||
               apDepthStencil->GetHeight() != mnHeight)
      {
        niError(niFmt(
          "Image invalid depth stencil size, expected %dx%d but is %dx%d.",
          mnWidth, mnHeight, apDepthStencil->GetWidth(),
          apDepthStencil->GetHeight()));
      }
    }
  ...
  {
    Ptr<iImage> newImg =
      niNew cImage(mptrGraphics, mnWidth, mnHeight, NULL, NULL, NULL);
  ...
  ///////////////////////////////////////////////
  inline void _AddDirtyRect(sRecti& dirtyRect, const sRecti& aRect,
                            const tBool abHasDirtyRect)
  {
  ...
  iBitmap2D* __stdcall GrabBitmap(eImageUsage aLock, const sRecti& aDirtyRect)
  {
    ...
    if (!(mnFlags & IMAGEFLAGS_BMPCREATED)) {
      ...
      mptrBitmap =
        mptrGraphics->CreateBitmap2DEx(mnWidth, mnHeight, _GetPixelFormat());
    ...
    if (mptrBitmap.IsOK()) {
      ...
      if ((mptrTexture.IsOK()) && (mnFlags & IMAGEFLAGS_TEXDIRTY)) {
        if (aLock != eImageUsage_TargetDiscard)
        { // don't blit if we discard the current content
          if (!mptrGraphics->BlitTextureToBitmap(mptrTexture, eInvalidHandle,
                                                 mptrBitmap, mrectTexDirtyRect,
                                                 mrectTexDirtyRect))
          {
      ...
      const tBool isTarget =
        (aLock == eImageUsage_Target || aLock == eImageUsage_TargetDiscard);
      if (isTarget) {
    ...
    if (aLock == eImageUsage_DepthStencil) {
      if (!(mnFlags & IMAGEFLAGS_DSCREATED)) {
        ...
        mptrDepthStencil = mptrGraphics->CreateTexture(
          IMAGE_NEWDSNAME, eBitmapType_2D, _A("D24S8"), 0, mnWidth, mnHeight, 0,
      ...
      const tBool isTarget =
        (aLock == eImageUsage_Target || aLock == eImageUsage_TargetDiscard);
      // create texture
      if (!(mnFlags & IMAGEFLAGS_TEXCREATED) ||
          (isTarget && !(mnFlags & IMAGEFLAGS_TEXRT)))
      {
        ...
        mptrTexture = mptrGraphics->CreateTexture(
          IMAGE_NEWTEXNAME, eBitmapType_2D, _GetPixelFormat()->GetFormat(), 0,
          mnWidth, mnHeight, 0,
...
///////////////////////////////////////////////
iImage* __stdcall cGraphics::CreateImageEx(tU32 anWidth, tU32 anHeight,
                                           iBitmap2D* apBmp, iTexture* apTex,
                                           iTexture* apDepthStencil)
{
  iImage* img =
    niNew cImage(this, anWidth, anHeight, apBmp, apTex, apDepthStencil);
  Image_SetDebugName(img, niFmt(_A("B%dx%d"), anWidth, anHeight));
  return img;
}
...
iImage* __stdcall cGraphics::CreateImageFromResource(iHString* ahspRes)
{
  ...
  if (!niIsOK(ptrBmp)) {
    niError(
      niFmt(_A("Can't load the bitmap from resource '%s'."), niHStr(ahspRes)));
    return NULL;
  }
  if (ptrBmp->GetType() != eBitmapType_2D) {
...
struct cImageGraphicsContext : public ni::ImplRC<iGraphicsContext> {
  ...
  cImageGraphicsContext(iImage* apImage, iGraphics* apGraphics)
  {
    mptrImage = apImage;
    Ptr<iTexture> rt =
      mptrImage->GrabTexture(eImageUsage_Target, sRecti::Null());
    Ptr<iTexture> ds =
      mptrImage->GrabTexture(eImageUsage_DepthStencil, sRecti::Null());
    mptrGraphicsContext =
      apGraphics->CreateContextForRenderTargets(rt, NULL, NULL, NULL, ds);
    niCheckIsOK(mptrGraphicsContext, ;);
    const tF32 fOrthoProjectionOffset =
      ultof(mptrGraphicsContext->GetDriver()->GetCaps(
        eGraphicsCaps_OrthoProjectionOffset));
    mptrGraphicsContext->GetFixedStates()->SetCameraViewMatrix(
      sMatrixf::Identity());
    mptrGraphicsContext->GetFixedStates()->SetCameraProjectionMatrix(
      MatrixProjection2D(fOrthoProjectionOffset,
                         Rectf(0, 0, rt->GetWidth(), rt->GetHeight()), 0.0f));
  }
  ...
  virtual void __stdcall ClearBuffers(tClearBuffersFlags clearBuffer,
                                      ...
                                      tI32 anStencil)
  {
    return mptrGraphicsContext->ClearBuffers(clearBuffer, anColor, afDepth,
                                             anStencil);
  }
