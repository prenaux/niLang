...
class intrusive_list_iterator {
 ...
 public:
  intrusive_list_iterator();
  explicit intrusive_list_iterator(
    pointer
      pNode); // Note that you can also construct an iterator from T via this, since value_type == node_type.
...
class intrusive_list_base {
 public:
  typedef eastl_size_t
    size_type; // See config.h for the definition of this, which defaults to size_t.
  typedef ptrdiff_t difference_type;

 protected:
  intrusive_list_node
    mAnchor; ///< Sentinel node (end). All data nodes are linked in a ring from this node.
 ...
 public:
  ...
  eastl_size_t size() const
    EA_NOEXCEPT; ///< Returns the number of elements in the list; O(n).
  void clear() EA_NOEXCEPT; ///< Clears the list; O(1). No deallocation occurs.
  void
  pop_front(); ///< Removes an element from the front of the list; O(1). The element must exist, but is not deallocated.
  void
  pop_back(); ///< Removes an element from the back of the list; O(1). The element must exist, but is not deallocated.
  EASTL_API void reverse()
    EA_NOEXCEPT; ///< Reverses a list so that front and back are swapped; O(n).

  EASTL_API bool validate()
    const; ///< Scans a list for linkage inconsistencies; O(n) time, O(1) space. Returns false if errors are detected, such as loops or branching.
...
class intrusive_list : public intrusive_list_base {
 ...
 public:
  ...
  intrusive_list(
    const this_type& x); ///< Creates an empty list; ignores the argument.
  ...
  this_type& operator=(
    const this_type& x); ///< Clears the list; ignores the argument.
  void swap(this_type&); ///< Swaps the contents of two intrusive lists; O(1).

  iterator begin()
    EA_NOEXCEPT; ///< Returns an iterator pointing to the first element in the list.
  const_iterator begin() const
    EA_NOEXCEPT; ///< Returns a const_iterator pointing to the first element in the list.
  const_iterator cbegin() const
    EA_NOEXCEPT; ///< Returns a const_iterator pointing to the first element in the list.

  iterator end()
    EA_NOEXCEPT; ///< Returns an iterator pointing one-after the last element in the list.
  const_iterator end() const
    EA_NOEXCEPT; ///< Returns a const_iterator pointing one-after the last element in the list.
  const_iterator cend() const
    EA_NOEXCEPT; ///< Returns a const_iterator pointing one-after the last element in the list.

  reverse_iterator rbegin()
    EA_NOEXCEPT; ///< Returns a reverse_iterator pointing at the end of the list (start of the reverse sequence).
  const_reverse_iterator rbegin() const
    EA_NOEXCEPT; ///< Returns a const_reverse_iterator pointing at the end of the list (start of the reverse sequence).
  const_reverse_iterator crbegin() const
    EA_NOEXCEPT; ///< Returns a const_reverse_iterator pointing at the end of the list (start of the reverse sequence).

  reverse_iterator rend()
    EA_NOEXCEPT; ///< Returns a reverse_iterator pointing at the start of the list (end of the reverse sequence).
  const_reverse_iterator rend() const
    EA_NOEXCEPT; ///< Returns a const_reverse_iterator pointing at the start of the list (end of the reverse sequence).
  const_reverse_iterator crend() const
    EA_NOEXCEPT; ///< Returns a const_reverse_iterator pointing at the start of the list (end of the reverse sequence).

  reference
  front(); ///< Returns a reference to the first element. The list must be non-empty.
  const_reference front()
    const; ///< Returns a const reference to the first element. The list must be non-empty.
  reference
  back(); ///< Returns a reference to the last element. The list must be non-empty.
  const_reference back()
    const; ///< Returns a const reference to the last element. The list must be non-empty.

  void push_front(
    value_type&
      x); ///< Adds an element to the front of the list; O(1). The element is not copied. The element must not be in any other list.
  void push_back(
    value_type&
      x); ///< Adds an element to the back of the list; O(1). The element is not copied. The element must not be in any other list.

  bool contains(const value_type& x)
    const; ///< Returns true if the given element is in the list; O(n). Equivalent to (locate(x) != end()).

  iterator locate(
    value_type&
      x); ///< Converts a reference to an object in the list back to an iterator, or returns end() if it is not part of the list. O(n)
  const_iterator locate(const value_type& x)
    const; ///< Converts a const reference to an object in the list back to a const iterator, or returns end() if it is not part of the list. O(n)

  iterator insert(
    const_iterator pos,
    value_type&
      x); ///< Inserts an element before the element pointed to by the iterator. O(1)
  iterator erase(
    const_iterator
      pos); ///< Erases the element pointed to by the iterator. O(1)
  iterator erase(
    const_iterator pos,
    const_iterator
      last); ///< Erases elements within the iterator range [pos, last). O(1)
  ...
  static void remove(
    value_type&
      value); ///< Erases an element from a list; O(1). Note that this is static so you don't need to know which list the element, although it must be in some list.
  ...
  void splice(const_iterator pos, intrusive_list& x, const_iterator first,
              const_iterator last);
...
inline bool operator==(
  const intrusive_list_iterator<T, PointerA, ReferenceA>& a,
...
template <typename T>
inline typename intrusive_list<T>::this_type& intrusive_list<T>::operator=(
  const this_type& /*x*/)
{
  // We intentionally ignore argument x.
...
template <typename T>
inline typename intrusive_list<T>::const_reverse_iterator intrusive_list<
  T>::rbegin() const EA_NOEXCEPT
{
  return const_reverse_iterator(
    const_iterator(static_cast<const T*>(&mAnchor)));
}

template <typename T>
inline typename intrusive_list<T>::const_reverse_iterator intrusive_list<
  T>::crbegin() const EA_NOEXCEPT
{
  return const_reverse_iterator(
    const_iterator(static_cast<const T*>(&mAnchor)));
}
...
template <typename T>
inline typename intrusive_list<T>::const_reverse_iterator intrusive_list<
  T>::rend() const EA_NOEXCEPT
{
  return const_reverse_iterator(
    const_iterator(static_cast<const T*>(mAnchor.mpNext)));
}

template <typename T>
inline typename intrusive_list<T>::const_reverse_iterator intrusive_list<
  T>::crend() const EA_NOEXCEPT
{
  return const_reverse_iterator(
    const_iterator(static_cast<const T*>(mAnchor.mpNext)));
}
...
{
  for (const intrusive_list_node* p = mAnchor.mpNext; p != &mAnchor;
       p = p->mpNext)
  {
    if (p == &x)
      return true;
...
template <typename T>
inline typename intrusive_list<T>::iterator intrusive_list<T>::locate(
  value_type& x)
{
  for (intrusive_list_node* p = (T*)mAnchor.mpNext; p != &mAnchor;
       p = p->mpNext)
  {
    if (p == &x)
...
template <typename T>
inline typename intrusive_list<T>::const_iterator intrusive_list<T>::locate(
...
inline typename intrusive_list<T>::iterator intrusive_list<T>::insert(
  const_iterator pos, value_type& x)
{
#if EASTL_VALIDATE_INTRUSIVE_LIST && EASTL_ASSERT_ENABLED
  if (x.mpNext || x.mpPrev)
...
template <typename T>
inline typename intrusive_list<T>::reverse_iterator intrusive_list<T>::erase(
...
void intrusive_list<T>::splice(const_iterator pos, intrusive_list& x)
{
  ...
  if (x.mAnchor.mpNext != &x.mAnchor) // If the list 'x' isn't empty...
  {
    intrusive_list_node& next = *const_cast<node_type*>(pos.mpNode);
    intrusive_list_node& prev = *static_cast<node_type*>(next.mpPrev);
    intrusive_list_node& insertPrev =
      *static_cast<node_type*>(x.mAnchor.mpNext);
    intrusive_list_node& insertNext =
      *static_cast<node_type*>(x.mAnchor.mpPrev);
                               ...
                               const_iterator i)
{
  ...
  iterator ii(
    const_cast<node_type*>(i.mpNode)); // Make a temporary non-const version.
                               ...
                               const_iterator first, const_iterator last)
{
  ...
  if (first != last) {
    ...
    intrusive_list_node& insertNext =
      *static_cast<node_type*>(last.mpNode->mpPrev);
...
void intrusive_list<T>::sort()
{
  ...
  if ((static_cast<node_type*>(mAnchor.mpNext) != &mAnchor) &&
      (static_cast<node_type*>(mAnchor.mpNext) !=
       static_cast<node_type*>(mAnchor.mpPrev)))
  {
