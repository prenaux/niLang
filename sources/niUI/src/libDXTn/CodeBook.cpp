...
{
  ni::tU32 *lpSrc, *lpDest;

  lpSrc = (ni::tU32 *)pData;
  lpDest = (ni::tU32 *)Vect.pData;
  ...
  if(Count)
  {
    ...
    while(pNode)
    {
      for(i=0; i<fCodeSize; i++)
        pVect->GetPtr()[i] = (ni::tU8)FtoL(pNode->V[i]);
...
void ImgCodeBook::FromImage(Image *pImg, Color *pForceColor)
{
  long    Size;
  Color   *pPix, *pDest;
  long    x, y;
...
void ImgCodeBook::FromImageUnique(Image *pImg, Color* pForceColor)
{
  long    Size;
  Color   *pPix, *pDest = NULL;
  long    x, y;
  long    Width, Height;
  cbVector  V, f;
