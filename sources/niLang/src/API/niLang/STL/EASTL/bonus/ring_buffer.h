...
///
  ...
  #define EASTL_RING_BUFFER_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX            \
      " ring_buffer" // Unless the user overrides something, this is "EASTL ring_buffer".
#endif

/// EASTL_RING_BUFFER_DEFAULT_ALLOCATOR
///
#ifndef EASTL_RING_BUFFER_DEFAULT_ALLOCATOR
  #define EASTL_RING_BUFFER_DEFAULT_ALLOCATOR \
    allocator_type(EASTL_RING_BUFFER_DEFAULT_NAME)
#endif
...
class ring_buffer {
 ...
 public
  ...
  Container
    c; // We follow the naming convention established for stack, queue, priority_queue and name this 'c'. This variable must always have a size of at least 1, as even an empty ring_buffer has an unused terminating element.

 protected:
  container_iterator
    mBegin; // We keep track of where our begin and end are by using Container iterators.
 ...
 public:
  ...
  ring_buffer(
    ...
    const allocator_type& allocator =
      EASTL_RING_BUFFER_DEFAULT_ALLOCATOR); // This function sets the capacity to be equal to the size of the initializer list.
  ...
  void set_capacity(
    size_type
      n); // Sets the capacity to the given value, including values less than the current capacity. Adjusts the size downward if n < size, by throwing out the oldest elements in the buffer.
  void reserve(
    size_type
      n); // Reserve a given capacity. Doesn't decrease the capacity; it only increases it (for compatibility with other containers' behavior).
...
template <typename T, typename Pointer, typename Reference, typename Container>
ring_buffer_iterator<T, Pointer, Reference, Container>& ring_buffer_iterator<
  T, Pointer, Reference, Container>::operator=(const iterator& x)
{
  mpContainer = x.mpContainer;
  mContainerIterator = x.mContainerIterator;
  return *this;
...
{
  typedef
    typename eastl::iterator_traits<container_iterator>::iterator_category IC;
  increment(n, IC());
  return *this;
}
...
void ring_buffer_iterator<T, Pointer, Reference, Container>::increment(
  difference_type n, EASTL_ITC_NS::input_iterator_tag)
{
...
template <typename T, typename Pointer, typename Reference, typename Container>
void ring_buffer_iterator<T, Pointer, Reference, Container>::increment(
  difference_type n, EASTL_ITC_NS::random_access_iterator_tag)
{
...
template <typename T, typename Pointer, typename Reference, typename Container>
inline ring_buffer_iterator<T, Pointer, Reference, Container> operator+(
  ptrdiff_t n, const ring_buffer_iterator<T, Pointer, Reference, Container>& x)
{
...
inline bool operator==(
  const ring_buffer_iterator<T, PointerA, ReferenceA, ContainerA>& a,
...
template <typename T, typename Pointer, typename Reference, typename Container>
inline bool operator!=(
  const ring_buffer_iterator<T, Pointer, Reference, Container>& a,
    ...
    : c(x.c)
{
  ...
  eastl::advance(
    mBegin,
    eastl::distance(
      const_cast<this_type&>(x).c.begin(),
      x.mBegin)); // We can do a simple distance algorithm here, as there will be no wraparound.
  eastl::advance(mEnd,
                 eastl::distance(const_cast<this_type&>(x).c.begin(), x.mEnd));
}
    ...
    : c() // Default construction with default allocator for the container.
{
  ...
  swap(
    x); // We are leaving x in an unusual state by swapping default-initialized members with it, as it won't be usable and can be only destructible.
}
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::this_type& ring_buffer<
  T, Container, Allocator>::operator=(const this_type& x)
{
  if (&x != this) {
    c = x.c;
    ...
    eastl::advance(
      ...
      eastl::distance(
        ...
        x.mBegin)); // We can do a simple distance algorithm here, as there will be no wraparound.
    eastl::advance(
      mEnd, eastl::distance(const_cast<this_type&>(x).c.begin(), x.mEnd));
  }
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::this_type& ring_buffer<
  T, Container, Allocator>::operator=(this_type&& x)
{
  swap(x);
  return *this;
}
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::this_type& ring_buffer<
  T, Container, Allocator>::operator=(std::initializer_list<value_type> ilist)
{
  assign(ilist.begin(), ilist.end());
  return *this;
}
...
{
  if (&x != this) {
    const difference_type dBegin = eastl::distance(
      c.begin(),
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<
  T, Container, Allocator>::begin() EA_NOEXCEPT
{
  return iterator(&c, mBegin);
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<
  T, Container, Allocator>::begin() const EA_NOEXCEPT
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<
  T, Container, Allocator>::cbegin() const EA_NOEXCEPT
{
  return const_iterator(
    const_cast<Container*>(&c),
    mBegin); // We trust that the const_iterator will respect const-ness.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<
  T, Container, Allocator>::end() EA_NOEXCEPT
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<
  T, Container, Allocator>::end() const EA_NOEXCEPT
{
  return const_iterator(
    const_cast<Container*>(&c),
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_iterator ring_buffer<
  T, Container, Allocator>::cend() const EA_NOEXCEPT
{
  return const_iterator(
    const_cast<Container*>(&c),
    mEnd); // We trust that the const_iterator will respect const-ness.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<
  T, Container, Allocator>::rend() EA_NOEXCEPT
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::size_type ring_buffer<
  T, Container, Allocator>::size() const EA_NOEXCEPT
{
  return mSize;
...
template <typename T>
struct has_overflow_allocator<
  T, void_t<decltype(declval<T>().get_overflow_allocator())>> : true_type {};
...
template <typename Container,
          bool UseOverflowAllocator = has_overflow_allocator<Container>()()>
...
//
template <typename Container,
          bool UseHeapTemporary = (sizeof(Container) >= EASTL_MAX_STACK_USAGE)>
struct ContainerTemporary {
  Container mContainer;

  ContainerTemporary(Container& parentContainer)
      : mContainer(Internal::GetFixedContainerCtorAllocator<Container>{}(
          parentContainer))
  {
  }
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::size_type ring_buffer<
  T, Container, Allocator>::capacity() const EA_NOEXCEPT
{
  EASTL_ASSERT(
    c.size() >=
    1); // This is required because even an empty ring_buffer has one unused termination element, somewhat like a \0 at the end of a C string.
...
void ring_buffer<T, Container, Allocator>::set_capacity(size_type n)
{
  ...
  if (n != capacity) // If we need to change capacity...
  {
    ...
    eastl::copy(
      itCopyBegin, end(),
      cTemp.get()
        .begin()); // The begin-end range may in fact be larger than n, in which case values will be overwritten.
    eastl::swap(c, cTemp.get());
...
void ring_buffer<T, Container, Allocator>::reserve(size_type n)
{
  ...
  if (
    n >
    (c.size() -
     1)) // If we need to grow in capacity... // (c.size() - 1) == capacity(); we are attempting to reduce function calls.
  {
    ContainerTemporary<Container> cTemp(c);
    cTemp.get().resize(n + 1);
    eastl::copy(begin(), end(), cTemp.get().begin());
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<
  T, Container, Allocator>::front()
{
  return *mBegin;
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reference ring_buffer<
  T, Container, Allocator>::front() const
{
  return *mBegin;
}
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<
  T, Container, Allocator>::back()
{
  // return *(end() - 1); // Can't use this because not all iterators support operator-.

  iterator temp(
    end()); // To do: Find a way to construct this temporary in the return statement.
  return *(
    --temp); // We can do it by making all our containers' iterators support operator-.
}

template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reference ring_buffer<
  T, Container, Allocator>::back() const
{
  // return *(end() - 1); // Can't use this because not all iterators support operator-.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<
  T, Container, Allocator>::push_back()
{
...
void ring_buffer<T, Container, Allocator>::pop_back()
{
  EASTL_ASSERT(
    mEnd !=
    mBegin); // We assume that size() > 0 and thus that there is something to pop.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<
  T, Container, Allocator>::push_front()
{
  if (EASTL_UNLIKELY(mBegin == c.begin()))
    mBegin = c.end();
...
void ring_buffer<T, Container, Allocator>::pop_front()
{
  EASTL_ASSERT(
    mBegin !=
    mEnd); // We assume that mEnd > mBegin and thus that there is something to pop.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reference ring_buffer<
  T, Container, Allocator>::operator[](size_type n)
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::const_reference ring_buffer<
  T, Container, Allocator>::operator[](size_type n) const
{
  // return *(begin() + n); // Can't use this because not all iterators support operator+.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<
  T, Container, Allocator>::insert(const_iterator position,
                                   const value_type& value)
{
  ...
  iterator itPosition(
    ...
    position
      .mContainerIterator); // We merely copy from const_iterator to iterator.
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<
  T, Container, Allocator>::erase(const_iterator position)
{
  iterator itPosition(
    position.mpContainer,
    position
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::iterator ring_buffer<
  T, Container, Allocator>::erase(const_iterator first, const_iterator last)
{
  iterator itFirst(
    first.mpContainer,
    first
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<
  T, Container, Allocator>::erase(const_reverse_iterator position)
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::reverse_iterator ring_buffer<
  T, Container, Allocator>::erase(const_reverse_iterator first,
                                  const_reverse_iterator last)
{
...
template <typename T, typename Container, typename Allocator>
typename ring_buffer<T, Container, Allocator>::container_type& ring_buffer<
  T, Container, Allocator>::get_container()
{
  return c;
}
...
{
  if (
    !c.validate()) // This requires that the container implement the validate function. That pretty much
    return false; // means that the container is an EASTL container and not a std STL container.

  if (
    c.empty()) // c must always have a size of at least 1, as even an empty ring_buffer has an unused terminating element.
    return false;

  if (size() > capacity())
    return false;

  if ((validate_iterator(begin()) & (isf_valid | isf_current)) !=
      (isf_valid | isf_current))
    return false;

  if ((validate_iterator(end()) & (isf_valid | isf_current)) !=
      (isf_valid | isf_current))
    return false;
...
template <typename T, typename Container, typename Allocator>
inline int ring_buffer<T, Container, Allocator>::validate_iterator(
