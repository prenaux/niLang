...
#undef GetPrevSibling
  ...
  #define TBSTYLE_EX_HIDECLIPPEDBUTTONS \
    0x00000010 // don't show partially obscured buttons
#endif
...
inline BOOL SendMessageW(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
...
*/
  ...
  #ifndef _MT
    #error \
      "Linking with multithreaded CRT required when WINUI_THREAD_SUPPORT defined."
  #endif
#else
...
#define WINUI_RELEASE(x)                     \
  {                                          \
    if (x) {                                 \
      niAssert(!IsBadReadPtr(x, sizeof(x))); \
      (x)->Release();                        \
      (x) = NULL;                            \
    }                                        \
  }
...
#define WINUI_BEGIN_DLG_MSG_MAP()                                 \
  virtual INT_PTR DlgProc(HWND hWnd, UINT message, WPARAM wParam, \
                          LPARAM lParam)                          \
  {                                                               \
    niAssert(m_hWnd == NULL || hWnd == m_hWnd);                   \
    m_hWnd = hWnd;                                                \
...
case __id: return __handler(wmEvent, lParam);
...
#define WINUI_COMMAND_BEGIN_MENU_HANDLER() \
  if (lParam == 0 && wmEvent == 0) {       \
    switch (wmId) {
#define WINUI_COMMAND_MENU_HANLDER(__id, __handler) \
case __id: return __handler(lParam);
...
case __message: return __handler(message, wParam, lParam);
#define WINUI_NOTIFY_HANDLER(__wmId, __handler) \
case __wmId: return __handler(wmNMHDR->code);
#define WINUI_NOTIFY_HANDLER_EX(__wmId, __handler) \
case __wmId: return __handler(wmNMHDR);

#define WINUI_COMMAND_OKCANCEL_HANDLERS()                                \
  if (wmId == IDOK &&                                                    \
      (lParam == 0 || lParam == (LPARAM)GetDlgItem(m_hWnd, IDOK)) &&     \
      wmEvent == 0)                                                      \
  {                                                                      \
    EndDialog(m_hWnd, 1);                                                \
    return 1;                                                            \
  ...
  if (wmId == IDCANCEL &&                                                \
      (lParam == 0 || lParam == (LPARAM)GetDlgItem(m_hWnd, IDCANCEL)) && \
      wmEvent == 0)                                                      \
  {                                                                      \
    EndDialog(m_hWnd, 0);                                                \
    return 1;                                                            \
...
class Win32Exception : public Exception {
 public:
  Win32Exception()
  {
    m_strErrorDesc = FormatLastWin32Err();
  };
};
...
*/
  #define WINUI_CHECK_LAST_ERR                     \
    {                                              \
      WinUI::CheckLastErr(_T(__FILE__), __LINE__); \
    }
...
WINUI_INL void CheckLastErr(const TCHAR* pszFile, int iLine)
{
  ...
  ::MessageBoxA(GetActiveWindow(), pszMessage, _T("WinUI Framework Error"),
                MB_ICONERROR);
...
{
  return GetOS() >= eWindowsNT351;
}

//! \brief Computes '(Col1 + Col2) / 2' with two 0xRRGGBBAA colors
WINUI_INL DWORD AverageRGBA(DWORD dwCol1, DWORD dwCol2)
{
  return (DWORD)(((((dwCol1 & 0xFF000000) >> 24) +
                   ((dwCol2 & 0xFF000000) >> 24)) >>
                  1)
                 << 24) |
         (((((dwCol1 & 0x00FF0000) >> 16) + ((dwCol2 & 0x00FF0000) >> 16)) >> 1)
          << 16) |
         (((((dwCol1 & 0x0000FF00) >> 8) + ((dwCol2 & 0x0000FF00) >> 8)) >> 1)
          << 8) |
         (((((dwCol1 & 0x000000FF) >> 0) + ((dwCol2 & 0x000000FF) >> 0)) >> 1)
          << 0);
}
...
WINUI_INL DWORD GetDllVersion(const TCHAR* pszDLLName)
{
  ...
  if (hLibrary != NULL) {
    pfnDLLGetVersion =
      (DLLGETVERSIONPROC)GetProcAddress(hLibrary, "DllGetVersion");
    ...
    if (pfnDLLGetVersion) {
      ...
      if (SUCCEEDED(hResult))
        dwVersion =
          WINUI_CREATE_DLL_VERSION(dvi.dwMajorVersion, dvi.dwMinorVersion);
    }
...
struct Font {
  ...
  {
    hFont = ::CreateFontA(-(int)aiSize, 0, 0, 0, FW_NORMAL, 0, 0, 0, 0, 0, 0, 0,
                          0, aszFace);
  }
...
struct Rect : public RECT {
  ...
  {
    return left == rc.left && right == rc.right && top == rc.top &&
           bottom == rc.bottom;
  };
...
class DC {
 public:
  ...
  {
    HFONT hFont = ::CreateFontA(-(int)iSize, 0, 0, 0, FW_NORMAL, 0, 0, 0, 0, 0,
                                0, 0, 0, pszFace);
    SelectObject(hFont);
    return hFont;
  };
...
class Bitmap {
 public:
...
#ifdef _UNICODE
    HBITMAP hBitmap =
      LoadBitmapW(GetMsWinHInstance(), MAKEINTRESOURCE(iResourceID));
#else
    HBITMAP hBitmap =
      LoadBitmapA(GetMsWinHInstance(), MAKEINTRESOURCE(iResourceID));
#endif
    if (hBitmap == NULL)
      return FALSE;
    Attach(hBitmap);
  ...
  BOOL Blt(HDC hDCDest, UINT iX, UINT iY)
  {
    ...
    if (!BitBlt(hDCDest, iX, iY, GetWidth(), GetHeight(), hDCSrc.GetDC(), 0, 0,
                SRCCOPY))
      return FALSE;
  ...
  {
    BITMAP bmp;
    if (niWin32API(GetObject)((HGDIOBJ)m_hBitmap, sizeof(BITMAP),
                              (LPVOID)&bmp) == 0)
...
class DIB {
 public:
  DIB(HDC hdc = NULL, ni::tU32 anWidth = 0, ni::tU32 anHeight = 0,
      ni::tU32 anBpp = 0)
  {
    if (!hdc || !anBpp || !anWidth || !anHeight)
      return;
  ...
  void Create(HDC hdc, ni::tU32 anWidth, ni::tU32 anHeight, ni::tU32 anBpp)
  {
    ...
    mwinBmp.Attach(CreateDIBSection(hdc, (BITMAPINFO*)&bih, DIB_RGB_COLORS,
                                    (void**)&mpBits, NULL, 0x0));
...
class ImageList {
 public:
  ...
  bool Create(UINT iFlags = ILC_COLOR, UINT iCX = 16, UINT iCY = 16,
              UINT iMaxItems = 32)
  {
  ...
  */
  ...
  {
    niAssert(m_hImgLst == NULL);
    m_hImgLst = ImageList_LoadBitmap(
      GetMsWinHInstance(), MAKEINTRESOURCE(iBitmapID), iCX, 32, 0x00FF00FF);
    if (m_hImgLst == NULL) {
      WINUI_CHECK_LAST_ERR
      return false;
    }
...
#ifdef _UNICODE
    HBITMAP hBmp =
      LoadBitmapW(GetMsWinHInstance(), MAKEINTRESOURCE(dwResource));
#else
    HBITMAP hBmp =
      LoadBitmapA(GetMsWinHInstance(), MAKEINTRESOURCE(dwResource));
#endif
    if (hBmp == NULL) {
      WINUI_CHECK_LAST_ERR
      return false;
  ...
  {
    return ImageList_SetImageCount(m_hImgLst, uNewCount);
  }
  inline int Add(HBITMAP hbmImage, HBITMAP hbmMask)
  {
    return ImageList_Add(m_hImgLst, hbmImage, hbmMask);
  }
  inline int ReplaceIcon(int i, HICON hicon)
  {
    return ImageList_ReplaceIcon(m_hImgLst, i, hicon);
  }
  inline COLORREF SetBkColor(COLORREF clrBk)
  {
    return ImageList_SetBkColor(m_hImgLst, clrBk);
  }
  inline COLORREF GetBkColor() const
  {
    return ImageList_GetBkColor(m_hImgLst);
  }
  inline BOOL SetOverlayImage(int iImage, int iOverlay)
  {
    return ImageList_SetOverlayImage(m_hImgLst, iImage, iOverlay);
  }
  inline BOOL Draw(int i, HDC hdcDst, int x, int y, UINT fStyle) const
  {
    return ImageList_Draw(m_hImgLst, i, hdcDst, x, y, fStyle);
  }
  inline BOOL Replace(int i, HBITMAP hbmImage, HBITMAP hbmMask)
  {
    return ImageList_Replace(m_hImgLst, i, hbmImage, hbmMask);
  }
  inline int AddMasked(HBITMAP hbmImage, COLORREF crMask)
  {
    return ImageList_AddMasked(m_hImgLst, hbmImage, crMask);
  }
  inline BOOL DrawEx(int i, HDC hdcDst, int x, int y, int dx, int dy,
                     COLORREF rgbBk, COLORREF rgbFg, UINT fStyle) const
  {
    return ImageList_DrawEx(m_hImgLst, i, hdcDst, x, y, dx, dy, rgbBk, rgbFg,
                            fStyle);
  }
  ...
  {
    return ImageList_Remove(m_hImgLst, i);
  }
  inline HICON GetIcon(int i, UINT flags) const
  {
    return ImageList_GetIcon(m_hImgLst, i, flags);
  }
  inline BOOL Copy(int iDst, HIMAGELIST himlSrc, int iSrc, UINT uFlags)
  {
    return ImageList_Copy(m_hImgLst, iDst, himlSrc, iSrc, uFlags);
  }
  inline BOOL BeginDrag(int iTrack, int dxHotspot, int dyHotspot)
  {
    return ImageList_BeginDrag(m_hImgLst, iTrack, dxHotspot, dyHotspot);
  }
  inline void EndDrag()
  {
    ImageList_EndDrag();
  }
  inline BOOL DragEnter(HWND hwndLock, int x, int y)
  {
    return ImageList_DragEnter(hwndLock, x, y);
  }
  inline BOOL DragLeave(HWND hwndLock)
  {
  ...
  {
    return ImageList_SetDragCursorImage(m_hImgLst, iDrag, dxHotspot, dyHotspot);
  }
  inline BOOL DragShowNolock(BOOL fShow)
  {
    return ImageList_DragShowNolock(fShow);
  }
  inline HIMAGELIST GetDragImage(POINT FAR* ppt, POINT FAR* pptHotspot)
  {
  ...
  inline HICON ExtractIcon(int i)
  {
    return GetIcon(i, 0);
  }
  inline BOOL GetIconSize(int FAR* cx, int FAR* cy)
  {
    return ImageList_GetIconSize(m_hImgLst, cx, cy);
  }
  inline BOOL SetIconSize(int cx, int cy)
  {
    return ImageList_SetIconSize(m_hImgLst, cx, cy);
  }
  inline BOOL GetImageInfo(int i, IMAGEINFO FAR* pImageInfo)
  {
    return ImageList_GetImageInfo(m_hImgLst, i, pImageInfo);
  }
  inline HIMAGELIST Merge(int i1, HIMAGELIST himl2, int i2, int dx, int dy)
  {
    return ImageList_Merge(m_hImgLst, i1, himl2, i2, dx, dy);
  }
  inline HIMAGELIST Duplicate()
  {
    return ImageList_Duplicate(m_hImgLst);
  }
...
class Registry {
  ...
  */
                 ...
                 DWORD dwDefault = 0, HKEY hBaseKey = HKEY_CURRENT_USER)
  {
    ...
    dwValOut = dwDefault;
    if (RegOpenKeyEx(hBaseKey, pszKey, 0, KEY_ALL_ACCESS, &hKey) !=
        ERROR_SUCCESS)
    {
      RegCloseKey(hKey);
      return FALSE;
    }
    if (RegQueryValueEx(hKey, pszValue, 0, &dwType, (LPBYTE)&dwValOut,
  ...
  BOOL WriteDWORD(LPCTSTR pszKey, LPCTSTR pszValue, DWORD dwValue,
                  HKEY hBaseKey = HKEY_CURRENT_USER)
  {
    HKEY hKey = NULL;
    DWORD dwDisp;
    if (RegCreateKeyEx(hBaseKey, pszKey, 0, NULL, NULL, KEY_ALL_ACCESS, NULL,
                       &hKey, &dwDisp) != ERROR_SUCCESS)
    {
      RegCloseKey(hKey);
      return FALSE;
    }
    if (RegSetValueEx(hKey, pszValue, 0, REG_DWORD, (LPBYTE)&dwValue,
...
class Clipboard {
  ...
  */
  ...
  {
    UINT iClipFormats[] = { CF_UNICODETEXT, CF_TEXT,         CF_OEMTEXT,
                            CF_BITMAP,      CF_METAFILEPICT, CF_SYLK,
                            CF_DIF,         CF_TIFF,         CF_DIB,
                            CF_PALETTE,     CF_PENDATA,      CF_RIFF,
                            CF_WAVE,        CF_ENHMETAFILE,  CF_HDROP,
                            CF_LOCALE,      CF_DIBV5 };

    return ::GetPriorityClipboardFormat(iClipFormats,
                                        sizeof(iClipFormats) / sizeof(UINT));
  };
  ...
  bool ClipboardDataReadLock(HANDLE& hClipData, LPVOID& pLock,
                             SIZE_T& iDataSize, int& iClipFormat)
  {
    if (!m_bClipOpened)
      return false;
...
#define CB_OPEN_IFNOT_END \
  ...
  bool SetClipboardText(const ni::cchar* aaszText, int len = -1)
  {
    ...
    if (!SetClipboardData(CF_UNICODETEXT, (LPVOID)wText.data(),
                          (len + 1) * sizeof(WCHAR)))
      bRet = false;
    CB_OPEN_IFNOT_END
  }
  bool SetClipboardText(const ni::uchar* aaszText, int len = -1)
  {
    CB_OPEN_IFNOT_BEGIN
    if (len < 0)
      len = (int)wcslen(aaszText);
    if (!SetClipboardData(CF_UNICODETEXT, (LPVOID)aaszText,
                          (len + 1) * sizeof(ni::uchar)))
      bRet = false;
    CB_OPEN_IFNOT_END
  }
 ...
 public:
                          ...
                          const Bitmap* pBmpChecked)
  {
    ...
    return ::SetMenuItemBitmaps(m_hMenu, nPosition, nFlags,
                                (HBITMAP)pBmpUnchecked->GetBitmap(),
                                (HBITMAP)pBmpChecked->GetBitmap());
  }
  BOOL LoadMenu(LPCTSTR lpszResourceName)
  {
    m_hMenu = ::LoadMenu(GetMsWinHInstance(), lpszResourceName);
    return m_hMenu != NULL;
  }
  BOOL LoadMenu(UINT nIDResource)
  {
    m_hMenu = ::LoadMenu(GetMsWinHInstance(), MAKEINTRESOURCE(nIDResource));
    return m_hMenu != NULL;
  }
  ...
  BOOL CheckMenuRadioItem(UINT nIDFirst, UINT nIDLast, UINT nIDItem,
                          UINT nFlags)
  {
...
class OpenFile {
 public:
  ...
  bool GetFile(const TCHAR* pszDlgTitle = _T("Open"),
               ...
               const TCHAR* pszDefExt = _T("*.*"), const TCHAR* pszFile = NULL,
               const TCHAR* pszInitDir = NULL, HWND hWndParent = NULL,
               DWORD dwFlags = 0)
  {
    OPENFILENAME ofn;
...
class BrowseForFolder {
 public:
  ...
  BOOL Browse(const TCHAR* pszTitle = _T("Browse"),
              UINT iFlags = BIF_RETURNONLYFSDIRS, HWND hWndOwner = NULL,
...
class BaseWindow : public ni::ImplRC<ni::iUnknown> {
 public:
  ...
  {
    m_bDestroy = TRUE;
    WINUI_VERIFY(Attach(hWnd, bDestroy));
  };
  ...
  */
  ...
  void CenterOnScreen()
  {
    ...
    SetWindowPos(NULL, iScreenCX / 2 - iWndCX / 2, iScreenCY / 2 - iWndCY / 2,
                 iWndCX, iWndCY);
  };
  ...
  virtual BOOL SetWindowPos(HWND hWndAfter, int iX, int iY, int iCX, int iCY,
                            UINT iFlags = SWP_NOZORDER)
  {
    niAssert(IsCreated());
    return ::SetWindowPos(m_hWnd, hWndAfter, iX, iY, iCX, iCY, iFlags);
  };
  virtual BOOL GetWindowRect(Rect& rcWnd)
  {
    niAssert(IsCreated());
    return ::GetWindowRect(m_hWnd, &rcWnd);
  };
  virtual BOOL SetWindowRect(Rect rcWnd)
  {
    niAssert(IsCreated());
    return SetWindowPos(NULL, rcWnd.left, rcWnd.top, rcWnd.right - rcWnd.left,
  ...
  {
    niAssert(IsCreated());
    return niWin32API(SendMessage)(m_hWnd, Msg, wParam, lParam);
  };
  virtual BOOL PostMessage(UINT Msg, WPARAM wParam, LPARAM lParam)
  {
    niAssert(IsCreated());
    return niWin32API(PostMessage)(m_hWnd, Msg, wParam, lParam);
  };
  virtual void SetParent(BaseWindow* pWnd)
  {
    niAssert(IsCreated());
    ::SetParent(m_hWnd, (pWnd != NULL) ? pWnd->GetHWND() : NULL);
  };
  ...
  {
    ::GetWindowText(m_hWnd, aszOut, nMaxChar);
  }
  virtual void SetText(const TCHAR* aszStr)
  {
    ::SetWindowText(m_hWnd, aszStr);
  }
  ...
  {
    niAssert(IsCreated());
    SendMessage(WM_SETFONT, (WPARAM)hFont, MAKELPARAM(bRedraw, 0));
  };
  HFONT GetFont() const
  {
    niAssert(IsCreated());
    return (HFONT)niWin32API(SendMessage)(m_hWnd, WM_GETFONT, 0, 0);
  };
  ...
  BOOL ScrollWindow(int xAmount, int yAmount, Rect* pRect = NULL,
                    Rect* pClipRect = NULL)
  {
    niAssert(IsCreated());
    return ::ScrollWindow(m_hWnd, xAmount, yAmount, pRect, pClipRect);
  };
  int ScrollWindowEx(int dx, int dy, Rect* pRectScroll, Rect* pRectClip,
                     HRGN hRgnUpdate, Rect* pRectUpdate, UINT uFlags)
  {
    niAssert(IsCreated());
    return ::ScrollWindowEx(m_hWnd, dx, dy, pRectScroll, pRectClip, hRgnUpdate,
                            pRectUpdate, uFlags);
  };
  int ScrollWindowEx(int dx, int dy, UINT uFlags, Rect* pRectScroll = NULL,
                     Rect* pRectClip = NULL, HRGN hRgnUpdate = NULL,
  ...
  {
    niAssert(IsCreated());
    return (HICON)niWin32API(SendMessage)(m_hWnd, WM_GETICON, bBigIcon, 0);
  };
  ...
  BOOL SetTopMost()
  {
    return ::SetWindowPos(m_hWnd, HWND_TOPMOST, 0, 0, 0, 0,
...
#ifdef _UNICODE
    m_hWnd =
      ::CreateWindowExW(dwExStyle, lpClassName, lpWindowName, dwStyle, 0, 0,
                        100, 100, (pParent != NULL) ? pParent->GetHWND() : NULL,
                        hMenu, GetMsWinHInstance(), (LPVOID)this);
#else
    m_hWnd =
      ::CreateWindowExA(dwExStyle, lpClassName, lpWindowName, dwStyle, 0, 0,
                        100, 100, (pParent != NULL) ? pParent->GetHWND() : NULL,
                        hMenu, GetMsWinHInstance(), (LPVOID)this);
#endif
    if (m_hWnd == NULL) {
...
*/
WINUI_INL LRESULT CALLBACK GlobalWndProc(HWND hWnd, UINT message, WPARAM wParam,
                                         LPARAM lParam)
{
  LPCREATESTRUCT cs;
  Window* pWnd = NULL;
...
WINUI_INL LRESULT CALLBACK GlobalDlgProc(HWND hWndDialog, UINT uMsg,
...
class Dialog : public BaseWindow {
 ...
 public:
  ...
  Dialog(const TCHAR* pszDialogTemplate)
      : mpszDialogTemplate(pszDialogTemplate){};
  Dialog(UINT anDlg)
      : mpszDialogTemplate(MAKEINTRESOURCE(anDlg)){};
  virtual ~Dialog(){};
  ...
  */
  ...
  {
    m_hWnd = CreateDialogParam(GetMsWinHInstance(), mpszDialogTemplate, hParent,
                               (DLGPROC)GlobalDlgProc, (LPARAM)this);
    if (m_hWnd == NULL) {
      WINUI_CHECK_LAST_ERR
      return FALSE;
    }
  ...
  */
  ...
  {
    m_hWnd = NULL;
    DialogBoxParam(GetMsWinHInstance(), mpszDialogTemplate, hParent,
                   (DLGPROC)GlobalDlgProc, (LPARAM)this);
  ...
  */
  virtual INT_PTR DlgProc(HWND hWndDialog, UINT uMsg, WPARAM wParam,
                          LPARAM lParam)
  {
    return FALSE;
  };
...
class StatusBar : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent,
              ...
              LONG_PTR iID = 0)
  {
    return Window::Create(dwStyle, NULL, STATUSCLASSNAME, NULL, pParent,
                          (UINT)iID);
  };
  ...
  */
  ...
  LRESULT OnSize(UINT message, WPARAM wParam, LPARAM lParam)
  {
    ...
    if (m_pStatusBar && m_pStatusBar->IsCreated()) {
      ...
      m_pStatusBar->SetWindowPos(0, rcClient.bottom - rcStatus.bottom, 0, 0,
...
class ReBar : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent,
              DWORD dwStyle = RBS_BANDBORDERS | RBS_DBLCLKTOGGLE |
                              RBS_REGISTERDROP | RBS_VARHEIGHT | CCS_NODIVIDER |
                              CCS_NOPARENTALIGN | WS_BORDER | WS_CHILD |
...
class ToolBar : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent,
              DWORD dwStyle = WS_CHILD | WS_VISIBLE | CCS_ADJUSTABLE |
                              TBSTYLE_FLAT | TBSTYLE_TOOLTIPS,
              DWORD dwStyleEx = 0, MENUOrID hMenu = (UINT)0)
  {
    BOOL bRet;
    bRet = Window::Create(dwStyle, dwStyleEx, TOOLBARCLASSNAME, NULL, pParent,
                          hMenu);
  ...
  {
    return (INT)SendMessage(TB_LOADIMAGES, (WPARAM)iStdBmpID,
  ...
  BOOL AddButtons(TBBUTTON* pButtons, UINT iNumButtons = 1)
  {
    BOOL bRet;
    bRet =
      (BOOL)SendMessage(TB_ADDBUTTONS, (WPARAM)iNumButtons, (LPARAM)pButtons);
    if (!bRet) {
      WINUI_CHECK_LAST_ERR
      return FALSE;
    }
...
class TabControl : public Window {
 public:
              ...
              DWORD dwStyleEx = 0, MENUOrID hMenu = (UINT)0)
  {
    BOOL bRet;
    bRet =
      Window::Create(dwStyle, dwStyleEx, WC_TABCONTROL, NULL, pParent, hMenu);
    return bRet;
  };
  ...
  BOOL AppendItem(LPCTSTR pszText, UINT iMask = TCIF_TEXT, int iImage = -1,
                  LPARAM lParam = 0)
  {
    TCITEM sItem;
  ...
  {
    return (HIMAGELIST)niWin32API(SendMessage)(m_hWnd, TCM_GETIMAGELIST, 0, 0L);
  }
  inline HIMAGELIST SetImageList(HIMAGELIST himl)
  {
    return (HIMAGELIST)niWin32API(SendMessage)(m_hWnd, TCM_SETIMAGELIST, 0,
                                               (LPARAM)(HIMAGELIST)(himl));
  }
  inline int GetItemCount() const
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_GETITEMCOUNT, 0, 0L);
  }
  inline BOOL GetItem(int iItem, TC_ITEM* pitem) const
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_GETITEM, (WPARAM)iItem,
                                         (LPARAM)(TC_ITEM FAR*)(pitem));
  }
  inline BOOL SetItem(int iItem, const TC_ITEM* pitem)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_SETITEM, (WPARAM)iItem,
                                         (LPARAM)(TC_ITEM FAR*)(pitem));
  }
  inline int InsertItem(int iItem, const TC_ITEM* pitem)
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_INSERTITEM, (WPARAM)iItem,
                                        (LPARAM)(const TC_ITEM FAR*)(pitem));
  }
  inline BOOL DeleteItem(int i)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_DELETEITEM, (WPARAM)(i),
                                         0L);
  }
  inline BOOL DeleteAllItems()
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_DELETEALLITEMS, 0, 0L);
  }
  inline BOOL GetItemRect(int i, RECT* prc) const
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_GETITEMRECT, (WPARAM)(i),
                                         (LPARAM)(RECT FAR*)(prc));
  }
  inline int GetCurSel() const
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_GETCURSEL, 0, 0);
  }
  inline int SetCurSel(int i)
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_SETCURSEL, (WPARAM)i, 0);
  }
  inline int HitTest(TC_HITTESTINFO* pinfo)
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_HITTEST, 0,
                                        (LPARAM)(TC_HITTESTINFO FAR*)(pinfo));
  }
  inline BOOL SetItemExtra(void* cb)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_SETITEMEXTRA, (WPARAM)(cb),
                                         0L);
  }
  inline int AdjustRect(BOOL bLarger, RECT* prc)
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_ADJUSTRECT, (WPARAM)bLarger,
                                        (LPARAM)(RECT FAR*)prc);
  }
  inline DWORD SetItemSize(int x, int y)
  {
    return (DWORD)niWin32API(SendMessage)(m_hWnd, TCM_SETITEMSIZE, 0,
                                          MAKELPARAM(x, y));
  }
  inline void RemoveImage(int i)
  {
    (void)niWin32API(SendMessage)(m_hWnd, TCM_REMOVEIMAGE, i, 0L);
  }
  inline void SetPadding(int cx, int cy)
  {
    (void)niWin32API(SendMessage)(m_hWnd, TCM_SETPADDING, 0,
                                  MAKELPARAM(cx, cy));
  }
  inline int GetRowCount() const
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_GETROWCOUNT, 0, 0L);
  }
  inline HWND GetToolTips() const
  {
    return (HWND)niWin32API(SendMessage)(m_hWnd, TCM_GETTOOLTIPS, 0, 0L);
  }
  inline void SetToolTips(HWND hwndTT)
  {
    (void)niWin32API(SendMessage)(m_hWnd, TCM_SETTOOLTIPS, (WPARAM)hwndTT, 0L);
  }
  inline int GetCurFocus() const
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_GETCURFOCUS, 0, 0);
  }
  inline int SetCurFocus(int i)
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_SETCURFOCUS, i, 0);
  }
  inline int SetMinTabWidth(int x)
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TCM_SETMINTABWIDTH, 0, x);
  }
  inline void DeselectAll(UINT fExcludeFocus)
  {
    (void)niWin32API(SendMessage)(m_hWnd, TCM_DESELECTALL, fExcludeFocus, 0);
  }
  inline BOOL HighlightItem(int i, UINT fHighlight)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_HIGHLIGHTITEM, (WPARAM)i,
                                         (LPARAM)MAKELONG(fHighlight, 0));
  }
  inline DWORD SetExtendedStyle(UINT dw)
  {
    return (DWORD)niWin32API(SendMessage)(m_hWnd, TCM_SETEXTENDEDSTYLE, 0, dw);
  }
  inline DWORD GetExtendedStyle() const
  {
    return (DWORD)niWin32API(SendMessage)(m_hWnd, TCM_GETEXTENDEDSTYLE, 0, 0);
  }
  inline BOOL SetUnicodeFormat(UINT fUnicode)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_SETUNICODEFORMAT,
                                         (WPARAM)(fUnicode), 0);
  }
  inline BOOL GetUnicodeFormat() const
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TCM_GETUNICODEFORMAT, 0, 0);
  }
 ...
 protected:
  ...
  LRESULT OnChangeCBChain(UINT message, WPARAM wParam, LPARAM lParam)
  {
    ...
    if (m_hWndChainNext != NULL && hWndRemove != m_hWndChainNext)
      niWin32API(SendMessage)(m_hWndChainNext, WM_CHANGECBCHAIN, wParam,
                              lParam);
  ...
  */
  ...
  BOOL Reshape(UINT iCX, UINT iCY)
  {
    ...
    if (m_bVertical) {
      ...
      m_pSecondPan->SetWindowPos(0, iEndUpper + iHlfSplitterWdh * 2, 0, 0,
                                 SWP_NOZORDER | SWP_NOSIZE);
    }
    else {
      ...
      m_pSecondPan->SetWindowPos(NULL, iEndLeft + iHlfSplitterWdh * 2, 0, 0, 0,
                                 SWP_NOZORDER | SWP_NOSIZE);
    }
...
class NotifyIcon : public Window {
  ...
  */
  BOOL Create(int iIconResourceID, Menu* pContextMenu = NULL)
  {
    ...
    m_sIcon.hIcon =
      LoadIcon(GetMsWinHInstance(), MAKEINTRESOURCE(iIconResourceID));
  ...
  /*! \brief Shows a ballon tooltip
    \return TRUE in case of success
  */
  BOOL ShowBalloon(const TCHAR* pszText, const TCHAR* pszTitle, UINT iTimeOut,
  ...
  NOTIFYICONDATA GetIconData() const
  {
    return m_sIcon;
  };
 ...
 protected:
  ...
  enum {
    _WMWINUI_NOTIFY_ICON = WM_USER + 1
  };
  ...
  */
  virtual LRESULT OnNotifyIcon(UINT message, WPARAM wParam, LPARAM lParam)
  {
    ...
    case WM_RBUTTONUP:
      if (m_pContextMenu) {
        iSelectionID = m_pContextMenu->TrackPopupMenu(ptCursor.x, ptCursor.y,
                                                      TPM_RETURNCMD, GetHWND());
        return OnContextMenuAction(iSelectionID);
      }
      break;

    case WM_LBUTTONDBLCLK: return OnIconDblClick(); break;

    case NIN_BALLOONUSERCLICK: return OnBalloonClick(); break;
...
class Static : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent, DWORD dwStyle = WS_VISIBLE | WS_CHILD,
              LONG_PTR iID = 0, DWORD dwExStyle = NULL,
              LPCTSTR lpWindowName = NULL)
  {
    return Window::Create(dwStyle, dwExStyle, _T("STATIC"), lpWindowName,
                          pParent, (HMENU)iID);
  };

  HICON SetIcon(HICON hIcon)
  {
    return ((HICON)niWin32API(SendMessage)(m_hWnd, STM_SETICON,
                                           (WPARAM)(HICON)(hIcon), 0L));
  }
  HICON GetIcon() const
  {
    return ((HICON)niWin32API(SendMessage)(m_hWnd, STM_GETICON, 0L, 0L));
  }
...
class Button : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent, DWORD dwStyle = WS_VISIBLE | WS_CHILD,
              ...
              LPCTSTR lpWindowName = NULL)
  {
    return Window::Create(dwStyle, dwExStyle, _T("BUTTON"), lpWindowName,
                          pParent, (HMENU)iID);
  };

  inline BOOL IsChecked() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, BM_GETCHECK, 0L, 0L));
  }
  inline ULONG GetCheck() const
  {
    return ((ULONG)niWin32API(SendMessage)(m_hWnd, BM_GETCHECK, 0L, 0L));
  }
  inline void SetCheck(BOOL bChecked)
  {
    ((void)niWin32API(SendMessage)(m_hWnd, BM_SETCHECK, (WPARAM)(bChecked),
                                   0L));
  }

  inline ULONG GetState() const
  {
    return ((ULONG)(DWORD)niWin32API(SendMessage)(m_hWnd, BM_GETSTATE, 0L, 0L));
  }
  inline void SetState(ULONG aState)
  {
    ((void)niWin32API(SendMessage)(m_hWnd, BM_SETSTATE, (WPARAM)(aState), 0L));
  }

  inline void SetStyle(ULONG aStyle, BOOL bRedraw = TRUE)
  {
    ((void)niWin32API(SendMessage)(m_hWnd, BM_SETSTYLE, (WPARAM)LOWORD(aStyle),
...
class ComboBox : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent, DWORD dwStyle = WS_VISIBLE | WS_CHILD,
              ...
              LPCTSTR lpWindowName = NULL)
  {
    return Window::Create(dwStyle, dwExStyle, _T("COMBOBOX"), lpWindowName,
                          pParent, (HMENU)iID);
  };

  inline int LimitText(int cchLimit)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_LIMITTEXT,
                                         (WPARAM)(cchLimit), 0L));
  }
  inline int GetEditSel() const
  {
    return ((DWORD)niWin32API(SendMessage)(m_hWnd, CB_GETEDITSEL, 0L, 0L));
  }
  inline int SetEditSel(int ichStart, int ichEnd)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_SETEDITSEL, 0L,
                                         MAKELPARAM((ichStart), (ichEnd))));
  }
  inline int GetCount() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_GETCOUNT, 0L, 0L));
  }
  inline int ResetContent()
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_RESETCONTENT, 0L, 0L));
  }
  inline int AddString(LPCTSTR lpsz)
  {
    return (
      (int)niWin32API(SendMessage)(m_hWnd, CB_ADDSTRING, 0L, (LPARAM)(lpsz)));
  }
  inline int InsertString(int index, LPCTSTR lpsz)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_INSERTSTRING,
                                         (WPARAM)(index), (LPARAM)(lpsz)));
  }
  inline int AddItemData(void* data)
  {
    return (
      (int)niWin32API(SendMessage)(m_hWnd, CB_ADDSTRING, 0L, (LPARAM)(data)));
  }
  inline int InsertItemData(int index, void* data)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_INSERTSTRING,
                                         (WPARAM)(index), (LPARAM)(data)));
  }
  inline int DeleteString(int index)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_DELETESTRING,
                                         (WPARAM)(index), 0L));
  }
  inline int GetLBTextLen(int index) const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_GETLBTEXTLEN,
                                         (WPARAM)(index), 0L));
  }
  inline int GetLBText(int index, LPTSTR lpszBuffer) const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_GETLBTEXT, (WPARAM)(index),
                                         (LPARAM)(LPTSTR)(lpszBuffer)));
  }
  ...
  inline void* GetItemData(int index) const
  {
    return ((void*)niWin32API(SendMessage)(m_hWnd, CB_GETITEMDATA,
                                           (WPARAM)(index), 0L));
  }
  inline int SetItemData(int index, void* data)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_SETITEMDATA,
                                         (WPARAM)(index), (LPARAM)(data)));
  }
  inline int FindString(int indexStart, LPCTSTR lpszFind) const
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, CB_FINDSTRING, (WPARAM)(indexStart), (LPARAM)(lpszFind)));
  }
  inline int FindItemData(int indexStart, void* data) const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_FINDSTRING,
                                         (WPARAM)(indexStart), (LPARAM)(data)));
  }
  inline int GetCurSel() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_GETCURSEL, 0L, 0L));
  }
  inline int SetCurSel(int index)
  {
    return (
      (int)niWin32API(SendMessage)(m_hWnd, CB_SETCURSEL, (WPARAM)(index), 0L));
  }
  inline int SelectString(int indexStart, LPCTSTR lpszSelect)
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, CB_SELECTSTRING, (WPARAM)(indexStart), (LPARAM)(lpszSelect)));
  }
  inline int SelectItemData(int indexStart, void* data)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_SELECTSTRING,
                                         (WPARAM)(indexStart), (LPARAM)(data)));
  }
  inline int Dir(UINT attrs, LPCTSTR lpszFileSpec)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_DIR, (WPARAM)(attrs),
                                         (LPARAM)(lpszFileSpec)));
  }
  inline int ShowDropdown(UINT fShow)
  {
    return ((BOOL)niWin32API(SendMessage)(m_hWnd, CB_SHOWDROPDOWN,
                                          (WPARAM)(fShow), 0L));
  }
  inline int FindStringExact(int indexStart, LPCTSTR lpszFind) const
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, CB_FINDSTRINGEXACT, (WPARAM)(indexStart), (LPARAM)(lpszFind)));
  }
  inline int GetDroppedState() const
  {
    return ((BOOL)niWin32API(SendMessage)(m_hWnd, CB_GETDROPPEDSTATE, 0L, 0L));
  }
  inline void GetDroppedControlRect(RECT* lprc)
  {
    ((void)niWin32API(SendMessage)(m_hWnd, CB_GETDROPPEDCONTROLRECT, 0L,
                                   (LPARAM)(lprc)));
  }
  inline int GetItemHeight() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_GETITEMHEIGHT, 0L, 0L));
  }
  inline int SetItemHeight(int index, int cyItem)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_SETITEMHEIGHT,
                                         (WPARAM)(index), (LPARAM)cyItem));
  }
  inline int GetExtendedUI() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_GETEXTENDEDUI, 0L, 0L));
  }
  inline int SetExtendedUI(UINT flags)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, CB_SETEXTENDEDUI,
                                         (WPARAM)(flags), 0L));
  }
...
class Header : public Window {
 public:
  Header(){};
  Header(HWND hWnd, BOOL bDestroy)
      : Window(hWnd, bDestroy){};
  virtual ~Header(){};

  BOOL Create(DWORD dwStyle = WS_VISIBLE | WS_CHILD | LVS_REPORT,
              Window* pParent = NULL, LONG_PTR iID = 0, DWORD dwExStyle = NULL,
              LPCTSTR lpWindowName = NULL)
  {
    return Window::Create(dwStyle, dwExStyle, WC_HEADER, lpWindowName, pParent,
                          (HMENU)iID);
  };
  ...
  int GetItemCount() const
  {
    ...
    return (int)niWin32API(SendMessage)(m_hWnd, HDM_GETITEMCOUNT, 0, 0L);
  }
  int InsertItem(int nPos, HDITEM* phdi)
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, HDM_INSERTITEM, nPos,
                                        (LPARAM)phdi);
  }
  BOOL DeleteItem(int nPos)
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, HDM_DELETEITEM, nPos, 0L);
  }
  BOOL GetItem(int nPos, HDITEM* pHeaderItem) const
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, HDM_GETITEM, nPos,
                                         (LPARAM)pHeaderItem);
  }
  BOOL SetItem(int nPos, HDITEM* pHeaderItem)
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, HDM_SETITEM, nPos,
                                         (LPARAM)pHeaderItem);
  }
  BOOL Layout(HDLAYOUT* pHeaderLayout)
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, HDM_LAYOUT, 0,
                                         (LPARAM)pHeaderLayout);
  }
  BOOL SetOrderArray(int iCount, LPINT piArray)
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, HDM_SETORDERARRAY,
  ...
  BOOL GetOrderArray(LPINT piArray, int iCount)
  {
    ...
    return (BOOL)niWin32API(SendMessage)(m_hWnd, HDM_GETORDERARRAY,
...
class ListBox : public Window {
 public:
  ...
  BOOL Create(DWORD dwStyle = WS_VISIBLE | WS_CHILD | LVS_REPORT,
              ...
              LPCTSTR lpWindowName = NULL)
  {
    return Window::Create(dwStyle, dwExStyle, WC_LISTVIEW, lpWindowName,
                          pParent, (HMENU)iID);
  };
  ...
  bool InsertColumn(UINT iColIdx, const TCHAR* pszColumnHeading, int nFormat,
                    INT iWidth = -1, INT aSubItem = -1)
  {
    LVCOLUMN col;
    col.mask = LVCF_TEXT;
  ...
  inline int GetCount() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_GETCOUNT, 0L, 0L));
  }
  //! Reset the content of the list box.
  inline void ResetContent()
  {
    niWin32API(SendMessage)(m_hWnd, LB_RESETCONTENT, 0L, 0L);
  }

  inline int InsertItem(UINT nMask, int nItem, LPCTSTR lpszItem, UINT nState,
  ...
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, LVM_INSERTITEM, 0,
                                        (LPARAM)pItem);
  }
  ...
  {
    niAssert(::IsWindow(m_hWnd));
    return InsertItem(LVIF_PARAM, nItem, NULL, 0, 0, 0, (LPARAM)pData);
  }
  ...
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_DELETESTRING,
                                         (WPARAM)(index), 0L));
  }

  inline int GetTextLen(int index) const
  {
    return (
      (int)niWin32API(SendMessage)(m_hWnd, LB_GETTEXTLEN, (WPARAM)(index), 0L));
  }
  inline int GetText(int index, const TCHAR* lpszBuffer) const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_GETTEXT, (WPARAM)(index),
                                         (LPARAM)(lpszBuffer)));
  }
  inline int FindString(int indexStart, const TCHAR* lpszFind)
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, LB_FINDSTRING, (WPARAM)(indexStart), (LPARAM)(lpszFind)));
  }
  inline int FindItemData(int indexStart, void* data)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_FINDSTRING,
                                         (WPARAM)(indexStart), (LPARAM)(data)));
  }
  inline int SetSel(int fSelect, int index)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_SETSEL, (WPARAM)(fSelect),
                                         (LPARAM)(index)));
  }
  inline int SelItemRange(int fSelect, int first, int last)
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, LB_SELITEMRANGE, (WPARAM)(fSelect), MAKELPARAM((first), (last))));
  }
  inline int GetCurSel()
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LVM_GETSELECTIONMARK, 0L, 0L));
  }
  inline int SetCurSel(int index)
  {
    return (
      (int)niWin32API(SendMessage)(m_hWnd, LB_SETCURSEL, (WPARAM)(index), 0L));
  }
  inline int GetSel(int index) const
  {
    return (
      (int)niWin32API(SendMessage)(m_hWnd, LB_GETSEL, (WPARAM)(index), 0L));
  }
  inline int GetSelCount() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_GETSELCOUNT, 0L, 0L));
  }
  inline int GetTopIndex() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_GETTOPINDEX, 0L, 0L));
  }
  inline int GetSelItems(int cItems, int* lpItems) const
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, LB_GETSELITEMS, (WPARAM)(cItems), (LPARAM)(int*)(lpItems)));
  }
  inline int SetTopIndex(int indexTop)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_SETTOPINDEX,
                                         (WPARAM)(indexTop), 0L));
  }
  inline void SetColumnWidth(int cxColumn)
  {
    ((void)niWin32API(SendMessage)(m_hWnd, LB_SETCOLUMNWIDTH,
                                   (WPARAM)(cxColumn), 0L));
  }
  inline int GetHorizontalExtent() const
  {
    return (
      (int)niWin32API(SendMessage)(m_hWnd, LB_GETHORIZONTALEXTENT, 0L, 0L));
  }
  inline void SetHorizontalExtent(int cxExtent)
  {
    ((void)niWin32API(SendMessage)(m_hWnd, LB_SETHORIZONTALEXTENT,
                                   (WPARAM)(cxExtent), 0L));
  }
  inline int SetTabStops(int cTabs, int* lpTabs)
  {
    return ((BOOL)niWin32API(SendMessage)(
      m_hWnd, LB_SETTABSTOPS, (WPARAM)(cTabs), (LPARAM)(int*)(lpTabs)));
  }
  inline int GetItemRect(int index, RECT* lprc)
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, LB_GETITEMRECT, (WPARAM)(index), (LPARAM)(RECT*)(lprc)));
  }
  inline int SetCaretIndex(int index)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_SETCARETINDEX,
                                         (WPARAM)(index), 0L));
  }
  inline int GetCaretIndex() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_GETCARETINDEX, 0L, 0L));
  }
  inline int FindStringExact(int indexStart, const TCHAR* lpszFind)
  {
    return ((int)niWin32API(SendMessage)(
      m_hWnd, LB_FINDSTRINGEXACT, (WPARAM)(indexStart), (LPARAM)(lpszFind)));
  }
  inline int SetItemHeight(int index, int cy)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_SETITEMHEIGHT,
                                         (WPARAM)(index), MAKELPARAM((cy), 0)));
  }
  inline int GetItemHeight(int index) const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_GETITEMHEIGHT,
                                         (WPARAM)(index), 0L));
  }
  inline int Dir(UINT attrs, const TCHAR* lpszFileSpec)
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, LB_DIR, (WPARAM)(attrs),
                                         (LPARAM)(lpszFileSpec)));
  }
  inline BOOL DeleteColumn(int nCol)
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, LVM_DELETECOLUMN, nCol, 0);
  }
  inline int GetColumnWidth(int nCol) const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, LVM_GETCOLUMNWIDTH, nCol, 0);
  }

  inline int GetItemCount() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, LVM_GETITEMCOUNT, 0, 0L);
  }
  inline BOOL DeleteAllItems()
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, LVM_DELETEALLITEMS, 0, 0L);
  }
  inline void SetItemCount(int nItems)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, LVM_SETITEMCOUNT, nItems, 0);
  }
  inline BOOL SortItems(PFNLVCOMPARE pfnCompare, DWORD_PTR dwData)
  {
    niAssert(::IsWindow(m_hWnd));
    niAssert((GetStyle() & LVS_OWNERDATA) == 0);
    return (BOOL)niWin32API(SendMessage)(m_hWnd, LVM_SORTITEMS, dwData,
                                         (LPARAM)pfnCompare);
  }
  inline BOOL SetItemState(int nItem, LVITEM* pItem)
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, LVM_SETITEMSTATE, nItem,
                                         (LPARAM)pItem);
  }
  inline UINT GetItemState(int nItem, UINT nMask) const
  {
    niAssert(::IsWindow(m_hWnd));
    return (UINT)niWin32API(SendMessage)(m_hWnd, LVM_GETITEMSTATE, nItem,
                                         nMask);
  }
                      ...
                      int nImage, UINT nState, UINT nStateMask, LPARAM lParam)
  {
    ...
    return SetItem(nItem, nSubItem, nMask, lpszItem, nImage, nState, nStateMask,
                   lParam, 0);
  }
  ...
  inline BOOL SetItemText(int nItem, int nSubItem, LPCTSTR lpszText)
  {
    ...
    return (BOOL)niWin32API(SendMessage)(m_hWnd, LVM_SETITEMTEXT, nItem,
                                         (LPARAM)&lvi);
  }
  ...
  inline ni::cString GetItemText(int nItem, int nSubItem) const
  {
    ...
    do {
      ...
      nRes = (int)niWin32API(SendMessage)(m_hWnd, LVM_GETITEMTEXT,
                                          (WPARAM)nItem, (LPARAM)&lvi);
    } while (nRes == nLen - 1);
    return str;
  }

  inline int GetItemText(int nItem, int nSubItem, LPTSTR lpszText,
                         int nLen) const
  {
    niAssert(::IsWindow(m_hWnd));
    LVITEM lvi;
    ...
    return (int)niWin32API(SendMessage)(m_hWnd, LVM_GETITEMTEXT, (WPARAM)nItem,
                                        (LPARAM)&lvi);
  }
  ...
  {
    niAssert(::IsWindow(m_hWnd));
    return SetItem(nItem, 0, LVIF_PARAM, NULL, 0, 0, 0, (LPARAM)pData);
  }
...
class TreeView : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent,
              DWORD dwStyle = WS_VISIBLE | WS_CHILD | TVS_HASLINES |
                              TVS_HASBUTTONS,
              LONG_PTR iID = 0, DWORD dwExStyle = NULL,
              LPCTSTR lpWindowName = NULL)
  {
    return Window::Create(dwStyle, dwExStyle, WC_TREEVIEW, lpWindowName,
                          pParent, (HMENU)iID);
  };
  ...
  */
  ...
  BOOL SetItemText(HTREEITEM hItem, const TCHAR* aszText,
  ...
  BOOL GetItemText(HTREEITEM hItem, ni::achar* aaszBuffer,
                   ni::tI32 anMaxChars) const
  {
    TVITEM item;
    memset(&item, 0, sizeof(TVITEM));
    item.mask = TVIF_TEXT;
  ...
  ni::cString GetItemText(HTREEITEM hItem) const
  {
    ...
    if (!GetItemText(hItem, aszText, 4096))
      return ni::cString();
    return aszText;
  }
  ...
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_DELETEITEM, 0,
  ...
  inline BOOL Expand(HTREEITEM hitem, UINT code)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_EXPAND, (WPARAM)code,
                                         (LPARAM)(HTREEITEM)(hitem));
  }
  inline BOOL GetItemRect(HTREEITEM hitem, RECT* prc, UINT code) const
  {
    return (*(HTREEITEM FAR*)prc = (hitem),
            (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_GETITEMRECT,
                                          (WPARAM)(code),
                                          (LPARAM)(RECT FAR*)(prc)));
  }
  inline UINT GetCount() const
  {
    return (UINT)niWin32API(SendMessage)(m_hWnd, TVM_GETCOUNT, 0, 0);
  }
  inline UINT GetIndent() const
  {
    return (UINT)niWin32API(SendMessage)(m_hWnd, TVM_GETINDENT, 0, 0);
  }
  inline BOOL SetIndent(int indent)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_SETINDENT, (WPARAM)indent,
                                         0);
  }
  inline HIMAGELIST GetImageList(int iImage)
  {
    return (HIMAGELIST)niWin32API(SendMessage)(m_hWnd, TVM_GETIMAGELIST, iImage,
                                               0);
  }
  inline HIMAGELIST SetImageList(HIMAGELIST himl, int iImage)
  {
    return (HIMAGELIST)niWin32API(SendMessage)(m_hWnd, TVM_SETIMAGELIST, iImage,
                                               (LPARAM)(HIMAGELIST)(himl));
  }
  inline HTREEITEM GetNextItem(HIMAGELIST hitem, UINT code) const
  {
    return (HTREEITEM)niWin32API(SendMessage)(
      m_hWnd, TVM_GETNEXTITEM, (WPARAM)code, (LPARAM)(HTREEITEM)(hitem));
  }
  inline HTREEITEM GetChild(HIMAGELIST hitem) const
  {
    return GetNextItem(hitem, TVGN_CHILD);
  }
  inline HTREEITEM GetNextSibling(HIMAGELIST hitem) const
  {
    return GetNextItem(hitem, TVGN_NEXT);
  }
  inline HTREEITEM GetPrevSibling(HIMAGELIST hitem) const
  {
    return GetNextItem(hitem, TVGN_PREVIOUS);
  }
  inline HTREEITEM GetParent(HIMAGELIST hitem) const
  {
  ...
  inline HTREEITEM GetNextVisible(HIMAGELIST hitem) const
  {
    return GetNextItem(hitem, TVGN_NEXTVISIBLE);
  }
  inline HTREEITEM GetPrevVisible(HIMAGELIST hitem) const
  {
  ...
  inline BOOL Select(HIMAGELIST hitem, UINT code)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_SELECTITEM, (WPARAM)code,
                                         (LPARAM)(HTREEITEM)(hitem));
  }
  inline BOOL SelectItem(HIMAGELIST hitem)
  {
    return Select(hitem, TVGN_CARET);
  }
  inline BOOL SelectDropTarget(HIMAGELIST hitem)
  {
    return Select(hitem, TVGN_DROPHILITE);
  }
  inline BOOL SelectSetFirstVisible(HIMAGELIST hitem)
  {
    return Select(hitem, TVGN_FIRSTVISIBLE);
  }
  inline BOOL GetItem(TV_ITEM* pitem) const
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_GETITEM, 0,
                                         (LPARAM)(TV_ITEM FAR*)(pitem));
  }
  inline BOOL SetItem(const TV_ITEM* pitem)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_SETITEM, 0,
                                         (LPARAM)(const TV_ITEM FAR*)(pitem));
  }
  inline HWND EditLabel(HTREEITEM hitem)
  {
    return (HWND)niWin32API(SendMessage)(m_hWnd, TVM_EDITLABEL, 0,
                                         (LPARAM)(HTREEITEM)(hitem));
  }
  inline HWND GetEditControl() const
  {
    return (HWND)niWin32API(SendMessage)(m_hWnd, TVM_GETEDITCONTROL, 0, 0);
  }
  inline UINT GetVisibleCount() const
  {
    return (UINT)niWin32API(SendMessage)(m_hWnd, TVM_GETVISIBLECOUNT, 0, 0);
  }
  inline HTREEITEM HitTest(TV_HITTESTINFO* pHitInfos)
  {
    return (HTREEITEM)niWin32API(SendMessage)(
      m_hWnd, TVM_HITTEST, 0, (LPARAM)(LPTV_HITTESTINFO)(pHitInfos));
  }
  inline HIMAGELIST CreateDragImage(HTREEITEM hitem)
  {
    return (HIMAGELIST)niWin32API(SendMessage)(m_hWnd, TVM_CREATEDRAGIMAGE, 0,
                                               (LPARAM)(HTREEITEM)(hitem));
  }
  inline BOOL SortChildren(HTREEITEM hitem, BOOL recurse)
  {
    return (BOOL)niWin32API(SendMessage)(
      m_hWnd, TVM_SORTCHILDREN, (WPARAM)recurse, (LPARAM)(HTREEITEM)(hitem));
  }
  inline BOOL EnsureVisible(HTREEITEM hitem)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_ENSUREVISIBLE, 0,
                                         (LPARAM)(HTREEITEM)(hitem));
  }
  inline BOOL SortChildrenCB(TV_SORTCB* psort, BOOL recurse)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_SORTCHILDRENCB,
                                         (WPARAM)recurse,
                                         (LPARAM)(LPTV_SORTCB)(psort));
  }
  inline BOOL EndEditLabelNow(UINT fCancel)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_ENDEDITLABELNOW,
                                         (WPARAM)fCancel, 0);
  }
  inline HWND SetToolTips(HWND hwndTT)
  {
    return (HWND)niWin32API(SendMessage)(m_hWnd, TVM_SETTOOLTIPS,
                                         (WPARAM)hwndTT, 0);
  }
  inline HWND GetToolTips() const
  {
    return (HWND)niWin32API(SendMessage)(m_hWnd, TVM_GETTOOLTIPS, 0, 0);
  }
  inline BOOL GetISearchString(HWND hwndTV, const TCHAR* lpsz) const
  {
    return (BOOL)niWin32API(SendMessage)((hwndTV), TVM_GETISEARCHSTRING, 0,
                                         (LPARAM)(LPTSTR)lpsz);
  }
  inline BOOL SetInsertMark(HTREEITEM hItem, UINT fAfter)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_SETINSERTMARK,
                                         (WPARAM)(fAfter), (LPARAM)(hItem));
  }
  inline BOOL SetUnicodeFormat(UINT fUnicode)
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_SETUNICODEFORMAT,
                                         (WPARAM)(fUnicode), 0);
  }
  inline BOOL GetUnicodeFormat() const
  {
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TVM_GETUNICODEFORMAT, 0, 0);
  }
  inline int SetItemHeight(int iHeight)
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TVM_SETITEMHEIGHT,
                                        (WPARAM)iHeight, 0);
  }
  inline int GetItemHeight() const
  {
    return (int)niWin32API(SendMessage)(m_hWnd, TVM_GETITEMHEIGHT, 0, 0);
  }
  inline COLORREF SetBkColor(COLORREF clr)
  {
    return (COLORREF)niWin32API(SendMessage)(m_hWnd, TVM_SETBKCOLOR, 0,
                                             (LPARAM)clr);
  }
  inline COLORREF SetTextColor(COLORREF clr)
  {
    return (COLORREF)niWin32API(SendMessage)(m_hWnd, TVM_SETTEXTCOLOR, 0,
                                             (LPARAM)clr);
  }
  inline COLORREF GetBkColor() const
  {
    return (COLORREF)niWin32API(SendMessage)(m_hWnd, TVM_GETBKCOLOR, 0, 0);
  }
  inline COLORREF GetTextColor() const
  {
    return (COLORREF)niWin32API(SendMessage)(m_hWnd, TVM_GETTEXTCOLOR, 0, 0);
  }
  inline UINT SetScrollTime(UINT uTime)
  {
    return (UINT)niWin32API(SendMessage)(m_hWnd, TVM_SETSCROLLTIME, uTime, 0);
  }
  inline UINT GetScrollTime()
  {
    return (UINT)niWin32API(SendMessage)(m_hWnd, TVM_GETSCROLLTIME, 0, 0);
  }
  inline COLORREF SetInsertMarkColor(COLORREF clr)
  {
    return (COLORREF)niWin32API(SendMessage)(m_hWnd, TVM_SETINSERTMARKCOLOR, 0,
                                             (LPARAM)clr);
  }
  inline COLORREF GetInsertMarkColor() const
  {
    return (COLORREF)niWin32API(SendMessage)(m_hWnd, TVM_GETINSERTMARKCOLOR, 0,
                                             0);
  }
...
class ProgressBar : public Window {
 public:
  ...
  {
    ((void)niWin32API(SendMessage)(m_hWnd, PBM_SETPOS, (WPARAM)(pos), 0L));
  };
  inline int GetPos() const
  {
    ((int)niWin32API(SendMessage)(m_hWnd, PBM_GETPOS, 0L, 0L));
  }

  inline void SetRange(int min, int max)
  {
    ((void)niWin32API(SendMessage)(m_hWnd, PBM_SETRANGE, 0L,
                                   MAKELPARAM(min, max)));
  };
  inline int GetRangeMin() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, PBM_GETRANGE, TRUE, 0L));
  }
  inline int GetRangeMax() const
  {
    return ((int)niWin32API(SendMessage)(m_hWnd, PBM_GETRANGE, FALSE, 0L));
  }
...
class Slider : public Window {
 public:
  ...
  BOOL Create(DWORD dwStyle = WS_VISIBLE | WS_CHILD, Window* pParent = NULL,
              ...
              LPCTSTR lpWindowName = NULL)
  {
    BOOL bSuccess = Window::Create(dwStyle, dwExStyle, TRACKBAR_CLASS,
                                   lpWindowName, pParent, (HMENU)iID);
    if (!bSuccess)
      return FALSE;
    return TRUE;
  ...
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_GETLINESIZE, 0, 0l);
  }
  int SetLineSize(int nSize)
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_SETLINESIZE, 0, nSize);
  }
  int GetPageSize() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_GETPAGESIZE, 0, 0l);
  }
  int SetPageSize(int nSize)
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_SETPAGESIZE, 0, nSize);
  }
  int GetRangeMax() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_GETRANGEMAX, 0, 0l);
  }
  int GetRangeMin() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_GETRANGEMIN, 0, 0l);
  }
  void SetRangeMin(int nMin, BOOL bRedraw)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_SETRANGEMIN, bRedraw, nMin);
  }
  void SetRangeMax(int nMax, BOOL bRedraw)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_SETRANGEMAX, bRedraw, nMax);
  }
  void ClearSel(BOOL bRedraw)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_CLEARSEL, bRedraw, 0l);
  }
  void GetChannelRect(LPRECT lprc) const
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_GETCHANNELRECT, 0, (LPARAM)lprc);
  }
  void GetThumbRect(LPRECT lprc) const
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_GETTHUMBRECT, 0, (LPARAM)lprc);
  }
  int GetPos() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_GETPOS, 0, 0l);
  }
  void SetPos(int nPos)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_SETPOS, TRUE, nPos);
  }
  void ClearTics(BOOL bRedraw)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_CLEARTICS, bRedraw, 0l);
  }
  UINT GetNumTics() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (UINT)niWin32API(SendMessage)(m_hWnd, TBM_GETNUMTICS, 0, 0l);
  }
  DWORD* GetTicArray() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (DWORD*)niWin32API(SendMessage)(m_hWnd, TBM_GETPTICS, 0, 0l);
  }
  int GetTic(int nTic) const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_GETTIC, nTic, 0L);
  }
  int GetTicPos(int nTic) const
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_GETTICPOS, nTic, 0L);
  }
  BOOL SetTic(int nTic)
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, TBM_SETTIC, 0, nTic);
  }
  void SetTicFreq(int nFreq)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_SETTICFREQ, nFreq, 0L);
  }
  HWND GetBuddy(BOOL fLocation) const
  {
    niAssert(::IsWindow(m_hWnd));
    return (HWND)niWin32API(SendMessage)(m_hWnd, TBM_GETBUDDY, fLocation, 0l);
  }
  HWND SetBuddy(HWND hWndBuddy, BOOL fLocation)
  {
    niAssert(::IsWindow(m_hWnd));
    return (HWND)niWin32API(SendMessage)(m_hWnd, TBM_SETBUDDY, fLocation,
                                         (LPARAM)hWndBuddy);
  }
  HWND GetToolTips() const
  {
    niAssert(::IsWindow(m_hWnd));
    return (HWND)niWin32API(SendMessage)(m_hWnd, TBM_GETTOOLTIPS, 0, 0L);
  }
  void SetToolTips(HWND hWnd)
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, TBM_SETTOOLTIPS, (WPARAM)hWnd, 0L);
  }
  int SetTipSide(int nLocation)
  {
    niAssert(::IsWindow(m_hWnd));
    return (int)niWin32API(SendMessage)(m_hWnd, TBM_SETTIPSIDE, nLocation, 0L);
  }
...
class Edit : public Window {
 public:
  ...
  BOOL Create(BaseWindow* pParent, DWORD dwStyle = WS_VISIBLE | WS_CHILD,
              ...
              LPCTSTR lpWindowName = NULL)
  {
    return Window::Create(dwStyle, dwExStyle, _T("EDIT"), lpWindowName, pParent,
                          (HMENU)iID);
  };
  ...
  {
    return (UINT)niWin32API(SendMessage)(m_hWnd, EM_GETSEL, (WPARAM)apBeg,
                                         (LPARAM)apEnd);
  };
  ...
  {
    niAssert(::IsWindow(m_hWnd));
    return (BOOL)niWin32API(SendMessage)(m_hWnd, EM_UNDO, 0, 0);
  }
  void Clear()
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, WM_CLEAR, 0, 0);
  }
  void Copy()
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, WM_COPY, 0, 0);
  }
  void Cut()
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, WM_CUT, 0, 0);
  }
  void Paste()
  {
    niAssert(::IsWindow(m_hWnd));
    niWin32API(SendMessage)(m_hWnd, WM_PASTE, 0, 0);
  }
  ...
  {
    niAssert(IsCreated());
    return niWin32API(SendMessage)(m_hWnd, EM_SETREADONLY, bEnable, 0);
  };
...
WINUI_INL BOOL InitCommonControls()
{
  ...
  ctrls.dwICC = ICC_ANIMATE_CLASS | ICC_BAR_CLASSES | ICC_COOL_CLASSES |
                ICC_DATE_CLASSES | ICC_HOTKEY_CLASS | ICC_INTERNET_CLASSES |
                ICC_LISTVIEW_CLASSES | ICC_PAGESCROLLER_CLASS |
...
struct cReg {
  ...
  bool LoadBinary(LPCTSTR pszSubKey, LPCTSTR pszValName, LPBYTE pbyteData,
                  DWORD dwSize);
  ...
  bool LoadNewBinary(LPCTSTR pszSubKey, LPCTSTR pszValName, LPBYTE* pbyteData,
                     DWORD* pdwSize);
  ...
  bool LoadString(LPCTSTR pszSubKey, LPCTSTR pszValName, LPTSTR pszString,
                  DWORD dwLength);
  ...
  bool LoadNewString(LPCTSTR pszSubKey, LPCTSTR pszValName, LPTSTR* pszString,
                     DWORD* pdwLength);
  ...
  bool LoadStringArr(LPCTSTR pszSubKey, LPCTSTR pszValName, LPTSTR pszStrings[],
                     DWORD dwCount);

  /* load a new array of strings */
  bool LoadNewStringArr(LPCTSTR pszValName, LPTSTR** pszStrings,
                        DWORD* pdwCount);

  /* load a new array of strings from subkey */
  bool LoadNewStringArr(LPCTSTR pszSubKey, LPCTSTR pszValName,
                        LPTSTR** pszStrings, DWORD* pdwCount);
  ...
  bool SaveBinary(LPCTSTR pszSubKey, LPCTSTR pszValName, const LPBYTE pbyteData,
                  DWORD dwSize);
  ...
  bool SaveStringArr(LPCTSTR pszValName, const LPCTSTR pszStrings[],
                     DWORD dwCount);

  /* store an array of strings to subkey */
  bool SaveStringArr(LPCTSTR pszSubKey, LPCTSTR pszValName,
                     const LPCTSTR pszStrings[], DWORD dwCount);
  ...
  bool HasEntries(LPCTSTR pszSubKey, DWORD* pdwSubKeyCount,
                  DWORD* pdwValueCount);
...
/* initializes registry value data */
inline void RegValInit(RegVal* pValData)
{
  niAssert(pValData);
...
/* frees registry value data */
inline void RegValFree(RegVal* pValData)
{
  niAssert(pValData);
...
/* get a number */
inline bool RegValGetNumber(const RegVal* pValData, DWORD* pdwNumber)
{
...
/* get binary data */
inline bool RegValGetBinary(const RegVal* pValData, LPBYTE pbyteData,
                            DWORD dwSize)
{
...
/* get new binary data */
inline bool RegValGetNewBinary(const RegVal* pValData, LPBYTE* pbyteData,
                               DWORD* pdwSize)
{
  niAssert(pValData && pbyteData);
...
/* get a new string */
inline bool RegValGetNewString(const RegVal* pValData, LPTSTR* pszString,
                               DWORD* pdwLength)
{
...
/* get a string */
inline bool RegValGetString(const RegVal* pValData, LPTSTR pszString,
                            DWORD dwLength)
{
  niAssert(pValData && pszString);
  if ((pValData->dwType == REG_SZ || pValData->dwType == REG_EXPAND_SZ ||
...
/* get an array of strings */
inline bool RegValGetStringArr(const RegVal* pValData, LPTSTR pszStrings[],
                               DWORD dwCount)
{
  niAssert(pValData);
  if (pValData->dwType == REG_MULTI_SZ) {
    ...
    if (dwCount >= dwRealCount) {
      ...
      for (pszString = pValData->pszString; *pszString;
           pszString += dwLength, pszDstString++)
      {
        dwLength = _tcslen(pszString) + 1;
...
/* get a new array of strings */
inline bool RegValGetNewStringArr(const RegVal* pValData, LPTSTR** pszStrings,
                                  DWORD* pdwCount)
{
  niAssert(pValData);
  if (pValData->dwType == REG_MULTI_SZ) {
    ...
    if (pszNewStrings) {
      ...
      *pdwCount = (DWORD)dwRealCount;
      for (pszString = pValData->pszString; *pszString;
           pszString += dwLength, pszNewStrings++)
      {
        dwLength = _tcslen(pszString) + 1;
        LPTSTR pszNewString = (LPTSTR)malloc(dwLength);
...
/* set a number */
inline void RegValSetNumber(RegVal* pValData, DWORD dwNumber)
{
  niAssert(pValData);
...
/* set binary data */
inline bool RegValSetBinary(RegVal* pValData, const LPBYTE pbyteData,
                            DWORD dwSize)
{
  niAssert(pValData && pbyteData);
...
/* set a string */
inline bool RegValSetString(RegVal* pValData, LPCTSTR pszString)
{
  niAssert(pValData && pszString);
...
inline bool RegValSetStringArr(RegVal* pValData, const LPCTSTR pszStrings[],
                               DWORD dwCount)
{
...
/* connect to remote computer registry */
inline HKEY RegConnect(HKEY hKey, LPCTSTR pszRemote)
{
  HKEY hSubKey;
...
/* open computer registry */
inline HKEY RegOpen(HKEY hKey, LPCTSTR pszSubKey, DWORD dwRights)
{
  HKEY hSubKey;
...
/* create computer registry */
inline HKEY RegCreate(HKEY hKey, LPCTSTR pszSubKey, DWORD dwRights)
{
  HKEY hSubKey;
  DWORD dwDisposition;
  if (RegCreateKeyEx(hKey, pszSubKey, 0, NULL, REG_OPTION_NON_VOLATILE,
                     dwRights, NULL, &hSubKey, &dwDisposition) == ERROR_SUCCESS)
  {
    return hSubKey;
  }
  return NULL;
...
/* close computer registry */
inline void RegClose(HKEY hKey)
{
  if (hKey)
    RegCloseKey(hKey);
}
...
/* load data of any type */
inline bool RegLoadVal(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                       RegVal* pValData)
{
  niAssert(pValData);
  ...
  if (hSubKey) {
    DWORD dwType, dwSize;
    if (RegQueryValueEx(hSubKey, pszValName, 0, &dwType, NULL, &dwSize) ==
        ERROR_SUCCESS)
    {
      if (dwType == REG_DWORD) {
        niAssert(dwSize == sizeof(DWORD));
        DWORD dwNumber;
        if (RegQueryValueEx(hSubKey, pszValName, 0, NULL, (LPBYTE)&dwNumber,
                            &dwSize) == ERROR_SUCCESS)
        {
          niAssert(dwSize == sizeof(DWORD));
          RegValFree(pValData);
          ...
          return true;
               ...
               dwType == REG_LINK || dwType == REG_MULTI_SZ ||
               dwType == REG_BINARY)
      {
        LPBYTE pbyteData = (LPBYTE)malloc(dwSize);
        if (pbyteData) {
          if (RegQueryValueEx(hSubKey, pszValName, 0, NULL, pbyteData,
                              &dwSize) == ERROR_SUCCESS)
          {
            RegValFree(pValData);
            pValData->dwType = dwType;
...
/* load a number */
inline bool RegLoadNumber(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                          DWORD* pdwNumber)
{
  niAssert(pdwNumber);
  HKEY hSubKey = pszSubKey ? RegOpen(hKey, pszSubKey, KEY_READ) : hKey;
  if (hSubKey) {
        ...
        ERROR_SUCCESS)
    {
      if (dwType == REG_DWORD) {
        niAssert(dwSize == sizeof(DWORD));
        if (RegQueryValueEx(hSubKey, pszValName, 0, NULL, (LPBYTE)pdwNumber,
                            &dwSize) == ERROR_SUCCESS)
        {
          niAssert(dwSize == sizeof(DWORD));
          if (hSubKey != hKey)
...
/* load binary data */
inline bool RegLoadBinary(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                          LPBYTE pbyteData, DWORD dwSize)
{
  ...
  if (hSubKey) {
    ...
    if (RegQueryValueEx(hSubKey, pszValName, 0, &dwType, NULL, &dwRealSize) ==
        ERROR_SUCCESS)
    {
      if (dwType == REG_BINARY && dwSize >= dwRealSize) {
        if (RegQueryValueEx(hSubKey, pszValName, 0, NULL, pbyteData,
                            &dwRealSize) == ERROR_SUCCESS)
        {
          if (hSubKey != hKey)
            RegClose(hSubKey);
          return true;
...
/* load new binary data */
inline bool RegLoadNewBinary(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                             LPBYTE* pbyteData, DWORD* pdwSize)
{
  niAssert(pbyteData);
  HKEY hSubKey = pszSubKey ? RegOpen(hKey, pszSubKey, KEY_READ) : hKey;
  if (hSubKey) {
        ...
        ERROR_SUCCESS)
    {
      if (dwType == REG_BINARY) {
        ...
        if (pbyteNewData) {
          if (RegQueryValueEx(hSubKey, pszValName, 0, NULL, pbyteNewData,
                              &dwRealSize) == ERROR_SUCCESS)
          {
            *pbyteData = pbyteNewData;
            *pdwSize = dwRealSize;
...
/* load a string */
inline bool RegLoadString(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                          LPTSTR pszString, DWORD dwLength)
{
  ...
  if (hSubKey) {
    ...
    if (RegQueryValueEx(hSubKey, pszValName, 0, &dwType, NULL, &dwRealLength) ==
        ERROR_SUCCESS)
    {
      ...
      {
        if (RegQueryValueEx(hSubKey, pszValName, 0, NULL, (LPBYTE)pszString,
                            &dwRealLength) == ERROR_SUCCESS)
        {
          if (hSubKey != hKey)
            RegClose(hSubKey);
          return true;
...
/* load a new string */
inline bool RegLoadNewString(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
  ...
  if (hSubKey) {
        ...
        ERROR_SUCCESS)
    {
          ...
          dwType == REG_MULTI_SZ)
      {
        ...
        if (pszNewString) {
          if (RegQueryValueEx(hSubKey, pszValName, 0, NULL,
                              (LPBYTE)pszNewString,
                              &dwRealLength) == ERROR_SUCCESS)
          {
            *pszString = pszNewString;
...
/* load an array of strings */
inline bool RegLoadStringArr(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                             LPTSTR pszStrings[], DWORD dwCount)
{
  RegVal Value;
...
/* load a new array of strings */
inline bool RegLoadNewStringArr(HKEY hKey, LPCTSTR pszSubKey,
                                LPCTSTR pszValName, LPTSTR** pszStrings,
                                DWORD* pdwCount)
{
  RegVal Value;
  if (RegLoadVal(hKey, pszSubKey, pszValName, &Value)) {
...
/* store data of any type */
inline bool RegSaveVal(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                       const RegVal* pValData)
{
  ...
  if (hSubKey) {
    LONG lResult;
    if (pValData->dwType == REG_DWORD) {
      lResult = RegSetValueEx(hSubKey, pszValName, 0, pValData->dwType,
                              (LPBYTE)&pValData->dwNumber, sizeof(DWORD));
    }
    ...
    {
      lResult = RegSetValueEx(hSubKey, pszValName, 0, pValData->dwType,
                              pValData->pbyteData, pValData->dwSize);
    }
    else {
...
/* store a number */
inline bool RegSaveNumber(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                          DWORD dwNumber)
{
  HKEY hSubKey = pszSubKey ? RegCreate(hKey, pszSubKey, KEY_WRITE) : hKey;
  if (hSubKey) {
    if (RegSetValueEx(hSubKey, pszValName, 0, REG_DWORD, (LPBYTE)&dwNumber,
                      sizeof(DWORD)) == ERROR_SUCCESS)
    {
      if (hSubKey != hKey)
        RegClose(hSubKey);
...
/* store binary data */
inline bool RegSaveBinary(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                          const LPBYTE pbyteData, DWORD dwSize)
{
  HKEY hSubKey = pszSubKey ? RegCreate(hKey, pszSubKey, KEY_WRITE) : hKey;
  if (hSubKey) {
    if (RegSetValueEx(hSubKey, pszValName, 0, REG_BINARY, pbyteData, dwSize) ==
        ERROR_SUCCESS)
    {
      if (hSubKey != hKey)
        RegClose(hSubKey);
...
/* store a string */
inline bool RegSaveString(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                          LPCTSTR pszString)
{
  HKEY hSubKey = pszSubKey ? RegCreate(hKey, pszSubKey, KEY_WRITE) : hKey;
  if (hSubKey) {
    if (RegSetValueEx(hSubKey, pszValName, 0, REG_SZ, (LPBYTE)pszString,
                      (DWORD)(_tcslen(pszString) + 1)) == ERROR_SUCCESS)
    {
      if (hSubKey != hKey)
        RegClose(hSubKey);
      return true;
...
/* store an array of strings */
inline bool RegSaveStringArr(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName,
                             const LPCTSTR pszStrings[], DWORD dwCount)
{
  RegVal Value;
...
inline bool RegDeleteSubKeys(HKEY hKey)
{
  DWORD dwSubKeyCnt, dwMaxSubKey;
  if (RegQueryInfoKey(hKey, NULL, NULL, 0, &dwSubKeyCnt, &dwMaxSubKey, NULL,
                      NULL, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
  {
    if (dwSubKeyCnt) {
      ...
      if (pszKeyName) {
        do {
          if (RegEnumKey(hKey, --dwSubKeyCnt, pszKeyName, dwMaxSubKey) ==
              ERROR_SUCCESS)
          {
...
/* delete the given value or key in the registry with all of its subkeys */
inline bool RegDeleteKey(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValName)
{
  HKEY hSubKey =
    pszSubKey ? RegOpen(hKey, pszSubKey, KEY_READ | KEY_WRITE) : hKey;
  if (hSubKey) {
    if (pszValName) {
...
/* check wether the given key has other subkeys and/or values */
inline bool RegHasEntries(HKEY hKey, LPCTSTR pszSubKey, DWORD* pdwSubKeyCount,
                          DWORD* pdwValueCount)
{
  HKEY hSubKey = pszSubKey ? RegOpen(hKey, pszSubKey, KEY_READ) : hKey;
  if (hSubKey) {
    if (RegQueryInfoKey(hSubKey, NULL, NULL, 0, pdwSubKeyCount, NULL, NULL,
                        pdwValueCount, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
    {
      if (hSubKey != hKey)
        RegClose(hSubKey);
      return true;
...
/* load data of any type from subkey */
inline bool cReg::LoadVal(LPCTSTR pszSubKey, LPCTSTR pszValName,
                          RegVal* pValData)
{
...
/* load a number from subkey */
inline bool cReg::LoadNumber(LPCTSTR pszSubKey, LPCTSTR pszValName,
                             DWORD* pdwNumber)
{
...
/* load binary data from subkey */
inline bool cReg::LoadBinary(LPCTSTR pszSubKey, LPCTSTR pszValName,
                             LPBYTE pbyteData, DWORD dwSize)
{
...
/* load new binary data */
inline bool cReg::LoadNewBinary(LPCTSTR pszValName, LPBYTE* pbyteData,
                                DWORD* pdwSize)
{
...
/* load new binary data from subkey */
inline bool cReg::LoadNewBinary(LPCTSTR pszSubKey, LPCTSTR pszValName,
                                LPBYTE* pbyteData, DWORD* pdwSize)
{
...
/* load a string */
inline bool cReg::LoadString(LPCTSTR pszValName, LPTSTR pszString,
                             DWORD dwLength)
{
...
/* load a string from subkey */
inline bool cReg::LoadString(LPCTSTR pszSubKey, LPCTSTR pszValName,
                             LPTSTR pszString, DWORD dwLength)
{
...
/* load a new string */
inline bool cReg::LoadNewString(LPCTSTR pszValName, LPTSTR* pszString,
                                DWORD* pdwLength)
{
...
/* load a new string from subkey */
inline bool cReg::LoadNewString(LPCTSTR pszSubKey, LPCTSTR pszValName,
                                LPTSTR* pszString, DWORD* pdwLength)
{
...
/* load an array of strings */
inline bool cReg::LoadStringArr(LPCTSTR pszValName, LPTSTR pszStrings[],
                                DWORD dwCount)
{
...
/* load an array of strings from subkey */
inline bool cReg::LoadStringArr(LPCTSTR pszSubKey, LPCTSTR pszValName,
                                LPTSTR pszStrings[], DWORD dwCount)
{
...
/* load a new array of strings */
inline bool cReg::LoadNewStringArr(LPCTSTR pszValName, LPTSTR** pszStrings,
                                   DWORD* pdwCount)
{
...
/* load a new array of strings from subkey */
inline bool cReg::LoadNewStringArr(LPCTSTR pszSubKey, LPCTSTR pszValName,
                                   LPTSTR** pszStrings, DWORD* pdwCount)
{
...
inline bool cReg::LoadString(LPCTSTR pszValName, ni::cString& sString)
{
  ...
  if (!RegLoadString(hKey, NULL, pszValName, aaszTmp, 4096))
    return false;
  sString = aaszTmp;
  return true;
}
...
{
  ni::achar aaszTmp[4096];
  if (!RegLoadString(hKey, pszSubKey, pszValName, aaszTmp, 4096))
    return false;
  sString = aaszTmp;
  return true;
}
...
/* store data of any type to subkey */
inline bool cReg::SaveVal(LPCTSTR pszSubKey, LPCTSTR pszValName,
                          const RegVal* pValData)
{
...
/* store a number to subkey */
inline bool cReg::SaveNumber(LPCTSTR pszSubKey, LPCTSTR pszValName,
                             DWORD dwNumber)
{
...
/* store binary data */
inline bool cReg::SaveBinary(LPCTSTR pszValName, const LPBYTE pbyteData,
                             DWORD dwSize)
{
...
/* store binary data to subkey */
inline bool cReg::SaveBinary(LPCTSTR pszSubKey, LPCTSTR pszValName,
                             const LPBYTE pbyteData, DWORD dwSize)
{
...
/* store a string to subkey */
inline bool cReg::SaveString(LPCTSTR pszSubKey, LPCTSTR pszValName,
                             LPCTSTR pszString)
{
...
/* store an array of strings */
inline bool cReg::SaveStringArr(LPCTSTR pszValName, const LPCTSTR pszStrings[],
                                DWORD dwCount)
{
...
/* store an array of strings to subkey */
inline bool cReg::SaveStringArr(LPCTSTR pszSubKey, LPCTSTR pszValName,
                                const LPCTSTR pszStrings[], DWORD dwCount)
{
...
/* check wether the given key has other subkeys and/or values in subkey */
inline bool cReg::HasEntries(LPCTSTR pszSubKey, DWORD* pdwSubKeyCount,
                             DWORD* pdwValueCount)
{
...
///////////////////////////////////////////////
inline BOOL SetClassesRootRegKey(const ni::achar* lpszKey,
                                 ...
                                 const ni::achar* lpszValueName = NULL)
{
  if (lpszValueName == NULL) {
    ...
    if (::RegCreateKey(HKEY_CLASSES_ROOT, lpszKey, &hKey) == ERROR_SUCCESS) {
      LONG lResult =
        ::RegSetValueEx(hKey, lpszValueName, 0, REG_SZ, (CONST BYTE*)lpszValue,
...
///////////////////////////////////////////////
inline const ni::achar* RegisterApp(const ni::achar* aszAppName,
                                    ...
                                    const ni::achar* aszDocDesc)
{
  ...
  ni::StrGetCommandPath(aszExe, niCountOf(aszExe), strCmdLine.Chars(),
                        &lExeLen);
  if (niStringIsOK(aszExe)) {
    ni::cString strBuffer;
    ni::cString strBuffer2;
    SetClassesRootRegKey(strBuffer2.Format(_A(".%s"), aszExt),
                         strBuffer.Format(_A("%s.Document"), aszAppName));
    SetClassesRootRegKey(strBuffer2.Format(_A(".%s\\ShellNew"), aszExt), _A(""),
                         _A("NullFile"));
    ...
    SetClassesRootRegKey(
      strBuffer2.Format(_A("%s.Document\\DefaultIcon"), aszAppName),
      strBuffer.Format(_A("%s,0"), aszExe));

    SetClassesRootRegKey(
      strBuffer2.Format(_A("%s.Document\\shell\\open\\command"), aszAppName),
      strBuffer.Format(_A("%s \"%%1\""), aszExe));
  }
  const ni::achar* pRet = strCmdLine.Chars() + lExeLen;
  while (*pRet && ni::StrIsSpace(*pRet)) {
    ++pRet;
  }
  return pRet;
}
...
class cTextDlg {
 public:
  cTextDlg(HINSTANCE ahInstance, const ni::achar* aaszTitle,
           const ni::achar* aaszText)
  ...
  int DoModal(ni::tBool abTopMost, HWND aParent = nullptr)
  {
    ...
    mDlgWnd = CreateWindowExW(
      ...
      sClassName, wTitle.begin(), WS_CAPTION | WS_SYSMENU | WS_POPUP,
      CW_USEDEFAULT, CW_USEDEFAULT, mWidth, mHeight, // width, height
      aParent, nullptr, mhInstance, this);
    ...
    SetWindowPos(mDlgWnd, nullptr, (screenWidth - width) / 2,
                 (screenHeight - height) / 2, 0, 0, // ignored with SWP_NOSIZE
 ...
 private:
  void CreateControls()
  {
    ...
    mEditWnd = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", wErrorText.begin(),
                               ...
                               mhInstance, nullptr);
    mOldEditProc = (WNDPROC)SetWindowLongPtrW(mEditWnd, GWLP_WNDPROC,
                                              (LONG_PTR)EditSubclassProc);
    SetPropW(mEditWnd, L"DialogPtr", (HANDLE)this);

    // Create OK button
    mOkWnd = CreateWindowExW(0, L"BUTTON", L"OK",
  ...
  static LRESULT CALLBACK _TextDlgProc(HWND hwnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam)
  {
  ...
  LRESULT DlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
  {
    ...
    case WM_SIZE:
      if (mEditWnd && mOkWnd) {
        ...
        // Resize edit control
        SetWindowPos(mEditWnd, nullptr, 10, 10, width - 20, height - 50,
                     SWP_NOZORDER);

        // Reposition OK button
        SetWindowPos(mOkWnd, nullptr, (width - 80) / 2, height - 35, 0, 0,
                     SWP_NOSIZE | SWP_NOZORDER);
      }
      break;
    }
