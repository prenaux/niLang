...
#define niMetaAssert_(n, condition) \
  typedef bool MetaAssert##n[2 * !!(condition)-1]

#define niMetaDeclareIsOfType(TYPE) \
  template <class T>                \
  struct MetaIOT_##TYPE {           \
    enum {                          \
      res = false                   \
    };                              \
  };                                \
  template <>                       \
  struct MetaIOT_##TYPE<TYPE> {     \
    enum {                          \
      res = true                    \
    };                              \
  };
...
#define niMetaSelect(i, T0, T1) \
  ni::MetaSelect<bool(i)>::template Type<T0, T1>::Res
...
class MetaCat {
 ...
 private:
  ...
  niMetaAssert(niMetaIsVoid(T1) ||
               sizeof(Res) == sizeof(T0) + sizeof(CheckedT1));
};
...
#define niMetaConditionalInherit(cnd, B0, B1) \
  ni::MetaConditionalInherit<(cnd), B0, B1>::Res
