...
struct HelloUIWidget : public cWidgetSinkImpl<> {
  ...
  tBool __stdcall OnSinkAttached() niImpl
  {
...
#ifdef TEST_ONPAINT_CANVAS_TEXT
    {
      ...
      _monoFont->SetSizeAndResolution(
        Vec2f(12, 12), 12, mpWidget->GetUIContext()->GetContentsScale());
      _bmpFont = mpWidget->GetGraphics()->LoadFont(_H("8x8"));
      _bmpFont->SetSizeAndResolution(
        Vec2f(16, 16), 8, mpWidget->GetUIContext()->GetContentsScale());
    }
#endif
...
#ifdef TEST_IPCAM
    {
      ...
      if (niIsOK(fp)) {
        Ptr<iVideoDecoder> decoder =
          mpWidget->GetGraphics()->CreateVideoDecoder(
            _H(fp->GetSourcePath()), fp, eVideoDecoderFlags_TargetTexture);
        if (decoder.IsOK()) {
          ...
          niWarning(
            niFmt("Can't create video decoder for '%s'.", fp->GetSourcePath()));
        }
      }
    }
#endif
    ...
    Ptr<iWidget> w = mpWidget->GetUIContext()->CreateWidget(
      "Button", mpWidget, Vec4f(0, 0, 200, 200), eWidgetStyle_HoldFocus,
      _HC(ID_Button));
    ...
    w = mpWidget->GetUIContext()->CreateWidget(
      "EditBox", mpWidget, Vec4f(0, 0, 350, 300),
      eWidgetStyle_HoldFocus | eWidgetEditBoxStyle_MultiLine, _HC(ID_EditBox));
    ...
    bottom_canvas(
      mpWidget, 150, ni::Callback1([](iCanvas* apCanvas) {
        ...
        if (image.IsOK()) {
          ...
          Ptr<iBitmap2D> bmp =
            image->GrabBitmap(eImageUsage_Target, sRecti::Null());
          ...
          // (yellow in canvas)
          apCanvas->BlitFill(Rectf(10, 10, 50, 50),
                             ULColorBuild(Vec3f(1, 1, 0)));
          apCanvas->Flush();
          apCanvas->GetGraphicsContext()->Display(0, sRecti::Zero());
  ...
  void bottom_canvas(iWidget* apWidget, tU32 anHeight, iCallback* apCallback)
  {
    ...
    Ptr<iWidget> w = apWidget->GetUIContext()->CreateWidget(
      "Button", apWidget, Vec4f(0, 0, img->GetWidth(), img->GetHeight()),
      eWidgetStyle_HoldFocus | eWidgetButtonStyle_NoFrame |
        eWidgetButtonStyle_IconCenter | eWidgetButtonStyle_IconFit,
      NULL);
    w->GetFont()->SetSizeAndResolution(
      Vec2f(18, 18), 18, mpWidget->GetUIContext()->GetContentsScale());
  ...
  tBool __stdcall OnCommand(iWidgetCommand* apCmd) niImpl
  {
             ...
             apCmd->GetID() == eWidgetEditBoxCmd_Modified)
    {
      ...
      if (niIsOK(wEB)) {
        QPtr<iWidget> wButton =
          wEB->GetUIContext()->GetRootWidget()->FindWidget(_HC(ID_Button));
        if (niIsOK(wButton)) {
  ...
  tBool __stdcall OnPaint(const sVec2f& avMousePos, iCanvas* apCanvas) niImpl
  {
#ifdef TEST_ONPAINT_CANVAS_TEXT
    {
      ...
      niLoop (i, blitTextLoopCount) {
        apCanvas->BlitFill(Rectf(20, 120, textWidth, textHeight), 0xFF555555);
        apCanvas->BlitText(mpWidget->GetFont(),
                           Rectf(20, 120, textWidth, textHeight), blitTextFlags,
                           _kText);
        apCanvas->Flush();
      }
      ...
      niLoop (i, blitTextLoopCount) {
        ...
        apCanvas->BlitFill(Rectf(r - 12, 20, 12, 12), 0xFF558855);
        const tF32 th = apCanvas
                          ->BlitText(_monoFont, Rectf(x, 20, w, 0),
                                     eFontFormatFlags_Right, _kText)
                          .GetHeight();

        apCanvas->BlitFill(Rectf(r - 16, 20 + th, 16, 16), 0xFF555588);
        apCanvas
          ->BlitText(_bmpFont, Rectf(x, 20 + th, w, 0), eFontFormatFlags_Right,
                     _kText)
          .GetHeight();
    ...
    if (niIsOK(_ipcamOverlay)) {
      ...
      apCanvas->BlitOverlay(
        Rectf(mpWidget->GetSize().x - TEST_IPCAM - 10,
              mpWidget->GetSize().y - (TEST_IPCAM / 4 * 3) - 32, TEST_IPCAM,
              TEST_IPCAM / 4 * 3),
        _ipcamOverlay);
      apCanvas->Flush();
    }
