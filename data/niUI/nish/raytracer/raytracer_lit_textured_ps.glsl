...
nish_std_PixelOutput niUIGpuFuncs_raytracer_lit_textured_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS, sampler aSS);
...
nish_std_RayDesc niUIGpuFuncs_MakeRayDesc(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms) {
  ...
  vec3 _tmp_P4 = dir;
  float _tmp_Q4 = 0.001;
  float _tmp_R4 = aUniforms.cameraFarClipPlane;
  return nish_std_RayDesc_new(_tmp_O4, _tmp_P4, _tmp_Q4, _tmp_R4);
}
niUIGpuFuncs_IntersectionResult niUIGpuFuncs_TraceRay_6_uint_Vec3_Vec3_float_float_RayInstances(uint aRayFlags, vec3 aOrigin, vec3 aDir, float aMinT, float aMaxT, accelerationStructureEXT aAS) {
  /* mut */ rayQueryEXT rayQuery/*__noinit__*/;
  rayQueryInitializeEXT(rayQuery,aAS,aRayFlags,255,aOrigin,aMinT,aDir,aMaxT);
  while(true) {
    bool _tmp_15 = rayQueryProceedEXT(rayQuery);
    if (!(_tmp_15)) {
      break;
    }
    uint candidateType = rayQueryGetIntersectionTypeEXT(rayQuery,false);
    bool _tmp_65 = (candidateType == nish_std_RayQueryCandidateIntersectionType_CandidateTriangle);
    if (_tmp_65) {
  ...
  bool _tmp_e5 = (intersectionType == nish_std_RayQueryIntersectionType_CommittedTriangle);
  if (_tmp_e5) {
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_lit_textured_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS, sampler aSS) {
  ...
  vec4 color;
  bool _tmp_M6 = niUIGpuFuncs__op__gte_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_Triangle);
  if (_tmp_M6) {
    vec3 worldNormal = intersection.worldNormal;
    vec3 worldPos = intersection.worldPos;
    vec3 matColor = (niUIGpuFuncs_RayInstanceData_SampleMaterialDiffuseColor(intersection.instData,aSS,intersection.uv).rgb);
    ...
    vec3 _tmp_s7 = (C*matColor);
    color = vec4(_tmp_s7.x,_tmp_s7.y,_tmp_s7.z,1.0);
  }
  else {
    bool _tmp_v7 = niUIGpuFuncs__op__lt_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_None);
    if (_tmp_v7) {
    ...
    else {
      {
        ...
        vec3 _tmp_J7 = niUIGpuFuncs_ComputeSimpleSkyColor(rayDesc.direction,sunDir);
        color = vec4(_tmp_J7.x,_tmp_J7.y,_tmp_J7.z,0.0);
      }
    }
  }
  vec4 _tmp_P7 = color;
  return nish_std_PixelOutput_new(_tmp_P7);
}
...
// Pixel Shader main: niUIGpuFuncs_raytracer_lit_textured_ps
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = niUIGpuFuncs_raytracer_lit_textured_ps(aInput, aUniforms, IN_1_aAS, IN_1_aSS);
