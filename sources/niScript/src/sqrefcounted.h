...
struct SQCollectable : public ni::iCollectable {
  ...
  inline ni::iUnknown* __stdcall _DoQueryInterface(iUnknown* apBase,
                                                   const ni::tUUID& aIID)
  {
    if (aIID == niGetInterfaceUUID(ni::iCollectable))
      return apBase;
    if (aIID == niGetInterfaceUUID(ni::iUnknown))
      return apBase;
    return NULL;
  }
  virtual void __stdcall ListInterfaces(ni::iMutableCollection* apLst,
                                        tU32) const
  {
  ...
  #define START_MARK()                                     \
    if (!(SQREF_READ(&mprotected_nNumRefs) & MARK_FLAG)) { \
      SQREF_WRITE(&mprotected_nNumRefs,                    \
                  SQREF_READ(&mprotected_nNumRefs) | MARK_FLAG);
