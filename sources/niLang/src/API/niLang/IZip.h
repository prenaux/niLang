...
typedef void*(__cdecl* zlib_alloc_func)(void* opaque,
                                        niNamespace(ni, tU32) items,
                                        niNamespace(ni, tU32) size);
typedef void(__cdecl* zlib_free_func)(void* opaque, void* address);
typedef niNamespace(ni, tU32)(__cdecl* zlib_in_func)(void*,
                                                     niNamespace(ni, tU8)**);
typedef niNamespace(ni, tI32)(__cdecl* zlib_out_func)(void*,
                                                      niNamespace(ni, tU8)*,
                                                      niNamespace(ni, tU32));
...
typedef struct gz_header_s {
  ...
  niNamespace(ni, tI32)
    extra_len; /* extra field length (valid if extra != Z_NULL) */
  niNamespace(ni,
              tI32) extra_max; /* space at extra (only when reading header) */
  niNamespace(ni,
              tU8)* name; /* pointer to zero-terminated file name or Z_NULL */
  niNamespace(ni, tI32) name_max; /* space at name (only when reading header) */
  niNamespace(ni,
              tU8)* comment; /* pointer to zero-terminated comment or Z_NULL */
  niNamespace(ni,
              tI32) comm_max; /* space at comment (only when reading header) */
...
typedef struct unz_file_info_s {
  ...
  niNamespace(ni,
              tU16) size_filename; /* filename length                 2 bytes */
  niNamespace(ni, tU16)
    size_file_extra; /* extra field length              2 bytes */
  niNamespace(ni, tU16)
    size_file_comment; /* file comment length             2 bytes */
  niNamespace(ni, tU16)
    disk_num_start; /* disk number start               2 bytes */
  niNamespace(ni,
              tU16) internal_fa; /* internal file attributes        2 bytes */
  niNamespace(ni,
              tU32) ZEXTERNal_fa; /* ZEXTERNal file attributes        4 bytes */
  //tm_unz tmu_date;
...
struct iZipArchWrite : public iUnknown {
  ...
  virtual tBool __stdcall AddFileBlock(const achar* aaszName, iFile* apFile,
                                       tI64 aSize) = 0;
  //! Adds a block of raw data as a file.
  //! {NoAutomation}
  //! \return eTrue if successfull, else eFalse.
  virtual tBool __stdcall AddRawBlock(const achar* aaszName, tPtr apData,
                                      tSize aSize) = 0;
};
...
struct iZip : public iUnknown {
  ...
  virtual tBool __stdcall ZipUncompressBuffer(tPtr apDest, tU32 anDestSize,
                                              tPtr apSrc, tU32 anSrcSize) = 0;
  virtual tBool __stdcall ZipUncompressBufferInFile(iFile* apDest,
                                                    tU32 anDestSize, tPtr apSrc,
                                                    tU32 anSrcSize) = 0;
  virtual tBool __stdcall ZipUncompressFile(iFile* apDest, tU32 anDestSize,
                                            iFile* apSrc, tU32 anSrcSize) = 0;
  virtual tBool __stdcall ZipUncompressFileInBuffer(tPtr apDest,
                                                    tU32 anDestSize,
                                                    iFile* apSrc,
                                                    tU32 anSrcSize) = 0;
  virtual tBool __stdcall ZipInflateFileInBuffer(tPtr apDest, tU32 anDestSize,
                                                 iFile* apSrc) = 0;
  //! The destination buffer must be at least 20% bigger than the source.
  virtual tU32 __stdcall ZipCompressBuffer(tPtr apDest, tPtr apSrc,
                                           tU32 anSrcSize, tU32 anLevel) = 0;
  virtual tU32 __stdcall ZipCompressBufferInFile(iFile* apDest, tPtr apSrc,
                                                 tU32 anSrcSize,
                                                 tU32 anLevel) = 0;
  virtual tU32 __stdcall ZipCompressFile(iFile* apDest, iFile* apSrc,
                                         tU32 anSrcSize, tU32 anLevel) = 0;
  //! @}
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall UnzGetGlobalInfo(tHandle file,
                                          unz_global_info* pglobal_info) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall UnzGetGlobalComment(tHandle file,
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall UnzLocateFile(tHandle file, const achar* szFileName,
                                       tBool bCaseSensitivity) = 0;
  //! {NoAutomation}
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall UnzReadCurrentFile(tHandle file, tPtr buf,
                                            tU32 len) = 0;
  //! {NoAutomation}
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall UnzGetLocalExtrafield(tHandle file, tPtr buf,
                                               tU32 len) = 0;
  //! @}
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall DeflateSetDictionary(z_streamp strm,
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall DeflateParams(z_streamp strm, tI32 level,
                                       tI32 strategy) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall DeflateTune(z_streamp strm, tI32 good_length,
                                     tI32 max_lazy, tI32 nice_length,
                                     tI32 max_chain) = 0;
  //! {NoAutomation}
  virtual tU32 __stdcall DeflateBound(z_streamp strm, tU32 sourceLen) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall DeflatePrime(z_streamp strm, tI32 bits,
                                      tI32 value) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall DeflateSetHeader(z_streamp strm, gz_headerp head) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall InflateSetDictionary(z_streamp strm,
                                              const tU8* dictionary,
                                              tI32 dictLength) = 0;
  //! {NoAutomation}
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall InflatePrime(z_streamp strm, tI32 bits,
                                      tI32 value) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall InflateGetHeader(z_streamp strm, gz_headerp head) = 0;

  //! {NoAutomation}
  virtual tI32 __stdcall InflateBack(z_streamp strm, zlib_in_func aIn,
                                     void* in_desc, zlib_out_func aOut,
                                     void* out_desc) = 0;
  //! {NoAutomation}
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall Compress(tU8* dest, tU32* destLen, const tU8* source,
                                  tU32 sourceLen) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall Compress2(tU8* dest, tU32* destLen, const tU8* source,
                                   tU32 sourceLen, tI32 level) = 0;
  //! {NoAutomation}
  virtual tU32 __stdcall CompressBound(tU32 sourceLen) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall Uncompress(tU8* dest, tU32* destLen, const tU8* source,
                                    tU32 sourceLen) = 0;
  //! {NoAutomation}
  virtual tU32 __stdcall Adler32(tU32 adler, const tU8* buf, tI32 len) = 0;
  //! {NoAutomation}
  virtual tU32 __stdcall Adler32Combine(tU32 adler1, tU32 adler2,
                                        tSize len2) = 0;
  //! {NoAutomation}
  ...
  //! {NoAutomation}
  virtual tI32 __stdcall DeflateInit2(z_streamp strm, tI32 level, tI32 method,
                                      tI32 windowBits, tI32 memLevel,
                                      tI32 strategy) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall InflateInit2(z_streamp strm, tI32 windowBits) = 0;
  //! {NoAutomation}
  virtual tI32 __stdcall InflateBackInit(z_streamp strm, tI32 windowBits,
                                         tU8* window) = 0;
  //! {NoAutomation}
  ...
  virtual iFile* __stdcall CreateFileZipBufferEncoder(iFileBase* apBaseFile,
                                                      tU32 anCLevel) = 0;
  //! Create a Zip buffer decoder file.
  //! \see iLang::CreateFileBufferDecoder
  virtual iFile* __stdcall CreateFileZipBufferDecoder(iFileBase* apBaseFile,
  ...
  virtual iFile* __stdcall CreateFileRawBufferEncoder(
    iFileBase* apBaseFile) = 0;
  //! Create a Raw buffer decoder file.
  //! \see iLang::CreateFileBufferDecoder
  virtual iFile* __stdcall CreateFileRawBufferDecoder(iFileBase* apBaseFile,
                                                      tSize aDecodedSize) = 0;
    //! @}
  ...
  virtual iZipArchWrite* __stdcall CreateZipArchive(
    iFile* apDest, const achar* aaszPwd,
    const tStringCVec* apStoredExtensions) = 0;
  //! Open a file which can write or read directly in a zip file.
  virtual iFile* __stdcall GZipOpen(iFileBase* apFile,
                                    tU32 aulCompressionMode) = 0;
  //! Open a file which can write or read directly in a zip file.
  virtual iFile* __stdcall ZipOpen(iFileBase* apFile,
                                   tU32 aulCompressionMode) = 0;
  #endif
  //! @}
};
