                                   ...
                                   char aExponentCharacter)
{
  ...
  static double_conversion::DoubleToStringConverter converter(
    ...
    aInfinitySymbol, aNaNSymbol, aExponentCharacter, -5, 21, 6, 0);
...
niExportFunc(tF64) StringToDouble(
  const char* aBuffer, tSize aBufferSize, tInt* apProcessedCharactersCount,
  tStringToDoubleFlags aFlags, tF64 aEmptyStringValue, tF64 aInvalidStringValue,
  const char* aInfinitySymbol, const char* aNaNSymbol)
{
  const double_conversion::StringToDoubleConverter converter(
    aFlags, aEmptyStringValue, aInvalidStringValue, aInfinitySymbol,
    aNaNSymbol);
  ...
  return converter.StringToDouble(aBuffer, aBufferSize,
                                  apProcessedCharactersCount);
}
  ...
  const char* aInfinitySymbol, const char* aNaNSymbol)
{
  ...
  if (!apProcessedCharactersCount)
    apProcessedCharactersCount = &processedCharacterCountTmp;
  return converter.StringToFloat(aBuffer, aBufferSize,
                                 apProcessedCharactersCount);
}
