...
#define niDTS_SetProp_(NAME, VAL, TYPE, MT)                   \
  ...
  if (niIsStringOK(MT) &&                                     \
      niFlagIs(aFlags, ni::eSerializeFlags_TypeInfoMetadata)) \
  {                                                           \
...
#define niDTS_GetPropFSet(NAME, TYPE) \
  Set##VAL(                           \
    (TYPE)apDT->GetEnumDefault(_A(#NAME), niFlagsExpr(TYPE), Get##VAL()));
...
#define niDTS_GetPropFSetI(NAME, I, TYPE) \
  Set##VAL(                               \
    I, (TYPE)apDT->GetEnumDefault(_A(#NAME), niFlagsExpr(TYPE), Get##VAL(I)));
