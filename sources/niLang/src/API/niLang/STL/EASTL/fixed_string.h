...
///
  ...
  #define EASTL_FIXED_STRING_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX             \
      " fixed_string" // Unless the user overrides something, this is "EASTL fixed_string".
#endif
...
class fixed_string
 ...
 public:
  ...
  typedef fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>
    this_type;
 ...
 protected:
  union // We define a union in order to avoid strict pointer aliasing issues with compilers like GCC.
  {
    value_type mArray[1];
    aligned_buffer_type
      mBuffer; // Question: Why are we doing this aligned_buffer thing? Why not just do an array of value_type, given that we are using just strings of char types.
  };
 ...
 public:
  ...
  fixed_string(
    const base_type& x, size_type position,
    size_type n = base_type::
      npos); // Currently we don't support overflowAllocator specification for other constructors, for simplicity.
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow,
                    OverflowAllocator>::fixed_string()
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow,
                    OverflowAllocator>::fixed_string(const base_type& x)
    : base_type(fixed_allocator_type(mBuffer.buffer))
{
#if EASTL_NAME_ENABLED
  get_allocator().set_name(x.get_allocator().get_name());
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow,
                    OverflowAllocator>::fixed_string(const value_type* p,
                                                     size_type n)
    : base_type(fixed_allocator_type(mBuffer.buffer))
{
#if EASTL_NAME_ENABLED
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow,
                    OverflowAllocator>::fixed_string(size_type n,
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow,
                    OverflowAllocator>::fixed_string(const value_type* pBegin,
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow,
                    OverflowAllocator>::fixed_string(CtorDoNotInitialize,
                                                     size_type n)
    : base_type(fixed_allocator_type(mBuffer.buffer))
{
#if EASTL_NAME_ENABLED
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline fixed_string<T, nodeCount, bEnableOverflow,
                    OverflowAllocator>::fixed_string(CtorSprintf,
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline typename fixed_string<T, nodeCount, bEnableOverflow,
                             OverflowAllocator>::this_type&
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(
  const this_type& x)
{
  if (this != &x) {
    clear();
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline void fixed_string<T, nodeCount, bEnableOverflow,
                         ...
                         OverflowAllocator>::set_capacity(size_type n)
{
  ...
  {
    const size_type allocSize =
      (n +
       1); // +1 because the terminating 0 isn't included in the supplied capacity value. So now n refers the amount of memory we need.

    if (
      can_overflow() &&
      (((uintptr_t)internalLayout().HeapBeginPtr() !=
        (uintptr_t)mBuffer.buffer) ||
       (allocSize >
        kMaxSize))) // If we are or would be using dynamically allocated memory instead of our fixed-size member buffer...
    {
      T* const pNewData = (allocSize <= kMaxSize) ? (T*)&mBuffer.buffer[0]
                                                  : DoAllocate(allocSize);
      T* const pCopyEnd = (n < nPrevSize)
                            ? (internalLayout().HeapBeginPtr() + n)
                            : internalLayout().HeapEndPtr();
      CharStringUninitializedCopy(
        internalLayout().HeapBeginPtr(), pCopyEnd,
        pNewData); // Copy [internalLayout().heap.mpBegin, pCopyEnd) to pNewData.
      if ((uintptr_t)internalLayout().HeapBeginPtr() !=
          (uintptr_t)mBuffer.buffer)
        DoFree(internalLayout().HeapBeginPtr(),
               internalLayout().GetHeapCapacity() + 1);

      internalLayout().SetHeapSize(
        (size_type)(pCopyEnd - internalLayout().HeapBeginPtr()));
...
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::max_size() const
{
  return kMaxSize;
}
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline bool fixed_string<T, nodeCount, bEnableOverflow,
                         OverflowAllocator>::full() const
{
  ...
  return ((size_t)(internalLayout().HeapEndPtr() -
                   internalLayout().HeapBeginPtr()) >= kMaxSize) ||
         ((void*)internalLayout().HeapBeginPtr() != (void*)mBuffer.buffer);
}
...
inline typename fixed_string<T, nodeCount, bEnableOverflow,
                             OverflowAllocator>::this_type
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::substr(
  size_type position, size_type n) const
{
  ...
  return fixed_string(
    internalLayout().HeapBeginPtr() + position,
    internalLayout().HeapBeginPtr() + position +
...
inline typename fixed_string<T, nodeCount, bEnableOverflow,
                             OverflowAllocator>::this_type
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::left(
  size_type n) const
{
  const size_type nLength = size();
  if (n < nLength)
    return fixed_string(internalLayout().HeapBeginPtr(),
                        internalLayout().HeapBeginPtr() + n);
  return *this;
}
...
inline typename fixed_string<T, nodeCount, bEnableOverflow,
                             OverflowAllocator>::this_type
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::right(
  size_type n) const
{
  const size_type nLength = size();
  if (n < nLength)
    return fixed_string(internalLayout().HeapEndPtr() - n,
                        internalLayout().HeapEndPtr());
  return *this;
}
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline const typename fixed_string<T, nodeCount, bEnableOverflow,
                                   OverflowAllocator>::overflow_allocator_type&
fixed_string<T, nodeCount, bEnableOverflow,
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
inline void fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>::
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator> operator+(
  const typename fixed_string<T, nodeCount, bEnableOverflow,
                              OverflowAllocator>::value_type* p,
  const fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>& b)
{
  typedef fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>
    this_type;

  const typename this_type::size_type n =
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator> operator+(
  typename fixed_string<T, nodeCount, bEnableOverflow,
                        OverflowAllocator>::value_type c,
  const fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>& b)
{
  typedef fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>
    this_type;
...
template <typename T, int nodeCount, bool bEnableOverflow,
          typename OverflowAllocator>
fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator> operator+(
  fixed_string<T, nodeCount, bEnableOverflow, OverflowAllocator>&& a,
