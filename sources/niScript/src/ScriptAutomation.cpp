...
niDeclareModuleTrace_(niScript, TraceRegisterInterface);
  #define TRACE_REGISTER_INTERFACE(FMT) \
    niModuleTrace_(niScript, TraceRegisterInterface, FMT)
  #define SHOULD_TRACE_REGISTER_INTERFACE() \
    niModuleShouldTrace_(niScript, TraceRegisterInterface)

niDeclareModuleTrace_(niScript, TraceScriptImport);
  #define TRACE_SCRIPT_IMPORT(FMT) \
    niModuleTrace_(niScript, TraceScriptImport, FMT)

niDeclareModuleProperty(niScript, RegisterProperties, ni::eTrue);
  #define SHOULD_REGISTER_PROPERTIES() \
    niModuleShouldTrace_(niScript, RegisterProperties)

  #pragma niTodo( \
      "Seems that when an exception is throw the stack is not properly popped, might be because of the active {} statement, check it")
  #pragma niTodo( \
      "Test when properties are in several interfaces that one object implements --- It works the first interface is used --- BUT WE SHOULD ISSUE A WARNING, or throw an exceptions so that ambiguities are resolvable")
  #pragma niTodo( \
      "Add op_Add, op_Sub, etc... automatic binding to metha-methods")
  #pragma niTodo("Cleanup the code")
  #pragma niTodo("Remove all code for Weak references")
  #pragma niTodo( \
      "Try to lighten sIUnknown, it's a bit heavy now, and used quite a lot...")
  #pragma niTodo( \
      "abHoldRef is forced to true all the time now, hopefully it wont cause any problem - it shouldnt...")
...
static int std_createinstance(HSQUIRRELVM v)
{
  ...
  if (top <= 0 || top > 3)
    return sq_throwerror(
      v, _A("std_createinstance: Invalid number of parameters."));

  if (!SQ_SUCCEEDED(sq_getstring(v, 2, &aszObjectType)))
    return sq_throwerror(
      v,
      _A(
        "std_createinstance: First parameter (object type) must be string containing the object type."));
  ...
  if (top > 1) {
    if (!SQ_SUCCEEDED(sqa_getvar(v, 3, &varA)))
      return sq_throwerror(
        v,
        _A(
          "std_createinstance: Second parameter (vara) must be a valid variant."));
  }
  ...
  if (top > 2) {
    if (!SQ_SUCCEEDED(sqa_getvar(v, 4, &varB)))
      return sq_throwerror(
        v,
        _A(
          "std_createinstance: Third parameter (varb) must be a valid variant."));
  }
  ...
  if (!niIsOK(pI)) {
    niSafeRelease(pI);
    return sq_throwerror(
      v, niFmt(_A("std_createinstance: Can't create an instance of '%s'."),
               aszObjectType));
  }

  cString strModule = aszObjectType;
  strModule = strModule.substr(0, strModule.find(_A(".")));
  SQObjectPtr objModuleDef = DoImportNative(
    *v->_ss, v->_roottable, v->_ss->_nativeimports_table, _H(strModule));
  if (sqa_getscriptobjtype(objModuleDef) == eScriptType_ErrorCode) {
    return sq_throwerror(
      v,
      niFmt(_A("std_createinstance: Can't import the module '%s': %s."),
            strModule.Chars(),
  ...
  if (!SQ_SUCCEEDED(sqa_pushIUnknown(v, pI))) {
    niSafeRelease(pI);
    return sq_throwerror(
      v, niFmt(_A("std_createinstance: Can't push the new instance of '%s'."),
               aszObjectType));
  }
...
static int std_import_ex(HSQUIRRELVM v, tBool abNew)
{
  ...
  if (numParams != 1) {
    niSqUnGuard(v);
    return sq_throwerror(
      v, niFmt("std_import: one parameter expected, %d provided.", numParams));
  }
  ...
  if (type != eScriptType_String && type != eScriptType_IUnknown) {
    ...
    return sq_throwerror(
      v,
      _A(
        "std_import: the first parameter should be a string or a iFile instance."));
  }
  ...
  if (type == eScriptType_IUnknown) {
    ...
    if (!SQ_SUCCEEDED(sqa_getIUnknown(v, 2, &pFile, niGetInterfaceUUID(iFile))))
    {
      niSqUnGuard(v);
      return sq_throwerror(
        v, _A("std_import: the given instance doesn't implement iFile."));
    }
    ptrFile = (iFile*)pFile;
  }
  else {
    if (!SQ_SUCCEEDED(sq_getstring(v, 2, &aszModule))) {
      niSqUnGuard(v);
      return sq_throwerror(
        v, _A("std_import: can't get first parameter's string."));
    }
    isScriptFile =
      StrEndsWithI(aszModule, ".ni") || StrEndsWithI(aszModule, ".nim") ||
      StrEndsWithI(aszModule, ".nio") || StrEndsWithI(aszModule, ".nip") ||
  ...
  if (ptrFile.IsOK() || isScriptFile) {
    ...
    if (!niIsStringOK(aszModule)) {
      ...
      return sq_throwerror(
        v,
        niFmt(_A("std_import: unnamed module cant be imported."), aszModule));
    }
      ...
      if (_table(v->_ss->_scriptimports_table)->Get(importKey, foundImported)) {
        if (!sq_istable(foundImported)) {
          ...
          return sq_throwerror(
            v,
            niFmt(
              "std_import: %s: got an invalid object from the imports table.",
              aszModule));
        }
        TRACE_SCRIPT_IMPORT(
          ("niScript-Import: ni script '%s' already imported.", aszModule));
        niSqUnGuard(v);
        ...
        TRACE_SCRIPT_IMPORT(
          ("niScript-Import: ni script '%s' not found, importing.", aszModule));
      }
    }
    else {
      TRACE_SCRIPT_IMPORT(
        ("niScript-Import: ni script '%s' NewImport, importing.", aszModule));
    }
    ...
    if (!ptrFile.IsOK()) {
      ...
      if (!ptrFile.IsOK()) {
        niSqUnGuard(v);
        return sq_throwerror(
          v, niFmt(_A("std_import: %s: cant open script module as a file."),
                   aszModule));
      }
    }
    ...
    Ptr<iScriptObject> ptrScript = pVM->Compile(
      ptrFile,
      ni::GetRootFS()->GetAbsolutePath(ptrFile->GetSourcePath()).Chars());
    if (!niIsOK(ptrScript)) {
      niSqUnGuard(v);
      return sq_throwerror(
        v, niFmt("std_import: %s: cant compile script.", aszModule));
    }
    ...
    if (!abNew) {
      ...
      if (!_table(v->_ss->_scriptimports_table)->NewSlot(importKey, thisTable))
      {
        niSqUnGuard(v);
        return sq_throwerror(
          v, niFmt("std_import: %s: cant add to imports table.", aszModule));
      }
    }
    ...
    if (!pVM->Call(1, eFalse)) {
      ...
      niSqUnGuard(v);
      return sq_throwerror(
        v, niFmt(_A("Can't call script module '%s' closure."), aszModule));
    }
    pVM->Pop(1);
  ...
  // Import a native script module
    ...
    SQObjectPtr objModuleDef = DoImportNative(
      *v->_ss, v->_roottable, v->_ss->_nativeimports_table, _H(aszModule));
    if (sqa_getscriptobjtype(objModuleDef) == eScriptType_ErrorCode) {
      niSqUnGuard(v);
      return sq_throwerror(
        v,
        niFmt(_A("std_import: Can't import the module '%s': %s."), aszModule,
              ((sScriptTypeErrorCode*)_userdata(objModuleDef))->_strErrorDesc));
    }

    TRACE_SCRIPT_IMPORT(
      ("niScript-Import: ni module '%s' imported.", aszModule));
    niSqUnGuard(v);
    v->Push(objModuleDef);
    return 1;
  }
...
static int std_queryinterface(HSQUIRRELVM v)
{
  ...
  case eScriptType_String:
    if (!SQ_SUCCEEDED(sq_gethstring(v, 3, &hspName))) {
      return sq_throwerror(
        v, "std_queryinterface, can't get interface string id.");
    }
    uuid = ni::GetLang()->GetInterfaceUUID(hspName);
    if (uuid == kuuidZero) {
      return sq_throwerror(
        v, niFmt("std_queryinterface, can't get uuid of interface '%s'.",
                 hspName));
    }
    break;
  default:
    return sq_throwerror(
      v, "std_queryinterface, parameter 2 is not a valid interface id");
  }

  if (!SQ_SUCCEEDED(sqa_getIUnknown(v, 2, &pIUnk, uuid)))
    return sq_throwerror(
      v,
      _A("std_queryinterface, the first parameter is not a valid iunknown."));
  if (!SQ_SUCCEEDED(sqa_pushIUnknown(v, pIUnk)))
    return sq_throwerror(
      v, niFmt("std_queryinterface, can't query interface '%s' (%s).", uuid,
               hspName));
...
static int iunknown_tostring(HSQUIRRELVM v)
{
  ...
  if (!pV->pObject)
    return sq_throwerror(
      v, _A("iunknown_tostring, [this] is an invalid iunknown."));
                             ...
                             const iUnknown* apI)
{
  if (!niIsOK(apI)) {
    ...
    if (setIIDs.empty()) {
      ...
      for (astl::set<cString>::iterator it = setIIDs.begin();
           it != setIIDs.end(); ++it)
      {
        astrOut << *it;
        if (++it == setIIDs.end())
...
static int iunknown_getimplementedinterfaces(HSQUIRRELVM v)
{
  ...
  if (!pV->pObject)
    return sq_throwerror(
      v,
      _A("iunknown_getimplementedinterfaces, [this] is an invalid iunknown."));
...
static int iunknown_isvalid(HSQUIRRELVM v)
{
  ...
  if (!pV->pObject)
    return sq_throwerror(
      v, _A("iunknown_isvalid, [this] is an invalid iunknown."));
...
static int iunknown_tointptr(HSQUIRRELVM v)
{
  ...
  if (!pV->pObject)
    return sq_throwerror(
      v, _A("iunknown_tointptr, [this] is an invalid iunknown."));
...
static int iunknown_GetNumRefs(HSQUIRRELVM v)
{
  ...
  if (!pV->pObject)
    return sq_throwerror(
      v, _A("iunknown_GetNumRefs, [this] is an invalid iunknown."));
...
static int iunknown_queryinterface(HSQUIRRELVM v)
{
  ...
  if (!pV->pObject)
    return sq_throwerror(
      v, _A("iunknown_queryinterface, [this] is an invalid iunknown."));

  iHString* hspName;
  if (!SQ_SUCCEEDED(sq_gethstring(v, -1, &hspName)))
    return sq_throwerror(
      v, _A("iunknown_queryinterface, the parameter is not a string."));

  iUnknown* p =
    pV->pObject->QueryInterface(ni::GetLang()->GetInterfaceUUID(hspName));
  if (p == NULL) {
    sq_pushnull(v);
  }
  else {
    if (!SQ_SUCCEEDED(sqa_pushIUnknown(v, p)))
      return sq_throwerror(
        v, _A("iunknown_queryinterface, can't push the queried interface."));
  }
...
static int iunknown_invalidate(HSQUIRRELVM v)
{
  ...
  if (!pV->pObject)
    return sq_throwerror(
      v, _A("iunknown_invalidate, [this] is an invalid iunknown."));
...
const SQObjectPtr& SQSharedState::GetInterfaceDelegate(const tUUID& aIID)
{
  ...
  if (it == mmapDelegates.end()) {
    ...
    if (aIID == niGetInterfaceUUID(iUnknown)) {
      ...
      table->SetDebugName(
        ni::GetLang()->GetInterfaceName(niGetInterfaceUUID(iUnknown)));
      niLet regFunc = [&](ain<tChars> aName, ain<SQFUNCTION> aFunc,
                          ain<nn<iHString>> ahspRetType) {
        return RegisterSQRegFunction(
          table,
          SQRegFunction{ .name = aName, .f = aFunc, .rettype = ahspRetType });
      };
      niPanicAssert(
        regFunc("_tostring", iunknown_tostring, _HC(typestr_string)));
      niPanicAssert(regFunc("GetImplementedInterfaces",
                            iunknown_getimplementedinterfaces,
                            _HC(typestr_string)));
      ...
      niPanicAssert(regFunc("ToIntPtr", iunknown_tointptr, _HC(typestr_int)));
      niPanicAssert(
        regFunc("GetNumRefs", iunknown_GetNumRefs, _HC(typestr_int)));
      niPanicAssert(regFunc("QueryInterface", iunknown_queryinterface,
                            _HC(typestr_iunknown)));
      niPanicAssert(
        regFunc("Invalidate", iunknown_invalidate, _HC(typestr_void)));
    }
    else {
      ...
      if (!pInterfaceDef) {
            ...
            if (niFlagIs(pMethodDef->mReturnType, eTypeFlags_MethodSetter)) {
              if (pMethodDef->mnNumParameters > 2) {
                niWarning(niFmt(
                  ...
                  pInterfaceDef->maszName, pMethodDef->maszName));
              }
              else {
                ...
                if (StrNCmp(p, _A("Set"), 3) != 0) {
                  ...
                  cString strName;
                  StringToPropertyName(strName, p);
                  if (SHOULD_TRACE_REGISTER_INTERFACE()) {
                    cString str;
                    str << pInterfaceDef->maszName << _A("::") << p << _A("/")
                        << pMethodDef->mnNumParameters << _A(" -> ") << strName;
                  ...
                  if (strName.IsEmpty()) {
                    ...
                    if (itProp != mapProps.end()) {
                      ...
                      astl::upsert(mapProps, strName,
                                   sPropertyMethods(pMethodDef, NULL));
                    }
                  }
                }
              }
                  ...
                  if (StrNCmp(p, _A("Get"), 3) != 0) {
                    ...
                    if (strName.IsEmpty()) {
                      ...
                      if (itProp != mapProps.end()) {
                        ...
                        astl::upsert(mapProps, strName,
                                     sPropertyMethods(NULL, pMethodDef));
                      }
                    }
                  }
                }
        ...
        if (!mapProps.empty()) {
          for (itProp = mapProps.begin(); itProp != mapProps.end(); ++itProp) {
            table->NewSlot(_H(itProp->first),
                           niNew sScriptTypePropertyDef(pInterfaceDef,
                                                        itProp->second.pSet,
                                                        itProp->second.pGet));
          }
        }
...
///////////////////////////////////////////////
int cScriptAutomation::GetIUnknown(HSQUIRRELVM v, int idx,
                                   iUnknown** appIUnknown, const tUUID& aIID)
{
  ...
  case eScriptType_Table: {
    ...
    cScriptDispatch* pScriptDispatch =
      down_cast<cScriptDispatch*>(table->mpDispatch);
    if (!pScriptDispatch) {
    ...
    if (aIID != kuuidZero) {
      *appIUnknown = pScriptDispatch->QueryInterface(aIID);
      if (!*appIUnknown) {
        ...
        niWarning(niFmt(
          _A(
            "IUnknown Table doesnt implement the required interface '%s' {%s}."),
          niHStr(ni::GetLang()->GetInterfaceName(aIID)), UUID_ASZ(aIID)));
        return SQ_ERROR;
      }
    }
    else {
  ...
  case eScriptType_String: {
    if (aIID != kuuidZero && aIID != niGetInterfaceUUID(iHString) &&
  ...
  case eScriptType_Int: {
    ...
    if (_int(o) == 0) {
      *appIUnknown = NULL;
      ...
      niWarning(
        niFmt("Only 'NULL' (0) integer can be passed as IUnknown, got '%s'.",
              (Var&)o._var));
      return SQ_ERROR;
    }
    break;
    ...
    if (aIID != kuuidZero) {
      *appIUnknown = _iunknown(o)->QueryInterface(aIID);
      if (!*appIUnknown) {
        ...
        niWarning(
          niFmt(_A("IUnknown (%s) doesnt implement interface '%s' {%s}."), str,
                niHStr(ni::GetLang()->GetInterfaceName(aIID)), UUID_ASZ(aIID)));
        return SQ_ERROR;
      }
    }
...
void cScriptAutomation::InitIntfDelegateLst(
  HSQUIRRELVM vm, cScriptAutomation::tIntfDelegateLst& alstInterfaces,
  iUnknown* apObj
  #ifdef GETIUNKNOWN_INDEX_VTABLE
  ,
  tBool abDynamicOnly
...
)
{
  ...
  Nonnull<tUUIDCVec> uuidLst{ tUUIDCVec::Create() };
  apObj->ListInterfaces(uuidLst,
  #ifdef GETIUNKNOWN_INDEX_VTABLE
                        abDynamicOnly ? eListInterfacesFlags_DynamicOnly
  ...
  niLoop (i, uuidLst->size()) {
    ...
    if (iid == niGetInterfaceUUID(iUnknown))
      continue;
    if (iid == niGetInterfaceUUID(iDispatch))
      continue;
  ...
  if (objBase == _null_) {
    niWarning(niFmt(
      _A("sqa_pushIUnknown, Can't get delegate of base interface '%s' {%s}."),
  ...
  ///////////////////////////////////////////////
  #ifdef GETIUNKNOWN_INDEX_VTABLE
cScriptAutomation::tIntfDelegateLst* cScriptAutomation::GetIUnknownVTable(
  iUnknown* apIUnknown)
{
  ...
  ///////////////////////////////////////////////
  #if defined GETIUNKNOWN_INDEX_VTABLE || defined GETIUNKNOWN_OBJECT
cScriptAutomation::sIUnknownEntry* cScriptAutomation::NewIUnknownEntry(
  iUnknown* apObj)
{
  // get oldest node...
...
cScriptAutomation::sIUnknownEntry* cScriptAutomation::GetIUnknownEntry(
  iUnknown* apIUnknown)
{
