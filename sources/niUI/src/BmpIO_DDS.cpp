...
struct BitmapLoader_DDS : public ImplRC<iBitmapLoader> {
                                            ...
                                            iFile* pFile) niImpl
  {
    ...
    else if (ddsHeader.ddspf.dwFlags == DDS_RGBA &&
    ...
    else if (ddsHeader.ddspf.dwFlags == DDS_RGB &&
             ddsHeader.ddspf.dwRGBBitCount == 32)
    {
      switch (ddsHeader.ddspf.dwRBitMask) {
    ...
    else if (ddsHeader.ddspf.dwFlags == DDS_ALPHAONLY &&
             ddsHeader.ddspf.dwRGBBitCount == 8)
    {
      strFormat = _A("A8");
    }
    else if (ddsHeader.ddspf.dwFlags == DDS_LUMINANCE &&
             ddsHeader.ddspf.dwRGBBitCount == 8)
    {
      switch (ddsHeader.ddspf.dwRBitMask) {
    ...
    if (strFormat.IsEmpty()) {
      niError(niFmt(
        "Can't load the format of the DDS image : %c%c%c%c, Flags: 0x%x, Bits: %d, RedMask:0x%x, GreenMask:0x%x, BlueMask:0x%x, AlphaMask:0x%x.",
        niFourCCA(ddsHeader.ddspf.dwFourCC)
          ? niFourCCA(ddsHeader.ddspf.dwFourCC)
          : '#',
        niFourCCB(ddsHeader.ddspf.dwFourCC)
          ? niFourCCB(ddsHeader.ddspf.dwFourCC)
          : '#',
        niFourCCC(ddsHeader.ddspf.dwFourCC)
          ? niFourCCC(ddsHeader.ddspf.dwFourCC)
          : '#',
        niFourCCD(ddsHeader.ddspf.dwFourCC)
          ? niFourCCD(ddsHeader.ddspf.dwFourCC)
          : '#',
        ddsHeader.ddspf.dwFlags, ddsHeader.ddspf.dwRGBBitCount,
...
#pragma niTodo("Implement FULL support for 3D textures.")
    ...
    // check if image is a cubemap
      ...
      Ptr<iBitmapCube> ptrCube = apGraphics->CreateBitmapCubeEx(
        ddsHeader.dwWidth, apGraphics->CreatePixelFormat(strFormat.Chars()));
        ...
        if (ddsHeader.dwCaps2 & _kCubeMapFaceFlags[i]) {
          ...
          if (ddsHeader.dwMipMapCount) {
            for (tU32 mip = 0; mip < pFace->GetNumMipMaps(); ++mip) {
              pFile->ReadRaw(pFace->GetMipMap(mip)->GetData(),
                             pFace->GetMipMap(mip)->GetSize());
            }
          }
        }
      }
    ...
    // 2d image
      ...
      Ptr<iBitmap2D> ptr2D = apGraphics->CreateBitmap2DEx(
      ...
      if (ddsHeader.dwMipMapCount) {
        for (tU32 mip = 0; mip < ptr2D->GetNumMipMaps(); ++mip) {
          pFile->ReadRaw(ptr2D->GetMipMap(mip)->GetData(),
                         ptr2D->GetMipMap(mip)->GetSize());
        }
      }
      ...
      if (!palette.empty()) {
        ...
        BmpUtils_BlitPaletteTo32Bits(convertedBmp->GetData(), ptr2D->GetData(),
                                     ptr2D->GetWidth() * ptr2D->GetHeight(),
                                     palette.data());
        ptrRet = convertedBmp;
      }
      else {
