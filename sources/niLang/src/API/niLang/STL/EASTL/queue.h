...
///
  ...
  #define EASTL_QUEUE_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX      \
      " queue" // Unless the user overrides something, this is "EASTL queue".
#endif
...
template <typename T,
          typename Container =
            eastl::deque<T, EASTLAllocatorType, DEQUE_DEFAULT_SUBARRAY_SIZE(T)>>
class queue {
 public:
 ...
 public:
  ...
  queue(
    std::initializer_list<value_type>
      ilist); // C++11 doesn't specify that std::queue has initializer list support.
...
template <typename T, typename Container>
inline typename queue<T, Container>::const_reference queue<
  T, Container>::front() const
{
  return c.front();
}
...
template <typename T, typename Container>
inline typename queue<T, Container>::const_reference queue<T, Container>::back()
  const
{
  return c.back();
}
...
{
  c.push_back(const_cast<value_type&>(
    value)); // const_cast so that intrusive_list can work. We may revisit this.
}
...
template <typename T, typename Container>
inline typename queue<T, Container>::container_type& queue<
  T, Container>::get_container()
{
  return c;
}

template <typename T, typename Container>
inline const typename queue<T, Container>::container_type& queue<
  T, Container>::get_container() const
{
  return c;
}

template <typename T, typename Container>
void queue<T, Container>::swap(this_type& x) EA_NOEXCEPT_IF(
  (eastl::is_nothrow_swappable<this_type::container_type>::value))
{
  using eastl::swap;
  swap(c, x.c);
...
template <typename T, typename Container>
inline bool operator==(const queue<T, Container>& a,
                       const queue<T, Container>& b)
{
  return (a.c == b.c);
}
...
template <typename T, typename Container>
inline void
swap(queue<T, Container>& a, queue<T, Container>& b) EA_NOEXCEPT_IF((
  eastl::is_nothrow_swappable<typename queue<T, Container>::container_type>::
