...
void SQFuncState::SetIntructionParams(int pos, int arg0, int arg1, int arg2,
                                      int arg3)
{
...
bool SQFuncState::IsLocal(unsigned int stkpos)
{
  if (stkpos >= _vlocals.size())
    return false;
  else if (_sqtype(_vlocals[stkpos]._name) != OT_NULL)
    return true;
  return false;
}
...
void SQFuncState::AddLineInfos(sVec2i aLineCol, bool lineop, bool force)
{
  ...
  if (!_lineinfos.empty() && _lineinfos.back()._line == aLineCol.x &&
...
void SQFuncState::AddInstruction(const SQInstruction& i)
{
  ...
  if (size > 0 && (_optimization || i.op == _OP_RETURN)) {
    ...
    niLet debugOpMutation = [&](ain<SQInstruction> pi) {
      niDebugFmt((
        "... SQFuncState::AddInstruction(%s): Modified prevInst:\n  From: %s\n    To: %s",
        _GetOpDesc(i.op), SQInstructionToString(prevInst),
        SQInstructionToString(pi)));
    };
  ...
  #define LOG_OP_TRANSFORM()
    ...
    switch (i.op) {
    case _OP_RETURN: {
      if (_parent && i._arg0 != 0xFF && prevInst.op == _OP_CALL &&
          _returnexp < size - 1)
      {
    ...
    case _OP_GET: {
      if (prevInst.op == _OP_LOAD && prevInst._arg0 == i._arg2 &&
    ...
    case _OP_PREPCALL: {
      if (prevInst.op == _OP_LOAD && prevInst._arg0 == i._arg1 &&
          !IsLocal(prevInst._arg0))
      {
        niVar& pi = _instructions[piIdx];
        pi.op = _OP_PREPCALLK;
    ...
    case _OP_MOVE: {
      if ((prevInst.op == _OP_GET || prevInst.op == _OP_ADD ||
           prevInst.op == _OP_SUB || prevInst.op == _OP_MUL ||
           prevInst.op == _OP_DIV || prevInst.op == _OP_SHIFTL ||
           prevInst.op == _OP_SHIFTR || prevInst.op == _OP_BWOR ||
           prevInst.op == _OP_BWXOR || prevInst.op == _OP_BWAND) &&
          (prevInst._arg0 == i._arg1))
      {
        niVar& pi = _instructions[piIdx];
        pi._arg0 = i._arg0;
        pi._ext |=
          i._ext; // we're keeping the previous opcode, we must preserve its _ext aswell
        _optimization = false;
        LOG_OP_TRANSFORM();
        return;
      }
    ...
    case _OP_LOADNULL: {
      if ((prevInst.op == _OP_LOADNULL && prevInst._arg0 == i._arg0 - 1) ||
          (prevInst.op == _OP_LOADNULLS &&
           prevInst._arg0 + prevInst._arg1 == i._arg0))
      {
        niVar& pi = _instructions[piIdx];
        pi.op = _OP_LOADNULLS;
...
void SQFuncState::SetTopInstructionOpExt(int opExt)
{
  ...
  niAssert(i.op == _OP_DELETE || i.op == _OP_NEWSLOT || i.op == _OP_INC ||
           i.op == _OP_PINC || i.op == _OP_DEC || i.op == _OP_PDEC ||
           i.op == _OP_MINUSEQ || i.op == _OP_PLUSEQ || i.op == _OP_MULEQ ||
           i.op == _OP_DIVEQ || i.op == _OP_SHIFTLEQ || i.op == _OP_SHIFTREQ ||
           i.op == _OP_USHIFTREQ || i.op == _OP_BWANDEQ || i.op == _OP_BWOREQ ||
           i.op == _OP_BWXOREQ || i.op == _OP_MODULOEQ || i.op == _OP_PLUSEQ ||
           i.op == _OP_SET || i.op == _OP_GET || i.op == _OP_GETK ||
           i.op == _OP_PREPCALL || i.op == _OP_PREPCALLK || i.op == _OP_LOAD);
  i._ext = opExt;
}
