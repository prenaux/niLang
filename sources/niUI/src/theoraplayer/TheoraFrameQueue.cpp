...
void TheoraFrameQueue::setSize(int n)
{
	...
	for (int i = 0;i < n; ++i)
	{
		...
		{
			niError(_ASTR("TheoraFrameQueue: unable to create ") + str(n) + " frames, out of memory. Created " + str((int) mQueue.size()) + " frames.");
			break;
		}
	}
	lock.release();
...
astl::list<TheoraVideoFrame*>& TheoraFrameQueue::_getFrameQueue()
{
	return mQueue;
}
