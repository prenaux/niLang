...
static int
ni_png_read(const char *pathname, unsigned int *w, unsigned int *h, unsigned char **yuv)
{
  FILE *fp;
  unsigned char header[8];
  ni_png_structp ni_png_ptr;
  ni_png_infop info_ptr;
  ni_png_infop end_ptr;
  ni_png_bytep row_data;
  ni_png_bytep *row_pointers;
  ni_png_color_16p bkgd;
  ni_png_uint_32 width;
  ...
  ni_png_ptr = ni_png_create_read_struct(PNG_LIBPNG_VER_STRING,
    NULL, NULL, NULL);
  if(!ni_png_ptr) {
    fprintf(stderr, "%s: error: %s\n",
  ...
  if(!info_ptr) {
    ...
    ni_png_destroy_read_struct(&ni_png_ptr, NULL, NULL);
    fclose(fp);
    return 1;
  }

  end_ptr = ni_png_create_info_struct(ni_png_ptr);
  if(!end_ptr) {
    fprintf(stderr, "%s: error: %s\n",
      pathname, "couldn't create png info structure");
    ni_png_destroy_read_struct(&ni_png_ptr, &info_ptr, NULL);
    fclose(fp);
    return 1;
  }

  ni_png_init_io(ni_png_ptr, fp);
  ni_png_set_sig_bytes(ni_png_ptr, 8);
  ni_png_read_info(ni_png_ptr, info_ptr);
  ni_png_get_IHDR(ni_png_ptr, info_ptr, &width, &height, &bit_depth, &color_type,
   &interlace_type, &compression_type, &filter_method);
  ni_png_set_expand(ni_png_ptr);
  if(bit_depth<8)ni_png_set_packing(ni_png_ptr);
  if(bit_depth==16)ni_png_set_strip_16(ni_png_ptr);
  if(!(color_type&PNG_COLOR_MASK_COLOR))ni_png_set_gray_to_rgb(ni_png_ptr);
  if(ni_png_get_bKGD(ni_png_ptr, info_ptr, &bkgd)){
    ni_png_set_background(ni_png_ptr, bkgd, PNG_BACKGROUND_GAMMA_FILE, 1, 1.0);
  }
  ...
  row_data = (ni_png_bytep)ni_png_malloc(ni_png_ptr,
    3*height*width*ni_png_sizeof(*row_data));
  row_pointers = (ni_png_bytep *)ni_png_malloc(ni_png_ptr,
    height*ni_png_sizeof(*row_pointers));
  for(y = 0; y < height; y++) {
    row_pointers[y] = row_data + y*(3*width);
  }
  ni_png_read_image(ni_png_ptr, row_pointers);
  ni_png_read_end(ni_png_ptr, end_ptr);
  ...
  *yuv = malloc(*w * *h * 3);
  ...
  ni_png_free(ni_png_ptr, row_pointers);
  ni_png_free(ni_png_ptr, row_data);
  ni_png_destroy_read_struct(&ni_png_ptr, &info_ptr, &end_ptr);
...
main(int argc, char *argv[])
{
...
#endif
  n = scandir (input_directory, &ni_png_files, include_files, alphasort);
  ...
  for(passno=(twopass==3?1:twopass);passno<=(twopass==3?2:twopass);passno++){
    ...
    snprintf(input_png, 1023,"%s/%s", input_directory, ni_png_files[0]->d_name);
    if(ni_png_read(input_png, &w, &h, &yuv)) {
    ...
    do {
      ...
      i++;
      if (!last) {
        snprintf(input_png, 1023,"%s/%s", input_directory, ni_png_files[i]->d_name);
        if(ni_png_read(input_png, &w, &h, &yuv)) {
          fprintf(stderr, "could not read %s\n", input_png);
          exit(1);
        }
