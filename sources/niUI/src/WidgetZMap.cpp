...
cWidget* cWidgetZMap::GetTargetByPos(
  const sVec2f& pos,
  ...
  tIntPtr apUserData) const
{
  ...
  {
    for (tWidgetPtrDeqCRIt lit = zit->second.rbegin();
         lit != zit->second.rend(); ++lit)
    {
      cWidget* ret =
        (*lit)->GetMessageTargetByPos(pos, apfnExcludeWidget, apUserData);
      if (ret)
        return ret;
    }
  }
