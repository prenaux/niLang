...
///////////////////////////////////////////////
static inline tBool __stdcall _UIInputSendMsg(
  cWidget* apWidget, tU32 anID, const Var& avarA = (Var&)niVarNull,
...
#undef INPUT_IS_DOWN
  ...
  if (apUICtx->mnHoverInputModifiers && apUICtx->_CanHover()) {
    ...
    apUICtx->_FingerMove(apUICtx->GetPrimaryFingerID(),
                         apUICtx->GetPrimaryFinger().mvPosition, eFalse);
  }
}
...
void __stdcall cUIContext::_KeyDown(tU32 aKey)
{
  ...
  const tU32 keyModToSend =
    mnInputModifiers | (isRepeat ? eUIInputModifier_KeyDownRepeat : 0);
  _UIInputKeyboardTrace(niFmt("KEYDOWN: %s, %s", niEnumToChars(eKey, aKey),
                              niFlagsToChars(eUIInputModifier, keyModToSend)));
  ...
  // System shortcuts
  {
    ...
    if (fullKey == eKey_Escape) {
      if (IsFingerDragging(GetPrimaryFingerID())) {
        EndFingerDrag(GetPrimaryFingerID(), GetPrimaryFinger().mvPosition,
                      eTrue);
      }
      else {
        QPtr<cWidget> target(GetInputMessageTarget());
        if (target.IsOK()) {
          _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-Cancel: %p (ID:%s)."),
    ...
    /// SubmitA/B/C ///
      ...
      // SubmitA
      ((fullKey == eKey_Enter) || (fullKey == eKey_NumPadEnter)) ||
      // SubmitB
      (fullKey == eKey_Space) ||
      // SubmitC
      (fullKey == (eKeyMod_Control | eKey_Enter)))
    {
      QPtr<cWidget> target(GetInputMessageTarget());
      if (target.IsOK()) {
        if ((niFlagIs(target->mnInputSubmitFlags,
                      eUIInputSubmitFlags_SubmitA) &&
             ((fullKey == eKey_Enter) || (fullKey == eKey_NumPadEnter))) ||
            (niFlagIs(target->mnInputSubmitFlags,
      ...
      if (fullKey == (eKeyMod_Control | eKey_C)) {
        QPtr<cWidget> target(GetInputMessageTarget());
        if (target.IsOK()) {
          _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-Copy: %p (ID:%s)."),
                              target.ptr(), niHStr(target->GetID())));
          ni::Ptr<iDataTable> dt =
      ...
      /// Cut ///
        ...
        QPtr<cWidget> target(GetInputMessageTarget());
        if (target.IsOK()) {
          _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-Cut: %p (ID:%s)."),
                              target.ptr(), niHStr(target->GetID())));
          ni::Ptr<iDataTable> dt =
            ni::GetLang()->GetClipboard(eClipboardType_System);
      ...
      /// Paste ///
        ...
        QPtr<cWidget> target(GetInputMessageTarget());
        if (target.IsOK()) {
          _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-Paste: %p (ID:%s)."),
                              target.ptr(), niHStr(target->GetID())));
          ni::Ptr<iDataTable> dt =
            ni::GetLang()->GetClipboard(eClipboardType_System);
      ...
      /// Undo ///
        ...
        QPtr<cWidget> target(GetInputMessageTarget());
        if (target.IsOK()) {
          _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-Undo: %p (ID:%s)."),
                              target.ptr(), niHStr(target->GetID())));
      ...
      /// Redo ///
        ...
        QPtr<cWidget> target(GetInputMessageTarget());
        if (target.IsOK()) {
          _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-Redo: %p (ID:%s)."),
                              target.ptr(), niHStr(target->GetID())));
          _UIInputSendMsg(target, eUIMessage_Redo, niVarNull, niVarNull);
        }
      }
      /// MoveFocus ///
      else if (fullKey == (eKeyMod_Shift | eKey_Tab) || fullKey == eKey_Tab) {
        QPtr<cWidget> target(GetInputMessageTarget());
        if (target.IsOK()) {
  ...
  // Send the key down message
  {
    QPtr<cWidget> target = GetInputMessageTarget();
    if (target.IsOK()) {
      _UIInputKeyboardTrace(niFmt("SEND KEYDOWN TO INPUT TARGET: '%s' (%p)",
                                  target->GetID(), (tIntPtr)target.ptr()));
...
void __stdcall cUIContext::_KeyUp(tU32 aKey)
{
  ...
  _UpdateInputModifiersFromFromKeys(this, aKey, mKeyIsDown, mnInputModifiers);
  _UIInputKeyboardTrace(
    niFmt("KEYUP: %s, %s", niEnumToChars(eKey, aKey),
...
{
  _UIInputKeyboardTrace(niFmt(
    "KEYCHAR: %c (%d), keyLeadingToKeyChar: %d, keyEatChar: %d, inputModifier: %s",
    ch, ch, aKeyLeadingToKeyChar, mKeyEatChar,
    niFlagsToChars(eUIInputModifier, mnInputModifiers)));
...
void __stdcall cUIContext::_LeftDblClick(tBool abTriggered)
{
  ...
  if (target.IsOK()) {
    ...
    sVec2f clientPos;
    if (abTriggered) { // Down
      _UIFocusTrace(niFmt(_A("### UICONTEXT _LeftDblClick: %p (ID:%s)."),
                          (tIntPtr)target.ptr(), target->GetID()));
      this->SetFocusInput(target, eTrue);
      if (target->MapToClientRect(mpos, clientPos)) {
        _UIInputTrace(
          niFmt(_A("### UICONTEXT-INPUT-LeftDoubleClick: %p (ID:%s)."),
                target.ptr(), niHStr(target->GetID())));
        _UIInputSendMsg(target, eUIMessage_LeftDoubleClick, clientPos, mpos);
      }
      else {
        _UIInputTrace(
          niFmt(_A("### UICONTEXT-INPUT-NCLeftDoubleClick: %p (ID:%s)."),
                target.ptr(), niHStr(target->GetID())));
...
void __stdcall cUIContext::_RightDblClick(tBool abTriggered)
{
  ...
  if (target.IsOK()) {
    ...
    sVec2f clientPos;
    if (abTriggered) { // Down
      _UIFocusTrace(niFmt(_A("### UICONTEXT _RightDblClick: %p (ID:%s)."),
                          (tIntPtr)target.ptr(), target->GetID()));
      this->SetFocusInput(target, eTrue);
      if (target->MapToClientRect(mpos, clientPos)) {
        _UIInputTrace(
          niFmt(_A("### UICONTEXT-INPUT-RightDoubleClick: %p (ID:%s)."),
                target.ptr(), niHStr(target->GetID())));
        _UIInputSendMsg(target, eUIMessage_RightDoubleClick, clientPos, mpos);
      }
      else {
        _UIInputTrace(
          niFmt(_A("### UICONTEXT-INPUT-NCRightDoubleClick: %p (ID:%s)."),
                target.ptr(), niHStr(target->GetID())));
...
void __stdcall cUIContext::_Wheel(tF32 value)
{
  ...
  if (GetPrimaryFinger().mpDragWidget.IsOK()) {
    ...
    target =
      this->GetMouseMessageTarget(eTrue); //this->GetInputMessageTarget();
  }
  ...
  if (target.IsOK()) {
    // this->SetFocusInput(target,eTrue);
    if ((target->GetClientRect() + target->GetAbsolutePosition())
          .Intersect(this->GetMousePos()))
    {
      _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-Wheel: %g, %p (ID:%s)."),
                          value, target.ptr(), niHStr(target->GetID())));
      _UIInputSendMsg(target, eUIMessage_Wheel, value,
                      this->GetCursorPosition());
    }
    else {
      _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-NCWheel: %g, %p (ID:%s)."),
                          value, target.ptr(), niHStr(target->GetID())));
      _UIInputSendMsg(target, eUIMessage_NCWheel, value,
                      this->GetCursorPosition());
    }
  }
  ...
  {
    _FingerMove(GetPrimaryFingerID(), GetPrimaryFinger().mvPosition, eFalse);
  }
}
...
void __stdcall cUIContext::_RelativeMouseMove(const sVec2f& aRelMove)
{
  ...
  if ((target->GetClientRect() + target->GetAbsolutePosition())
        .Intersect(this->GetMousePos()))
  {
    _UIInputSendMsg(target, eUIMessage_FingerRelativeMove,
                    FINGER_POINTER_ID_MIN + ePointerButton_Left,
                    Vec3f(aRelMove.x, aRelMove.y, 1.0));
  }
  else {
    _UIInputSendMsg(target, eUIMessage_NCFingerRelativeMove,
                    FINGER_POINTER_ID_MIN + ePointerButton_Left,
                    Vec3f(aRelMove.x, aRelMove.y, 1.0));
  }
}

#define FPARAMS(CLIENTPOS) \
  anFinger, Vec3f(CLIENTPOS.x, CLIENTPOS.y, f.GetPressure())

///////////////////////////////////////////////
tBool __stdcall cUIContext::_FingerUpdatePosition(tU32 anFinger,
                                                  const sVec3f& avNewPos)
{
...
///////////////////////////////////////////////
void __stdcall cUIContext::_FingerClick(tU32 anFinger, const sVec3f& avNewPos,
                                        tBool abTriggered)
{
  ...
  if (isPrimaryFinger && (!abTriggered) &&
      (mnLastPrimaryFinger != FINGER_ID_POINTER_LEFT))
  {
  ...
  if ((anFinger == FINGER_ID_POINTER_RIGHT) &&
      IsFingerDragging(GetPrimaryFingerID()))
  {
    EndFingerDrag(mnLastPrimaryFinger, avNewPos, eTrue);
    return;
  }

  _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-_FingerClick[%d]: %d"), anFinger,
                      abTriggered));
  ...
  if (target.IsOK()) {
    ...
    sVec2f clientPos;
    if (abTriggered) { // Down
      if (isPrimaryFinger || (FINGER_IS_POINTER(mnLastPrimaryFinger) &&
                              FINGER_IS_POINTER(anFinger)))
      {
        _UIFocusTrace(niFmt(
          _A("### UICONTEXT _FingetClick down primary finger: %p (ID:%s)."),
          (tIntPtr)target.ptr(), target->GetID()));
        this->SetFocusInput(target, eTrue);
      }
      if (target->MapToClientRect(mpos, clientPos)) {
        _UIInputTrace(
          niFmt(_A("### UICONTEXT-INPUT-FingerDown[%d]: %p (ID:%s)."), anFinger,
                target.ptr(), niHStr(target->GetID())));
        ...
        _UIInputTrace(
          niFmt(_A("### UICONTEXT-INPUT-NCFingerDown[%d]: %p (ID:%s)."),
                anFinger, target.ptr(), niHStr(target->GetID())));
      ...
      if (target->MapToClientRect(mpos, clientPos)) {
        _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-FingerUp[%d]: %p (ID:%s)."),
                            anFinger, target.ptr(), niHStr(target->GetID())));
        ...
        _UIInputTrace(
          niFmt(_A("### UICONTEXT-INPUT-NCFingerUp[%d]: %p (ID:%s)."), anFinger,
                target.ptr(), niHStr(target->GetID())));
...
{
  return (this->mfHoverDelay > 0.0f) &&
         (!mnHoverInputModifiers ||
          (mnHoverInputModifiers == mnInputModifiers));
}

///////////////////////////////////////////////
void cUIContext::_FingerMove(tU32 anFinger, sVec3f avNewPos,
                             tBool abOnlyIfChanged)
{
  CHECK_FINGER(anFinger, ;);
  ...
  // different target, send the finger leave message
  {
    ...
    if (moveTarget.IsOK() && moveTarget.ptr() != target.ptr()) {
      ...
      _UIInputMMTrace(niFmt(_A("### UICONTEXT-INPUT-FingerLeave: %p (ID:%s)."),
                            moveTarget.ptr(), niHStr(moveTarget->GetID())));
  ...
  // different target, send the finger leave message
  {
    ...
    if (ncMoveTarget.IsOK() && ncMoveTarget.ptr() != target.ptr()) {
      ...
      _UIInputSendMsg(ncMoveTarget, eUIMessage_NCFingerLeave,
                      FPARAMS(clientPos));
      if (isPrimaryFinger) {
        f._CancelHoverTimers();
      ...
      _UIInputMMTrace(
        niFmt(_A("### UICONTEXT-INPUT-NCFingerLeave: %p (ID:%s)."),
              ncMoveTarget.ptr(), niHStr(ncMoveTarget->GetID())));
  ...
  if (target.IsOK()) {
    ...
    if (target->MapToClientRect(mpos, clientPos)) {
      ...
      // different target, send the finger enter message
      {
        ...
        if (moveTarget.ptr() != target.ptr()) {
          ...
          _UIInputMMTrace(
            niFmt(_A("### UICONTEXT-INPUT-FingerEnter: %p (ID:%s)."),
                  target.ptr(), niHStr(target->GetID())));
        }
      }
      ...
      _UIInputMMTrace(
        niFmt(_A("### UICONTEXT-INPUT-FingerMove[%s,%s]: %p (ID:%s)."),
              clientPos, mpos, target.ptr(), niHStr(target->GetID())));
      // update the hover
      ...
      // different target, send the nc finger enter message
      {
        ...
        if (ncMoveTarget.ptr() != target.ptr()) {
          ...
          _UIInputMMTrace(
            niFmt(_A("### UICONTEXT-INPUT-NCFingerEnter: %p (ID:%s)."),
                  target.ptr(), niHStr(target->GetID())));
        }
      }
      ...
      _UIInputMMTrace(niFmt(_A("### UICONTEXT-INPUT-NCFingerMove: %p (ID:%s)."),
                            target.ptr(), niHStr(target->GetID())));
      // update the hover
      if (_CanHover() && isPrimaryFinger) {
        QPtr<cWidget> ncMoveTarget(f.mpwNCMoveTarget);
        ...
        {
          f._CancelHoverTimers();
          ncMoveTarget->SetTimer(eWidgetSystemTimer_NCHover,
                                 this->mfHoverDelay);
        }
      }
    }
    ...
    if (isPrimaryFinger) {
      _UIInputMMTrace(niFmt(_A("### UICONTEXT-INPUT-SetCursor: %p (ID:%s)."),
                            target.ptr(), niHStr(target->GetID())));
  ...
  if (f.mpDragWidget.IsOK()) {
    if (_CanSendFingerDragMessage(anFinger)) {
      ...
      // different target, send the finger leave message
      {
        ...
        if (ptrDragMoveTarget.IsOK() &&
            (ptrDragMoveTarget.ptr() != target.ptr() || !f.mpDragWidget.IsOK()))
        {
          _UIInputSendMsg(ptrDragMoveTarget, eUIMessage_DragFingerLeave,
                          anFinger, niVarNull);
          if (isPrimaryFinger) {
            _UIInputSendMsg(ptrDragMoveTarget, eUIMessage_DragMouseLeave,
                            niVarNull, niVarNull);
          }
          _UIDragTrace(
            niFmt(_A("### UICONTEXT-INPUT-DragFingerLeave: %p (ID:%s)."),
      ...
      // different target, send the finger leave message
      {
        ...
        if (ptrNCDragMoveTarget.IsOK() &&
            (ptrNCDragMoveTarget.ptr() != target.ptr() ||
             !f.mpDragWidget.IsOK()))
        {
          _UIInputSendMsg(ptrNCDragMoveTarget, eUIMessage_NCDragFingerLeave,
                          anFinger, niVarNull);
          if (isPrimaryFinger) {
            _UIInputSendMsg(ptrNCDragMoveTarget, eUIMessage_NCDragMouseLeave,
                            niVarNull, niVarNull);
          }
          _UIDragTrace(niFmt(
            _A("### UICONTEXT-INPUT-NCDragFingerLeave: %p (ID:%s)."),
        ...
        if (target->MapToClientRect(mpos, clientPos)) {
          {
            ...
            if (ptrDragMoveTarget.ptr() != target.ptr()) {
              ...
              _UIInputSendMsg(target, eUIMessage_DragFingerEnter,
                              FPARAMS(clientPos));
              if (isPrimaryFinger) {
                _UIInputSendMsg(target, eUIMessage_DragMouseEnter, clientPos,
                                mpos);
              }
              _UIDragTrace(
                niFmt(_A("### UICONTEXT-INPUT-DragFingerEnter: %p (ID:%s)."),
                      target.ptr(), niHStr(target->GetID())));
            }
          }
          _UIInputSendMsg(target, eUIMessage_DragFingerMove,
                          FPARAMS(clientPos));
          if (isPrimaryFinger) {
            _UIInputSendMsg(target, eUIMessage_DragMouseMove, clientPos, mpos);
          }
          _UIDragTrace(
            niFmt(_A("### UICONTEXT-INPUT-DragFingerMove: %p (ID:%s)."),
                  target.ptr(), niHStr(target->GetID())));
        }
        else {
          {
            ...
            if (ptrNCDragMoveTarget.ptr() != target.ptr()) {
              ...
              _UIInputSendMsg(target, eUIMessage_NCDragFingerEnter,
                              FPARAMS(clientPos));
              if (isPrimaryFinger) {
                _UIInputSendMsg(target, eUIMessage_NCDragMouseEnter, clientPos,
                                mpos);
              }
              _UIDragTrace(
                niFmt(_A("### UICONTEXT-INPUT-NCDragFingerEnter: %p (ID:%s)."),
                      target.ptr(), niHStr(target->GetID())));
            }
          }
          _UIInputSendMsg(target, eUIMessage_NCDragFingerMove,
                          FPARAMS(clientPos));
          if (isPrimaryFinger) {
            _UIInputSendMsg(target, eUIMessage_NCDragMouseMove, clientPos,
                            mpos);
          }
          _UIDragTrace(
            niFmt(_A("### UICONTEXT-INPUT-NCDragFingerMove: %p (ID:%s)."),
                  target.ptr(), niHStr(target->GetID())));
        }
      }
    }
  ...
  else {
    {
      ...
      if (ptrDragMoveTarget.IsOK() &&
          (ptrDragMoveTarget.ptr() != target.ptr() || !f.mpDragWidget.IsOK()))
      {
        _UIInputSendMsg(ptrDragMoveTarget, eUIMessage_DragFingerLeave, anFinger,
                        niVarNull);
        if (isPrimaryFinger) {
          _UIInputSendMsg(ptrDragMoveTarget, eUIMessage_DragMouseLeave,
                          niVarNull, niVarNull);
    ...
    }
    {
      ...
      // different target, send the finger leave message
      if (ptrNCDragMoveTarget.IsOK() &&
          (ptrNCDragMoveTarget.ptr() != target.ptr() || !f.mpDragWidget.IsOK()))
      {
        _UIInputSendMsg(ptrNCDragMoveTarget, eUIMessage_NCDragFingerLeave,
                        anFinger, niVarNull);
        if (isPrimaryFinger) {
...
///////////////////////////////////////////////
void __stdcall cUIContext::SendWindowMessage(eOSWindowMessage aMsg,
                                             const Var& avarA, const Var& avarB)
{
  ...
  case eOSWindowMessage_MouseButtonUp: {
    ...
    if (fingerIndex < knNumFingers) {
      const sFinger& f = GetPrimaryFinger();
      _FingerClick(fingerId, f.mvPosition,
                   (aMsg == eOSWindowMessage_MouseButtonDown));
    }
    break;
  }
  case eOSWindowMessage_MouseMove: {
    ...
    _FingerMove(GetPrimaryFingerID(),
                Vec3f(clientPos.x, clientPos.y, f.mvPosition.z), eTrue);
    break;
  }
  ...
  case eOSWindowMessage_FingerDown: {
    ...
    const sVec3f fingerPos =
      avarB.GetVec3fValue() / Vec3f(mfContentsScale, mfContentsScale, 1.0f);
...
tBool __stdcall cUIContext::SetCursor(iOverlay* pCursor)
{
      ...
      _CanSendFingerDragMessage(GetPrimaryFingerID()))
  {
    ...
    if (!ptrTarget.IsOK() ||
        niFlagIsNot(ptrTarget->GetStyle(), eWidgetStyle_DragDestination))
    {
...
///////////////////////////////////////////////
void __stdcall cUIContext::InputFingerMove(tU32 anFinger,
                                           const sVec3f& avPosition)
{
  _FingerMove(anFinger, avPosition, eTrue);
}
void __stdcall cUIContext::InputFingerRelativeMove(tU32 anFinger,
                                                   const sVec3f& avRelMove)
{
  CHECK_FINGER(anFinger, ;);
  Ptr<cWidget> target = this->GetFingerMessageTarget(anFinger, eTrue);
  _UIInputTrace(niFmt(
    _A("### UICONTEXT-INPUT-FingerRelativeMove: %p (%s) [%d]: %s"),
    target.ptr(), target.IsOK() ? target->GetID() : NULL, anFinger, avRelMove));
  if (target.IsOK()) {
    sFinger& f = GET_FINGER(anFinger);
    sVec2f mpos = f.GetPosition() - target->GetAbsolutePosition();
    sVec2f clientPos;
    if (target->MapToClientRect(mpos, clientPos)) {
      ...
      _UIInputSendMsg(target, eUIMessage_FingerRelativeMove, anFinger,
                      avRelMove);
    }
    else {
      _UIInputTrace(niFmt(_A("### UICONTEXT-INPUT-NCFingerRelativeMove")));
      _UIInputSendMsg(target, eUIMessage_NCFingerRelativeMove, anFinger,
                      avRelMove);
    }
  }
}
void __stdcall cUIContext::InputFingerPress(tU32 anFinger,
                                            const sVec3f& avPosition,
                                            tBool abDown)
{
...
void __stdcall cUIContext::InputKeyChar(tU32 aCharCodePoint,
...
void __stdcall cUIContext::InputPinch(const tF32 afScale,
                                      const eGestureState aState)
{
  Ptr<cWidget> target = this->GetMouseMessageTarget(eTrue);
  if (!target.IsOK())
    return;
...
{
  switch (aPointer) {
  case ePointerButton_Left: _LeftDblClick(eTrue); break;
  case ePointerButton_Right: _RightDblClick(eTrue); break;
  default: break;
  }
}
...
void __stdcall cUIContext::InputGameCtrl(iGameCtrl* apGameController)
{
  ...
  if (target.IsOK()) {
    ...
    astl::map<tU32, tU32>::iterator it =
      mmapGameCtrlInputMasks.find(apGameController->GetIndex());
    if (it == mmapGameCtrlInputMasks.end()) {
      ...
      niLoop (i, ni::Min(apGameController->GetNumButtons(), 32)) {
        ...
        if (!niFloatIsZero(v)) {
          mask |= niBit(i);
          // GameCtrlButtonDown
          _UIInputSendMsg(target, eUIMessage_GameCtrlButtonDown,
                          apGameController, i);
        }
      }
      it = astl::upsert(mmapGameCtrlInputMasks, apGameController->GetIndex(),
                        mask);
    }
    else {
      ...
      niLoop (i, ni::Min(apGameController->GetNumButtons(), 32)) {
        ...
        if (!niFloatIsZero(v)) {
          ...
          if (wasMask & niBit(i)) {
            _UIInputSendMsg(target, eUIMessage_GameCtrlButtonUp,
                            apGameController, i);
          }
        }
      }
