...
//----------------------------------------------------------------------------
//
// Section: RasterizerStates
//
//----------------------------------------------------------------------------
template <tBool IS_RO>
class cRasterizerStates : public ImplRC<iRasterizerStates>,
...
#define END()                                       \
  astl::upsert(mmapRasterizerStates, (tIntPtr)cs,   \
...
///////////////////////////////////////////////
iRasterizerStates* __stdcall cGraphics::GetCompiledRasterizerStates(
  tIntPtr aHandle) const
{
  if (!aHandle)
    return NULL;
  tRSMap::const_iterator it = mmapRasterizerStates.find(aHandle);
  if (it == mmapRasterizerStates.end())
    return NULL;
