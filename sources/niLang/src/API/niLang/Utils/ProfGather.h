...
// SPDX-License-Identifier: MIT
  ...
  #error \
    "ProfGather is already included, you should make sure to include it only once to avoid definition conflicts."
#else
...
static ni::sProfilerZoneStack** niProf_stack_ptr;
    ...
    #define niProf_Begin_Cache(z)                    \
      ...
      static ni::sProfilerZoneStack* niProf_cache =  \
        ni::GetProf()->GetZoneStackDummy()
    ...
    #define niProf_Begin_Code(z)                                                      \
      ((ni::niProf_stack_ptr =                                                        \
          ni::GetProf()                                                               \
            ->GetZoneStack()), /* check the cached Zone_Stack and update if needed */ \
       (niProf_cache->parent != (*ni::niProf_stack_ptr)                               \
          ? niProf_cache = ni::GetProf()->StackAppend(&z)                             \
          : 0),                                                                       \
       ++niProf_cache->total_entry_count,                                             \
       ni::GetProf()->GetTimeStamp(                                                   \
         &ni::niProf_time), /* stop the timer on the parent zone stack */             \
       ((*ni::niProf_stack_ptr)->total_self_ticks +=                                  \
        ni::niProf_time - (*ni::niProf_stack_ptr)                                     \
                            ->t_self_start), /* make cached stack current */          \
       (*ni::niProf_stack_ptr) =                                                      \
         niProf_cache, /* start the timer on this stack */                            \
       (*ni::niProf_stack_ptr)->t_self_start = ni::niProf_time, 0)

    #define niProf_End_Raw()                                                  \
      (ni::GetProf()->GetTimeStamp(&ni::niProf_time),                         \
       (ni::niProf_stack_ptr =                                                \
          ni::GetProf()                                                       \
            ->GetZoneStack()), /* stop timer for current zone stack */        \
       (*ni::niProf_stack_ptr)->total_self_ticks +=                           \
       ni::niProf_time -                                                      \
       (*ni::niProf_stack_ptr)->t_self_start, /* make parent chain current */ \
       (*ni::niProf_stack_ptr) =                                              \
         (*ni::niProf_stack_ptr)                                              \
           ->parent, /* start timer for parent zone stack */                  \
