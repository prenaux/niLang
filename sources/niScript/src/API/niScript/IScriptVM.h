...
struct iScriptObjectEnumSink : public iUnknown {
  ...
  virtual tBool __stdcall OnScriptObjectEnumSinkFound(
    iScriptVM* apVM, iScriptObject* apKey, iScriptObject* apValue) = 0;
};
...
struct iScriptVM : public iScriptingHost {
  ...
  virtual iScriptObject* __stdcall Compile(iFile* apFile,
                                           const achar* aaszName = NULL) = 0;
  ...
  //! \remark This does not run the compiled code.
  virtual iScriptObject* __stdcall CompileString(
    const achar* aaszCode, const achar* aaszName = NULL) = 0;
  //! Compile the given file and get the specified IUnknown object.
  virtual iUnknown* __stdcall CompileGetIUnknownObject(
  ...
  virtual tBool __stdcall Run(iScriptObject* apThis,
                              const achar* aaszCommand) = 0;
  //! Collect garbage.
  ...
  virtual eScriptRaiseErrorMode __stdcall SetRaiseErrorMode(
    eScriptRaiseErrorMode aMode) = 0;
  ...
  virtual tBool __stdcall RegisterFunction(const sMethodDef* apFunction,
                                           const achar* aaszName = NULL) = 0;
  //! @}
  ...
  virtual tInterfaceCVec<iFileSystem>* __stdcall GetImportFileSystems()
    const = 0;
  ...
  virtual iScriptObject* __stdcall CreateTable(iScriptObject* apDelegate = NULL,
                                               tI32 anNumPop = 1) = 0;
  //! Create a new script object.
  virtual iScriptObject* __stdcall CreateObject(tI32 anIndex = eInvalidHandle,
                                                tI32 anNumPop = 1) = 0;
  ...
  virtual iScriptObject* __stdcall CreateObjectGet(
    const achar* aaszKey,
    eScriptObjectType aRequiredType = eScriptObjectType_Last, tI32 anNumPop = 1,
    tBool abTry = eFalse) = 0;
  //! @}
  ...
  //! Write a closure.
  virtual tBool __stdcall WriteClosure(iFile* apFile,
                                       iScriptObject* apObject) = 0;
  //! Read a closure.
  ...
  //! {NoAutomation}
  virtual tBool __stdcall ScriptCall(iScriptObject* apThis,
                                     const achar* aaszFunc, const Var* apParams,
                                     tU32 anNumParams, Var* apRet) = 0;
  ...
  virtual Ptr<iScriptObject> __stdcall ScriptVar(iScriptObject* apThis,
                                                 const achar* aaszVar,
                                                 tBool abTry) = 0;

  //! Evaluate the specified string.
  virtual tBool __stdcall EvalString(iHString* ahspContext,
                                     const ni::achar* aaszCode) = 0;
...
niExportFunc(ni::iUnknown*) New_niScript_ScriptVM(const ni::Var&,
                                                  const ni::Var&);
...
///////////////////////////////////////////////
inline tBool ScriptCall(iScriptVM* apVM, iScriptObject* apThis,
                        const achar* aaszFunc, const Var* apParams,
                        tU32 anNumParams, Var* apRet)
{
  if (!apVM) {
    apVM = ni::QueryInterface<iScriptVM>(
      ni::GetLang()->GetGlobalInstance("niLang.ScriptVM"));
    if (!apVM) {
...
///////////////////////////////////////////////
inline Ptr<iScriptObject> ScriptVar(iScriptVM* apVM, iScriptObject* apThis,
...
///////////////////////////////////////////////
inline ni::tBool ScriptAddImportDir(ni::iScriptVM* apVM,
                                    ...
                                    ni::iFileSystem* apFS = ni::GetRootFS())
{
  ...
  if (apFS->FileExists(absPath.Chars(), eFileAttrFlags_AllDirectories)) {
    tInterfaceCVec<iFileSystem>& importFileSystems =
      *apVM->GetImportFileSystems();
    ...
    niLoopr (ri, importFileSystems.size()) {
      ...
      if (fs.IsOK()) {
        ...
        {
          niWarning(
            niFmt("Include directory '%s' already added.", absPath.Chars()));
          return ni::eTrue;
        }
      }
    }
    ...
    niWarning(niFmt("Can't add script import directory '%s', it doesn't exist.",
                    absPath.Chars()));
    return ni::eFalse;
  }
}
...
inline ni::tBool ScriptAddToolkitImportDir(
  ni::iScriptVM* apVM, const char* aTKName, const char* aDirInTK,
  ni::iFileSystem* apFS = ni::GetRootFS())
{
...
///////////////////////////////////////////////
inline ni::Ptr<iScriptVM> ScriptCreateOrGetDefaultVM(
  ni::iFileSystem* apFS = ni::GetRootFS())
{
