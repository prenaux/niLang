...
{
  if (nb >= m_max_blocks) {
    agg_real** new_coords = new agg_real*[(m_max_blocks + block_pool) * 2];

    unsigned char** new_cmds =
      (unsigned char**)(new_coords + m_max_blocks + block_pool);

    if (m_coord_blocks) {
      memcpy(new_coords, m_coord_blocks, m_max_blocks * sizeof(agg_real*));
  ...
  m_coord_blocks[nb] =
    new agg_real[block_size * 2 +
                 block_size / (sizeof(agg_real) / sizeof(unsigned char))];
...
//------------------------------------------------------------------------
void path_storage::arc_to(agg_real rx, agg_real ry, agg_real angle,
                          bool large_arc_flag, bool sweep_flag, agg_real x,
                          agg_real y)
{
...
//------------------------------------------------------------------------
void path_storage::arc_rel(agg_real rx, agg_real ry, agg_real angle,
                           bool large_arc_flag, bool sweep_flag, agg_real dx,
                           agg_real dy)
{
  rel_to_abs(&dx, &dy);
...
//------------------------------------------------------------------------
void path_storage::curve3(agg_real x_ctrl, agg_real y_ctrl, agg_real x_to,
                          agg_real y_to)
{
...
//------------------------------------------------------------------------
void path_storage::curve4(agg_real x_ctrl1, agg_real y_ctrl1, agg_real x_ctrl2,
                          agg_real y_ctrl2, agg_real x_to, agg_real y_to)
{
...
//------------------------------------------------------------------------
void path_storage::curve4(agg_real x_ctrl2, agg_real y_ctrl2, agg_real x_to,
                          agg_real y_to)
{
  agg_real x0 = 0;
  agg_real y0 = 0;
...
//------------------------------------------------------------------------
unsigned path_storage::perceive_polygon_orientation(unsigned start,
                                                    unsigned end)
{
...
void path_storage::invert_polygon(unsigned start, unsigned end)
{
  ...
  while (end > start) {
    ...
    agg_real* start_ptr =
      m_coord_blocks[start_nb] + ((start & block_mask) << 1);
    ...
    *end_ptr = tmp_xy;
    ...
    m_cmd_blocks[start_nb][start & block_mask] =
      m_cmd_blocks[end_nb][end & block_mask];
                                                   ...
                                                   path_flags_e orientation)
{
  ...
  while (start < m_total_vertices && !is_vertex(command(start)))
    ++start;

  // Skip all insignificant move_to
  while (start + 1 < m_total_vertices && is_move_to(command(start)) &&
         is_move_to(command(start + 1)))
    ++start;

  // Find the last vertex
  unsigned end = start + 1;
  while (end < m_total_vertices && !is_next_poly(command(end)))
    ++end;
