...
#if (defined(__USE_ISOC99) || (defined(_MSL_C99) && (_MSL_C99 == 1))) && \
  !defined(EA_COMPILER_IS_C99)
  #define EA_COMPILER_IS_C99 1
#endif
...
#if defined(__GNUC__)
  #if (((__GNUC__ * 100) + __GNUC_MINOR__) >= \
       302) // Also, GCC defines _HAS_C9X.
    #define EA_COMPILER_HAS_C99_TYPES \
      1 // The compiler is not necessarily a C99 compiler, but it defines C99 types.
      ...
      #define __STDC_CONSTANT_MACROS \
        1 // This tells the GCC compiler that we want it to use its native C99 types.
    #endif
  #endif
#endif
...
//
  ...
  #define EA_COMPILER_INTMAX_SIZE \
    16 // intmax_t is __int128_t (GCC extension) and is 16 bytes.
#else
...
//
#if defined( \
  __GNUC__) // We can't use GCC 4's __builtin_offsetof because it mistakenly complains about non-PODs that are really PODs.
  #define EA_OFFSETOF(struct_, member_)                              \
    ((size_t)(((uintptr_t) & reinterpret_cast<const volatile char&>( \
                               (((struct_*)65536)->member_))) -      \
              65536))
#else
...
#if !defined( \
  EA_ALIGN_MAX) // If the user hasn't globally set an alternative value...
  #if defined( \
    EA_PROCESSOR_ARM) // ARM compilers in general tend to limit automatic variables to 8 or less.
    #define EA_ALIGN_MAX_STATIC 1048576
    #define EA_ALIGN_MAX_AUTOMATIC \
      1 // Typically they support only built-in natural aligment types (both arm-eabi and apple-abi).
  #elif defined(EA_PLATFORM_APPLE)
    #define EA_ALIGN_MAX_STATIC 1048576
    #define EA_ALIGN_MAX_AUTOMATIC 16
  #else
    #define EA_ALIGN_MAX_STATIC \
      1048576 // Arbitrarily high value. What is the actual max?
...
// is still used, but the user will need to use EA_POSTFIX_ALIGN before the constructor parameters.
  ...
  #define EA_ALIGNED(variable_type, variable, n) \
    variable_type variable __attribute__((aligned(n)))
...
// Metrowerks does not support packed alignment attributes.
#elif defined(EA_COMPILER_INTEL) || defined(CS_UNDEFINED_STRING) || \
  (defined(EA_COMPILER_MSVC) && (EA_COMPILER_VERSION >= 1300))
  ...
  #define EA_ALIGNED(variable_type, variable, n) \
    EA_ALIGN(n) variable_type variable
...
// Arm brand compiler
  ...
  #define EA_POSTFIX_ALIGN(n)
  #define EA_ALIGNED(variable_type, variable, n) \
    __align(n) variable_type variable
...
//
  ...
  #if defined(__GNUC__) && \
    !defined(              \
      __EDG__) // EDG typically #defines __GNUC__ but doesn't implement init_priority.
...
//
    ...
    #define EA_ANALYSIS_ASSUME(x) \
      __analysis_assume(!!(       \
        x)) // !! because that allows for convertible-to-bool in addition to bool.
  #else
    #define EA_ANALYSIS_ASSUME(x)
  #endif
#endif
...
//
    ...
    #define EA_DISABLE_VC_WARNING(w) \
      __pragma(warning(push)) __pragma(warning(disable : w))
  #else
    #define EA_DISABLE_VC_WARNING(w)
  #endif
#endif
...
//
    ...
    #define EA_ENABLE_VC_WARNING_AS_ERROR(w) \
      __pragma(warning(push)) __pragma(warning(error : w))
  #else
    #define EA_ENABLE_VC_WARNING_AS_ERROR(w)
  #endif
#endif
  ...
  #if defined(_MSC_VER)
    #define EA_DISABLE_VC_WARNING_AS_ERROR() __pragma(warning(pop))
  #else
...
//
  ...
  #if defined(EA_COMPILER_GNUC) && \
    (EA_COMPILER_VERSION >=        \
     4006) // Can't test directly for __GNUC__ because some compilers lie.
    #define EA_DISABLE_GCC_WARNING(w) \
      _Pragma("GCC diagnostic push") _Pragma(EAGCCWHELP2(w))
  #elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004)
    #define EA_DISABLE_GCC_WARNING(w) _Pragma(EAGCCWHELP2(w))
  #else
    #define EA_DISABLE_GCC_WARNING(w)
  #endif
#endif
    ...
    #define EA_RESTORE_GCC_WARNING() _Pragma("GCC diagnostic pop")
  #else
...
//
    ...
    #define EA_ENABLE_GCC_WARNING_AS_ERROR(w) \
      _Pragma("GCC diagnostic push") _Pragma(EAGCCWERRORHELP2(w))
  #elif defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4004)
    #define EA_ENABLE_GCC_WARNING_AS_ERROR(w) _Pragma(EAGCCWERRORHELP2(w))
  #else
    #define EA_ENABLE_GCC_WARNING_AS_ERROR(w)
  #endif
#endif
  ...
  #if defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4006)
    #define EA_DISABLE_GCC_WARNING_AS_ERROR() _Pragma("GCC diagnostic pop")
  #else
...
//
    ...
    #define EA_DISABLE_CLANG_WARNING(w) \
      _Pragma("clang diagnostic push") _Pragma(EACLANGWHELP2(w))
  #else
    #define EA_DISABLE_CLANG_WARNING(w)
  #endif
#endif
  ...
  #if defined(EA_COMPILER_CLANG) || defined(EA_COMPILER_CLANG_CL)
    #define EA_RESTORE_CLANG_WARNING() _Pragma("clang diagnostic pop")
  #else
...
//
    ...
    #define EA_ENABLE_CLANG_WARNING_AS_ERROR(w) \
      _Pragma("clang diagnostic push") _Pragma(EACLANGWERRORHELP2(w))
  #else
    #define EA_ENABLE_CLANG_WARNING_AS_ERROR(w)
  #endif
#endif
  ...
  #if defined(EA_COMPILER_CLANG) || defined(EA_COMPILER_CLANG_CL)
    #define EA_DISABLE_CLANG_WARNING_AS_ERROR() _Pragma("clang diagnostic pop")
  #else
...
//
  ...
  // EDG-based compilers are inconsistent in how the implement warning pragmas.
    ...
    #define EA_DISABLE_EDG_WARNING(w) \
      _Pragma("control %push diag") _Pragma(EAEDGWHELP1(w))
  #else
    #define EA_DISABLE_EDG_WARNING(w)
  #endif
#endif
  ...
  #if defined(EA_COMPILER_EDG) && !defined(EA_COMPILER_INTEL) && \
    !defined(EA_COMPILER_RVCT)
    #define EA_RESTORE_EDG_WARNING() _Pragma("control %pop diag")
  #else
...
//
    ...
    (defined CS_UNDEFINED_STRING && CS_UNDEFINED_STRING >= 0x4200)
...
//
  ...
  //     IBM Visual Age
    ...
    (defined(EA_COMPILER_CLANG) && defined(EA_PLATFORM_WINDOWS))
    ...
    (!defined(_WCHAR_T) &&          \
     (__EDG_VERSION__ <             \
      400)) // EDG prior to v4 uses _WCHAR_T to indicate if wchar_t is native. v4+ may define something else, but we're not currently aware of it.
    #define EA_WCHAR_T_NON_NATIVE 1
  #endif
#endif
...
#ifndef EA_RESTRICT
  #if defined(EA_COMPILER_MSVC) && \
    (EA_COMPILER_VERSION >= 1400) // If VC8 (VS2005) or later...
    #define EA_RESTRICT __restrict
  #elif defined(EA_COMPILER_CLANG)
    #define EA_RESTRICT __restrict
  #elif defined( \
    EA_COMPILER_GNUC) // Includes GCC and other compilers emulating GCC.
    #define EA_RESTRICT \
      __restrict // GCC defines 'restrict' (as opposed to __restrict) in C99 mode only.
  #elif defined(EA_COMPILER_ARM)
    #define EA_RESTRICT __restrict
...
//
    ...
    (EA_COMPILER_VERSION > 1300) // If VC7 (VS2003) or later...
...
//
  ...
  #if defined(EA_COMPILER_MSVC)
    #define EA_PASCAL_FUNC(funcname_and_paramlist) \
      __stdcall funcname_and_paramlist
  #elif defined(EA_COMPILER_GNUC) && defined(EA_PROCESSOR_X86)
    #define EA_PASCAL_FUNC(funcname_and_paramlist) \
      __attribute__((stdcall)) funcname_and_paramlist
  #else
...
// equal to 3 really only indicates that SSE3 or greater is supported.
    ...
    #define EA_SSE 2
  #elif defined(EA_PROCESSOR_X86) && defined(_MSC_FULL_VER) && \
    !defined(__NOSSE__) && defined(_M_IX86_FP)
    #define EA_SSE _M_IX86_FP
  #elif defined(EA_PROCESSOR_X86) && defined(EA_COMPILER_INTEL) && \
    !defined(__NOSSE__)
    #define EA_SSE 1
...
//
#ifndef EA_ABM
  #if defined(__ABM__) || defined(EA_PLATFORM_XBOXONE) || \
    defined(EA_PLATFORM_PS4)
    #define EA_ABM 1
  #else
    #define EA_ABM 0
  #endif
...
//    #endif
//
#if defined(_MSC_VER) || defined(__GNUC__) || defined(__EDG__) || \
  defined(__APPLE__)
...
#ifndef EA_SEALED
  #if defined(EA_COMPILER_MSVC) && \
    (EA_COMPILER_VERSION >= 1400) // VS2005 (VC8) and later
...
//
  ...
  #if defined(EA_COMPILER_MSVC) && \
    (EA_COMPILER_VERSION >= 1300) // VS2003 (VC7) and later
    ...
    #define EA_NON_COPYABLE(EAClass_)                                 \
     private:                                                         \
      EA_DISABLE_VC_WARNING(                                          \
        4822); /* local class member function does not have a body	*/ \
...
//  };
//
#define EA_DISABLE_COPY_CTOR(ClassName) \
  ClassName(const ClassName&) EA_FUNCTION_DELETE
...
//  };
//
#define EA_DISABLE_MOVE_CTOR(ClassName) \
  ClassName(ClassName&&) EA_FUNCTION_DELETE
...
//  };
//
#define EA_DISABLE_ASSIGNMENT_OPERATOR(ClassName) \
  ClassName& operator=(const ClassName&) EA_FUNCTION_DELETE
...
//  };
//
#define EA_DISABLE_MOVE_OPERATOR(ClassName) \
  ClassName& operator=(ClassName&&) EA_FUNCTION_DELETE
...
struct EANonCopyable {
    ...
    defined(                                         \
      __EDG__) // EDG doesn't appear to behave properly for the case of defaulted constructors; it generates a mistaken warning about missing default constructors.
  EANonCopyable()
  {
  } // Putting {} here has the downside that it allows a class to create itself,
  ~EANonCopyable()
  {
  } // but avoids linker errors that can occur with some compilers (e.g. Green Hills).
  #else
...
//
    ...
    (defined(__i386__) ||                                            \
     defined(                                                        \
       __x86_64__)) // GCC 4.4+ - Seems to work only on x86/Linux so far. However, GCC 4.4 itself appears broken and screws up parameter passing conventions.
    #define EA_OPTIMIZE_OFF() \
      _Pragma("GCC push_options") _Pragma("GCC optimize 0")
  #elif defined(EA_COMPILER_CLANG) && \
