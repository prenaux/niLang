...
static ni::eProfilerReportMode displayed_quantity;
...
void Prof_init_highlevel()
{
  ...
  displayed_quantity = ni::eProfilerReportMode_SelfTime;
  ...
  #ifdef Prof_ENABLED
static Prof_Zone* expand = &niProf_region__global;
  #else
static Prof_Zone* expand = NULL;
  #endif

void Prof_set_report_mode(ni::eProfilerReportMode desired)
{
...
static void update_history(Prof_Zone_Stack* c)
{
  ...
  if (record == NULL) {
    record = (Profile_Tracker_Data_Record*)niMalloc(sizeof(*record));
...
void Prof_update(int record_data)
{
  #ifdef Prof_ENABLED
  niProf_Begin(iprof_update);

  static History_Scalar int_timestamps_per_second;
  static ni::tI64 last_int_timestamp;
  static ni::tI64 current_int_timestamp;

  int i;
  double now, dt;
  ni::tI64 timestamp_delta;
  ...
  for (i = 0; i < NUM_PROFILE_TRACKER_HISTORY_SLOTS; i++) {
    precomputed_factors[i] =
      ni::Pow(0.1f, (float)(dt / times_to_reach_90_percent[i]));
  }

  precomputed_factors[0] = 0; // instantaneous.

  Prof_get_timestamp(&current_int_timestamp);
  if (update_index == 0) {
    sum = 0;
    Prof_traverse(sum_times);
    if (sum == 0)
      sum = 1;
    timestamp_delta = (ni::tI64)sum;
  }
  else {
    timestamp_delta = current_int_timestamp - last_int_timestamp;
    if (timestamp_delta == 0)
      timestamp_delta = 1;
  }

  last_int_timestamp = current_int_timestamp;
  timestamps_per_second = (double)timestamp_delta / dt;

  if (update_index < NUM_THROWAWAY_UPDATES) {
    eternity_set(&int_timestamps_per_second, timestamps_per_second);
  }
  else {
    update(&int_timestamps_per_second, timestamps_per_second,
           precomputed_factors);
  }

  {
    ...
    ss_val = int_timestamps_per_second.values[ss_slot];
    ss_variance =
      int_timestamps_per_second.variances[ss_slot] - ss_val * ss_val;
...
{
  int i;
  Prof_Report* pob = (Prof_Report*)niMalloc(sizeof(*pob));
  pob->num_record = n;
  pob->record =
    (Prof_Report_Record*)niMalloc(sizeof(*pob->record) * pob->num_record);
...
static void propogate_to_zone(Prof_Zone_Stack* c)
{
  ...
  #endif
    if (displayed_quantity == ni::eProfilerReportMode_HierTime) {
...
Prof_Report* Prof_create_report(void)
{
  ...
  if (displayed_quantity == ni::eProfilerReportMode_CallGraph)
    s = 3;
  else
    s = 1;
  ...
  avg_frame_time = get_value(&frame_time); //frame_time.values[slot];
  if (avg_frame_time == 0)
  ...
  case ni::eProfilerReportMode_SelfTime:
    displayed_quantity_name = "sort self";
    break;
  case ni::eProfilerReportMode_HierTime:
    displayed_quantity_name = "sort hier";
    break;
  case ni::eProfilerReportMode_CallGraph:
    displayed_quantity_name = "call graph";
    break;
  }

  pob->title[0] = (char*)niMalloc(BUFSIZ);
  ...
  if (speedstep_warning)
    pob->title[1] = ni::StrDupModule(
      "WARNING: SpeedStep-like timer inconsistencies detected.  Results are unreliable.");
  ...
  if (displayed_quantity == ni::eProfilerReportMode_CallGraph) {
    ...
    for (i = 0; i < pob->num_record; ++i) {
      if (pob->record[i].values[0] || pob->record[i].values[1] ||
  ...
  pob->header[0] = ni::StrDupModule("zone");
  if (displayed_quantity == ni::eProfilerReportMode_HierTime) {
    pob->header[1] = ni::StrDupModule("hier");
    pob->header[2] = ni::StrDupModule("self");
  }
  else {
    pob->header[1] = ni::StrDupModule("self");
    pob->header[2] = ni::StrDupModule("hier");
  }
  pob->header[3] = ni::StrDupModule("count");
...
void Prof_move_cursor(int num)
{
  cursor += num;
}

void Prof_set_cursor(int num)
{
  cursor = num;
}
...
void Prof_select(void)
{
  ...
  if (b->hilight >= 0) {
    ...
    if (z != NULL) {
      expand = (Prof_Zone*)z;
      displayed_quantity = ni::eProfilerReportMode_CallGraph;
    }
  }
  Prof_free_report(b);
  update_cursor = 1;
                ...
                void* data)
{
  ...
  for (i = 0; i < Prof_num_zones; ++i) {
    if (h >= num_frames) {
      ...
      callback(id(Prof_zones[i]), 0, num_frames - h,
               &zone_history[i][NUM_FRAME_SLOTS - (num_frames - h)], data);
    }
  }
  ...
  if (display_frame != 0) {
    float value[2] = { 2.0, 0 };
    callback(0, NUM_FRAME_SLOTS - display_frame,
             NUM_FRAME_SLOTS - display_frame, value, data);
  }
  #endif
}
