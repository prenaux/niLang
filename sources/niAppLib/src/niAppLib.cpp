...
#define APPLIB_DEFAULT_WINDOW_TITLE \
  (niIsStringOK(aaszTitle) ? aaszTitle : "niAppLib")
...
struct AppWindow
    : public ImplRC<iMessageHandler, eImplFlags_Default, iWidgetSink> {
 private:
  AppWindow(AppWindow&);
 ...
 public:
  ...
  AppWindow(astl::non_null<AppContext*> apContext)
      : mpContext(apContext)
  ...
  virtual tBool __stdcall OnStartup(iOSWindow* apWnd, iRunnable* apOnStarted)
  {
    ...
    mpContext->_window->SetRefreshTimer(
      mpContext->_config.windowRefreshTimerForeground);
    ...
    ni::GetLang()->SetGlobalInstance(niGetInterfaceID(iOSWindow),
                                     mpContext->_window);

    // Initialize graphics
    mpContext->_graphics =
      (iGraphics*)niCreateInstance(niUI, Graphics, niVarNull, niVarNull);
    niCheck(mpContext->_graphics.IsOK(), eFalse);

    niCheck(
      mpContext->_graphics->InitializeDriver(mpContext->_config.graphicsDriver),
      eFalse);

    mpContext->_graphics->RegisterSystemFonts();

    mpContext->_graphicsContext = mpContext->_graphics->CreateContextForWindow(
      mpContext->_window, _A("R8G8B8A8"), _A("D24S8"),
      mpContext->_config.swapInterval, mpContext->_config.backBufferFlags);
    niCheck(mpContext->_graphicsContext.IsOK(), eFalse);

    // Initialize the UI context
    mpContext->_uiContext = (iUIContext*)niCreateInstance(
      niUI, UIContext, mpContext->_graphicsContext.ptr(),
      mpContext->GetContentsScale());
    niCheck(mpContext->_uiContext.IsOK(), eFalse);
    ni::GetLang()->SetGlobalInstance(niGetInterfaceID(iUIContext),
                                     mpContext->_uiContext);
  ...
  virtual tBool __stdcall OnUpdate()
  {
    ...
    mpContext->_window->UpdateWindow(
      mpContext->_config.backgroundUpdate ? false : true);
  ...
  void __stdcall _Paint()
  {
    niProf(AppLib_Paint);
    if ((!mpContext->_uiContext.IsOK()) ||
        (!mpContext->_graphicsContext.IsOK()))
      return;
  ...
  virtual tBool __stdcall OnWindowMessage(const tU32 anMsg, const Var& avarA,
                                          const Var& avarB)
  {
    switch (anMsg) {
    ...
    case eOSWindowMessage_KeyUp: {
      switch (avarA.mU32) {
      case eKey_LControl: niFlagOff(mnDebugKeyMod, eKeyMod_Control); break;
      case eKey_LAlt: niFlagOff(mnDebugKeyMod, eKeyMod_Alt); break;
      case eKey_LShift: niFlagOff(mnDebugKeyMod, eKeyMod_Shift); break;
      }
      break;
    }
    case eOSWindowMessage_KeyDown:
      switch (avarA.mU32) {
      case eKey_LControl: niFlagOn(mnDebugKeyMod, eKeyMod_Control); break;
      case eKey_LAlt: niFlagOn(mnDebugKeyMod, eKeyMod_Alt); break;
      case eKey_LShift: niFlagOn(mnDebugKeyMod, eKeyMod_Shift); break;
      default: {
        ...
        case eKey_R | (eKeyMod_Control | eKeyMod_Alt | eKeyMod_Shift): {
          ni::GetProf()->SetReportMode(
            (eProfilerReportMode)((((ni::tU32)ni::GetProf()->GetReportMode()) +
                                   1) %
  ...
  virtual tBool __stdcall OnRootMessage(iWidget* w, tU32 msg, const Var& a,
                                        const Var& b)
  {
    return eFalse;
  }
  ...
  virtual tBool __stdcall OnWidgetSink(iWidget* w, tU32 msg, const Var& a,
                                       const Var& b) niOverride
  {
        ...
        if (msg == eUIMessage_ContextAfterDraw) {
          if (mpContext->_config.drawFPS) {
            ...
            if (!mptrFontHUD.IsOK()) {
              ...
              mptrFontHUD->SetSizeAndResolution(
                sVec2f::Zero(), 16, w->GetUIContext()->GetContentsScale());
            }

            static const tU32 consoleTextColor =
              ULColorBuild(200, 200, 80, 255);
            ...
            if (mpContext->_config.drawFPS >= 2) {
              Ptr<iProfDraw> drawer =
                mpContext->_uiContext->CreateProfDraw(canvas, font);
              ni::GetProf()->DrawGraph(
                drawer, 5, widgetRect.GetHeight() - fontHeight, 2, 10);

              const tF32 consoleW = (mpContext->_config.drawFPS >= 3)
...

              {
                ...
                const sRectf rect(widgetRect.GetLeft(), widgetRect.GetTop(),
                                  consoleW, h);
                ...
                niLoop (i, lastLogs.size()) {
                  canvas->BlitText(
                    font, r, eFontFormatFlags_ClipH | eFontFormatFlags_Border,
                    lastLogs[i].Chars());
              ...
              if (mpContext->_config.drawFPS >= 3) {
                ni::GetProf()->DrawTable(drawer, consoleW + 5, 5,
            ...
            if (canvas.IsOK()) {
              ...
              cString fpsText =
                niFmt("fps: %g (ms: %d) liveAlloc: %d (objs: %d) ren: %s",
                      ni::GetLang()->GetAverageFrameRate(),
                      (ni::GetLang()->GetFrameTime() * 1000.0),
                      memStats.x - memStats.y, memStats.z - memStats.w,
              ...
              const cString buildText =
                ni::GetLang()->GetProperty("ni.app.build_text");
              if (!buildText.empty() &&
                  ((textSize.GetRight() +
                    buildText.length() * font->GetMaxCharWidth()) <
                   widgetRect.GetWidth()))
              {
                canvas->BlitText(
                  font,
                  Vec4f(5, widgetRect.GetHeight() - fontHeight - 5,
                        widgetRect.GetWidth() - 5, widgetRect.GetHeight() - 5),
                  eFontFormatFlags_Right, buildText.Chars());
              }
              canvas->Flush();
            }
          }
                                       ...
                                       const Var& avarB) niOverride
  {
    ...
    if (msgId == eOSWindowMessage_Close) {
      ...
      mpContext->_window->SetRefreshTimer(
        mpContext->_config.windowRefreshTimerBackground);
    }
...
tBool AppStartup(astl::non_null<AppContext*> apContext, iOSWindow* apWindow,
                 iRunnable* apOnStarted, iRunnable* apOnShutdown)
{
  niTry {
    ...
    if (ni::GetLang()->HasProperty("windowRefreshTimerForeground")) {
      apContext->_config.windowRefreshTimerForeground =
        ni::GetLang()->GetProperty("windowRefreshTimerForeground").Float();
    }
    if (ni::GetLang()->HasProperty("windowRefreshTimerBackground")) {
      apContext->_config.windowRefreshTimerBackground =
        ni::GetLang()->GetProperty("windowRefreshTimerBackground").Float();
    }
    ...
    if (ni::GetLang()->HasProperty("fixedFPS")) {
      apContext->_config.fixedFPS =
        ni::GetLang()->GetProperty("fixedFPS").Long();
    }
    if (ni::GetLang()->HasProperty("drawFPS")) {
      apContext->_config.drawFPS = ni::GetLang()->GetProperty("drawFPS").Long();
    }
    else {
      ni::GetLang()->SetProperty("drawFPS",
                                 niFmt("%d", apContext->_config.drawFPS));
    }
    if (ni::GetLang()->HasProperty("renderer")) {
      apContext->_config.graphicsDriver =
        _H(ni::GetLang()->GetProperty("renderer"));
    }
    if (ni::GetLang()->HasProperty("swapinterval")) {
      apContext->_config.swapInterval =
        ni::GetLang()->GetProperty("swapinterval").Long();
    }
    if (ni::GetLang()->HasProperty("noaa")) {
      const ni::tBool bNoAA =
        ni::GetLang()->GetProperty("noaa").Bool(ni::eTrue);
      if (bNoAA) {
        apContext->_config.backBufferFlags &= ~eTextureFlags_RTAA4Samples;
      }
    }
    if (ni::GetLang()->HasProperty("windowRect")) {
      apContext->_config.windowCentered =
        ni::GetLang()->GetProperty("windowCentered").Bool(eFalse);
      apContext->_config.windowRect =
        ni::sRectf(ni::GetLang()
                     ->Eval(ni::GetLang()->GetProperty("windowRect").Chars())
                     ->GetVec4())
          .ToInt();
    }
    ...
    if (!((AppWindow*)apContext->_appWnd.ptr())
           ->OnStartup(apWindow, apOnStarted))
    {
...
void AppGenericResize(astl::non_null<AppContext*> apContext, ni::tI32 w,
                      ni::tI32 h)
{
...
void AppGenericKey(astl::non_null<AppContext*> apContext, eKey aKey,
                   tBool abIsDown)
{
  if (!apContext->_appWnd.IsOK())
    return;
  ...
  if (gw.IsOK()) {
    gw->GenericSendMessage(abIsDown ? eOSWindowMessage_KeyDown
                                    : eOSWindowMessage_KeyUp,
                           aKey, niVarNull);
  }
}

void AppGenericInputString(astl::non_null<AppContext*> apContext,
                           const ni::achar* aaszString)
{
  if (!apContext->_appWnd.IsOK())
    return;
...
void AppGenericFingerMove(astl::non_null<AppContext*> apContext, tI32 fingerId,
                          tF32 x, tF32 y, tF32 pressure)
{
  if (!apContext->_appWnd.IsOK())
    return;
  ...
  QPtr<iOSWindowGeneric> gw = apContext->_window.ptr();
  if (gw.IsOK()) {
    gw->GenericSendMessage(eOSWindowMessage_FingerMove, fingerId,
                           Vec3f(x, y, pressure));
  }
}

void AppGenericFingerRelativeMove(astl::non_null<AppContext*> apContext,
                                  tI32 fingerId, tF32 rx, tF32 ry,
                                  tF32 pressure)
{
  if (!apContext->_appWnd.IsOK())
    return;
  ...
  QPtr<iOSWindowGeneric> gw = apContext->_window.ptr();
  if (gw.IsOK()) {
    gw->GenericSendMessage(eOSWindowMessage_FingerRelativeMove, fingerId,
                           Vec3f(rx, ry, pressure));
  }
}

void AppGenericFingerPress(astl::non_null<AppContext*> apContext, tI32 fingerId,
                           tBool isDown, tF32 x, tF32 y, tF32 pressure)
{
  if (!apContext->_appWnd.IsOK())
    return;
  ...
  QPtr<iOSWindowGeneric> gw = apContext->_window.ptr();
  if (gw.IsOK()) {
    gw->GenericSendMessage(isDown ? eOSWindowMessage_FingerDown
                                  : eOSWindowMessage_FingerUp,
                           fingerId, Vec3f(x, y, pressure));
  }
}
...
void AppGenericPinch(astl::non_null<AppContext*> apContext, tF32 scale,
...
tBool AppGenericStartup(astl::non_null<AppContext*> apContext,
                        const achar* aaszTitle, ni::tI32 anWidth,
                        ni::tI32 anHeight, iRunnable* apOnStarted,
                       ...
                       ni::iRunnable* apOnShutdown)
{
  ...
  niTry {
    ...
    Ptr<iOSWindow> ptrWindow = ni::GetLang()->CreateWindow(
      NULL, APPLIB_DEFAULT_WINDOW_TITLE, r, 0, apContext->_config.windowStyle);
    if (!ptrWindow.IsOK()) {
      niError("Can't create window.");
      return eFalse;
    }
  ...
  niTry {
    for (;;) {
      ...
      if (apContext->_config.maxFPS != 0) {
        const tF64 sleepSecs =
          (1.0 / (tF64)apContext->_config.maxFPS) - frameTime;
        if (sleepSecs > 0.0) {
