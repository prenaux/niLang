...
class fixed_substring : public basic_string<T> {
 ...
 private:
  void SetInternalHeapLayout(value_type* pBeginPtr, size_type nSize,
                             size_type nCap)
  {
 ...
 public:
  ...
  fixed_substring(const base_type& x, size_type position,
                  size_type n = base_type::npos)
      : base_type()
  {
#if EASTL_NAME_ENABLED
    get_allocator().set_name(x.get_allocator().get_name());
  ...
  this_type& assign(const base_type& x)
  {
    ...
    SetInternalHeapLayout(const_cast<value_type*>(x.data()), x.size(),
                          x.size());
    return *this;
  }
  ...
  {
    // By design, we need to cast away const-ness here.
    SetInternalHeapLayout(const_cast<value_type*>(p), (size_type)CharStrlen(p),
                          (size_type)CharStrlen(p));
    return *this;
  }
  ...
  {
    // By design, we need to cast away const-ness here.
    SetInternalHeapLayout(const_cast<value_type*>(pBegin),
                          (size_type)(pEnd - pBegin),
                          (size_type)(pEnd - pBegin));
    return *this;
  }
