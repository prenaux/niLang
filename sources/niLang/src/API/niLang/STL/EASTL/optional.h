...
EA_DISABLE_VC_WARNING(
  4582 4583) // constructor/destructor is not implicitly called
...
struct bad_optional_access : public std::logic_error {
  bad_optional_access()
      : std::logic_error("eastl::bad_optional_access exception")
  {
  }
...
///
template <typename T,
          bool IsTriviallyDestructible = eastl::is_trivially_destructible_v<T>>
struct optional_storage {
  ...
  template <typename TT = T,
            typename = eastl::enable_if_t<eastl::is_copy_constructible_v<TT>>>
  inline optional_storage(const optional_storage& other)
  ...
  template <typename U, typename... Args,
            typename = eastl::enable_if_t<eastl::is_constructible_v<
              T, std::initializer_list<U>&, Args&&...>>>
  inline explicit optional_storage(in_place_t, std::initializer_list<U> ilist,
                                   Args&&... args)
      : engaged(true)
  {
    ::new (eastl::addressof(val))
      value_type{ ilist, eastl::forward<Args>(args)... };
  }
...
class optional : private Internal::optional_storage<T> {
 ...
 public:
  ...
  static_assert(!eastl::is_reference<value_type>::value,
                "eastl::optional of a reference type is ill-formed");
  static_assert(!eastl::is_same<value_type, in_place_t>::value,
                "eastl::optional of a in_place_t type is ill-formed");
  static_assert(!eastl::is_same<value_type, nullopt_t>::value,
                "eastl::optional of a nullopt_t type is ill-formed");
  ...
  inline EA_CONSTEXPR optional(value_type&& value)
    EA_NOEXCEPT_IF(eastl::is_nothrow_move_constructible_v<T>)
  ...
  template <typename U, typename... Args,
            typename = eastl::enable_if_t<eastl::is_constructible_v<
              T, std::initializer_list<U>&, Args&&...>>>
  inline explicit optional(in_place_t, std::initializer_list<U> ilist,
                           Args&&... args)
  ...
  inline optional& operator=(optional&& other) EA_NOEXCEPT_IF(
    EA_NOEXCEPT(eastl::is_nothrow_move_assignable<value_type>::value&&
  ...
  template <class U, typename = typename eastl::enable_if<
                       eastl::is_same<eastl::decay_t<U>, T>::value>::type>
  ...
  EA_CONSTEXPR inline explicit operator bool() const
  {
    return engaged;
  }

  EA_CONSTEXPR inline bool has_value() const EA_NOEXCEPT
  {
    return engaged;
  }
  ...
  {
    return engaged ? *get_value_address()
                   : static_cast<value_type>(eastl::forward<U>(default_value));
  }
  ...
  inline void swap(optional& other)
    EA_NOEXCEPT_IF(eastl::is_nothrow_move_constructible<T>::value&&
                     eastl::is_nothrow_swappable<T>::value)
  {
    using eastl::swap;
    if (engaged == other.engaged) {
      if (engaged)
      ...
      if (engaged) {
        ...
        construct_value(
          eastl::move(*((value_type*)eastl::addressof(other.val))));
...
void swap(optional<T>& lhs, optional<T>& rhs)
  EA_NOEXCEPT_IF(EA_NOEXCEPT(lhs.swap(rhs)))
{
  lhs.swap(rhs);
}
...
//
template <class T>
inline EA_CONSTEXPR bool operator==(const optional<T>& lhs,
...
//
template <class T>
inline EA_CONSTEXPR bool operator==(const optional<T>& opt,
                                    eastl::nullopt_t) EA_NOEXCEPT
{
  return !opt;
}

template <class T>
inline EA_CONSTEXPR bool operator==(eastl::nullopt_t,
                                    const optional<T>& opt) EA_NOEXCEPT
{
  return !opt;
}
...
template <class T>
inline EA_CONSTEXPR bool operator<(const optional<T>&,
                                   eastl::nullopt_t) EA_NOEXCEPT
{
  return false;
}
...
inline EA_CONSTEXPR optional<T> make_optional(std::initializer_list<U> il,
                                              Args&&... args)
{
