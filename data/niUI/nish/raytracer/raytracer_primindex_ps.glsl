...
nish_std_PixelOutput niUIGpuFuncs_raytracer_primindex_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS);
...
nish_std_PixelOutput niUIGpuFuncs_raytracer_primindex_ps(nish_std_PixelInput aInput, niUIGpuFuncs_RayUniforms aUniforms, accelerationStructureEXT aAS) {
  ...
  if (_tmp_R4) {
    ...
    if (_tmp_35) {
      ...
      {
        vec3 _tmp_85 = nish_std_UIntToHashColor(intersection.primIndex);
        color = vec4(_tmp_85.x,_tmp_85.y,_tmp_85.z,1.0);
      }
    }
  }
  else {
    bool _tmp_b5 = niUIGpuFuncs__op__lt_2__IntersectionTypeValue__IntersectionTypeValue(intersection.type,niUIGpuFuncs_IntersectionType_None);
    if (_tmp_b5) {
  ...
  vec4 _tmp_p5 = color;
  return nish_std_PixelOutput_new(_tmp_p5);
}
...
// Pixel Shader main: niUIGpuFuncs_raytracer_primindex_ps
...
void main(void) {
  ...
  nish_std_PixelOutput _rval_ = niUIGpuFuncs_raytracer_primindex_ps(aInput, aUniforms, IN_1_aAS);
