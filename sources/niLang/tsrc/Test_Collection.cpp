...
TEST_FIXTURE(FCollection, MutableVector)
{
  ...
  ni::Nonnull<ni::iMutableCollection> vec{ CreateCollectionVector(
    ni::eType_U32) };
  CHECK(vec->IsEmpty());
  CHECK(vec->GetSize() == 0);
...
TEST_FIXTURE(FCollection, MutableMap)
{
  ni::Nonnull<ni::iMutableCollection> map{
    ni::cMutableCollectionImpl<ni::CollectionTraitsMap<
      ni::CollectionTraitsString, ni::CollectionTraitsU32>>::Create()
  };
  ...
  niLoopIterator(it, map)
  {
    ...
    if (first) {
      ...
      CHECK(map->Put("99",
                     78)); // use 99 as key, cause I want it to be the last one
      CHECK(map->Remove("1"));
