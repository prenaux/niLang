...
class renderer_markers : public renderer_primitives<BaseRenderer> {
 public:
  ...
  void diamond(int x, int y, int r)
  {
    if (visible(x, y, r)) {
      if (r) {
        ...
        int dx = 0;
        do {
          base_type::ren().blend_pixel(x - dx, y + dy, base_type::line_color(),
                                       cover_full);
          base_type::ren().blend_pixel(x + dx, y + dy, base_type::line_color(),
                                       cover_full);
          base_type::ren().blend_pixel(x - dx, y - dy, base_type::line_color(),
          ...
          if (dx) {
            base_type::ren().blend_hline(x - dx + 1, y + dy, x + dx - 1,
    ...
    if (visible(x, y, r)) {
      if (r) {
        ...
        if (r <= 2)
          r6++;
        r >>= 1;
        base_type::ren().blend_hline(x - r6, y, x - r, base_type::line_color(),
                                     cover_full);
        base_type::ren().blend_hline(x + r, y, x + r6, base_type::line_color(),
                                     cover_full);
        base_type::ren().blend_vline(x, y - r6, y - r, base_type::line_color(),
                                     cover_full);
        base_type::ren().blend_vline(x, y + r, y + r6, base_type::line_color(),
                                     cover_full);
      }
      else {
        base_type::ren().blend_pixel(x, y, base_type::fill_color(), cover_full);
    ...
    if (visible(x, y, r)) {
      if (r) {
        ...
        do {
          ...
          base_type::ren().blend_pixel(x + dy, y + dx, base_type::line_color(),
                                       cover_full);
          base_type::ren().blend_pixel(x + dy, y - dx, base_type::line_color(),
                                       cover_full);

          if (ei.dy() && dx) {
            base_type::ren().blend_vline(x + dy, y - dx + 1, y + dx - 1,
                                         base_type::fill_color(), cover_full);
          }
          ++ei;
        } while (dy < r8);
        base_type::ren().blend_vline(x + dy, y - dx, y + dx,
  ...
  void semiellipse_up(int x, int y, int r)
  {
    if (visible(x, y, r)) {
      if (r) {
        ...
        base_type::ren().blend_hline(x - dx, y - dy - 1, x + dx,
                                     base_type::line_color(), cover_full);
      }
      else {
        base_type::ren().blend_pixel(x, y, base_type::fill_color(), cover_full);
  ...
  void triangle_up(int x, int y, int r)
  {
    if (visible(x, y, r)) {
      if (r) {
        ...
        base_type::ren().blend_hline(x - dx, y - dy, x + dx,
                                     base_type::line_color(), cover_full);
      }
      else {
        base_type::ren().blend_pixel(x, y, base_type::fill_color(), cover_full);
  ...
  void four_rays(int x, int y, int r)
  {
    if (visible(x, y, r)) {
      if (r) {
        ...
        base_type::solid_rectangle(x + r3 + 1, y + r3 + 1, x - r3 - 1,
                                   y - r3 - 1);
      }
      else {
        base_type::ren().blend_pixel(x, y, base_type::fill_color(), cover_full);
  ...
  void cross(int x, int y, int r)
  {
    if (visible(x, y, r)) {
      if (r) {
        base_type::ren().blend_vline(x, y - r, y + r, base_type::line_color(),
                                     cover_full);
        base_type::ren().blend_hline(x - r, y, x + r, base_type::line_color(),
                                     cover_full);
      }
      else {
        base_type::ren().blend_pixel(x, y, base_type::fill_color(), cover_full);
  ...
  void xing(int x, int y, int r)
  {
    if (visible(x, y, r)) {
      if (r) {
        int dy = -r * 7 / 10;
        do {
          base_type::ren().blend_pixel(x + dy, y + dy, base_type::line_color(),
                                       cover_full);
          base_type::ren().blend_pixel(x - dy, y + dy, base_type::line_color(),
                                       cover_full);
          base_type::ren().blend_pixel(x + dy, y - dy, base_type::line_color(),
  ...
  void marker(int x, int y, int r, marker_e type)
  {
    ...
    case end_of_markers: niPanicUnreachable("end_of_markers"); break;
    }
  }
  ...
  void markers(int n, const T* x, const T* y, T r, marker_e type)
  {
    ...
    if (r == 0) {
      do {
        base_type::ren().blend_pixel(int(*x), int(*y), base_type::fill_color(),
                                     cover_full);
        ++x;
        ++y;
      } while (--n);
  ...
  //--------------------------------------------------------------------
  template <class T>
  void markers(int n, const T* x, const T* y, const T* r, const color_type* fc,
               marker_e type)
  {
    if (n <= 0)
      return;
