...
EA_DISABLE_SN_WARNING(
  828); // The EDG SN compiler has a bug in its handling of variadic template arguments and mistakenly reports "parameter "args" was never referenced"
...
///
  ...
  #define EASTL_SLIST_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX      \
      " slist" // Unless the user overrides something, this is "EASTL slist".
#endif
...
#else
  typedef SListNodeBase
    base_node_type; // We use SListNodeBase instead of SListNode<T> because we don't want to create a T.
#endif
...
class slist : public SListBase<T, Allocator> {
 ...
 public:
  ...
  explicit slist(size_type n, const allocator_type& allocator =
                                EASTL_SLIST_DEFAULT_ALLOCATOR);
  slist(size_type n, const value_type& value,
        const allocator_type& allocator = EASTL_SLIST_DEFAULT_ALLOCATOR);
  ...
  void splice(const_iterator position, this_type& x, const_iterator first,
              const_iterator last);
  void splice(const_iterator position, this_type&& x);
  void splice(const_iterator position, this_type&& x, const_iterator i);
  void splice(const_iterator position, this_type&& x, const_iterator first,
  ...
  void splice_after(const_iterator position, this_type& x, const_iterator first,
                    const_iterator last);
  void splice_after(const_iterator position, this_type&& x);
  void splice_after(const_iterator position, this_type&& x, const_iterator i);
  void splice_after(const_iterator position, this_type&& x,
  ...
  void splice_after(
    const_iterator position, const_iterator before_first,
    const_iterator
      before_last); // before_first and before_last come from a source container.
  void splice_after(
    const_iterator position,
    const_iterator previous); // previous comes from a source container.
 ...
 protected:
  ...
  template <typename InputIterator>
  node_type* DoInsertAfter(SListNodeBase* pNode, InputIterator first,
                           InputIterator last);

  template <typename Integer>
  node_type* DoInsertAfter(SListNodeBase* pNode, Integer n, Integer value,
                           true_type);
  ...
  node_type* DoInsertValuesAfter(SListNodeBase* pNode, size_type n,
                                 const value_type& value);
...
inline SListNodeBase* SListNodeInsertAfter(SListNodeBase* pPrevNode,
                                           SListNodeBase* pNode)
{
...
inline SListNodeBase* SListNodeGetPrevious(SListNodeBase* pNodeBase,
                                           const SListNodeBase* pNode)
{
...
inline const SListNodeBase* SListNodeGetPrevious(const SListNodeBase* pNodeBase,
                                                 const SListNodeBase* pNode)
{
  while (pNodeBase && (pNodeBase->mpNext != pNode))
    pNodeBase = pNodeBase->mpNext;
...
inline void SListNodeSpliceAfter(SListNodeBase* pNode,
                                 SListNodeBase* pNodeBeforeFirst,
...
inline SListIterator<T, Pointer, Reference>::SListIterator(
  const SListNodeBase* pNode)
    : mpNode(static_cast<node_type*>((SListNode<T>*)const_cast<SListNodeBase*>(
        pNode))) // All this casting is in the name of making runtime debugging much easier on the user.
{
  // Empty
}
...
template <typename T, typename Pointer, typename Reference>
inline typename SListIterator<T, Pointer, Reference>::reference SListIterator<
...
template <typename T, typename Pointer, typename Reference>
inline typename SListIterator<T, Pointer, Reference>::pointer SListIterator<
  T, Pointer, Reference>::operator->() const
{
  return &mpNode->mValue;
}
...
template <typename T, typename Pointer, typename Reference>
inline typename SListIterator<T, Pointer, Reference>::this_type& SListIterator<
...
template <typename T, typename Allocator>
inline const typename SListBase<T, Allocator>::allocator_type& SListBase<
...
template <typename T, typename Allocator>
inline typename SListBase<T, Allocator>::allocator_type& SListBase<
...
template <typename T, typename Allocator>
void SListBase<T, Allocator>::set_allocator(const allocator_type& allocator)
{
  EASTL_ASSERT(
    (mAllocator == allocator) ||
    (static_cast<node_type*>(mNode.mpNext) ==
     NULL)); // We can only assign a different allocator if we are empty of elements.
  mAllocator = allocator;
}
...
{
  node_type* const pNodeNext =
    static_cast<node_type*>((base_node_type*)pNode->mpNext);
...
template <typename T, typename Allocator>
SListNodeBase* SListBase<T, Allocator>::DoEraseAfter(SListNodeBase* pNode,
                                                     SListNodeBase* pNodeLast)
{
  node_type* pNodeCurrent =
    static_cast<node_type*>((base_node_type*)pNode->mpNext);

  while (pNodeCurrent != (base_node_type*)pNodeLast) {
    node_type* const pNodeTemp = pNodeCurrent;
    pNodeCurrent =
      static_cast<node_type*>((base_node_type*)pNodeCurrent->mpNext);
    pNodeTemp->~node_type();
...
template <typename T, typename Allocator>
inline slist<T, Allocator>::slist(size_type n, const value_type& value,
...
{
  DoInsertAfter((SListNodeBase*)&mNode,
                const_iterator((SListNodeBase*)x.mNode.mpNext),
                const_iterator(NULL), false_type());
}
...
template <typename T, typename Allocator>
inline slist<T, Allocator>::slist(std::initializer_list<value_type> ilist,
                                  const allocator_type& allocator)
    : base_type(allocator)
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<T, Allocator>::begin()
  EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::begin()
  const EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_iterator slist<
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<T, Allocator>::end()
  EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::end()
  const EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_iterator slist<T, Allocator>::cend()
  const EA_NOEXCEPT
{
  return const_iterator(NULL);
}
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<
  T, Allocator>::before_begin() EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_iterator slist<
  T, Allocator>::before_begin() const EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_iterator slist<
  T, Allocator>::cbefore_begin() const EA_NOEXCEPT
{
  return const_iterator((SListNodeBase*)&mNode);
}

template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<T, Allocator>::previous(
  const_iterator position)
{
  return iterator(SListNodeGetPrevious((SListNodeBase*)&mNode,
                                       (SListNodeBase*)position.mpNode));
}

template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_iterator slist<
  T, Allocator>::previous(const_iterator position) const
{
  return const_iterator(SListNodeGetPrevious((SListNodeBase*)&mNode,
                                             (SListNodeBase*)position.mpNode));
}

template <typename T, typename Allocator>
inline typename slist<T, Allocator>::reference slist<T, Allocator>::front()
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::const_reference slist<
...
{
  SListNodeInsertAfter((SListNodeBase*)&mNode,
                       (SListNodeBase*)DoCreateNode(value));
#if EASTL_SLIST_SIZE_CACHE
  ++mSize;
#endif
}
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::reference slist<T, Allocator>::push_front()
{
...
template <typename T, typename Allocator>
typename slist<T, Allocator>::this_type& slist<T, Allocator>::operator=(
  const this_type& x)
{
  if (&x != this) {
...
template <typename T, typename Allocator>
template <
  typename InputIterator> // It turns out that the C++ std::list specifies a two argument
inline void slist<T, Allocator>::assign(
  InputIterator first,
  InputIterator
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::size_type slist<T, Allocator>::size() const
  EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert(
  const_iterator position)
{
  return iterator((SListNodeBase*)DoInsertValueAfter(
    SListNodeGetPrevious((SListNodeBase*)&mNode,
                         (SListNodeBase*)position.mpNode),
    value_type()));
}
...
template <typename T, typename Allocator>
inline void slist<T, Allocator>::insert(const_iterator position, size_type n,
                                        const value_type& value)
{
  // To do: get rid of DoAssignValues and put its implementation directly here.
  DoInsertValuesAfter(SListNodeGetPrevious((SListNodeBase*)&mNode,
                                           (SListNodeBase*)position.mpNode),
                      n, value);
}
...
template <typename T, typename Allocator>
template <typename InputIterator>
inline void slist<T, Allocator>::insert(const_iterator position,
                                        InputIterator first, InputIterator last)
{
  DoInsertAfter(SListNodeGetPrevious((SListNodeBase*)&mNode,
                                     (SListNodeBase*)position.mpNode),
                first, last);
}

template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<T, Allocator>::insert_after(
  const_iterator position)
{
...
{
  return iterator(
    (SListNodeBase*)DoInsertValueAfter((SListNodeBase*)position.mpNode, value));
}
...
{
  return iterator((SListNodeBase*)DoInsertValuesAfter(
    (SListNodeBase*)position.mpNode, n, value));
}
...
{
  return iterator((SListNodeBase*)DoInsertAfter(
    (SListNodeBase*)position.mpNode, ilist.begin(), ilist.end(), false_type()));
}
...
{
  return iterator((SListNodeBase*)DoInsertAfter((SListNodeBase*)position.mpNode,
                                                first, last));
}
...
template <class... Args>
inline typename slist<T, Allocator>::iterator slist<
  T, Allocator>::emplace_after(const_iterator position, Args&&... args)
{
  return iterator((SListNodeBase*)DoInsertValueAfter(
    position.mpNode, eastl::forward<Args>(args)...));
}

template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase(
  const_iterator position)
{
  return DoEraseAfter(SListNodeGetPrevious((SListNodeBase*)&mNode,
                                           (SListNodeBase*)position.mpNode));
}
...
{
  return DoEraseAfter(
    SListNodeGetPrevious((SListNodeBase*)&mNode, (SListNodeBase*)first.mpNode),
    (SListNodeBase*)last.mpNode);
}

template <typename T, typename Allocator>
inline typename slist<T, Allocator>::iterator slist<T, Allocator>::erase_after(
  const_iterator position)
{
...
{
  return iterator(DoEraseAfter((SListNodeBase*)before_first.mpNode,
                               (SListNodeBase*)last.mpNode));
}
...
void slist<T, Allocator>::remove(const value_type& value)
{
  ...
  while (pNode && pNode->mpNext) {
    if (static_cast<node_type*>(pNode->mpNext)->mValue == value)
      DoEraseAfter((SListNodeBase*)
                     pNode); // This will take care of modifying pNode->mpNext.
    else
      pNode = pNode->mpNext;
  }
}
  ...
  if (first != last) // If there is anything to splice...
  {
    if (mAllocator == x.mAllocator) {
      ...
      SListNodeSpliceAfter(
        SListNodeGetPrevious((SListNodeBase*)&mNode,
                             (SListNodeBase*)position.mpNode),
        SListNodeGetPrevious((SListNodeBase*)&x.mNode,
                             (SListNodeBase*)first.mpNode),
        SListNodeGetPrevious((SListNodeBase*)first.mpNode,
                             (SListNodeBase*)last.mpNode));
    }
    else {
                                 ...
                                 const_iterator i)
{
  return splice(
    position, x,
    i); // This will splice_after(const_iterator, this_type&, const_iterator)
}
                                 ...
                                 const_iterator first, const_iterator last)
{
  return splice(
    position, x, first,
    last); // This will splice(const_iterator, this_type&, const_iterator, const_iterator)
}

template <typename T, typename Allocator>
inline void slist<T, Allocator>::splice_after(const_iterator position,
                                              this_type& x)
{
  if (!x.empty()) // If there is anything to splice...
  {
    if (mAllocator == x.mAllocator) {
      SListNodeSpliceAfter((SListNodeBase*)position.mpNode,
                           (SListNodeBase*)&x.mNode);
...
{
  return splice_after(
    position, x); // This will call splice_after(const_iterator, this_type&)
}
                                              ...
                                              this_type&& x, const_iterator i)
{
  return splice_after(
    position, x,
    i); // This will call splice_after(const_iterator, this_type&, const_iterator)
}
                                              ...
                                              const_iterator last)
{
  return splice_after(
    position, x, first,
    last); // This will call splice_after(const_iterator, this_type&, const_iterator, const_iterator)
}
                                              ...
                                              const_iterator before_last)
{
  if (before_first != before_last) // If there is anything to splice...
  {
    ...
    if (i == iEnd) // If the input came from an external range...
      mSize += (size_type)eastl::distance(
        before_first,
        before_last); // Note that we have no way of knowing how to decrementing the size from the external container, assuming it came from one.
    else {
      EASTL_FAIL_MSG(
        "slist::splice_after: Impossible to decrement source mSize. Use the other splice_after function instead.");
    }
#endif

    // Insert the range of [before_first + 1, before_last + 1) after position.
    SListNodeSpliceAfter((SListNodeBase*)position.mpNode,
                         (SListNodeBase*)before_first.mpNode,
                         (SListNodeBase*)before_last.mpNode);
  }
}
                                              ...
                                              const_iterator previous)
{
  ...
  SListNodeSpliceAfter((SListNodeBase*)position.mpNode,
                       (SListNodeBase*)previous.mpNode,
                       (SListNodeBase*)previous.mpNode->mpNext);
}
...
{
  if (mNode.mpNext)
    mNode.mpNext = static_cast<node_type*>(
      (base_node_type*)SListNodeReverse((SListNodeBase*)mNode.mpNext));
}

template <typename T, typename Allocator>
template <typename... Args>
inline typename slist<T, Allocator>::node_type* slist<
  T, Allocator>::DoCreateNode(Args&&... args)
{
  node_type* const pNode =
    DoAllocateNode(); // pNode is of type node_type, but it's uninitialized memory.
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::node_type* slist<T,
                                                      Allocator>::DoCreateNode()
{
...
template <typename T, typename Allocator>
template <typename InputIterator>
void slist<T, Allocator>::DoAssign(InputIterator first, InputIterator last,
                                   false_type)
{
...
template <typename InputIterator>
inline typename slist<T, Allocator>::node_type* slist<
  T, Allocator>::DoInsertAfter(SListNodeBase* pNode, InputIterator first,
                               InputIterator last)
{
...
template <typename Integer>
inline typename slist<T, Allocator>::node_type* slist<
  T, Allocator>::DoInsertAfter(SListNodeBase* pNode, Integer n, Integer value,
                               true_type)
{
                               ...
                               InputIterator last, false_type)
{
  for (; first != last; ++first) {
    pNode = SListNodeInsertAfter((SListNodeBase*)pNode,
                                 (SListNodeBase*)DoCreateNode(*first));
#if EASTL_SLIST_SIZE_CACHE
    ++mSize;
#endif
  }
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::node_type* slist<
  T, Allocator>::DoInsertValueAfter(SListNodeBase* pNode)
{
#if EASTL_SLIST_SIZE_CACHE
  pNode =
    SListNodeInsertAfter((SListNodeBase*)pNode, (SListNodeBase*)DoCreateNode());
  ++mSize;
  return static_cast<node_type*>((base_node_type*)pNode);
#else
  return static_cast<node_type*>((base_node_type*)SListNodeInsertAfter(
    (SListNodeBase*)pNode, (SListNodeBase*)DoCreateNode()));
#endif
}
...
template <typename... Args>
inline typename slist<T, Allocator>::node_type* slist<
  T, Allocator>::DoInsertValueAfter(SListNodeBase* pNode, Args&&... args)
{
  SListNodeBase* pNodeNew =
    (SListNodeBase*)DoCreateNode(eastl::forward<Args>(args)...);
...
template <typename T, typename Allocator>
inline typename slist<T, Allocator>::node_type* slist<
  T, Allocator>::DoInsertValuesAfter(SListNodeBase* pNode, size_type n,
                                     const value_type& value)
{
...
template <typename T, typename Allocator>
inline bool operator<(const slist<T, Allocator>& a,
                      const slist<T, Allocator>& b)
{
