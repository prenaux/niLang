...
TEST_FIXTURE(FFileSystem, GetAbsolutePath)
{
  ...
  {
    cString abs =
      ni::GetRootFS()->GetAbsolutePath("../../niLang/data/niUI/error.dds");
  ...
  {
    cString abs =
      ni::GetRootFS()->GetAbsolutePath("./.././../niLang/data/niUI/");
    CHECK_EQUAL(dataDir + "niUI/", abs);
  }
...

  {
    ...
    cString abs =
      ni::GetRootFS()->GetAbsolutePath("a/b///c//../data/.//../niUI///");
...
TEST_FIXTURE(FFileSystem, GetAbsolutePathUnicodePath)
{
  ...
  cString abs =
    ni::GetRootFS()->GetAbsolutePath("../../niLang/data/你好αβ/error.dds");
...
TEST_FIXTURE(FFileSystem, GetAbsolutePathExpandHome)
{
  ...
  {
    cString abs =
      ni::GetRootFS()->GetAbsolutePath("~/data/你好αβ/../error.dds");
  ...
  {
    cString abs =
      ni::GetRootFS()->GetAbsolutePath("~/data/../你好/../αβ/error.dds");
...
TEST_FIXTURE(FFileSystem, RootFSCaseInsensitive)
{
  ...
  cString workDirTmp =
    cPath(workDir, AZEROSTR).AddDirectoryBack("_tmp/Test_niLang").GetPath();
  ...
  {
    Ptr<iFile> fpHello =
      ni::GetRootFS()->FileOpen(filePath.Chars(), eFileOpenMode_Write);
  ...
  CHECK_EQUAL(1, ni::GetRootFS()->FileEnum(filePath.Chars(),
                                           eFileAttrFlags_AllFiles, NULL));
  CHECK_EQUAL(1, ni::GetRootFS()->FileEnum(filePathLwr.Chars(),
                                           eFileAttrFlags_AllFiles, NULL));

  {
    ...
    CHECK_EQUAL(11,
                ff.FileSize()); // size of the test string we write in the file
  ...
  CHECK_EQUAL(
    filePath.GetPath(),
    FindFirstFilePath(cPath(workDirTmp, "hellotest.txt").Chars()).GetPath());
  CHECK(filePath.GetPath().IEq(
    FindFirstFilePath(cPath(workDirTmpLwr, "hellotest.txt").Chars())
      .GetPath()));

  // Make dir / rmdir should base case insensitive
  {
    const cPath dirPath =
      cPath(workDirTmp, NULL).AddDirectoryBack("craZZZyDir");
    const cPath dirPathLwr =
      cPath(workDirTmp, NULL).AddDirectoryBack("crazzzydir");
    ...
    CHECK(
      dirPath.GetPath().IEq(FindFirstDirPath(dirPathLwr.Chars()).GetPath()));
    {
      FindFile ff;
    ...
    CHECK_EQUAL(eTrue,
                ni::DirExists(
                  dirPathLwr.Chars())); // DirExists should be case insensitive
  ...
  CHECK_EQUAL(
    1, ni::GetRootFS()->FileExists(cPath(workDirTmp, "hellotest.txt").Chars(),
...
TEST_FIXTURE(FFileSystem, FindExeCI)
{
...

  {
    ...
    CHECK_EQUAL(
      eFalse, ni::FileExists((_ASTR("./squirrel_") + exeFileNameLwr).Chars()));
  }

  {
    ...
    CHECK_EQUAL(
      eFalse, ni::FileExists((_ASTR("./squirrel_") + exeFileNameUpr).Chars()));
  }
}
...
{
  const cString filePathA =
    UnitTest::GetTestOutputFilePath(m_testName, "fs_get_time_a.txt");
  const cString filePathB =
    UnitTest::GetTestOutputFilePath(m_testName, "fs_get_time_b.txt");
  ...
  // Test time that are the within the same second
  {
    ...
    Ptr<iFile> fp =
      ni::GetRootFS()->FileOpen(filePathA.Chars(), eFileOpenMode_Write);
    fp->WriteString("Hello test A!");
  }
  {
    ni::GetRootFS()->FileDelete(filePathB.Chars());
    Ptr<iFile> fp =
      ni::GetRootFS()->FileOpen(filePathB.Chars(), eFileOpenMode_Write);
    fp->WriteString("Hello test B!");
  }

  Ptr<iFile> fpA =
    ni::GetRootFS()->FileOpen(filePathA.Chars(), eFileOpenMode_Read);
  Ptr<iTime> ta = ni::CreateTimeZero();
  CHECK(fpA->GetTime(eFileTime_LastWrite, ta));
  Ptr<iFile> fpB =
    ni::GetRootFS()->FileOpen(filePathB.Chars(), eFileOpenMode_Read);
