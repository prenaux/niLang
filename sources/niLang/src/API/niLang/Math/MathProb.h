...
// p[n], q[n], a[n*2]
inline void ProbSampleBuildAliasMethodArrays(const tF64* p, const tSize n,
                                             tF64* q, tU32* a)
{
  niAssert(p && n >= 1);
...
inline tBool ProbSampleAliasMethod(tU32* r, const tSize nres, const tF64* q,
                                   const tU32* a, const tSize n,
...
template <typename TRNG>
inline tBool ProbSampleAliasMethod(astl::non_null<tU32CVec*> r,
                                   const tSize nres,
                                   ...
                                   TRNG&& rngGetNormalizedFloat)
{
  ...
  ProbSampleBuildAliasMethodArrays(apProbs->data(), apProbs->size(),
                                   amq->data(), ama->data());
  r->resize(nres);
  ProbSampleAliasMethod(r->data(), nres, amq->data(), ama->data(),
                        apProbs->size(), rngGetNormalizedFloat);
  return eTrue;
}
                                   ...
                                   int4* aPRNG = ni_prng_global())
{
  return ProbSampleAliasMethod(
    r, nres, apProbs, [&]() -> ni::tF64 { return ni_prng_next_f64(aPRNG); });
}
