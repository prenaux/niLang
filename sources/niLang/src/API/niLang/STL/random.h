...
#ifdef __SIZEOF_INT128__
    eastl::conditional_t<sizeof(TUINT) <= sizeof(ni::tU64), ni::tU64,
                         unsigned __int128>,
#else
    ni::tU64,
#endif // __SIZEOF_INT128__
    Xp, sizeof(TUINT) * CHAR_BIT - 1>::value;
};
...
class independent_bits_engine {
 ...
 private:
  ...
  typedef typename eastl::make_unsigned<typename eastl::conditional<
    (sizeof(engine_result_type) < sizeof(result_type)), result_type,
    engine_result_type>::type>::type working_result_type;
  ...
  static constexpr working_result_type _kRp =
    (TENGINE::max() - TENGINE::min()) + working_result_type(1);

  static constexpr ni::tSize _km = ct_log2<working_result_type, _kRp>::value;
  static constexpr ni::tSize _kWDt =
    eastl::numeric_limits<working_result_type>::digits;
  static constexpr ni::tSize _kEDt =
    eastl::numeric_limits<engine_result_type>::digits;

 public:
  independent_bits_engine(TENGINE& e, ni::tSize w)
      : _engine(e)
      , _w(w)
  {
    _n = _w / _km + (_w % _km != 0);
    ...
    _mask0 =
      _w0 > 0 ? engine_result_type(~0) >> (_kEDt - _w0) : engine_result_type(0);
...
class uniform_int_distribution {
  static_assert(eastl::is_integral<TINT>::value,
                "uniform_int_distribution: TINT must be integral.");
 ...
 public:
  ...
  uniform_int_distribution(TINT a = 0,
                           TINT b = eastl::numeric_limits<TINT>::max())
      : mParam(a, b)
  {
  }
  ...
  result_type operator()(Generator& g, const param_type& params)
  {
    ...
    if (range == 0) {
      ...
      ni::tSize bits =
        eastl::numeric_limits<uresult_type>::digits - astl::countl_zero(range);
      if ((range & (range + 1)) != 0) {
        ++bits;
      }
...
template <typename TUINT, ni::tSize w, ni::tSize n, ni::tSize m, ni::tSize r,
          TUINT a, ni::tSize u, TUINT d, ni::tSize s, TUINT b, ni::tSize t,
...
struct sMersenneTwister {
  ...
  explicit sMersenneTwister(result_type seed = default_seed)
  {
    this->seed(seed);
  }
  ...
  void twist()
  {
    static constexpr result_type lower_mask =
      (static_cast<result_type>(1) << r) - 1;
    static constexpr result_type upper_mask = ~lower_mask;

    for (ni::tSize i = 0; i < n; ++i) {
      result_type x =
        (_state[i] & upper_mask) + (_state[(i + 1) % n] & lower_mask);
      result_type xA = x >> 1;
      if (x & 0x1) {
        xA ^= a;
      }
...
using tMersenneTwisterU32 =
  sMersenneTwister<ni::tU32, 32, 624, 397, 31, 0x9908b0df, 11, 0xffffffff, 7,
                   0x9d2c5680, 15, 0xefc60000, 18, 1812433253>;

using tMersenneTwisterU64 =
  sMersenneTwister<ni::tU64, 64, 312, 156, 31, 0xb5026f5aa96619e9ULL, 29,
                   0x5555555555555555ULL, 17, 0x71d67fffeda60000ULL, 37,
                   0xfff7eee000000000ULL, 43, 6364136223846793005ULL>;

template <typename TTYPE>
struct sMTEngineTpl {
  static_assert(eastl::is_same_v<TTYPE, ni::tU32> ||
                  eastl::is_same_v<TTYPE, ni::tU64>,
                "sMTEngineTpl only accepts tU32 or tU64 as template parameter");

  using TMT = eastl::conditional_t<eastl::is_same_v<TTYPE, ni::tU32>,
                                   tMersenneTwisterU32, tMersenneTwisterU64>;
  TMT _mt;
  ...
  sMTEngineTpl(const sMTEngineTpl& other)
      : _mt(other._mt)
  {
  }
             ...
             UniformRandomNumberGenerator&& urng)
{
  if (first != last) {
    ...
    typedef typename astl::uniform_int_distribution<unsigned_difference_type>
    ...
    for (RandomAccessIterator i = first + 1; i != last; ++i)
      eastl::iter_swap(i, first + uid(urng, uniform_int_distribution_param_type(
                                              0, i - first)));
  }
}
