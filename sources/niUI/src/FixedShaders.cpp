...
#define GET_FPS_VEC4_PARAMS                                        \
  sVec4f &aOut, sFixedShaderConstant *c, iDrawOperation *apDrawOp, \
    iGraphicsContext *apContext, const sMaterialDesc *apMat,       \
    const sFixedStatesDesc *apFixedStates, const tBool isFlippedRT

#define GET_FPS_MAT4_PARAMS                                          \
  sMatrixf &aOut, sFixedShaderConstant *c, iDrawOperation *apDrawOp, \
    iGraphicsContext *apContext, const sMaterialDesc *apMat,         \
    const sFixedStatesDesc *apFixedStates, const tBool isFlippedRT
...
{
  if (c->getVector) {
    c->getVector(aOut, c, apDrawOp, apContext, apMat, apFixedStates,
                 isFlippedRT);
    return eTrue;
  }
  return eFalse;
...
{
  if (c->getMatrix) {
    c->getMatrix(aOut, c, apDrawOp, apContext, apMat, apFixedStates,
                 isFlippedRT);
    return eTrue;
  }
  return eFalse;
...
#define FIXEDSTATE_PROJ(STATE)                                  \
  (isFlippedRT ? MatrixFlipProjectionY(apFixedStates->m##STATE) \
               : apFixedStates->m##STATE)
...
#define EVALEXPR(MAT, WHICH, TYPE, DEFAULT)                              \
  ((MAT->mptrExpressions[eMaterialExpression_##WHICH].IsOK())            \
     ? (MAT->mptrExpressions[eMaterialExpression_##WHICH]->Eval().IsOK() \
          ? MAT->mptrExpressions[eMaterialExpression_##WHICH]            \
              ->GetEvalResult()                                          \
              ->Get##TYPE()                                              \
          : (DEFAULT))                                                   \
     : (DEFAULT))
...
#define FPS_MATRIX(NAME) \
  FPS_GetMatrix(eFPSMatrix_##NAME, apDrawOp, apFixedStates, isFlippedRT)

static tIntPtr _fpsMulCache[eFPSMatrix_Last] = { 0 };

static __forceinline sMatrixf _MatrixMul(eFPSMatrix aMatrix, const sMatrixf& A,
                                         const sMatrixf& B)
{
  return A * B;
}
static __forceinline sMatrixf _MatrixMul(eFPSMatrix aMatrix, const sMatrixf& A,
                                         const sMatrixf& B, const sMatrixf& C)
{
  return A * B * C;
}
...
static __forceinline sMatrixf _MatrixMulInv(eFPSMatrix aMatrix,
                                            const sMatrixf& A,
                                            const sMatrixf& B)
{
  sMatrixf r, t = A * B;
...
///////////////////////////////////////////////
static inline sMatrixf FPS_GetMatrix(eFPSMatrix aMatrix,
                                     iDrawOperation* apDrawOp,
                                     const sFixedStatesDesc* apFixedStates,
                                     const tBool isFlippedRT)
{
  switch (aMatrix) {
  case eFPSMatrix_CameraView: return FIXEDSTATE(CameraViewMatrix);
  case eFPSMatrix_CameraProj: return FIXEDSTATE_PROJ(CameraProjectionMatrix);
  case eFPSMatrix_View: return FIXEDSTATE(ViewMatrix);
  case eFPSMatrix_Proj: return FIXEDSTATE_PROJ(ProjectionMatrix);
  case eFPSMatrix_World: return apDrawOp->GetMatrix();
  case eFPSMatrix_CameraViewProj:
    return _MatrixMul(aMatrix, FIXEDSTATE(CameraViewMatrix),
                      FIXEDSTATE_PROJ(CameraProjectionMatrix));
  case eFPSMatrix_CameraInvViewProj:
    return _MatrixMulInv(aMatrix, FIXEDSTATE(CameraViewMatrix),
                         FIXEDSTATE_PROJ(CameraProjectionMatrix));
  ...
  case eFPSMatrix_WorldViewProj:
    return _MatrixMul(aMatrix, apDrawOp->GetMatrix(), FIXEDSTATE(ViewMatrix),
                      FIXEDSTATE_PROJ(ProjectionMatrix));
  case eFPSMatrix_ViewProj:
    return _MatrixMul(aMatrix, FIXEDSTATE(ViewMatrix),
                      FIXEDSTATE_PROJ(ProjectionMatrix));
  case eFPSMatrix_InvWorld: return _MatrixInv(aMatrix, apDrawOp->GetMatrix());
  case eFPSMatrix_InvView: return _MatrixInv(aMatrix, FIXEDSTATE(ViewMatrix));
  case eFPSMatrix_InvProj:
    return _MatrixInv(aMatrix, FIXEDSTATE_PROJ(ProjectionMatrix));
  case eFPSMatrix_InvViewProj:
    return _MatrixMulInv(aMatrix, FIXEDSTATE(ViewMatrix),
                         FIXEDSTATE_PROJ(ProjectionMatrix));
  case eFPSMatrix_InvWorldView:
    return _MatrixMulInv(aMatrix, apDrawOp->GetMatrix(),
                         FIXEDSTATE(ViewMatrix));
...
static void FPS_GET_DECL(fpsMaterialChannelColorDiffuseModulate)(
  GET_FPS_VEC4_PARAMS)
{
...
static void FPS_GET_DECL(fpsMaterialChannelColorPassthrough)(
  GET_FPS_VEC4_PARAMS)
{
  aOut = apMat->mChannels[c->nSubItemIndex].mColor;
}
...
static void FPS_GET_DECL(fpsTime)(GET_FPS_VEC4_PARAMS)
{
  ...
  const sVec4f vTime = { (ni::tF32)time, (ni::tF32)time,
                         ni::Sin((ni::tF32)time), (ni::tF32)frameTime };
  aOut = vTime;
}
...
{
  sVec3f v;
  VecTransformNormal(v, VecNormalize(v, sVec3f::One()), FPS_MATRIX(World));
  // xyz: scale.xyz
...
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetTranslation(t, FPS_MATRIX(View)),
                     FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
...
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetForward(t, FPS_MATRIX(View)),
                     FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
...
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetUp(t, FPS_MATRIX(View)), FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
static void FPS_GET_DECL(fpsWorldViewRight)(GET_FPS_VEC4_PARAMS)
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetRight(t, FPS_MATRIX(View)),
                     FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
...
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetTranslation(t, FPS_MATRIX(CameraView)),
                     FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
...
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetForward(t, FPS_MATRIX(CameraView)),
                     FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
...
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetUp(t, FPS_MATRIX(CameraView)),
                     FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
...
{
  sVec3f t;
  VecTransformNormal(t, MatrixGetRight(t, FPS_MATRIX(CameraView)),
                     FPS_MATRIX(InvWorld));
  aOut = Vec4(t);
}
...
static void FPS_GET_DECL(fpsViewportClampTexCoos)(GET_FPS_VEC4_PARAMS)
{
  ...
  aOut = Vec4(vp.x / fW + border.x, vp.y / fH + border.y, vp.z / fW - border.x,
              vp.w / fH - border.y);
}
static void FPS_GET_DECL(fpsViewportRes)(GET_FPS_VEC4_PARAMS)
{
  ...
  aOut =
    Vec4((fVPW / fRTW), (fVPH / fRTH), fVPX + 0.5f / fRTW, fVPY + 0.5f / fRTH);
}
...
#define DEF_FPSCONST3(NAME, BASENAME, REGTYPE, MAXSIZE, CONSTTYPE, ITEMINDEX, \
                      SUBITEMINDEX, GETTER)                                   \
  {                                                                           \
    sFixedShaderConstant c;                                                   \
    c.hspName = _H(#NAME);                                                    \
...
#define DEF_MAP_CONSTS(NAME, IDX, COLORMODE)                                   \
  DEF_FPSCONST3(fpsMaterial##NAME##Color##IDX, fpsMaterial##NAME##Color,       \
                Float, 1, MaterialColor, IDX, eMaterialChannel_##NAME,         \
                fpsMaterialChannelColor##COLORMODE);                           \
  DEF_FPSCONST3(fpsMaterial##NAME##Res##IDX, fpsMaterial##NAME##Res, Float, 1, \
                MaterialVector, IDX, eMaterialChannel_##NAME,                  \
                fpsMaterialChannelRes);
...
///////////////////////////////////////////////
tBool sFixedShaderConstants::InitializeShaderConstants(
  iShaderConstants* apBuffer) const
{
  for (sFixedShaderConstants::tMap::const_iterator it = mmapConstants.begin();
       it != mmapConstants.end(); ++it)
  {
    const sFixedShaderConstant& c = it->second;
    niConstCast(sFixedShaderConstant&, c).nConstantBufferIndex =
      apBuffer->AddConstant(c.hspName, c.regType, c.nMaxSize);
  }
  return eTrue;
}
...
static __forceinline tBool _UpdateShaderConstants(
  iShaderConstants* apDestConsts, iGraphicsContext* apContext,
  iDrawOperation* apDrawOp, const tBool isFlippedRT)
{
  ...
  const sFixedStatesDesc* pFixedStates =
    (const sFixedStatesDesc*)apContext->GetFixedStates()->GetDescStructPtr();

  {
    niProfileBlock(FPSConstants_Update_Uniforms);
    const sShaderConstantsDesc* shaderConstDesc =
      (const sShaderConstantsDesc*)apDestConsts->GetDescStructPtr();
    // Update the requested constants
    niLoopit (sShaderConstantsDesc::tConstMap::const_iterator, it,
              shaderConstDesc->mmapConstants)
    {
      ...
      if (c->getMatrix) {
        sMatrixf v;
        c->getMatrix(v, c, apDrawOp, apContext, pMaterial, pFixedStates,
                     isFlippedRT);
        apDestConsts->SetFloatMatrix(it->second, v);
      }
      else if (c->getVector) {
        sVec4f v;
        c->getVector(v, c, apDrawOp, apContext, pMaterial, pFixedStates,
                     isFlippedRT);
...
///////////////////////////////////////////////
niExportFunc(tBool) FixedShaders_UpdateConstants(iGraphicsContext* apContext,
                                                 iShaderConstants* apBuffer,
                                                 iDrawOperation* apDrawOp)
{
  niCheck(niIsOK(apContext), eFalse);
  niCheck(niIsOK(apBuffer), eFalse);
  const tBool isFlippedRT = niFlagIs(apContext->GetRenderTarget(0)->GetFlags(),
