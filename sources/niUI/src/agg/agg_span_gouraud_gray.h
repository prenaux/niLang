...
class span_gouraud_gray : public span_gouraud<ColorT> {
 ...
 private:
  ...
  struct gray_calc {
    ...
    void calc(agg_real y)
    {
      agg_real k = (y - m_y1) * m_1dy;
      if (k < 0.0)
        k = 0.0;
      if (k > 1.0)
        k = 1.0;
 ...
 public:
  ...
  span_gouraud_gray(const color_type& c1, const color_type& c2,
                    const color_type& c3, agg_real x1, agg_real y1, agg_real x2,
                    agg_real y2, agg_real x3, agg_real y3, agg_real d = 0)
  ...
  void prepare()
  {
    ...
    m_swap = cross_product(coord[0].x, coord[0].y, coord[2].x, coord[2].y,
