...
class cImageMap : public ImplRC<ni::iImageMap, eImplFlags_Default> {
 ...
 public:
  ///////////////////////////////////////////////
  cImageMap(iGraphics* apGraphics, const achar* aaszBaseName,
            const achar* aaszPxf)
  {
    ZeroMembers();
  ...
  {
    anSize = ni::Min(
      anSize, mptrGraphics->GetDriverCaps(eGraphicsCaps_Texture2DMaxSize));
    mnPageSize = anSize;
    if (mnPageMipMaps) {
      mnPageMipMaps =
        ni::ComputeNumPow2Levels(mnPageSize >> 1, mnPageSize >> 1);
    }
  }
  ...
  {
    mnPageMipMaps = ni::Min(
      anNumMipMaps, ni::ComputeNumPow2Levels(mnPageSize >> 1, mnPageSize >> 1));
  }
  ...
  cImagePacker* __stdcall _CreateImagePacker(tU32 anIndex)
  {
    ...
    cImagePacker* packer =
      niNew cImagePacker(mptrGraphics, mnPageSize, mnPageSize, abUseMipMaps,
                         niHStr(mhspBaseName), niHStr(mhspPxf));
    if (niIsOK(packer)) {
      Image_SetDebugName(packer->mptrImage, niFmt(_A("IMAGEMAP%p_Page%d"),
                                                  (tIntPtr)this, anIndex));
    }
    return packer;
  }
  virtual iOverlay* __stdcall AddImage(iHString* ahspName, iBitmap2D* apBitmap)
  {
    ...
    niLoopit (tPageVec::iterator, it, mvPages) {
      ...
      if (page.packer->GetIsFull())
        continue;
      sRecti rect = page.packer->InsertBitmap(bmp, _kbInsertFiltered,
                                              PACKER_INSERT_PADDING);
      if (rect != sRecti::Null()) {
    ...
    if (mnMaxNumPages && mvPages.size() > mnMaxNumPages) {
      niError(niFmt(
        _A("Max number of pages '%d' reached, cant create an additional page."),
        mnMaxNumPages));
      return NULL;
    }
    ...
    sRecti rect =
      page.packer->InsertBitmap(bmp, _kbInsertFiltered, PACKER_INSERT_PADDING);
    if (rect == sRecti::Null()) {
                                                   ...
                                                   iHString* ahspRes)
  {
    ...
    if (!image.IsOK()) {
      niError(niFmt(_A("Can't add image '%s' from resource '%s'."),
                    niHStr(ahspName), niHStr(ahspRes)));
      return NULL;
    }
    return image.GetRawAndSetNull();
  }
                                                  ...
                                                  tU32 anMaxRes, tU32 anMinRes)
  {
    ...
    if (HStringIsEmpty(ahspName)) {
      cPath path(
        (cString(niHStr(ahspFolder)) + _A("/") + cString(niHStr(ahspRes)))
          .Chars());
    ...
    while (curRes >= anMinRes) {
      ...
      buildPath.SetPath(
        (buildPath.GetPath() + cString(niHStr(ahspRes))).Chars());
      buildPath.SetExtension("png"); // icons are all in png
      cString foundPath =
        mptrGraphics->URLFindBitmapFilePath(buildPath.Chars(), NULL);
      if (foundPath.IsNotEmpty()) {
        ...
        if (fp.IsOK()) {
          ...
          if (!curBmp.IsOK()) {
            niError(niFmt(
              _A(
                "Can't open image '%s' resolution '%d' from resource '%s', icon set '%s'."),
              niHStr(hspName), curRes, niHStr(ahspRes), niHStr(ahspFolder)));
            return NULL;
          }
        }
      }
      ...
      if (bmp.IsOK()) {
        if (!curBmp.IsOK()) {
          ...
          curBmp = prevBmp->CreateResized(prevBmp->GetWidth() / 2,
                                          prevBmp->GetHeight() / 2);
        }
        niAssert(curBmp.IsOK());
        bmp->GetMipMap(curMip)->Blit(curBmp, 0, 0, 0, 0, curBmp->GetWidth(),
                                     curBmp->GetHeight());
        prevBmp = curBmp;
        ++curMip;
        ...
        if (!bmp->CreateMipMaps(0, eFalse)) {
          niError(niFmt(
            _A(
              "Can't create mipmaps for image '%s' resolution '%d' from resource '%s', icon set '%s'."),
            niHStr(hspName), curRes, niHStr(ahspRes), niHStr(ahspFolder)));
          return NULL;
        }
        prevBmp = curBmp;
    ...
    if (!bmp.IsOK()) {
      niError(niFmt(
        _A(
          "Can't open image '%s' at any resolution from resource '%s', icon set '%s'."),
        niHStr(hspName), niHStr(ahspRes), niHStr(ahspFolder)));
      return NULL;
    }

    for (; curMip < bmp->GetNumMipMaps(); ++curMip) {
      niAssert(prevBmp.IsOK());
      Ptr<iBitmap2D> curBmp = prevBmp->CreateResized(prevBmp->GetWidth() / 2,
                                                     prevBmp->GetHeight() / 2);
      niAssert(curBmp.IsOK());
      bmp->GetMipMap(curMip)->Blit(curBmp, 0, 0, 0, 0, curBmp->GetWidth(),
                                   curBmp->GetHeight());
    ...
    if (!image.IsOK()) {
      niError(
        niFmt(_A("Can't add image '%s' from resource '%s', icon set '%s'."),
              niHStr(hspName), niHStr(ahspRes), niHStr(ahspFolder)));
      return NULL;
    }
    return image.GetRawAndSetNull();
...
#pragma niTodo("Implement")
  ...
  {
    Ptr<iOverlay> image =
      mptrGraphics->CreateOverlayImage(page.packer->GetImage());
  ...
  tBool __stdcall Serialize(ni::iFile* apFile, tImageMapSerializeFlags aFlags)
  {
    ...
    if (niFlagIs(aFlags, eImageMapSerializeFlags_Read)) {
      ...
      // Header
      {
        ...
        const eBlendMode defaultImageBlendMode =
          (eBlendMode)apFile->ReadBitsPackedU32();
        ...
        if (nPageSize >
            (tU32)pGraphics->GetDriverCaps(eGraphicsCaps_Texture2DMaxSize))
        {
          niError(niFmt(
            _A(
              "Page size '%d' is too big for this graphics driver (max is '%d')"),
            nPageSize,
            pGraphics->GetDriverCaps(eGraphicsCaps_Texture2DMaxSize)));
          return eFalse;
        }
      ...
      niLoop (i, numPages) {
        ...
        QPtr<iBitmap2D> targetBmp =
          page.packer->_GetTargetBitmap(sRecti::Null());
        if (!targetBmp.IsOK()) {
        ...
        if (!bmp->GetPixelFormat()->IsSamePixelFormat(
              targetBmp->GetPixelFormat()))
        {
          niError(niFmt(
            _A(
              "Mismatch of the number of pixel format: loaded '%s', page is '%s'."),
        ...
        if (bmp->GetNumMipMaps() != targetBmp->GetNumMipMaps()) {
          niError(niFmt(
            _A("Mismatch of the number of mipmaps: loaded '%d', page is '%d'."),
        ...
        if (bmp->GetWidth() != targetBmp->GetWidth()) {
          niError(niFmt(
            _A("Mismatch of the bitmap width: loaded '%d', page is '%d'."),
        ...
        if (bmp->GetHeight() != targetBmp->GetHeight()) {
          niError(niFmt(
            _A("Mismatch of the bitmap height: loaded '%d', page is '%d'."),
        ...
        niLoop (j, bmp->GetNumMipMaps() + 1) {
          ...
          dstLevel->Blit(srcLevel, 0, 0, 0, 0, srcLevel->GetWidth(),
                         srcLevel->GetHeight());
        }
        ...
        niLoop (j, numRects) {
          ...
          if (page.packer->InsertRect(rect, PACKER_INSERT_PADDING) ==
              sRecti::Null())
          {
            niError(niFmt(
              _A("Can't insert rect '%d' to texture for packer '%d'."), j, i));
            Clear();
            return eFalse;
          }
        }
      ...
      niLoop (i, numImages) {
        ...
        if (pageIndex >= mvPages.size()) {
          niError(niFmt(_A("Invalid page index '%d' when loading image '%d."),
                        pageIndex, i));
          Clear();
          return eFalse;
        }
    ...
    /// Write //////////////////////////////////////////
      ...
      niLoopit (tImageMap::iterator, it, mmapImages) {
        ...
        if (pageIndex == eInvalidHandle) {
          niError(
            niFmt(_A("Can't find page index for image '%s'"), niHStr(name)));
        }
...
///////////////////////////////////////////////
iImageMap* __stdcall cGraphics::CreateImageMap(const achar* aaszBaseName,
                                               const achar* aaszPxf)
{
