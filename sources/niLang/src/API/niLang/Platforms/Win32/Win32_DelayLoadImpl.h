...
static const wchar_t kDLLLoadSearchPaths[] =
  DEFINE_TO_WSTR(DLLLOAD_SEARCH_PATHS);

static void DLLLoad_ShowError(const wchar_t* basePath, const wchar_t* dllName,
...
static const wchar_t* DLLLoad_GetNextPath(const wchar_t* paths, size_t* pos)
{
...
static HMODULE DLLLoad_TryLoadInPath(const wchar_t* basePath,
                                     ...
                                     const wchar_t* dllName)
{
  ...
  if (wcscmp(searchPath, L".") == 0) {
    // search in basePath
    _snwprintf(fullPath, MAX_PATH, L"%s\\%s", basePath, dllName);
  }
  else if (*searchPath == L'.') {
    // relative path
    _snwprintf(fullPath, MAX_PATH, L"%s\\%s\\%s", basePath, searchPath,
               dllName);
  }
  else {
    // toolkit path
...
static HMODULE DLLLoad_TryLoadInSearchPaths(const wchar_t* basePath,
                                            const wchar_t* dllName,
                                            const wchar_t* searchPaths)
{
  const wchar_t* path;
