...
tBool __stdcall cGraphics::RegisterSystemFonts()
{
  ...
  registerFontFromPath("Sans Bold Italic",
                       "niUI://fonts/Roboto-BoldItalic.ttf");
  registerFontFromPath("Serif", "niUI://fonts/PlayfairDisplay-Regular.ttf");
  registerFontFromPath("Serif Italic",
                       "niUI://fonts/PlayfairDisplay-Italic.ttf");
  registerFontFromPath("Serif Bold", "niUI://fonts/PlayfairDisplay-Bold.ttf");
  registerFontFromPath("Serif Bold Italic",
                       "niUI://fonts/PlayfairDisplay-BoldItalic.ttf");
...
Ptr<tStringCMap> __stdcall cGraphics::EnumOSFonts() const
{
  ...
  for (tFontFileLstIt it = lst.begin(); it != lst.end(); ++it) {
    ...
    if (!ni::GetRootFS()->FileExists(path.GetPath().Chars(),
...
///////////////////////////////////////////////
tBool __stdcall cGraphics::RegisterSystemFont(iHString* ahspName,
                                              ...
                                              iFont* apFont)
{
  ...
  if (apFont) {
    niLog(Info, niFmt("Registered System font object '%s' ('%s', '%s')",
                      ahspName, apFont->GetFilePath(), apFont->GetName()));
  }
  else {
    niLog(Info, niFmt("Registered System font file '%s' ('%s')", ahspName,
                      ahspFilePath));
  }
...
///////////////////////////////////////////////
iFont* __stdcall cGraphics::FindFont(iHString* ahspName,
                                     tFontLoadFlags aFlags) const
{
  ...
  niLoop (i, vNames.size()) {
    ...
    niLoop (i, mmapBaseFonts.GetSize()) {
      ...
      if (f && (f->GetName() == hspName || f->GetFilePath() == hspName ||
...
iFont* __stdcall cGraphics::LoadFontEx(iHString* ahspName,
                                       tFontLoadFlags aFlags)
{
  ...
  niLoop (i, vNames.size()) {
    ...
    niLoop (i, mmapSystemFonts.GetSize()) {
      ...
      if (sf.hspName == hspName || sf.hspFileName == hspName ||
          sf.hspFilePath == hspName || regex->DoesMatch(niHStr(sf.hspName)))
      {
        if (sf.ptrFont.IsOK()) {
  ...
  while (!pathFont.GetPath().IsEmpty()) {
    ...
    if (!ptrFile.IsOK()) {
      niWarning(niFmt(_A("Font '%s', can't open font file '%s'."),
                      niHStr(ahspName), pathFont.GetPath().Chars()));
      break;
    }
...
void cGraphics::_RegisterBaseFont(cFont* apFont)
{
  ...
  niLog(Info,
        niFmt("Registered Base Font '%s' (family: %s, style: %s, filepath: %s)",
              apFont->GetName(), apFont->GetFamilyName(),
              apFont->GetStyleName(), apFont->GetFilePath()));
    ...
    if (mmapBaseFonts.GetItem(i) == apFont) {
      ...
      niLog(
        ...
        niFmt(
          ...
          apFont->GetName(), apFont->GetFamilyName(), apFont->GetStyleName(),
          apFont->GetFilePath()));
...
static tU8 _DefaultFontData[256 * 8] = {
  ...
  0xFF,
  0xFF,
  /* Char(00) NUL */ // NULL is white and to be used as background color
  0x7E,
  0x81,
  0xA5,
  0x81,
...
static inline void _Draw8BitFontChar(const tU8* apFontData, iBitmap2D* apBmp,
                                     tU32 aX, tU32 aY, tU32 aC)
{
...
tBool cFont::_CreateFromFile(iHString* ahspName, iFile* pFile)
{
  ...
  // Load TTF font
  if (ext.IEq("ttf") || ext.IEq("ttc") || ext.IEq("otf") || ext.IEq("otc") ||
      ext.IEq("pfa") || ext.IEq("pfb"))
  {
  ...
  // Load a bitmap font
    ...
    if (!ptrBmp.ptr()) {
      niError(
        niFmt(_A("Can't load 2d bitmap from '%s'."), pFile->GetSourcePath()));
      return eFalse;
    }
...
tBool cFont::_Create8x8(iHString* ahspName)
{
  ...
  Ptr<iBitmap2D> ptrBmp =
    mptrImage->GrabBitmap(eImageUsage_Target, sRecti::Null());
  niCheckIsOK(ptrBmp, eFalse);

  ptrBmp->Clear();
  for (tU32 c = 0; c < 256; ++c) {
    _Draw8BitFontChar(_DefaultFontData, ptrBmp, (c & 0xF) << 3, (c >> 4) << 3,
                      c);
  }
...
{
  return mptrTTF.IsOK() ? mptrTTF->GetMaxCharSize()
                        : mptrImage->GetWidth() / 16;
}
...
iFont* cFont::CreateFontInstance(iMaterial* apMaterial)
{
  ...
  if (niIsOK(apMaterial)) {
    ...
    if (ptrNewFont->mStates.mbMaterialInstanced) {
      ...
      ptrNewFont->mStates.mptrMaterial =
        ptrNewFont->mStates.mptrMaterial->Clone();
      ptrNewFont->mStates.mbMaterialInstanced = eFalse;
    }
    ptrNewFont->mStates.mptrMaterial->SetChannelTexture(eMaterialChannel_Base,
                                                        NULL);
  }
...
void cFont::SetFiltering(tBool abFiltering)
{
  mStates.mptrMaterial->SetChannelSamplerStates(
    eMaterialChannel_Base, abFiltering ? kFontFilterOnSS : kFontFilterOffSS);
}
tBool cFont::GetFiltering() const
{
  return mStates.mptrMaterial->GetChannelSamplerStates(eMaterialChannel_Base) ==
         kFontFilterOnSS;
}
...
///////////////////////////////////////////////
void __stdcall cFont::SetSizeAndResolution(const sVec2f& avSize,
                                           const tU32 anResolution,
                                           const tF32 afContentsScale)
{
  if (!this->GetIsFixedResolution()) {
    niAssert(niIsOK(mptrTTF));
    mStates.SetResolution(ni::Clamp(
      (tU32)((tF32)anResolution * afContentsScale), 2, GetMaxResolution()));
  }

  // Set the size
  if (avSize == sVec2f::Zero()) {
    mStates.mvSize =
      Vec2f(mStates.mnResolution, mStates.mnResolution) / afContentsScale;
  }
  else {
...
///////////////////////////////////////////////
sRectf __stdcall cFont::ComputeTextSize(const sRectf& aRect,
                                        const achar* aaszText,
                                        tFontFormatFlags aFormat)
{
  astl::vector<sTextLineMetric> tm;
  sRectf bb = TextLayout_Compute(this, aRect, aaszText, ni::StrLen(aaszText),
                                 &aFormat, tm);
  return bb;
}

///////////////////////////////////////////////
sRectf __stdcall cFont::DrawText(ni::iCanvas* apCanvas, const sRectf& aRect,
                                 ...
                                 tFontFormatFlags aFlags)
{
  ...
  niLoop (pass, numPasses) {
    ...
    TextLayout_PushToCanvas(
      tm.data(), (tU32)tm.size(), apCanvas, this, color,
      Vec3f(offsetX, offsetY, afZ),
...
tBool __stdcall cFont::CacheText(const achar* aaszText)
{
  if (mptrTTF.IsOK()) {
    ...
    while (!itText.is_end()) {
      ...
      mptrTTF->RenderCodepoint(ch, mStates.mnResolution,
                               mStates.mbDistanceField);
    }
  }
  return eTrue;
}
...
tBool __stdcall cFont::CacheRange(tU32 anFirst, tU32 anLast)
{
  if (mptrTTF.IsOK()) {
    for (tU32 i = anFirst; i <= anLast; ++i) {
      mptrTTF->RenderCodepoint(i, mStates.mnResolution,
                               mStates.mbDistanceField);
    }
  }
  return eTrue;
...
///////////////////////////////////////////////
const sFontGlyph* cFont::GetGlyphFromCodepoint(tU32 anChar,
                                               sFontGlyph* apTmpStorage) const
{
  if (mptrTTF.IsOK()) {
    return mptrTTF->RenderCodepoint(anChar, mStates.mnResolution,
                                    mStates.mbDistanceField);
  }
  else {
    ...
    const tF32 fontSizeByRes =
      ni::FDiv(this->mStates.GetWidth(), (tF32)this->mStates.mnResolution);
...
///////////////////////////////////////////////
const sFontGlyph* cFont::GetGlyphFromIndex(tU32 anGlyphIndex,
                                           sFontGlyph* apTmpStorage) const
{
  if (mptrTTF.IsOK()) {
    return mptrTTF->RenderGlyphIndex(anGlyphIndex, mStates.mnResolution,
                                     mStates.mbDistanceField);
  }
  else {
...
{
  return mStates.GetWidth() * rectTexCoo.GetWidth() * GetTextureWidth() *
...
{
  return mStates.GetHeight() * rectTexCoo.GetHeight() * GetTextureHeight() *
         mStates.mfInvResolution;
}
...
tF32 __stdcall cFont::GetKerning(tU32 anPrevChar, tU32 anChar) const
{
  if (anPrevChar && mptrTTF.IsOK()) {
    ...
    return (tF32)mptrTTF->GetKerning(mStates.mnResolution, anPrevChar, anChar) *
           scale;
  }
  else {
    return 0;
...
tF32 __stdcall cFont::GetAdvance(tU32 anChar) const
{
  if (mptrTTF.IsOK()) {
    const tF32 scale = ni::FDiv(mStates.GetWidth(), (tF32)mStates.mnResolution);
    return mptrTTF->GetAdvanceCodepoint(mStates.mnResolution,
                                        mStates.mbDistanceField, anChar) *
           scale;
  }
  else {
    return mStates.GetWidth();
...
iOverlay* __stdcall cFont::GetGlyphOverlay(tU32 anGlyphIndex) const
{
  ...
  niLet glyphTex = niCheckNN(
    glyphTex, g->img->GrabTexture(eImageUsage_Source, sRecti::Null()), nullptr);
  niLet glyphSize = glyphTexCoo.GetSize() * Vec2f((tF32)glyphTex->GetWidth(),
                                                  (tF32)glyphTex->GetHeight());
  niLet overlay =
    niCheckNN(overlay, graphics->CreateOverlayTexture(glyphTex), nullptr);
...
///////////////////////////////////////////////
sVec2i __stdcall cFont::BlitCharEx(iBitmap2D* apDestBmp, tI32 anX, tI32 anY,
                                   ...
                                   eBlendMode aBlendMode)
{
  ...
  Ptr<iBitmap2D> pSrcBmp =
    g->img->GrabBitmap(eImageUsage_Source, sRecti::Null());
  niCheckIsOK(pSrcBmp, sVec2i::Zero());

  anX += (tI32)g->lead.x;
  anY += (tI32)g->lead.y;
  const sRecti rectSrc{ (tI32)(g->texCoo.x * (float)pSrcBmp->GetWidth()),
  ...
  const sVec2i charSize = { anW <= 0 ? rectSrc.GetWidth() : anW,
                            anH <= 0 ? rectSrc.GetHeight() : anH };
  if (anChar > 32) {
    if (charSize.x > 0 && charSize.y > 0 && charSize != rectSrc.GetSize()) {
      if (aBlendMode != eBlendMode_NoBlending) {
        apDestBmp->BlitAlphaStretch(
          pSrcBmp, rectSrc.Left(), rectSrc.Top(), anX, anY, rectSrc.GetWidth(),
          rectSrc.GetHeight(), charSize.x, charSize.y, colFront,
          sColor4f::Zero(), eBlendMode_Translucent);
      }
      else {
        apDestBmp->BlitStretch(pSrcBmp, rectSrc.Left(), rectSrc.Top(), anX, anY,
                               rectSrc.GetWidth(), rectSrc.GetHeight(),
                               charSize.x, charSize.y);
      }
    }
    else {
      if (aBlendMode != eBlendMode_NoBlending) {
        apDestBmp->BlitAlpha(pSrcBmp, rectSrc.Left(), rectSrc.Top(), anX, anY,
                             rectSrc.GetWidth(), rectSrc.GetHeight(),
                             ePixelFormatBlit_Normal, colFront,
                             sColor4f::Zero(), eBlendMode_Translucent);
      }
      else {
        apDestBmp->Blit(pSrcBmp, rectSrc.Left(), rectSrc.Top(), anX, anY,
                        rectSrc.GetWidth(), rectSrc.GetHeight(),
...
cString __stdcall cFont::GetGlyphName(const tU32 anGlyphIndex) const
{
  ...
  char buffer[2048] = { 0 };
  FT_Error error =
    FT_Get_Glyph_Name(face, anGlyphIndex, buffer, sizeof(buffer));
  if (!error) {
    return buffer;
  }
  else {
...
///////////////////////////////////////////////
sVec2f __stdcall cFont::GetGlyphPath(iVGPath* apPath, tU32 anGlyphIndex,
                                     const sVec2f& avOffset,
                                     const tF32 afScale) const
{
  if (mptrTTF.IsOK()) {
    return mptrTTF->GetGlyphPath(apPath, anGlyphIndex, mStates.mnResolution,
                                 avOffset, afScale);
  }
  else {
    return sVec2f::Zero();
