...
///
  ...
  #define EASTL_LIST_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX     \
      " list" // Unless the user overrides something, this is "EASTL list".
#endif
...
struct ListNodeBase {
  ...
  void insert(ListNodeBase* pNext)
    EA_NOEXCEPT; // Inserts this standalone node before the node pNext in pNext's list.
  void remove()
    EA_NOEXCEPT; // Removes this node from the list it's in. Leaves this node's mpNext/mpPrev invalid.
  ...
  void reverse()
    EA_NOEXCEPT; // Reverses the order of nodes in the circular list this node is a part of.
  static void swap(ListNodeBase& a, ListNodeBase& b)
    EA_NOEXCEPT; // Swaps the nodes a and b in the lists to which they belong.

  void insert_range(ListNodeBase* pFirst, ListNodeBase* pFinal)
    EA_NOEXCEPT; // Differs from splice in that first/final aren't in another list.
  static void remove_range(ListNodeBase* pFirst,
                           ListNodeBase* pFinal) EA_NOEXCEPT; //
...
#else
  typedef ListNodeBase
    base_node_type; // We use ListNodeBase instead of ListNode<T> because we don't want to create a T.
#endif
...
class list : public ListBase<T, Allocator> {
 ...
 public:
  ...
  explicit list(size_type n,
                const allocator_type& allocator = EASTL_LIST_DEFAULT_ALLOCATOR);
 ...
 protected:
  ...
  template <typename InputIterator>
  void DoInsert(ListNodeBase* pNode, InputIterator first, InputIterator last,
                false_type);
...
inline void ListNodeBase::splice(ListNodeBase* first,
                                 ListNodeBase* last) EA_NOEXCEPT
{
...
inline void ListNodeBase::insert_range(ListNodeBase* pFirst,
                                       ListNodeBase* pFinal) EA_NOEXCEPT
{
...
inline void ListNodeBase::remove_range(ListNodeBase* pFirst,
                                       ListNodeBase* pFinal) EA_NOEXCEPT
{
...
inline ListIterator<T, Pointer, Reference>::ListIterator(
  const ListNodeBase* pNode) EA_NOEXCEPT
    : mpNode(const_cast<node_type*>(reinterpret_cast<const node_type*>(
...
inline ListIterator<T, Pointer, Reference>::ListIterator(const iterator& x)
  EA_NOEXCEPT : mpNode(const_cast<node_type*>(x.mpNode))
{
  // Empty
}

template <typename T, typename Pointer, typename Reference>
inline typename ListIterator<T, Pointer, Reference>::this_type ListIterator<
  T, Pointer, Reference>::next() const EA_NOEXCEPT
{
...
template <typename T, typename Pointer, typename Reference>
inline typename ListIterator<T, Pointer, Reference>::this_type ListIterator<
  T, Pointer, Reference>::prev() const EA_NOEXCEPT
{
  return ListIterator(mpNode->mpPrev);
}

template <typename T, typename Pointer, typename Reference>
inline typename ListIterator<T, Pointer, Reference>::reference ListIterator<
  T, Pointer, Reference>::operator*() const EA_NOEXCEPT
{
  return mpNode->mValue;
}

template <typename T, typename Pointer, typename Reference>
inline typename ListIterator<T, Pointer, Reference>::pointer ListIterator<
  T, Pointer, Reference>::operator->() const EA_NOEXCEPT
{
  return &mpNode->mValue;
}
...
template <typename T, typename Pointer, typename Reference>
inline typename ListIterator<T, Pointer, Reference>::this_type& ListIterator<
  T, Pointer, Reference>::operator++() EA_NOEXCEPT
{
...
template <typename T, typename Pointer, typename Reference>
inline typename ListIterator<T, Pointer, Reference>::this_type ListIterator<
  T, Pointer, Reference>::operator++(int) EA_NOEXCEPT
{
  this_type temp(*this);
  mpNode = static_cast<node_type*>(mpNode->mpNext);
  return temp;
...
template <typename T, typename Allocator>
const typename ListBase<T, Allocator>::allocator_type& ListBase<
...
template <typename T, typename Allocator>
typename ListBase<T, Allocator>::allocator_type& ListBase<
...
template <typename T, typename Allocator>
inline void ListBase<T, Allocator>::set_allocator(
  const allocator_type& allocator)
{
  EASTL_ASSERT(
    (mAllocator == allocator) ||
    (static_cast<node_type*>(mNode.mpNext) ==
     &mNode)); // We can only assign a different allocator if we are empty of elements.
  mAllocator = allocator;
}

template <typename T, typename Allocator>
inline typename ListBase<T, Allocator>::node_type* ListBase<
  T, Allocator>::DoAllocateNode()
{
  node_type* pNode = (node_type*)allocate_memory(mAllocator, sizeof(node_type),
                                                 EASTL_ALIGN_OF(T), 0);
...
{
  DoInsert((ListNodeBase*)&mNode, const_iterator((ListNodeBase*)x.mNode.mpNext),
           const_iterator((ListNodeBase*)&x.mNode), false_type());
}
...
template <typename T, typename Allocator>
typename list<T, Allocator>::iterator inline list<T, Allocator>::begin()
  EA_NOEXCEPT
{
...
template <typename T, typename Allocator>
inline typename list<T, Allocator>::reverse_iterator list<
...
template <typename T, typename Allocator>
inline typename list<T, Allocator>::const_reverse_iterator list<
  ...
  EA_NOEXCEPT
{
  ...
  return (size_type)eastl::distance(const_iterator((ListNodeBase*)mNode.mpNext),
                                    const_iterator((ListNodeBase*)&mNode));
  #endif
#endif
}

template <typename T, typename Allocator>
typename list<T, Allocator>::this_type& list<T, Allocator>::operator=(
  const this_type& x)
{
...
template <typename T, typename Allocator>
inline typename list<T, Allocator>::reference list<T, Allocator>::push_front()
{
  ...
  ++mSize;
#endif
  return static_cast<node_type*>(mNode.mpNext)
    ->mValue; // Same as return front();
}
...
template <typename T, typename Allocator>
inline typename list<T, Allocator>::reference list<T, Allocator>::push_back()
{
  node_type* const pNode = DoCreateNode();
  ...
  ++mSize;
#endif
  return static_cast<node_type*>(mNode.mpPrev)
    ->mValue; // Same as return back();
}
  ...
  const_iterator position, InputIterator first, InputIterator last)
{
  iterator itPrev(position.mpNode);
  --itPrev;
  DoInsert((ListNodeBase*)position.mpNode, first, last,
  ...
  const_iterator position, std::initializer_list<value_type> ilist)
{
  iterator itPrev(position.mpNode);
  --itPrev;
  DoInsert((ListNodeBase*)position.mpNode, ilist.begin(), ilist.end(),
           false_type());
  return ++itPrev; // Inserts in front of position, returns iterator to new elements.
}
...
template <typename T, typename Allocator>
inline typename list<T, Allocator>::reverse_iterator list<T, Allocator>::erase(
...
template <typename T, typename Allocator>
typename list<T, Allocator>::reverse_iterator list<T, Allocator>::erase(
  const_reverse_iterator first, const_reverse_iterator last)
{
...
{
  for (iterator first((ListNodeBase*)mNode.mpNext), last((ListNodeBase*)&mNode);
       first != last;)
  {
    iterator temp(first);
    ++temp;
...
#if EASTL_LIST_SIZE_CACHE
    if (x.mSize) {
      ((ListNodeBase*)position.mpNode)
        ->splice((ListNodeBase*)x.mNode.mpNext, (ListNodeBase*)&x.mNode);
      mSize += x.mSize;
      x.mSize = 0;
    }
#else
...
inline void list<T, Allocator>::splice(const_iterator position, this_type&& x)
{
  return splice(position,
                x); // This will call splice(const_iterator, const this_type&);
}

template <typename T, typename Allocator>
inline void list<T, Allocator>::splice(const_iterator position, list& x,
                                       const_iterator i)
{
  if (mAllocator == x.mAllocator) {
    ...
    if ((position != i) && (position != i2)) {
      ((ListNodeBase*)position.mpNode)
        ->splice((ListNodeBase*)i.mpNode, (ListNodeBase*)i2.mpNode);
...
template <typename T, typename Allocator>
inline void list<T, Allocator>::splice(const_iterator position,
                                       list<T, Allocator>&& x, const_iterator i)
{
  return splice(
    position, x,
    i); // This will call splice(const_iterator, const this_type&, const_iterator);
}
                                       ...
                                       const_iterator last)
{
  if (mAllocator == x.mAllocator) {
    ...
    if (n) {
      ((ListNodeBase*)position.mpNode)
        ->splice((ListNodeBase*)first.mpNode, (ListNodeBase*)last.mpNode);
      mSize += n;
      x.mSize -= n;
    }
#else
                                       ...
                                       const_iterator last)
{
  return splice(
    position, x, first,
    last); // This will call splice(const_iterator, const this_type&, const_iterator, const_iterator);
}
...
template <typename T, typename Allocator>
template <typename Compare>
typename list<T, Allocator>::iterator list<T, Allocator>::DoSort(
  iterator i1, iterator end2, size_type n, Compare& compare)
{
  ...
  case 3: {
    ...
    if (
      compare(
        *--end2,
        *i1)) // At this point, i1 refers to the second element in this three element segment.
    {
      end2.mpNode->remove();
      end2.mpNode->insert(i1.mpNode);
    }
  ...
  i1 =
    DoSort(i1, end1, nMid,
           compare); // Return the new beginning of the first sorted sub-range.
  iterator i2 =
    DoSort(end1, end2, n - nMid,
           compare); // Return the new beginning of the second sorted sub-range.
  ...
  for (++i1; (i1 != end1) && (i2 != end2);
       ++i1) // while still working on either segment...
  {
    if (
      compare(
        *i2,
        *i1)) // If i2 is less than i1 and it needs to be merged in front of i1...
    {
...
template <typename T, typename Allocator>
template <typename... Args>
inline typename list<T, Allocator>::node_type* list<T, Allocator>::DoCreateNode(
  Args&&... args)
{
...
template <typename T, typename Allocator>
inline typename list<T, Allocator>::node_type* list<T,
...
template <typename T, typename Allocator>
template <typename InputIterator>
void list<T, Allocator>::DoAssign(InputIterator first, InputIterator last,
                                  false_type)
{
...
template <typename T, typename Allocator>
template <typename Integer>
inline void list<T, Allocator>::DoInsert(ListNodeBase* pNode, Integer n,
...
template <typename T, typename Allocator>
template <typename InputIterator>
inline void list<T, Allocator>::DoInsert(ListNodeBase* pNode,
                                         InputIterator first,
...
inline void list<T, Allocator>::DoSwap(this_type& x)
{
  ListNodeBase::swap(
    (ListNodeBase&)mNode,
    (ListNodeBase&)x
      .mNode); // We need to implement a special swap because we can't do a shallow swap.
  eastl::swap(
    mAllocator,
