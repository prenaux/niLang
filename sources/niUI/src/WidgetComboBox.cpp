...
struct sComboBoxButtonSink : public cWidgetSinkImpl<> {
  ...
  sComboBoxButtonSink(cComboBoxWidget* aComboBox)
      : _comboBox(aComboBox)
  {
  }
...
struct sComboBoxDroppedSink : public cWidgetSinkImpl<> {
  ...
  sComboBoxDroppedSink(cComboBoxWidget* aComboBox)
      : _comboBox(aComboBox)
  {
  }
...
{
  // Edit box style
  tU32 editstyle = eWidgetStyle_NoMoveFocus | eWidgetStyle_HoldFocus |
...
void __stdcall cComboBoxWidget::ListInterfaces(iMutableCollection* apLst,
                                               tU32 anFlags) const
{
...
{
  if (niFlagIs(mnStatus, CBSTATUS_SHOWING) || !mpwDroppedBox.IsOK() ||
      mpwDroppedBox->GetVisible())
    return;
  ...
  if (menu.IsOK()) {
    ...
    mpwDroppedBox->SetAbsolutePosition(
      Vec2(pos.x + cbSize.x - menuSize.x, lbY));
    menu->Open();
  }
  else {
    QPtr<iWidgetListBox> lb = mpwDroppedBox.ptr();
    if (lb.IsOK()) {
      ...
      tU32 nNumLines =
        ni::Min(mnNumLines, (lb.IsOK()) ? lb->GetNumItems() : mnNumLines);
      ...
      if (lbY + h > screenRect.GetBottom()) {
        if (lbY + (lineHeight * 5) < screenRect.GetBottom()) {
          ...
          nNumLines =
            ni::Max(1, tU32((screenRect.GetBottom() - lbY) / lineHeight) - 1);
...
tBool cComboBoxWidget::OnWidgetSink(iWidget* apWidget, tU32 nMsg,
                                    const Var& varParam0, const Var& varParam1)
{
  ...
  case eUIMessage_SinkAttached: {
    ...
    if (niFlagIsNot(mpWidget->GetStyle(),
  ...
  case eUIMessage_Command: {
    ...
    if (w->GetID() == niGetConstHString(ComboBox_EditBox)) {
      ...
      if (menu.IsOK()) {
        ...
        QPtr<iWidgetListBox> lb = mpwDroppedBox.ptr();
        if (lb.IsOK() && niFlagIsNot(mnStatus, CBSTATUS_SETLBTEXT) &&
...
void cComboBoxWidget::_LostFocus()
{
  if (!mpWidget->GetHasFocus() &&
      ...
      (!mpwEditBox->GetHasFocus()) && (!mpwButton->GetHasFocus()))
  {
    _HideList();
  }
}
...
tBool cComboBoxWidget::_SetDropped(iWidget* apDB, tBool abAdd)
{
  ...
  if (apDB && ((apDB == mpwDroppedBox && abAdd) ||
  ...
  if (!abAdd || !mpwDroppedBox.IsOK()) {
    mpwDroppedBox = mpWidget->GetUIContext()->CreateWidget(
      _A("Label"), mpWidget, sRectf(0, 0, 100, 80), eWidgetStyle_Free);
    mpwDroppedBox->SetText(
      _H(niFmt(_A("No Drop Widget Set -%s- "), niHStr(mpWidget->GetID()))));
  }
