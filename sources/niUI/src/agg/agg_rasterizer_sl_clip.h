...
struct ras_conv_dbl {
  ...
  static int xi(agg_real v)
  {
    return iround(v * poly_subpixel_scale);
  }
  static int yi(agg_real v)
  {
    return iround(v * poly_subpixel_scale);
  }
  static agg_real upscale(agg_real v)
  {
    return v;
  }
  static agg_real downscale(int v)
  {
    return v / agg_real(poly_subpixel_scale);
  }
};
...
class rasterizer_sl_clip {
 ...
 private:
  ...
  AGG_INLINE void line_clip_y(Rasterizer& ras, coord_type x1, coord_type y1,
                              coord_type x2, coord_type y2, unsigned f1,
                              unsigned f2) const
  {
    f1 &= 10;
    f2 &= 10;
