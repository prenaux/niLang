...
#if defined(niIntelX86) || defined(niAlpha)
  #define BF_ASM 0 /* 1 */
  #define BF_SCALE 1
#else
  #define BF_ASM 0
...
char* CryptoKDFCryptBlowFish(const char* key, const char* setting, char* output,
                             int size)
{
  struct {
    BF_ctx ctx;
   ...
   */
  if (ni::StrLen(setting) < 29) {
    niError("Invalid salt length.");
    return NULL;
  }

  if (setting[0] != '$' || setting[1] != '2' ||
      (setting[2] != 'a' && setting[2] != 'x') || setting[3] != '$' ||
      setting[4] < '0' || setting[4] > '3' || setting[5] < '0' ||
      setting[5] > '9' || (setting[4] == '3' && setting[5] > '1') ||
      setting[6] != '$')
  {
    niError("Invalid salt prefix.");
    return NULL;
  }
...
niExportFunc(achar*) CryptoKDFGenSaltBlowfish(iCryptoRand* apRand, tU32 aRounds,
                                              achar* aaszOutput)
{
  niCheck(aaszOutput != NULL, NULL);
  *aaszOutput = '\0';
  niCheckIsOK(apRand, NULL);
  niCheck((aRounds >= 4 && aRounds <= 31), NULL);

  aaszOutput[0] = '$';
  aaszOutput[1] = '2';
  aaszOutput[2] = 'a';
  aaszOutput[3] = '$';
  aaszOutput[4] = '0' + aRounds / 10;
  aaszOutput[5] = '0' + aRounds % 10;
  aaszOutput[6] = '$';

  tU8 randBytes[16];
  apRand->RandRaw(randBytes, 16);
  BF_encode(&aaszOutput[7], (const BF_word*)randBytes, 16);
  aaszOutput[7 + 22] = '\0';
...
niExportFunc(achar*) CryptoKDFCrypt(const achar* aaszKey, const achar* aaszSalt,
                                    achar* aaszOutput)
{
  niCheck(niStringIsOK(aaszKey), NULL);
  niCheck(niStringIsOK(aaszSalt), NULL);
  return CryptoKDFCryptBlowFish(aaszKey, aaszSalt, aaszOutput,
                                knCryptoKDFCryptMaxLen);
}
