...
char* CryptoKDFCryptBlowFish(const char *key, const char *setting, char *output, int size)
{
  struct {
    BF_ctx    ctx;
    BF_key    expanded_key;
...
niExportFunc(achar*) CryptoKDFCrypt(const achar* aaszKey, const achar* aaszSalt, achar* aaszOutput) {
  ...
  return CryptoKDFCryptBlowFish(aaszKey, aaszSalt, aaszOutput, knCryptoKDFCryptMaxLen);
}
