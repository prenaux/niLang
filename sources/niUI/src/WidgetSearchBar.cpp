...
//
//=================================================================
class cSearchEngineFilePattern
    : public ImplRC<iSearchEngine, eImplFlags_Default> {
 public:
  cSearchEngineFilePattern()
  {
  }
  ...
  virtual void __stdcall Search(const achar* aSearchTerm,
                                astl::vector<tSearchItem>& avResult)
  {
...
void __stdcall cWidgetSearchBar::ListInterfaces(iMutableCollection* apLst,
                                                tU32 anFlags) const
{
...
void cWidgetSearchBar::_InitLayout()
{
  mptrInputBox = mpWidget->GetUIContext()->CreateWidget(
    _A("EditBox"), mpWidget, sRectf(0, 0, 0, 36),
  ...
  mptrShowBox = mpWidget->GetUIContext()->CreateWidget(
    _A("ListBox"), mpWidget, sRectf(0, 0, 0, 0),
    eWidgetStyle_HoldFocus | eWidgetStyle_NotifyParent,
...
tBool __stdcall cWidgetSearchBar::OnWidgetSink(iWidget* apWidget, tU32 nMsg,
                                               const Var& varParam0,
                                               const Var& varParam1)
{
...
void cWidgetSearchBar::_OnKeyDown(tU32 key, iUnknown* apOriginalWidget)
{
  ...
  switch (key) {
  case eKey_Up: {
    if ((apOriginalWidget != mptrShowBox.ptr()) &&
        ptrListBox->GetSelected() > 0)
    {
  ...
  case eKey_Down: {
    if ((apOriginalWidget != mptrShowBox.ptr()) &&
        ptrListBox->GetSelected() < ptrListBox->GetNumItems() - 1)
    {
      ptrListBox->SetSelected(ptrListBox->GetSelected() + 1);
      ptrListBox->AutoScroll();
      ptrEditBox->MoveCursorEnd(eTrue);
