...
struct sQuadStrip {
  ...
  void OutputVertices(const cFVFDescription& fvfDesc, cFVFStream& fvfStream,
                      cFVFVertex& fvfVertex, tIndex*& pInds, tU32& nCount,
...
void QuadricDiskSweepQuads(tQuadStripVec& vQuads, tF32 innerRadius,
                           tF32 outerRadius, tI32 slices, tI32 loops,
                           tF32 startAngle, tF32 sweepAngle, tBool abCW,
                           tU32 aulColor, const sMatrixf& amtxUV,
                           tU32& nNumTris, tU32& nNumVerts, tU8 anAxis,
                           sVec3f avOffset, tBool abInvertWinding);

tBool FillGeometryFromQuads(iGeometry* apGeom, const tQuadStripVec& avQuads,
                            tU32 anNumVerts, tU32 anNumTris,
                            const sMatrixf& amtxUV, tBool abCW)
{
  ...
  cFVFStream fvfStream(
    &fvfDesc, apGeom->GetVertexArray()->Lock(0, 0, eLock_Normal), anNumVerts);
  ...
  for (tQuadStripVecCIt it = avQuads.begin(); it != avQuads.end(); ++it) {
    it->OutputVertices(fvfDesc, fvfStream, fvfVertex, pInds, nCount, amtxUV,
                       abCW);
  }
...
iGeometry* CreateGeometryFromQuads(iGraphics* apGraphics,
                                   const tQuadStripVec& avQuads,
                                   tU32 anNumVerts, tU32 anNumTris,
                                   tGeometryCreateFlags aFlags, tFVF aFVF,
                                   const sMatrixf& amtxUV, tBool abCW)
{
  // generate the geometry
  iGeometry* pGeom = apGraphics->CreateGeometryPolygonal(
    anNumVerts, anNumTris * 3, aFlags, aFVF);
  if (!niIsOK(pGeom)) {
    niSafeRelease(pGeom);
    return NULL;
  }

  if (!FillGeometryFromQuads(pGeom, avQuads, anNumVerts, anNumTris, amtxUV,
                             abCW))
  {
    niSafeRelease(pGeom);
    return NULL;
...
iGeometry* QuadricCylinder(iGraphics* apGraphics, tGeometryCreateFlags aFlags,
                           tFVF aFVF, tF32 baseRadius, tF32 topRadius,
                           tF32 height, tI32 slices, tI32 stacks, tBool abCW,
                           tU32 aulColor, const sMatrixf& amtxUV, tBool abCap,
                           tBool abCentered)
{
  tF32 da, r, dr, dz;
  tF32 z, nz;
  ...
  if (abCap) {
    if (baseRadius > niEpsilon2) {
      QuadricDiskSweepQuads(vQuads, 0.0f, baseRadius, slices, stacks, 0, ni2Pif,
                            abCW, aulColor, amtxUV, nNumTris, nNumVerts, 'z',
                            Vec3<tF32>(0, 0, 0 + heightOffset), eTrue);
    }
    if (topRadius > niEpsilon2) {
      QuadricDiskSweepQuads(vQuads, 0.0f, topRadius, slices, stacks, 0, ni2Pif,
                            abCW, aulColor, amtxUV, nNumTris, nNumVerts, 'z',
                            Vec3<tF32>(0, 0, height + heightOffset), eFalse);
    }
  }

  iGeometry* pGeom = CreateGeometryFromQuads(
    apGraphics, vQuads, nNumVerts, nNumTris, aFlags, aFVF, amtxUV, abCW);
  if (!niIsOK(pGeom)) {
...
iGeometry* QuadricSphere(iGraphics* apGraphics, tGeometryCreateFlags aFlags,
                         tFVF aFVF, tF32 radius, tI32 slices, tI32 stacks,
                           ...
                           sVec3f avOffset, tBool abInvertWinding)
{
  ...
  for (loop = 0; loop < loops; loop++) {
    ...
    for (slice = 0; slice <= slices; slice++) {
      ...
      case 'x':
      case 'X':
        strip.Position(0 + avOffset.x, y + avOffset.y, x + avOffset.z);
        break;
      case 'y':
      case 'Y':
        strip.Position(x + avOffset.x, 0 + avOffset.y, y + avOffset.z);
        break;
      case 'z':
      case 'Z':
        strip.Position(x + avOffset.x, y + avOffset.y, 0 + avOffset.z);
        break;
      }
      strip.Color(aulColor);
      strip.Normal(0, 0, abCW ? 1.0f : -1.0f);
...
iGeometry* QuadricDiskSweep(iGraphics* apGraphics, tGeometryCreateFlags aFlags,
                            tFVF aFVF, tF32 innerRadius, tF32 outerRadius,
                            tI32 slices, tI32 loops, tF32 startAngle,
                            tF32 sweepAngle, tBool abCW, tU32 aulColor,
                            const sMatrixf& amtxUV)
{
  ...
  QuadricDiskSweepQuads(vQuads, innerRadius, outerRadius, slices, loops,
                        startAngle, sweepAngle, abCW, aulColor, amtxUV,
                        nNumTris, nNumVerts, 'z', sVec3f::Zero(), eFalse);
...
tBool QuadricDiskSweepEx(iGeometry* apGeom, tF32 innerRadius, tF32 outerRadius,
                         tI32 slices, tI32 loops, tF32 startAngle,
                         tF32 sweepAngle, tBool abCW, tU32 aulColor,
                         const sMatrixf& amtxUV)
{
  ...
  for (loop = 0; loop < loops; loop++) {
    ...
    for (slice = 0; slice <= slices; slice++) {
      ...
      strip.Normal(0, 0, abCW ? 1.0f : -1.0f);
      strip.TexCoo(0, 0);
      strip.Position((radius + delta_radius) * sinf(angle),
                     (radius + delta_radius) * cosf(angle), 0.0f);
      strip.Color(aulColor);
      strip.Normal(0, 0, abCW ? 1.0f : -1.0f);
      strip.TexCoo(0, 0);
...
iGeometry* QuadricDisk(iGraphics* apGraphics, tGeometryCreateFlags aFlags,
                       tFVF aFVF, tF32 innerRadius, tF32 outerRadius,
                       tI32 slices, tI32 loops, tBool abCW, tU32 aulColor,
                       const sMatrixf& amtxUV)
{
  return QuadricDiskSweep(apGraphics, aFlags, aFVF, innerRadius, outerRadius,
                          slices, loops, 0.0, ni2Pif, abCW, aulColor, amtxUV);
}
...
//----------------------------------------------------------------------------
struct Mesh {
  Mesh(tPtr apVerts, tU32 anNumVerts, tU32 anVertStride, tIndex* apInds,
       tU32 anNumFaces)
  {
    _verts = apVerts;
  ...
  {
    return Vec3<tIndex>(_inds[fi * 3 + 0], _inds[fi * 3 + 1],
                        _inds[fi * 3 + 2]);
  }
...
{ // segs, face, row, column.
  ...
  return 4 + (s - 1) * 6 + f * (s - 1) * (s - 2) / 2 + (r - 2) * (r - 1) / 2 +
         c - 1;
}
...
{ // segs, face, row, column.
  ...
  if (c == r) {
    if (f < 4)
      return 6 + (s - 1) * ((f + 1) % 4) + r - 1;
    return 6 + (s - 1) * f + r - 1;
  }

  return 6 + (s - 1) * 12 + f * (s - 1) * (s - 2) / 2 + (r - 1) * (r - 2) / 2 +
         c - 1;
}
...
{ // segs, face, row, column.
  ...
  if (c == 0)
    return 5 + (s - 1) * f + r - 1;
  if (c == r)
    return 5 + (s - 1) * ((f + 1) % 4) + r - 1;

  return 5 + (s - 1) * 8 + f * (s - 1) * (s - 2) / 2 + (r - 1) * (r - 2) / 2 +
         c - 1;
}
...
{ // segs, face, row, column
  ...
  if (r == s) { // Bottom edge
    if (f < 5) {
      return 12 + (5 + f) * (s - 1) + c - 1;
    }
    if (f < 10) {
      return 12 + (20 + (f + 4) % 5) * (s - 1) + c - 1;
    }
    if (f < 15) {
      return 12 + (f - 5) * (s - 1) + s - 1 - c;
    }
  ...
  if (c == 0) { // Left edge
    ...
    if (f < 10) {
      return 12 + (f % 5 + 15) * (s - 1) + r - 1;
    }
    if (f < 15) {
      return 12 + ((f + 1) % 5 + 15) * (s - 1) + s - 1 - r;
    }
  ...
  if (c == r) { // Right edge
    if (f < 5) {
      return 12 + ((f + 1) % 5) * (s - 1) + r - 1;
    }
    if (f < 10) {
      return 12 + (f % 5 + 10) * (s - 1) + r - 1;
    }
    if (f < 15) {
      return 12 + (f % 5 + 10) * (s - 1) + s - 1 - r;
    }
    return 12 + (f % 5 + 25) * (s - 1) + r - 1;
  }

  // Not an edge or corner.
  return 12 + 30 * (s - 1) + f * (s - 1) * (s - 2) / 2 + (r - 1) * (r - 2) / 2 +
         c - 1;
}
...
{ // segs, face, row, column
  ...
  if (r == s) { // Bottom edge
    ...
    case 0:
      if (frow)
        return 26 + (5 + f0) * (s - 1) + s - 1 - c;
      return 26 + (5 + f0) * (s - 1) + c - 1;
    case 1:
      if (parity)
        return 26 + (h0 + 25) * (s - 1) + c - 1;
      return 26 + (h0 + 30) * (s - 1) + c - 1;
    case 2:
      if (parity)
        return 26 + (h1 + 30) * (s - 1) + s - 1 - c;
  ...
  if (c == r) { // Right edge
    ...
    case 0:
      if (frow)
        return 26 + (15 + f0) * (s - 1) + s - 1 - r;
      return 26 + f1 * (s - 1) + r - 1;
    case 1:
      if (parity)
        return 26 + (h0 + 15) * (s - 1) + r - 1;
      return 26 + (h0 + 10) * (s - 1) + r - 1;
    case 2:
      if (parity)
        return 26 + (37 + h0 * 4) * (s - 1) + s - 1 - r;
  ...
  // Not an edge or corner.
  return 26 + 65 * (s - 1) + f * (s - 1) * (s - 2) / 2 + (r - 1) * (r - 2) / 2 +
         c - 1;
}
...
void Geosphere_Count(int basetype, float radius, int segs, int hemi,
                     tU32& nverts, tU32& nfaces, tU32& nsections)
{
  ...
  if (hemi) {
    nfaces = nsections * segs * segs +
             ((basetype == 5) ? (segs * 10) : (segs * nsections));
    nverts = nsections * (segs - 1) * (segs - 2) / 2 +
             1; // face verts, including origin.
    switch (basetype) {
    case 1: nverts += 6 * (segs - 1) + 4; break;
    case 3: nverts += 8 * (segs - 1) + 5; break;
    case 5: nverts += 65 * (segs - 1) + 26; break;
    }
  }
  else {
    nfaces = nsections * segs * segs;
...
void Geosphere_Build(Mesh& mesh, int basetype, float radius, int segs, int hemi,
                     const tU32 nverts, const tU32 nfaces, const tU32 nsections)
{
  int i;
  int nf = 0, nv = 0;
  ...
  case 0: // Based on tetrahedron
    ...
    mesh.setVert(nv++, radius * (Sqrt(8.f / 9.f)), (float)0.0,
                 -radius / ((float)3.));
    mesh.setVert(nv++, -radius * (Sqrt(2.f / 9.f)), radius * (Sqrt(2.f / 3.f)),
                 -radius / ((float)3.));
    mesh.setVert(nv++, -radius * (Sqrt(2.f / 9.f)), -radius * (Sqrt(2.f / 3.f)),
                 -radius / ((float)3.));
    ...
    // Face vertices
    for (i = 1; i < segs - 1; i++)
      SphericalInterpolate(mesh, 4 + i, 4 + (segs - 1) + i, &nv, i + 1);
    for (i = 1; i < segs - 1; i++)
      SphericalInterpolate(mesh, 4 + (segs - 1) + i, 4 + 2 * (segs - 1) + i,
                           &nv, i + 1);
    for (i = 1; i < segs - 1; i++)
      SphericalInterpolate(mesh, 4 + 2 * (segs - 1) + i, 4 + i, &nv, i + 1);
    for (i = 1; i < segs - 1; i++)
      SphericalInterpolate(mesh, 4 + 4 * (segs - 1) + i, 4 + 3 * (segs - 1) + i,
                           &nv, i + 1);
    break;
  ...
  case 2: //Based on the Octahedron
    ...
    for (face = 0; face < 4; face++)
      SphericalInterpolate(mesh, 0, face + 1, &nv, segs);
    for (face = 0; face < 4; face++)
      SphericalInterpolate(mesh, 5, face + 1, &nv, segs);
    for (face = 0; face < 4; face++)
      SphericalInterpolate(mesh, face + 1, (face + 1) % 4 + 1, &nv, segs);

    // Face vertices
    for (face = 0; face < 4; face++) {
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 6 + face * (segs - 1) + i,
                             6 + ((face + 1) % 4) * (segs - 1) + i, &nv, i + 1);
      }
    }
    for (face = 0; face < 4; face++) {
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 6 + ((face + 1) % 4 + 4) * (segs - 1) + i,
                             6 + (face + 4) * (segs - 1) + i, &nv, i + 1);
      }
    }
    break;
  ...
  case 3: // Hemioctahedron -- this one's perfect every time.
    ...
    for (face = 0; face < 4; face++) {
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 5 + face * (segs - 1) + i,
                             5 + ((face + 1) % 4) * (segs - 1) + i, &nv, i + 1);
      }
    }
  ...
  case 4: // Based on the Icosahedron
    ...
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, 0, face + 1, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 1, (face + 1) % 5 + 1, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 1, face + 6, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 1, (face + 4) % 5 + 6, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 6, (face + 1) % 5 + 6, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, 11, face + 6, &nv, segs);

    // Face vertices: 4 rows of 5 faces each.
    for (face = 0; face < 5; face++) {
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 12 + face * (segs - 1) + i,
                             12 + ((face + 1) % 5) * (segs - 1) + i, &nv,
                             i + 1);
      }
    }
    for (face = 0; face < 5; face++) {
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 12 + (face + 15) * (segs - 1) + i,
                             12 + (face + 10) * (segs - 1) + i, &nv, i + 1);
      }
    }
    for (face = 0; face < 5; face++) {
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(
          mesh, 12 + ((face + 1) % 5 + 15) * (segs - 1) + segs - 2 - i,
          12 + (face + 10) * (segs - 1) + segs - 2 - i, &nv, i + 1);
      }
    }
    for (face = 0; face < 5; face++) {
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 12 + ((face + 1) % 5 + 25) * (segs - 1) + i,
                             12 + (face + 25) * (segs - 1) + i, &nv, i + 1);
      }
    }
    break;
  ...
  case 5: // HemiIcosahedron, based loosely on a segs-2 icosahedron
    ...
    nv += 5;
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 6, (face + 1) % 5 + 6, &nv, 2);
    ...
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 1, face + 6, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 1, face + 11, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 1, (face + 4) % 5 + 11, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, face + 6, face + 11, &nv, segs);
    for (face = 0; face < 5; face++)
      SphericalInterpolate(mesh, (face + 4) % 5 + 11, face + 6, &nv, segs);
    for (face = 0; face < 5; face++) {
    ...
    for (face = 0; face < 10; face++)
      SphericalInterpolate(mesh, face + 16, (face + 1) % 10 + 16, &nv, segs);

    // Face vertices: representing 40 faces, 8 groups of 5.
    for (face = 0; face < 5; face++) { // Top 5:
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 26 + face * (segs - 1) + i,
                             26 + ((face + 1) % 5) * (segs - 1) + i, &nv,
                             i + 1);
      }
    }
    for (face = 0; face < 5; face++) { // Stellation of above
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(
          mesh, 26 + ((face + 1) % 5 + 20) * (segs - 1) + segs - 2 - i,
          26 + (face + 15) * (segs - 1) + segs - 2 - i, &nv, i + 1);
      }
    }
    for (face = 0; face < 5; face++) { // Rest of this level
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 26 + (face + 20) * (segs - 1) + i,
                             26 + (face + 10) * (segs - 1) + i, &nv, i + 1);
      }
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 26 + (face + 10) * (segs - 1) + i,
                             26 + (face + 15) * (segs - 1) + i, &nv, i + 1);
      }
    }
    for (face = 0; face < 10; face++)
    { // Downward-pointing faces in lowest level
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(
          mesh, 26 + (face * 2 + 36) * (segs - 1) + segs - 2 - i,
          26 + (face * 2 + 35) * (segs - 1) + segs - 2 - i, &nv, i + 1);
      }
    }
    for (face = 0; face < 10; face++) { // Upward-pointing faces in lowest level
      int f9 = (face + 9) % 10;
      for (i = 1; i < segs - 1; i++) {
        SphericalInterpolate(mesh, 26 + (f9 * 2 + 36) * (segs - 1) + i,
                             26 + (face * 2 + 35) * (segs - 1) + i, &nv, i + 1);
      }
    }
...
///////////////////////////////////////////////
iGeometry* GeoSphere(iGraphics* apGraphics, tGeometryCreateFlags aFlags,
                     tFVF aFVF, int type, tBool abHemi, tF32 radius,
                     tI32 slices, tBool abCW, tU32 aulColor,
                     const sMatrixf& amtxUV)
{
  tU32 nNumVerts, nNumFaces, nNumSections;

  Geosphere_Count(type, radius, slices, eFalse, nNumVerts, nNumFaces,
                  nNumSections);

  iGeometry* pGeom =
    apGraphics->CreateGeometryPolygonal(nNumVerts, nNumFaces * 3, aFlags, aFVF);
  if (!niIsOK(pGeom)) {
    niError(_A("Can't create destination geometry."));
    return NULL;
  }
  ...
  cFVFStream fvfStream(
    &fvfDesc, pGeom->GetVertexArray()->Lock(0, 0, eLock_Normal), nNumVerts);
  tIndex* pInds = (tIndex*)pGeom->GetIndexArray()->Lock(0, 0, eLock_Normal);

  Mesh mesh(fvfStream.GetBase(), nNumVerts, fvfDesc.GetStride(), pInds,
            nNumFaces);
  Geosphere_Build(mesh, type, radius, slices, abHemi, nNumVerts, nNumFaces,
                  nNumSections);
