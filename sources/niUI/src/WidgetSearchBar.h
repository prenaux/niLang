...
struct iSearchEngine : public iUnknown {
  ...
  virtual void __stdcall SetCorpus(
    const astl::vector<tSearchItem>& avCorpus) = 0;
  virtual void __stdcall Search(const achar* aSearchTerm,
                                astl::vector<tSearchItem>& avResult) = 0;
};

class cWidgetSearchBar
    : public ImplRC<iWidgetSink, eImplFlags_Default, ni::iWidgetSearchBar> {
