...
typedef Var(__ni_export_call_decl* tpfnCallback2)(const Var& avarA,
                                                  const Var& avarB);

template <typename T>
struct sRunnable : public ni::ImplRC<iRunnable> {
  const T _lambda;
  sRunnable(const T& aLambda)
      : _lambda(aLambda)
  {
  }
...
struct sCallback1
    : public ni::ImplRC<iCallback, eImplFlags_DontInherit1, iRunnable> {
  const T _lambda;
  sCallback1(const T& aLambda)
      : _lambda(aLambda)
  {
  }
  virtual Var __stdcall Run()
  {
    return _lambda(niVarCast(niVarNull));
  }
  virtual Var __stdcall RunCallback(const Var& avarA, const Var& avarB)
  {
...
struct sCallback2
    : public ni::ImplRC<iCallback, eImplFlags_DontInherit1, iRunnable> {
  const T _lambda;
  sCallback2(const T& aLambda)
      : _lambda(aLambda)
  {
  }
  virtual Var __stdcall Run()
  {
    return _lambda(niVarNull, niVarNull);
  }
  virtual Var __stdcall RunCallback(const Var& avarA, const Var& avarB)
  {
...
typedef void(__ni_export_call_decl* tpfnMessageHandler)(tU32 anMsg,
                                                        const Var& avarA,
                                                        const Var& avarB);
...
template <typename T>
Nonnull<iMessageHandler> MessageHandler(
  const T& aLambda, const tU64 anThreadID = ni::ThreadGetCurrentThreadID())
{
...
struct sConcurrentExecute {
  iExecutor* _executor;
  sConcurrentExecute(iExecutor* apExecutor)
      : _executor(apExecutor)
  {
  }
  ...
  #define niExec_(OBJ, ...) \
    ni::sConcurrentExecute(OBJ) << [__VA_ARGS__]() -> ni::tBool
  #define niExec(KIND, ...)                                          \
    ni::sConcurrentExecute(ni::GetConcurrent()->GetExecutor##KIND()) \
      << [__VA_ARGS__]() -> ni::tBool

struct sConcurrentSubmit {
  iExecutor* _executor;
  sConcurrentSubmit(iExecutor* apExecutor)
      : _executor(apExecutor)
  {
  }
  template <typename TReleaseFun>
  ...
  #define niSubmit_(OBJ, ...) \
    ni::sConcurrentSubmit(OBJ) << [__VA_ARGS__]() -> ni::tBool
  #define niSubmit(KIND, ...)                                       \
    ni::sConcurrentSubmit(ni::GetConcurrent()->GetExecutor##KIND()) \
      << [__VA_ARGS__]() -> ni::tBool
...
struct sMessageHandlerBuilder {
  ...
  #define niMessageHandler(...)                    \
    ni::sMessageHandlerBuilder() << [__VA_ARGS__]( \
      ni::tU32 anMsg, const ni::Var& avarA, const ni::Var& avarB)
