...
struct integral_constant {
  ...
  EA_CONSTEXPR operator value_type() const EA_NOEXCEPT
  {
    return value;
  }
  EA_CONSTEXPR value_type operator()() const EA_NOEXCEPT
  {
    return value;
  }
};
...
//
template <bool bCondition, class ConditionIsTrueType,
...
template <bool bCondition, class ConditionIsTrueType,
          class ConditionIsFalseType>
using type_select_t = typename type_select<bCondition, ConditionIsTrueType,
...
#define EASTL_TYPE_TRAIT_is_same_CONFORMANCE \
  1 // is_same is conforming; doesn't make mistakes.
...
template <typename T>
struct is_const_value : public eastl::false_type {};
template <typename T>
struct is_const_value<const T*> : public eastl::true_type {};
template <typename T>
struct is_const_value<const volatile T*> : public eastl::true_type {};

template <typename T>
struct is_const : public eastl::is_const_value<T*> {};
template <typename T>
struct is_const<T&> : public eastl::false_type {
}; // Note here that T is const, not the reference to T. So is_const is false. See section 8.3.2p1 of the C++ standard.
...
template <typename T>
struct is_volatile_value : public eastl::false_type {};
template <typename T>
struct is_volatile_value<volatile T*> : public eastl::true_type {};
template <typename T>
struct is_volatile_value<const volatile T*> : public eastl::true_type {};

template <typename T>
struct is_volatile : public eastl::is_volatile_value<T*> {};
template <typename T>
struct is_volatile<T&> : public eastl::false_type {
}; // Note here that T is volatile, not the reference to T. So is_const is false. See section 8.3.2p1 of the C++ standard.
...
#define EASTL_TYPE_TRAIT_is_reference_CONFORMANCE \
  1 // is_reference is conforming; doesn't make mistakes.

template <typename T>
struct is_reference : public eastl::false_type {};
template <typename T>
struct is_reference<T&> : public eastl::true_type {};
...
template <typename>
struct is_function : public eastl::false_type {};

#if EA_PLATFORM_PTR_SIZE == 4 && defined(EA_PLATFORM_MICROSOFT) && \
  defined(_MSC_EXTENSIONS)
// __cdecl specialization
template <typename ReturnValue, typename... ArgPack>
struct is_function<ReturnValue __cdecl(ArgPack...)> : public eastl::true_type {
};

template <typename ReturnValue, typename... ArgPack>
struct is_function<ReturnValue __cdecl(
  ArgPack...,
  ...)> // The second ellipsis handles the case of a function that takes ellipsis, like printf.
...
#else
template <typename ReturnValue, typename... ArgPack>
struct is_function<ReturnValue(ArgPack...)> : public eastl::true_type {};

template <typename ReturnValue, typename... ArgPack>
struct is_function<ReturnValue(
  ArgPack...,
  ...)> // The second ellipsis handles the case of a function that takes ellipsis, like printf.
    : public eastl::true_type {};
#endif
...
#define EASTL_TYPE_TRAIT_remove_const_CONFORMANCE \
  1 // remove_const is conforming.
...
#define EASTL_TYPE_TRAIT_remove_volatile_CONFORMANCE \
  1 // remove_volatile is conforming.
...
struct remove_cv {
  typedef
    typename eastl::remove_volatile<typename eastl::remove_const<T>::type>::type
      type;
};
...
#define EASTL_TYPE_TRAIT_add_reference_CONFORMANCE \
  1 // add_reference is conforming.
...
#if defined(_MSC_VER) && \
  (_MSC_VER <=           \
   1600) // VS2010 and earlier mistakenly report: "cannot add a reference to a zero-sized array." Actually they are allowed, but there's nothing we can do about it under VS2010 and earlier.
template <typename T>
...
#define EASTL_TYPE_TRAIT_remove_cvref_CONFORMANCE \
  1 // remove_cvref is conforming.

template <typename T>
struct remove_cvref {
  typedef typename eastl::remove_volatile<typename eastl::remove_const<
    typename eastl::remove_reference<T>::type>::type>::type type;
};
...
#define EASTL_TYPE_TRAIT_add_lvalue_reference_CONFORMANCE \
  1 // add_lvalue_reference is conforming.
...
struct static_min<I0, I1, in...> {
  static const size_t value =
    ((I0 <= I1) ? static_min<I0, in...>::value : static_min<I1, in...>::value);
};
...
struct static_max<I0, I1, in...> {
  static const size_t value =
    ((I0 >= I1) ? static_max<I0, in...>::value : static_max<I1, in...>::value);
};
...
EA_DISABLE_VC_WARNING(
  4472) // 'endian' is a native enum: add an access specifier (private/public) to declare a managed enum
enum class endian {
