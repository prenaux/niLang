...
struct sJSCCWindow : public ni::ImplRC<ni::iOSWindow, ni::eImplFlags_Default> {
  ...
  sJSCCWindow()
  {
    ...
    niLog(Info, niFmt("WebGL.AA: %y, WebGL.Translucent: %y", bWebGLAA,
                      bWebGLTranslucent));
    ...
    const EGLint s_configAttribs[] = {
      ...
      EGL_SAMPLE_BUFFERS, bWebGLAA ? 1 : 0, EGL_NONE
    };

    const EGLint eglSurfaceAttrs[] = { EGL_RENDER_BUFFER, EGL_BACK_BUFFER,
                                       EGL_NONE };

    const EGLint eglContextAttribList[] = { EGL_CONTEXT_CLIENT_VERSION, 2,
                                            EGL_NONE };
    ...
    eglsurface =
      eglCreateWindowSurface(egldisplay, eglconfig, NULL, eglSurfaceAttrs);
  ...
  ///////////////////////////////////////////////
  __forceinline tBool _SendMessage(eOSWindowMessage aMsg,
...
{
  TRACE_JSCC_CAPI(
    ("... niJSCC_SetProperty: name: '%s' value: '%s'", name, value));
...
niExportJSCC(void)
  niJSCC_WndNotifyResize(ni::tI32 w, ni::tI32 h, ni::tF32 afContentsScale)
{
...
niExportJSCC(void) niJSCC_WndInputKey(ni::eKey aKey, ni::tBool abIsDown)
{
  ...
  CHECK_WINDOW(;);
  wnd->_SendMessage(abIsDown ? eOSWindowMessage_KeyDown
...
{
  TRACE_JSCC_MOUSE_MOVE(
    ("... niJSCC_WndInputMouseRelativeMove: rx: %g ry: %g", rx, ry));
  CHECK_WINDOW(;);
  wnd->_SendMessage(eOSWindowMessage_RelativeMouseMove, Vec2f(rx, ry));
}
niExportJSCC(void)
  niJSCC_WndInputMouseButton(tI32 aPointerButton, ni::tBool abIsDown)
{
  TRACE_JSCC_MOUSE_BT(("... niJSCC_WndInputMouseButton: button: %d, isDown: %d",
                       aPointerButton, abIsDown));
  CHECK_WINDOW(;);
  wnd->_SendMessage(abIsDown ? eOSWindowMessage_MouseButtonDown
...
{
  TRACE_JSCC_MOUSE_BT(
    ("... niJSCC_WndInputMouseDoubleClick: button: %d", aPointerButton));
  CHECK_WINDOW(;);
...
niExportJSCC(void)
  niJSCC_WndInputFingerMove(ni::tI32 fingerIdentifier, ni::tF32 x, ni::tF32 y,
                            ni::tF32 pressure)
{
  TRACE_JSCC_FINGER(
    ("... niJSCC_WndInputFingerMove: id: %d x: %g y: %g pressure: %g",
  ...
  if (fingerIndex >= 0) {
    ...
    wnd->_SendMessage(eOSWindowMessage_FingerMove, fingerIndex,
                      Vec3f(x, y, pressure));
    {
      const ni::tF32 relativeMoveSpeedScale = wnd->GetContentsScale();
      const ni::sVec2f delta = (Vec2f(x, y) - _lastTouchPosition[fingerIndex]) *
                               relativeMoveSpeedScale;
      const sVec3f rv = Vec3f(delta.x, delta.y, pressure);
      TRACE_JSCC_FINGER(
        ("... JSCC: FingerRelativeMove: %d, %s", fingerIndex, rv));
...
niExportJSCC(void)
  niJSCC_WndInputFingerPress(ni::tI32 fingerIdentifier, ni::tBool isDown,
                             ni::tF32 x, ni::tF32 y, ni::tF32 pressure)
{
  TRACE_JSCC_FINGER(
    ("... niJSCC_WndInputFingerPress: id: %d x: %g y: %g pressure: %g",
     fingerIdentifier, x, y, pressure));
  CHECK_WINDOW(;);
