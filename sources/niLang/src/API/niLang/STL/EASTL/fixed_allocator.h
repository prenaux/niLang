  ...
  void* allocate(size_t n, int /*flags*/ = 0)
  {
    ...
    if (pLink) // If we have space...
    {
      ...
      if (mpNext != mpCapacity) {
        ...
        mpNext =
          reinterpret_cast<Link*>(reinterpret_cast<char8_t*>(mpNext) + n);
...
class EASTL_API fixed_allocator_with_overflow : public fixed_pool_base {
 public:
  ...
  ///
  fixed_allocator_with_overflow(
    const char* pName = EASTL_FIXED_POOL_DEFAULT_NAME)
      : fixed_pool_base(NULL)
  ...
  /// initialize them.
  ///
  fixed_allocator_with_overflow& operator=(
    const fixed_allocator_with_overflow& x)
  {
 ...
 protected:
  EASTLAllocatorType
    mOverflowAllocator; // To consider: Allow the user to define the type of this, presumably via a template parameter.
  void*
    mpPoolBegin; // To consider: We have these member variables and ideally we shouldn't need them. The problem is that
  void*
    mpPoolEnd; //              the information about the pool buffer and object size is stored in the owning container
  eastl_size_t
    mnNodeSize; //              and we can't have access to it without increasing the amount of code we need and by templating
...
bool operator==(const fixed_allocator_with_overflow& a,
...
inline bool operator==(const fixed_allocator_with_overflow&,
