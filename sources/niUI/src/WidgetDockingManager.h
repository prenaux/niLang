...
class cWidgetDockingManager
 ...
 public:
  ...
  tU32 __stdcall AddDockArea(tU32 anParent, tU32 aPos, sRectf aRect,
                             tBool abLocal);
  ...
  sRectf GetDockPositionRect(tRectEdgesFlags aDockPos, iWidget* apArea,
                             sVec2f avRootPos, tBool abLocal);
 ...
 public:
  ...
  struct sOverlayWidgetSink : public ImplRC<iWidgetSink> {
    sOverlayWidgetSink(cWidgetDockingManager* apDockManager)
        : mpDockManager(apDockManager)
    {
    }
    tBool __stdcall OnWidgetSink(iWidget* apWidget, tU32 anMsg, const Var& aA,
                                 const Var& aB);
