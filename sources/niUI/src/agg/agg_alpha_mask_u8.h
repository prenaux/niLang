 ...
 public:
  ...
  cover_type pixel(int x, int y) const
  {
    if (x >= 0 && y >= 0 && x < (int)m_rbuf->width() &&
        y < (int)m_rbuf->height())
    {
      return (cover_type)m_mask_function.calculate(m_rbuf->row_ptr(y) +
                                                   x * Step + Offset);
    }
    return 0;
  }
  ...
  cover_type combine_pixel(int x, int y, cover_type val) const
  {
    if (x >= 0 && y >= 0 && x < (int)m_rbuf->width() &&
        y < (int)m_rbuf->height())
    {
      return (cover_type)((cover_full +
                           val * m_mask_function.calculate(
  ...
  void combine_hspan(int x, int y, cover_type* dst, int num_pix) const
  {
    ...
    do {
      *covers = (cover_type)((cover_full +
                              (*covers) * m_mask_function.calculate(mask)) >>
                             cover_shift);
      ++covers;
      mask += Step;
    } while (--count);
...
class amask_no_clip_u8 {
 public:
  ...
  cover_type pixel(int x, int y) const
  {
    return (cover_type)m_mask_function.calculate(m_rbuf->row_ptr(y) + x * Step +
                                                 Offset);
  }
  ...
  {
    return (cover_type)((cover_full +
                         val * m_mask_function.calculate(m_rbuf->row_ptr(y) +
                                                         x * Step + Offset)) >>
                        cover_shift);
  }
  ...
  void combine_hspan(int x, int y, cover_type* dst, int num_pix) const
  {
    ...
    do {
      *dst =
        (cover_type)((cover_full + (*dst) * m_mask_function.calculate(mask)) >>
                     cover_shift);
      ++dst;
      mask += Step;
    } while (--num_pix);
