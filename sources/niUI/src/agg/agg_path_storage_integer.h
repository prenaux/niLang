...
struct vertex_integer {
  ...
  unsigned vertex(agg_real* x_, agg_real* y_, agg_real dx = 0, agg_real dy = 0,
                  agg_real scale = 1.0) const
  {
    *x_ = dx + (agg_real(x >> 1) / coord_scale) * scale;
    *y_ = dy + (agg_real(y >> 1) / coord_scale) * scale;
    switch (((y & 1) << 1) | (x & 1)) {
...
class path_storage_integer {
 public:
  ...
  {
    m_storage.add(
      vertex_integer_type(x_ctrl, y_ctrl, vertex_integer_type::cmd_curve3));
    m_storage.add(
      vertex_integer_type(x_to, y_to, vertex_integer_type::cmd_curve3));
  }
  ...
  {
    m_storage.add(
      vertex_integer_type(x_ctrl1, y_ctrl1, vertex_integer_type::cmd_curve4));
    m_storage.add(
      vertex_integer_type(x_ctrl2, y_ctrl2, vertex_integer_type::cmd_curve4));
    m_storage.add(
      vertex_integer_type(x_to, y_to, vertex_integer_type::cmd_curve4));
  }
  ...
  {
    rect_d bounds(1e30, 1e30, -1e30, -1e30);
    if (m_storage.size() == 0) {
...
class serialized_integer_path_adaptor {
 public:
  ...
  //--------------------------------------------------------------------
  serialized_integer_path_adaptor(const int8u* data, unsigned size, agg_real dx,
                                  agg_real dy)
      : m_data(data)
      , m_end(data + size)
      , m_ptr(data)
  ...
  //--------------------------------------------------------------------
  void init(const int8u* data, unsigned size, agg_real dx, agg_real dy,
            agg_real scale = 1.0)
  {
    m_data = data;
    m_end = data + size;
